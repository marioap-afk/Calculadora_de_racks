...
- **Estado:** **propuesto**
- **Fecha:** 2026-09-12 (propuesto; borrador corregido con Proposal V2 y con Proposal V3 el mismo día)
- **Decisores:** Mario Pérez, Owner del repositorio (**acepta o rechaza**; pendiente). La aceptación **no** es
  precondición de la caracterización (G3): se pide **después de G3**, si G3 confirma el contrato, y **es precondición de
  G4**. Coordinador de I-52 y Arquitecto de I-52 (consenso técnico **pendiente** sobre la Proposal); Claude (redacción)
- **Iniciativa relacionada:** I-52 — `feature/rackmirror-espejo-semantico`
  ([contrato](../initiatives/I-52-rackmirror-espejo-semantico.md),
  [Discovery](../initiatives/I-52-discovery.md), [Proposal V1](../initiatives/I-52-proposal-v1.md) y
  [Proposal V2](../initiatives/I-52-proposal-v2.md) (historial), [Proposal V3](../initiatives/I-52-proposal-v3.md),
...
> sin publicación anterior de otro 0036 en ningún ref. ADR-0035 (I-50) está aceptado en `main` y ADR-0037 (I-53), publicado
> después y con otro número, está aceptado en su rama: no hay colisión. Antes de pedir la aceptación del Owner se vuelve
> a buscar 0036 en todos los refs; si apareciera una publicación anterior, este ADR se renumera antes de la aceptación.
> Una vez `aceptado` no se renumera, y dos ADR aceptados con el mismo número detienen el trabajo hasta que decida el
> Owner (Proposal V3 §16).
...
5. **Fail-closed con tres dimensiones separadas.** Cada propiedad de un rack concreto tiene una **clase de
   ...
   **estado de evidencia** (demostrada en código, pendiente de caracterización, verificada) y una **disposición
   ...
   de los builders queda pendiente de caracterización; una contradicción reabre la Proposal, no se parchea. Una holgura
   gráfica anclada a un lado ya demostrada en código (tope del Selectivo, tope posterior activo de Push Back) es
   `UNKNOWN` y falla cerrado hasta que se apruebe una regla explícita.
6. **Reflectores puros en Application sobre el sustrato real de cada kind**, despachados por kind fuera del comando.
   ...
   - **Metadata desconocida.** Se distingue la metadata **portadora**, cuyo contrato es conservarse exactamente (sobre,
     envoltorio del proyecto, valores de propiedades vinculables, propiedades de rack de I-54), de la metadata
     **semántica** del payload que se somete a `μ_k`. Un miembro desconocido no vacío en el payload semántico es `UNKNOWN`
     y falla cerrado, salvo declaración tipada de invariancia o de transformador; los portadores se conservan.
   - **Cobertura.** Cada miembro material de los tipos authored y cada propiedad vinculable tiene exactamente una
     clasificación de espejo; un miembro nuevo sin clasificar detiene el trabajo.
7. **Autoridad pura de planes por vista en Application** y un **materializador genérico en el Plugin** que crea la
   ...
   canónicas. Su convergencia con los productores de planes existentes (`RACKEDITAR` → Actualizar, el ejecutor de
   variables y el paso del estado del editor al sistema) se demuestra **antes** de que el comando la consuma. El comando
...
8. **Colocación canónica.** Con la reflexión de la hoja `G`, la colocación efectiva de la fuente
   ...
   geométrico, también tras cambiar una variable, y los textos y cotas se regeneran legibles. Las definiciones nuevas se
   crean con `Origin = 0`.
...
10. **Portadores intactos.** `SchemaVersion`, `ExtensionData` del sobre y del envoltorio, `PropertyValues` (incluidos
    tipos desconocidos), literales congelados, `VariableId`, **`DimensionViews`** (I-50, integrada: requisito normativo,
    con el entero exacto) y las propiedades de rack de I-54 viajan desde el origen. El espejo no crea, modifica,
    ...
    deshacer la equivalencia reflejada.
11. **Fuente canónica.** Solo `BlockReference` de Model Space, no MINSERT, con `Normal = +Z`, rotación finita y escala
    ...
    ignoran como si no fueran racks. La línea se captura en el UCS con Z paralela a la de WCS y se convierte a WCS. Las
...
12. **Atomicidad semántica, no de infraestructura.**
    ...
    y el conjunto de entradas del registro realmente leídas se re-verifican antes de mutar. La importación de la
...
13. **Mano y simetría de bloques.** Ningún bloque DWG se asume simétrico. Una diferencia de mano entre el plan
    ...
    única** en Application con: nombre exacto del bloque, vista o familia, **eje local** de simetría y **centro** medido en
    el **marco local de inserción de la pieza después de aplicar `T(−Origin)` de su definición** (nunca en WCS, en el
    marco del rack ni en coordenadas crudas de la definición), respaldada por caracterización, evidencia del Owner y la
    identidad de la biblioteca y del manifiesto de catálogo usados. El centro es constante; una forma afín sobre un
    parámetro dinámico solo se admite si la caracterización la demuestra. Una declaración nunca puede hacer equivalente
    una pieza con holgura anclada a un lado, y no se admiten declaraciones para familias cuya holgura está clasificada
    `UNKNOWN`. Ninguna holgura, offset gráfico o parámetro con semántica de lado se asume simétrico: sin regla, falla
    cerrado.
14. **Verificación dinámica por rack sobre todas las vistas admisibles.** Antes de pedir la línea, cada rack lógico
    verifica sobre su diseño reflejado: la ausencia de metadata semántica desconocida; las reglas de dependencia de las
    propiedades vinculadas; la conmutación de **todas** las vistas admisibles que el rack podría materializar después con
...
- Negativas / costos aceptados:
  ...
  - fallan cerrado hasta nueva decisión: los Selectivos con topes, los **Push Back con tope posterior activo**, los
    **medios frentes cuyo ancho depende de una propiedad vinculada** (y los desviadores que dependen de ellos), las
    **filas de parrillas o tarimas desbordadas**, las cabeceras de plantilla con diagonales automáticas, las
    definiciones con BASE movida, las referencias dinámicas, anónimas o anotativas, los payloads con **metadata semántica
    desconocida** y todo rack con **alguna vista admisible** que no conmute;
  - la equivalencia exige declarar o caracterizar la mano de los bloques DWG, que no están versionados;
  - lo importado de la biblioteca puede sobrevivir a un fallo;
  - toda propiedad nueva de los tipos authored exige clasificar su regla de espejo antes de integrarse;
...
- I-54 Proposal V3 D-21 (invariante de preservación del sobre, que el espejo cumple).
...
- **Borrador V3** — este texto, con Proposal V3. Cambios respecto del V2, por la revisión de Arquitecto de Proposal V2
  (`Architect: CHANGES REQUIRED — PROPOSAL V3`) y la orden del Coordinador:
