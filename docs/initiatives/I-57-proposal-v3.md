# I-57 — Shared View Foundation — Proposal V3

```text
Estado                    = PROPOSAL V3 — NOT CONSENSUS
...
Coordinator input         = AGREED WITH ARCHITECT FINDINGS on V2 · I57-AR-01/02 applied here
Architect input           = CHANGES REQUIRED on V2 · exact-SHA review required on V3
```
...
`.`— y viaja por `requirement -> Plugin library query -> ensure/import`, si el flujo consumidor permite importar.
...
`GeneratedViewBaseName`, pero V3 no los exige: si elige strings debe preservar la misma separacion semantica.
...
```text
typed plan -> LibraryBlockRequirements -> Plugin Library Query -> Ensure/import if consumer flow permits
...
V4 y repetir consenso; nunca parchear producto dentro de F1.

### 5.1 Characterization Blockers de F1

| Blocker | Expected/fail-closed | Owner | Gate | Si contradice V3 |
|---|---|---|---|---|
| matriz CT-04 completa | cada celda produce una sola address/disposition; sin evidencia = Invalid/STOP | I-57 | F1, requerido por F2 | STOP → Proposal V4 |
| convenciones de extremos CT-05 | cada fixture fija eje/cara, span, centro y offset; ambiguedad = frame no disponible | I-57 | F1, requerido por F3 | STOP → Proposal V4 |
| comparators no Selectivos | cada kind demuestra Single/Divergent/Unreadable; falta de comparator = Unreadable | I-57, adapter por kind | F1, requerido por F6 | STOP → Proposal V4 |
| inventario de productores legacy | lista cerrada por autoridad y consumidor; productor no censado bloquea delegacion | I-57 | F1, requerido por F2-F6 | STOP → Proposal V4 |
...
`HeaderRunPlan`, obtienen las keys de sus instancias tipadas. Query solo observa; importacion permanece una accion
separada y no altera el significado de `FOUND`/`MISSING`.
...
| F6 | AUTH-12..13 | requirements/query/comparators, BLK-ID-1..6, CT-BLK/AUTH |
...
`cd42db03becff42f98b047e61c46689c17a69670`; V3 no la edita ni la reabre. ADR-0044 se publica **propuesto** y
...
**Open Material arquitectonico: NONE.** V3 cierra I57-AR-01 con §3.2, BLK-ID-1..6 y CT-BLK, y cierra el cambio
documental I57-AR-02 al reapuntar ADR-0044. Las disposiciones AUTH-01..14, payload, capas, ADR-0034, frame,
Resolve/Plan, selection, comparators y limite de F1 permanecen como en V2. Minor: nombres de namespaces/tipos,
codigos diagnosticos y ubicacion de fixtures.

**Architect Review de V2:** Reviewed SHA `71f87db0c4ee980976c25573f217173b139bc166`; verdict `CHANGES REQUIRED`;
I57-AR-01 HIGH/MATERIAL e I57-AR-02 LOW/NON-MATERIAL. El Coordinator acepta ambos hallazgos. Esa aceptacion no es
un veredicto formal sobre V3: Coordinator y Architect deben revisar el mismo SHA exacto de V3.

Hasta veredictos `AGREED` sobre el mismo SHA: Consensus NOT REACHED, Implementation BLOCKED, F1 NOT OPEN.

## 10. Adversarial check focal de V3

PASS documental:

1. `BaseName` ausente de library no afecta requirements presentes.
2. Una requirement presente conserva su key exacta.
3. Una requirement ausente produce `MISSING` aunque `BaseName` exista en library.
4. El suffix del generated name no cambia requirements.
5. Una requirement key con `.` no se sanitiza.
6. Una requirement key que parece rack name sigue siendo library key.
7. Un `BaseName` que parece library key sigue siendo generated-view base.
8. Requirements duplicadas se normalizan solo conforme el contrato vigente del query, sin renombrarlas.
9. Requirements empty/whitespace no se convierten en `BaseName`.
10. Cero requirements en un plan de geometria pura no consulta `BaseName`.
11. Cantilever permanece sin dependencia de library blocks.
12. `HeaderRunPlan` conserva las keys de piezas de sus instancias.
13. Query e import siguen separados; consultar antes o despues no cambia los espacios de identidad.
14. El sanitizer del nombre de vista no alcanza library keys.
15. Collision resolution no agrega, quita ni modifica requirements.

PASS de regresion: V3 no cambia disposiciones AUTH-01..14; no referencia R2 como reconciliacion vigente; R3 conserva
