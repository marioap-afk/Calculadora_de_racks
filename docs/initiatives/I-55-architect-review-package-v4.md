# I-55 — Paquete de revision del Arquitecto independiente (Proposal V4)

```text
PROPOSAL V4 — NOT CONSENSUS
Coordinator      = REVIEW REQUIRED
Architect formal = PENDING   ← se solicita a un Arquitecto INDEPENDIENTE de la sesion que redacto V2, V3 y V4
Owner            = PENDING
Consensus        = NOT REACHED
Implementation   = BLOCKED
Open Material    = M-01 (propio) · adopcion de X-1..X-8 por I-52
CQ-01            = decidida por el Coordinador en G2E (redibujo atomico de hermanas en Insertar)

Objeto de la revision  = docs/initiatives/I-55-proposal-v4.md @ fe70d7a76c77f0c0eec01f242924a7a50a2adc4c (§8)
...
Sustituye a            = docs/initiatives/I-55-architect-review-package-v3.md (historico; no se emitio veredicto sobre V3)
Base del codigo citado = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (la rama no tiene cambios de produccion)
Paralela cruzada       = I-52 Proposal V14 @ e59bc89 (docs/initiatives/I-52-proposal-v14.md en feature/rackmirror-espejo-semantico)
```

> **Que es este paquete.** Material autonomo para que un Arquitecto **independiente** emita el veredicto formal sobre V4. El ejecutor
...
>   incorporados.
> - Revision adversarial de V4 antes del commit (Proposal V4 §23), centrada en el redibujo atomico de CQ-01.
>
...
X-1..X-8:  COMPATIBLE | COMPATIBLE WITH CLARIFICATION | MATERIAL CONFLICT, contra I-52 V14
Version:   solo para el SHA exacto revisado
```

## 2. Que cambio de V2 a V3 y de V3 a V4
...
**De V3 a V4 (solo CQ-01, Proposal V4 §0.4).** D-15 pasa a redibujo atomico de hermanas antes de Insertar (PREPARE → una MUTATE →
POST); separacion explicita entre redibujo de hermanas y colocacion de vistas nuevas; conjuntos mutable y de solo lectura; huerfanas en la
MUTATE o en el primer jig; seam y envoltorios minimos por kind (§11.2); estados `PREPARE_FAILED`, `REDRAW_ROLLED_BACK`, `REDRAW_APPLIED`,
`PLACEMENT_CANCELLED_PARTIAL_BATCH`, `PLACEMENT_FAILED_PARTIAL_BATCH`, `COMPLETED`; INV-RED-1..3 e INV-BATCH-1; gate G9 dividido en G9a y
G9b; X-4 con dos operaciones (crear y redefinir) y X-5/X-6 frente a I-52 V14.
...
- **Preguntas.** ¿La separacion en tres responsabilidades es correcta y compartible con I-52 V14 §3.7/§3.8? ¿Clasificar como `Coerced` las
  dos formas del lateral Dinamico sin seccion es correcto? ¿La politica de `RACKEDITAR` por kind esta bien descrita?

### A-3 — Transaccion de ID18 y redibujo atomico de hermanas (D-06, D-15; §11)

- **Diseno.** Dos operaciones. (1) Redibujo de hermanas existentes en Insertar: variante → gate → clasificacion (READ-ONLY = dependientes
  de xref; MUTABLE = REDRAW y ERASE) → PREPARE de todas las unidades sin escribir estado del rack (solo importa biblioteca) → **una**
  transaccion que redefine cada hermana en la transaccion del llamador, escribe su payload, borra las huerfanas y acumula las anidadas
  obsoletas → un commit → POST (purga consolidada, renombres cosmeticos, un `Regen`). Cualquier excepcion en MUTATE → sin commit →
  `REDRAW_ROLLED_BACK`. (2) Colocacion de vistas nuevas tras el commit: una transaccion por jig (`P/Drawing/BlockPlacement.cs:174-201`);
  Esc o Enter conservan el redibujo y lo colocado. Si todas las vistas son huerfanas, se borran en la transaccion del primer jig.
- **Evidencia.** Precedente `P/ProjectVariableMutationExecutor.cs:140-303`; seams `P/Systems/Shared/SystemBlockWriter.cs:82-116`,
  `P/Systems/Shared/ViewBlockDraw.cs:65-112`, `P/Drawing/LateralHeaderDrawService.cs:60-107`,
  `P/Drawing/Cantilever/CantileverViewMaterializer.cs:63-90`; bucles vigentes que confirman por vista (`P/RackSelectivoCommands.cs:166-243`
  y equivalentes); renombre con transaccion propia y cosmetico (`P/Drawing/RackBlockRenamer.cs:21-58`); borrado de huerfanas con
  transaccion propia (`P/RackCommandSupport.cs:210-275`); importador fuera de transaccion (`P/Drawing/BlockLibraryImporter.cs:55-67`).
- **Preguntas.** ¿Revierte AutoCAD por completo la redefinicion, el payload y el borrado si la transaccion se descarta sin commit, dado lo
  que hacen `RedefineSystemBlock` y `RedefineBlock`? ¿Es correcto dejar la importacion en PREPARE y los renombres en POST? ¿Es correcta la
  regla de ubicacion del borrado de huerfanas? ¿Es correcto dejar fuera las dependientes de xref (OM-25)? ¿Esta completa la maquina de
  estados de §11.4?

### A-9 — Seam atomico y envoltorios por kind (D-15; §11.2; mapa G9a)

- **Diseno.** Plan y ejecucion puros en Application con un puerto de transaccion; unidades `HeaderRun` (sobre `PreparedViewRedraw`),
  `CantileverCurves` y `Erase` en el Plugin; envoltorios de dos metodos para Dinamico (lateral, frontal, planta), Push Back (lateral,
  frontal, planta) y planta de cabecera, con la misma lambda de plan que su `RedrawInPlace`; sin modificar `SystemBlockWriter.cs`,
  `LateralHeaderDrawer.cs` ni `CantileverViewMaterializer.cs`; `ProjectVariableMutationExecutor` intacto (OM-26).
- **Preguntas.** ¿Es el conjunto minimo sin framework paralelo? ¿Dan G9a (sin cablear) y G9b (cableado) un RED/GREEN claro? ¿Cubren las
  pruebas (1)-(15) del mapa lo que no puede comprobarse sin AutoCAD, con guardas de fuentes solo donde es inevitable?
...
- **Diseno.** Renombre puro de `DimensionViewKind` (`A/Systems/Shared/DimensionViewPolicy.cs:11-16`) a `A/Views/RackViewKind.cs`; no se
  ...
  G6 lo hace; si la guarda de enums de I-52 V14 §6.7 ya lo alcanza, decide el Coordinador de I-52.
...
### D-15 — Redibujo atomico de hermanas antes de Insertar (§11.1)

Ver A-3 y A-9. ¿Son INV-AUTH-1, INV-RED-1, INV-RED-2, INV-RED-3 e INV-BATCH-1 las invariantes correctas y verificables?
...
Contrastar con I-52 V14: [V13-D08] y [V14-D08] (§3.3, §3.7, §3.8, §5.1, §5.4, §6.1, §8.1..§8.5, §14.2 G4..G7, §20.1, CT-04, CT-05 y CT-16
provisionales); §15.4 y [V14-D14] (filas de G2C como entrada vinculante; diferencias de V3 registradas sin adoptar); §8.2
(`CreateInTransaction`); §19 (lista STOP de archivos); archivos excluidos (§20.3); protocolo (§15.3); ST-7..ST-10 (§4.1) y tolerancia de
escala (§4.3).
...
| X-4 | ¿Dos operaciones de la misma familia sin mezclar: **crear** en la transaccion del llamador (I-52 G6, consumida en I-55 G15) y **redefinir** en la transaccion del llamador (existente, I-47 G9, consumida por el redibujo atomico de I-55 G9a sin modificarla)? ¿Requisito estructural puro en `A/Drawing/LibraryBlockRequirement.cs` con regla de primer gate? |
...
| Redibujo atomico de hermanas | Application + guarda | `RackSiblingRedrawPlanTests`, `RackSiblingRedrawRunTests`, `RackSiblingRedrawUnits*Tests`, `SiblingRedrawSeamGuardTests` (G9a); `RackSiblingGateWiringGuardTests` (G9b) |
| Lote | Application | `RackViewBatchPlanTests` (G11: cancelar tras el redibujo) |
...
Filtros por nombre de clase con conteo; sin rutas modales en pruebas de UI. Matriz completa: mapa V4 §T.

## 5. Lo que no se pide al Arquitecto

Las elecciones de producto de M-01 y OD-1..OD-8 (Owner via Coordinador), la decision CQ-01 (ya tomada por el Coordinador), la aceptacion
de ADR-0042 (Owner) y la prioridad entre iniciativas (Coordinadores).

## 6. Riesgos tecnicos

Mapa V4 §R: R-01 regresion al re-enrutar; R-03 I-52 no adopta la tabla; R-05 un fallo de redibujo ya no aplica nada; R-10 y R-11
descriptores o paridad inconsistentes; R-16 la delegacion del BOM cambia un resultado; R-17 una transaccion para todas las hermanas; R-19
claves duplicadas; R-20 acoplamiento de X-2; R-21 sentido por mayoria; R-23 guardas de fuentes vigentes; R-24 envoltorio con otro plan.
...
| Revision adversarial de V4 | Proposal V4 §23 | Ataques de G2E sobre el redibujo atomico |
