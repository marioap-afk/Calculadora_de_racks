# I-52 — Proposal V16: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V16 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V16 se redacta SIN rebase, sobre ae8640a)
...
> Proposal V1..V14  = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd ·
>                     636f7fd · 2275f21 · 915a520 · dd45b0f · e59bc89 (historial)
> Proposal V15      = ae8640aac1ca2b06a21305e91bab591d9d5f2925   (historial; CI 34892904248 4/4)
> Revision Arq. V15 = «Architect: CHANGES REQUIRED — PROPOSAL V16»; sin BLOCKER; HIGH AR15-01 (EditingSessionPolicy no
>                     observable ni implementable: la ausencia de un modo no inventariado no se puede demostrar en
>                     ejecucion); MEDIUM AR15-02 (mutadores internos de AutoCAD y overrules o reactores fuera de
>                     G-M24 y de la politica de huellas); LOW L1..L3; cambios exactos V16-01..V16-06 de la revision
>                     (registro: docs/automation/decisions/I-52.md §92)
> Coordinador       = orden de V16 con V16-01..V16-14 vinculantes (registro §93)
> main y paralelas  = §15.1 (preflight de V16 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V16 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V16 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V16 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V15 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V15, sus revisiones de Arquitecto y esta V16 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V16 la registra
> como **indicativa**, ligada a su SHA-256, y no como contrato. Las revisiones de V10 y V11 no inspeccionaron la
> biblioteca: midieron codigo a SHA exacto y leyeron los ADR y las Proposals de las paralelas. La revision de V12
> contrasto V12 con el comportamiento documentado de AutoCAD (apertura parcial, carga bajo demanda de referencias
> externas y otros mecanismos de visualizacion). La revision de V13 midio en main los productores de contexto
> (`LateralHeaderDrawer`, `LayerHelper`, `CantileverViewMaterializer`), leyo la Amendment A3 de I-49 y contrasto V13 con
> el comportamiento documentado de AutoCAD para OOPS, REFEDIT, el Editor de bloques, los marcos de recorte y `XLOADCTL`.
> La revision de V14 contrasto V14 con el comportamiento documentado de AutoCAD para ARRAYEDIT Source y la ventana de
> prueba de bloques y midio en main los sitios de apertura para escritura del Plugin. La revision de V15 contrasto la
> politica de sesiones con la documentacion de AutoCAD (`ARRAYEDITSTATE`, `Overrule.Overruling`) y midio en main la
> ausencia de mutadores internos, overrules y suscripciones a eventos de AutoCAD. CT-49 y CT-50 deben confirmarlo en
> AutoCAD 2025. Ninguna revision ejecuto AutoCAD.

---

## 0. Reconciliacion V15 → V16

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`), V11 (`2275f21`), V12 (`915a520`), V13 (`dd45b0f`), V14 (`e59bc89`), V15
(`ae8640a`) y el Discovery (`339b3ab`) **se conservan sin cambios**. **Donde V15 y V16 difieran, manda V16.** Las marcas
`[V15-D01]`..`[V15-D15]` siguen vigentes salvo donde una marca `[V16-Dnn]` las modifica, reabre o sustituye (§0.4); las
marcas `[V14-Dnn]`, `[V13-Dnn]`, `[V12-Dnn]`, `[V11-Dnn]`, `[V10-Dnn]`, `[V9-Dnn]`, `[V8-Dnn]`, `[V7-Dnn]`, `[V6-Dnn]` y
`[V5-Dnn]` siguen como las dejaron V15, V14, V13, V12, V11, V10, V9, V8, V7 y V6 (§0.5 a §0.14).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V16** | V16 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V16. **Debe ser NONE** |

### 0.2 Matriz `[V16-D01]`..`[V16-D16]`

En la columna «Origen»: `AR15-nn` y `L1`..`L3` son los hallazgos de la revision de V15; `R:V16-nn`, su cambio exacto;
`C:V16-nn`, la precision vinculante del Coordinador en la orden de este gate. Las dos series V16-nn se numeran por
separado. **Donde el cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision
vinculante del Coordinador**: la revision agrupaba en un cambio (R:V16-01) la autoridad observable, el metodo cerrado,
el guard de host y la lectura fallida; el Coordinador los separa (C:V16-01..C:V16-04), fija que el resultado de
ejecucion es PASS solo con todas las condiciones y que un modo relevante sin detector fiable es un STOP de
caracterizacion; separa la guarda de codigo de RackCad (C:V16-05, C:V16-06) de la seguridad del dibujo frente a
overrules de terceros (C:V16-07); y reduce la identidad de XREF al contenido cargado actual y su completitud (C:V16-09).
**Precisiones del ejecutor:** (1) las señales de identidad del host (producto, vertical y version) son detectores del
propio `EditingSessionAuthoritySet` y su lectura fallida tambien da E12; (2) E12 es el codigo de P0; la misma causa
detectada al re-verificar da E10 en R6 y excepcion con rollback (E11) al inicio de MUTATE, con el mensaje de la causa,
como ya hacia ST-20; (3) las lecturas de los detectores son de solo lectura y nunca cambian variables ni sesiones; (4)
medido en `main @ dad4e77`, `src/` no usa ningun mutador interno de la familia nueva, no registra overrules y no se
suscribe a eventos de `Database`, `Document`, `Editor` ni `Application` de AutoCAD; (5) con un overrule activo de
terceros que CT-49 no pueda acotar por clase, la regla afecta a todos los objetos posteriores de las clases que pueda
alterar, y el riesgo de reduccion de alcance se registra en R-58.

| Marca | Origen | V15 | Correccion V16 | Estado | Donde |
|---|---|---|---|---|---|
| `[V16-D01]` | AR15-01 · R:V16-01 · C:V16-01 | `EditingSessionPolicy` exigia E12 para una sesion «no caracterizada o no demostrablemente ausente», pero no definia con que autoridad de ejecucion se demuestra la ausencia de un modo no inventariado: cabian dos lecturas conformes (comprobar solo lo inventariado y dejar pasar lo desconocido, o no poder probar nunca la ausencia y dar siempre E12) | **EDITING SESSION AUTHORITY SET** (`EditingSessionAuthoritySet`, nombre conceptual): conjunto finito y observable de detectores que CT-49 establece y que se congela con el contrato; la ejecucion **no** descubre modos desconocidos; resultado **PASS** si y solo si el host esta caracterizado, cada detector requerido se lee con exito, cada detector clasificado `BLOCKED` reporta inactivo y cada autoridad generica requerida reporta que no hay sesion de edicion activa; en otro caso E12; un «modo no caracterizado» es un resultado de **caracterizacion**, no una sonda de ejecucion: si CT-49 descubre un modo relevante sin detector dedicado fiable ni autoridad generica fiable que lo cubra ⇒ STOP y contradiccion material ⇒ Proposal posterior; `EditingSessionPolicy` sigue siendo la politica (`CHARACTERIZED_SAFE` o `BLOCKED`) y el conjunto es su autoridad observable; se prohiben expresamente las dos lecturas degeneradas | CLOSED BY V16 (contrato); conjunto concreto: DEFERRED TO G3 WITH FAIL-CLOSED | §1.3 L-34; §2.3; §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 E12; §11.3 puntos 5 y 14; §12 CT-49, T-M73; §13 M-35; §17 R-57; ADR-0036 decisiones 11 y 13 |
| `[V16-D02]` | AR15-01 · R:V16-01 · C:V16-02 | CT-49 debia hacer un «inventario de otros modos de edicion en sitio» sin metodo cerrado; `ARRAYEDITSTATE` no estaba nombrado | **CLOSED DISCOVERY METHOD**: CT-49 documenta un metodo de descubrimiento cerrado para AutoCAD 2025 que inspecciona y clasifica como minimo las **variables de estado de edicion de solo lectura** (`REFEDITNAME`, `BLOCKEDITOR`, `BLOCKTESTWINDOW`, `ARRAYEDITSTATE` y toda otra variable de estado de edicion o de sesion hallada al revisar de forma exhaustiva el catalogo de variables del sistema y la superficie de API aplicables), las **autoridades de transaccion** (`LongTransactionManager`, transaccion larga activa, pertenencia al working set o autoridad equivalente) y las **autoridades de comando y de sesion** (estado del comando activo relevante para la edicion en sitio persistente y toda otra autoridad generica de host o de sesion descubierta); cada señal queda `DETECTOR`, `IRRELEVANT` o `UNSUPPORTED / NO RELIABLE DETECTOR`; nunca se afirma seguridad exhaustiva desde una lista corta ad hoc | CLOSED BY V16 (contrato); ejecucion del metodo: DEFERRED TO G3 WITH FAIL-CLOSED | §11.3 punto 5; §12 CT-49; ADR-0036 decision 13 |
| `[V16-D03]` | AR15-01 · R:V16-01 · C:V16-03 | La caracterizacion de CT-49 se daba por valida en cualquier host que cargue el Plugin | **HOST GUARD**: la caracterizacion de CT-49 solo vale para el producto AutoCAD, la vertical o host y la version mayor o familia de API caracterizados; en ejecucion, host o version no caracterizados ⇒ E12 «entorno de edicion no caracterizado»; nunca se supone que una vertical se comporta como AutoCAD sin verticales | CLOSED BY V16 (contrato); identidad concreta del host: DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 E12; §12 CT-49; §13 M-35; ADR-0036 decisiones 11 y 13 |
| `[V16-D04]` | AR15-01 · R:V16-01 · C:V16-04 | Sin regla para un detector que falla al leerse | **DETECTOR READ FAILURE**: si un detector requerido del `EditingSessionAuthoritySet` lanza una excepcion, no esta disponible, no puede consultarse o devuelve un estado no clasificable ⇒ E12; **nunca** «lectura fallida ⇒ inactivo»; se comprueba en P0, R6 y MUTATE | CLOSED BY V16 | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 E12; §11.3 punto 5; §12 CT-49, T-M73; §13 M-35; §17 R-57; ADR-0036 decisiones 11 y 13 |
| `[V16-D05]` | AR15-02 · R:V16-02 · C:V16-05 | G-M24 no censaba los mutadores internos de AutoCAD, que escriben objetos sin apertura para escritura en el sitio del llamador | **INTERNAL AUTOCAD MUTATOR FAMILY** en G-M24(B): `BlockTableRecord.SynchronizeAttributes`; `LayerStateManager.RestoreLayerState` y `LayerStateManager.Restore*`; `Database.ReloadXrefs`, `Database.ResolveXrefs` y `Database.BindXrefs`; `AssocManager.EvaluateTopLevelNetwork` y `AssocManager.Evaluate*`; `Database.Audit`; `BlockReference.ResetBlock` y `BlockReference.ConvertToStaticBlock`; y equivalentes que se descubran; cada call-site relevante `DECLARED` o `EXCLUDED` con motivo; un mutador interno nuevo sin clasificar ⇒ G-M24 RED | CLOSED BY V16 (contrato); implementacion: G5..G7 | §8.7; §12 CT-50, T-M75, G-M24; ADR-0036 decision 7 |
| `[V16-D06]` | AR15-02 · R:V16-02 · C:V16-06 | G-M24 no censaba el registro de overrules ni las suscripciones a eventos que provocan escrituras diferidas | **OVERRULE / REACTOR REGISTRATION FAMILY** en G-M24(B): `DrawableOverrule`, `ObjectOverrule`, `TransformOverrule` y demas subclases de `Overrule`; `Overrule.AddOverrule` y `Overrule.RemoveOverrule`; eventos de `Database` (`ObjectModified`, `ObjectOpenedForModify`, `ObjectErased`, `ObjectAppended` y afines); suscripciones de `Document` o `Editor` a comandos o eventos que puedan causar escrituras diferidas; cada registro o suscripcion de RackCad `DECLARED` o `EXCLUDED` con motivo; T-M75 añade RED para un mutador interno y para un registro de overrule o reactor, ademas de los tres casos de V15 | CLOSED BY V16 (contrato); implementacion: G5..G7 | §8.7; §12 CT-50, T-M75, G-M24; ADR-0036 decision 7 |
| `[V16-D07]` | AR15-02 · R:V16-02 · C:V16-07 | La `ModelSpaceFootprintPolicy` calculaba las huellas desde la geometria de la base de datos, sin considerar que un overrule o reactor de terceros cambie la representacion dibujada | **THIRD-PARTY OVERRULE FOOTPRINT POLICY**, separada de la guarda de codigo de RackCad: CT-49 caracteriza la autoridad disponible (`Overrule.Overruling` o equivalente); primer corte: si la representacion dibujable puede estar afectada por un overrule o reactor activo y no se demuestra equivalencia u ocupacion conservadora ⇒ `VisualFootprint = UNKNOWN`, fail-closed cuando sea material para ST-19; no se inspeccionan los internos de plugins de terceros | CLOSED BY V16 (contrato); autoridad concreta: DEFERRED TO G3 WITH FAIL-CLOSED | §1.3 L-37; §11.3 punto 5; §12 CT-49, T-M73; §17 R-58; ADR-0036 decision 11 |
| `[V16-D08]` | L1 · R:V16-03 · C:V16-08 | La precedencia causal de los campos era determinista pero no explicitaba las dependencias mixtas | **MIXED FIELD DEPENDENCIES**: para un campo con entradas dependientes de edicion y entradas dependientes de estado, al calcular la envolvente de estado las entradas dependientes de edicion quedan **fijas** en su valor actual y solo varian las dependientes de estado entre los resultados de estado admitidos; ejemplos obligatorios: `Area(objeto) + Date`, `Length(objeto) + valor de DataLink`, propiedad de un objeto anidado + variable de estado, dependencia de estado de un XREF + geometria editada por separado; todo resultado que requiera EDITAR contenido logico queda fuera de la garantia; `DataLink` sigue siendo la excepcion explicita de ESTADO; sin cota conservadora de estado ⇒ `UNKNOWN` | CLOSED BY V16 | §1.3 L-37; §11.3 punto 5; §12 CT-49 |
| `[V16-D09]` | L2 · R:V16-04 · C:V16-09 | La identidad de XREF añadia un disparador de ejecucion («contenido cargado que puede no corresponder a su resolucion actual») sobreconservador e impreciso | **XREF IDENTITY SIMPLIFIED**: la seguridad de ejecucion usa el **contenido cargado actual** y su **completitud**; la ejecucion **no** compara identidades de archivos futuros; frontera: el mismo contenido cargado o efectivo que vuelve a estar disponible por un estado ⇒ ESTADO; una recarga o resolucion que introduce contenido cambiado o distinto ⇒ EDICION futura; no resuelto ahora ⇒ `UNKNOWN` ahora; se retira el disparador de V15 salvo que CT-49 demuestre que hace falta para la seguridad actual | CLOSED BY V16 | §1.3 L-37; §11.3 punto 5; §12 CT-49, T-M73 |
| `[V16-D10]` | L3 · R:V16-05 · C:V16-10 | I-56 registrado en `18401da` (Proposal V1) | **I-56 CURRENT**: ultimo observado `fde03863f42b805d5503dbd0d0ece3a7dceb4678` (G2.1: Proposal V2 PUBLISHED y NOT CONSENSUS; Coordinator REVIEW REQUIRED; Architect NOT OPEN; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE); en el re-fetch previo al commit, `f97a8b0` (G3 de I-56: Architect Review de la Proposal V2 = CHANGES REQUIRED — PROPOSAL V3; Coordinator sobre V2 = AGREED; Consensus NOT REACHED; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE; un archivo de docs); I-52 sigue grandfathered y ninguna regla de la Proposal V2 se aplica a I-52; solo hechos | CLOSED BY V16 (registro) | §0.3; §15.1; §15.2; ADR-0036 referencias |
| `[V16-D11]` | C:V16-11 | Casos de G3 de V15 | Casos obligatorios nuevos: **autoridad de sesiones** (conjunto cerrado de detectores con exito; lectura fallida de un detector ⇒ E12; host o version no soportados ⇒ E12; `ARRAYEDITSTATE` activo e inactivo; autoridad relevante de comando o de sesion activa; modo descubierto sin detector fiable ⇒ STOP de caracterizacion); **G-M24** (mutador interno ⇒ RED; registro de overrule ⇒ RED; registro de reactor o escritura diferida ⇒ RED); **representacion de terceros** (overrule activo ⇒ `UNKNOWN` salvo soporte conservador demostrado); **campos** (`Area + Date`; `Length + DataLink`; propiedad anidada editada + entrada de estado; estado de XREF + dependencia editada); **XREF** (mismo contenido cargado por otra ruta; archivo externo reemplazado sin recargar; recarga tras cambiar el archivo; estado no resuelto actual); G3 sigue NOT OPEN; contradiccion material ⇒ Proposal V17 | CLOSED BY V16 (plan); ejecucion: G3 y G5..G7 | §12 CT-49, CT-50, T-M73, T-M75; §14.2 G3 |
| `[V16-D12]` | C:V16-12 | L-34, L-37 y M-35 sin la dependencia del conjunto cerrado de detectores, el host, la lectura fallida ni los overrules de terceros | **O-1 PRACTICAL LIMITATIONS**: L-34 y M-35 exponen la dependencia del conjunto cerrado de detectores, el host o la version no soportados, la lectura fallida de un detector, ARRAYEDIT, la ventana de prueba de bloques y demas sesiones bloqueantes, y que un modo relevante sin detector descubierto en CT-49 bloquea el avance; L-37 expone que un overrule de terceros puede dejar la huella `UNKNOWN`, la envolvente causal de los campos mixtos y la regla del contenido cargado actual de los XREF. O-1 no se pide | CLOSED BY V16 | §1.3 L-34 y L-37; §13 M-35; §17 R-57, R-58 |
| `[V16-D13]` | R:V16-06 · C:V16-13 | ADR-0036 alineado con V15 | ADR-0036 sigue **PROPOSED**; decision 7 con los mutadores internos y los registros de overrules o reactores; decisiones 11 y 13 con el `EditingSessionAuthoritySet`, el metodo cerrado de descubrimiento, el guard de host, la lectura fallida de detectores, la politica de huellas frente a overrules de terceros, la envolvente de campos mixtos y la regla simplificada de XREF; hechos de I-56 actualizados; ningun resultado de G3 como hecho | CLOSED BY V16 | `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md` |
| `[V16-D14]` | R:V16-06 · C:V16-14 | Contradiccion material de V15 ⇒ Proposal V16 | Nueva matriz `[V16-Dnn]`; `[V15-D01]`, `[V15-D03]`, `[V15-D10]` y `[V15-D12]` se reabren y se cierran, y `[V15-D02]`, `[V15-D04]`, `[V15-D05]` y `[V15-D09]` se precisan; `OPEN MATERIAL = NONE` para la revision; toda contradiccion material de V16 ⇒ **Proposal V17**; proceso: acuerdo de Coordinador y Arquitecto sobre V16 → consenso tecnico → rebase sobre el main VIGENTE → reconciliacion obligatoria I-52/I-55/I-49 → estado formal vigente de I-55 y diferencias X-* resueltos → autoridad FINAL de I-49 para RS-3 → freeze por SHA exacto → re-check exact-SHA → O-1 → G3; I-52 sigue grandfathered; si G3 contradice materialmente ⇒ Proposal V17; si no, el Owner acepta ADR-0036 → G4 | CLOSED BY V16 (proceso) | §0; §14.1; §14.2; §19; §21 |
| `[V16-D15]` | Precision al redactar | Preflight de V15 | Preflight de V16 (§15.1): I-52 `ae8640a` exacta (arbol limpio, sin stash); main `dad4e77` sin cambios; I-49 `c8cfee2` sin cambios (A3-R1, re-revision exacta PENDING); I-55 `8068368` sin cambios (Proposal V4 `fe70d7a`); I-56 `fde0386` en el preflight (G2.1 Proposal V2, solo docs, CI `34893284825` success) y `f97a8b0` en el re-fetch (G3 Architect Review de la V2 = CHANGES REQUIRED — PROPOSAL V3, solo docs, CI `34895649923` success); sin ramas, produccion ni ADR nuevos; censo 35/35/35/35/33; biblioteca `b4ca2248…d560e8`; clasificacion: I-56 NON-MATERIAL; I-49 e I-55 sin cambios desde V15; sin STOP | CLOSED BY V16 (registro) | §15.1; §15.2 |
| `[V16-D16]` | Precision al redactar | Terminologia de V15 | **Terminologia**: `EditingSessionAuthoritySet` (nombre conceptual, no schema persistido) es la autoridad observable de `EditingSessionPolicy`; sus señales se clasifican `DETECTOR`, `IRRELEVANT` o `UNSUPPORTED / NO RELIABLE DETECTOR`; «host caracterizado» incluye producto, vertical y version mayor o familia de API; sin numeros nuevos de ST, PRE, E, CT, T-M ni G-M (CT-49, CT-50 y T-M75 añaden casos; R-58 es un riesgo nuevo) | CLOSED BY V16 | §4.1; §7.9; §9.1; §11.3 punto 5; §12; §17 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V17 o reduccion de alcance |
...
| **Contexto de evaluacion destino de las huellas visuales** (simulacion de la importacion con DRAWING FIRST y cobertura transitiva de simbolos, `[V10-D02]`, `[V10-D09]`, `[V11-D16]`) | colision o simbolo que no se puede simular como lo resolvera MUTATE ⇒ huella `UNKNOWN` (E10 en ST-19, X-22 en orden de plan) y, en piezas con cambio de mano, X-17a | G3 (CT-36) y G6/G7 (T-M63) |
| **Elegibilidad de la presentacion resuelta** (`ResolvedPresentationEligibility`: `LinetypeTableRecord` efectivo del dibujo destino por `Explicit`, `ByLayer(Named)`, `InheritLayer`, `InheritBlock`, cadenas de capa `0`, presentacion de creacion de la referencia de pieza y presentacion exterior de ST-13, con DRAWING WINS; `[V11-D01]`) | tipo de linea complejo con formas o texto, registro no determinable o contenido no caracterizado ⇒ `UNKNOWN` (X-17b en la evidencia de la pieza; huella `UNKNOWN`: E10 en ST-19, X-22 en el orden de plan) | G3 (CT-36, CT-49, CT-50) y G6/G7 (T-M63, T-M71) |
...
| **Mecanismo y consumo reales del contexto de materializacion** (`MaterializationContextReadSet` con cierre transitivo de texto y cota, `[V9-D05]`, `[V10-D04]`, `[V10-D05]`, `[V11-D06]`) | consumo no determinable ⇒ `UNKNOWN` (X-17a); productor asimetrico ⇒ `UNKNOWN`; observacion consumida cambiada antes de MUTATE ⇒ E10/E11 | G3 (CT-50, CT-36, CT-49) |
| **Guarda durable de los productores de contexto** (G-M24 con censo por familias de simbolos de API, incluidas la escritura o mutacion de objetos o contexto existentes, los mutadores internos de AutoCAD y los registros de overrules o reactores, `[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`, `[V15-D03]`, `[V16-D05]`, `[V16-D06]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas, o un call-site productor, consumidor, de escritura, de mutador interno o de registro de overrule o reactor nuevo sin clasificar ⇒ RED | G3 (CT-50: censo inicial), G5..G7 y despues de integrar |
...
| **Ocupacion de modelo conservadora por clase** (anchos geometricos transformados con la transformacion compuesta, con magnitud uniforme y reflexiones admitidas, mascaras, limites de texto y de cota, marcos, anidados; `[V9-D02]`, `[V10-D01]`, `[V11-D07]`, `[V12-D08]`) | huella no garantizada conservadora, magnitud no uniforme o anidado no soportado ⇒ `UNKNOWN` (E10 en ST-19, X-22 en orden de plan) | G3 (CT-36, CT-49) |
| **`ModelSpaceFootprintPolicy` por tipo exacto y por `ContentAvailability`/`VisibilityMechanism`** (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`) | clase, componente, estado o mecanismo no caracterizado ⇒ huella `UNKNOWN` ⇒ E10 si el objeto es posterior (ST-19); `Extents3d` solo donde CT-49 lo demuestre cota conservadora | G3 (CT-49) y G6/G7 (T-M73) |
| **Huella de orden latente por mecanismo** (capas, `Entity.Visible`, `ANNOALLVISIBLE`, XREF, filtros espaciales, recortes, estados dinamicos, campos, atributos invisibles y `ATTDISP`, aislamiento de objetos, visualizacion de proxies y datos externos de tabla; `[V11-D03]`, `[V12-D01]`..`[V12-D06]`, `[V13-D05]`) | contenido que puede reaparecer sin cambiar el orden y sin huella conservadora, o mecanismo no clasificado ⇒ `UNKNOWN` ⇒ E10; nunca contenido ausente = huella vacia; las ediciones futuras quedan fuera de la garantia (`[V13-D06]`, L-37) | G3 (CT-49) y G6/G7 (T-M73) |
| **Disponibilidad de XREF** (`[V12-D03]`, `[V13-D03]`) | descargado, no resuelto u overlay anidado no disponible ⇒ `UNKNOWN`; cargado solo con contenido completo y huella latente conservadora demostrados, y cargado bajo demanda con contenido que puede no estar en memoria ⇒ `UNKNOWN`; nunca se abren archivos externos, se fuerzan cargas ni se cambian variables | G3 (CT-49) y G6/G7 (T-M73) |
| **Recortes** (XCLIP, filtros espaciales, imagen, underlay, nube de puntos, capas internas; `[V12-D04]`) | ocupacion latente sin recortar o conservadora; si no puede calcularse ⇒ `UNKNOWN` | G3 (CT-49) |
| **Visibilidad dinamica de referencias posteriores** (`[V12-D05]`) | sin union conservadora de los estados activables ⇒ `UNKNOWN` | G3 (CT-49) |
| **Campos** (`DBText`, `MText`, atributos, celdas de tabla y otras clases; `[V12-D06]`, `[V15-D04]`, `[V16-D08]`) | sin cota conservadora para todos los resultados alcanzables solo por cambios de ESTADO, con las entradas dependientes de edicion fijas en su valor actual ⇒ `UNKNOWN`; el valor actual no es autoridad; un cambio de resultado causado por la EDICION del contenido referenciado queda fuera de la garantia (EDIT CAUSE > FIELD REEVALUATION) | G3 (CT-49) |
| **Ocupacion de modelo dependiente del entorno**, tambien en la copia (`PDMODE`, `PDSIZE`, `LTSCALE`, `MSLTSCALE`, representaciones anotativas, metricas y sustitucion de fuentes, estilos anotativos, `DIMSCALE = 0`, estilo hijo y lo que descubran CT-49 y CT-50; `[V11-D04]`, `[V12-D07]`) | sin ocupacion conservadora demostrada bajo todos los estados admitidos ⇒ `UNKNOWN` o limitacion explicita de O-1; con ella, estados declarados como observaciones consumidas | G3 (CT-49, CT-50) |
...
| **Superposiciones con todo objeto posterior a la fuente y orden efectivo en Model Space** (`[V9-D01]`, `[V9-D04]`, `[V10-D06]`, `[V10-D07]`, `[V11-D02]`, `[V11-D03]`, `[V12-D01]`, `[V12-D10]`) | superposicion material o no clasificable, huella no disponible o no conservadora, objeto posterior con huella `UNKNOWN` aunque parezca lejos, `XLINE`/`RAY` sin interseccion analitica, conflicto de ordenes de pantalla y trazado con copias superpuestas o contexto de orden no legible ⇒ E10 (ST-19) | G3 (CT-49) |
| **Completitud de la base de datos y de la enumeracion** (`DatabaseVisibilityCompleteness`, `[V13-D01]`, `[V13-D04]`, `[V14-D04]`, `[V15-D01]`) | prerrequisitos globales no demostrados, incluida la politica cerrada de sesiones de edicion ⇒ E12 en P0 (re-verificados en R6 con E10 y en MUTATE con E11); completitud relativa a la fuente no demostrada ⇒ E10 en R2 (temprano) o R6 (definitivo), E11 en MUTATE; nunca «no cargado» = «no ocupa»; nunca se clasifica una huella sobre una enumeracion incompleta; los objetos borrados no forman parte de la enumeracion (ERASE/OOPS = edicion, `[V14-D02]`) | G3 (CT-49) y G6/G7 (T-M73) |
| **Apertura parcial del dibujo** (`[V13-D02]`) | `Database.IsPartiallyOpened` o el mecanismo equivalente que caracterice CT-49 ⇒ E12 con mensaje propio; sin PARTIALOAD, cargas forzadas, cambios de capas ni mutacion; sin API confirmada, la completitud no se demuestra ⇒ fail-closed | G3 (CT-49) |
| **Politica cerrada de sesiones de edicion y su autoridad observable** (`EditingSessionPolicy` con `EditingSessionAuthoritySet`: conjunto cerrado de detectores fijado por CT-49 con un metodo de descubrimiento cerrado —variables de estado de edicion de solo lectura, autoridades de transaccion y autoridades de comando y de sesion— y guard de host; `[V14-D01]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`) | PASS solo con host caracterizado, lectura con exito de todos los detectores requeridos, todos los detectores `BLOCKED` inactivos y todas las autoridades genericas sin sesion activa; en otro caso E12 con mensaje por causa (E10 en R6 y E11 en MUTATE si aparece despues); lectura fallida ⇒ E12, nunca inactivo; host o version no caracterizados ⇒ E12; la ejecucion no descubre modos desconocidos: un modo relevante sin detector fiable descubierto en CT-49 ⇒ STOP de caracterizacion; sin REFCLOSE, BCLOSE, ARRAYCLOSE ni cambios de sesion | G3 (CT-49) y G6/G7 (T-M73) |
| **Frontera exacta entre estado y edicion** (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`, `[V15-D04]`, `[V15-D05]`, `[V16-D08]`, `[V16-D09]`) | estado dentro de la garantia, con huella latente conservadora o `UNKNOWN`; edicion futura fuera (ERASE/OOPS, redefinicion, XREF editado y recargado o resuelto a contenido distinto, fuente de imagen, underlay o nube de puntos reemplazada, parametro dinamico que cambia geometria); `DataLink` refrescado sigue siendo estado; un campo se clasifica por la causa del cambio de su resultado, con las entradas de edicion fijas; la seguridad de ejecucion de un XREF usa su contenido cargado actual y su completitud, sin comparar identidades futuras | contrato (sin diferir) |
| **Representacion alterada por overrules o reactores de terceros** (`Overrule.Overruling` o la autoridad que caracterice CT-49; `[V16-D07]`) | representacion dibujable que puede estar afectada por un overrule o reactor activo sin equivalencia u ocupacion conservadora demostrada ⇒ huella `UNKNOWN` ⇒ E10 cuando sea material para ST-19; nunca se inspeccionan los internos de plugins de terceros | G3 (CT-49) y G6/G7 (T-M73) |
| **Marcos y contornos de recorte** (`XCLIPFRAME`, `FRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, `PDFFRAME`, `DWFFRAME`, `DGNFRAME`, `POINTCLOUDCLIPFRAME`; `[V14-D06]`) | huella latente = contenido sin recortar ∪ contorno o marco que pueda hacerse visible; marco no acotable ⇒ `UNKNOWN`; modelos de coordinacion en el inventario, posiblemente `UNKNOWN` | G3 (CT-49) y G6/G7 (T-M73) |
| **Cola ordenada de Model Space posterior a cada fuente** (`ModelSpaceOrderTailSnapshot`, `[V11-D05]`) | objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada ⇒ E10 en PREPARE; excepcion y rollback en MUTATE | G6/G7 (T-M69) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V17 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V17 | G3 (CT-37, CT-38) |
...
| **Guardas de I-54 en main** (T-GRD-01..T-GRD-08, `[V8-D19]`, `[V10-D13]`) | sin llamadas de I-52 a sus autoridades (G-M22); T-GRD-02 re-medido en el SHA de reconciliacion (hoy 7; la Proposal V2 de I-55 preve otra llamada a `Compose`, `[V12-D17]`) y T-GRD-08 reapuntados con motivo (T-M16); G-M18 las aplica | rebase, G5..G7 |
| **Guardas de I-49 G6 y autoridad final de I-49** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones, con la correccion G6-C2 de `75f1862`; ADR-0041 vigente, cuya D19 **no** es la autoridad de RS-3; el Amendment A3, revisado por A3-R1 en `c8cfee2`, cambia la semantica futura de las observaciones del `PlanReadSet` de D19 y D20 y pide un ADR de reemplazo sin numero; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V13-D15]`, `[V14-D07]`, `[V15-D06]`, `[V15-D07]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18; RS-3 = PENDING I-49 FINAL AUTHORITY mientras no exista la autoridad final acordada, aceptada y congelada de I-49 (V6 + A1 + A2 + A3 revisado, ADR de reemplazo y Consensus Freeze nuevo), y con ese estado **I-52 FREEZE = BLOCKED**; si I-49 se cancela o abandona antes, sin desbloqueo automatico ni eliminacion silenciosa de RS-3: FREEZE BLOCKED hasta una decision del Coordinador en una Proposal (A, B o C de R-55) | reconciliacion antes del freeze de I-52 |
| **Reconciliacion con I-55 (e I-49 donde aplique) antes del freeze** (View Placement & Projection, Proposal V4 `fe70d7a` en revision con Arquitecto formal PENDING, tras la decision CQ-01 de su Coordinador sobre la V3; tabla X-1..X-8 acordada en la revision tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3 reclasifico como no formal, aceptada del lado de I-55 como entrada vinculante, y diferencias de sus Proposals V3 y V4 pendientes de revision formal y reconciliacion; ADR-0042 propuesto que complementa a ADR-0010; autoridades compartidas y sus ubicaciones provisionales; `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V13-D08]`, `[V13-D09]`, `[V14-D08]`, `[V14-D14]`, `[V15-D08]`, R-45) | sin reconciliacion registrada de las autoridades compartidas y sin el estado formal de la Proposal vigente de I-55 (hoy V4) y de las entradas X-* resuelto no hay freeze; la reconciliacion parte de las ultimas entradas X-* acordadas; una autoridad incompatible congelada por cualquiera de las dos ⇒ ningun freeze para ninguna hasta resolverlo; I-55 aplica la misma regla; I-52 no espera al consenso de I-55 | tras el consenso tecnico y el rebase, antes del freeze |
| **Autoridad de proceso frente a I-56** (Initiative Workflow V2 en G3 `f97a8b0`: Proposal V2 `fde0386` NOT CONSENSUS, con Coordinator AGREED sobre la V2, Architect CHANGES REQUIRED — PROPOSAL V3 y Owner NOT REQUESTED; Evidence Audit COMPLETE; I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; `[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`, `[V16-D10]`) | ninguna regla de las Proposals V1 o V2 de I-56 se aplica a I-52; reglas de I-56 no integradas e hipotesis del Evidence Audit no se adoptan; antes de cada transicion del proceso se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo | cada transicion del proceso |

### 0.4 Estado de las marcas de V15 bajo V16

| Marca V15 | Estado en V16 |
|---|---|
| `[V15-D01]` | **reabierta y cerrada** por `[V16-D01]` (autoridad observable), `[V16-D02]` (metodo cerrado), `[V16-D03]` (guard de host) y `[V16-D04]` (lectura fallida) |
| `[V15-D02]` | vigente y precisada por `[V16-D02]` (`ARRAYEDITSTATE` en el metodo cerrado) |
| `[V15-D03]` | **reabierta y cerrada** por `[V16-D05]` (mutadores internos) y `[V16-D06]` (overrules y reactores) |
| `[V15-D04]` | vigente y precisada por `[V16-D08]` (dependencias mixtas) |
| `[V15-D05]` | vigente y precisada por `[V16-D09]` (contenido cargado actual y completitud, sin comparar identidades futuras) |
| `[V15-D06]` | vigente |
| `[V15-D07]` | vigente |
| `[V15-D08]` | vigente |
| `[V15-D09]` | vigente como registro de I-56 en `18401da` y precisada por `[V16-D10]` (Proposal V2 `fde0386`) |
| `[V15-D10]` | **reabierta y cerrada** por `[V16-D11]` |
| `[V15-D11]` | vigente y ampliada por `[V16-D12]` |
| `[V15-D12]` | **reabierta y cerrada** por `[V16-D13]` |
| `[V15-D13]` | vigente como registro de la matriz de V15; el proceso lo sustituye `[V16-D14]` (Proposal V17) |
| `[V15-D14]` | vigente como registro del preflight de V15 y precisada por `[V16-D15]` |
| `[V15-D15]` | vigente y precisada por `[V16-D16]` |

### 0.5 Estado de las marcas de V14 bajo V15 (fijado por V15; sin cambios en V16)

| Marca V14 | Estado (fijado por V15) |
|---|---|
| `[V14-D01]` | **reabierta y cerrada** por `[V15-D01]` (politica cerrada de sesiones de edicion) y `[V15-D02]` (ARRAYEDIT Source y ventana de prueba de bloques) |
| `[V14-D02]` | vigente (ERASE/OOPS como edicion futura) |
| `[V14-D03]` | **reabierta y cerrada** por `[V15-D04]` (precedencia causal de los campos) y `[V15-D05]` (identidad de contenido de los XREF) |
| `[V14-D04]` | vigente (P0 global, R2 temprano, R3 solo rechaza, R6 definitivo, MUTATE con rollback); los prerrequisitos globales incluyen la politica de `[V15-D01]` |
| `[V14-D05]` | **reabierta y cerrada** por `[V15-D03]` (familia de escritura sobre objetos o contexto existentes; tres violaciones RED en T-M75) |
| `[V14-D06]` | vigente (marcos de recorte y modelos de coordinacion) |
| `[V14-D07]` | vigente y precisada por `[V15-D06]` (A3-R1 y redaccion de la autoridad vigente) y `[V15-D07]` (salida ante la cancelacion de I-49) |
| `[V14-D08]` | vigente (ubicaciones compartidas provisionales) |
| `[V14-D09]` | vigente y ampliada por `[V15-D11]` |
| `[V14-D10]` | vigente; los hechos de I-56 los actualiza `[V15-D09]` |
| `[V14-D11]` | **reabierta y cerrada** por `[V15-D10]` |
| `[V14-D12]` | **reabierta y cerrada** por `[V15-D12]` |
| `[V14-D13]` | vigente como registro de la matriz de V14; el proceso lo sustituye `[V15-D13]` (Proposal V16 y estado formal de I-55 antes del freeze) |
| `[V14-D14]` | vigente como registro del preflight de V14 y precisada por `[V15-D08]` (marcadores de I-55) y `[V15-D14]` (A3-R1 de I-49 e I-56 `18401da`) |
| `[V14-D15]` | vigente y precisada por `[V15-D15]` |

### 0.6 Estado de las marcas de V13 bajo V14 (fijado por V14; sin cambios en V15 ni V16)

| Marca V13 | Estado (fijado por V14) |
|---|---|
| `[V13-D01]` | **reabierta y cerrada** por `[V14-D01]` (sesiones de edicion) y `[V14-D04]` (prerrequisitos globales frente a completitud relativa a la fuente) |
| `[V13-D02]` | vigente y precisada por `[V14-D04]` (P0 solo global) |
| `[V13-D03]` | vigente y precisada por `[V14-D03]` (resolucion al mismo contenido) y `[V14-D09]` (`XLOADCTL` por defecto) |
| `[V13-D04]` | vigente y precisada por `[V14-D04]` (orden P0/R2/R3/R6) |
| `[V13-D05]` | vigente y precisada por `[V14-D06]` (marcos de recorte y modelos de coordinacion) |
| `[V13-D06]` | **reabierta y cerrada** por `[V14-D02]` (ERASE/OOPS) y `[V14-D03]` (precedencia exacta) |
| `[V13-D07]` | vigente y precisada por `[V14-D05]` (familias de API ampliadas) |
| `[V13-D08]` | vigente y precisada por `[V14-D08]` (ubicaciones de §14.2 provisionales) |
| `[V13-D09]` | vigente como registro de I-55 en `d091eeb` y precisada por `[V14-D14]` (Proposal V3 `8e35a51` y reclasificacion de G2C) |
| `[V13-D10]` | vigente; los hechos de I-56 los actualiza `[V14-D10]` |
| `[V13-D11]` | **reabierta y cerrada** por `[V14-D09]` |
| `[V13-D12]` | **reabierta y cerrada** por `[V14-D11]` |
| `[V13-D13]` | **reabierta y cerrada** por `[V14-D12]` |
| `[V13-D14]` | **reabierta y cerrada** por `[V14-D13]` (Proposal V15 y autoridad final de I-49 antes del freeze) |
| `[V13-D15]` | vigente como registro del preflight de V13 y precisada por `[V14-D07]` (A3 afecta a D19 y D20) y `[V14-D14]` |
| `[V13-D16]` | vigente y precisada por `[V14-D15]` |

### 0.7 Estado de las marcas de V12 (fijado por V13; sin cambios en V14, V15 ni V16)

| Marca V12 | Estado (fijado por V13) |
|---|---|
| `[V12-D01]` | **reabierta y cerrada** por `[V13-D01]` (completitud de la base de datos), `[V13-D02]` (apertura parcial), `[V13-D03]` (carga bajo demanda de XREF), `[V13-D04]` (completitud de la enumeracion) y `[V13-D05]` (mecanismos añadidos) |
| `[V12-D02]` | **reabierta y cerrada** por `[V13-D03]` y `[V13-D05]` |
| `[V12-D03]` | **reabierta y cerrada** por `[V13-D03]` |
| `[V12-D04]`, `[V12-D05]` | vigentes |
| `[V12-D06]` | vigente y precisada por `[V13-D05]` (vinculos de datos y formulas de tabla) y `[V13-D06]` (evaluacion de campos y vinculos dentro de la garantia) |
| `[V12-D07]`, `[V12-D08]` | vigentes |
| `[V12-D09]` | vigente y precisada por `[V13-D07]` (censo por familias de simbolos de API) |
| `[V12-D10]` | vigente |
| `[V12-D11]` | vigente y precisada por `[V13-D08]` (marca provisional ampliada; X-1..X-8 como entrada vinculante) |
| `[V12-D12]` | vigente y precisada por `[V13-D15]` (Amendment A3 de I-49, solo docs, con ADR de reemplazo de ADR-0041 pendiente) |
| `[V12-D13]` | vigente y precisada por `[V13-D10]` (reglas de transicion; I-52 grandfathered) |
| `[V12-D14]` | **reabierta y cerrada** por `[V13-D12]` |
| `[V12-D15]` | **reabierta y cerrada** por `[V13-D13]` |
| `[V12-D16]` | **reabierta y cerrada** por `[V13-D14]` (Proposal V14 y entradas X-* acordadas) |
| `[V12-D17]` | vigente como registro de I-55 `1e12414` y precisada por `[V13-D08]` y `[V13-D09]` |
| `[V12-D18]` | vigente como registro del preflight de V12 y precisada por `[V13-D15]` |
| `[V12-D19]` | vigente y precisada por `[V13-D11]` |
| `[V12-D20]` | vigente y precisada por `[V13-D16]` |

### 0.8 Estado de las marcas de V11 (fijado por V12; sin cambios en V13, V14, V15 ni V16)

| Marca V11 | Estado (fijado por V12) |
|---|---|
| `[V11-D01]` | vigente |
| `[V11-D02]` | **reabierta y cerrada** por `[V12-D01]` (mecanismo), `[V12-D02]` (inventario obligatorio), `[V12-D03]` (XREF), `[V12-D04]` (recortes), `[V12-D05]` (visibilidad dinamica) y `[V12-D06]` (campos) |
| `[V11-D03]` | **reabierta y cerrada** por `[V12-D01]`, `[V12-D02]`, `[V12-D04]` y `[V12-D05]` |
| `[V11-D04]` | vigente y precisada por `[V12-D06]` (campos) y `[V12-D07]` (entorno tambien en la copia) |
| `[V11-D05]` | vigente |
| `[V11-D06]` | vigente y precisada por `[V12-D07]` (estilos anotativos, `DIMSCALE = 0`, estilo hijo y fuentes en CT-50) |
| `[V11-D07]` | vigente y precisada por `[V12-D08]` (magnitud uniforme con reflexiones) |
| `[V11-D08]` | vigente y precisada por `[V12-D09]` (censo global de sitios productores) |
| `[V11-D09]` | vigente |
| `[V11-D10]` | vigente y precisada por `[V12-D11]` (autoridades compartidas provisionales) y `[V12-D17]` (I-55 V2, X-1..X-8) |
| `[V11-D11]` | vigente y precisada por `[V12-D12]` (I-49 `75f1862`, D5..D8) |
| `[V11-D12]` | vigente y precisada por `[V12-D17]` (ADR-0042 V2 complementa ADR-0010) |
| `[V11-D13]` | **reabierta y cerrada** por `[V12-D14]` |
| `[V11-D14]` | **reabierta y cerrada** por `[V12-D15]` |
| `[V11-D15]` | **reabierta y cerrada** por `[V12-D16]` (Proposal V13 y relectura del WORKFLOW) |
| `[V11-D16]` | vigente |
| `[V11-D17]` | vigente y precisada por `[V12-D10]` y `[V12-D19]` |
| `[V11-D18]` | vigente como registro del preflight de V11 y precisada por `[V12-D12]` y `[V12-D18]` |
| `[V11-D19]` | vigente y precisada por `[V12-D20]` |

### 0.9 Estado de las marcas de V10 (fijado por V11; sin cambios en V12, V13, V14, V15 ni V16)

| Marca V10 | Estado (fijado por V11) |
|---|---|
| `[V10-D01]` | **reabierta y cerrada** por `[V11-D02]` (`ModelSpaceFootprintPolicy`), `[V11-D03]` (visibilidad), `[V11-D04]` (entorno) y `[V11-D07]` (anchos de modelo transformados) |
| `[V10-D02]` | **reabierta y cerrada** por `[V11-D01]` (elegibilidad de la presentacion resuelta) y `[V11-D16]` (base auxiliar por pieza) |
| `[V10-D03]` | vigente y precisada por `[V11-D05]` (cola completa de objetos posteriores) y `[V11-D06]` (entradas transitivas) |
| `[V10-D04]` | vigente y precisada por `[V11-D06]` |
| `[V10-D05]` | vigente y precisada por `[V11-D06]` (cierre transitivo) y `[V11-D08]` (G-M24) |
| `[V10-D06]` | vigente |
| `[V10-D07]` | vigente y precisada por `[V11-D02]` (`XLINE`/`RAY` dentro de la politica por tipo exacto) |
| `[V10-D08]` | vigente y precisada por `[V11-D09]` (sin chip de alias y sin «/» final) |
| `[V10-D09]` | vigente y precisada por `[V11-D16]` |
| `[V10-D10]` | vigente y ampliada por `[V11-D02]` (inventario por tipo exacto de los objetos del dibujo) |
| `[V10-D11]`, `[V10-D12]`, `[V10-D13]` | vigentes |
| `[V10-D14]` | sustituida por `[V11-D11]` (ADR-0041 aceptado) |
| `[V10-D15]` | vigente |
| `[V10-D16]` | **reabierta y cerrada** por `[V11-D14]` |
| `[V10-D17]` | **reabierta y cerrada** por `[V11-D13]` |
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V17) |
| `[V10-D19]` | vigente y precisada por `[V11-D19]` |
| `[V10-D20]` | vigente y precisada por `[V11-D17]` |
| `[V10-D21]` | sustituida por el preflight de V11 (`[V11-D18]`) |
| `[V10-D22]` | vigente |
| `[V10-D23]` | vigente para main e I-53D; lo que registraba de I-49 lo sustituye `[V11-D11]` y lo de I-55, `[V11-D10]` y `[V11-D12]` |

### 0.10 Estado de las marcas de V9 (fijado por V10; sin cambios en V11, V12, V13, V14, V15 ni V16)

| Marca V9 | Estado (fijado por V10) |
|---|---|
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V17) |
| `[V9-D16]` | vigente |
...
### 0.11 Estado de las marcas de V8 (fijado por V9; sin cambios en V10..V16)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V17) |
...
### 0.12 Estado de las marcas de V7 (fijado por V8; sin cambios en V9..V16)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V17) |
...
### 0.13 Estado de las marcas de V6 (fijado por V7; sin cambios en V8..V16)
...
### 0.14 Estado de las marcas de V5 (fijado por V6; sin cambios en V7..V16)
...
### 0.15 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V16) |
|---|---|
...
### 0.16 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR15-01 (`EditingSessionPolicy` no observable ni implementable; `ARRAYEDITSTATE`; host; lectura fallida) | `[V16-D01]`..`[V16-D04]`, `[V16-D11]`, `[V16-D12]` |
| AR15-02 (mutadores internos de AutoCAD; overrules y reactores en la guarda y en las huellas) | `[V16-D05]`..`[V16-D07]`, `[V16-D11]`, `[V16-D12]` |
| L1 (dependencias mixtas de campos) | `[V16-D08]`, `[V16-D11]` |
| L2 (identidad de XREF en ejecucion) | `[V16-D09]`, `[V16-D11]` |
| L3 (I-56 Proposal V2) | `[V16-D10]` |
| Cambios exactos R:V16-01..R:V16-06 de la revision | V16-01 → D01..D04; V16-02 → D05..D07; V16-03 → D08; V16-04 → D09; V16-05 → D10; V16-06 → D13, D14 |
| Precisiones del Coordinador C:V16-01..C:V16-14 | V16-01 → D01; V16-02 → D02; V16-03 → D03; V16-04 → D04; V16-05 → D05; V16-06 → D06; V16-07 → D07; V16-08 → D08; V16-09 → D09; V16-10 → D10; V16-11 → D11; V16-12 → D12; V16-13 → D13; V16-14 → D14 |
| ADR-0036 (decisiones 7, 11 y 13 y referencias) | `[V16-D13]` |
| Paralelas y main observados en el preflight y en el re-fetch de V16 | `[V16-D10]`, `[V16-D15]` |
| Precisiones del ejecutor | identidad del host como detector, codigos por etapa, lecturas de solo lectura, censo de `src/` en `dad4e77` y alcance de overrules de terceros (§0.2), `[V16-D15]`, `[V16-D16]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V16; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V17 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-3** | **Evidencia de simetrias** | ningun bloque se asume simetrico; la prueba es la **equivalencia visual-geometrica** por estado evaluado, con politica cerrada de clases y variantes, fuentes visuales simbolicas tipadas con la elegibilidad de su presentacion resuelta en el dibujo destino, `VisibilityPath` y orden visual dentro de la pieza y entre piezas sobre huellas visuales de ocupacion de modelo, conservadoras y refrescadas tras `EnsureForPlan` (`GeometrySymmetryResult` y `VisualFootprintResult`, §11.3; `[V11-D01]`); la confirmacion del Owner **nunca** es la unica prueba | G3 (CT-36, CT-47, CT-48, CT-49) y G9 (M-15, M-27, M-32, M-36) |
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V16; si la contradice ⇒ Proposal V17; **precondicion de G4** |
...
| **L-27** | **Seleccion todo-o-nada**: un solo rack seleccionado que falle cualquier precondicion, representabilidad o verificacion aborta **todo** el comando; **no** se reflejan los demas | fin sin mutacion (E1..E12) | PDC-3, §9.1 |
...
| **L-30** | **Pieza con una clase o variante fuera de la politica cerrada** (§11.3 punto 6.1): subclase de una clase soportada (`MInsertBlock` anidado), definicion anidada anotativa, referencia anidada con XCLIP, con atributos o con definicion XREF, overlay o dependiente, grosor, normal, elevacion o ancho fuera de la tabla, tipo de linea **efectivo** complejo (con formas o texto), no determinable o no caracterizado en el dibujo destino, sea su fuente `Explicit`, `ByLayer`, una capa `0` heredada, la presentacion de creacion de la referencia de pieza o la presentacion exterior de ST-13 (`[V11-D01]`), HATCH de degradado (hoy `TARIMA_GENERICA`, §1.4) o de patron, SOLID/TRACE/HATCH no canonizable de forma inequivoca, o propiedad, entrada o estado sin clasificar o `UNSUPPORTED` en el inventario cerrado (`[V9-D08]`) | fail-closed | X-17b |
...
| **L-34** | **Orden visual en Model Space no demostrable**: una copia se superpone de forma visualmente material, o no clasificable, con **cualquier** objeto de Model Space posterior a su fuente en el orden efectivo (otra fuente admitida, un objeto seleccionado pero ignorado, uno no seleccionado u otro rack), o alguna huella visual o el contexto de orden no pueden garantizarse (`[V9-D01]`, `[V9-D04]`). **Todo objeto posterior con huella `UNKNOWN` hace fallar el comando aunque visualmente parezca estar lejos del destino de la copia**, porque `UNKNOWN` significa que no existe autoridad para demostrar la no superposicion (`[V12-D10]`). **Un dibujo abierto parcialmente hace fallar el comando** antes de pedir la linea, sin cargar lo que falta ni cambiar capas (`[V13-D02]`); **una edicion de referencia en curso (REFEDIT), una transaccion larga, el Editor de bloques abierto, la ventana de prueba de bloques abierta o la edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) tambien bloquean RACKMIRROR** antes de pedir la linea, sin cerrar ninguna sesion, y **tambien lo bloquean un host o una version de AutoCAD no caracterizados, la lectura fallida de cualquier detector del conjunto cerrado de sesiones y una autoridad generica que reporte una sesion de edicion activa**, aunque el usuario no vea ninguna sesion abierta; el comando depende del conjunto cerrado de detectores que establezca CT-49, y un modo relevante sin detector fiable descubierto alli detiene el avance del proyecto (`[V14-D01]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`); **ERASE/OOPS es edicion futura y no esta protegido**: un objeto borrado que OOPS restaura despues no forma parte de la garantia (`[V14-D02]`, L-37); **si la completitud de la enumeracion de Model Space o del contenido relevante de sus objetos no puede demostrarse, el comando falla** (`[V13-D01]`, `[V13-D04]`); **un XREF cargado bajo demanda puede hacer fallar el comando aunque se muestre como cargado**, si su contenido potencialmente visible puede no estar en memoria, y puede ocurrir en dibujos corrientes: CT-49 caracteriza el comportamiento con el `XLOADCTL` por defecto y, sin contenido completo demostrado, falla (`[V13-D03]`, `[V14-D09]`); y **un objeto `UNKNOWN` puede hacerlo fallar aunque parezca estar lejos** (`[V13-D11]`). En particular lo hacen fallar: un `XLINE` o un `RAY` posterior a una fuente, mientras CT-49 no caracterice una huella o interseccion analitica fiable (`[V10-D07]`); dos fuentes admitidas con orden relativo distinto en pantalla y en trazado cuyas copias se superponen de forma material, o no clasificable, porque ningun orden de creacion conserva los dos (`[V10-D06]`); un objeto posterior de una clase, un componente, un estado o un mecanismo que la `ModelSpaceFootprintPolicy` no caracteriza (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`); un objeto posterior cuya ocupacion **latente** se superpone o no puede acotarse aunque hoy no se vea o no este cargada —capa apagada, inutilizada o inutilizada en una ventana, `Entity.Visible = false`, representaciones anotativas, XREF descargado o no resuelto u overlay anidado no disponible (siempre `UNKNOWN` en el primer corte), contenido recortado por XCLIP u otro filtro espacial o por el recorte de una imagen, un underlay o una nube de puntos, estados de visibilidad dinamica sin union conservadora, atributos invisibles o `ATTDISP`, objetos ocultos o aislados, proxies cuyos graficos no se muestran o tablas con vinculos de datos o formulas sin cota conservadora (`[V13-D05]`)—, porque el orden persiste si ese mecanismo cambia (`[V11-D03]`, `[V12-D03]`..`[V12-D05]`); un objeto posterior con campos sin cota conservadora para todos sus resultados (`[V12-D06]`); un objeto posterior cuya representacion depende del entorno (`PDMODE`/`PDSIZE`, escalas de tipo de linea, representaciones anotativas o metricas de fuente) sin huella conservadora demostrada bajo todos los estados admitidos (`[V11-D04]`); y una cola de Model Space posterior a una fuente que cambia entre R6 y MUTATE (`[V11-D05]`) | fail-closed en P0, antes de pedir la linea (E12: apertura parcial o politica de sesiones de edicion sin PASS: REFEDIT, transaccion larga, Editor de bloques, ventana de prueba de bloques, ARRAYEDIT Source, host no caracterizado o lectura fallida de un detector), o en PREPARE, antes de MUTATE (E10); en MUTATE, excepcion y rollback | ST-19, ST-20, PRE-16, PRE-18, PRE-19 |
...
| **L-36** | **Presentacion interna regenerada bajo el contexto vigente** (`[V9-D05]`, `[V10-D05]`, `[V10-D12]`): los drawers no asignan explicitamente determinadas propiedades de las referencias internas de la copia (piezas y colocaciones de grupos) ni de sus anotaciones y cotas; CT-50 debe caracterizar que mecanismo y que valores efectivos aplica AutoCAD 2025 y que observaciones consume cada productor (`MaterializationContextReadSet`, con cierre transitivo: por ejemplo `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT`, `CETRANSPARENCY`, `CPLOTSTYLE`, el registro efectivo del estilo de cota con su estilo de texto, sus bloques de flecha y sus tipos de linea, el registro efectivo del estilo de texto con sus metricas de fuente, o registros de capa; la `AnnotationScale` authored es intencion del plan y `CANNOSCALE` solo entra si CT-50 demuestra su consumo, `[V11-D06]`). Si la huella de esas anotaciones, cotas o piezas depende del entorno —estilo de texto o de cota anotativo, `DIMSCALE = 0`, estilo hijo de la familia (`<estilo>$0`), fuente ausente o sustituida (`FONTALT`)— y CT-50 no demuestra una huella conservadora bajo todos los estados admitidos, la huella es `UNKNOWN` (E10 en ST-19, X-22 en el orden de plan; `[V12-D07]`). Como `RACKEDITAR` → Actualizar, **no** se copian fisicamente desde la definicion vieja, asi que la copia puede verse distinta de una fuente dibujada con otro contexto. La presentacion exterior de la referencia se conserva (ST-13) | regenerada como Actualizar; observacion consumida cambiada antes de MUTATE, productor asimetrico o huella dependiente del entorno sin cota conservadora ⇒ fail-closed (X-17a, X-22, E10/E11) | §8.7; ST-13; PRE-17; CT-50 |
| **L-37** | **Superposiciones solo de presentacion** (`[V10-D01]`, `[V10-D07]`): la huella visual representa **ocupacion de modelo en WCS** y **no** se dilata por `LineWeight` de AutoCAD, `LWDISPLAY`, grosores de CTB/STB, milimetros de papel ni escala de trazado. El primer corte **no** garantiza conservar el orden de dibujo cuando dos objetos solo se superponen visualmente por esas anchuras de linea, de papel o de pantalla y sus soportes de modelo no se superponen. Tampoco clasifica analiticamente `XLINE` ni `RAY`: sin huella o interseccion analitica caracterizada por CT-49 su huella es `UNKNOWN` y un `XLINE` o `RAY` posterior a una fuente hace fallar el comando (L-34). Y un objeto posterior cuya representacion depende del entorno y no tiene una huella conservadora demostrada por CT-49 bajo todos los estados que admite el contrato tiene huella `UNKNOWN` y puede hacer fallar el comando (L-34, `[V11-D04]`). Lo mismo vale para los mecanismos de carga, visibilidad y recorte (XREF descargado o no resuelto, filtros espaciales y recortes, estados de visibilidad dinamica y campos) y para la ocupacion dependiente del entorno de la **propia copia** (estilos anotativos, `DIMSCALE = 0`, estilo hijo y fuentes): sin huella latente conservadora la huella es `UNKNOWN` y hace fallar el comando aunque el objeto parezca estar lejos (L-34, L-36, `[V12-D01]`..`[V12-D07]`, `[V12-D10]`). **Frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`).** La garantia de huella latente cubre el **estado**: visibilidad (capas, `Entity.Visible` y representaciones anotativas); carga y descarga de XREF; que el mismo contenido cargado o efectivo de un XREF vuelva a estar disponible por un estado, con cualquier ruta (`[V15-D05]`, `[V16-D09]`); carga bajo demanda; recortes, con sus marcos; **solo** la visibilidad dinamica; evaluacion de campos cuyo resultado cambia **solo** por un estado admitido (`[V15-D04]`); refresco de `DataLink`, aunque su fuente externa haya cambiado (cota conservadora o `UNKNOWN`); visualizacion de proxies; aislamiento de objetos; y estado de visualizacion de anotacion y de entorno. **No** cubre la **edicion futura**: **ERASE/OOPS** y la restauracion programatica de objetos borrados; editar geometria; BEDIT o redefinir un bloque; editar un texto a mano; editar la geometria o una propiedad de un objeto referenciado por un campo, aunque el campo solo se reevalue despues (EDIT CAUSE > FIELD REEVALUATION); mover o estirar; un parametro dinamico —o una tabla de busqueda— que cambia la geometria, aunque tambien cambie la visibilidad; editar el archivo externo de un XREF y recargarlo; recargar o resolver un XREF introduciendo contenido cambiado o **distinto**, cualquiera sea la ruta; y reemplazar la fuente o el contenido de una imagen, un underlay o una nube de puntos. La seguridad de ejecucion de un XREF usa su contenido cargado actual y su completitud, sin comparar identidades de archivos futuros, y un XREF no resuelto ahora es `UNKNOWN` (`[V16-D09]`). En un campo con entradas de edicion y de estado, las de edicion quedan fijas y solo varian las de estado (`[V16-D08]`). **Un overrule o reactor de terceros** que pueda alterar la representacion dibujada de un objeto sin equivalencia u ocupacion conservadora demostradas deja su huella `UNKNOWN` y puede hacer fallar el comando aunque el objeto parezca lejos (`[V16-D07]`, R-58). RACKMIRROR no garantiza la invariancia del orden de dibujo frente a ediciones futuras arbitrarias. Si el Owner exige la garantia para superposiciones solo de presentacion, es un cambio de alcance para una Proposal posterior | orden no garantizado para superposiciones solo de presentacion; `XLINE`/`RAY` posterior, objeto posterior con mecanismo de carga, visibilidad o recorte sin huella latente conservadora y huella dependiente del entorno sin cota conservadora, tambien en la copia ⇒ E10; sin garantia frente a ediciones futuras del contenido | ST-19; §11.3 puntos 5 y 6.6 |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10, V11, V12, V13, V14, V15 y V16) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10, V11, V12, V13, V14, V15 y V16
(ST-19 completo, huellas visuales de ocupacion de modelo en el contexto de evaluacion destino, frescura tras
`EnsureForPlan`, orden de plan sobre pares invertidos, read-set transitivo del contexto de materializacion, elegibilidad
de la presentacion resuelta, politica de huellas por tipo exacto y por mecanismo de los objetos del dibujo, cola
completa de Model Space, entorno de la copia, transformaciones uniformes reflejadas, completitud de la base de datos y
de la enumeracion, carga bajo demanda de XREF y mecanismos latentes añadidos) no se han medido sobre esta biblioteca y
las miden CT-36, CT-48, CT-49 y CT-50. Estado: **DEFERRED TO G3** con fail-closed (X-17a, X-17b, X-22, X-23).
...
salvo los de PREPARE (re-verificacion de ST-20 y de definiciones, ST-19 temprano y definitivo, frescura de las huellas
visuales, cola de Model Space y read-set del contexto de materializacion, E10). La vista del Owner es la envolvente de
§1.3; la evidencia indicativa de la biblioteca inspeccionada esta en §1.4.
...
| **Pieza con tipo de linea efectivo complejo, no determinable o no caracterizado en el dibujo destino** | una capa homonima del dibujo con otro tipo de linea (DRAWING WINS), una capa `0` heredada de la referencia de pieza o de la referencia exterior, o un nombre `Explicit` cuyo registro del dibujo es complejo: la evidencia simbolica no demuestra que la representacion resuelta este soportada (`[V11-D01]`) | X-17b (§11.3 puntos 6.1 y 6.2); huella `UNKNOWN` |
| **Objeto posterior sin huella de orden soportada** | clase, componente, estado o mecanismo no caracterizado por la `ModelSpaceFootprintPolicy`, ocupacion latente no acotable con independencia de la visibilidad, la carga o el recorte, o representacion dependiente del entorno sin huella conservadora (`[V11-D02]`..`[V11-D04]`, `[V12-D01]`, `[V12-D02]`); falla aunque el objeto parezca estar lejos (`[V12-D10]`) | E10 (ST-19) |
| **XREF posterior descargado, no resuelto o con overlay anidado no disponible** | su contenido no esta en el dibujo y puede reaparecer sin cambiar el orden; RACKMIRROR no abre archivos externos (`[V12-D03]`) | E10 (ST-19) |
| **Dibujo abierto parcialmente** | los objetos no cargados no estan en la cola de Model Space y reaparecerian con su orden original debajo de la copia; RACKMIRROR no ejecuta PARTIALOAD, no fuerza cargas y no cambia capas (`[V13-D02]`) | E12 (ST-20), antes de pedir la linea; E10 en R6 o E11 en MUTATE si aparece despues |
| **Edicion de referencia en curso (REFEDIT), transaccion larga o Editor de bloques abierto** | el working set temporal vuelve a su definicion al cerrar la sesion y las copias podrian incorporarse a el; en el Editor de bloques el espacio de trabajo no es Model Space; RACKMIRROR no ejecuta REFCLOSE ni BCLOSE (`[V14-D01]`, `[V15-D01]`) | E12 (ST-20), antes de pedir la linea; E10 en R6 o E11 en MUTATE si aparece despues |
| **Edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) o ventana de prueba de bloques abierta** | en ARRAYEDIT Source los objetos fuente se editan en Model Space y, segun el comportamiento documentado de AutoCAD (a confirmar por CT-49), los objetos nuevos se aplican a la fuente de la matriz, de modo que ST-19 razonaria sobre geometria temporal y las copias podrian confirmarse en otra estructura estable; la ventana de prueba de bloques es un entorno temporal de prueba; RACKMIRROR no razona sobre la geometria fuente temporal ni sobre su replicacion y no ejecuta ARRAYCLOSE ni cierra la ventana (`[V15-D02]`) | E12 (ST-20), antes de pedir la linea; E10 en R6 o E11 en MUTATE si aparece despues |
| **Sin PASS del `EditingSessionAuthoritySet`: host o version no caracterizados, lectura fallida de un detector o autoridad generica con sesion activa** | la ejecucion no descubre modos desconocidos: solo consulta el conjunto cerrado de detectores que CT-49 establecio para el host caracterizado; un host distinto, una lectura que falla o devuelve un estado no clasificable, o una autoridad generica (transaccion larga, working set, comando o sesion) que reporta actividad no permiten demostrar la ausencia de sesiones de edicion; un modo relevante sin detector fiable descubierto en CT-49 es un STOP de caracterizacion, no un fallo de ejecucion (`EditingSessionPolicy`, `[V15-D01]`, `[V16-D01]`..`[V16-D04]`) | E12 (ST-20), antes de pedir la linea, con mensaje por causa; E10 en R6 o E11 en MUTATE si aparece despues |
| **Cola posterior a una fuente o contenido relevante no demostrablemente completos** | sin completitud relativa a la fuente no se clasifica ninguna huella (`[V13-D01]`, `[V13-D04]`, `[V14-D04]`) | E10 (ST-20 en R2, temprano, o en R6, definitivo) |
| **XREF posterior cargado bajo demanda con contenido que puede no estar en memoria** | capas inutilizadas o contenido fuera del recorte que la carga bajo demanda con indices (`XLOADCTL`, `INDEXCTL`) puede no cargar; contenido completo no demostrado por CT-49 (`[V13-D03]`) | E10 (ST-19) |
| **Objeto posterior con atributos invisibles, oculto o aislado, proxy o tabla con datos externos, sin metodo latente conservador** | `AttributeReference.Invisible` o `ATTDISP`, `HIDEOBJECTS`, `ISOLATEOBJECTS` u `OBJECTISOLATIONMODE`, `PROXYSHOW` o `PROXYGRAPHICS`, `DataLink` o formulas de tabla (`[V13-D05]`) | E10 (ST-19) |
| **Objeto posterior recortado sin ocupacion latente calculable** | XCLIP u otro filtro espacial, recorte de imagen, de underlay o de nube de puntos, o capas internas o controles de visualizacion que ocultan contenido que puede reaparecer (`[V12-D04]`) | E10 (ST-19) |
| **Objeto posterior recortado con marco o contorno no acotable** | el marco de `XCLIPFRAME`, `FRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, `PDFFRAME`, `DWFFRAME`, `DGNFRAME` o `POINTCLOUDCLIPFRAME` puede quedar fuera del contenido sin recortar y hacerse visible (`[V14-D06]`) | E10 (ST-19) |
| **Referencia posterior con visibilidad dinamica sin union conservadora de sus estados** | el estado actual no basta (`[V12-D05]`) | E10 (ST-19) |
| **Objeto posterior con campos sin cota conservadora** | su contenido cambia al evaluarse; el valor actual no es autoridad (`[V12-D06]`) | E10 (ST-19) |
| **Anotacion, cota o pieza de la copia con huella dependiente del entorno sin cota conservadora** | estilo de texto o de cota anotativo, `DIMSCALE = 0`, estilo hijo de la familia, fuente ausente o sustituida (`[V12-D07]`) | huella `UNKNOWN`: E10 en ST-19, X-22 en el orden de plan |
| **Cola de Model Space posterior a una fuente que cambia antes de MUTATE** | objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada entre R6 y MUTATE (`[V11-D05]`) | E10 en PREPARE; E11 en MUTATE |
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V13-D08]`).** El valor de colocacion y su composicion y
> descomposicion sobre `Transform2D` son fundacion compartida con I-55 (X-7, acordada en la revision tecnica adversarial
> de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y
> V4 pendientes de revision formal y reconciliacion, con custodio I-52: `Transform2D` mas un valor de colocacion, con la
> reflexion como politica de I-52 y la rigidez como politica de I-55). El algebra de la reflexion y las comprobaciones
> de esta seccion son de I-52; nombre, ubicacion y forma de API del valor de colocacion no los congela I-52 sola (§8.3,
> §14.1, §15.4).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** La taxonomia de tipo de vista y el
> codec de `View`/`Section` son fundacion compartida con I-55 (X-2, acordada en la revision tecnica adversarial de G2C @
> `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y V4
> pendientes de revision formal y reconciliacion; ADR-0042 decisiones 2 y 8, propuesto). La semantica que I-52 necesita
> —la lectura de produccion, la forma canonica y el fallo cerrado de lo no inequivoco— puede acordarse; propiedad,
> namespace, ubicacion, forma de API y codec neutral definitivo no los congela I-52 sola: se fijan en la reconciliacion
> obligatoria previa al freeze, con X-2 como entrada vinculante (§14.1, §15.4). Tambien son provisionales el origen y el
> tramo del eje de la vista que fijan `c` (X-8) y la caracterizacion unica CT-04.
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** La exposicion de `μ_k` y la politica
> de fallo cerrado son de I-52; el conjunto de vistas soportadas por sistema (`A_k` = disponible ∩ expuesto, segun X-2,
> acordada en la revision tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal,
> con las diferencias de sus Proposals V3 y V4 pendientes de revision formal y reconciliacion), su matriz normativa y la
> disponibilidad sobre el sistema resuelto comparten fundacion con la matriz de I-55, y su ubicacion y forma de API se
> fijan en la reconciliacion (§14.1, §15.4).
...
```text
Antes de C1, una vez por comando: ST-20, prerrequisitos globales (P0): apertura parcial o EditingSessionPolicy
         sin PASS en su EditingSessionAuthoritySet (host no caracterizado, lectura fallida de un detector, detector
         BLOCKED activo o autoridad generica con sesion activa: REFEDIT, transaccion larga, Editor de bloques,
         ventana de prueba de bloques, ARRAYEDIT Source) ⇒ E12 (fin, sin linea); se re-verifican en R6 (E10) y en
         MUTATE (E11)
...
PREPARE (depende de la linea): R2 enumera de forma temprana la cola posterior a cada fuente; R3 puede rechazar con
         ST-19 temprano, nunca aprobar; R6 re-enumera, decide la completitud relativa a la fuente (ST-20) y, solo si
         pasa, ST-19 definitivo con huellas de ocupacion de modelo en el contexto destino, tras releer el dibujo real
         (§9.1, PRE-18, PRE-19)
```
...
| **ST-20** | P0 (prerrequisitos globales, antes de C1); PREPARE (R2 temprano y R6 definitivo, completitud relativa a la fuente, antes de ST-19 definitivo); inicio de MUTATE | **Completitud de la base de datos y de la enumeracion** (`DatabaseVisibilityCompleteness`, nombre conceptual; `[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`, `[V14-D02]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`). **(G) Prerrequisitos globales, P0**, conocibles antes de las colas relativas a cada fuente: la base de datos del dibujo no esta abierta parcialmente (`Database.IsPartiallyOpened` o el mecanismo equivalente que caracterice CT-49); y la **politica cerrada de sesiones de edicion** (`EditingSessionPolicy`, nombre conceptual) se cumple segun su **autoridad observable**, el `EditingSessionAuthoritySet` (nombre conceptual): un conjunto finito de detectores que CT-49 establece con un metodo de descubrimiento cerrado —variables de estado de edicion de solo lectura (`REFEDITNAME`, `BLOCKEDITOR`, `BLOCKTESTWINDOW`, `ARRAYEDITSTATE` y las demas que encuentre la revision exhaustiva del catalogo de variables y de la superficie de API de AutoCAD 2025), autoridades de transaccion (`LongTransactionManager`, transaccion larga activa, pertenencia al working set o equivalente) y autoridades de comando y de sesion— y que se congela con el contrato; cada señal descubierta queda `DETECTOR`, `IRRELEVANT` o `UNSUPPORTED / NO RELIABLE DETECTOR`. La ejecucion **no** descubre modos desconocidos: el resultado es **PASS** si y solo si el host (producto, vertical y version mayor o familia de API) esta caracterizado, cada detector requerido se lee con exito, cada detector clasificado `BLOCKED` reporta inactivo y cada autoridad generica requerida reporta que no hay sesion de edicion activa; en cualquier otro caso ⇒ E12. En el primer corte son `BLOCKED` la edicion de referencia en curso, la transaccion larga, el Editor de bloques, la ventana de prueba de bloques y la edicion de la fuente de una matriz asociativa (ARRAYEDIT Source), sin razonar sobre su geometria fuente temporal ni sobre su replicacion. Una lectura que lanza una excepcion, no esta disponible, no puede consultarse o devuelve un estado no clasificable ⇒ E12, **nunca** inactivo; host o version no caracterizados ⇒ E12 «entorno de edicion no caracterizado». Un modo «no caracterizado» es un resultado de **caracterizacion**, no una sonda de ejecucion: si CT-49 descubre un modo relevante sin detector dedicado fiable ni autoridad generica fiable que lo cubra ⇒ STOP y contradiccion material ⇒ Proposal V17; se prohiben las dos lecturas degeneradas (dejar pasar lo no inventariado y fallar siempre por no poder probar lo desconocido); y cualquier otro prerrequisito global que caracterice CT-49. P0 **no** promete la completitud de la cola por fuente. **(S) Completitud relativa a la fuente**, despues de clasificar las fuentes y de la LINE: R2 construye la enumeracion temprana de la cola posterior a cada fuente y las entradas de huella; R3 solo puede rechazar; R6 re-enumera y demuestra de forma definitiva que la cola contiene todo objeto de Model Space posterior a la fuente en el orden efectivo y que el contenido relevante de cada objeto enumerado esta suficientemente disponible para una huella latente conservadora; cuando la falta de disponibilidad es de un objeto concreto (XREF descargado, no resuelto o cargado bajo demanda con contenido que puede no estar en memoria), la etapa `ContentAvailability` lo clasifica `UNKNOWN`, que tambien hace fallar ST-19 (§11.3 punto 5). Los objetos borrados no forman parte de la enumeracion relevante: ERASE/OOPS es edicion futura (`[V14-D02]`). «No esta cargado» o «no aparece en la base de datos» **nunca** se interpreta como «no ocupa». RACKMIRROR no ejecuta PARTIALOAD, REFCLOSE, BCLOSE ni ARRAYCLOSE, no fuerza cargas ni el cierre de ningun editor, ventana o sesion, no mueve entidades del working set, no abre archivos externos, no altera capas, no cambia variables del dibujo y no muta el dibujo ni la sesion. Mientras CT-49 no establezca el `EditingSessionAuthoritySet`, las API exactas y la deteccion fiable en AutoCAD 2025, los prerrequisitos no se demuestran | prerrequisito global incumplido o no demostrable ⇒ **E12** en P0, con mensaje propio por causa («dibujo abierto parcialmente», «edicion de referencia en curso (REFEDIT): cierrela antes de reflejar», «Editor de bloques abierto: cierrelo antes de reflejar», «ventana de prueba de bloques abierta: cierrela antes de reflejar», «matriz asociativa en edicion (ARRAYEDIT): cierre la edicion antes de reflejar», «transaccion larga activa», «entorno de edicion no caracterizado» y «no se pudo leer el estado de sesion de edicion»), y ⇒ **E10** si aparece al re-verificarlo en R6; completitud relativa a la fuente no demostrable ⇒ **E10** en R2 (rechazo temprano) o en R6 (decision definitiva); cualquier cambio de ese estado al inicio de MUTATE ⇒ excepcion con rollback (**E11**); nunca se clasifica una huella sobre una enumeracion incompleta |
| **ST-19** | PREPARE | Orden visual en Model Space (`[V9-D01]`, `[V9-D04]`, `[V9-D19]`, `[V10-D01]`..`[V10-D03]`, `[V10-D06]`, `[V10-D07]`, `[V11-D02]`..`[V11-D05]`, `[V12-D01]`..`[V12-D08]`, `[V12-D10]`, `[V13-D01]`..`[V13-D05]`, `[V14-D01]`..`[V14-D06]`): su decision definitiva solo se toma si ST-20 demuestra en R6 la completitud relativa a la fuente, con los prerrequisitos globales de P0 re-verificados, y R3 puede rechazar antes pero nunca aprobar; sigue el orden logico de etapas de §11.3 punto 5 (`[V13-D04]`, `[V14-D04]`); las copias se crean en el **orden efectivo relativo de sus fuentes** (orden de dibujo de Model Space con su `SortentsTable` y el contexto de orden que fije CT-49); cada copia queda despues de todo objeto existente. Para cada copia `c` de su fuente `s` y **todo** objeto existente `o` de Model Space con `o ≠ s` que va **despues** de `s` en el orden efectivo —otra fuente admitida seleccionada, un objeto seleccionado pero ignorado por C1..C3, un objeto no seleccionado, otro rack o cualquier entidad— se clasifica la superposicion de `VisualFootprint(c)` y `VisualFootprint(o)` (§11.3 punto 5) con las clases de §11.3 punto 6.5 ampliadas a Model Space (rellenos, degradados, solidos, wipeouts, imagenes, OLE, texto con mascara, transparencia no opaca y referencias que los contengan). Las huellas son **ocupacion de modelo en WCS**, sin anchuras de linea, de papel ni de pantalla (L-37): `VisualFootprint(c)` = union de las huellas de las instancias de los planes **definitivos** de cada vista seleccionada, con el nombre destino, evaluadas en el `DestinationEvaluationContext` (§11.3 punto 4) y construidas **despues** de transformar a WCS con `P'` con magnitud uniforme y reflexiones admitidas (`[V12-D08]`), mas las de sus anotaciones y cotas, con la regla de entorno aplicada tambien a la copia (`[V12-D07]`); `VisualFootprint(o)` = ocupacion de modelo **latente** del objeto segun la `ModelSpaceFootprintPolicy` de su tipo exacto en tiempo de ejecucion y de su `ContentAvailability` y sus `VisibilityMechanism` (`SUPPORTED(metodo)` o `UNKNOWN`, nunca un fallback optimista; `Extents3d` solo en las clases donde CT-49 demuestre que es una cota conservadora tras añadir todas las ocupaciones de modelo necesarias; sin dilatar por `LineWeight`; `XLINE` y `RAY` ⇒ `UNKNOWN` salvo huella o interseccion analitica caracterizada por CT-49), que representa todo contenido que pueda volverse visible sin cambiar la relacion de orden: independiente de la visibilidad transitoria (capa apagada, inutilizada o inutilizada en una ventana, `Entity.Visible = false`, `ANNOALLVISIBLE`), de la carga (XREF descargado, no resuelto u overlay anidado no disponible ⇒ `UNKNOWN`; XREF cargado bajo demanda cuyo contenido potencialmente visible puede no estar en memoria ⇒ `UNKNOWN` salvo contenido completo demostrado por CT-49, `[V13-D03]`; nunca contenido ausente = huella vacia, sin forzar cargas y sin abrir archivos externos), de los recortes (XCLIP, filtros espaciales y recortes de imagen, underlay o nube de puntos: ocupacion sin recortar ∪ contorno o marco que pueda hacerse visible, o `UNKNOWN`, `[V14-D06]`), de los estados de visibilidad dinamica (union conservadora de los estados activables o `UNKNOWN`) de los campos (cota conservadora para todos los resultados o `UNKNOWN`) y de los atributos invisibles y `ATTDISP`, el aislamiento de objetos, la visualizacion de proxies y los vinculos de datos y formulas de tabla (metodo latente conservador o `UNKNOWN`, `[V13-D05]`), porque el orden persiste si ese mecanismo cambia; un mecanismo no clasificado ⇒ `UNKNOWN`, y, si su representacion depende del entorno, `SUPPORTED` solo con una ocupacion conservadora demostrada bajo todos los estados que admite el contrato (`[V11-D02]`..`[V11-D04]`). Todo objeto posterior con huella `UNKNOWN` hace fallar ST-19 aunque parezca estar lejos del destino, porque no existe autoridad para demostrar la no superposicion (`[V12-D10]`). Los objetos anteriores a `s` no se clasifican; la fuente `s` es excepcion deliberada (la copia puede quedar encima de ella); las copias entre si siguen el orden de sus fuentes. Si CT-49 encuentra contextos de pantalla y de trazado con ordenes distintos, «posterior» significa posterior en cualquiera de ellos y, si dos fuentes admitidas tienen orden relativo distinto entre esos contextos y sus copias se superponen de forma material o no clasificable, no existe un orden de creacion que conserve los dos. La evaluacion anterior a `EnsureForPlan` es rechazo anticipado; la decision definitiva se toma tras releer el dibujo real, con las huellas refrescadas y la cola de Model Space posterior a cada fuente enumerada de nuevo en R6 (`ModelSpaceOrderTailSnapshot`: identidades ordenadas, clases y contexto efectivo de orden), que se vuelve a enumerar al inicio de MUTATE; un objeto anterior que sigue siendo anterior no invalida (PRE-18, `[V11-D05]`) | superposicion **material o no clasificable**, huella no disponible, infinita o dudosa, `XLINE`/`RAY` posterior sin interseccion analitica, ordenes de pantalla y de trazado contradictorios con copias superpuestas, orden efectivo no legible, huella que cambia tras importar sin re-demostrarse, objeto posterior de una clase, un componente, un estado o un mecanismo no soportados por la `ModelSpaceFootprintPolicy` (aunque parezca lejos), XREF posterior descargado o no resuelto, XREF cargado bajo demanda con contenido que puede no estar en memoria, atributos invisibles, aislamiento, proxies o datos externos de tabla sin metodo latente conservador, recorte sin ocupacion latente calculable, visibilidad dinamica sin union conservadora, campos sin cota conservadora, ocupacion latente no acotable, representacion dependiente del entorno sin huella conservadora (tambien en la copia) o cola de Model Space cambiada (objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada) ⇒ **fail-closed (E10)** en PREPARE, antes de MUTATE (`[V8-D25]`); en MUTATE, excepcion y rollback (E11) |
...
definitiva se toma tras releer el dibujo real, con las huellas, el orden de plan y ST-19 refrescados (PRE-18). ST-19 no
toma su decision definitiva si ST-20 no demuestra en R6 la completitud relativa a la fuente con los prerrequisitos
globales vigentes (`[V13-D01]`, `[V14-D04]`). Las superposiciones que solo existen por anchuras de linea, de papel o de
pantalla no se clasifican (L-37).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** El nucleo neutral de seleccion es
> fundacion compartida con I-55 (X-1, acordada en la revision tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3
> de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y V4 pendientes de revision formal y
> reconciliacion, con custodio I-52 y CT-16 como caracterizacion unica). Los hechos neutrales y la equivalencia de la
> fachada son necesarios para I-52; propiedad, namespace, ubicacion y forma de API definitivos del nucleo no los congela
> I-52 sola: se fijan en la reconciliacion y G4 avisa a I-49 y a I-55 antes de tocarlo (§14.1, §15.4).
...
equivalencia de la fachada: **STOP → Proposal V17**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V13-D08]`).** El comparador authored por kind
> (`IsSameAuthority`) es fundacion compartida con I-55 (X-3, acordada en la revision tecnica adversarial de G2C @
> `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y V4
> pendientes de revision formal y reconciliacion, con custodio I-52: el comparador vive en el contrato del kind, fuera
> del reflector del espejo, que solo lo referencia). La politica de miembros de I-52 (las vistas seleccionadas) y el
> fallo E3 son de I-52; donde se declara el comparador y su forma de API se fijan en la reconciliacion (§6.1, §14.1,
> §15.4).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V13-D08]`).** `DescribeView`, `ViewExposure` (con `c` y `σ'`),
> `AdmissibleViews` e `IsSameAuthority` consumen fundacion compartida con I-55 (X-2, X-3 y X-8, acordadas en la revision
> tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de
> sus Proposals V3 y V4 pendientes de revision formal y reconciliacion): decodificacion sintactica, disponibilidad sobre
> el sistema resuelto, descriptor de origen y tramo del eje y comparador authored en el contrato del kind. `Assess`,
> `Reflect`, `μ_k`, `Expone` y `F` son politica de I-52. La forma de API de este contrato, en lo que toca a esas
> autoridades, no la congela I-52 sola (§14.1, §15.4).
...
- **Si I-49 integra antes** (hoy: V6 y Amendments A1 y A2 congelados, ADR-0041 aceptado en lugar de ADR-0040, que ya
  reemplazaba a ADR-0038; G5 sintactico `892a601` (antes `977f873`) y G6 `988ab9c` (antes `5f969cc`) con el nucleo
  semantico —enlace y evaluacion de un arbol— y la correccion G6-C2 `75f1862` del cualificador de clave exacta, en su
  rama, sin integrar, sin dominio aplicado y sin `PlanReadSet`; `[V9-D12]`, `[V9-D21]`, `[V10-D14]`, `[V11-D11]`,
  `[V12-D12]`): el dominio que su descriptor declara **como dato** y aplica al valor efectivo (I-49 V6 P24.5: `>= 0` en
  las dos propiedades Selectivas) es la cota `b`. V-DEP la consume como dato; I-52 no la reimplementa ni la infiere. Se
  re-mide en la reconciliacion (§15.3). Un dominio `>= 0` **no** convierte en universal la evidencia visual-geometrica
  de DEP-09/DEP-10a..c (sigue siendo un continuo de estados).
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V = 1 ⇒ W = max(70, 76 + 3)  = 79 < 80: la fila de N1 se apila desde la izquierda en el original y en la copia.
    V16: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V = 4 ⇒ W = 92 ≥ 88;  V = 1 ⇒ W = 83 < 88: la frontal del fondo 1 desborda.
    V16: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V16 ⇒ **STOP → Proposal V17**.
...
| la misma guarda, evolucionada por I-49 G5 | `c4880af` (tras los rebases de I-49, `fb9f630`, `977f873` y `892a601`, mismo contenido), `:183-203` | `ExpressionParser` pasa a permitirse solo en Application; `FormulaParser`, `DependencyGraph`, `RackPropertyReference` y `rackProperty` siguen prohibidos en las tres capas | al reconciliar con I-49 rige la version vigente en ese SHA |
| `LengthUnitsAuthorityGuardTests` (nueva) y `ExpressionCoreGuardTests`/`ExpressionParserGuardTests` extendidas | I-49 G6 `988ab9c` (antes `5f969cc`, mismo patch-id) y correccion G6-C2 `75f1862` (guardas del nucleo ajustadas) | ningun archivo de producto fuera de `RackCad.Application.Units` (y del alias `StructuralSectionUnits`) declara literales o conversiones de unidades de longitud (`25.4`, `InchesPerFoot`, sufijos `[mm]`); el nucleo y las unidades no nombran otras capas ni tipos dimensionales | I-52 no convierte unidades ni nombra el nucleo; al reconciliar con I-49 rige la version vigente (CT-46, G-M18, `[V9-D21]`, `[V10-D14]`, `[V12-D12]`) |
...
| `EL_DOMINIO_ENTERO_TAMPOCO_CONOCE_EL_NUCLEO_DE_EXPRESIONES` y `ExpressionCoreGuardTests` | `c4880af` (`fb9f630`, `977f873` y `892a601` tras los rebases) | Domain no nombra el nucleo de expresiones; el nucleo no nombra otras capas ni motores externos | I-52 no toca Domain ni `RackCad.Application.Expressions` |
...
## 7. Matriz de representabilidad V16
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V16)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V17**. T-M47 lo hace ejecutable desde
G5.
...
| X-04 | Colocacion y presentacion de la referencia | `P' = G·P·F`; presentacion exterior de la fuente conservada (ST-13); piezas internas regeneradas como Actualizar con el read-set transitivo del contexto de materializacion (§8.7, `[V11-D06]`) | R | CODE_SUPPORTED | ALLOW | §3; §4.1; §8.7 |
...
| X-17a | Pieza con cambio de mano sin equivalencia visual-geometrica verificada (`GeometrySymmetryResult`): sonda no disponible, sin paridad con el materializador, sin punto de observacion soportado o sin contexto reproducible de la base auxiliar (`[V8-D07]`); BTR requeridos ausentes en la base auxiliar tras el intento de clonado (`[V9-D07]`); simetria no verificada (incluida la canonicalizacion conservadora); ruta anidada estable (`[V8-D11]`) o correspondencia ambigua; eje oblicuo; productor que aplica el contexto de materializacion de forma asimetrica entre las dos generaciones o consumo del contexto no determinable (`[V9-D05]`, `[V10-D05]`); o base auxiliar que no puede simular el contexto de evaluacion destino de la pieza (`[V10-D02]`, `[V10-D09]`), incluida una base por pieza que no reproduce exactamente la resolucion global (`[V11-D16]`) | la equivalencia no esta demostrada | UK | CODE_SUPPORTED | FAIL_CLOSED | §8.7; §11.3 |
| X-17b | Geometria evaluada con una clase o variante fuera de la politica cerrada (`[V8-D01]`..`[V8-D05]`, `[V9-D08]`): subclase de una clase soportada (`MInsertBlock`), definicion anidada anotativa, XCLIP anidado, referencia anidada con atributos o con definicion XREF, overlay o dependiente, grosor, normal, elevacion o ancho fuera de la tabla, tipo de linea **efectivo** complejo, no determinable o no caracterizado en el dibujo destino, cualquiera que sea su fuente (`Explicit`, `ByLayer`, capa `0` heredada, presentacion de creacion de la referencia de pieza o presentacion exterior de ST-13; `[V11-D01]`), HATCH de degradado o patron, SOLID/TRACE/HATCH no canonizable, propiedad, entrada o estado sin clasificar o `UNSUPPORTED` en el inventario cerrado, o un ciclo de anidados | fuera de la politica del primer corte | UK | CODE_SUPPORTED | FAIL_CLOSED | §11.3 puntos 6.1, 6.2, 8 y 14 |
...
| X-22 | Fuentes visuales simbolicas tipadas, `SourceKind`, `PolylineGeneration`, `LinetypePatternContext` o `VisibilityPath` distintas entre homologas, mecanismo de visibilidad no modelado, orden visual relativo no demostrado dentro de la pieza, u orden visual de plan no conservado entre instancias de pieza cuyo orden relativo cambia y cuyas huellas se superponen de forma material, o huella visual `UNKNOWN` (incluida la de una presentacion resuelta no elegible, `[V11-D01]`) o superposicion no clasificable en un par invertido, tambien al refrescar las huellas tras `EnsureForPlan` (`[V8-D12]`, `[V9-D03]`, `[V10-D03]`); la igualdad de fuentes simbolicas (pregunta A) nunca sustituye a la elegibilidad de la presentacion resuelta (pregunta B, X-17b) | la apariencia de la copia podria no ser el espejo del original en algun estado | UK | CODE_SUPPORTED | FAIL_CLOSED | §11.3 puntos 5 y 6.2..6.6 |
...
| PRE-14 | Sonda no disponible, fallida, sin paridad con el materializador, sin punto de observacion soportado o sin contexto reproducible de la base auxiliar para una pieza con cambio de mano; BTR requeridos ausentes en la base auxiliar tras el intento de clonado (el retorno de `EnsureBlocks` es diagnostico); postcondicion del dibujo incumplida o contenido de la cache de biblioteca alterado (§11.3 puntos 4 y 14); huella visual de una instancia enumerada no disponible, contexto de evaluacion destino no reproducible o elegibilidad de la presentacion resuelta no determinable (§11.3 puntos 4, 5 y 6.2, `[V10-D02]`, `[V11-D01]`) | FAIL_CLOSED (E6, X-17a o X-22) | P7 | §11.3 |
...
| PRE-16 | Orden visual en Model Space no demostrable para alguna copia frente a todo objeto posterior a su fuente, huella visual o contexto de orden no disponibles, `XLINE`/`RAY` posterior sin interseccion analitica u ordenes de pantalla y de trazado contradictorios entre fuentes admitidas con copias superpuestas, u objeto posterior sin huella de orden soportada por la `ModelSpaceFootprintPolicy`, con ocupacion latente no acotable, XREF cargado bajo demanda con contenido que puede no estar en memoria, mecanismo añadido sin metodo latente conservador o con representacion dependiente del entorno sin huella conservadora (ST-19, `[V9-D01]`, `[V9-D04]`, `[V10-D06]`, `[V10-D07]`, `[V11-D02]`..`[V11-D04]`, `[V13-D03]`, `[V13-D05]`) | FAIL_CLOSED (E10) | PREPARE: rechazo anticipado antes de `EnsureForPlan` y decision definitiva despues (PRE-18) | §4.1; §9.1; CT-49 |
| PRE-17 | Observacion consumida del `MaterializationContextReadSet`, con su cierre transitivo (registros de estilo de cota y de texto, bloques de flecha, tipos de linea y metricas de fuente consumidos), distinta entre SNAPSHOT, PREPARE y MUTATE, o consumo que G3 no pudo determinar (§8.7, `[V9-D05]`, `[V10-D05]`, `[V11-D06]`); un estado no consumido no invalida | FAIL_CLOSED (E10/E11; `UNKNOWN` si el consumo no se determina) | PREPARE/MUTATE | §8.7; §9.2 |
| PRE-18 | Tras `EnsureForPlan`, el `FootprintInputFingerprint` de alguna instancia cuya huella visual se uso (con o sin cambio de mano, de una vista seleccionada o de una vista admisible afectada por simbolos importados o colisiones de nombres) difiere y, recalculadas la huella, el orden visual de plan y ST-19, no se demuestran; la `ModelSpaceOrderTailSnapshot` enumerada de nuevo en R6 o al inicio de MUTATE difiere (objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada; un objeto anterior que sigue siendo anterior no invalida); o esa postcondicion cambia al inicio de MUTATE (`[V10-D03]`, `[V11-D05]`) | FAIL_CLOSED (E10, con X-22 si falla el orden de plan; en MUTATE, excepcion y rollback) | PREPARE tras `EnsureForPlan`; inicio de MUTATE | §9.1; §9.2; §11.3 punto 5 |
| PRE-19 | `DatabaseVisibilityCompleteness` no demostrable (ST-20, `[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`): **prerrequisito global** incumplido o no demostrable —dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente) o `EditingSessionPolicy` sin PASS en su `EditingSessionAuthoritySet`: host o version no caracterizados, lectura fallida o no clasificable de un detector requerido, detector `BLOCKED` activo —edicion de referencia en curso (`REFEDITNAME` no vacio), transaccion larga, Editor de bloques abierto (`BLOCKEDITOR = 1`), ventana de prueba de bloques abierta (`BLOCKTESTWINDOW`), edicion de la fuente de una matriz asociativa (`ARRAYEDITSTATE`)— o autoridad generica que reporta una sesion de edicion activa, segun el conjunto cerrado de CT-49—; o **completitud relativa a la fuente** no demostrable (cola posterior a una fuente o contenido relevante); o cualquiera de esos estados cambiado entre P0, R2, R6 y el inicio de MUTATE; sin PARTIALOAD, REFCLOSE, BCLOSE, ARRAYCLOSE, cargas forzadas ni cambios de capas, variables o sesion | FAIL_CLOSED antes de ST-19 definitivo (E12 en P0 para los prerrequisitos globales; E10 en R2 o R6; E11 en MUTATE) | P0; PREPARE (R2 y R6); inicio de MUTATE | §4.1 ST-20; §9.1; CT-49 |
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** La autoridad «sistema resuelto →
> plan de vista», el descriptor de origen y tramo del eje de cada vista (el tramo que devuelve `ViewPlanResult`, X-8) y
> la resolucion del diseño que hoy precede al plan son fundacion compartida con I-55 (X-2 y X-8, acordadas en la
> revision tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las
> diferencias de sus Proposals V3 y V4 pendientes de revision formal y reconciliacion; ADR-0042 decisiones 1 y 8,
> propuesto, que parte del sistema ya resuelto). X-2 acuerda `Build` = `Resolve` + `Plan`, con `Plan` para todas las
> direcciones y la resolucion compartida con los handlers del BOM, como entrada vinculante de la reconciliacion. La
> semantica que I-52 necesita puede acordarse; propiedad, namespace, ubicacion y forma definitiva de la autoridad no los
> congela I-52 sola (§14.1, §15.4).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** El primitivo «definicion + sobre +
> referencia» en la transaccion del llamador es fundacion compartida con I-55 (X-4, acordada en la revision tecnica
> adversarial de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus
> Proposals V3 y V4 pendientes de revision formal y reconciliacion, con custodio I-52: Z y presentacion explicitas y
> comprobacion de faltantes por instancia); ST-13, ST-19, ST-20, PRE-17, PRE-18, PRE-19, el
> `MaterializationContextReadSet` y la equivalencia visual siguen siendo politica de I-52 fuera del primitivo.
> Propiedad, ubicacion, forma de API y primitivo neutral definitivo no los congela I-52 sola (§14.1, §15.4).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** El valor de colocacion y su
> composicion sobre `Transform2D` son fundacion compartida con I-55 (X-7, acordada en la revision tecnica adversarial de
> G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y V4
> pendientes de revision formal y reconciliacion): la rigidez es invariante de I-55 y la reflexion es politica de I-52,
> y el valor no lleva `Origin` ni Z. Propiedad, ubicacion y forma de API definitivas se fijan en la reconciliacion
> (§3.3, §14.1, §15.4).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** El codec de `View`/`Section` y la
> taxonomia de tipo de vista son fundacion compartida con I-55 (X-2, acordada en la revision tecnica adversarial de G2C
> @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las diferencias de sus Proposals V3 y V4
> pendientes de revision formal y reconciliacion, con custodio I-55). La entrada acordada situa el nucleo de
> decodificacion fuera de `Mirror/` y deja en `Mirror/` `A_k` y la exposicion; se adopta o se vuelve a acordar en la
> reconciliacion. La semantica que I-52 necesita puede acordarse; propiedad, namespace, ubicacion y forma de API del
> codec no los congela I-52 sola (§3.7, §14.1, §15.4).

La decodificacion `SectionRaw → semantica → canonica`, la exposicion y el conjunto `A_k` (§3.7, §3.8) viven en
Application, fuera del comando. **Su ubicacion no la fija V16**: puede quedar junto al contrato de cada kind o en un
codec neutral compartido con I-55, y se decide en la reconciliacion previa al freeze; `Mirror/` (§20.1) solo aloja lo
propio del espejo y la politica de fallo cerrado que I-52 aplica a la disposicion del codec (`[V12-D11]`). El comando no
contiene `switch` de vistas. Las lecturas actuales del Plugin (`RackSelectivoCommands.cs:126-177`;
`RackDinamicoCommands.cs:209-239`, `:392-393`; `RackPushBackCommands.cs:218-258`, `:427-450`;
`RackCantileverCommands.cs:295-314`, `:501-540`; `RackCabeceraCommands.cs:239-240`;
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** El comparador authored por kind y
> las lineas base de C-2 (C2-2a, C2-2b, C2-4, G-M9) son fundacion compartida con I-55 (X-3 y X-6, acordadas en la
> revision tecnica adversarial de G2C @ `d091eeb`, que la Proposal V3 de I-55 reclasifico como no formal, con las
> diferencias de sus Proposals V3 y V4 pendientes de revision formal y reconciliacion; X-6 pide un `Plan` compartido de
> la planta Cantilever con `PlantaVisibility` y un fixture de C-2 con brazos y tensores visibles). Si ADR-0042 se acepta
> e I-55 re-enruta Insertar antes que I-52, los fixtures de C2-2b cumplen sus precondiciones nuevas (propiedades
> personalizadas acotadas al `RackId` y redibujo previo sin fallos) y la segunda iniciativa en integrar re-establece C-2
> (`[V12-D17]`, `[V13-D09]`). Propiedad y ubicacion del comparador no los congela I-52 sola (§5.4, §14.1, §15.4).
...
### 8.7 Contexto de materializacion (`[V9-D05]`, `[V10-D04]`, `[V10-D05]`, `[V10-D12]`, `[V11-D06]`, `[V11-D08]`)

Marcas de V12 a V16 en esta seccion: `[V12-D07]` (regla de entorno tambien en la copia), `[V12-D09]` (censo global de
G-M24), `[V13-D07]` (censo por familias de simbolos de API), `[V14-D05]` (familias ampliadas), `[V15-D03]` (escritura o
mutacion de objetos o contexto existentes), `[V16-D05]` (mutadores internos de AutoCAD) y `[V16-D06]` (registros de
overrules o reactores).
...
AutoCAD 2025 y que observaciones consume cada productor.** Hechos del codigo (`1b091be`, sin cambios en `ba497f1` ni en
`dad4e77`):
...
- las anotaciones son `DBText` con capa `RACKCAD_ANOTACIONES`, altura y alineacion explicitas, sin estilo de texto,
  color, tipo de linea, grosor ni transparencia asignados, y `AdjustAlignment` recalcula su posicion con las metricas
  de la fuente del estilo resuelto (`:266-280`);
- las cotas son `RotatedDimension` en la capa `RACKCAD_COTAS`, con el estilo con nombre del plan si existe en el dibujo
  (la busqueda `table.Has(styleName)`, `:385`, es en si una observacion) o, si no, con el **`DIMSTYLE` actual** del
  dibujo y sobreescrituras de tamaño solo en ese caso (`:338-392`, `:391`); el resto del registro de estilo —estilo de
  texto, bloques de flecha, tipos de linea, relleno, unidades y sufijos— sigue gobernando la geometria que
  `RecomputeDimensionBlock` construye (`:375`);
...
- la `AnnotationScale` del rack es un dato **authored** del diseño que fija la altura del texto y de las cotas del plan
  (`SelectiveAnnotations.cs:15`, `:21`; `DynamicViewDecorations.cs:53`), y ningun archivo de `src/` lee `CANNOSCALE`.
...
MaterializationContext (nombre no contractual; no es schema persistido)
  observaciones candidatas: CLAYER · CECOLOR · CELTYPE · CELTSCALE · CELWEIGHT · CETRANSPARENCY · CPLOTSTYLE ·
                            PSTYLEMODE · registro efectivo del estilo de cota (el con nombre del plan si existe
                            en el dibujo, con el resultado de esa busqueda, o DIMSTYLE actual) · registro
                            efectivo del estilo de texto del DBText · registros de capa RACKCAD_ANOTACIONES y
                            RACKCAD_COTAS · registros de las capas de rol del Cantilever · CANNOSCALE y el
                            contexto de anotacion del dibujo SOLO si CT-50 demuestra su consumo · todo otro estado
                            que descubran CT-36, CT-49 o CT-50; casos obligatorios de CT-50 ([V12-D07]): estilo
                            de texto anotativo, estilo de cota anotativo, DIMSCALE = 0, estilo hijo de la familia
                            (<estilo>$0), fuente ausente y FONTALT o sustitucion de fuentes
  la AnnotationScale authored del rack es intencion del plan: no es una observacion del dibujo ([V11-D06])
...
MaterializationContextReadSet (nombre ilustrativo)
  ...
  en la operacion (piezas y colocaciones de HeaderRun, DBText, cotas, curvas Cantilever), con CIERRE TRANSITIVO:
    cota    ⇒ registro efectivo del estilo de cota · estilo de texto que use · bloques de flecha que use · tipos de
               linea, relleno y demas propiedades que afecten a su geometria o a su huella · estilo hijo de la
               familia si AutoCAD lo aplica · contexto de la ventana si DIMSCALE = 0 · CANNOSCALE si el estilo es
               anotativo y CT-50 demuestra su consumo · lo que CT-50 demuestre material
    DBText  ⇒ registro efectivo del estilo de texto · metricas de fuente o tipografia que su huella necesite ·
               fuente disponible o sustituida (FONTALT) · CANNOSCALE si el estilo es anotativo y CT-50 lo demuestra
    registros de presentacion resueltos que decidan la elegibilidad de una pieza (§11.3 punto 6.2, [V11-D01])
    ...
    CANNOSCALE                                    ⇒ no, salvo que CT-50 demuestre su consumo
  consumo, directo o transitivo, que G3 no puede determinar ⇒ UNKNOWN ⇒ FAIL_CLOSED
...
Regla
  ...
  huella de la copia (DBText, cotas, anotaciones, piezas o anidados) dependiente del entorno sin cota conservadora
    bajo todos los estados admitidos ⇒ UNKNOWN (E10 en ST-19, X-22 en el orden de plan) o limitacion de O-1 ([V12-D07])
  ...
  G-M24 protege a los productores con dos lados ([V11-D08], [V12-D09]):
    A) productores y consumidores de contexto declarados: si uno empieza o deja de asignar una propiedad visual,
       cambia el uso de capas, estilos o valores por defecto, crea una clase de entidad relevante nueva o cambia la
       resolucion de estilos ⇒ RED
    B) ProducerSiteCensus por familias de simbolos de API ([V13-D07], [V14-D05], [V15-D03]), no solo por new
       sintactico o AppendEntity, en los sitios del Plugin y del camino de materializacion:
         construccion y alta de entidades: subclases relevantes de Entity · AppendEntity ·
           AddNewlyCreatedDBObject · Activator.CreateInstance y construccion por reflexion
         importacion y clonado: ReadDwgFile · Database.Insert · Wblock · WblockCloneObjects · DeepCloneObjects ·
           Clone · GetTransformedCopy · Explode · ExplodeToOwnerSpace
         tablas de simbolos: BlockTable.Add · LayerTable.Add · LinetypeTable.Add · TextStyleTable.Add ·
           DimStyleTable.Add · SymbolTable.Add y patrones de insercion de registros
         cabeceras y valores por defecto de Database: Dimstyle · Clayer · Cecolor · Celtype · Textstyle ·
           Cannoscale · GetDimstyleData · propiedades visuales o por defecto equivalentes · GetSystemVariable ·
           SetDatabaseDefaults
         evaluacion grafica: RecomputeDimensionBlock · EvaluateHatch · GenerateLayout · RecordGraphicsModified y
           TransformBy donde sean relevantes
         atributos: AttributeCollection.AppendAttribute · SetAttributeFromBlock
         comando y COM: Editor.Command · SendStringToExecute · ModelSpace.Add* · InsertBlock · envoltorios
           equivalentes
         escritura o mutacion de objeto o contexto existente ([V15-D03]):
           apertura para escritura: Transaction.GetObject(..., ForWrite) · ObjectId abierto para escritura ·
             UpgradeOpen · OpenCloseTransaction · envoltorios equivalentes
           presentacion de entidades: Layer/LayerId · Color · Linetype · LinetypeScale · LineWeight ·
             Transparency · Visible · PlotStyleName · SetPropertiesFrom · TransformBy donde sea material
           registros de simbolo existentes: setters de LayerTableRecord · LinetypeTableRecord ·
             TextStyleTableRecord · DimStyleTableRecord · Database.SetDimstyleData · equivalentes
           estado global: SetSystemVariable · setters de Clayer · Cecolor · Celtype · Textstyle · Dimstyle ·
             equivalentes de Database
         mutador interno de AutoCAD ([V16-D05]): BlockTableRecord.SynchronizeAttributes ·
           LayerStateManager.RestoreLayerState · LayerStateManager.Restore* · Database.ReloadXrefs ·
           Database.ResolveXrefs · Database.BindXrefs · AssocManager.EvaluateTopLevelNetwork ·
           AssocManager.Evaluate* · Database.Audit · BlockReference.ResetBlock · BlockReference.ConvertToStaticBlock ·
           equivalentes que se descubran
         registro de overrule o reactor ([V16-D06]): DrawableOverrule · ObjectOverrule · TransformOverrule · demas
           subclases de Overrule · Overrule.AddOverrule · Overrule.RemoveOverrule · eventos de Database
           (ObjectModified · ObjectOpenedForModify · ObjectErased · ObjectAppended y afines) · suscripciones de
           Document o Editor a comandos o eventos que puedan causar escrituras diferidas
         delegacion en helpers o factorias
       cada call-site relevante DECLARED o EXCLUDED con motivo; call-site nuevo sin clasificar ⇒ RED; no depende
       de los nombres actuales de archivos o clases ni de hashes
    RED hasta actualizar CT-50, este read-set, el FootprintInputFingerprint y las pruebas (T-M75)
```
...
- Las huellas visuales de anotaciones y cotas consumen los registros de estilo de cota y de texto, con su cierre
  transitivo, las metricas de fuente y los registros de capa cuando CT-50 lo demuestre, y `CANNOSCALE` solo si CT-50
  demuestra su consumo (§11.3 punto 5); esas observaciones entran en su `FootprintInputFingerprint` (§9.2, PRE-18). Si
  esas huellas dependen del entorno (estilos anotativos, `DIMSCALE = 0`, estilo hijo, fuentes ausentes o sustituidas)
  sin cota conservadora bajo todos los estados admitidos, son `UNKNOWN` (`[V12-D07]`).
- Consecuencia para O-1 (L-36): las piezas internas de la copia se regeneran como Actualizar; no se copian fisicamente
  ...
  pieza (§1.4).
- No crea obligacion de tocar los drawers existentes (§20.3); G-M8 y G-M11 no cambian, y G-M24 vigila, sin modificarlos,
  a los productores declarados y, con su censo por familias de simbolos de API, todo call-site productor, consumidor o
  de escritura nuevo del camino de materializacion, clonados, copias transformadas, explosiones, construccion por
  reflexion, aperturas para escritura con mutacion de presentacion, mutaciones de registros de simbolo existentes,
  escrituras de estado global, mutadores internos de AutoCAD y registros de overrules o reactores incluidos
  (`[V12-D09]`, `[V13-D07]`, `[V15-D03]`, `[V16-D05]`, `[V16-D06]`).
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           ...
           prerrequisitos globales de ST-20 (apertura parcial y lectura del EditingSessionAuthoritySet) y estado de
           carga de los XREF ([V13-D01]..[V13-D03], [V14-D01], [V15-D01], [V16-D01])

PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  P0  prerrequisitos globales de ST-20             apertura parcial (Database.IsPartiallyOpened o el mecanismo
                                                   de CT-49) o EditingSessionPolicy sin PASS en el
                                                   EditingSessionAuthoritySet: host no caracterizado, lectura
                                                   fallida o no clasificable de un detector requerido, detector
                                                   BLOCKED activo (REFEDITNAME, transaccion larga, BLOCKEDITOR,
                                                   BLOCKTESTWINDOW, ARRAYEDITSTATE) o autoridad generica con
                                                   sesion activa ⇒ E12 con mensaje por causa, antes de C1 y de la
                                                   linea; la ejecucion no descubre modos desconocidos; no promete
                                                   la completitud de la cola por fuente; lecturas de solo
                                                   lectura, sin PARTIALOAD, REFCLOSE, BCLOSE, ARRAYCLOSE ni
                                                   cambios ([V13-D01], [V13-D02], [V14-D01], [V14-D04], [V15-D01],
                                                   [V15-D02], [V16-D01]..[V16-D04])
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R2  enumeracion TEMPRANA de la cola posterior a cada fuente y completitud relativa a la fuente temprana (ST-20;
      fallo ⇒ E10, [V14-D04]) · huellas visuales en el DestinationEvaluationContext de las instancias de esos planes,
      anotaciones y cotas incluidas, con la elegibilidad de su presentacion resuelta ([V11-D01]) y la regla de entorno
      tambien en la copia ([V12-D07]), y huellas de orden de los objetos de Model Space posteriores a cada fuente
      segun la ModelSpaceFootprintPolicy de su tipo exacto y de su ContentAvailability y sus VisibilityMechanism,
      latentes frente a visibilidad, carga, carga bajo demanda de XREF, recortes, estados dinamicos, campos, atributos
      invisibles, aislamiento, proxies y datos externos de tabla, siempre sobre una enumeracion completa y en el orden
      logico de etapas ([V11-D02]..[V11-D04], [V12-D01]..[V12-D06], [V13-D03]..[V13-D05]; §4.1, §11.3 punto 5)
  R3  ST-19 temprano (opcional): rechazo anticipado frente a todo objeto posterior, antes de importar; nunca aprueba
      ([V14-D04])
  ...
  R6  enumerar de NUEVO todos los objetos de Model Space posteriores a cada fuente y capturar la
      ModelSpaceOrderTailSnapshot (identidades ordenadas, clases, contexto efectivo de orden) · re-verificar los
      prerrequisitos globales, incluido el EditingSessionAuthoritySet completo (lectura fallida ⇒ fallo), y decidir
      la completitud relativa a la fuente
      DEFINITIVA (ST-20; fallo ⇒ E10 antes de ST-19) · orden visual de plan (§11.3 punto 6.6) y ST-19 re-evaluados
      con las huellas refrescadas y con esa cola: decision DEFINITIVA (PRE-18, PRE-19, [V11-D05], [V13-D01],
      [V14-D04], [V15-D01])
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           ...
           PRE-17, PRE-18) · prerrequisitos globales (EditingSessionAuthoritySet completo incluido) y completitud
           relativa a la
           fuente de ST-20 re-verificados (PRE-19, [V14-D04], [V15-D01]) · enumeracion de NUEVO de la cola posterior
           a cada fuente frente a la
           ModelSpaceOrderTailSnapshot de R6: objeto añadido, eliminado, reordenado, movido
           de antes a despues de la fuente o con identidad o clase cambiada ⇒ excepcion y rollback ([V11-D05]) ·
           todas las definiciones nuevas
           (MissingInstances ⇒ excepcion)
...
en el orden efectivo, entran en la huella de la referencia (§9.2) y MUTATE los re-verifica. Si CT-49 encuentra contextos
de pantalla y de trazado con ordenes efectivos distintos, «posterior» significa posterior en cualquiera de ellos, y dos
fuentes admitidas con orden relativo distinto entre esos contextos cuyas copias se superponen de forma material, o no
clasificable, ⇒ E10. Los prerrequisitos globales de ST-20, incluida la politica cerrada de sesiones de edicion, se
comprueban en P0 y se re-verifican **siempre** en R6 y al inicio de MUTATE; la completitud relativa a la fuente se
evalua de forma temprana en R2 y de forma definitiva en R6, antes de ST-19 definitivo, y R3 solo puede rechazar. Con un
prerrequisito global incumplido o con una cola no demostrablemente completa, ST-19 no toma ninguna decision que permita
mutar (`[V13-D01]`, `[V13-D02]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`).

### 9.2 Huella de la referencia, read-sets, `FootprintInputFingerprint` y cola de Model Space
...
y `TraceFingerprint` es la trazabilidad de la evidencia de simetria (§11.3 punto 10). La `ModelSpaceOrderTailSnapshot`
es la cola ordenada de los objetos de Model Space posteriores a cada fuente (identidades ordenadas, clases y contexto
efectivo de orden); la `ModelSpaceFootprintPolicy` clasifica por tipo exacto la huella de orden de esos objetos (§11.3
punto 5), y la `ResolvedPresentationEligibility` decide si la presentacion resuelta de una primitiva esta soportada
(§11.3 punto 6.2). `ContentAvailability` y `VisibilityMechanism` son los ejes por mecanismo de esa politica, y
`ProducerSiteCensus` es el lado B de G-M24, por familias de simbolos de API (§8.7). `DatabaseVisibilityCompleteness`
(ST-20) reune los prerrequisitos globales de P0 y la completitud relativa a la fuente de R2 y R6, que preceden a ST-19
definitivo. Ninguno sustituye a otro (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`, `[V13-D16]`, `[V14-D15]`).
...
| orden efectivo en Model Space de las fuentes y su contexto de pantalla y de trazado (`SortentsTable` y las variables que fije CT-49) y, desde R6, la **`ModelSpaceOrderTailSnapshot`**: enumeracion completa y de nuevo de **todos** los objetos posteriores a cada fuente, con identidades ordenadas, clases y contexto efectivo de orden, y la huella de orden de cada uno con sus observaciones consumidas y el estado de sus mecanismos (`ContentAvailability`, `VisibilityMechanism`), incluido el estado de carga bajo demanda de los XREF usados (`XLOADCTL`, `INDEXCTL` y lo que caracterice CT-49, `[V13-D03]`); los prerrequisitos globales (apertura parcial, REFEDIT o transaccion larga, Editor de bloques) y la completitud relativa a la fuente (ST-20, `[V13-D01]`, `[V14-D01]`, `[V14-D04]`); al inicio de MUTATE se vuelve a enumerar, sin limitarse a los objetos capturados antes | objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada, o huella de orden, estado de un mecanismo o estado de carga cambiados, o completitud ya no demostrable, invalida la demostracion del orden; un objeto anterior que sigue siendo anterior no invalida (ST-19, ST-20, `[V8-D25]`, `[V9-D01]`, `[V11-D05]`, `[V13-D01]`) |
| `MaterializationContextReadSet` (§8.7): solo las observaciones del contexto de materializacion que los productores y las huellas visuales realmente consumen, con su cierre transitivo (`[V11-D06]`) | la generacion de las dos mitades, las huellas y la materializacion usan las mismas observaciones; un estado no consumido no invalida (`[V9-D05]`, `[V10-D05]`, PRE-17) |
| `FootprintInputFingerprint` de **toda** instancia cuya huella visual se uso (desde P7; recalculado al releer el dibujo real tras `EnsureForPlan`): identidades de las definiciones reales, registros de simbolos resueltos usados (incluidos los que deciden la `ResolvedPresentationEligibility`, `[V11-D01]`), observaciones consumidas del contexto de materializacion con su cierre transitivo, estado dinamico efectivo, identidades de las dependencias anidadas, estados del entorno de los que dependa una huella `SUPPORTED`, tambien en la copia (`[V11-D04]`, `[V12-D07]`), estado de disponibilidad y de mecanismo de los objetos posteriores y de su contenido, incluidos la carga bajo demanda de los XREF y los mecanismos añadidos (`[V12-D01]`..`[V12-D06]`, `[V13-D03]`, `[V13-D05]`), y cualquier otra entrada que altere la huella | la huella visual que decide ST-19 y el orden de plan describe lo que MUTATE materializa (`[V10-D03]`, PRE-18) |
...
| RS-3 Dependencias transitivas | **solo si I-49 integra antes**: las observaciones de su `PlanReadSet` (resultado de cada simbolo realmente usado, incluidas las dependencias de expresiones) segun la **autoridad final** de I-49. **Hoy: PENDING I-49 FINAL AUTHORITY** (`[V14-D07]`, `[V15-D06]`): el Amendment A3 (`f6f0991`), revisado por A3-R1 (`c8cfee2`), cambia la semantica futura de las observaciones del `PlanReadSet` de D19 —diagnosticos de un simbolo no evaluado, su orden, la cadena representativa de cada `DependencyFailed` y `RootCauses` por SCC; segun A3-R1, D19 se lee con M3, M4 y M10(a) y su texto aceptado sigue siendo cierto— y el dato `Upstream` de D20, que A3-R1 cambia directamente, con un ADR de reemplazo pendiente; I-52 adopta esas observaciones con la autoridad de I-49, sin calculo propio. Solo como hecho historico, D19 de ADR-0040 = D19 de ADR-0041 antes de A3; ADR-0041 D19 **no** es la autoridad de RS-3. Mientras la autoridad final acordada, aceptada y congelada no este disponible, **I-52 FREEZE = BLOCKED** (§14.1); si I-49 se cancela o abandona antes, no hay desbloqueo automatico ni eliminacion silenciosa de RS-3 (R-55, `[V15-D07]`) | I-49 V6 + A1 + A2 + A3 revisado, ADR de reemplazo de ADR-0041 y Consensus Freeze nuevo, releidos antes del freeze de I-52; hoy vigente ADR-0041 (aceptado @ `2e8090a`, que reemplaza a ADR-0040; `[V11-D11]`), no apto para congelar RS-3 |
...
| E10 | Prerrequisito global de ST-20 incumplido al re-verificarlo en R6, o completitud relativa a la fuente no demostrable en R2 (rechazo temprano) o en R6 (decision definitiva) (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`, `[V14-D04]`), orden visual en Model Space no demostrable frente a algun objeto posterior a una fuente, huella visual o contexto de orden no disponibles, `XLINE`/`RAY` posterior sin interseccion analitica u ordenes de pantalla y de trazado contradictorios con copias superpuestas (ST-19, como rechazo anticipado antes de importar o como decision definitiva despues), fallo de plan, bloque ausente tras importar, definicion real de una pieza con cambio de mano cambiada y ya no equivalente en geometria, fuentes visuales, visibilidad u orden visual (PRE-13), huella visual u orden de plan que ya no se demuestran al refrescar tras `EnsureForPlan` (PRE-18), huella de la referencia, read-set del registro u observacion consumida del `MaterializationContextReadSet` cambiados (PRE-10, PRE-17), objeto posterior sin huella de orden soportada, tambien lejos del destino (clase, componente, estado o mecanismo no caracterizado, XREF descargado o no resuelto, XREF cargado bajo demanda con contenido que puede no estar en memoria, recorte, visibilidad dinamica o campos sin cota, atributos invisibles, aislamiento, proxies o datos externos de tabla sin metodo latente conservador; `[V11-D02]`..`[V11-D04]`, `[V12-D01]`..`[V12-D06]`, `[V12-D10]`, `[V13-D03]`, `[V13-D05]`), huella de la copia dependiente del entorno sin cota conservadora (`[V12-D07]`) o cola de Model Space cambiada en R6 (`[V11-D05]`) | PREPARE | fin; **ninguna** definicion, referencia ni payload RackCad nuevo; lo importado por `EnsureForPlan` puede permanecer (§9.5) |
| E11 | Excepcion en MUTATE, incluidas la huella de la referencia, los read-sets, el `FootprintInputFingerprint`, la cola de Model Space posterior a una fuente o el estado de ST-20 cambiados respecto de la postcondicion de PREPARE (`[V11-D05]`, `[V13-D01]`) y `MissingInstances.Count > 0` | MUTATE | sin commit ⇒ rollback de **toda** la operacion semantica |
| E12 | Prerrequisito global de ST-20 incumplido o no demostrable en P0 (PRE-19; `[V13-D01]`, `[V13-D02]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`): dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente) o `EditingSessionPolicy` sin PASS en su `EditingSessionAuthoritySet`: host o version no caracterizados, lectura fallida o no clasificable de un detector requerido, detector `BLOCKED` activo (edicion de referencia en curso, transaccion larga, Editor de bloques, ventana de prueba de bloques o edicion de la fuente de una matriz asociativa) o autoridad generica con una sesion de edicion activa (`[V16-D01]`..`[V16-D04]`). E12 **no** cubre la completitud relativa a la fuente, que va en E10 | P0 | fin, sin linea y cero mutacion, con mensaje propio por causa («dibujo abierto parcialmente», «edicion de referencia en curso (REFEDIT): cierrela antes de reflejar», «Editor de bloques abierto: cierrelo antes de reflejar», «ventana de prueba de bloques abierta: cierrela antes de reflejar», «matriz asociativa en edicion (ARRAYEDIT): cierre la edicion antes de reflejar», «transaccion larga activa», «entorno de edicion no caracterizado», «no se pudo leer el estado de sesion de edicion»), nunca un aviso; sin PARTIALOAD, REFCLOSE, BCLOSE, ARRAYCLOSE, cargas forzadas ni cambios de capas, variables o sesion; si aparece despues de P0, E10 en R6 o E11 en MUTATE |
...
| PV-6 | Si I-49 integra antes (hoy: V6 y Amendments A1 y A2 congelados (`3f17c21`), ADR-0041 aceptado en lugar de ADR-0040; G5 sintactico `892a601`, antes `977f873` y `c4880af`, y G6 `988ab9c`, antes `5f969cc`, con enlace y evaluacion de un arbol en su rama, mas la correccion G6-C2 `75f1862`, que cierra G6; Amendment A3 (`f6f0991`) con revision exacta del Arquitecto = CHANGES REQUIRED y su revision A3-R1 (`c8cfee2`, solo docs) pendiente de re-revision exacta del Arquitecto y del Owner; REPLACEMENT ADR REQUIRED, sin numero, y Consensus Freeze nuevo pendientes; G7 y G8 BLOCKED; sin integrar y sin `PlanReadSet` ni dominio aplicado; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V15-D06]`): RACKMIRROR invoca la autoridad efectiva integrada, preserva `expression` como portador, adopta su `PlanReadSet` para el read-set segun la autoridad final (§9.2.1) y su dominio de consumidor como cota autoritativa de V-DEP (§6.5.5), y **no** reimplementa su evaluacion ni un segundo motor de expresiones (P24.5 y P24.8 de I-49 V6). **Autoridad final (`[V14-D07]`, `[V15-D06]`, `[V15-D07]`):** A3, revisado por A3-R1, cambia la semantica futura de las observaciones del `PlanReadSet` de D19 (leida con M3, M4 y M10(a)) y D20, y pide un ADR de reemplazo y un Consensus Freeze nuevo; ADR-0041 D19 no es la autoridad vigente de RS-3; antes del freeze de I-52 se relee la autoridad final acordada, aceptada y congelada de I-49 (V6, A1, A2, A3 revisado, ADR de reemplazo, freeze nuevo y estado de implementacion) y, si no esta disponible, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED; si I-49 se cancela o abandona antes, FREEZE BLOCKED hasta la decision del Coordinador de R-55 |
...
3. **Ruta primaria: STATE-EVALUATED VISUAL-GEOMETRIC EVIDENCE.** Para cada pieza de **cualquier** vista admisible del
   ...
   **Fuente de la definicion.** Si el bloque ya existe en el dibujo, la evidencia corresponde a **esa** definicion,
   ...
   Como `GeometrySymmetryResult` compara **fuentes** y no valores resueltos (punto 6), las otras propiedades de las
   capas homonimas del dibujo **no** invalidan la **igualdad de fuentes simbolicas** (pregunta A), pero **si** pueden
   invalidar la **elegibilidad de la variante soportada** (pregunta B, `ResolvedPresentationEligibility`, punto 6.2,
   `[V11-D01]`), que se resuelve en el dibujo destino, igual que la huella visual (punto 4, `[V10-D02]`). El resultado
   depende de la **biblioteca efectiva** (L-28).
4. **Sin mutacion del DWG del usuario ni de la cache.** La evaluacion se hace en solo lectura o clonando y evaluando en una
   ...
   **dibujo actual** si la definicion ya existe en el; si solo existe en la biblioteca, la autoridad inicial es la base
   ...
   dibujo. Si el contexto relevante no puede reproducirse o verificarse en la base auxiliar ⇒ `UNKNOWN` (X-17a). La
   elegibilidad de la presentacion resuelta (punto 6.2) **no** se decide con este contexto: se resuelve siempre en el
   `DestinationEvaluationContext` (`[V11-D01]`). Ninguna fuente se lee despues de un clonado que haya cambiado ese
   contexto (por ejemplo, entre modos de estilos de trazado distintos). CT-36 incluye un dibujo con estilos de trazado
   con nombre (STB) y otro dependiente del color (CTB). Las observaciones consumidas del contexto de materializacion
   (§8.7) se reproducen en la base auxiliar en lo que puedan afectar a la evaluacion.
   ...
   **Contexto de evaluacion destino (`DestinationEvaluationContext`, `[V10-D02]`, `[V10-D09]`).**
   ...
   DestinationEvaluationContext (nombre ilustrativo; no es schema persistido)
     ...
     resuelve tambien la ResolvedPresentationEligibility de cada primitiva (punto 6.2): LinetypeTableRecord efectivo
       y demas registros de presentacion por Explicit, ByLayer(Named), InheritLayer, InheritBlock, cadenas de capa 0,
       presentacion de creacion de la referencia de pieza (CT-50) y presentacion exterior de ST-13, con DRAWING WINS
       ([V11-D01])
   ```

   Si una base auxiliar se comparte entre piezas o entre rutas, rige la misma precedencia; una base por pieza solo es
   admisible si reproduce **exactamente** la misma resolucion global que la precedencia DRAWING FIRST con un unico
   `EnsureForPlan`: mismo estado del dibujo previo a importar, mismo conjunto de nombres importados y misma biblioteca
   efectiva identificada por SHA-256; si no puede demostrarse ⇒ `UNKNOWN` (`[V11-D16]`). La regla del contexto de la
   autoridad origen (parrafo anterior) queda limitada a `GeometrySymmetryResult` donde corresponda. CT-36 y T-M63
   incluyen una colision de nombre de bloque anidado y otra de nombre de simbolo (por ejemplo, una capa con el mismo
   nombre y otras propiedades en el dibujo y en la biblioteca), y los casos de tipo de linea efectivo complejo de
   `[V11-D13]`.
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   VisualFootprintResult {                 // nombres no contractuales; no es schema persistido
       ...
       Frame                               // siempre WCS (plano XY de §3.1): la geometria local ocupada completa,
                                           //   anchos reales de modelo incluidos, se transforma con la
                                           //   transformacion compuesta (anidados, instancia y P')
   ...
   - **Construccion (`[V11-D07]`, `[V12-D08]`).** La geometria local ocupada **completa** —incluidos los anchos reales
     de modelo: ancho geometrico, mascara, marco y limites de texto o de cota— se transforma con la transformacion
     compuesta completa: anidados con su transformacion completa, la instancia y `P'`, con la escala uniforme `s` de la
     fuente (ST-11). **Magnitud uniforme** significa `|sx| = |sy| = s`: los signos de reflexion se admiten, y
     `MirroredX` o `MirroredY` —las colocaciones de grupo y las referencias de pieza con `Scale3d(-1, 1, 1)` o
     `(±1, ±1, 1)`, `LateralHeaderDrawer.cs:62`, `:147` y `:313` @ `dad4e77`— no implican por si solos una escala no
     uniforme. Con magnitud uniforme, reflejada o no, cada ancho de modelo escala por la magnitud acumulada:
     `ancho' = |s| · ancho`; en una transformacion anidada, si la magnitud efectiva sigue soportada y uniforme, la
     geometria ocupada se transforma normalmente, y si no ⇒ `UNKNOWN`. Nunca se transforma una linea media para añadirle
     despues un ancho local sin transformar, ninguna anchura de presentacion entra en la huella y `LineWeight` sigue
     fuera.
   - **Nunca subestima la ocupacion de modelo.** Si no se puede garantizar que cubre la ocupacion (ancho geometrico,
     ...
     `Status = UNKNOWN`.
   - **Contexto (`[V10-D02]`, `[V11-D01]`).** Las huellas de las instancias del plan se evaluan en el
     `DestinationEvaluationContext` (punto 4): representan lo que MUTATE materializara en el dibujo destino, nunca lo
     que resolveria la biblioteca, y usan la elegibilidad de su presentacion resuelta: una primitiva no elegible (punto
     6.2) produce una huella `UNKNOWN`.
   - **Quien la produce (P7b y PREPARE).** Para toda instancia que enumere P7a, tenga o no cambio de mano: las piezas de
     ...
     la envolvente de texto y de cota que caracterice CT-49 bajo las observaciones que consumen (registros efectivos de
     estilo de cota y de texto con su cierre transitivo, metricas de fuente y registros de capa; `CANNOSCALE` solo si
     CT-50 demuestra su consumo; §8.7, `[V11-D06]`). En Model Space (ST-19), la huella de orden de un objeto existente
     la decide la `ModelSpaceFootprintPolicy` de su tipo exacto (bloque siguiente, §4.1).
   - **`Extents3d` (`[V11-D02]`).** Solo es admisible en las clases donde CT-49 demuestre que, tras añadir todas las
     ocupaciones de modelo necesarias de esa clase, es una cota conservadora; nunca se dilata por `LineWeight`; una
     extension no disponible, infinita o dudosa ⇒ `UNKNOWN`.
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`, `[V13-D01]`..`[V13-D05]`).** Autoridad
     conceptual distinta para los objetos arbitrarios del dibujo que ST-19 clasifica:

     ```text
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       precondicion ([V13-D01], ST-20): DatabaseVisibilityCompleteness demostrada; si no, no se clasifica ningun objeto
       orden logico obligatorio ([V13-D04]); si una etapa falla ⇒ UNKNOWN o fail-closed y nunca se salta a la huella:
         completitud de la base de datos → completitud de la enumeracion → tipo exacto en ejecucion → componentes →
         ContentAvailability → VisibilityMechanism → ModelSpaceFootprintPolicy → VisualFootprint
       clave: tipo EXACTO en tiempo de ejecucion del objeto de Model Space (nunca por herencia)
       ejes obligatorios ([V12-D01]): ContentAvailability (cargado completo · cargado bajo demanda con contenido que
         puede no estar en memoria · descargado · no resuelto · no disponible; [V13-D03]) y VisibilityMechanism
         (mecanismos por los que el contenido puede estar oculto, no cargado o recortado hoy y reaparecer despues sin
         cambiar el orden)
       resultado: SUPPORTED(metodo) | UNKNOWN por clase, componente y mecanismo; nunca un fallback optimista
       SUPPORTED exige cubrir la clase, sus componentes y todos sus mecanismos
       inventario minimo de clases (CT-49): Line · Arc · Circle · Ellipse · Spline · LWPOLYLINE · Solid · Trace ·
         Hatch · BlockReference con sus atributos · DBText · MText con fondo y columnas · Dimension con flechas,
         relleno y demas componentes · MLeader · Table · Point · Wipeout · raster y underlay · XLINE/RAY · nube de
         puntos · modelos de coordinacion (NWD, NWC y similares; [V14-D06]) · proxy o entidad custom (no es
         obligatorio soportarlas todas)
       inventario minimo de mecanismos (CT-49, [V12-D02], [V13-D05]); cada caso SUPPORTED(metodo conservador) o
       UNKNOWN:
         base de datos    apertura parcial (Database.IsPartiallyOpened o el mecanismo de CT-49; ST-20, [V13-D02])
         sesiones         EditingSessionPolicy: CHARACTERIZED_SAFE | BLOCKED por sesion, con su autoridad
                          observable EditingSessionAuthoritySet (conjunto cerrado de detectores de CT-49: variables
                          REFEDITNAME · BLOCKEDITOR · BLOCKTESTWINDOW · ARRAYEDITSTATE · demas del catalogo;
                          LongTransactionManager y working set; autoridades de comando y de sesion; cada señal
                          DETECTOR | IRRELEVANT | UNSUPPORTED); PASS solo con host caracterizado, todas las
                          lecturas con exito, detectores BLOCKED inactivos y autoridades genericas sin sesion;
                          lectura fallida o host no caracterizado ⇒ E12; la ejecucion no descubre modos; modo
                          relevante sin detector en CT-49 ⇒ STOP de caracterizacion; P0, re-verificado en R6 y
                          MUTATE ([V14-D01], [V15-D01], [V15-D02], [V16-D01]..[V16-D04])
         visibilidad      capa apagada · capa inutilizada · inutilizada en ventana · Entity.Visible ·
                          representaciones anotativas y ANNOALLVISIBLE
         XREF             cargado · descargado · no resuelto · overlay y disponibilidad de anidados descargados ·
                          carga bajo demanda: XLOADCTL · INDEXCTL · indice de capas · indice espacial · estados
                          relacionados que se descubran ([V13-D03])
         BlockReference   estados de visibilidad dinamica · AttributeCollection · disponibilidad de la definicion
                          referenciada
         atributos        AttributeReference.Invisible · ATTDISP ([V13-D05])
         aislamiento      HIDEOBJECTS · ISOLATEOBJECTS · OBJECTISOLATIONMODE ([V13-D05])
         proxy            PROXYSHOW · PROXYGRAPHICS · otro estado de visualizacion de proxies que se descubra
         overrules        Overrule.Overruling o la autoridad que caracterice CT-49; representacion dibujable que
                          puede estar afectada por un overrule o reactor activo de terceros sin equivalencia u
                          ocupacion conservadora demostradas ⇒ UNKNOWN; sin inspeccionar plugins de terceros
                          ([V16-D07])
         filtros          XCLIP · ACAD_FILTER · otros filtros espaciales soportados
         marcos           XCLIPFRAME · FRAME · IMAGEFRAME · WIPEOUTFRAME · PDFFRAME · DWFFRAME · DGNFRAME ·
                          POINTCLOUDCLIPFRAME · equivalentes; huella = contenido sin recortar ∪ marco ([V14-D06])
         raster, underlay limites de recorte · capas internas y controles de visualizacion · disponibilidad del
         y nube de puntos contenido o estado descargado
         campos           DBText · MText · atributos · celdas de tabla · otras clases con campos que se descubran
         tabla y datos    DataLink · formulas de tabla · contenido que depende de una actualizacion ([V13-D05])
         externos
       clase, componente, estado o mecanismo no caracterizado ⇒ huella UNKNOWN ⇒ E10 si el objeto es posterior (ST-19)
       nunca «contenido ausente hoy = huella vacia» si puede reaparecer sin cambiar el orden; nunca «no esta cargado» o
         «no aparece en la base de datos» = «no ocupa»
       frontera ([V13-D06], [V14-D02], [V14-D03], [V15-D04], [V15-D05], [V16-D08], [V16-D09]): estado dentro
         (visibilidad, carga, mismo contenido cargado o efectivo disponible de nuevo con cualquier ruta, carga bajo
         demanda, recortes, solo visibilidad dinamica, campos cuyo resultado cambia solo por estado, DataLink,
         proxies, aislamiento, anotacion y entorno);
         edicion fuera (ERASE/OOPS, geometria, redefinicion, texto, mover, parametro dinamico que cambia geometria,
         edicion del contenido referenciado por un campo, XREF editado o recargado o resuelto con contenido
         cambiado o distinto, fuente de imagen, underlay o nube de puntos reemplazada); EDIT CAUSE > FIELD
         REEVALUATION con las entradas de edicion fijas; XREF: contenido cargado actual y completitud, sin comparar
         identidades futuras; no resuelto ⇒ UNKNOWN
       Extents3d solo donde CT-49 demuestre que es cota conservadora tras añadir todas las ocupaciones de modelo
     ```

   - **Ocupacion latente por mecanismo (`[V11-D03]`, `[V12-D01]`).** Para ST-19, la huella de orden representa **todo**
     contenido que pueda volverse visible **sin cambiar** la relacion de orden de dibujo. No se excluye geometria porque
     hoy su capa este apagada, inutilizada o inutilizada en una ventana, porque `Entity.Visible = false` ni por el valor
     actual de `ANNOALLVISIBLE`, y tampoco porque hoy no este cargada, este recortada, pertenezca a otro estado de
     visibilidad dinamica o dependa del valor actual de un campo: la relacion de orden persiste si ese mecanismo cambia.
     Un mecanismo no clasificado, o una clase o representacion que no pueda producir una ocupacion conservadora
     independiente de sus mecanismos ⇒ `UNKNOWN` ⇒ E10. **Nunca** «contenido ausente hoy = huella vacia» si puede
     reaparecer sin cambiar el orden. `VisibilityPath` (punto 6.3) sigue rigiendo la equivalencia visual de las piezas;
     esta regla es especifica de ST-19 sobre los objetos del dibujo, y las huellas de las copias tampoco excluyen
     geometria por visibilidad.
   - **XREF (`[V12-D03]`).** Primer corte: XREF descargado ⇒ `UNKNOWN`; XREF no resuelto ⇒ `UNKNOWN`; overlay anidado o
     contenido anidado no disponible ⇒ `UNKNOWN`; XREF cargado ⇒ `SUPPORTED` solo si CT-49 demuestra una huella latente
     conservadora que incluya el contenido anidado relevante, las capas del XREF, sus mecanismos de recorte y sus
     dependencias de presentacion. RACKMIRROR **no** abre archivos externos durante la operacion para adivinar geometria
     ausente. **Contenido cargado actual (`[V15-D05]`, `[V16-D09]`):** la seguridad de ejecucion usa el **contenido
     cargado actual** del XREF y su **completitud** (carga bajo demanda incluida); RACKMIRROR **no** compara identidades
     de archivos futuros ni abre archivos externos. En la frontera, que el mismo contenido cargado o efectivo vuelva a
     estar disponible por un estado (carga, descarga o resolucion), con cualquier ruta, es ESTADO; una recarga o
     resolucion que introduce contenido cambiado o distinto es EDICION futura; un XREF no resuelto ahora es `UNKNOWN`
     ahora. Asi, dos rutas al mismo contenido cargado usan cada una su contenido cargado; un archivo reemplazado sin
     recargar usa el contenido cargado, y su recarga posterior es edicion; y una recarga tras cambiar el archivo es
     edicion. Solo se añadiria un disparador de ejecucion sobre la identidad si CT-49 demuestra que la seguridad actual
     lo necesita.
   - **Completitud de la base de datos y de la enumeracion (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`,
     `[V14-D04]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`).** Tiene dos partes. Los **prerrequisitos
     globales** se comprueban en P0, antes de conocer las fuentes: la base de datos no esta abierta parcialmente y la
     **politica cerrada de sesiones de edicion** (`EditingSessionPolicy`, nombre conceptual) se cumple segun su
     **autoridad observable**, el `EditingSessionAuthoritySet` (nombre conceptual): un conjunto finito de detectores que
     CT-49 establece con un metodo de descubrimiento cerrado —variables de estado de edicion de solo lectura
     (`REFEDITNAME`, `BLOCKEDITOR`, `BLOCKTESTWINDOW`, `ARRAYEDITSTATE` y las demas que encuentre la revision exhaustiva
     del catalogo de variables y de la superficie de API de AutoCAD 2025), autoridades de transaccion
     (`LongTransactionManager`, transaccion larga activa, pertenencia al working set o equivalente) y autoridades de
     comando y de sesion— y que se congela con el contrato; cada señal descubierta queda `DETECTOR`, `IRRELEVANT` o
     `UNSUPPORTED / NO RELIABLE DETECTOR`. La ejecucion **no** descubre modos desconocidos: el resultado es **PASS** si
     y solo si el host (producto, vertical y version mayor o familia de API) esta caracterizado, cada detector requerido
     se lee con exito, cada detector clasificado `BLOCKED` reporta inactivo y cada autoridad generica requerida reporta
     que no hay sesion de edicion activa; en cualquier otro caso ⇒ E12. En el primer corte son `BLOCKED` la edicion de
     referencia en curso, la transaccion larga, el Editor de bloques, la ventana de prueba de bloques y la edicion de la
     fuente de una matriz asociativa (ARRAYEDIT Source), sin razonar sobre su geometria fuente temporal ni sobre su
     replicacion. Una lectura que lanza una excepcion, no esta disponible, no puede consultarse o devuelve un estado no
     clasificable ⇒ E12, **nunca** inactivo; host o version no caracterizados ⇒ E12 «entorno de edicion no
     caracterizado». Un modo «no caracterizado» es un resultado de **caracterizacion**, no una sonda de ejecucion: si
     CT-49 descubre un modo relevante sin detector dedicado fiable ni autoridad generica fiable que lo cubra ⇒ STOP y
     contradiccion material ⇒ Proposal V17; se prohiben las dos lecturas degeneradas (dejar pasar lo no inventariado y
     fallar siempre por no poder probar lo desconocido). Un modo descubierto solo puede ser `CHARACTERIZED_SAFE` con
     deteccion fiable, sin geometria temporal en Model Space que ST-19 pueda enumerar y sin incorporar objetos nuevos a
     otra estructura; reclasificar como seguro un estado del inventario minimo exige Proposal V17. Un fallo ⇒ E12, y los
     prerrequisitos se re-verifican siempre en R6 y al inicio de MUTATE. La **completitud relativa a la fuente** se
     evalua despues de clasificar las fuentes y de la LINE: R2 enumera de forma temprana la cola posterior a cada fuente
     y las entradas de huella, R3 solo puede rechazar, y R6 re-enumera y decide la completitud definitiva —toda la cola
     en el orden efectivo y el contenido relevante de cada objeto enumerado (definicion referenciada, anidados,
     atributos y contenido de XREF) enumerado o clasificado `UNKNOWN`— antes de ST-19 definitivo. Las etapas siguen el
     orden logico del bloque anterior; si una falla, el resultado es `UNKNOWN` o fail-closed y nunca se salta a la
     huella. Un objeto que no esta cargado o que no aparece en la base de datos **nunca** se trata como un objeto que no
     ocupa. Los objetos borrados no forman parte de la enumeracion relevante: borrarlos y restaurarlos (ERASE/OOPS) es
     edicion futura (`[V14-D02]`). RACKMIRROR no ejecuta PARTIALOAD, REFCLOSE, BCLOSE ni ARRAYCLOSE, no fuerza cargas,
     no altera capas, no cambia variables del dibujo y no muta ninguna sesion.
   - **Carga bajo demanda de XREF (`[V13-D03]`).** «XREF cargado» no basta: se clasifica ademas si su contenido
     potencialmente visible esta completamente disponible. Con carga bajo demanda (`XLOADCTL`) e indices de capas o
     espaciales (`INDEXCTL`), el contenido en capas inutilizadas o fuera del recorte puede no estar en memoria; salvo
     que CT-49 demuestre contenido completo, el XREF es `UNKNOWN`. No se fuerzan cargas, no se abren archivos externos y
     no se cambian variables del dibujo; el estado usado entra en la `ModelSpaceOrderTailSnapshot` y en el
     `FootprintInputFingerprint`.
   - **Recortes (`[V12-D04]`).** Un recorte **no** reduce la huella de orden latente si quitarlo o cambiarlo puede hacer
     reaparecer contenido sin cambiar el orden. Para XCLIP y demas filtros espaciales, recortes de imagen, de underlay y
     de nube de puntos, y recortes por capas internas o controles de visualizacion, se usa la ocupacion latente sin
     recortar o conservadora si puede calcularse; si no ⇒ `UNKNOWN`. **Marcos (`[V14-D06]`):** la huella latente de un
     objeto recortado es la ocupacion conservadora del contenido sin recortar **∪** la ocupacion del contorno o marco
     que pueda hacerse visible (`XCLIPFRAME`, `FRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, `PDFFRAME`, `DWFFRAME`, `DGNFRAME`,
     `POINTCLOUDCLIPFRAME` y equivalentes); si la ocupacion del marco no puede acotarse ⇒ `UNKNOWN`. Los modelos de
     coordinacion (NWD, NWC y clases similares) entran en el inventario por tipo exacto y pueden quedar `UNKNOWN`.
   - **Visibilidad dinamica (`[V12-D05]`).** Para un `BlockReference` posterior con estados de visibilidad dinamica, el
     estado actual **no** basta: si CT-49 puede construir la union conservadora de todos los estados que pueden
     activarse sin cambiar el orden ⇒ `SUPPORTED`; si no ⇒ `UNKNOWN`. El primer corte no obliga a enumerar
     universalmente los estados.
   - **Campos (`[V12-D06]`, `[V15-D04]`, `[V16-D08]`).** Si la ocupacion de modelo de un objeto depende de la evaluacion
     de un campo (`DBText`, `MText`, atributos, celdas de tabla u otras clases con campos que se descubran), es
     `SUPPORTED` solo con una cota conservadora valida para **todos** los resultados que admite el contrato; si no ⇒
     `UNKNOWN`. El valor actual o `FIELDEVAL` no son autoridad suficiente. **Precedencia causal:** la evaluacion de un
     campo no decide por si sola si un cambio es ESTADO o EDICION; se clasifica por la **causa** del cambio del
     resultado. Es ESTADO si el resultado cambia porque solo cambia un estado admitido: visibilidad, carga o resolucion,
     una variable del sistema, la fecha o la hora, el nombre del archivo, el estado del conjunto de planos, el guardado
     o el trazado, el refresco de `DataLink` u otro estado admitido explicitamente. Es EDICION si cambia porque se edito
     el contenido logico referenciado: la geometria o una propiedad del objeto referenciado (por ejemplo, un campo
     `AcObjProp` con el area de una polilinea), una edicion manual de texto o de contenido u otra edicion futura. **EDIT
     CAUSE > FIELD REEVALUATION.** Por eso «todos los resultados que admite el contrato» son los alcanzables **solo**
     por cambios de ESTADO; sin una cota conservadora sobre ellos ⇒ `UNKNOWN`. `DataLink` sigue siendo la decision
     especial explicita: datos externos cambiados + refresco = ESTADO. **Dependencias mixtas (`[V16-D08]`):** en un
     campo con entradas dependientes de edicion y entradas dependientes de estado, la envolvente de estado mantiene
     **fijas** las entradas dependientes de edicion en su valor actual y solo hace variar las dependientes de estado
     entre los resultados de estado admitidos: en `Area(objeto) + Date` el area queda fija y varia la fecha; en
     `Length(objeto) + valor de DataLink` la longitud queda fija y varia el valor de `DataLink`; en una propiedad de un
     objeto anidado mas una variable de estado, la propiedad queda fija y varia la variable; en una dependencia de
     estado de un XREF mas geometria editada por separado, la geometria queda fija y varia el estado del XREF. Todo
     resultado que requiera EDITAR contenido logico queda fuera de la garantia; sin cota conservadora de estado ⇒
     `UNKNOWN`.
   - **Mecanismos añadidos (`[V13-D05]`, `[V14-D06]`).** Atributos invisibles (`AttributeReference.Invisible`) y
     `ATTDISP`; objetos ocultos o aislados (`HIDEOBJECTS`, `ISOLATEOBJECTS`, `OBJECTISOLATIONMODE`); visualizacion de
     proxies (`PROXYSHOW`, `PROXYGRAPHICS`); y vinculos de datos (`DataLink`), formulas de tabla y contenido que depende
     de una actualizacion. Cada caso es `SUPPORTED(metodo latente conservador)` o `UNKNOWN`; el estado actual de
     visualizacion o el valor ya actualizado no son autoridad suficiente.
   - **Overrules y reactores de terceros (`[V16-D07]`).** Esta regla de seguridad del dibujo es distinta de la guarda de
     codigo de RackCad (G-M24): un overrule o un reactor registrado por otra aplicacion puede cambiar la representacion
     dibujada de un objeto, de modo que su geometria de la base de datos ya no basta para demostrar su
     `VisualFootprint`. CT-49 caracteriza la autoridad disponible (`Overrule.Overruling` o equivalente) y, si la hay, su
     alcance por clase. Primer corte: si la representacion dibujable de un objeto puede estar afectada por un overrule o
     reactor activo y no se demuestra equivalencia u ocupacion conservadora ⇒ `VisualFootprint = UNKNOWN`, que hace
     fallar ST-19 cuando el objeto es material para el orden (posterior a una fuente) y, en la copia, el orden de plan;
     si CT-49 no puede acotar por clase, la regla alcanza a todas las clases que el overrule activo pueda alterar.
     RACKMIRROR no inspecciona los internos de plugins de terceros (R-58).
   - **Frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`, `[V15-D04]`, `[V15-D05]`, `[V16-D08]`,
     `[V16-D09]`).** **ESTADO, dentro de la garantia:** visibilidad (capas, `Entity.Visible` y representaciones
     anotativas); carga y descarga de XREF; el mismo contenido cargado o efectivo de un XREF disponible de nuevo por un
     estado, con cualquier ruta; carga bajo demanda; recortes, con sus marcos; **solo** la visibilidad dinamica;
     evaluacion de campos cuyo resultado cambia solo por un estado admitido; refresco de `DataLink`; visualizacion de
     proxies; aislamiento de objetos; y estado de visualizacion de anotacion y de entorno. **EDICION FUTURA, fuera:**
     ERASE/OOPS y la restauracion programatica de objetos borrados; edicion de geometria; BEDIT o redefinicion de
     bloque; edicion manual de texto; edicion de la geometria o de una propiedad de un objeto referenciado por un campo;
     mover o estirar; un parametro dinamico que cambia la geometria; editar el archivo externo de un XREF y recargarlo;
     recargar o resolver un XREF introduciendo contenido cambiado o **distinto**, cualquiera sea la ruta; y reemplazar
     la fuente o el contenido de una imagen, un underlay o una nube de puntos. **Precedencias:** el refresco de
     `DataLink` sigue siendo ESTADO aunque su fuente externa haya cambiado, y el contrato exige una cota conservadora
     para todos sus resultados o `UNKNOWN`; una operacion dinamica (tabla de busqueda o parametro combinado) que cambia
     la geometria ademas de la visibilidad es EDICION, y una tabla de busqueda que solo cambia la visibilidad es ESTADO;
     un cambio del resultado de un campo causado por la edicion del contenido referenciado es EDICION (EDIT CAUSE >
     FIELD REEVALUATION); en un campo mixto las entradas de edicion quedan fijas y solo varian las de estado; la
     seguridad de ejecucion de un XREF usa su contenido cargado actual y su completitud, sin comparar identidades de
     archivos futuros, y un XREF no resuelto ahora es `UNKNOWN`. CT-49 puede caracterizar la restauracion del handle y
     la posicion en el orden tras OOPS, pero eso no la incorpora a la garantia. RACKMIRROR no garantiza la invariancia
     del orden de dibujo frente a ediciones futuras arbitrarias (L-37).
   - **Ocupacion dependiente del entorno (`[V11-D04]`, `[V12-D07]`).** `PDMODE`, `PDSIZE`, `LTSCALE`, `MSLTSCALE`, las
     representaciones anotativas, las metricas de fuente o de texto y lo que descubran CT-49 y CT-50: si CT-49 demuestra
     una ocupacion de modelo conservadora bajo **todos** los estados relevantes que admite el contrato, la clase es
     `SUPPORTED` y esos estados se declaran como observaciones consumidas en el `FootprintInputFingerprint` del objeto;
     si no, `UNKNOWN`. La regla **no** es solo para objetos posteriores: se aplica tambien a las huellas de la propia
     copia —`DBText`, cotas, anotaciones, piezas y contenido anidado—, con los casos obligatorios de CT-50 (estilo de
     texto anotativo, estilo de cota anotativo, `DIMSCALE = 0`, estilo hijo de la familia `<estilo>$0`, fuente ausente,
     `FONTALT` o sustitucion de fuentes y toda dependencia del entorno que se descubra); sin huella conservadora ⇒
     `UNKNOWN` (E10 en ST-19, X-22 en el orden de plan) o limitacion explicita de O-1 (L-36, L-37). No se inventa una
     formula universal. Un objeto posterior asi, sin huella conservadora, puede hacer fallar el comando (L-34, L-37).
   ...
   - **Frescura (`[V10-D03]`).** Cada huella lleva su `FootprintInputFingerprint`: identidades de las definiciones
     reales, registros de simbolos resueltos usados (incluidos los que deciden la elegibilidad de la presentacion,
     `[V11-D01]`), observaciones consumidas del contexto de materializacion con su cierre transitivo, estado dinamico
     efectivo, identidades de las dependencias anidadas, estados del entorno de una huella `SUPPORTED`, tambien en la
     copia (`[V11-D04]`, `[V12-D07]`), estado de disponibilidad y de mecanismo, incluidos la carga bajo demanda de XREF
     y los mecanismos añadidos (`[V12-D01]`, `[V13-D03]`, `[V13-D05]`), y cualquier otra entrada que altere la huella.
     Tras `EnsureForPlan` se relee el dibujo real; toda huella cuyo fingerprint cambie se vuelve a producir y se
     re-evaluan el orden de plan y ST-19 (PRE-18). No se usa `TraceFingerprint` para esto (punto 10).
   - **Consumo.** ST-19: superposicion con una huella `UNKNOWN` ⇒ E10, y un objeto posterior con huella `UNKNOWN` falla
     aunque parezca estar lejos del destino, porque no existe autoridad para demostrar la no superposicion
     (`[V12-D10]`). Orden de plan (punto 6.6): huella `UNKNOWN` en un par invertido ⇒ X-22. Las piezas con cambio de
     mano siguen necesitando `GeometrySymmetryResult`.
   - **Una sola autoridad** de extraccion de huellas y de sus margenes de modelo (G-M23). CT-36 y CT-49 caracterizan las
     ocupaciones reales por clase; T-M71 las prueba con dobles en Core. La `ModelSpaceFootprintPolicy`, con sus ejes
     `ContentAvailability` y `VisibilityMechanism`, es la unica clasificacion de los objetos del dibujo, solo se aplica
     sobre una enumeracion completa (ST-20) y T-M73 prueba su consumo (`[V11-D02]`..`[V11-D04]`,
     `[V12-D01]`..`[V12-D06]`, `[V13-D01]`..`[V13-D05]`).
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   | Cualquier primitiva cuyo **tipo de linea efectivo**, resuelto en el `DestinationEvaluationContext` por su `ResolvedPresentationEligibility` (6.2), sea complejo (con formas o texto), no determinable o no caracterizado, sea su fuente `Explicit`, `ByLayer(Named)`, `InheritLayer` o `InheritBlock` | `UNKNOWN` ⇒ `FAIL_CLOSED` en el primer corte, salvo evidencia expresa posterior (`[V8-D05]`, `[V11-D01]`) |
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V17.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - tipo de linea: por nombre de la tabla de tipos de linea, sin distinguir mayusculas como los simbolos de AutoCAD;
       la igualdad del nombre no decide si esta soportado: un nombre cuyo registro efectivo en el dibujo destino es
       complejo ⇒ `UNKNOWN` (6.1 y elegibilidad resuelta);
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V16 no introduce tolerancia; CT-36 registra
   ...
   - **Contrato por propiedad** (estados de la API; CT-36 confirma los exactos y un estado encontrado y no modelado ⇒
     ...
     | `Linetype` | `ByLayer`, `ByBlock`, tipo con nombre | `Explicit(nombre)`, `ByLayer(Named)`, `InheritLayer`, `InheritBlock`; tipo **efectivo** complejo, no determinable o no caracterizado en el dibujo destino, por cualquiera de esas fuentes ⇒ `UNKNOWN` (elegibilidad resuelta, `[V11-D01]`) |
   ...
   - **Elegibilidad de la presentacion resuelta (`ResolvedPresentationEligibility`, `[V11-D01]`).** Son dos preguntas
     **separadas**, y ninguna sustituye a la otra:

     ```text
     A) igualdad simbolica   VisualSource(izquierda, P) == VisualSource(derecha, P)        (este punto 6.2)
     B) elegibilidad         ¿la representacion RESUELTA de cada primitiva esta soportada?  (punto 6.1)

     por primitiva:
       VisualSource → resolucion en el DestinationEvaluationContext (punto 4) → ResolvedPresentationEligibility
       cadena de resolucion, segun corresponda (DRAWING WINS):
         Explicit(nombre)             registro del dibujo destino con ese nombre
         ByLayer(Named(capa))         registro de la capa del dibujo destino y su tipo de linea
         InheritLayer / InheritBlock  cadenas de capa 0 y ByBlock hasta la referencia de pieza; su presentacion de
                                      creacion (la que aplique AutoCAD 2025 segun CT-50, p. ej. CLAYER o CELTYPE) y,
                                      si hereda de la referencia exterior, la presentacion de ST-13
       Linetype: LinetypeTableRecord EFECTIVO del dibujo destino
         tipo complejo con formas o texto | registro no determinable | contenido no caracterizado ⇒ UNKNOWN
         tambien si LinetypeSource = ByLayer o heredada, y tambien si un nombre Explicit existe en el dibujo con
         contenido complejo
       los registros resueltos usados entran en FootprintInputFingerprint, en el MaterializationContextReadSet y en
         la evidencia y su fingerprint cuando la elegibilidad depende de ellos (punto 10)
     ```

     Las otras propiedades de las capas homonimas del dibujo no invalidan la igualdad de fuentes simbolicas (A), pero
     pueden invalidar la elegibilidad de la variante soportada (B). La elegibilidad se decide en el contexto destino
     aunque la evidencia de simetria use el contexto de la autoridad origen (punto 4). CT-36, CT-49 y CT-50 caracterizan
     la cadena real; T-M63 y T-M71 la prueban.
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V16 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V16 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V16 no introduce una tolerancia nueva: CT-36
...
10. **`TraceFingerprint`.** No prueba simetria. Solo sirve para **trazabilidad**, **deteccion de obsolescencia** y
    ...
    **`LinetypeScale`**, `LinetypePatternContext`, **`VisibilityPath`**, `FillSignature`, relaciones de orden visual
    materiales, definiciones anidadas por identidad estable, BTR evaluados, **transformacion efectiva**, contexto de la
    base auxiliar (punto 4) y los registros resueltos de los que dependa la elegibilidad de la presentacion de cada
    primitiva (punto 6.2, `[V11-D01]`); en **serializacion canonica** (ordenada por contenido, nunca por el orden de
    iteracion del BTR, que DRAWORDER, SORTENTS o las ediciones alteran; el orden visual se registra como relacion, no
    como orden de iteracion). Los bits exactos sirven para la trazabilidad; la equivalencia usa tolerancias.
    `TraceFingerprint` **no** es `FootprintInputFingerprint` (§9.2): las entradas de la huella visual se fijan aparte y
    se re-verifican para toda instancia cuya huella se uso (PRE-18, `[V10-D03]`, `[V10-D19]`).
...
12. **Estados futuros por variables.** La verificacion de un estado concreto **no** basta si el mismo rack puede cambiar ese
    ...
    (`892a601`, antes `977f873` y `c4880af`) y G6 (`988ab9c`, antes `5f969cc`: enlace y evaluacion de un arbol, sin
    dominio ni `PlanReadSet`), con su correccion G6-C2 (`75f1862`), no cambian esto (`[V9-D12]`, `[V9-D21]`,
    `[V10-D14]`, `[V12-D12]`); si I-49 integra su dominio y su `PlanReadSet`, se consumen sin analisis propio.
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Acceso a la cache de biblioteca (`[V7-D15]`, `[V8-D10]`).** La `Database` cacheada por `BlockLibraryImporter`
    ...
    base auxiliar privada con el contexto del punto 4 (autoridad origen para la evidencia de simetria;
      DestinationEvaluationContext para la huella visual y la elegibilidad de la presentacion resuelta)
    ...
    **Metodo de inventario cerrado (`[V9-D08]`, `[V12-D01]`, `[V13-D01]`, `[V13-D04]`, `[V13-D05]`).** CT-36 (contenido
    de las piezas evaluadas) y CT-49 (referencia fuente y objetos de Model Space, con la definicion referenciada y sus
    componentes) declaran como descubren propiedades, estado y mecanismos, con un harness fuera de `src/`:
    ...
    por tipo exacto en ejecucion:
      ...
      en objetos de Model Space tambien: definicion referenciada y disponibilidad de su contenido (XREF cargado,
        descargado, no resuelto u overlay, y carga bajo demanda con indices) · estados de visibilidad dinamica ·
        recortes de imagen, underlay y nube de puntos · capas internas y controles de visualizacion · campos ·
        atributos invisibles y ATTDISP · aislamiento de objetos · visualizacion de proxies · vinculos de datos y
        formulas de tabla · marcos y contornos de recorte · modelos de coordinacion ([V12-D01], [V12-D02],
        [V13-D03], [V13-D05], [V14-D06])
      antes de clasificar ([V13-D01], [V13-D04], [V14-D01], [V14-D04], [V15-D01], [V16-D01]): prerrequisitos
        globales en P0 (apertura parcial y PASS del EditingSessionAuthoritySet) y completitud relativa a la fuente
        en R2 y R6 (cola
        posterior a cada fuente y contenido relevante); sin completitud demostrada no se clasifica ninguna
        propiedad ni ningun mecanismo de los objetos de Model Space (ST-20); los objetos borrados no se enumeran
        (ERASE/OOPS = edicion, [V14-D02])
    ...
    clasificacion de cada propiedad, entrada o estado encontrado (exactamente una):
      ...
      en objetos de Model Space (ST-19), ademas, cada mecanismo encontrado: SUPPORTED(metodo conservador) o UNKNOWN

    sin clasificar o UNSUPPORTED ⇒ UNKNOWN (contenido evaluado, X-17b), E4 (referencia fuente, ST-18) o huella
      UNKNOWN ⇒ E10 (objeto de Model Space posterior, ST-19); prerrequisito global no demostrable ⇒ E12 (P0);
      completitud relativa a la fuente no demostrable ⇒ E10 (R2 o R6)
    ```
    ...
    **STOP → Proposal V17**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V16 abre **Proposal V17**.
...
| CT-04 | Decodificacion de `View`/`Section` de produccion por kind (tabla de §3.7), incluidos `−1` legado, secciones fuera de rango, `2..3` en Push Back simple y `View` vacia. **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V13-D08]`): caracterizacion unica compartida con I-55 (X-2; su nombre en I-55 es `RackViewEnvelopeReadingCharacterizationTests`), ejecutada por el primer G3 que llegue; nombre y suite se fijan en la reconciliacion | Core + guarda de texto de las lecturas del Plugin |
| CT-05 | Tramos y centros `c` por (kind, vista) desde la geometria resuelta. **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V13-D08]`): caracterizacion unica compartida con I-55 (X-8, `RackViewFrameCharacterizationTests`: ejes con signo, origen fisico y `[K_min, K_max]` por sistema, tipo y variante), ejecutada por el primer G3 que llegue; nombre y suite se fijan en la reconciliacion | Core |
...
| CT-16 | **Semantica observable de `RackDuplicationPlan`**: textos completos, orden de grupos, orden de referencias, avisos y errores, sobre codigo intacto (§5.1). **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V13-D08]`): caracterizacion unica compartida con I-55 (X-1), sobre codigo intacto y antes de cualquier extraccion, ejecutada por el primer G3 que llegue; nombre y suite se fijan en la reconciliacion | Core |
...
| **CT-46** | **Guardas cruzadas vigentes**: relee en el SHA de la reconciliacion las guardas de texto de `ProjectVariablesConformanceTests.cs` (main y, si integra, I-49), C-08/C-10, las del nucleo de expresiones (extendidas por I-49 G6 `988ab9c`, antes `5f969cc`, con `LengthUnitsAuthorityGuardTests` sobre todo `src/`, y ajustadas por la correccion G6-C2 `75f1862`) y T-GRD-01..T-GRD-08 de I-54, ya en `main @ ba497f1` (incluidas las del borde fisico, T-GRD-04..T-GRD-07, y los censos por nombre de comandos, ayuda y ventanas, T-GRD-08); fija la lista exacta que G-M18 aplica a los archivos de I-52 y la autoridad vigente de I-49 (ADR-0041, aceptado y que reemplaza a ADR-0040, `[V8-D18]`, `[V11-D11]`, `[V12-D12]`); **autoridad final de I-49** (`[V14-D07]`): antes del freeze de I-52 relee V6, A1, A2, A3, el ADR de reemplazo de ADR-0041, el Consensus Freeze nuevo y el estado de implementacion, con la semantica final de D19 (M2, M3, M4 y M10(a)) y de D20 (M10(b)); sin esa autoridad, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED | Core |
| **CT-47** | **Corpus indicativo de la biblioteca actual**: recalcula el **SHA-256** del archivo antes de usar la evidencia (identidad; ruta, tamaño y fecha son diagnostico) y, si cambia, no reutiliza la evidencia previa; recalcula en `acad.exe`, con evaluacion de estados, la tabla de §1.4 por pieza y vista con las reglas de V16 (politica cerrada de variantes con inventario cerrado, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, `VisibilityPath`, `LinetypeScale` y contexto de patron, orientacion, multiplicidad, orden visual dentro de la pieza y entre piezas sobre huellas de ocupacion de modelo en el contexto destino y pares invertidos, degradados, falsos positivos y negativos de la canonicalizacion) y la compara con la evidencia indicativa; una reduccion material de la envolvente ⇒ Proposal V17 | evidencia registrada |
...
| **CT-49** | **Presentacion de la referencia fuente y orden en Model Space en `acad.exe`** (`[V8-D13]`, `[V8-D14]`, `[V9-D01]`, `[V9-D04]`, `[V9-D08]`, `[V9-D09]`, `[V10-D01]`, `[V10-D03]`, `[V10-D06]`, `[V10-D07]`), **fuera de `src/`**: **inventario cerrado** de las propiedades y el estado de `Entity` y `BlockReference` de AutoCAD 2025 con el metodo de §11.3 punto 14, clasificados como `TRANSPORTED` (ST-13), `PLACEMENT`, `COMMON_CONTEXT`, `NON_VISUAL` o `UNSUPPORTED` (ST-18), sin nada sin clasificar; lectura y copia fiel de las propiedades de ST-13, con `PlotStyleName` en un dibujo STB y la derivacion del color en uno CTB; deteccion de XCLIP y otras entradas de `ACAD_FILTER`, referencias de atributo, entradas del diccionario de extension y XData de efecto visual; **orden efectivo** de Model Space con `SortentsTable`, `SORTENTS`, `DRAWORDERCTL` y cualquier contexto adicional encontrado, en pantalla y en trazado, y el conflicto entre ambos para dos fuentes seleccionadas cuyas copias se superponen (E10); **huellas de ocupacion de modelo** de copias y de objetos existentes (`Extents3d` solo ampliado por ocupaciones de modelo, nunca por `LineWeight`, `LWDISPLAY`, CTB/STB ni escala de trazado; mascaras y marcos; envolventes de texto y de cota bajo `DIMSTYLE`, `TEXTSTYLE` y escala de anotacion; `XLINE` y `RAY` con huella o interseccion analitica fiable o `UNKNOWN`); clasificacion de la superposicion de una copia con **todo** objeto posterior a su fuente: otra fuente seleccionada posterior, objeto seleccionado e ignorado posterior, objeto no seleccionado posterior, objeto coincidente, `XLINE` o extension no finita, mascaras, rellenos, grosor, degradados, wipeouts, imagenes, OLE y referencias que los contengan; orden resultante de las copias creadas en el orden de sus fuentes; **frescura**: una huella de biblioteca distinta de la del dibujo real tras `EnsureForPlan`, en una pieza sin cambio de mano, que la repeticion de ST-19 y del orden de plan detecta; valores efectivos de creacion observables en la referencia y en las piezas nuevas (con CT-50); **`ModelSpaceFootprintPolicy`** por tipo exacto (`[V11-D02]`) con el inventario minimo `Line`, `Arc`, `Circle`, `Ellipse`, `Spline`, `LWPOLYLINE`, `Solid`, `Trace`, `Hatch`, `BlockReference` con atributos, `DBText`, `MText` con fondo y columnas, `Dimension` con flechas, relleno y demas componentes, `MLeader`, `Table`, `Point` con `PDMODE`/`PDSIZE`, `Wipeout`, raster y underlay, `XLINE`/`RAY`, nube de puntos y proxy o entidad custom, cada clase `SUPPORTED(metodo)` o `UNKNOWN` y `Extents3d` solo donde se demuestre cota conservadora; **visibilidad** (`[V11-D03]`): capa apagada, inutilizada o inutilizada en ventana, `Entity.Visible = false` y su reactivacion posterior, con huella de orden sin cambio y conservadora; **contexto global** (`[V11-D04]`): clasificaciones `SUPPORTED`/`UNKNOWN` bajo todos los estados admitidos y read-set de lo realmente consumido; **cola posterior** (`[V11-D05]`): objeto añadido tras R6, eliminado o reordenado y objeto que pasa de antes a despues de la fuente o al reves; tipo de linea efectivo complejo de la presentacion exterior y de los objetos posteriores (`[V11-D01]`); **mecanismos** (`[V12-D01]`, `[V12-D02]`): inventario obligatorio de visibilidad, XREF, `BlockReference`, filtros espaciales, raster, underlay y nube de puntos, y campos, cada caso `SUPPORTED(metodo conservador)` o `UNKNOWN`, con mecanismo no clasificado ⇒ `UNKNOWN`; **XREF** (`[V12-D03]`): cargado, descargado, no resuelto y overlay anidado no disponible, sin abrir archivos externos; **recortes** (`[V12-D04]`): XCLIP, recorte de imagen, recorte de underlay y recorte o capas internas de nube de puntos donde existan, con ocupacion sin recortar o `UNKNOWN`; **visibilidad dinamica** (`[V12-D05]`): estados visibles A y B y estados latentes no disponibles ⇒ `UNKNOWN`; **campos** (`[V12-D06]`): en `DBText`, `MText`, atributo y celda de tabla donde se soporte, con cota conservadora para todos los resultados o `UNKNOWN`; **objeto `UNKNOWN` lejano** (`[V12-D10]`): hace fallar ST-19; **base de datos** (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`): dibujo normal abierto completo y dibujo abierto parcialmente, con la API exacta de AutoCAD 2025 (`Database.IsPartiallyOpened` o el mecanismo equivalente) y la completitud de la enumeracion de Model Space y del contenido relevante; apertura parcial ⇒ fallo antes de ST-19, sin PARTIALOAD, cargas forzadas ni cambios de capas o de variables; **carga bajo demanda de XREF** (`[V13-D03]`): estados de `XLOADCTL` e `INDEXCTL`, indice de capas, indice espacial y toda API o estado relacionado que se descubra; XREF cargado con contenido incompleto ⇒ `UNKNOWN` o fallo; contenido completo demostrable ⇒ candidato admitido; **atributos** (`[V13-D05]`): `AttributeReference.Invisible` y `ATTDISP`; **aislamiento de objetos**: oculto, aislado y restaurado con `HIDEOBJECTS`, `ISOLATEOBJECTS` y `OBJECTISOLATIONMODE`; **proxy**: `PROXYSHOW` y `PROXYGRAPHICS`; **tabla**: `DataLink` y actualizacion de formulas; cada mecanismo `SUPPORTED(metodo latente conservador)` o `UNKNOWN`; **sesiones de edicion** (`[V14-D01]`): REFEDIT activo, REFEDIT cerrado, Editor de bloques activo y Model Space normal, con las API exactas de AutoCAD 2025 (`REFEDITNAME`, transaccion larga o edicion en sitio, `BLOCKEDITOR` o sus equivalentes) y sin cerrar ni mutar ninguna sesion; **ERASE/OOPS** (`[V14-D02]`): solo caracterizacion de la restauracion del handle y de la posicion en el orden tras OOPS, demostrando que la redaccion lo deja fuera de la garantia; **estado/edicion** (`[V14-D03]`): XREF que resuelve al mismo archivo o contenido, XREF que resuelve a otro contenido, XREF externo editado y recargado, `DataLink` con la fuente externa cambiada y refrescado, cambio de visibilidad dinamica y tabla de busqueda dinamica que cambia la geometria; **marcos** (`[V14-D06]`): `XCLIPFRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, marcos PDF, DWF y DGN y marco de recorte de nube de puntos, con un contorno fuera del contenido sin recortar, y modelos de coordinacion (NWD, NWC); **`XLOADCTL` por defecto** (`[V14-D09]`): comportamiento de la carga bajo demanda con el valor por defecto de la estacion y contenido completo demostrable o no; **politica cerrada de sesiones de edicion** (`[V15-D01]`, `[V15-D02]`): deteccion fiable en AutoCAD 2025 de REFEDIT, transaccion larga (`LongTransactionManager`), Editor de bloques, ventana de prueba de bloques (`BLOCKTESTWINDOW`) y edicion de la fuente de una matriz asociativa (ARRAYEDIT Source), con los casos ARRAYEDIT Source activo y cerrado, ventana de prueba de bloques activa y cerrada, REFEDIT y Editor de bloques activos y cerrados y estado de sesion desconocido o no detectable ⇒ E12; inventario de otros modos de edicion en sitio, cada uno `CHARACTERIZED_SAFE` (con demostracion) o `BLOCKED`; sin cerrar ni mutar ninguna sesion; **`EditingSessionAuthoritySet`** (`[V16-D01]`..`[V16-D04]`): **metodo de descubrimiento cerrado** documentado para AutoCAD 2025 que inspecciona y clasifica como minimo las variables de estado de edicion de solo lectura (`REFEDITNAME`, `BLOCKEDITOR`, `BLOCKTESTWINDOW`, `ARRAYEDITSTATE` y toda otra variable de estado de edicion o de sesion hallada revisando de forma exhaustiva el catalogo de variables del sistema y la superficie de API aplicables), las autoridades de transaccion (`LongTransactionManager`, transaccion larga activa, pertenencia al working set o equivalente) y las autoridades de comando y de sesion (estado del comando activo relevante para la edicion en sitio persistente y toda otra autoridad generica de host o de sesion), con cada señal clasificada `DETECTOR`, `IRRELEVANT` o `UNSUPPORTED / NO RELIABLE DETECTOR` y sin afirmar seguridad exhaustiva desde una lista corta; **guard de host**: producto, vertical y version mayor o familia de API caracterizados y su deteccion; **casos**: conjunto cerrado de detectores con exito (PASS); lectura fallida, excepcion, detector no disponible o estado no clasificable ⇒ E12; host o version no soportados ⇒ E12; `ARRAYEDITSTATE` activo e inactivo; autoridad relevante de comando o de sesion activa ⇒ E12; modo relevante descubierto sin detector fiable ⇒ STOP de caracterizacion y Proposal V17; **campos por causa** (`[V15-D04]`, `[V16-D08]`): campo de propiedad de objeto (`AcObjProp` o equivalente) que referencia geometria, con el objeto referenciado cambiado por EDICION (fuera de la garantia) frente a una reevaluacion solo por ESTADO (cota para los resultados alcanzables por estado o `UNKNOWN`), y `DataLink` refrescado como estado; **dependencias mixtas**: `Area(objeto) + Date`, `Length(objeto) + valor de DataLink`, propiedad de un objeto anidado editado + variable de estado y dependencia de estado de un XREF + geometria editada por separado, con las entradas de edicion fijas en su valor actual; **overrules de terceros** (`[V16-D07]`): autoridad `Overrule.Overruling` o equivalente, overrule activo que altera una clase posterior ⇒ `UNKNOWN` salvo soporte conservador demostrado, y alcance por clase si puede acotarse; **dinamicos**: tabla de busqueda que solo cambia la visibilidad (estado) frente a la que cambia visibilidad y geometria o estiramiento (edicion); **XREF por contenido cargado actual** (`[V15-D05]`, `[V16-D09]`): mismo contenido cargado por otra ruta (huella del contenido cargado), archivo externo reemplazado sin recargar (huella del contenido cargado; la recarga posterior es edicion), recarga tras cambiar el archivo (edicion) y XREF no resuelto ahora (`UNKNOWN`), sin comparar identidades de archivos futuros ni abrir archivos externos | evidencia registrada; Core consume la clasificacion |
| **CT-50** | **Contexto de materializacion y su read-set en `acad.exe`** (`[V9-D05]`, `[V10-D04]`, `[V10-D05]`), **fuera de `src/`**: que mecanismo y que valores efectivos aplica AutoCAD 2025 a las propiedades que los drawers no asignan explicitamente —referencias de `LateralHeaderDrawer` (colocaciones de grupos y piezas), `DBText` de anotaciones, cotas y curvas de `CantileverViewMaterializer`—, al menos `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT`, `CETRANSPARENCY`, `CPLOTSTYLE` y `PSTYLEMODE`, el `DIMSTYLE` actual y su registro, el `TEXTSTYLE` efectivo, la escala de anotacion, los registros de capa `RACKCAD_ANOTACIONES`, `RACKCAD_COTAS` y de las capas de rol, y cualquier otro estado que se descubra; **read-set por productor**: cambiar un estado que el productor no consume (por ejemplo `CLAYER` en un productor que fija la capa) no invalida, y cambiar uno consumido (`DIMSTYLE`, `TEXTSTYLE` u otro) si, durante la peticion de la linea y entre PREPARE y MUTATE; consumo no determinable ⇒ `UNKNOWN`; comprobar que `Π(D)` y `Π(μ_k D)` usan las mismas observaciones; productor que las aplique de forma asimetrica ⇒ `UNKNOWN`; efecto sobre la presentacion interna frente a una fuente dibujada con otro contexto (L-36); **cierre transitivo** (`[V11-D06]`): cambio de una dependencia del estilo de cota (estilo de texto, bloques de flecha, tipos de linea, relleno), cambio de `TEXTSTYLE` o de las metricas de fuente, busqueda del estilo con nombre del plan y `AnnotationScale` authored frente a `CANNOSCALE`, que solo entra si se demuestra su consumo; presentacion de creacion de la referencia de pieza (`CLAYER`, `CELTYPE`) como eslabon de la elegibilidad resuelta (`[V11-D01]`); sitios productores que fija G-M24 (`[V11-D08]`) y censo inicial de su lado B por familias de simbolos de API (construccion y alta de entidades, importacion y clonado, altas en tablas de simbolos, cabeceras y valores por defecto de `Database`, evaluacion grafica, atributos, rutas de comando y COM, delegacion en helpers, escritura o mutacion de objetos o contexto existentes, mutadores internos de AutoCAD y registros de overrules o reactores), con cada call-site `DECLARED` o `EXCLUDED` con motivo y anclado como minimo en la creacion de `LateralHeaderDrawer` (`LateralHeaderDrawer.cs:244`), su `RecomputeDimensionBlock` (`:375`) y su `db.Dimstyle` (`:391`), el `layerTable.Add` de `LayerHelper` (`LayerHelper.cs:26`) y las altas de bloque y de capa de `CantileverViewMaterializer` (`CantileverViewMaterializer.cs:48`, `:111`, `:195`) (`[V12-D09]`, `[V13-D07]`, `[V14-D05]`); **censo inicial de escrituras** (`[V15-D03]`), medido en `main @ dad4e77`: 35 sitios de apertura para escritura en 15 archivos del Plugin —en los productores declarados `LateralHeaderDrawer.cs:31`, `:91`, `:92`, `:117`, `:169`, `:225`, `CantileverViewMaterializer.cs:44`, `:71`, `:75`, `:87`, `:107`, `:125`, `:187` y `LayerHelper.cs:24` (identicos en `1b091be`), y fuera de ellos `BlockPlacement.cs:87`, `:154`, `:194`, `RackBlockData.cs:27`, `:33`, `:43`, `RackCloner.cs:31`, `:60`, `LateralHeaderDrawService.cs:279`, `RackCommandSupport.cs:250`, `:256`, `RackBlockRenamer.cs:48`, `RackDuplicarCommands.cs:290`, `StructuralSectionMaterializer.cs:52`, `:83`, `RackLayoutCommands.cs:223`, `RackLayoutCommands.Fill.cs:449`, `CustomPropertiesData.cs:146`, `:154` y `ProjectVariablesData.cs:113`, `:126`—, cada uno `DECLARED` o `EXCLUDED` con motivo; hoy ninguno muta la presentacion de una entidad existente ni un registro de simbolo existente (las asignaciones de presentacion de `CantileverViewMaterializer.cs:214-217` y `:280-283` son sobre entidades nuevas de un productor declarado, lado A, y `RackCloner.cs:60` cambia el texto de una etiqueta clonada por RACKDUPLICAR) y ningun sitio de `src/` usa `SetPropertiesFrom`, `SetSystemVariable`, `SetDimstyleData` ni `SetDatabaseDefaults`; **censo inicial de mutadores internos y de overrules o reactores** (`[V16-D05]`, `[V16-D06]`), medido en `main @ dad4e77`: ningun sitio de `src/` usa `SynchronizeAttributes`, `LayerStateManager`, `ReloadXrefs`, `ResolveXrefs`, `BindXrefs`, `AssocManager`, `Audit`, `ResetBlock` ni `ConvertToStaticBlock`, registra overrules ni se suscribe a eventos de `Database`, `Document`, `Editor` o `Application` de AutoCAD; **entorno de la copia** (`[V12-D07]`): estilo de texto anotativo, estilo de cota anotativo, `DIMSCALE = 0`, estilo hijo de la familia (`<estilo>$0`), fuente ausente, `FONTALT` o sustitucion de fuentes y toda dependencia del entorno que se descubra, con huella conservadora bajo todos los estados admitidos o `UNKNOWN` | evidencia registrada; Core consume la clasificacion |
...
| **T-M71** | `VisualFootprintResult` (`[V9-D02]`, `[V10-D01]`..`[V10-D04]`, `[V10-D07]`): autoridad separada de `GeometrySymmetryResult`; **ocupacion de modelo en WCS** que nunca subestima (soporte de modelo de los trazos, ancho geometrico real, rellenos, mascaras con tamaño en modelo, limites de texto y de cota bajo su contexto resuelto, marcos, anidados aplanados); regiones construidas **despues** de transformar a WCS: una fuente con `s ≠ 1` y escalas anidadas producen la ocupacion de la geometria dibujada; **no** se dilata por `LineWeight`, `LWDISPLAY`, CTB/STB, milimetros de papel ni escala de trazado; evaluada en el `DestinationEvaluationContext`; `FootprintInputFingerprint` presente y distinto de `TraceFingerprint`; huella no disponible, infinita o no garantizada conservadora ⇒ `UNKNOWN`; `XLINE`/`RAY` sin interseccion analitica ⇒ `UNKNOWN`; ST-19 con huella `UNKNOWN` ⇒ E10; orden de plan con huella `UNKNOWN` ⇒ X-22; P7a enumera tambien las instancias sin cambio de mano; `Extents3d` solo en las clases declaradas conservadoras; **ancho de modelo** (`[V11-D07]`): la geometria ocupada completa, anchos incluidos, se transforma con la transformacion compuesta (`|s|` con escala uniforme, escalas anidadas; no uniforme no soportada ⇒ `UNKNOWN`), nunca una linea media transformada mas un ancho local sin transformar; **elegibilidad** (`[V11-D01]`): primitiva no elegible ⇒ huella `UNKNOWN`; **magnitud uniforme con reflexion** (`[V12-D08]`): `MirroredX`, `MirroredY`, `(-1,1)`, `(1,-1)`, `(-1,-1)` y anidados reflejados uniformes con `ancho' = |s| · ancho`, y no uniforme ⇒ `UNKNOWN`; **entorno en la copia** (`[V12-D07]`): huella dependiente del entorno sin cota conservadora ⇒ `UNKNOWN`; dobles en Core | G6 |
| **T-M72** | Contexto de materializacion y `MaterializationContextReadSet` (`[V9-D05]`, `[V9-D19]`, `[V10-D04]`, `[V10-D05]`): el read-set contiene solo las observaciones que el productor o la huella consumen; cambiar un estado no consumido (por ejemplo `CLAYER` para un productor que fija la capa) **no** invalida; cambiar uno consumido (`DIMSTYLE` de una cota sin estilo con nombre; `TEXTSTYLE` de un `DBText`, si CT-50 lo demuestra) ⇒ E10 en PREPARE y excepcion y rollback en MUTATE; consumo no determinable ⇒ `UNKNOWN`; `Π(D)` y `Π(μ_k D)` con las mismas observaciones; productor que aplica el contexto de forma asimetrica ⇒ `UNKNOWN` (X-17a); ST-13 sigue fijando la presentacion exterior; ningun drawer existente cambia; **cierre transitivo** (`[V11-D06]`): cambiar una dependencia consumida del estilo de cota o las metricas de fuente consumidas invalida; `CANNOSCALE` no invalida salvo consumo demostrado; la `AnnotationScale` authored es intencion del plan; **entorno de la copia** (`[V12-D07]`): el estilo anotativo, `DIMSCALE = 0`, el estilo hijo y la fuente disponible o sustituida que se consumen entran en el read-set y su cambio invalida | G6 (consumo puro) y G7 (captura) |
| **T-M73** | `ModelSpaceFootprintPolicy` (`[V11-D02]`..`[V11-D04]`): clasificacion por tipo exacto (una subclase no hereda el soporte); clase, componente o estado no caracterizado ⇒ `UNKNOWN` ⇒ E10 en ST-19; `Extents3d` solo en las clases declaradas conservadoras; ninguna rama optimista por defecto; huella de orden identica con la capa apagada, inutilizada o inutilizada en ventana, con `Entity.Visible = false` o con otro `ANNOALLVISIBLE`; ocupacion dependiente del entorno `SUPPORTED` solo con sus estados declarados como consumidos y `UNKNOWN` en otro caso; **mecanismos** (`[V12-D01]`..`[V12-D06]`): mecanismo no clasificado ⇒ `UNKNOWN`; XREF descargado, no resuelto u overlay anidado no disponible ⇒ `UNKNOWN`, nunca huella vacia; recorte ⇒ ocupacion sin recortar o `UNKNOWN`; visibilidad dinamica ⇒ union conservadora de estados o `UNKNOWN`; campos ⇒ cota para todos los resultados o `UNKNOWN`; **objeto `UNKNOWN` lejano** ⇒ E10 (`[V12-D10]`); **completitud** (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`): apertura parcial ⇒ E12 en P0 sin pedir la linea, y E10 o E11 si aparece despues; enumeracion no demostrablemente completa ⇒ fallo antes de clasificar; ninguna etapa del orden logico se salta; ningun camino ejecuta PARTIALOAD, fuerza cargas, abre archivos externos o cambia capas o variables; **carga bajo demanda** (`[V13-D03]`): XREF cargado con contenido que puede no estar en memoria ⇒ `UNKNOWN`, y con contenido completo demostrado ⇒ clasificacion normal; **mecanismos añadidos** (`[V13-D05]`): atributo invisible con `ATTDISP`, objeto oculto o aislado, proxy y tabla con `DataLink` o formulas sin metodo latente conservador ⇒ `UNKNOWN`; **sesiones** (`[V14-D01]`): REFEDIT activo o Editor de bloques abierto ⇒ E12 en P0 sin cerrar sesiones, y E10 o E11 si aparecen despues; **orden** (`[V14-D04]`): P0 no evalua colas por fuente; completitud relativa a la fuente en R2 (temprana) y R6 (definitiva) con E10; R3 nunca aprueba; **frontera** (`[V14-D02]`, `[V14-D03]`): los objetos borrados no entran en la enumeracion; **marcos** (`[V14-D06]`): huella = contenido sin recortar ∪ marco que pueda hacerse visible, o `UNKNOWN`; modelos de coordinacion por tipo exacto; **politica de sesiones** (`[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`): evaluacion pura sobre la lectura del `EditingSessionAuthoritySet` con dobles: PASS solo con host caracterizado, todas las lecturas con exito, detectores `BLOCKED` inactivos y autoridades genericas sin sesion; ARRAYEDIT Source activo, ventana de prueba de bloques abierta, transaccion larga, host no caracterizado y lectura fallida o no clasificable ⇒ E12 en P0 sin cerrar sesiones, y E10 o E11 si aparecen despues; ninguna lectura fallida se trata como inactiva; ningun camino razona sobre geometria fuente temporal ni descubre modos en ejecucion; **campos por causa** (`[V15-D04]`, `[V16-D08]`): la cota se construye sobre los resultados alcanzables solo por ESTADO, con las entradas dependientes de edicion fijas en su valor actual, o `UNKNOWN`; **overrules de terceros** (`[V16-D07]`): representacion que puede estar afectada por un overrule activo sin soporte conservador ⇒ `UNKNOWN`; **XREF** (`[V15-D05]`, `[V16-D09]`): la huella usa el contenido cargado actual y su completitud, sin comparar identidades de archivos futuros; XREF no resuelto ⇒ `UNKNOWN`; dobles en Core | G6 (consumo puro) y G7 (captura) |
| **T-M74** | Ayuda sin alias (`[V11-D09]`), en `tests/RackCad.UI.Tests`: `RackCommandHelpWindow` no crea el chip de alias para una entrada con `Alias` nulo o vacio y lo sigue creando para las demas; el censo por pares representa el alias ausente como estado explicito, sin «/» final; no se inventa alias | G7 |
| **T-M75** | Censo de sitios productores de G-M24 por familias de simbolos de API (`[V12-D09]`, `[V13-D07]`, `[V14-D05]`, `[V15-D03]`, `[V16-D05]`, `[V16-D06]`): un call-site inventado que construye una entidad relevante, la añade con `AppendEntity` o `AddNewlyCreatedDBObject`, da de alta un registro en una tabla de simbolos, reevalua graficos, añade un atributo, invoca un comando o COM, abre para escritura un objeto o registro existente y muta su presentacion, escribe estado global, invoca un mutador interno de AutoCAD, registra un overrule o se suscribe a un evento que pueda causar escrituras diferidas, o delega la creacion en un helper nuevo, sin declararse ni excluirse con motivo, pone G-M24 en RED. **Cinco violaciones artificiales obligatorias:** (1) **un productor basado en clonado** (`DeepCloneObjects`, `WblockCloneObjects` o `Clone` hacia el camino de materializacion) sin clasificar ⇒ RED; (2) **una lectura de cabecera o de contexto de `Database`** (`Database.Dimstyle`, `Clayer`, `Textstyle` o equivalente) sin clasificar ⇒ RED; (3) **una mutacion de presentacion de un objeto existente** (apertura para escritura con `UpgradeOpen` o `OpenMode.ForWrite` seguida de un setter de `Layer`, `Color`, `Linetype`, `LineWeight`, `Transparency`, `Visible` o `PlotStyleName`, o de `SetPropertiesFrom`) sin clasificar ⇒ RED; (4) **un mutador interno de AutoCAD** (`BlockTableRecord.SynchronizeAttributes`, `Database.ReloadXrefs` o equivalente) sin clasificar ⇒ RED; (5) **un registro de overrule o de reactor** (`Overrule.AddOverrule` o una suscripcion a `Database.ObjectModified` o equivalente que pueda causar escrituras diferidas) sin clasificar ⇒ RED. Un productor declarado que cambia lo que asigna, usa o crea tambien; la guarda no depende de los nombres actuales de archivos o clases ni de hashes | G5..G7 |
...
| **G-M23** | Autoridad unica de huellas visuales (`[V9-D02]`, `[V10-D01]`..`[V10-D03]`): un solo productor de `VisualFootprintResult` para piezas, curvas, anotaciones, cotas y objetos de Model Space; ningun archivo del espejo usa `GeometrySymmetryResult` como huella, `TraceFingerprint` como `FootprintInputFingerprint` ni `Extents3d`/`GeometricExtents` sin la declaracion de §11.3 punto 5; ningun archivo dilata una huella por `LineWeight`, `LWDISPLAY`, grosores de CTB/STB, milimetros de papel o escala de trazado, ni construye regiones antes de transformar a WCS; ninguna huella de instancia del plan se resuelve con el contexto de la biblioteca; ningun consumidor trata una huella `UNKNOWN` como no superpuesta; ninguna clasificacion de objetos del dibujo fuera de la `ModelSpaceFootprintPolicy`, ningun fallback optimista para una clase no caracterizada, ninguna exclusion de geometria por la visibilidad transitoria en la huella de orden y ningun ancho local sin transformar (`[V11-D02]`, `[V11-D03]`, `[V11-D07]`); ninguna exclusion de contenido por carga, recorte, estado dinamico o valor actual de un campo, ninguna huella vacia por contenido ausente y ninguna magnitud reflejada tratada como no uniforme (`[V12-D01]`..`[V12-D06]`, `[V12-D08]`); ningun archivo del espejo ejecuta o solicita PARTIALOAD, cargas forzadas de contenido (por ejemplo, recargar o resolver referencias externas), la apertura de archivos externos ni cambios de capas o de variables del dibujo para completar una huella, y ninguna clasificacion de huella se ejecuta sin ST-20 ni fuera del orden logico de etapas (`[V13-D01]`..`[V13-D04]`); ningun archivo del espejo ejecuta REFCLOSE, BCLOSE ni el cierre forzado de una sesion de edicion, ni mueve entidades del working set, y ninguna huella de un objeto recortado omite el marco o contorno que pueda hacerse visible (`[V14-D01]`, `[V14-D06]`) |
| **G-M24** | Guarda durable de productores de contexto (`[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`, `[V15-D03]`, `[V16-D05]`, `[V16-D06]`), estructural y no solo de hashes de archivos, con **dos lados**: (A) productores y consumidores de contexto declarados (`LateralHeaderDrawer`, `CantileverViewMaterializer`, `LayerHelper` y todo productor que identifique CT-50), con las propiedades visuales que asigna cada uno, el uso de capas, estilos y valores por defecto, las clases de entidad que crea y la resolucion de estilos; (B) `ProducerSiteCensus` por familias de simbolos de API, no solo por `new` sintactico o `AppendEntity`, en los sitios del Plugin y del camino de materializacion: **construccion y alta** (subclases relevantes de `Entity`, `AppendEntity`, `AddNewlyCreatedDBObject`, `Activator.CreateInstance` y construccion por reflexion); **importacion y clonado** (`ReadDwgFile`, `Database.Insert`, `Wblock`, `WblockCloneObjects`, `DeepCloneObjects`, `Clone`, `GetTransformedCopy`, `Explode`, `ExplodeToOwnerSpace`); **tablas de simbolos** (`BlockTable.Add`, `LayerTable.Add`, `LinetypeTable.Add`, `TextStyleTable.Add`, `DimStyleTable.Add`, `SymbolTable.Add` y patrones de insercion de registros); **cabeceras y valores por defecto de `Database`** (`Dimstyle`, `Clayer`, `Cecolor`, `Celtype`, `Textstyle`, `Cannoscale`, `GetDimstyleData`, propiedades equivalentes, `GetSystemVariable` y `SetDatabaseDefaults`); **evaluacion grafica** (`RecomputeDimensionBlock`, `EvaluateHatch`, `GenerateLayout`, `RecordGraphicsModified` y `TransformBy` donde sean relevantes); **atributos** (`AttributeCollection.AppendAttribute`, `SetAttributeFromBlock`); **comando y COM** (`Editor.Command`, `SendStringToExecute`, `ModelSpace.Add*`, `InsertBlock` y envoltorios equivalentes); **escritura o mutacion de objeto o contexto existente**: apertura para escritura (`Transaction.GetObject(..., OpenMode.ForWrite)`, apertura o `GetObject` de un `ObjectId` para escritura, `UpgradeOpen`, `OpenCloseTransaction` y envoltorios equivalentes), mutacion de presentacion de entidades (`Layer`/`LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `Visible`, `PlotStyleName`, `SetPropertiesFrom` y `TransformBy` donde sea material), mutacion de registros de simbolo existentes (setters de `LayerTableRecord`, `LinetypeTableRecord`, `TextStyleTableRecord` y `DimStyleTableRecord`, `Database.SetDimstyleData` y equivalentes) y escritura de estado global (`SetSystemVariable` y setters de `Clayer`, `Cecolor`, `Celtype`, `Textstyle`, `Dimstyle` y equivalentes de `Database`); **mutador interno de AutoCAD** (`BlockTableRecord.SynchronizeAttributes`, `LayerStateManager.RestoreLayerState` y `Restore*`, `Database.ReloadXrefs`, `Database.ResolveXrefs`, `Database.BindXrefs`, `AssocManager.EvaluateTopLevelNetwork` y `Evaluate*`, `Database.Audit`, `BlockReference.ResetBlock`, `BlockReference.ConvertToStaticBlock` y equivalentes que se descubran); **registro de overrule o reactor** (`DrawableOverrule`, `ObjectOverrule`, `TransformOverrule` y demas subclases de `Overrule`, `Overrule.AddOverrule` y `Overrule.RemoveOverrule`, eventos de `Database` como `ObjectModified`, `ObjectOpenedForModify`, `ObjectErased` u `ObjectAppended`, y suscripciones de `Document` o `Editor` a comandos o eventos que puedan causar escrituras diferidas); lectura o resolucion de capas, estilos, valores por defecto o contexto de materializacion; y delegacion en helpers o factorias, cada call-site relevante `DECLARED` o `EXCLUDED` con motivo. Un call-site productor, consumidor, de escritura, de mutador interno o de registro de overrule o reactor nuevo sin clasificar, o un productor declarado que empieza o deja de asignar una propiedad visual, cambia ese uso, crea una clase relevante nueva o cambia la resolucion de estilos, pasa a RED hasta actualizar CT-50, el `MaterializationContextReadSet`, el `FootprintInputFingerprint` y las pruebas (T-M75); no depende solo de los nombres actuales de archivos o clases ni de hashes |
...
| **M-35** | Orden en Model Space: dos racks seleccionados que se superponen, con DRAWORDER explicito, conservan su orden relativo en las copias; un sombreado o wipeout **posterior** a la fuente que se superpone con el destino de la copia hace fallar el comando en PREPARE, sin mutacion semantica, tanto si esta seleccionado (e ignorado) como si no; una copia con relleno que cae sobre otra fuente seleccionada posterior tambien falla; una copia sobre su propia fuente queda encima (ST-19); con dos fuentes seleccionadas cuyo orden difiere en pantalla y en trazado y copias que se superponen, el comando falla; un `XLINE` o `RAY` posterior a la fuente, aunque este lejos, hace fallar el comando (L-34); dos objetos que solo se tocan por el grosor de linea visible no tienen garantia de orden (L-37); un objeto posterior en una capa apagada o inutilizada que se superpone con el destino de la copia tambien hace fallar el comando; un objeto posterior de una clase sin huella soportada (por ejemplo un proxy) o dependiente del entorno sin huella conservadora hace fallar el comando **aunque este lejos del destino**, porque `UNKNOWN` significa que no hay autoridad para demostrar la no superposicion (L-34, L-37, `[V12-D10]`); un XREF posterior descargado o no resuelto, un objeto posterior recortado cuyo contenido oculto se superpone con el destino y un bloque dinamico posterior con estados de visibilidad sin union conservadora tambien hacen fallar el comando, sin mutacion semantica (`[V12-D03]`..`[V12-D05]`); un objeto añadido a Model Space despues de la ultima enumeracion y antes de MUTATE hace fallar sin mutacion semantica; un dibujo abierto parcialmente hace fallar el comando antes de pedir la linea, con mensaje propio y sin cargar lo que falta (`[V13-D02]`); si la completitud de la enumeracion no puede demostrarse, el comando falla (`[V13-D01]`); un XREF posterior que se muestra cargado, pero cuya carga bajo demanda puede dejar contenido fuera de memoria, hace fallar el comando (`[V13-D03]`); un objeto posterior `UNKNOWN` lo hace fallar aunque parezca lejos; y un atributo invisible, un objeto oculto o aislado, un proxy o una tabla con vinculo de datos posteriores, sin metodo latente conservador, tambien (`[V13-D05]`); una edicion de referencia (REFEDIT) en curso o el Editor de bloques abierto hacen fallar el comando antes de pedir la linea, con mensaje propio y sin cerrar la sesion (`[V14-D01]`); un objeto posterior borrado antes del comando y restaurado despues con OOPS **no** esta protegido, porque es edicion futura (`[V14-D02]`, L-37); un XREF posterior cargado con el `XLOADCTL` por defecto puede hacer fallar el comando en un dibujo corriente (`[V14-D09]`); un objeto recortado cuyo marco visible se superpone con el destino tambien lo hace fallar (`[V14-D06]`); la edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) en curso o la ventana de prueba de bloques abierta hacen fallar el comando antes de pedir la linea, con mensaje propio y sin cerrar la edicion ni la ventana, y tambien lo hacen fallar un host o una version de AutoCAD no caracterizados y la lectura fallida de un detector del conjunto cerrado de sesiones (`[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`); un campo que muestra una propiedad de un objeto cuya geometria se edita despues no esta protegido frente a esa edicion, mientras que su reevaluacion solo por estado queda protegida o hace fallar el comando (`[V15-D04]`); y un XREF posterior cuya ruta pasa a otro archivo con el mismo contenido efectivo sigue protegido, mientras que uno resuelto a un contenido efectivo distinto es edicion futura (`[V15-D05]`) |
...
| **M-37** | Contexto de materializacion: con `CLAYER`, `CECOLOR`, `CELTSCALE` o `DIMSTYLE` distintos de los que habia al dibujar la fuente, la copia se regenera como Actualizar (piezas internas, anotaciones y cotas con las observaciones que consume cada productor) y la presentacion exterior de la referencia sigue siendo la de la fuente (ST-13); durante la peticion de la linea, cambiar un estado que el productor consume (por ejemplo `DIMSTYLE` con cotas sin estilo con nombre) hace fallar sin mutacion semantica (PRE-17), y cambiar uno que no consume no lo hace (read-set, segun caracterice CT-50); cambiar el estilo de texto del estilo de cota vigente, o la fuente del estilo de texto consumido, durante la peticion de la linea hace fallar; cambiar `CANNOSCALE` no lo hace salvo consumo demostrado; la copia se regenera con la `AnnotationScale` authored del rack; con un estilo de cota o de texto anotativo, un estilo con nombre con `DIMSCALE = 0` o una fuente sustituida, la copia se regenera o falla cerrado segun caracterice CT-50 (`[V12-D07]`) |

---

## 14. Proceso y gates

### 14.1 Secuencia normativa (sustituye a la de V10)
...
→ rebase / reconciliacion sobre el main VIGENTE         (solo docs; hoy main = dad4e77 con I-53 E1, I-53S
                                                          ...
                                                          §15.1)
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55 ↔ I-49, con las ultimas entradas X-*
                                                          acordadas como entrada vinculante: hoy la tabla X-1..X-8
                                                          acordada en la revision tecnica adversarial de G2C
                                                          (d091eeb), que la Proposal V3 de I-55 reclasifico como no
                                                          formal, aceptada del lado de I-55, con las diferencias de
                                                          sus Proposals V3 y V4 pendientes de revision formal y
                                                          reconciliacion, §15.4; registra la propiedad de las
                                                          autoridades compartidas, todas PROVISIONAL UNTIL
                                                          CROSS-INITIATIVE RECONCILIATION en V16: taxonomia de
                                                          tipo de vista, codec semantico de View/Section,
                                                          «sistema resuelto → plan de vista» (Resolve + Plan) con
                                                          origen y tramo del eje de la vista, DescribeView y
                                                          ViewExposure, comparador authored, primitivo de
                                                          materializacion del Plugin, valor de colocacion y
                                                          transformaciones de proyeccion, nucleo de seleccion y
                                                          caracterizaciones unicas CT-04, CT-05 y CT-16, con sus
                                                          ubicaciones fisicas tambien provisionales; sin
                                                          autoridades duplicadas; [V11-D10], [V12-D11], [V13-D08],
                                                          [V14-D08], [V15-D08])
     autoridad incompatible congelada por cualquiera antes de reconciliar → STOP: ningun freeze para ninguna
     desviacion de una entrada X-* acordada → se vuelve a acordar con I-55 antes del freeze; nunca unilateral
     ADR-0042 aceptado antes del freeze de I-52 → releer la autoridad de C-2 y de Actualizar y reconciliar
→ estado formal de I-55 resuelto                          (revision formal de su Proposal vigente, hoy V4
                                                          fe70d7a, y estado de las entradas X-* y de las
                                                          diferencias de V3 y V4 registrado; d091eeb no es
                                                          consenso formal; [V15-D08], [V15-D13], [V15-D14],
                                                          [V16-D14])
→ autoridad FINAL de I-49 disponible para RS-3            (V6 + A1 + A2 + A3 revisado por A3-R1, ADR de reemplazo
                                                          de ADR-0041, Consensus Freeze nuevo y estado de
                                                          implementacion, acordados, aceptados, congelados y
                                                          releidos; ADR-0041 D19 no es la autoridad de RS-3;
                                                          [V14-D07], [V15-D06])
     no disponible → RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED
     I-49 cancelada o abandonada antes → sin desbloqueo automatico ni eliminacion silenciosa de RS-3; FREEZE
       BLOCKED hasta que el Coordinador decida en una Proposal: A (retirar de I-52 el soporte futuro de
       expresiones de I-49), B (adoptar otra autoridad integrada) o C (mantener el freeze bloqueado); R-55,
       [V15-D07]
→ SOLO ENTONCES freeze por SHA exacto + re-check exact-SHA   (Coordinador y Arquitecto)
     si el rebase o la reconciliacion cambian contenido normativo material → vuelve a revision
...
→ G3 characterization                                    (ADR-0036 NO es precondicion; sonda solo fuera de src/;
                                                          I-55 sin implementacion: solo reconciliacion documental)
...
→ G4 implementation                                      (ADR-0036 aceptado ES precondicion; aviso previo a I-49 e
                                                          I-55 antes de tocar la fundacion compartida)

Proceso de I-52 ([V12-D13], [V13-D10], [V13-D14], [V14-D13], [V15-D09], [V15-D13], [V16-D10], [V16-D14]): I-52 esta
grandfathered y sigue el workflow bajo el que se reclamo. Antes de cada transicion se relee la autoridad de proceso
vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante
grandfathered aplique, I-52 conserva su workflow de reclamo, salvo que una autoridad de proceso integrada cambie
expresamente las reglas de transicion aplicables a las iniciativas grandfathered. Nunca se adoptan reglas de I-56 no
integradas; ninguna regla de las Proposals V1 y V2 de I-56 se aplica a I-52
```
...
de una caracterizacion o de una prueba con el contrato congelado ⇒ STOP → Proposal V17**; nunca un parche silencioso.

| Gate | Entrada | Archivos permitidos | RED esperado | PASS requerido | Parada | Evidencia |
|---|---|---|---|---|---|---|
| **G2** (este) | revision de V15 | `docs/**` | — | consenso de Coordinador y Arquitecto sobre la misma version (V16); despues, rebase/reconciliacion sobre el main vigente, **reconciliacion obligatoria entre iniciativas** (I-55 e I-49 donde aplique; propiedad de las autoridades compartidas marcadas como provisionales, con la ultima tabla X-1..X-8 acordada como entrada vinculante, las diferencias de las Proposals V3 y V4 de I-55 por decidir y el estado formal de su Proposal vigente resuelto; `[V11-D10]`, `[V12-D11]`, `[V13-D08]`, `[V14-D14]`, `[V15-D08]`), relectura de la autoridad de proceso vigente con sus reglas de transicion (`[V12-D13]`, `[V13-D10]`, `[V16-D10]`), autoridad final de I-49 disponible para RS-3 o freeze bloqueado, sin desbloqueo automatico si I-49 se cancela (`[V14-D07]`, `[V15-D06]`, `[V15-D07]`), freeze por SHA, re-check normativo y **O-1** del Owner sobre la envolvente de §1.3; contrato reescrito; hallazgos laterales en `ideas-futuras.md` | desacuerdo material ⇒ Proposal V17 | SHA acordado, SHA de freeze + CI |
| **G3** Characterization | freeze de G2 + O-1 aceptada (**sin** ADR aceptado) | `tests/RackCad.Tests/**` (caracterizaciones y fixtures nuevos); `tests/RackCad.UI.Tests/**` si una caracterizacion lo exige; `docs/automation/evidence/**`; la viabilidad de la sonda se demuestra **fuera de `src/`** (`tools/`, harness externo o evidencia del Owner) **en `acad.exe` / AutoCAD 2025**, en la ubicacion que fije el Coordinador al abrir G3; **ninguna sonda productiva en `src/`** | ninguno: CT-01..CT-50 pasan sobre produccion intacta | Core completo (UI si se toco); **viabilidad visual-geometrica de la sonda en `acad.exe` (CT-36: politica cerrada de clases y variantes con inventario cerrado, huellas visuales de ocupacion de modelo en el contexto destino (DRAWING FIRST, colisiones de nombres) y su frescura tras `EnsureForPlan`, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, contexto de la base auxiliar STB y CTB, `VisibilityPath` con estados de capas, ancestros y anotativos, `LinetypeScale` y contexto de patron, fase, segmentacion y multiplicidad, orden visual dentro de la pieza, SOLID/TRACE/HATCH con `HatchLoopTypes`, `applied` y punto de observacion, valores efectivos, `Origin` y transformacion efectiva, ruta anidada estable, degradados, clonado por `EnsureBlocks` con la cache intacta, identidad SHA-256, `DBMOD`/`HANDSEED`/UNDO/`WorkingDatabase`) antes que `PlanPlacementEvidence` (CT-06)**; corpus indicativo con la huella de la biblioteca (CT-47); orden visual de plan por kind y vista con pares invertidos y huellas (CT-48); presentacion de la referencia fuente con inventario cerrado, huellas y orden en Model Space frente a todo objeto posterior, con ordenes de pantalla y de trazado contradictorios y `XLINE`/`RAY` (CT-49); read-set del contexto de materializacion (CT-50); elegibilidad de la presentacion resuelta, `ModelSpaceFootprintPolicy`, huella de orden independiente de la visibilidad, ocupacion dependiente del entorno, cola posterior, cierre transitivo de texto y cota, anchos de modelo transformados y ayuda sin alias con los casos de `[V11-D13]` (CT-36, CT-49, CT-50); huella latente por mecanismo con XREF, recortes, visibilidad dinamica y campos, entorno de la copia, magnitud uniforme con reflexion, censo inicial de sitios productores y objeto `UNKNOWN` lejano con los casos de `[V12-D14]` (CT-36, CT-49, CT-50); completitud de la base de datos con dibujo completo y abierto parcialmente, carga bajo demanda de XREF, atributos invisibles y `ATTDISP`, aislamiento de objetos, proxies, `DataLink` y formulas de tabla, y productor artificial por clonado con los casos de `[V13-D12]` (CT-49, CT-50, T-M75); I-55 sin implementacion, solo reconciliacion documental posterior; ERASE/OOPS solo caracterizado y fuera de la garantia, sesiones de edicion (REFEDIT activo y cerrado, Editor de bloques y Model Space normal; ARRAYEDIT Source activo y cerrado; ventana de prueba de bloques activa y cerrada; conjunto cerrado de detectores con exito; lectura fallida de un detector ⇒ E12; host o version no soportados ⇒ E12; `ARRAYEDITSTATE` activo e inactivo; autoridad relevante de comando o de sesion activa ⇒ E12; modo relevante sin detector fiable ⇒ STOP de caracterizacion), estado frente a edicion (XREF al mismo o a otro contenido, mismo contenido cargado por otra ruta, archivo externo reemplazado sin recargar, recarga tras cambiar el archivo, XREF no resuelto actual, XREF editado y recargado, `DataLink` con fuente cambiada, visibilidad dinamica, tabla de busqueda solo de visibilidad y tabla de busqueda que cambia visibilidad y geometria o estiramiento; campo de propiedad de objeto con el objeto referenciado editado frente a una reevaluacion solo por estado; campos mixtos `Area + Date`, `Length + DataLink`, propiedad anidada editada + entrada de estado y estado de XREF + dependencia editada; overrule activo de terceros ⇒ `UNKNOWN` salvo soporte conservador), marcos de recorte y lectura de cabecera, productor por clonado, apertura para escritura con mutacion de presentacion, mutador interno y registro de overrule o de reactor en rojo con los casos de `[V14-D11]`, `[V15-D10]` y `[V16-D11]` (CT-49, CT-50, T-M75); I-49 sin implementacion, solo el requisito de reconciliacion; cierre transitivo sobre el contrato real, enums, retirados con RM-7 y guardas cruzadas (CT-31, CT-32, CT-42, CT-46); campos start/end de cabecera (CT-45); tabla de `c`; auditoria de anclajes por pieza (CT-17); semantica observable de I-51; decodificacion de secciones e historia de `View`; lineas base de store; topes, con el tope posterior de Push Back activo por defecto y `OffCells` latentes (CT-15, CT-43); estabilidad 9b incondicional con casos A/B, cotas, cambio de gobernante, BOM, firmas de estado y DEP-10a..c (CT-27, CT-28, CT-37, CT-38, CT-44); S-14 asociado; enumeracion `A_k`; tarimas del Selectivo y de Push Back (CT-20); precedencia de fuente (CT-39); read-set (CT-40); costuras de Actualizar e Insertar (CT-26); baseline de I-53 y clasificacion de I-53S (CT-41); fuentes dinamicas/anonimas; V-RT-STORE separado de C-2; **regla de cierre: ninguna fila `G3_PENDING` ni `EvidenceStatus` implicito** (§7.0) | contradiccion material con V16; sonda no viable, sin paridad, sin punto de observacion o sin contexto reproducible; clases o variantes reales no soportadas; huellas visuales no garantizables o contexto de evaluacion destino no simulable en clases enteras; productor asimetrico del contexto de materializacion o consumo no determinable; biblioteca real que reduce materialmente la envolvente mas alla de O-1; canonicalizacion materialmente ambigua ⇒ **STOP → Proposal V17** | SHA + conteos + CI + registros de evidencia con la huella de la biblioteca. Tras PASS: se pide **O-4** |
| **G4** Geometria y nucleo neutral | G3 PASS **y ADR-0036 aceptado por el Owner** | `src/RackCad.Application/Geometry/*`; nucleo neutral y fachada (**PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V14-D08]`): la ubicacion en `src/RackCad.Application/Persistence/` es probable y no se congela; X-1) (incluye `RackDuplicationPlan.cs`, **con aviso previo a I-49 y a I-55** y con la propiedad del nucleo neutral fijada en la reconciliacion entre iniciativas, `[V11-D10]`, `[V12-D11]`); pruebas | T-M01..T-M05, T-M21 y T-M41 (comparadores puros) en rojo sobre andamiaje | verdes; T1–T15 de I-51 y CT-16 identicos (textos, orden, avisos, errores); `RackDuplicarCommands.cs` sin cambios; tolerancia de escala fijada y registrada; **sin cambio observable de RACKDUPLICAR** | cambio observable de RACKDUPLICAR | SHA + conteos + CI |
| **G5** Reflectores y representabilidad | G4 | contrato, registros y tablas de clasificacion propios del espejo en `src/RackCad.Application/Mirror/`; reflector por kind en `src/RackCad.Application/Systems/<Kind>/` y `RackFrames/` (nunca en `Systems/Shared`); **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V14-D08]`): `DescribeView`, `ViewExposure`, `IsSameAuthority`, el codec de vistas y `A_k` no se ubican ni se fijan en su forma de API hasta la reconciliacion (X-2, X-3, X-8); contrato de datos `GeometrySymmetryResult` (valores efectivos, variantes y firma visual tipada) y su consumo puro; declaracion de invariancia de metadata vacia; allowlist de portadores; registro de retirados con RM-7; autoridad V-DEP con DEP-09 y DEP-10a..c; guarda sobre el contrato de serializacion; pruebas. Divisible en G5a Selectivo, G5b Dinamico y Push Back, G5c Cantilever y cabecera | T-M06..T-M11, T-M18 (portador), T-M22, T-M24, T-M25, T-M28, T-M31, T-M32, T-M34, T-M35, T-M36, T-M38, T-M39, T-M42..T-M45, T-M47, T-M49..T-M52, T-M58, G-M18 y G-M22 (archivos de Application) en rojo | verdes; sustrato real; topes Selectivo y Push Back `UK`; H-06 `UK`; H-10..H-12 clasificadas; obligaciones 9 y 9b; V-DEP; V-META del payload, exterior, retirados, enums y colisiones; cierre transitivo con enums; guardas de texto vigentes; sin `G3_PENDING`; `WithDesign` ausente | una fila `R` o `RN` resulta no equivalente ⇒ STOP → Proposal V17 | SHA por subgate + CI |
| **G6** Planes, materializador y **C-2** | G5 | autoridades de plan por kind en Application; `SystemBlockWriter.cs` (`CreateInTransaction`); materializador nuevo en `src/RackCad.Plugin/Systems/Shared/`; **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V14-D08]`): la ubicacion de `Resolve` + `Plan` (incluida `Application.Views`), del primitivo de materializacion y de `CreateInTransaction`, y su forma de API, se fijan en la reconciliacion (X-2, X-4, X-6); **sonda visual-geometrica productiva en el Plugin** si la division final la situa aqui (si no, en G7), con costura unica de parametros compartida con el materializador o prueba T-M56; pruebas Core y `tests/RackCad.UI.Tests/**` para C-2 | T-M12..T-M14, **T-M17**, **T-M17b**, T-M23, T-M26, T-M27, **T-M29**, **T-M30**, T-M33, T-M40, T-M48, T-M53..T-M57, T-M59..T-M67, T-M70 y T-M71 (si la sonda nace aqui), T-M72, T-M73 (consumo puro) y T-M75, G-M9, G-M16, G-M17 y G-M19..G-M24 en rojo | conmutacion y V-VIEW-ALL verdes con resultados visual-geometricos y orden visual de plan sobre huellas; `MissingInstances` = fallo duro; **C2-1, C2-2a, C2-2b, C2-3 y C2-4 GREEN en CI** (Core y UI; C2-4 sobre el arbol combinado con I-53S, ya integrada en main, `[V8-D17]`, y con I-53D, integrada en main antes del Candidato (`dad4e77`), incluidos T-M17 y T-M17b del Dinamico, `[V10-D15]`, `[V10-D23]`) con G-M9, nombre logico, estado efectivo, `View` y `Section` canonicas; builders y drawers sin cambio de comportamiento; Application sin `Autodesk.*`. **G6 no cierra sin C-2** | necesitar cambiar un builder o un drawer existente; divergencia de C-2 ⇒ decidir C-1 con Coordinador y Arquitecto antes de cerrar G6 | SHA + CI + build Debug de Plugin |
| **G7** Comando RACKMIRROR | G6 cerrado (**C-2 verde**, incluido C2-2b) | `src/RackCad.Plugin/RackMirrorCommands.cs` (nuevo) y SNAPSHOT propio; sonda visual-geometrica productiva si no nacio en G6; archivos de guardas; `SelectiveEditorOpenTests.cs` (censo `B + 1`; hoy 35 → 36), `CustomPropertiesCommandGuardTests.cs` y `CustomPropertiesHelpCensusTests.cs` (T-GRD-08 de I-54, en main, reapuntados con motivo y sin debilitar: `RACKMIRROR` por nombre, entrada de ayuda sin alias y alias verificado solo si no esta vacio, `[V10-D08]`); `src/RackCad.UI/RackCommandReference.cs` (entrada de `RACKMIRROR` sin alias); `src/RackCad.UI/RackCommandHelpWindow.cs` (sin chip de alias para una entrada sin alias) y su prueba de UI (T-M74, `[V11-D09]`). `RackDuplicarCommands.cs` **solo** si se decide extraer un SNAPSHOT compartido (§5.2); **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** (`[V14-D08]`): cualquier helper de SNAPSHOT o de seleccion compartido y su ubicacion se fijan en la reconciliacion (X-1) | T-M15, T-M16, T-M37, T-M41 (MUTATE), T-M46 (captura), T-M68 y T-M69, T-M72 y T-M73 (captura), T-M74, T-M75, T-M53..T-M57, T-M59..T-M67, T-M70 y T-M71 (si la sonda nace aqui), G-M1..G-M8 y G-M10..G-M24 en rojo por violacion temporal | guardas verdes; censo `B + 1` con `B` re-medido (`[V9-D06]`), ayuda +1 entrada y registros de alias +0 (`[V10-D08]`); presentacion de la referencia fuente (ST-13, ST-18), orden en Model Space frente a todo objeto posterior con frescura tras `EnsureForPlan`, cola de Model Space enumerada de nuevo en R6 y en MUTATE y huellas de orden por la `ModelSpaceFootprintPolicy` con sus mecanismos (ST-19, PRE-18, `[V11-D02]`..`[V11-D05]`, `[V12-D01]`..`[V12-D06]`), censo de sitios productores de G-M24 verde (`[V12-D09]`) y read-set del contexto de materializacion (PRE-17) verdes; build Debug de Plugin; **rendimiento de la sonda medido (R-24)**; mensajes R-A/R-B fijados (T-M50); sin extraccion, archivos y guardas de RACKDUPLICAR intactos (G-M12) | C-2 no verde ⇒ G7 no arranca; debilitar una guarda de I-51, de main (incluidas las de I-53) o de texto (G-M18) | SHA + conteos + CI |
...
### 15.1 main y paralelas observadas en el preflight de V16
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ba497f1, que integraba I-54 sobre 104ef3a; sin cambios desde V11;
                                                     ancla de las citas de V16: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = ae8640aac1ca2b06a21305e91bab591d9d5f2925 (V15; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = c8cfee28ce568856728393515314b0b4a6a03243 (sin cambios: A3-R1,
                                                     solo docs, CI 34886985747 success, sobre A3 f6f0991, cuya
                                                     revision exacta del Arquitecto fue CHANGES REQUIRED; resuelve
                                                     MAT-1..MAT-5; segun A3-R1 cambian directamente D20, D18
                                                     puntos 1-4 y D14 decision 7 de ADR-0041 y D19 se lee con A3;
                                                     re-revision exacta del Arquitecto y Owner PENDING;
                                                     REPLACEMENT ADR REQUIRED, sin numero; Consensus Freeze nuevo
                                                     pendiente; G6 CLOSED en 75f1862; G7 y G8 BLOCKED; sin
                                                     integrar)
I-55 feature/creacion-de-vistas                    = 806836883ef18d06644cb69099aa21b8e0a875f1 (sin cambios desde
                                                     V15; G2E: Proposal V4 fe70d7a, CI 34889818061 success,
                                                     con la decision CQ-01 de su Coordinador (redibujo atomico de
                                                     hermanas antes de Insertar), mapa V4 y ADR-0042 revisado, y
                                                     paquetes de revision V4 8068368, CI 34890384637 success;
                                                     Coordinador de I-55 REVIEW REQUIRED; Arquitecto formal
                                                     PENDING; G2C sigue como revision tecnica adversarial no
                                                     formal; diferencias de V3 y V4 sin adoptar; ADR-0042
                                                     propuesto; solo docs; sin produccion; sin integrar)
I-56 docs/initiative-workflow-v2                   = f97a8b0bfe5813740ac250c1eb8a75ffaa2ae9a8 (fde0386 en el preflight;
                                                     Initiative Workflow V2:
                                                     G2.1 Proposal V2 fde0386, CI 34893284825 success, y G3
                                                     Architect Review de la V2 f97a8b0, CI 34895649923 success;
                                                     Architect CHANGES REQUIRED — PROPOSAL V3; Coordinator sobre V2
                                                     AGREED; NOT CONSENSUS; Owner NOT REQUESTED; WORKFLOW V2 NOT
                                                     EFFECTIVE; I-49, I-52 e I-55 grandfathered; solo docs; sin
                                                     integrar)
...
Censo de [CommandMethod(                          = main dad4e77: 35 · I-49: 35 · I-55: 35 · I-56: 35 · I-52: 33
ADR 0035..0042                                     = 0035, 0037 y 0039 aceptados en main; 0036 propuesto solo en I-52;
                                                     0038 y 0040 reemplazados y 0041 aceptado en I-49; 0042 propuesto
                                                     en I-55; I-56 sin ADR; sin numeros posteriores ni colision
Biblioteca de evidencia                            = SHA-256 b4ca2248…d560e8, sin cambios
Ramas nuevas                                       = ninguna
Produccion nueva                                   = ninguna (la ultima, G6-C2 de I-49, ya la registro V12)
Numeros de ADR reservados                          = ninguno publicado (el ADR de reemplazo de I-49 no tiene numero)
```
...
**main desde la publicacion de V15.** Sin cambios: `dad4e77`. V16 no añade citas de codigo nuevas: los censos de
mutadores internos, overrules y suscripciones a eventos de AutoCAD se miden en `dad4e77` sin sitios.

**main entre la publicacion de V14 y la de V15 (registrado por V15).** Sin cambios: `dad4e77`. Las citas de codigo
nuevas de V15 son el censo inicial de escrituras de CT-50, medido en `dad4e77`; las de los productores declarados
(`LateralHeaderDrawer.cs`, `CantileverViewMaterializer.cs` y `LayerHelper.cs`) son identicas en `1b091be`.

**main entre la publicacion de V13 y la de V14 (registrado por V14).** Sin cambios: `dad4e77`. Las citas de codigo
nuevas de V14 (`LateralHeaderDrawer.cs:244`, `:375` y `:391`; `LayerHelper.cs:26`; `CantileverViewMaterializer.cs:48`,
`:111` y `:195`) son identicas en `1b091be` y en `dad4e77`.

**main entre la publicacion de V12 y la de V13 (registrado por V13).** Sin cambios: `dad4e77`. V13 no añade citas de
codigo nuevas: su contenido nuevo es de proceso, de contrato y de caracterizacion en AutoCAD 2025 (CT-49).

**main entre la publicacion de V11 y la de V12 (registrado por V12).** Sin cambios: `dad4e77`. Ninguna cita de V12 se
desplaza respecto de V11; las citas nuevas de V12 (`LateralHeaderDrawer.cs:62`, `:147` y `:313`) se leen en `dad4e77` y
son identicas en `1b091be`.

**main entre la publicacion de V10 y la de V11 (registrado por V11).** Sin cambios: `dad4e77`. Ninguna cita de V11 se
desplaza respecto de V10; las citas nuevas de V11 (`LateralHeaderDrawer.cs:266-280`, `:375`, `:385`;
`SelectiveAnnotations.cs:15`, `:21`; `DynamicViewDecorations.cs:53`; `RackCommandHelpWindow.cs:90-104`) son identicas en
`1b091be` y en `dad4e77`, y `CustomPropertiesHelpCensusTests.cs:106` se lee en `dad4e77`.

**Cambios de paralelas desde la publicacion de V15.**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-56 `fde0386`** | G2.1: Proposal V2 de Workflow V2 (`docs/initiatives/I-56-proposal-v2.md`, un archivo nuevo, solo documentacion), que reconcilia CR-01..CR-08 del Coordinador de I-56 sobre la Proposal V1: PROPOSAL V2 — NOT CONSENSUS; Coordinator REVIEW REQUIRED; Architect NOT OPEN; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE y `WORKFLOW_V2_EFFECTIVE_SHA` inexistente; declara que no modifica ningun contrato activo (I-49, I-52, I-55) y que no se aplica a sus reclamos, ramas y contratos existentes; la regla que propone para iniciativas grandfathered solo regiria con la V2 efectiva y aprobada; CI `34893284825` success | **NON-MATERIAL**: ninguna regla de la Proposal V2 se aplica a I-52 ni cambia una regla de transicion vinculante hoy; solo se actualizan los hechos (`[V16-D10]`) |
| **I-56 `f97a8b0`** | durante la redaccion de V16 (encontrado en el re-fetch previo al commit), G3: Architect Review de la Proposal V2 exacta (`docs/initiatives/I-56-architect-review-v2.md`, un archivo nuevo, solo documentacion; la Proposal V2 no se modifica): 0 BLOCKER, 10 HIGH, 21 MEDIUM y 9 LOW; ARCHITECT = CHANGES REQUIRED — PROPOSAL V3; Coordinator sobre V2 = AGREED; Consensus NOT REACHED; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE; CI `34895649923` success | **NON-MATERIAL**: una revision de una Proposal sin consenso no crea reglas vinculantes ni cambia el invariante grandfathered de I-52; solo se actualizan los hechos (`[V16-D10]`, `[V16-D15]`) |
| **I-49 `c8cfee2`** | sin cambios desde V15 | contrato NON-MATERIAL y coordinacion MATERIAL, como en V15 |
| **I-55 `8068368`** | sin cambios desde V15 | contrato NON-MATERIAL y coordinacion HIGH / ACTIVE, como en V15 |

**Cambios de paralelas entre la publicacion de V14 y la de V15 (registrados por V15).**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-49 `c8cfee2`** | A3-R1: corrige el Amendment A3 tras la revision exacta de su Arquitecto (CHANGES REQUIRED, MAT-1..MAT-5; notas N-3..N-5), con un commit nuevo sobre `f6f0991` que solo edita el archivo de A3 (V6, A1, A2, ADR-0041 y los freezes no se editan): `RootCauses` por SCC (MAT-1); sugerencia de recuperacion de §7.2 solo con fuentes `Upstream` y raices iguales, con razones estructuradas de bloqueo M10(d) (MAT-2); aridad estatica antes de evaluar (MAT-3); `CycleRoot` solo en un ciclo simple (MAT-4); compatibilidad con V6 limitada a los escenarios lineales enumerados (MAT-5); censo ampliado: cambian directamente ADR-0041 D20, D18 puntos 1-4 y D14 decision 7, y se leen con A3 D4, D8, D11, D14 decisiones 1 y 6, D15, D18 puntos 5-7, D19, D20 (pruebas), D22, D23, D24 y Consecuencias; pruebas T-A3-22..T-A3-50; REPLACEMENT ADR REQUIRED; re-revision exacta del Arquitecto y Owner PENDING; G7 y G8 BLOCKED; CI `34886985747` success | contrato de I-52 **NON-MATERIAL**: RS-3 delega en la autoridad final de I-49, que sigue pendiente; I-52 no consume D18 ni D14 decision 7; que A3-R1 clasifique D19 como «leida con A3» no cambia que la semantica futura de las observaciones del `PlanReadSet` cambia (M3, M4 y M10(a)); coordinacion **MATERIAL**: RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED; V15 corrige la redaccion residual sobre D19 y añade la salida ante la cancelacion de I-49 (`[V15-D06]`, `[V15-D07]`, `[V15-D14]`); sin STOP |
| **I-56 `18401da`** | G2: Proposal V1 de Workflow V2 (`docs/initiatives/I-56-proposal-v1.md`, un archivo nuevo, solo documentacion): PROPOSAL V1 — NOT CONSENSUS; Coordinator y Architect REVIEW REQUIRED; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE y `WORKFLOW_V2_EFFECTIVE_SHA` inexistente; declara que no modifica ni se aplica a los contratos activos de I-49, I-52 e I-55 (grandfathered) y cita a I-52 solo como control del Evidence Audit; CI `34887275886` success | **NON-MATERIAL**: ninguna regla de la Proposal V1 se aplica a I-52; solo se actualizan los hechos (`[V15-D09]`) |
| **I-55 `8068368`** | durante la redaccion de V15 (encontrado en el re-fetch previo al commit), G2E: Proposal V4 (`fe70d7a`, CI `34889818061` success), mapa de implementacion V4, ADR-0042 revisado, contrato y registro de I-55 y paquetes de revision V4 de Coordinador y de un Arquitecto independiente (`8068368`, CI `34890384637` success); solo docs. La decision vinculante CQ-01 de su Coordinador sustituye el redibujo de hermanas confirmado por vista de la V3 por un redibujo atomico antes de Insertar (PREPARE sin escribir estado del rack → una MUTATE para todos los redibujos y los borrados de huerfanas → un commit → POST) y deja la colocacion de vistas nuevas en una transaccion por jig tras ese commit; ADR-0042 decision 6 y alternativas precisadas; Actualizar sin cambio. V4 §17.2 contrasta con I-52 V14 y conserva X-1..X-8 y sus reglas de extraccion, con tres precisiones: X-4 con dos operaciones de la misma familia (crear, de I-52 G6, y redefinir en la transaccion del llamador, existente de I-47 G9, que consume el redibujo atomico de I-55 G9a y cuyos sitios nuevos se declararian en G-M24), X-5 con reporte en G9a y G9b y los disparadores de V14 §15.3, y X-6 con productor G9b, mapa de invocaciones G-M9/CT-26 de Insertar cambiado y C2-2b con redibujo todo o nada; tabla de adopcion para este registro (§17.2.1), pendiente de I-52; Arquitecto formal PENDING; Owner PENDING | contrato de I-52 **NON-MATERIAL**: ADR-0042 sigue propuesto y V15 ya condiciona C-2, Actualizar e Insertar a su aceptacion antes del freeze; la operacion de redefinir no es parte de lo que I-52 extrae, y la familia de escritura de G-M24 (`[V15-D03]`) cubre sus sitios; coordinacion **HIGH / ACTIVE**: la reconciliacion decide adoptar, volver a acordar o escalar las diferencias de V3 y V4, y la segunda iniciativa en integrar re-establece C-2 con el Insertar vigente entonces; V15 no las adopta (`[V15-D08]`, `[V15-D14]`); sin STOP |

**Cambios de paralelas entre la publicacion de V13 y la de V14 (registrados por V14).**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-56 `0d66df2`** | G1 `c6839e2`: Evidence Audit del flujo de iniciativas (`docs/initiatives/I-56-evidence-audit.md`, solo documentacion; muestra de iniciativas cerradas con I-52 como control activo no agregado; hipotesis H1-H9); G1.1 `0d66df2`: corrige la clasificacion normativa del audit en restricciones vinculantes de I-56 (H.1), politica V1 vigente (H.2) y alternativas rechazadas o diferidas (H.3), sin cambiar cifras; Evidence Audit COMPLETE; Proposal NOT STARTED; no modifica documentos normativos compartidos, producto, pruebas, CI ni iniciativas activas; CI `34877304129` y `34879848606` success | **NON-MATERIAL**: el audit no es politica; ninguna hipotesis cambia I-52, que sigue grandfathered con WORKFLOW V2 NOT EFFECTIVE; solo se actualizan los hechos (`[V14-D10]`) |
| **I-49 `f6f0991`** | sin cambios desde V13 | contrato NON-MATERIAL; V14 corrige el alcance de A3 (D19 y D20) y hace de la autoridad final de I-49 una condicion del freeze (`[V14-D07]`) |
| **I-55 `110cd39`** | durante la redaccion de V14, G2D: Proposal V3 (`8e35a51`, CI `34881178359` success), mapa de implementacion V3, ADR-0042 revisado y paquetes de revision V3 de Coordinador y Arquitecto independiente (`110cd39`, CI `34881680833` success); solo docs. El Coordinador de I-55 reclasifica la revision de G2C como revision tecnica adversarial (no veredicto formal del Arquitecto), acepta sus hallazgos y la tabla X-1..X-8 del lado de I-55; Architect formal PENDING. V3 §17.2 contrasta con I-52 V13 y propone diferencias: X-1 primer gate simetrico (I-52 G4 o I-55 G13); **X-2 y X-8 con extractor unico I-55** (G6, G7a, G7b), lo que acopla G4..G6 de I-52 a esos gates o exige una excepcion registrada; X-4 añade un requisito estructural puro (`A/Drawing/LibraryBlockRequirement.cs`) y una consulta tras importar, por primer gate; X-5 añade comprobaciones de I-52 antes de G4 y G6; X-7 añade la descomposicion de la transformacion fuente y la tolerancia de escala; y una tabla de adopcion para el registro de I-52 (§17.2.1). Declara que no hay MATERIAL CONFLICT de contenido y que un rechazo del acoplamiento de X-2 se escala a los dos Coordinadores. ADR-0042 sigue propuesto y complementa a ADR-0010, con nota posterior fechada solo si se acepta | contrato de I-52 **NON-MATERIAL**: las autoridades afectadas y sus ubicaciones ya son provisionales y V14 no fija su propiedad; coordinacion **HIGH / ACTIVE**: las diferencias de V3 no estan acordadas y la reconciliacion debe aceptarlas, volver a acordarlas o escalarlas; V14 no las adopta (§15.4); sin STOP (`[V14-D14]`) |

**Cambios de paralelas entre la publicacion de V12 y la de V13 (registrados por V13).**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-55 `d091eeb`** | G2C sobre la Proposal V2 (`f84f303`, paquete `1e12414`): orden G2C del Coordinador de I-55 y revision de su Arquitecto = CHANGES REQUIRED — PROPOSAL V3 (sin BLOCKER ni HIGH; MEDIUM AR2-01..AR2-06; LOW AR2-07..AR2-20); A-8: ADR-0042 como complemento de ADR-0010, con nota posterior fechada en ADR-0010 si se acepta y redaccion «complementa y precisa» a fijar en su Proposal V3 (el archivo en `d091eeb` aun dice «complementa y enmienda»); reconciliacion X-1..X-8 **AGREED** (8 de 8) con custodio, iniciativa que extrae, gate de creacion, orden de consumo y riesgo, sin MATERIAL CONFLICT, contrastada con la Proposal V12 de I-52 (ningun veredicto cambia) y con los requisitos del lado de I-52 a adoptar antes de su freeze; hallazgo H-14 fuera de I-55; Coordinador de I-55 sin AGREED y Owner PENDING; CI `34870781445` success; solo docs | arquitectura de I-52 **NON-MATERIAL**: ningun X presenta conflicto material y V12 ya dejaba el reparto a la reconciliacion; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**. V13 registra X-1..X-8 como entrada vinculante de la reconciliacion y amplia la marca provisional (§15.4, `[V13-D08]`) y corrige los hechos de I-55 (`[V13-D09]`); sin STOP |
| **I-49 `f6f0991`** | durante la redaccion de V13, Amendment A3 de V6 (`docs/initiatives/I-49-proposal-v6-amendment-a3-multi-cause-dependency-failures.md`, un archivo nuevo; sin produccion, pruebas, ADR ni freezes): el STOP de su G7 antes de RED encontro que V6 solo define el fallo lineal; A3 fija U1-U4 y el modelo M1-M10 (diagnosticos de un simbolo no evaluado, orden, `DependencyFailed` con cadena representativa, `RootCauses` derivado y, en M10, `RootCauses` como dato estable adicional de la observacion del `PlanReadSet` y como dato de `Upstream`); cambia el dato estable `Upstream` de ADR-0041 D20 (`RepairDecisionObservation`): REPLACEMENT ADR REQUIRED, sin numero asignado y con censo previo de todas las ramas; el freeze `3f17c21` queda historico para G7; Coordinador de I-49 AGREED con M1-M10, Arquitecto PENDING sobre A3 exacto y Owner PENDING; G6 CLOSED en `75f1862`; G7 y G8 BLOCKED | contrato de I-52 **NON-MATERIAL**: I-52 no consume D20 ni `RepairBrokenRack`, y el `PlanReadSet` solo le llega si I-49 integra antes, con su autoridad de lectura y sus observaciones transitivas, sin calculo propio (RS-3, PV-6); coordinacion **MATERIAL**: al reconciliar se leen V6 + A1 + A2 + A3, el ADR de reemplazo y el freeze nuevo, y el censo de ADR de §16 verifica su numero frente a 0036; no dispara el reporte de §15.3 (sin `PlanReadSet` de produccion, dominio aplicado, ProjectVariables, Persistence ni Systems); sin STOP (`[V13-D15]`) |
| **I-56 `12fb509`** | sin cambios desde V12 | NON-MATERIAL; V13 corrige la redaccion de la transicion (`[V13-D10]`) |

**Cambios de paralelas entre la publicacion de V11 y la de V12 (registrados por V12).**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-49 `75f1862`** | G6-C2 sobre el Consensus Freeze `3f17c21`: correccion del cualificador de clave exacta de A2 en 7 archivos de `src/RackCad.Application/Expressions/` (`SymbolId`, `ExpressionLexer`, `ExpressionToken`, `ExpressionSyntax`, `ExpressionSyntaxParser`, `ExpressionBinder` y `ExpressionFormatter`) y 10 de pruebas, incluidas `ExpressionCoreGuardTests` y `ExpressionParserGuardTests`; CI `34862975299` success; sin `ProjectVariables`, `Persistence`, `Systems`, Plugin, UI, `PlanReadSet` ni dominio aplicado; G6 sin cierre registrado y G7 bloqueado | contrato de I-52 **NON-MATERIAL**: I-52 no usa la sintaxis del cualificador, y D19, D20 y la identidad textual de `VariableId` no cambian; coordinacion **MATERIAL**: la linea base de CT-46 y G-M18 incluye las guardas del nucleo que G6-C2 cambia; no dispara el reporte de §15.3 (`[V12-D12]`) |
| **I-55 `1e12414`** | Proposal V2 de View Placement & Projection (`f84f303`, NOT CONSENSUS; CI `34864075451` success), mapa de implementacion V2, paquetes de revision V2 de Coordinador y Arquitecto y ADR-0042 revisado; solo docs. Open Material M-01 (propio) y X-1..X-8 con I-52, con X-7 y X-8 nuevos; I-55 adopta la reconciliacion obligatoria antes de su propio Consensus Freeze; ADR-0042 sigue propuesto y ahora complementa y enmienda ADR-0010 en vez de reemplazarlo (vista adicional sobre un rack materializado o una intencion de creacion aceptada; dos precondiciones nuevas de Insertar; Actualizar sin cambio) | arquitectura de I-52 hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**. Su X-5 es el reporte previsto en §15.3: se re-evaluan §8.1, §8.7, el read-set, PRE-17, PRE-18, T-GRD-02 y `B`, sin cambio hoy; autoridades compartidas provisionales (`[V12-D11]`, `[V12-D17]`) |
| **I-56 `12fb509`** (rama nueva `docs/initiative-workflow-v2`) | reclamo `50ba035` y bootstrap `bd864a4` sobre `dad4e77` (contrato de Initiative Workflow V2 y fila en ROADMAP de iniciativas transversales) y, durante la redaccion de V12, correccion G0.1 `12fb509` solo del contrato: alcance confirmado (proceso de las iniciativas futuras), decision del Owner requerida antes de que rija la V2, invariante de transicion (I-49, I-52 e I-55 grandfathered; una iniciativa grandfathered termina bajo el workflow con que se reclamo; I-56 no reescribe contratos activos) e invariante de vigencia (ninguna politica de la V2 rige sin Coordinator y Architect AGREED sobre la misma version y Owner APPROVED); Evidence Audit y Proposal NOT STARTED; CI `34866844206` success | **NON-MATERIAL** hoy: no cambia ninguna norma vigente y declara a I-52 grandfathered; vigilancia: si integra un WORKFLOW cambiado antes del freeze de I-52 o de un gate posterior, se relee la autoridad de proceso vigente, incluidas sus reglas de transicion; no se adoptan reglas no integradas (`[V12-D13]`) |

**Cambios de paralelas entre la publicacion de V10 y la de V11 (registrados por V11).**

| Paralela | Cambio | Relacion con I-52 |
|---|---|---|
| **I-49 `3f17c21`** | rebase de la rama sobre `dad4e77` (G5 `892a601`, A1 `f69e903`, aceptacion de ADR-0040 `a86c958`, Consensus Freeze V6 + A1 `f95464a`, G6 `988ab9c` y A2 `7c1eed4`; `git range-diff` identico salvo el contexto de ROADMAP y del indice de ADR, y el mismo patch-id de `src` y `tests` en G6; punta previa en el tag `archive/i-49-a2-pre-rebase-5a3714f`); ADR-0041 **propuesto** (`8cefd59`) con consenso exacto sobre A2 (`bcbf570`), **aceptado** por el Owner (`2e8090a`) y que **reemplaza a ADR-0040**; nuevo Consensus Freeze sobre V6, A1, A2 y ADR-0041 (`3f17c21`); G6 sigue NOT CLOSED y G7 BLOCKED; sin produccion nueva | contrato de I-52 **NON-MATERIAL**: ADR-0041 es la autoridad vigente y conserva identicos D19 (`PlanReadSet`) y D20; D5 añade la validez neutral de la clave de `projectVariable` sin cambiar la identidad ni la igualdad de `VariableId`; D6 y D7 solo cambian las formas de referencia y la sintaxis del cualificador, D8 solo su redaccion, y D1..D4 y D9..D25 son identicos. Coordinacion **MATERIAL**: el freeze habilita la correccion acotada de G6 en `RackCad.Application.Expressions`, que CT-46 y G-M18 releen al reconciliar (`[V11-D11]`) |
| **I-55 `39fa759`** | Proposal V1 de View Placement & Projection (`d1918ab`, NOT CONSENSUS), mapa de implementacion y paquetes de revision de Coordinador y Arquitecto (`39fa759`); ADR-0042 **propuesto**, sucesor propuesto de ADR-0010, que declara conservada la semantica de Actualizar e Insertar; solo docs. §17.2 de su Proposal reconcilia con I-52 en X-1..X-6 (seleccion; plan por vista y codec de `View`/`Section`; comparador authored; primitivo de materializacion; archivos STOP y NO TOCAR; lineas base de C-2) y declara que no crea autoridades paralelas | arquitectura de I-52 hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**. Rige la reconciliacion obligatoria antes del freeze con STOP simetrico (`[V11-D10]`); ADR-0042 bajo vigilancia (`[V11-D12]`) |

**Cambios de paralelas entre la publicacion de V9 y la de V10 (registrados por V10).**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V16 declaraba
como ultimos observados main `dad4e77`, I-52 `ae8640a`, I-49 `c8cfee2`, I-55 `8068368` e I-56 `fde0386`; el preflight de
V16 los confirmo, con ADR-0041 aceptado, ADR-0042 propuesto, sin ramas nuevas, produccion nueva ni numeros de ADR
reservados, y el re-fetch previo al commit encontro los mismos tips salvo I-56 en `f97a8b0` (Architect Review de su
Proposal V2, solo docs) (`[V16-D15]`). La historia de paralelas anterior esta en Proposal V15 §15.1, en Proposal V14
§15.1, en Proposal V13 §15.1, en Proposal V12 §15.1, en Proposal V11 §15.1, en Proposal V10 §15.1, en Proposal V9 §15.1,
en Proposal V8 §15.1, en Proposal V7 §15.1 y en el registro §42, §48, §54, §60, §66, §72, §78, §84 y §90. La numeracion
de ADR esta en §16.
...
| **I-49** (V6 y Amendments A1 y A2 congelados en `3f17c21`; ADR-0041 **aceptado**, que reemplaza a ADR-0040, que a su vez reemplazo a ADR-0038; rama rebasada sobre `dad4e77`: G5 sintactico `892a601`, G6 `988ab9c` productivo y la correccion G6-C2 `75f1862`, que cierra G6, el Amendment A3 `f6f0991`, solo docs, con revision exacta del Arquitecto = CHANGES REQUIRED, y su revision A3-R1 `c8cfee2`, solo docs y pendiente de re-revision exacta, que cambian la semantica futura de las observaciones del `PlanReadSet` de D19 y D20 y piden un ADR de reemplazo de ADR-0041 sin numero y un Consensus Freeze nuevo, con el freeze `3f17c21` historico para G7; G7 y G8 BLOCKED; sin integrar) | `expression` en `PropertyValues`; `PlanReadSet` (D19); dominio del consumidor declarado por el descriptor (P24.5); su freeze exige que la prueba de supervivencia de `expression` (P24.8) se escriba contra `RackDuplicationPlan` y el restamp vigentes en main; guardas de texto evolucionadas en G5 y, en G6 y G6-C2, `LengthUnitsAuthorityGuardTests` sobre todo `src/` y las guardas del nucleo; ADR-0041 como sucesor aceptado de ADR-0040 (frente a ADR-0040 cambian solo D5..D8: validez de la clave, formas de referencia, sintaxis del cualificador y redaccion del recuento de tokens; D19, D20 y D25 identicos a los de ADR-0040, un hecho historico anterior a A3) | **I-52 coordina con I-49 antes de tocar `RackDuplicationPlan.cs`** (G4); T-M19. Si I-49 integra antes: su `PlanReadSet`, segun su autoridad final, es RS-3 (§9.2.1), su dominio es la cota `b` de V-DEP, su descriptor se clasifica en la guarda (§6.7) y sus guardas de texto rigen (G-M18, CT-46). `8ed15f7` es **NON-MATERIAL** (`[V8-D18]`) y `5f969cc` tambien lo es para el contrato, sin `PlanReadSet` ni dominio (`[V9-D21]`, `[V10-D14]`); A2 con ADR-0041 aceptado y la correccion G6-C2 solo cambian la validez de la clave y la sintaxis del cualificador de `VariableId`, sin tocar lo que I-52 consume (`[V11-D11]`, `[V12-D12]`): las referencias de I-52 a ADR-0038 y a ADR-0040 se leen en ADR-0041; que D19 de ADR-0041 sea identico al de ADR-0040 es solo un hecho historico anterior a A3, y ADR-0041 D19 **no** es la autoridad vigente de RS-3. A3, revisado por A3-R1 (`c8cfee2`), cambia la semantica futura de las observaciones del `PlanReadSet` de D19 —diagnosticos de un simbolo no evaluado, orden, cadena representativa y `RootCauses` por SCC; segun A3-R1, D19 se lee con M3, M4 y M10(a)— y el `Upstream` de D20, que A3-R1 cambia directamente junto con D18 puntos 1-4 y D14 decision 7, que I-52 no consume. Para el contrato actual es **NON-MATERIAL** solo porque RS-3 delega en la autoridad final integrada de I-49; para la reconciliacion es **MATERIAL**: antes del freeze de I-52 se relee esa autoridad final y, si no esta disponible, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED (`[V14-D07]`, `[V15-D06]`); si I-49 se cancela o abandona antes, no hay desbloqueo automatico ni eliminacion silenciosa de RS-3 (R-55, `[V15-D07]`). V16 **no** presupone el motor |
| **I-54** (**integrada** en `main @ ba497f1`; ADR-0039 **aceptado** en main; rama remota retirada) | `CustomProperties` (`JsonElement?`) heredado y normalizado por `Compose`, con la regla F; T-GRD-01..T-GRD-03 (sobre y restamp), T-GRD-04..T-GRD-07 (borde fisico) y T-GRD-08 (censos de comandos, ayuda y ventanas) en main; Custom Properties de alcance Proyecto en el NOD `RACKCAD_CUSTOM_PROPERTIES` y de alcance Rack en el sobre; `RACKPROPIEDADES`/`RPR`; **D-21 vigente** (el espejo cumple: A y despues B); residuales F-14a/F-14b vigentes | baseline de main: materialidad arquitectonica NON-MATERIAL; coordinacion y guardas cruzadas MATERIAL (`[V8-D19]`, `[V10-D13]`). **CA-4** portador RACK-LEVEL que se preserva con la copia; **CA-4b** Custom Properties de alcance PROYECTO que el espejo no lee para reflejar el rack y no copia, refleja, transforma ni escribe (§6.4, `[V9-D10]`, `[V10-D11]`); G-M22; G-M18 aplica T-GRD-01..T-GRD-07; T-M16 reapunta T-GRD-08 (ayuda sin alias, `[V10-D08]`); el gate que introduce la llamada a `Compose` del espejo reapunta T-GRD-02 (hoy 7 → 8, re-medido en el SHA de reconciliacion); T-GRD-01 y T-GRD-03 compatibles con ID-5 e ID-6; F-14a ⇒ E2 (ST-2c); F-14b y regla F ⇒ E8 en P10 (§9.4); T-M20 |
| **I-55** (View Placement & Projection; rama `feature/creacion-de-vistas` @ `8068368`, sobre `dad4e77`; Proposal V4 `fe70d7a` con Coordinador de I-55 REVIEW REQUIRED y Arquitecto formal PENDING, tras la decision CQ-01 sobre la V3 (redibujo atomico de hermanas antes de Insertar); revision de G2C reclasificada como revision tecnica adversarial; tabla X-1..X-8 de G2C aceptada del lado de I-55 y diferencias de V3 sin adoptar; Open Material M-01; Owner PENDING; ADR-0042 propuesto; solo docs) | ID17, ID18 e ID19 sobre una foundation de preparacion de vistas antes de materializar y una colocacion de grupo con una transformacion rigida comun; §17.2 de su Proposal V2 reconcilia con I-52 en X-1..X-8: nucleo neutral de seleccion (X-1); plan desde el sistema resuelto y codec de `View`/`Section` neutral con politica por consumidor (X-2); comparador authored por kind (X-3); primitivo «definicion + sobre + referencia» con las politicas de I-52 (ST-13, ST-19, PRE-17, PRE-18, read-set) fuera del primitivo (X-4); reporte antes de fijar archivos o autoridades que I-52 vigila, que la Proposal V2 ya dispara (X-5); re-establecimiento de C-2 por la segunda en integrar (X-6); valor de colocacion sobre `Transform2D` con rigidez de I-55 y reflexion de I-52 (X-7); un descriptor de origen y tramo del eje por vista (X-8); la revision tecnica adversarial de G2C de I-55 (`d091eeb`), que su Proposal V3 reclasifico como no formal, los acuerda los ocho, con las diferencias de V3 pendientes de revision formal y reconciliacion (§15.4). ADR-0042 complementa ADR-0010 (vista adicional; dos precondiciones nuevas de Insertar; Actualizar sin cambio) y su Proposal V3 lo registra como complemento de ADR-0010, que sigue aceptado, con nota posterior fechada solo si se acepta; V3 §17.2 propone diferencias en X-1, X-2, X-4, X-5, X-7 y X-8, y V4 §17.2 precisa X-4, X-5 y X-6 por CQ-01 (§15.4) | arquitectura hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**; I-52 **no** espera al consenso de I-55. Tras el consenso tecnico sobre V16 y el rebase, **reconciliacion obligatoria antes del freeze** de la propiedad de las autoridades compartidas, con la ultima tabla X-1..X-8 acordada como entrada vinculante y las diferencias de las Proposals V3 y V4 de I-55 por decidir (§15.4), y marcadas como provisionales en V16 (taxonomia de tipo de vista, codec de `View`/`Section`, «sistema resuelto → plan de vista» con origen y tramo del eje, comparador authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, nucleo de seleccion), sin autoridades duplicadas; siguen siendo de I-52 ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`; I-55 aplica la misma regla y el STOP simetrico es mutuo; si ADR-0042 se acepta antes del freeze de I-52, se releen las autoridades de C-2, Actualizar e Insertar; la segunda iniciativa en integrar re-establece C-2; antes del freeze, el estado formal de la Proposal V3 y de las entradas X-* queda resuelto y `d091eeb` no se presenta como consenso formal (R-45, `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`, `[V15-D08]`) |
| **I-56** (Initiative Workflow V2; rama `docs/initiative-workflow-v2` @ `f97a8b0`, sobre `dad4e77`; G1 Evidence Audit `c6839e2` y correccion G1.1 `0d66df2`; Evidence Audit COMPLETE; G2 Proposal V1 `18401da` con CHANGES REQUIRED de su Coordinador y G2.1 Proposal V2 `fde0386` NOT CONSENSUS, con Coordinator AGREED, Architect CHANGES REQUIRED — PROPOSAL V3 (G3 `f97a8b0`) y Owner NOT REQUESTED; solo documentacion y proceso; I-49, I-52 e I-55 grandfathered; WORKFLOW V2 NOT EFFECTIVE y `WORKFLOW_V2_EFFECTIVE_SHA` inexistente) | puede cambiar el proceso de iniciativas (WORKFLOW, AGENTS, AUTOMATION_PLAN, `TEMPLATE.md` y afines) cuando su Proposal alcance consenso, el Owner la apruebe y se integre; hoy solo toca su contrato, su fila de ROADMAP, el Evidence Audit, que no es normativo, y Proposals sin consenso que declaran no modificar ni aplicarse a los contratos, reclamos y ramas de I-49, I-52 e I-55 | **NON-MATERIAL** hoy; ninguna regla de las Proposals V1 o V2 se aplica a I-52; el WORKFLOW vigente sigue siendo la autoridad hasta que I-56 integre y, aun integrada, su invariante de transicion deja a I-52 bajo el workflow de su reclamo y ninguna politica de la V2 rige sin acuerdo tecnico y aprobacion del Owner; antes de cada transicion del proceso de I-52 se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo; nunca se adoptan reglas no integradas ni hipotesis del Evidence Audit (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`, `[V16-D10]`) |
| Documental | Filas de ROADMAP, final de `ideas-futuras.md`, `docs/adr/README.md` (0035, 0037 y 0039 aceptados en main; 0036 propuesto de I-52; 0038 REPLACED, 0040 REPLACED y 0041 ACCEPTED de I-49; 0042 PROPOSED de I-55), censo de comandos `B → B + 1` (hoy `B = 35` en `main @ dad4e77`), censos por nombre y por pares de T-GRD-08 en main, `src/RackCad.UI/RackCommandReference.cs` y `SelectiveEditorOpenTests.cs` | Quien integre despues conserva todas las filas en orden numerico, re-mide los censos (por numero, por nombre y por pares de ayuda) y serializa esos archivos |
...
I-49, I-55 e I-56 (y de cualquier iniciativa nueva; I-54 e I-53D ya estan integradas en main); leer sus Proposals y
contratos en el SHA exacto; buscar `docs/adr/0036-*` y citas de «ADR-0036» en **todos** los refs (§16); comprobar los
archivos de §19; si `main` avanzo, rebase segun WORKFLOW antes de escribir codigo; y, tras el consenso tecnico y el
rebase, la reconciliacion obligatoria con I-55 e I-49 antes del freeze (§14.1, `[V11-D10]`, `[V12-D11]`); antes de cada
transicion, releer la autoridad de proceso vigente con sus reglas de transicion (`[V12-D13]`, `[V13-D10]`). Ademas:

- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  guardas de texto (CT-46). ADR-0041 esta aceptado y reemplaza a ADR-0040: las referencias de I-52 a ADR-0038 y a
  ADR-0040 se leen en ADR-0041 (`[V8-D18]`, `[V11-D11]`); que D19 y el dominio del consumidor no cambiaran entre
  ADR-0040 y ADR-0041 es solo un hecho historico anterior a A3, y ADR-0041 D19 **no** es la autoridad vigente de RS-3
  (`[V15-D06]`). Su G6 (`988ab9c`, antes `5f969cc`) añade el nucleo semantico y la autoridad de unidades con
  `LengthUnitsAuthorityGuardTests` sobre todo `src/`, y la correccion G6-C2 (`75f1862`) ajusta el cualificador de clave
  exacta y las guardas del nucleo solo en `Expressions`: al reconciliar, CT-46 las relee y G-M18 las aplica a los
  archivos de I-52. El Amendment A3 (`f6f0991`, solo docs, con revision exacta del Arquitecto = CHANGES REQUIRED) y su
  revision A3-R1 (`c8cfee2`, solo docs, pendiente de re-revision exacta) cambian la semantica futura de las
  observaciones del `PlanReadSet` de D19 (que A3-R1 clasifica como leida con M3, M4 y M10(a)) y D20, y piden un ADR de
  reemplazo de ADR-0041 sin numero y un Consensus Freeze nuevo. **Antes del freeze de I-52** se relee obligatoriamente
  la autoridad final acordada, aceptada y congelada de I-49 —V6, A1, A2, A3 revisado, ADR de reemplazo, Consensus Freeze
  nuevo y estado de implementacion— y, si no esta disponible, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE =
  BLOCKED (`[V14-D07]`); si I-49 se cancela o abandona antes, no hay desbloqueo automatico ni eliminacion silenciosa de
  RS-3, y el freeze sigue bloqueado hasta la decision del Coordinador de R-55 (`[V15-D07]`); el censo de §16 verifica el
  numero del ADR de reemplazo. Si un gate posterior de I-49 introduce `PlanReadSet` o dominio aplicado, o toca
  ProjectVariables, Persistence o Systems, reportar antes de continuar (`[V9-D21]`, `[V10-D14]`, `[V11-D11]`,
  `[V12-D12]`, `[V13-D15]`, `[V14-D07]`, `[V15-D06]`).
- **I-54 (integrada en `main @ ba497f1`):** sale del protocolo de ramas paralelas. Tras el rebase, re-medir CA-4 y G-M22
  sobre main, aplicar T-GRD-01..T-GRD-07 en G-M18 y reapuntar con motivo T-GRD-02 (hoy 7 → 8, re-medido en el SHA de
  reconciliacion, en el gate que introduce la llamada a `Compose` del espejo) y T-GRD-08 (T-M16: `RACKMIRROR` por nombre
  y entrada de ayuda sin alias; `[V10-D08]`, `[V10-D13]`); un cambio posterior en main que toque el sobre, el
  compositor, el restamp, `ProjectVariables` o las autoridades de CA-4 o CA-4b se reporta antes de continuar.
...
- **I-55 (`feature/creacion-de-vistas` @ `8068368`, View Placement & Projection, Proposal V4 en revision con Arquitecto
  formal PENDING, tabla X-1..X-8 acordada en la revision tecnica adversarial de G2C (`d091eeb`, no formal) y aceptada
  del lado de I-55, y diferencias de V3 y V4 sin adoptar y pendientes de revision formal, solo docs; gate de
  coordinacion activo, `[V14-D14]`, `[V15-D08]`, `[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`):**
  I-52 no espera a su consenso, pero no congela sin la reconciliacion obligatoria posterior al consenso tecnico sobre
  V16 y al rebase, con X-1..X-8 como entrada vinculante (§15.4), que registra la propiedad de las autoridades
  compartidas que V16 marca como provisionales —taxonomia de tipo de vista, codec semantico de `View`/`Section`,
  autoridad «sistema resuelto → plan de vista» con origen y tramo del eje de la vista, comparador authored, primitivo de
  materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, y nucleo de seleccion (X-1..X-8)—,
  sin autoridades duplicadas. Siguen siendo de I-52, aunque consuman un primitivo compartido, ST-13, ST-18, ST-19,
  PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`. Si cualquiera de las
  dos intenta congelar una autoridad incompatible antes de reconciliar: STOP, ningun freeze para ninguna (R-45, §19);
  I-55 V2 aplica la misma regla a su Consensus Freeze. G4 avisa a I-55 antes de tocar la fundacion compartida (nucleo de
  seleccion en `RackDuplicationPlan.cs`). Si ADR-0042 cambia de estado, se re-evalua antes del siguiente commit y, si se
  acepta antes del freeze de I-52, se releen las autoridades que usan C-2, Actualizar e Insertar. Cuando un gate de I-55
  toque rutas de materializacion, `RackEmbedComposer.Compose`, `SystemBlockWriter.cs`, comandos o
  `RackCommandReference.cs` (hoy previsto en sus G5..G15, X-5), se re-evaluan §8.1, §8.7,
  `MaterializationContextReadSet`, PRE-17, PRE-18, T-GRD-02 y el censo `B`; la segunda iniciativa en integrar
  re-establece C-2 sobre el main combinado. Si I-55 publica otra Proposal o cambia algun X (como la Proposal V4, que
  precisa X-4, X-5 y X-6), se relee antes del siguiente commit y la reconciliacion parte de la ultima version acordada.
  Las ubicaciones fisicas de §14.2 G4..G7 que tocan X-1..X-8 siguen provisionales (`[V14-D08]`).
- **I-56 (`docs/initiative-workflow-v2`, Initiative Workflow V2, G3 `f97a8b0`, solo documentacion y proceso;
  `[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`, `[V16-D10]`):** NON-MATERIAL hoy; Evidence Audit COMPLETE y
  Proposal V2 NOT CONSENSUS (Coordinator AGREED, Architect CHANGES REQUIRED — PROPOSAL V3, Owner NOT REQUESTED); su
  contrato declara a I-49, I-52 e I-55 grandfathered y que ninguna politica de la V2 rige sin acuerdo tecnico y
  aprobacion del Owner (WORKFLOW V2 NOT EFFECTIVE), y sus Proposals declaran no aplicarse a los contratos, reclamos y
  ramas de I-52. Ninguna hipotesis del Evidence Audit ni regla de las Proposals cambia la politica de I-52, que sigue el
  workflow bajo el que se reclamo. Antes de cada transicion del proceso de I-52 (consenso, rebase, reconciliacion,
  freeze, re-check, O-1, G3 y gates posteriores) se relee la autoridad de proceso vigente en `origin/main`; las futuras
  autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique,
  I-52 conserva su workflow de reclamo; nunca se adoptan reglas de I-56 no integradas.
- Recalcular el SHA-256 de la biblioteca de evidencia antes de reutilizarla (CT-47, `[V7-D17]`) y censar 0035..0042, y
  cualquier numero posterior, en el indice de ADR y en todos los refs.

### 15.4 Entrada vinculante de la reconciliacion: X-1..X-8 de I-55 (`[V13-D08]`)

Fuente: revision de la Proposal V2 de I-55 registrada en `feature/creacion-de-vistas` @ `d091eeb` (G2C), que el
Coordinador de I-55 reclasifico en G2D (`8e35a51`) como revision tecnica adversarial y no como veredicto formal del
Arquitecto: `d091eeb` **no** es consenso formal (`[V15-D08]`). La tabla X-1..X-8 queda **aceptada del lado de I-55** por
su Coordinador, sin MATERIAL CONFLICT con I-52; el Arquitecto formal de I-55 sigue PENDING y su Owner tambien, y las
diferencias de sus Proposals V3 y V4 siguen pendientes de revision formal y reconciliacion. V13 las registro y V16 las
mantiene como **entrada vinculante** del gate de reconciliacion posterior al consenso tecnico y al rebase (§14.1): la
reconciliacion parte de las ultimas entradas X-* acordadas, y una desviacion se vuelve a acordar con I-55 antes del
freeze, nunca de forma unilateral. V16 **no** adopta todavia los requisitos del lado de I-52 ni congela namespace,
ubicacion ni forma de API; la politica propia del espejo sigue siendo de I-52.

| X | Nucleo compartido acordado (resumen) | Politica que conserva I-52 | Custodio / extrae | Secciones de I-52 afectadas |
|---|---|---|---|---|
| **X-1** Seleccion | clasificacion de fuentes de I-51 con predicado de kind inyectado, clave `RackId`/`Definition(handle)`, agrupacion y deduplicacion conservando referencias, consistencia de `Kind` y nombre, mensajes inyectados; transformaciones, `Origin`, MINSERT, banderas y dependencia de XREF en el SNAPSHOT de cada llamador | ST-* (payload antes del filtro de Model Space; `Origin ≠ 0` falla; `Id` en blanco → clave de definicion) | custodio I-52; extrae I-52 G4 (I-55 G13 solo si I-52 no lo extrajo, con la misma especificacion) | §5.1; CT-16 |
| **X-2** Taxonomia, codec, resolucion y plan | taxonomia de tipo de vista; `Decode` sintactico; disponibilidad sobre el sistema resuelto; `Resolve` (tambien para el BOM) y `Plan` para todas las direcciones, con laterales y planta Cantilever con `PlantaVisibility` | `σ'` canonica; `Coerced`, `Invalid` o no disponible falla; `Expone`, `F`, `μ_k`; `A_k` = disponible ∩ expuesto; `Build` = `Resolve` + `Plan` | custodio I-55; extrae la primera que llegue (I-55 G6/G7 o I-52 G5/G6) | §3.7; §3.8; §6.1; §8.1; §8.4; §20.1; CT-04 |
| **X-3** Comparador authored | `IsSameAuthority(miembros)` por kind en el contrato del kind, fuera del reflector del espejo, excluyendo la metadata por vista | miembros = vistas seleccionadas; E3 | custodio I-52; extrae I-52 G5 o I-55 G14 | §5.4; §6.1; §8.5 |
| **X-4** Primitivo de materializacion | despacho por familia de plan + `RackBlockData.Write` + referencia en Model Space con Z y presentacion explicitas; sin lock, commit, regeneracion ni purga; faltantes por instancia | ST-13, ST-19, PRE-17, PRE-18, `MaterializationContextReadSet` y huellas en el llamador (V13 añade alli ST-20 y PRE-19) | custodio I-52; extrae I-52 G6 (I-55 G15 solo si llega antes, con la firma acordada) | §8.2; §20.1 |
| **X-5** Protocolo de archivos y autoridades | sin autoridad: reporte antes de fijar archivos y STOP simetrico | reevaluar §8.1, §8.7, read-set, PRE-17, PRE-18, T-GRD-02 y `B`; G4 avisa a I-55 | — | §15.3; §19 |
| **X-6** Lineas base de C-2 | un juego de fixtures y un mapa G-M9 sobre el `main` combinado; `Plan` compartido de la planta Cantilever con `PlantaVisibility`; fixture con brazos y tensores visibles | C-2 por defecto; C-1 solo condicional | custodio I-52; la segunda en integrar re-establece C-2 | §8.5 |
| **X-7** Valor de colocacion | `Transform2D` + un valor de colocacion, sin `Origin` ni Z | reflexion; escala de la fuente; Z de la fuente | custodio I-52 (`Application/Geometry`); extrae I-52 G4 (I-55 G14 solo si llega antes) | §3.3; §8.3 |
| **X-8** Origen y tramo del eje | un descriptor por (sistema, tipo, variante) desde el sistema resuelto: ejes con signo, origen fisico local, `[K_min, K_max]` por eje y convencion de extremos | `c` = centro del tramo del eje reflejado; desplazamiento δ registrado (CT-06), nunca ajustado; solo vistas admisibles | custodio I-55; extrae la primera | §3.7; §6.1; §8.1; CT-05 |

**Requisitos del lado de I-52 pendientes de adoptar en la reconciliacion** (registrados por I-55; V13 no los adopta
todavia):
...
- §8.1: `Build` = `Resolve` + `Plan`, con `Plan` para todas las direcciones y la resolucion compartida con los handlers
  del BOM.
- §8.4 y §20.1: el nucleo de decodificacion en `Application/Views` segun I-55, y `Mirror/` con `A_k` y la exposicion.
- §8.2: primitivo con Z y presentacion explicitas y comprobacion de faltantes por instancia.
- C-2: fixture de planta Cantilever con brazos y tensores visibles.
- Caracterizaciones unicas: CT-04, CT-05 y CT-16.

**Proposal V3 de I-55 (`8e35a51`, publicada durante la redaccion de V14; `[V14-D14]`).** V3 §17.2 propone diferencias
con la tabla anterior, que **no** estan acordadas y que ni V14, ni V15 ni V16 adoptan: X-1 con primer gate simetrico
(I-52 G4 o I-55 G13); **X-2 y X-8 con extractor unico I-55** (G6, G7a y G7b), lo que acoplaria G4..G6 de I-52 a esos
gates o exigiria una excepcion registrada de §14.2 y §20.3; X-4 con un requisito estructural puro de bloques
(`A/Drawing/LibraryBlockRequirement.cs`) y una consulta tras importar, por primer gate; X-5 con comprobaciones de I-52
antes de G4 y G6; y X-7 con la descomposicion de la transformacion fuente y la tolerancia de escala. Tambien propone una
tabla de adopcion para el registro de I-52 (§17.2.1). La reconciliacion posterior al consenso tecnico decide si I-52
adopta esas diferencias, las vuelve a acordar o las escala a los dos Coordinadores; mientras tanto la entrada vinculante
sigue siendo la ultima acordada, las ubicaciones fisicas siguen provisionales y el estado formal de la Proposal vigente
de I-55 y de las diferencias de V3 y V4 debe quedar resuelto antes del freeze (`[V14-D08]`, `[V15-D08]`).

**Proposal V4 de I-55 (`fe70d7a`, publicada durante la redaccion de V15; `[V15-D14]`).** La decision CQ-01 de su
Coordinador sustituye el redibujo de hermanas confirmado por vista por un redibujo atomico antes de Insertar (PREPARE
sin escribir estado del rack → una MUTATE → un commit → POST), con la colocacion de vistas nuevas en una transaccion por
jig tras ese commit, y ADR-0042 decision 6 lo recoge; Actualizar no cambia. V4 §17.2 conserva la tabla X-1..X-8, sus
reglas de extraccion y las diferencias de V3, y precisa tres entradas: X-4 con dos operaciones de la misma familia
(crear en la transaccion del llamador, de I-52 G6, y redefinir en la transaccion del llamador, existente de I-47 G9, que
consume el redibujo atomico de I-55 y no se extrae; sus sitios nuevos se declararian en G-M24), X-5 con reporte en G9a y
G9b y los disparadores de V14 §15.3, y X-6 con productor G9b, el mapa de invocaciones G-M9/CT-26 de Insertar cambiado y
C2-2b con redibujo todo o nada. Esas precisiones **no** estan acordadas y ni V15 ni V16 las adoptan; la reconciliacion
decide sobre las diferencias de V3 y V4, la familia de escritura de G-M24 (`[V15-D03]`) ya cubre los sitios de redefinir
y, si ADR-0042 se acepta antes del freeze de I-52, se releen las autoridades de C-2, Actualizar e Insertar.

**Contraste con V16.** Ningun X presenta contradiccion material con V16: V16 no fija la propiedad ni la ubicacion fisica
de ninguna autoridad compartida, mantiene ST-20, PRE-19 y E12 —con la politica cerrada de sesiones de edicion— como
politica propia del espejo fuera del primitivo (X-4) y amplia la marca provisional a las secciones que esos requisitos
nombran y, desde V14, a las ubicaciones de §14.2 G4..G7 (`[V14-D08]`); desde V15 los marcadores ya no presentan
`d091eeb` como consenso formal (`[V15-D08]`). Si I-55 publica otra Proposal o cambia algun X antes de la reconciliacion,
se relee y la reconciliacion parte de la ultima version acordada.
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V16.
- Congela: espejo semantico por copia; `μ_k` canonica por kind; admisibilidad por vista con seccion canonica y
  ...
  visual-geometrica** evaluada por estado sobre la definicion real, con politica cerrada de clases y variantes, fuentes
  visuales simbolicas tipadas, `VisibilityPath`, orden visual dentro de la pieza y entre piezas, primitivas canonicas
  conservadoras con `SourceKind`, ruta anidada estable, parametros efectivos con paridad exacta, transformacion
  efectiva, base auxiliar con el contexto de la autoridad origen, cache de biblioteca aislada y accedida por su API
  publica, degradado fail-closed e identidad SHA-256 de la evidencia; presentacion exterior de la referencia fuente
  conservada, orden en Model Space demostrado frente a todo objeto posterior a cada fuente sobre una base de datos y una
  enumeracion completas y sin sesiones de edicion no caracterizadas, huellas visuales de ocupacion de modelo en el
  contexto destino y refrescadas tras `EnsureForPlan`, y read-set del contexto de materializacion; obligacion 9b y
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V16 ningun otro ref contiene `docs/adr/0036-*`; las citas de
   «ADR-0036» en I-49, I-53, I-54 e I-55 se refieren a este ADR.
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   coinciden (`[V5-D30]`). Todas son posteriores a 0036 y usan otro numero: **sin colision**. ADR-0041 lo publico I-49
   en `8cefd59` (`2026-09-13T23:32:44-06:00`) como sucesor de ADR-0040 y el Owner lo **acepto** en `2e8090a`
   (`2026-09-14T08:17:11-06:00`), reemplazando a ADR-0040. ADR-0042 lo publico I-55 en `d1918ab`
   (`2026-09-14T00:45:13-06:00`), **propuesto**; con la Proposal V2 (`f84f303`) ya no se presenta como sucesor de
   ADR-0010, sino como complemento que enmienda reglas acotadas, y con la Proposal V3 (`8e35a51`) ADR-0042 complementa a
   ADR-0010, que sigue aceptado, con una nota posterior fechada en ADR-0010 solo si el Owner acepta ADR-0042; con la
   Proposal V4 (`fe70d7a`) ADR-0042 sigue propuesto y precisa su decision 6 con el redibujo atomico de las hermanas
   antes de Insertar. El Amendment A3 de I-49 (`f6f0991`), revisado por A3-R1 (`c8cfee2`), anuncia un ADR de reemplazo
   de ADR-0041 sin numero asignado —cambia directamente D20, D18 puntos 1-4 y D14 decision 7, y D19 se lee con A3—, que
   se censara frente a 0036 cuando se publique; en el preflight de V16 no hay numeros reservados publicados. I-56 no
   tiene ADR. No hay numeros posteriores a 0042 en ningun ref (`[V11-D11]`, `[V11-D12]`, `[V12-D12]`, `[V12-D17]`,
   `[V13-D09]`, `[V15-D14]`, `[V16-D15]`).
...
no contradice materialmente V16, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V17. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V17 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V17 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V17 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V17 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V17 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V17 |
...
| **R-45** | I-55 (View Placement & Projection, Proposal V4 `fe70d7a` en revision con Arquitecto formal PENDING, tabla X-1..X-8 de la revision tecnica adversarial G2C, no formal, aceptada del lado de I-55 y diferencias de V3 y V4 sin adoptar —entre ellas X-2 y X-8 con extractor unico I-55, que acoplaria los gates de I-52, y el redibujo atomico de hermanas en Insertar con C2-2b todo o nada—, M-01 abierta, y ADR-0042 propuesto que complementa a ADR-0010; solo docs) preve una foundation de preparacion de vistas antes de materializar y una colocacion de grupo con una transformacion rigida comun (ID19) que cruzan la autoridad pura de planes por vista de I-52 (§8.1, C2-1), su materializador (G6), el contexto de materializacion (§8.7), el codec de `View`/`Section`, el origen y tramo del eje de la vista y la colocacion `P'`: dos autoridades paralelas podrian divergir o duplicarse | I-52 no depende de I-55 ni espera a su consenso; las autoridades compartidas quedan **provisionales** (`[V12-D11]`, `[V13-D08]`) y la reconciliacion parte de la ultima tabla X-1..X-8 acordada como entrada vinculante y decide sobre las diferencias de las Proposals V3 y V4 de I-55 (§15.4), con el estado formal de su Proposal vigente resuelto; **antes del freeze** (no antes de G3) se hace la reconciliacion obligatoria que registra su propiedad, sin duplicarlas, con STOP simetrico que I-55 V2 tambien adopta; G4 avisa a I-55; si ADR-0042 se acepta antes del freeze, se releen C-2, Actualizar e Insertar; la segunda iniciativa en integrar re-establece C-2 (`[V11-D10]`, `[V12-D17]`, `[V13-D09]`, `[V15-D08]`, `[V15-D14]`) |
| **R-46** | La elegibilidad de la presentacion resuelta (`[V11-D01]`) puede dejar en `UNKNOWN` piezas de dibujos cuyos usuarios usan tipos de linea complejos en capas homonimas, en la capa de la fuente o en `CLAYER`/`CELTYPE`, y reducir el alcance util | caracterizacion en CT-36, CT-49 y CT-50; fail-closed explicito (X-17b, L-30); si afecta a kinds enteros o a configuraciones habituales ⇒ Proposal V17 y O-1 se reevalua |
| **R-47** | La `ModelSpaceFootprintPolicy`, la huella latente independiente de la visibilidad y la politica de entorno (`[V11-D02]`..`[V11-D04]`) pueden hacer fallar RACKMIRROR con frecuencia en dibujos con objetos posteriores de clases no soportadas, en capas ocultas o dependientes del entorno | inventario de CT-49 con tantas clases `SUPPORTED(metodo)` como se puedan demostrar; E10 atribuible; declarado en O-1 (L-34, L-37); reduccion material ⇒ Proposal V17 |
| **R-48** | G-M24 puede pasar a RED ante un cambio legitimo de un productor de contexto | es el comportamiento buscado: obliga a re-caracterizar CT-50 y a actualizar el read-set, el `FootprintInputFingerprint` y las pruebas antes de seguir (`[V11-D08]`) |
| **R-49** | La politica latente por mecanismo (`[V12-D01]`..`[V12-D06]`) puede hacer fallar RACKMIRROR con frecuencia en dibujos con XREF descargados o no resueltos, objetos recortados, bloques dinamicos con estados de visibilidad o campos en cuadros de rotulo y notas posteriores a las fuentes, aunque esten lejos del destino | inventario de mecanismos de CT-49 con tantos casos `SUPPORTED(metodo conservador)` como se puedan demostrar; E10 atribuible que nombra el objeto y el mecanismo; declarado en O-1 (L-34, L-37); reduccion material ⇒ Proposal V17 y O-1 se reevalua |
| **R-50** | La regla de entorno en la copia (`[V12-D07]`) puede dejar en `UNKNOWN` anotaciones y cotas de copias en dibujos con estilos anotativos, `DIMSCALE = 0` o fuentes sustituidas | casos obligatorios de CT-50; fail-closed (E10, X-22) o limitacion explicita de O-1 (L-36, L-37); si afecta a configuraciones habituales ⇒ Proposal V17 y O-1 se reevalua |
| **R-51** | El censo de sitios productores de G-M24 (`[V12-D09]`, `[V13-D07]`) puede exigir mantenimiento y dar RED ante refactors legitimos; I-56 puede integrar un WORKFLOW que cambie la secuencia de gates antes del freeze de I-52 | para G-M24 es el comportamiento buscado: clasificar el call-site (`DECLARED` o `EXCLUDED` con motivo) y re-caracterizar si hace falta; para I-56, releer la autoridad de proceso vigente antes de cada transicion, leer toda autoridad futura junto con sus reglas de transicion y conservar el workflow de reclamo mientras el invariante grandfathered aplique (`[V12-D13]`, `[V13-D10]`) |
| **R-52** | ST-20 (`[V13-D01]`..`[V13-D04]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`) puede hacer fallar RACKMIRROR en dibujos abiertos parcialmente, con una edicion de referencia en curso, una transaccion larga, el Editor de bloques o la ventana de prueba de bloques abiertos, la edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) en curso, un host o una version de AutoCAD no caracterizados, la lectura fallida de un detector del `EditingSessionAuthoritySet`, una autoridad generica con sesion activa o XREF cargados bajo demanda con indices, que pueden aparecer en dibujos corrientes con el `XLOADCTL` por defecto; y mientras CT-49 no establezca el conjunto cerrado de detectores ningun prerrequisito se demuestra | E12 atribuible por causa antes de pedir la linea y E10 que nombra el XREF; CT-49 caracteriza el metodo cerrado, los detectores, el guard de host y los estados de `XLOADCTL` e `INDEXCTL` antes de cualquier implementacion; declarado en O-1 (L-34, M-35); reduccion material ⇒ Proposal V17 y O-1 se reevalua (R-57) |
| **R-53** | El censo de G-M24 por familias de simbolos de API ampliadas (`[V13-D07]`, `[V14-D05]`, `[V15-D03]`) puede clasificar muchos call-sites ajenos al camino de materializacion (por ejemplo, `TransformBy`, lecturas de cabecera de `Database`, altas en tablas de simbolos o aperturas para escritura de otros comandos: hoy 35 sitios de apertura para escritura en 15 archivos del Plugin) | «donde sea relevante» y `EXCLUDED` con motivo; el censo inicial lo fija CT-50 con sus anclajes minimos y el censo de escrituras de `dad4e77`, y la forma concreta se decide en G5..G7 sin tocar los drawers |
| **R-54** | La frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`, `[V15-D04]`, `[V15-D05]`) deja fuera las ediciones futuras (ERASE/OOPS, un XREF editado y recargado o resuelto a contenido efectivo distinto, una redefinicion, un texto editado, la edicion del objeto referenciado por un campo o una tabla de busqueda que cambia geometria pueden quedar por encima o por debajo de la copia), mientras que el refresco de `DataLink` queda dentro y una tabla con `DataLink` sera en la practica `UNKNOWN`; un campo cuyo resultado solo varia por estado admitido (fecha, nombre de archivo, conjunto de planos, trazado) puede quedar `UNKNOWN` si no hay cota para esos resultados | limitacion declarada en O-1 (L-37); E10 atribuible para los datos externos y los campos sin cota; una garantia frente a ediciones seria un cambio de alcance para una Proposal posterior |
| **R-55** | La autoridad final de I-49 (V6 + A1 + A2 + A3 revisado por A3-R1, ADR de reemplazo y Consensus Freeze nuevo) puede no estar disponible cuando I-52 este lista para congelar, lo que bloquea el freeze de I-52 aunque I-49 no vaya a integrar antes; e I-49 podria cancelarse o abandonarse antes de tener autoridad final (`[V14-D07]`, `[V15-D06]`, `[V15-D07]`) | RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED; se reporta al Coordinador en la reconciliacion; ninguna congelacion sobre ADR-0041 D19, que no es la autoridad vigente; si I-49 se cancela o abandona, **no** hay desbloqueo automatico ni eliminacion silenciosa de RS-3: el Coordinador decide en una Proposal (A) retirar del alcance de I-52 el soporte futuro de expresiones de I-49, (B) adoptar otra autoridad integrada o (C) mantener el freeze bloqueado, y hasta esa decision el freeze sigue BLOCKED |
| **R-56** | Los marcos y contornos de recorte (`[V14-D06]`) pueden aumentar la huella de objetos recortados y provocar mas E10 por superposicion o `UNKNOWN` | CT-49 mide los marcos por variable y clase; E10 atribuible; declarado en O-1 (L-34, M-35) |
| **R-57** | Si CT-49 no puede establecer para AutoCAD 2025 un metodo de descubrimiento cerrado y un `EditingSessionAuthoritySet` con deteccion fiable —de ARRAYEDIT Source, de la ventana de prueba de bloques, de una transaccion larga o de otro modo relevante de edicion en sitio, directamente o mediante una autoridad generica—, o si el host de uso no es el caracterizado, RACKMIRROR fallaria con E12 en todos los dibujos o no podria demostrarse seguro (`[V15-D01]`, `[V15-D02]`, `[V16-D01]`..`[V16-D04]`) | CT-49 establece el metodo, el conjunto y el guard de host antes de cualquier implementacion; un modo relevante sin detector fiable ⇒ STOP de caracterizacion; sin conjunto cerrado o con colapso de alcance ⇒ reduccion material de la envolvente ⇒ **Proposal V17 obligatoria** y O-1 se reevalua; la regla nunca se relaja en implementacion, nunca trata una lectura fallida como inactiva y nunca se sustituye por un aviso |
| **R-58** | Un overrule o reactor activo de otra aplicacion puede dejar `UNKNOWN` las huellas de clases enteras de objetos posteriores, y hacer fallar RACKMIRROR en todos los dibujos de las estaciones que lo carguen, sobre todo si CT-49 no puede acotar su alcance por clase (`[V16-D07]`) | CT-49 caracteriza `Overrule.Overruling` o la autoridad equivalente y, si existe, su alcance por clase; E10 atribuible; declarado en O-1 (L-37); nunca se inspeccionan los internos de plugins de terceros ni se supone equivalencia; reduccion material ⇒ Proposal V17 y O-1 se reevalua |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10, V11, V12, V13, V14, V15 y V16 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V9-8** | main, paralelas, G3 y proceso (C:V10-13..C:V10-17) | I-54 integrada y fuera del protocolo de ramas paralelas; I-49 sin cambios; I-53D con T-M17 y T-M17b; G3 ampliado; desde V10, contradiccion material ⇒ Proposal V11 (historial; sustituida por CR-V10-8) | §15; §12; §14.1 |
| **CR-V10-1** | Elegibilidad de la presentacion resuelta (C:V11-01) | Preguntas A (igualdad simbolica) y B (elegibilidad resuelta) separadas; resolucion en el `DestinationEvaluationContext` con DRAWING WINS por `Explicit`, `ByLayer`, herencia, presentacion de creacion y ST-13; tipo de linea efectivo complejo, no determinable o no caracterizado ⇒ `UNKNOWN` | §11.3 puntos 3, 4, 6.1, 6.2 y 10 |
| **CR-V10-2** | Huellas de objetos del dibujo (C:V11-02..C:V11-04) | `ModelSpaceFootprintPolicy` por tipo exacto, `SUPPORTED(metodo)` o `UNKNOWN`, sin fallback optimista; huella de orden latente e independiente de la visibilidad; ocupacion dependiente del entorno `SUPPORTED` solo con conservadurismo demostrado bajo todos los estados admitidos | §4.1 ST-19; §11.3 punto 5; L-34; L-37 |
| **CR-V10-3** | Cola completa de Model Space (C:V11-05) | `ModelSpaceOrderTailSnapshot` enumerada de nuevo en R6 y al inicio de MUTATE; un cambio ⇒ fail-closed o rollback | §9.1; §9.2; PRE-18 |
| **CR-V10-4** | Contexto transitivo de texto y cota (C:V11-06) | Read-set con cierre transitivo; `AnnotationScale` authored frente a `CANNOSCALE` | §8.7; PRE-17 |
| **CR-V10-5** | Anchos de modelo (C:V11-07) | Geometria ocupada completa transformada con la transformacion compuesta; no uniforme no soportada ⇒ `UNKNOWN` | §11.3 punto 5 |
| **CR-V10-6** | Guarda durable y ayuda sin alias (C:V11-08, C:V11-09) | G-M24 estructural sobre los productores de contexto; ventana de ayuda sin chip de alias y censo sin «/» final (T-M74) | §8.7; §13; §20 |
| **CR-V10-7** | Reconciliacion con I-55 e I-49 (C:V11-10..C:V11-12) | Reconciliacion obligatoria antes del freeze con la propiedad de las seis autoridades compartidas; STOP simetrico; R-45 antes del freeze; ADR-0041 vigente; ADR-0042 bajo vigilancia | §14.1; §15; §16; §19 |
| **CR-V10-8** | G3, ADR y proceso (C:V11-13..C:V11-15) | G3 ampliado; ADR-0036 alineado; desde V11, contradiccion material ⇒ Proposal V12 (historial; sustituida por CR-V11-8) | §12; §13; §14.1; ADR-0036 |
| **CR-V11-1** | Huella latente por mecanismo (C:V12-01, C:V12-02) | `ContentAvailability` y `VisibilityMechanism` obligatorios; inventario minimo de mecanismos; nunca contenido ausente = huella vacia | §11.3 puntos 5 y 14; CT-49 |
| **CR-V11-2** | XREF, recortes, visibilidad dinamica y campos (C:V12-03..C:V12-06) | XREF descargado, no resuelto u overlay anidado no disponible ⇒ `UNKNOWN`; recorte ⇒ ocupacion sin recortar o `UNKNOWN`; union de estados o `UNKNOWN`; cota para todos los resultados de un campo o `UNKNOWN` | §11.3 punto 5; CT-49; T-M73 |
| **CR-V11-3** | Entorno tambien en la copia (C:V12-07) | estilos anotativos, `DIMSCALE = 0`, estilo hijo y fuentes en CT-50; sin huella conservadora ⇒ `UNKNOWN` o limitacion de O-1 | §8.7; §11.3 punto 5; CT-50 |
| **CR-V11-4** | Transformacion uniforme con reflexion (C:V12-08) | `|sx| = |sy| = s`, signos admitidos, `ancho' = |s| · ancho`; anidado no soportado ⇒ `UNKNOWN` | §11.3 punto 5; CT-36; T-M71 |
| **CR-V11-5** | G-M24 con censo global (C:V12-09) | lado A declarado y lado B `ProducerSiteCensus`; sitio nuevo sin clasificar ⇒ RED | §8.7; G-M24; T-M75 |
| **CR-V11-6** | O-1 y autoridades compartidas (C:V12-10, C:V12-11) | `UNKNOWN` aunque parezca lejos en L-34 y M-35; L-37 ampliada; autoridades compartidas PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION | §1.3; §3.7; §8.1..§8.5; §20.1 |
| **CR-V11-7** | Paralelas (C:V12-12, C:V12-13) | I-49 `75f1862` con G6-C2; hechos de ADR-0041 y del indice corregidos; I-55 V2 y ADR-0042 V2; vigilancia de I-56 | §15; §16 |
| **CR-V11-8** | G3, ADR y proceso (C:V12-14..C:V12-16) | G3 ampliado; ADR-0036 alineado; relectura del WORKFLOW antes de cada transicion; desde V12, contradiccion material ⇒ Proposal V13 (historial; sustituida por CR-V12-8) | §12; §13; §14.1; ADR-0036 |
| **CR-V12-1** | Completitud de la base de datos y apertura parcial (C:V13-01, C:V13-02) | `DatabaseVisibilityCompleteness` antes de ST-19; apertura parcial ⇒ E12 con mensaje propio y re-verificacion en R6 y MUTATE; sin PARTIALOAD ni cargas forzadas | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 |
| **CR-V12-2** | Carga bajo demanda de XREF y completitud de la enumeracion (C:V13-03, C:V13-04) | «XREF cargado» no basta: `UNKNOWN` salvo contenido completo demostrado; orden logico de etapas sin saltos | §11.3 puntos 5 y 14; CT-49; T-M73 |
| **CR-V12-3** | Mecanismos latentes añadidos (C:V13-05) | atributos invisibles y `ATTDISP`, aislamiento, proxies y datos externos de tabla, cada uno `SUPPORTED(metodo latente conservador)` o `UNKNOWN` | §11.3 punto 5; CT-49 |
| **CR-V12-4** | Frontera entre estado y edicion (C:V13-06) | estados que no cambian el orden dentro de la garantia; ediciones futuras fuera | §1.3 L-37; §11.3 punto 5 |
| **CR-V12-5** | Censo de G-M24 por simbolos de API (C:V13-07) | familias de simbolos; call-site `DECLARED` o `EXCLUDED` con motivo; T-M75 con productor por clonado | §8.7; G-M24; T-M75 |
| **CR-V12-6** | X-1..X-8 y hechos de I-55 (C:V13-08, C:V13-09) | entrada vinculante de la reconciliacion; marca provisional ampliada; I-55 `d091eeb`; «complementa y precisa» | §3.3..§8.5; §15.4; §20.1 |
| **CR-V12-7** | I-56 y O-1 (C:V13-10, C:V13-11) | reglas de transicion con I-52 grandfathered; L-34, L-37 y M-35 | §1.3; §13; §14.1; §15; §19 |
| **CR-V12-8** | G3, ADR y proceso (C:V13-12..C:V13-14) | G3 ampliado; ADR-0036 alineado; entradas X-* acordadas en la reconciliacion; desde V13, contradiccion material ⇒ Proposal V14 (historial; sustituida por CR-V13-8) | §12; §13; §14.1; ADR-0036 |
| **CR-V13-1** | Sesiones de edicion activas (C:V14-01) | REFEDIT o transaccion larga y Editor de bloques ⇒ E12 en P0, re-verificados en R6 y MUTATE; sin REFCLOSE ni BCLOSE | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 |
| **CR-V13-2** | ERASE/OOPS y precedencia estado/edicion (C:V14-02, C:V14-03) | ERASE/OOPS edicion futura; resolucion al mismo contenido = estado; a otro contenido = edicion; `DataLink` estado aunque cambie su fuente; tabla de busqueda que cambia geometria = edicion | §1.3 L-37; §11.3 punto 5 |
| **CR-V13-3** | Orden P0/R2/R3/R6 (C:V14-04) | P0 solo global; R2 temprano; R3 solo rechaza; R6 definitivo; MUTATE revierte | §4.1; §9.1; §9.4 |
| **CR-V13-4** | Censo de G-M24 ampliado (C:V14-05) | cabeceras de `Database`, tablas de simbolos, evaluacion grafica, importacion, atributos y comando/COM; T-M75 con dos violaciones | §8.7; CT-50; T-M75; G-M24 |
| **CR-V13-5** | Marcos de recorte (C:V14-06) | huella = contenido sin recortar ∪ marco, o `UNKNOWN`; modelos de coordinacion | §11.3 punto 5; CT-49 |
| **CR-V13-6** | I-49 A3 y ubicaciones compartidas (C:V14-07, C:V14-08) | A3 cambia D19 y D20; autoridad final de I-49 antes del freeze o FREEZE BLOCKED; §14.2 G4..G7 provisionales | §9.2.1; §10.2; §14.1; §14.2; §15.3 |
| **CR-V13-7** | O-1 e I-56 (C:V14-09, C:V14-10) | L-34, L-37 y M-35 con REFEDIT, Editor de bloques, ERASE/OOPS y `XLOADCTL`; I-56 en `0d66df2` sin efecto de politica | §1.3; §13; §15 |
| **CR-V13-8** | G3, ADR y proceso (C:V14-11..C:V14-13) | G3 ampliado; ADR-0036 alineado; desde V14, contradiccion material ⇒ Proposal V15 | §12; §13; §14.1; ADR-0036 |
| **CR-V14-1** | Politica cerrada de sesiones de edicion (C:V15-01, C:V15-02) | `EditingSessionPolicy` con `CHARACTERIZED_SAFE` o `BLOCKED`; no caracterizado o no demostrablemente ausente ⇒ E12; ARRAYEDIT Source y ventana de prueba de bloques activos ⇒ E12; sin REFCLOSE, BCLOSE ni ARRAYCLOSE | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4; §11.3 punto 5; CT-49 |
| **CR-V14-2** | Familia de escritura de G-M24 (C:V15-03) | apertura para escritura, mutacion de presentacion, registros de simbolo existentes y estado global; T-M75 con tres violaciones; censo inicial de escrituras en CT-50 | §8.7; CT-50; T-M75; G-M24 |
| **CR-V14-3** | Precedencia causal de los campos (C:V15-04) | EDIT CAUSE > FIELD REEVALUATION; resultados alcanzables solo por estado; `DataLink` sigue siendo estado | §1.3 L-37; §11.3 punto 5 |
| **CR-V14-4** | Identidad de contenido de los XREF (C:V15-05) | mismo contenido efectivo con cualquier ruta = estado; contenido efectivo distinto = edicion; identidad no demostrable ⇒ `UNKNOWN` | §1.3 L-37; §11.3 punto 5; CT-49 |
| **CR-V14-5** | Autoridad vigente y cancelacion de I-49 (C:V15-06, C:V15-07) | ADR-0041 D19 no es la autoridad de RS-3; A3-R1 registrado; salida A, B o C decidida por el Coordinador, sin desbloqueo automatico | §9.2.1; §10.2; §14.1; §15; §17 R-55 |
| **CR-V14-6** | Marcadores de I-55 e I-56 vigente (C:V15-08, C:V15-09) | `d091eeb` no es consenso formal; diferencias de V3 pendientes; Proposal V1 de I-56 sin efecto sobre I-52 | §3.3; §3.7; §3.8; §5.1; §5.4; §6.1; §8.1..§8.4; §15 |
| **CR-V14-7** | G3 y O-1 (C:V15-10, C:V15-11) | casos de sesiones, campos, dinamicos, XREF y G-M24; L-34, L-37 y M-35 ampliados | §1.3; §12; §13; §14.2 |
| **CR-V14-8** | ADR y proceso (C:V15-12, C:V15-13) | ADR-0036 alineado; estado formal de I-55 antes del freeze; desde V15, contradiccion material ⇒ Proposal V16 | §14.1; §16; ADR-0036 |
| **CR-V15-1** | Autoridad observable de sesiones (C:V16-01) | `EditingSessionAuthoritySet`; PASS solo con host caracterizado, lecturas con exito, detectores `BLOCKED` inactivos y autoridades genericas sin sesion; la ejecucion no descubre modos; modo sin detector ⇒ STOP de caracterizacion | §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4; §11.3 punto 5 |
| **CR-V15-2** | Metodo cerrado, host y lectura fallida (C:V16-02..C:V16-04) | variables de estado de edicion (`ARRAYEDITSTATE` incluida), autoridades de transaccion y de comando; `DETECTOR`, `IRRELEVANT` o `UNSUPPORTED`; host no caracterizado ⇒ E12; lectura fallida ⇒ E12, nunca inactivo | §11.3 punto 5; CT-49; T-M73; R-57 |
| **CR-V15-3** | Mutadores internos (C:V16-05) | `SynchronizeAttributes`, `LayerStateManager.Restore*`, `ReloadXrefs`/`ResolveXrefs`/`BindXrefs`, `AssocManager.Evaluate*`, `Audit`, `ResetBlock`, `ConvertToStaticBlock` y equivalentes en G-M24 | §8.7; CT-50; T-M75; G-M24 |
| **CR-V15-4** | Overrules y reactores de RackCad (C:V16-06) | registro de overrules y suscripciones con escrituras diferidas en G-M24; T-M75 con cinco violaciones | §8.7; CT-50; T-M75; G-M24 |
| **CR-V15-5** | Overrules de terceros en las huellas (C:V16-07) | representacion afectada por overrule o reactor activo sin soporte conservador ⇒ `UNKNOWN`; sin inspeccionar plugins | §1.3 L-37; §11.3 punto 5; CT-49; T-M73; R-58 |
| **CR-V15-6** | Campos mixtos y XREF (C:V16-08, C:V16-09) | entradas de edicion fijas; XREF por contenido cargado actual y completitud, sin comparar identidades futuras | §1.3 L-37; §11.3 punto 5; CT-49; T-M73 |
| **CR-V15-7** | I-56, G3 y O-1 (C:V16-10..C:V16-12) | I-56 en `fde0386` sin efecto sobre I-52; casos G3 nuevos; L-34, L-37 y M-35 ampliados | §1.3; §12; §13; §14.2; §15 |
| **CR-V15-8** | ADR y proceso (C:V16-13, C:V16-14) | ADR-0036 alineado; desde V16, contradiccion material ⇒ Proposal V17 | §14.1; §16; ADR-0036 |
...
- **AR15-01: autoridad observable.** La revision de V15 agrupaba autoridad, metodo, host y lectura fallida; el
  Coordinador los separa y fija PASS solo con todas las condiciones (`[V16-D01]`..`[V16-D04]`). Precisiones del
  ejecutor: la identidad del host es un detector del propio conjunto; la misma causa detectada al re-verificar da E10 en
  R6 y E11 en MUTATE; las lecturas no cambian variables ni sesiones. Si CT-49 no puede cerrar el metodo, el alcance
  colapsa y la Proposal V17 es obligatoria (R-57).
- **Overrules de terceros (`[V16-D07]`).** Es una regla de seguridad del dibujo, distinta de la guarda de codigo; en
  estaciones con plugins que registren overrules puede dejar `UNKNOWN` clases enteras (R-58). La alternativa de
  inspeccionar plugins de terceros se descarta.
- **XREF simplificado (`[V16-D09]`).** Retirar el disparador de identidad de V15 no relaja la seguridad: la huella sigue
  siendo la del contenido cargado y su completitud, y cualquier recarga con contenido distinto es edicion futura.
- **AR14-01: politica frente a censo.** La revision de V14 pedia un censo cerrado de sesiones; el Coordinador lo
  convierte en `EditingSessionPolicy` con dos resultados y fija el primer corte de ARRAYEDIT Source y de la ventana de
  prueba de bloques (`[V15-D01]`, `[V15-D02]`). Precision del ejecutor: el inventario minimo activo es `BLOCKED` y
  reclasificarlo exige Proposal V17, y la re-verificacion en R6 y MUTATE se hace siempre. Sin deteccion fiable, la regla
  por defecto bloquea RACKMIRROR (R-57).
- **A3-R1 de I-49 (`[V15-D06]`).** A3-R1 clasifica D19 como «leida con A3» y no como enmendada directamente; V15
  conserva la formula de la orden (A3 cambia la semantica futura de las observaciones del `PlanReadSet`) sin afirmar que
  el texto de D19 cambia. La clasificacion no altera RS-3 = PENDING I-49 FINAL AUTHORITY.
- **Cancelacion de I-49 (`[V15-D07]`).** Deliberadamente no hay desbloqueo automatico: la decision A, B o C es del
  Coordinador en una Proposal (R-55).
- **Campos y `DataLink` (`[V15-D04]`).** La regla causal (EDIT CAUSE > FIELD REEVALUATION) y la excepcion explicita de
  `DataLink` (datos externos cambiados + refresco = ESTADO) conviven: `DataLink` es el unico cambio de fuente externa
  que se admite como estado.
- **Identidad de XREF (`[V15-D05]`).** La identidad es la del contenido efectivo; en ejecucion solo se necesita cuando
  el contenido cargado puede no corresponder a la resolucion actual, y nunca se abren archivos externos para
  demostrarla.
- **AR13-01: el Coordinador separa sesiones y borrado.** La revision de V13 agrupaba en un cambio (R:V14-01) las
  sesiones de edicion y los objetos borrados; el Coordinador los separa, decide que ERASE/OOPS es edicion futura y fija
  la precedencia exacta entre estado y edicion (`[V14-D01]`..`[V14-D03]`). Precision del ejecutor: E12 lleva un mensaje
  por causa y los prerrequisitos globales se re-verifican siempre en R6 y en MUTATE.
- **OOPS fuera de la garantia (`[V14-D02]`).** Un objeto borrado antes del comando y restaurado despues puede quedar por
  encima o por debajo de la copia; es una limitacion declarada (L-37, R-54), no un fallo cerrado. CT-49 solo lo
  caracteriza.
- **Precedencia de `DataLink` (`[V14-D03]`).** El refresco de `DataLink` es estado aunque su fuente externa haya
  cambiado, mientras que editar y recargar un XREF es edicion; por eso una tabla con `DataLink` solo es `SUPPORTED` con
  cota para todos sus resultados y en la practica sera `UNKNOWN` (R-54).
- **Autoridad final de I-49 (`[V14-D07]`).** Condiciona el freeze de I-52 aunque RS-3 solo se use si I-49 integra antes;
  puede retrasar el freeze (R-55).
- **Censo ampliado (`[V14-D05]`).** «Donde sea relevante» evita clasificar sin motivo `TransformBy`, lecturas de
  cabecera o altas en tablas de simbolos fuera del camino de materializacion; toda exclusion lleva su motivo (R-53).
- **Elegibilidad resuelta (`[V11-D01]`).** La igualdad de fuentes simbolicas sigue siendo la prueba de equivalencia
  entre homologas; la elegibilidad de la presentacion resuelta es una pregunta aparte que se decide en el dibujo destino
  aunque la evidencia de simetria use el contexto de la autoridad origen. Precision del ejecutor: la base auxiliar por
  pieza solo vale si reproduce exactamente la resolucion global (`[V11-D16]`).
- **Cola de Model Space (`[V11-D05]`).** R6 y el inicio de MUTATE enumeran de nuevo todos los objetos posteriores a cada
  fuente; un objeto anterior que sigue siendo anterior no invalida.
- **G-M24 (`[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`).** Tiene dos lados: productores declarados y censo de
  sitios productores por familias de simbolos de API ampliadas; no depende de nombres de archivos ni de hashes. Su forma
  concreta (analisis estructural, lista declarada y exclusiones con motivo) se fija en G5..G7 sin tocar los drawers
  (§20.3), y CT-50 fija el censo inicial.
- **Ayuda sin alias (`[V11-D09]`).** La unica produccion de UI añadida es no pintar el chip de una entrada sin alias en
  `RackCommandHelpWindow`; el censo de ventanas no cambia.
- **Entorno en la copia (`[V12-D07]`).** La regla de entorno se aplica a las huellas de la copia; si una configuracion
  habitual (por ejemplo un estilo de cota anotativo) deja anotaciones en `UNKNOWN`, la alternativa es una limitacion
  explicita de O-1 decidida con el Owner, no una huella optimista (R-50).
- **Reflexion uniforme (`[V12-D08]`).** Es una aclaracion: las colocaciones y piezas con `MirroredX` o `MirroredY`
  tienen magnitud uniforme; solo lo no conforme o un anidado no soportado es `UNKNOWN`.
- **Autoridades compartidas (`[V12-D11]`, `[V13-D08]`).** V13 fija la semantica que I-52 necesita y deja propiedad,
  namespace, ubicacion, forma de API y primitivo neutral a la reconciliacion; ninguna seccion fija ya la ubicacion del
  codec, y la marca provisional cubre tambien §3.3, §5.4, §6.1, CT-04, CT-05 y CT-16.
- **I-56 (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`, `[V16-D10]`).** Vigilancia de proceso, sin adoptar reglas
  no integradas ni hipotesis del Evidence Audit; sus Proposals V1 (`18401da`) y V2 (`fde0386`, con Architect CHANGES
  REQUIRED en `f97a8b0`) no tienen consenso y declaran no aplicarse a I-52; su contrato declara a I-52 grandfathered y
  WORKFLOW V2 NOT EFFECTIVE; las futuras autoridades de proceso se leen con sus reglas de transicion y, mientras el
  invariante aplique, I-52 conserva su workflow de reclamo.
- **Terminologia (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`, `[V13-D16]`, `[V15-D15]`, `[V16-D16]`).** Huella de la
  referencia, huella visual, `FootprintInputFingerprint`, `TraceFingerprint`, `ResolvedPresentationEligibility`,
  `ModelSpaceFootprintPolicy`, `ModelSpaceOrderTailSnapshot`, `ContentAvailability`, `VisibilityMechanism`,
  `ProducerSiteCensus`, `DatabaseVisibilityCompleteness`, `EditingSessionPolicy` y `EditingSessionAuthoritySet` son
  autoridades o registros distintos (§9.2).
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V17 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V16 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V16 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V17 y O-1 se reevalua.
...
- **ADR-0041 aceptado en I-49 (`[V11-D11]`, `[V12-D12]`, `[V13-D15]`, `[V14-D07]`, `[V15-D06]`).** Reemplaza a ADR-0040
  y, frente a ADR-0040, solo cambia D5..D8; que conservara D19 `PlanReadSet` y el dominio del consumidor es un hecho
  historico anterior a A3. La correccion G6-C2 (`75f1862`) solo toca `Expressions`. ADR-0041 es la autoridad aceptada
  vigente de I-49, pero su D19 **no** es la autoridad de RS-3 ni es apta para congelarlo: el Amendment A3 (`f6f0991`),
  revisado por A3-R1 (`c8cfee2`), cambia la semantica futura de las observaciones del `PlanReadSet` de D19 —leida con
  M3, M4 y M10(a)— y el `Upstream` de D20. V14 corrigio a V13, que presentaba A3 como un cambio de D20 que no tocaba el
  contrato de I-52: el contrato actual sigue NON-MATERIAL solo porque RS-3 delega en la autoridad final de I-49, y antes
  del freeze de I-52 esa autoridad final acordada, aceptada y congelada debe estar disponible y releida o RS-3 = PENDING
  I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED; si I-49 se cancela o abandona, el freeze sigue bloqueado hasta la
  decision del Coordinador (R-55). D20 es la `RepairDecisionObservation` de `RepairBrokenRack`, que I-52 no usa.
- **I-55 (`[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`, R-45).** I-52 no espera al consenso de I-55,
  pero no congela sin la reconciliacion obligatoria de las autoridades compartidas; hoy no hay contradiccion material
  presente: la tabla X-1..X-8 de G2C (`d091eeb`), reclasificada como revision tecnica adversarial y aceptada del lado de
  I-55 por su Coordinador, no tiene MATERIAL CONFLICT con I-52, y la Proposal V3 (`8e35a51`) propone diferencias sin
  conflicto de contenido que la reconciliacion debe decidir, I-55 adopta la misma regla de reconciliacion y ADR-0042
  complementa ADR-0010 sin cambiar Actualizar. V13 marco esas autoridades como provisionales y registro X-1..X-8 como
  entrada vinculante; V16 mantiene ambas cosas, no presenta `d091eeb` como consenso formal, registra sin adoptarlas las
  precisiones de la Proposal V4 de I-55 (`fe70d7a`: redibujo atomico de hermanas en Insertar y X-4, X-5 y X-6) y no
  decide el reparto (`[V15-D08]`, `[V15-D14]`).
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V17** o reduccion
  de alcance.
...
- Un cambio posterior en main altera la frontera estado del editor → sistema mas alla de lo registrado en §15.1 (I-53S
  ...
  **reportar antes de continuar** (§15.3). Como I-53S e I-53D integraron antes del Candidato de I-52, G6/G7 no cierran
  ...
  `[V10-D23]`).
- **Reconciliacion entre iniciativas (`[V11-D10]`).** Sin la reconciliacion obligatoria I-52/I-55 (e I-49 donde aplique)
  registrada —propiedad de la taxonomia de tipo de vista, del codec semantico de `View`/`Section`, de la autoridad
  «sistema resuelto → plan de vista», del comparador authored, del primitivo de materializacion del Plugin y de las
  transformaciones de colocacion y proyeccion— **no hay freeze**. Si cualquiera de las dos iniciativas intenta congelar
  una autoridad incompatible con la otra antes de reconciliar: **STOP**, ningun freeze para ninguna hasta resolverlo.
  Una autoridad duplicada: **prohibida**. Si ADR-0042 se acepta antes del freeze de I-52, se relee la autoridad que usan
  C-2, Actualizar e Insertar y se reconcilia antes de congelar. La reconciliacion parte de las ultimas entradas X-*
  acordadas como entrada vinculante (hoy X-1..X-8, §15.4), y una desviacion unilateral: **prohibida**. Las partes
  marcadas PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (§3.3, §3.7, §3.8, §5.1, §5.4, §6.1, §8.1..§8.5, §14.2
  G4..G7, §20.1, CT-04, CT-05 y CT-16) no se congelan por I-52 sola, tampoco en su ubicacion fisica (`[V12-D11]`,
  `[V13-D08]`, `[V14-D08]`). **Sin la autoridad final de I-49 disponible y releida para RS-3 (V6, A1, A2, A3, ADR de
  reemplazo, Consensus Freeze nuevo y estado de implementacion): RS-3 = PENDING I-49 FINAL AUTHORITY y no hay freeze**
  (`[V14-D07]`).
- **Proceso vigente (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`).** I-52 esta grandfathered y sigue el workflow bajo el que
  se reclamo; ninguna hipotesis del Evidence Audit de I-56 es politica. Antes de cada transicion del proceso se relee la
  autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y,
  mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo; adoptar una regla de I-56 no
  integrada: **prohibido**.
...
- Se intenta crear una copia sin la presentacion exterior de su referencia fuente, asignar un estilo de trazado
  ...
  **todo** objeto posterior a cada fuente (ST-19), mutar sin refrescar las huellas visuales tras `EnsureForPlan`
  ...
  (PRE-17): **prohibido**.
- Se intenta tratar la igualdad de fuentes simbolicas como elegibilidad de la presentacion resuelta, resolver esa
  elegibilidad con el contexto de la biblioteca, clasificar un objeto del dibujo con un fallback optimista o fuera de la
  `ModelSpaceFootprintPolicy`, excluir geometria de una huella de orden por la visibilidad transitoria, añadir un ancho
  local sin transformar a una linea media transformada, revisar solo los objetos posteriores ya capturados en lugar de
  enumerar de nuevo la cola o cambiar un productor de contexto con G-M24 en RED: **prohibido** (G-M23, G-M24,
  `[V11-D01]`..`[V11-D05]`, `[V11-D07]`, `[V11-D08]`).
- Se intenta tratar contenido ausente, descargado, no resuelto o recortado como huella vacia, abrir archivos externos
  para adivinar geometria ausente, usar solo el estado dinamico actual o el valor actual de un campo como autoridad de
  la huella de orden, aplicar la regla de entorno solo a los objetos posteriores, tratar una magnitud reflejada uniforme
  como no uniforme o integrar un sitio productor o consumidor sin clasificar en el censo de G-M24: **prohibido** (G-M23,
  G-M24, `[V12-D01]`..`[V12-D09]`).
- Se intenta ejecutar ST-19 o clasificar huellas con la base de datos abierta parcialmente o sin completitud demostrada,
  saltar una etapa del orden logico, tratar un XREF cargado bajo demanda como completo sin demostracion, ejecutar
  PARTIALOAD, forzar cargas, abrir archivos externos o cambiar capas o variables para completar una huella, tratar como
  no ocupados atributos invisibles, objetos ocultos o aislados, proxies o datos externos de tabla, o integrar un
  call-site productor sin clasificar por familias de simbolos de API: **prohibido** (G-M23, G-M24,
  `[V13-D01]`..`[V13-D05]`, `[V13-D07]`).
- Se intenta ejecutar RACKMIRROR con una edicion de referencia en curso o con el Editor de bloques abierto, ejecutar
  REFCLOSE o BCLOSE, forzar el cierre de una sesion o mover entidades del working set, evaluar en P0 la completitud de
  la cola por fuente o aprobar en R3, tratar ERASE/OOPS, un XREF resuelto a otro contenido o una fuente de imagen,
  underlay o nube de puntos reemplazada como estado protegido, omitir el marco de un objeto recortado, integrar un
  call-site de las familias ampliadas sin clasificar, congelar RS-3 sobre ADR-0041 D19 o fijar la ubicacion fisica de
  una responsabilidad compartida antes de la reconciliacion: **prohibido** (G-M23, G-M24, `[V14-D01]`..`[V14-D08]`).
- Se intenta ejecutar RACKMIRROR con ARRAYEDIT Source, la ventana de prueba de bloques, una transaccion larga u otro
  estado de sesion de edicion no caracterizado o no demostrablemente ausente, ejecutar ARRAYCLOSE o cerrar la ventana,
  razonar sobre la geometria fuente temporal de una matriz asociativa, reclasificar como `CHARACTERIZED_SAFE` un estado
  del inventario minimo sin Proposal V17, integrar un sitio de escritura o mutacion de objetos o contexto existentes sin
  clasificar, tratar como estado protegido el cambio de un campo causado por la edicion del contenido referenciado,
  clasificar un XREF por su cadena de ruta en lugar de por su contenido efectivo, presentar ADR-0041 D19 o `d091eeb`
  como autoridad o consenso vigente, o desbloquear el freeze o eliminar RS-3 porque I-49 se cancele o abandone sin
  decision del Coordinador: **prohibido** (G-M24, `[V15-D01]`..`[V15-D08]`).
- Se intenta descubrir modos de edicion desconocidos en ejecucion, tratar una lectura fallida o no clasificable de un
  detector como inactiva, ejecutar en un host o una version no caracterizados, congelar el `EditingSessionAuthoritySet`
  sin el metodo de descubrimiento cerrado de CT-49, dejar pasar un modo relevante sin detector fiable en lugar de
  detenerse, integrar un mutador interno de AutoCAD o un registro de overrule o de reactor sin clasificar, calcular la
  huella de un objeto que un overrule o reactor activo puede alterar sin equivalencia u ocupacion conservadora
  demostradas, inspeccionar internos de plugins de terceros, variar las entradas de edicion de un campo mixto o comparar
  en ejecucion identidades de archivos XREF futuros: **prohibido** (G-M24, `[V16-D01]`..`[V16-D09]`).
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V17.
...
- Debilitar una guarda de I-51, de main (incluidas las de I-53 y las de I-54, T-GRD-01..T-GRD-08) o de I-49 para
  ...
  de la ayuda se verifica si existe y su ausencia es un estado valido); G-M24 tampoco se relaja para aceptar un cambio
  de productor sin re-caracterizar.
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`, `[V13-D08]`).** Las ubicaciones de esta tabla que
> tocan fundacion compartida con I-55 —nucleo neutral en `Persistence/`, valor de colocacion junto a `Transform2D`,
> autoridades de plan por kind con `Resolve` + `Plan`, codec de vistas y `A_k`, primitivo de materializacion en
> `Systems/Shared/` y comparador authored— son probables y no las congela I-52 sola: se fijan en la reconciliacion
> previa al freeze, con X-1..X-8 como entrada vinculante (§14.1, §15.4).
...
| Application | carpeta nueva del espejo `src/RackCad.Application/Mirror/` (namespace `RackCad.Application.Mirror`; **nunca** `Systems/Shared`): contrato de reflector, registro, plan del espejo, politica de fallo cerrado del espejo sobre la disposicion del codec de vistas (la ubicacion del codec y de `A_k` se fija en la reconciliacion, `[V12-D11]`), equivalencia, contrato de datos `GeometrySymmetryResult` (con valores efectivos, variantes, fuentes visuales simbolicas tipadas, `VisibilityPath`, contexto de patron y orden visual) y su consumo, orden visual de plan sobre huellas (§11.3 punto 6.6) y consumo de `VisualFootprintResult`, declaracion de invariancia de metadata, allowlist de portadores, registro de retirados, autoridad V-DEP comun y tablas de cobertura del exterior; nombres que respetan las guardas de texto vigentes (G-M18) |
...
| Plugin | `RackMirrorCommands.cs` (nuevo) y SNAPSHOT propio; materializador en `Systems/Shared/`; `Systems/Shared/SystemBlockWriter.cs` (`CreateInTransaction`); **sonda visual-geometrica por estado** (solo lectura o `Database` auxiliar privada: contexto de la autoridad origen para la evidencia de simetria y `DestinationEvaluationContext` con DRAWING FIRST para la huella visual; definiciones de biblioteca solo por `BlockLibraryImporter.EnsureBlocks` y verificacion de bloques), politica cerrada de variantes, canonicalizacion conservadora, fuentes visuales simbolicas tipadas, `VisibilityPath`, contexto de patron, ruta anidada estable, orden visual, transformacion efectiva y `TraceFingerprint`, en G6 o G7 (§11.3); presentacion de la referencia fuente (ST-13, ST-18), orden en Model Space (ST-19) con relectura del dibujo real y frescura de las huellas tras `EnsureForPlan` (PRE-18) y `MaterializationContextReadSet` (§8.7) en el SNAPSHOT, PREPARE y `CreateReference`; productor unico de huellas visuales de ocupacion de modelo con su `FootprintInputFingerprint` (`VisualFootprintResult`, G-M23); costura unica de aplicacion de parametros compartida con el materializador si es viable (G-M19); `ModelSpaceFootprintPolicy` y captura de la `ModelSpaceOrderTailSnapshot` en R6 y al inicio de MUTATE; resolucion de la elegibilidad de la presentacion en el `DestinationEvaluationContext` (`[V11-D01]`..`[V11-D05]`) |
| Plugin | `RackDuplicarCommands.cs` **solo** si se extrae un SNAPSHOT compartido; comandos de `RACKEDITAR` **solo** si se activa C-1 |
| UI | `RackCommandReference.cs` (entrada de ayuda de `RACKMIRROR` sin alias) y `RackCommandHelpWindow.cs` (sin chip de alias para una entrada sin alias, `[V11-D09]`) |

### 20.2 Pruebas

Caracterizaciones CT-01..CT-50; pruebas T-M01..T-M75 (con T-M17b); guardas G-M1..G-M24 (G-M24 estructural con
productores declarados y censo global de sitios productores por familias de simbolos de API ampliadas, `[V12-D09]`,
`[V13-D07]`, `[V14-D05]`); evidencia de viabilidad de la sonda en `acad.exe` y corpus indicativo con la huella de la
biblioteca en `docs/automation/evidence/` y, si hace falta, un harness en `tools/` (nunca en `src/` durante G3);
`SelectiveEditorOpenTests.cs` (censo `B + 1`), los censos de T-GRD-08 de I-54, ya en main, reapuntados con motivo
(`CustomPropertiesCommandGuardTests.cs`, `CustomPropertiesHelpCensusTests.cs`) y el censo T-GRD-02 de
`CustomPropertiesEnvelopeGuardTests.cs` clasificado con motivo (hoy 7 → 8, re-medido en el SHA de reconciliacion);
guardas de I-51 reapuntadas en `SelectiveDuplicationFailClosedTests.cs` solo si se extrae el SNAPSHOT; pruebas de UI en
`tests/RackCad.UI.Tests` para C-2 (G6), la de la ayuda sin alias (T-M74, G7) y las caracterizaciones de UI (G3).
...
salvo consumo, `Catalogs/*` salvo consumo (I-19), `KindHandlers/*`, editores WPF (la ventana de ayuda
`RackCommandHelpWindow.cs` solo se toca en G7 para no pintar el chip de una entrada sin alias, `[V11-D09]`), `assets/`,
catalogos, biblioteca de bloques, `.github/`, ADR aceptados, y la baseline de I-53 en main:
`src/RackCad.Application/Systems/Shared/Header*` con sus guardas (`HeaderBatchContractTests.cs`,
...
G2 = V16 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd), V10 (636f7fd), V11 (2275f21), V12 (915a520), V13 (dd45b0f), V14 (e59bc89) y V15 (ae8640a) =
     historial
     revision de V15: Architect: CHANGES REQUIRED — PROPOSAL V16
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V16, rebase sobre el main vigente, reconciliacion con I-55 e I-49 con
                     las ultimas entradas X-* acordadas, estado formal de I-55 resuelto, autoridad final de I-49
                     disponible para RS-3, freeze, re-check y O-1; antes de cada transicion, relectura de la autoridad
                     de proceso vigente con sus reglas de transicion; I-52 grandfathered)
