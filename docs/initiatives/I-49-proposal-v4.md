# I-49 — Proposal V4: motor de expresiones paramétricas (Expression Engine, ID22B)

> # ⚠ COORDINATOR PROPOSAL V4 — ARCHITECT RE-REVIEW V3 RECONCILED — NOT CONSENSUS
>
> # ⛔ Implementation remains BLOCKED
>
> ```text
> Proposal Version = V4
> Coordinator      = AGREED WITH V4
> Architect        = PENDING FINAL REVIEW
>
> A-03             = CLOSED
> Open A           = CLOSED
> Open B           = CLOSED
> Schema           = V-0
> CR-1..CR-5       = CLOSED IN V4
>
> ADR              = REQUIRED, not yet written
> Implementation   = BLOCKED
> ```
>
> Gate **G2D**. V4 reconcilia la **Architect Re-review de V3** (`Architect = CHANGES REQUIRED`: CR-1 MEDIUM, CR-2 a
> CR-5 LOW) sobre la base exacta de V3. El Coordinador **acepta** los cinco, y ninguno queda abierto (§0.1).
>
> - La re-revisión confirmó A-01…A-12 y A-14…A-18, aceptó el remedio modificado de A-03 y aceptó las precisiones de
>   `#GUID` y del token de `Type`. A-13 queda cerrado **sin ampliación** (CR-1).
> - Las decisiones cerradas en V3 **no se reabren** (§0.5).
>
> ```text
> Base V3          = 4df9480638bbeb9a95549a690db098b7f32a28ea   (I-49-proposal-v3.md; CI 34725828605 success)
...
> main observado   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (merge de I-51; no cambia ningún archivo que V4 cite)
> Override vigente = OWNER_OVERRIDE_I49_I50_PARALLEL   (docs/automation/decisions/I-49.md)
> Paralelas        = §12 (tips observados al abrir G2D)
> Estado de gates  = G0 · G1 HECHAS · V1–V3 historial · G2D V4 EN CURSO · G3 PENDIENTE
> ```

---

## 0. V3 → Architect Re-review → V4 Reconciliation

### 0.1 Tabla de reconciliación de CR-1…CR-5
...
**Resultado**: 5 de 5 cerrados en V4. Ninguno queda abierto.

### 0.2 Veredictos de la re-revisión que V4 conserva

| Tema | Veredicto de la re-revisión | Estado en V4 | Dónde |
|---|---|---|---|
| A-01…A-12, A-14…A-18 | SATISFIED | Sin cambio | §0.4 |
| A-13 | NOT SATISFIED (ampliación de V3) | **ACCEPTED sin ampliación** por CR-1 | §0.4, P9.7 |
| A-03 modificado | ACCEPTED | CLOSED; CR-2 completa su aplicación en propagación | §0.3, P8.10 |
...
### 0.3 A-03 — remedio modificado (ACCEPTED; CR-2 completa su aplicación)
...
- **Cómo lo aplica V4** (P8.10) **[V4 · CR-2]**. Para cada símbolo del cierre `I`, igual para literal y expresión:
...
### 0.4 V2 → Architect Review → Coordinator Reconciliation (heredada de V3)

La tabla de V3 se conserva como historial de la reconciliación de A-01…A-18. La única fila que cambia en V4 es
A-13, que queda **ACCEPTED sin ampliación**.

| # | Severidad (Architect) | Regla V2 | Decisión del Architect | Decisión del Coordinador | Regla vigente en V4 |
|---|---|---|---|---|---|
...
| **A-03** | HIGH | P21.7, P17.7 | Sin dominio en variables; dominio de la propiedad sobre el efectivo de toda fuente no literal | **ACEPTADA EN PRINCIPIO y MODIFICADA**; la re-revisión la **acepta** y CR-2 completa su aplicación en propagación (§0.3) | P5.5, P8.10, P21.7, P23.6, P24.5 |
...
| **A-05** | MEDIUM | P21.6 | La re-lectura de commit compara el conjunto leído del plan, no solo `I` | **ACEPTADA**: `PlanReadSet` contractual, declarado en V4 como revisión acotada de V8-R05 (CR-5) | P21.6 |
...
### 0.5 Decisiones cerradas (no se reabren en V4)

| Tema | Estado | Dónde |
|---|---|---|
| A-03 modificado (tres fronteras) | CLOSED — ACCEPTED | §0.3, P8.10 |
...
| `PlanReadSet`, también sin `RegistryMutation` | CLOSED | P21.6 |
...
### 0.6 Qué conserva V4 de V3

Todo lo que no enumera §0.1: las dos superficies de expresión con el mismo control; el modelo híbrido con `=X`
canónico; el núcleo neutral; ids en todo lo persistido; la gramática aritmética con la regla de la coma; las unidades
como conversión numérica; el grafo derivado; la detección iterativa de ciclos; el orden determinista; `Rename`
registry-only; un plan, una transacción, un commit y un `Regen`; el literal congelado; las garantías de C4, Enter,
Escape y LostFocus; los 21 escenarios de P28.5 (con T-V3-10, T-V3-13 y T-V3-16 ajustados); ADR REQUIRED sin número.
...
- **Es** la Proposal V4 del Coordinador, con `Coordinator = AGREED WITH V4`, sometida a **revisión final** del
  Arquitecto. Cada regla está **propuesta** hasta que haya consenso.
- **No es** consenso, **no** autoriza implementación, **no** es un ADR y **no** modifica V1, V2, V3 ni el
...
- **Architect Review de V2** (`CHANGES REQUIRED`) y **decisiones del Coordinador de G2C**, reconciliadas en §0.4.
- **Architect Re-review de V3** (`CHANGES REQUIRED`, CR-1…CR-5) y **decisiones del Coordinador de G2D**,
  reconciliadas en §0.1.
...
### 2.3 Lo que V4 extiende o revisa
...
| Decisiones de I-48 §4 (V8-R01): re-lectura y re-acreditación en commit | La re-lectura también valida ids y re-evalúa el `PlanReadSet`, y ocurre también en planes sin `RegistryMutation` que escriben racks | P21.6 |
| **I-48 Proposal V8 §6 (V8-R05)**: concurrencia fuera de alcance; sin comparación entre preflight y commit, sin concurrencia optimista y sin rechazo de plan obsoleto **[V4 · CR-5]** | **Revisión ACOTADA**: un valor que participó en el `PlanReadSet` y cambia entre preflight y commit → `ABORT BEFORE WRITE`. Afecta también a planes que ya existían, como un cambio de valor con solo referencias directas. Sin hash, token global, comparación completa, locks ni rechazo por cambios fuera del `PlanReadSet`. **Exige ADR y aceptación del Owner** | DR-8, P21.6, §9, §13 |
...
| 21 | propagation | R1–R3; contrato raíz A–D; `PlanReadSet` como revisión acotada de V8-R05; reparación generalizada; un plan y un `Regen` | A-01 · A-03 · A-05 · A-12 · OPEN B · **CR-2** · **CR-4** · **CR-5** |
...
| 28 | tests | 21 escenarios de G2C (tres ajustados) y 11 escenarios de G2D; guarda de unidades acotada | V3 · **CR-1…CR-5** |
| 29 | migration | Sin migración de datos ni de versión | — |
| 30 | non-goals | Actualizado con las decisiones cerradas, CR-1, CR-4 y CR-5 | V3 · **CR-1** · **CR-4** · **CR-5** |
...
- **P1.11 — Texto sin `=`.** Sigue siendo un literal parseado **exactamente como hoy** en cada superficie: regla
  localizada de ADR-0015 en RACKVARIABLES e invariante en el editor vinculable. V4 no resuelve esa divergencia
...
- **P5.3 — Ámbitos.** Existe **uno** activo, `Project`: un valor por dibujo. Queda reservado `Rack`: un valor por
  ...
  expresión de propiedad se evalúa en el contexto de su rack y en V4 solo ve símbolos `Project`.
...
- **P5.7 — Tres fronteras de validez** **[V3 · A-03 modificada]** (DR-5; §0.3):
...
- **P6.7 — Contexto de una propiedad de rack.** Para evaluar la expresión de una propiedad, el resolver usa el
  contexto del snapshot y los valores **ya evaluados** de `RegistryEvaluation` (P14.9). En V4 no añade símbolos
  propios del rack.
...
- **P8.8 — Expresión de propiedad o de definición.** Puede referenciar cualquier variable de `VariableType`
  numérico; en V4 todos lo son, porque solo existe `Length`. El resultado es un `double` que el consumidor
...
- **P8.10 — Contrato raíz del `VariableType`** **[V3 · A-03 modificada · V4 · CR-2]** (§0.3). El adaptador lo
  ...
  - **`Length` al escribir.** El contrato vigente del producto es `> 0`: RACKVARIABLES rechaza cualquier otro valor
    ...
    §10.3, §11.4). En V4 se aplica a **cada símbolo del cierre `I`** del estado «después», según su estado «antes»:
...
- **P9.6 — Clasificación de las constantes existentes** **[V4 · CR-1]** (hechos verificados en `a4d88f1`). Una
  ...
  | Constante | Ubicación | Papel | Clase | Tratamiento en V4 |
  |---|---|---|---|---|
...
- **P10.6 — Candidatos evaluados y DIFERIDOS: `ROUND`, `CEILING`, `FLOOR`.**
  ...
  | Pregunta que bloquea | Por qué no se decide en V4 |
  |---|---|
...
- **P12.2** — Nodos: los símbolos de la tabla (en V4, las variables de proyecto). Aristas: dueña → dependencia, con
...
- **P13.6** — Las expresiones de propiedad **no** forman ciclos: nada puede referenciar una propiedad en V4 (ID21
...
- **P15.2 — Severidad.** En V4 todo diagnóstico es `Error`: no hay avisos que permitan continuar.
- **P15.3 — Catálogo de V4.**
...
- **P15.8 — Cambios respecto de V2.**
  ...
  - Por A-03 modificada: `OutOfRange` **se conserva** para la raíz de una variable, pero solo al escribir. El
    ...
    §0.3, y la re-revisión aceptó ese remedio. Al leer ninguna variable recibe `OutOfRange`, lo que coincide con la
...
- **P18.2 — Qué hace hoy una build I-47/I-48** ante lo que V4 persiste (hechos del código en `a4d88f1`):
...
- **P18.7 — Qué NO justifica V-0.** El número de archivos tocados no es un argumento. Tampoco el precedente de I-50
  sobre el mismo portador, que V4 registra solo como hecho de coordinación (§12).
...
- **P21.6 — `PlanReadSet` y re-lectura de commit** **[V3 · A-05 · V4 · CR-5]**. V2 afirmaba que el efectivo de cada
  rack dependía solo de los valores de `I` y de su authored. Es falso: un rack planificado puede leer variables fuera
  de `I`.
  ...
  - **Secuencia en commit**, dentro de Application (`RegistryCommit.Prepare`, Discovery §3.1), no en el Plugin:
    ...
    ```text
    lastRead   = Read(...)                                // también sin RegistryMutation si el plan lee símbolos
  ...
  - **También sin `RegistryMutation`.** Hoy el ejecutor solo re-lee el registro cuando el plan lo escribe
    (`src/RackCad.Plugin/ProjectVariableMutationExecutor.cs:252-275`). En V4, todo plan cuyos racks lean símbolos,
    como una reparación que deja fuentes sanas, re-lee, re-acredita y valida su `PlanReadSet` en la misma
    transacción **antes** de escribir la primera vista, aunque no escriba el registro. Un plan cuyos racks no leen
    ningún símbolo no necesita esa re-lectura.
  - **Una sola autoridad.** La lógica vive en Application y el ejecutor del Plugin solo la invoca; el Plugin sigue
    siendo el único que lee el NOD (ADR-0006). La evaluación de la re-lectura es la `RegistryEvaluation` de ese
    snapshot, y se compara contra los valores esperados que el plan transporta como datos: no hay una segunda
  ...
  - **Revisión ACOTADA de la política de concurrencia de I-48 (V8-R05)** **[V4 · CR-5]**. La regla nueva es
    ...
    - Un cambio ajeno al `PlanReadSet`: **NO ABORT**. Los errores **fuera** del `PlanReadSet` tampoco bloquean ese
      commit (§5.2, decisión 9).
    - **Qué revisa.** V8-R05 (I-48 Proposal V8 §6) excluía comparar el registro de preflight con el de commit, la
      concurrencia optimista y el rechazo de planes obsoletos. V4 introduce una versión acotada de las tres cosas,
      limitada al `PlanReadSet`, y la aplica también a planes que ya existían: cambiar el valor de X en un rack que
      además lee Y por referencia directa aborta si Y cambió. Se conserva el contrato central de V8-R05: una
      re-lectura que cambió pero sigue siendo usable **no** se rechaza por el mero hecho de haber cambiado.
    - **Qué no cubre.** El authored de los racks no entra en el `PlanReadSet`: el bucle de RACKVARIABLES es modal y
      re-lee en cada vuelta (Discovery §10.1). El reconciliador de RACKEDITAR no produce un `MutationPlan` (P21.12).
  ...
  - **Casos fijados por prueba**:
    - **A-05**: en el rack R, `verticalClearance = X` (X está en `I`) y `palletTolerance = Y + 1` (Y no). Si Y cambia
      entre preflight y commit, el commit aborta antes de escribir porque Y ∈ `PlanReadSet`;
    ...
    - **cambio ajeno**: una variable Z que ningún rack del plan lee cambia en la re-lectura; el commit sigue;
...
- **P21.12 — Edición desde RACKEDITAR.**
  ...
  - **Sin `PlanReadSet`** **[V4 · CR-5]**. El reconciliador no produce un `MutationPlan`: resuelve contra la lectura
    única del comando y escribe dentro del mismo comando modal (`RackSelectivoCommands.cs:66,144-151`). V4 no le
...
- **P23.12 — Qué cambia respecto del contrato de I-48**, y que el ADR tiene que formalizar (§9):
  ...
  | Comportamiento de I-48 | V4 | Pines afectados (Discovery §14.2–§14.3) |
  |---|---|---|
...
- **P23.13 — `RackSelectiveWindow`.** Su `Describe` usa hoy `Text.TrimStart('=')` como nombre de la variable
  (Discovery §9.6). V4 lo sustituye por el texto de estado que produce la sesión: un cambio de un miembro.
...
- **P24.8 — Duplicación y restamp.** La copia conserva `PropertyValues`, entradas `expression` incluidas, con los
  ...
  el Plugin. V4 no toca ese camino y prueba en un archivo **propio**, contra el restamp integrado, que una entrada
...
- **P25.5** — Las expresiones de propiedad son el consumidor natural de un futuro ámbito `Rack`: el contexto de P6.7
  es donde ID20 añadiría símbolos del rack. V4 no añade ninguno.
...
- **P25.7 — Lo que ID20 tendrá que resolver, y V4 no**: `Rack.Frentes` es un conteo **por fondo**, `Rack.Niveles`
...
- **P26.4 — Lo que añadiría ID23, y V4 no**: sus namespaces y ámbitos, un resultado entero y la semántica de redondeo
  para `Quantity` (hoy `int`, Discovery §16.3; P10.6), la persistencia de fórmulas de BOM y su propio ADR.
- **P26.5** — Sin ensamblado separado en V4 (§10, ALT-17): la guarda marca la frontera.
...
- **P27.2 — ID29 (Impact Preview)**. Se expone **antes** de ejecutar:
  ...
  Hoy el preflight solo expone los racks (Discovery §16.4). V4 lo usa en los mensajes de RACKVARIABLES.
...
- **P28.5 — Escenarios obligatorios añadidos en V3** **[V3]**. Cada uno es una prueba de comportamiento con su gate:
  ...
  | T-V3-10 | Carrera del `PlanReadSet` (A-05) | `verticalClearance = X`, `palletTolerance = Y + 1`; Y cambia entre preflight y commit → aborto antes de escribir porque Y ∈ `PlanReadSet`; los casos heredados, ajenos y sin `RegistryMutation` están en T-V4-07 a T-V4-09 | G9 |
...
- **P28.7** — V4 no exige refactorizar los fixtures duplicados que el Discovery contó (≈25 clases, Discovery §14.1).
  Las pruebas nuevas pueden introducir un único constructor de registros y fuentes con expresiones.
- **P28.8** — Checklist de validación del Owner: §8.3.
...
- **P30.12** — Control de concurrencia más allá de la revisión acotada del `PlanReadSet` (P21.6): hash de snapshot,
  token global, comparación completa del registro, rechazo por cambios fuera del `PlanReadSet` o locks adicionales.
...
9. **Re-lectura de commit** (P21.6) **[V4 · CR-5]**:
   - re-lectura estructuralmente ilegible → aborto;
   - re-acreditar y validar ids;
   - re-evaluar el `PlanReadSet` y abortar antes de escribir si algo falla o si cambió un valor que participó en él,
     también en planes sin `RegistryMutation` que escriben racks;
   - los cambios y errores fuera del `PlanReadSet` no abortan ese commit;
...
- **Mensaje de recuperación condicional** **[V4 · CR-4]**. El diagnóstico nunca promete siempre «corregir X
  ...
  2. **Condición de promesa.** Para una fuente con causa superior y causa raíz única X, el diagnóstico puede prometer
     ...
     consumen el cierre de X tienen causa superior con causa raíz exactamente X. Solo entonces un `ChangeDefinition`
     válido de X deja esos racks resueltos sin reparar.
  ...
  6. **Sin promesas futuras.** El diagnóstico no afirma una recuperación que no pueda demostrar en el estado
     diagnosticado. «Permitiría» significa que R1 no bloqueará la corrección; la definición nueva de X sigue sujeta a
     R2 y al contrato raíz (P8.10).
...
> §8). La división de G5–G10 puede ajustarse en la revisión final.
...
| **G2** | Proposal | V1 `b15e40a` → V2 `9aef7d0` (`Architect = CHANGES REQUIRED`) → V3 `4df9480` (re-revisión: `CHANGES REQUIRED`, CR-1…CR-5) → **V4** (G2D) → revisión final del Architect, hasta `Architect = AGREED` | Architect Reviews | **EN CURSO** (G2D) |
...
14. Mensaje de recuperación: un rack cuya única fuente inválida depende de una variable rota muestra «Corregir
    <variable> permitiría recuperar esta fuente sin eliminarla.», y corregirla la recupera; si el rack tiene además
    una fuente con fallo propio, el mensaje no promete recuperación y muestra la fórmula que eliminaría la
    reparación.
...
- **Por qué es obligatorio.** V4 cambia contratos persistidos (P17), extiende ADR-0034 (§2.3), revisa de forma
...
- **Lo que tendrá que explicar de forma explícita**, tras G3:
  ...
  - el motor adimensional y las **tres fronteras de validez**: motor sin dominio, contrato raíz del `VariableType`
    ...
    endurecimiento declarado (§0.3, P8.10, P24.5);
  ...
  - los veredictos de `UnlinkAllAndDelete` y `RepairBrokenRack`, con el mensaje de recuperación condicional que no
    relaja R1 ni la decisión C4-13 de I-47 (§7);
  ...
  - **la revisión ACOTADA de V8-R05**: el `PlanReadSet`, con y sin `RegistryMutation`, sin hash, token global,
    ...
    tiene que aceptar antes de implementar** (DR-8, P21.6);
...
| ALT-12 | **`ROUND`, `CEILING` y `FLOOR`** ya en V4 | **Diferida**: semántica abierta (P10.6); confirmado por el Architect |
...
| ALT-17 | Núcleo en un **ensamblado separado** | **Diferida**: frontera más dura, pero añade un proyecto a builds, CI y despliegue; V4 usa namespace + guarda (P26.5) |
...
| ALT-21 | **Sin contrato en la raíz** de las variables definidas por expresión, con `A = -2` válido (remedio del Architect para A-03) | **Rechazada por el Coordinador** y aceptada así en la re-revisión: la validez dependería de la sintaxis (§0.3) |
...
| Pruebas | Núcleo nuevo, ≈20 archivos existentes, la evolución de P28.4, los 21 escenarios de P28.5 y los 11 de P28.6 | Discovery §14 |
...
- El commit re-evalúa el `PlanReadSet`, también en planes sin `RegistryMutation`, y eso revisa V8-R05 (P21.6).
...
- `RackSelectiveWindow`, archivo que también tocará I-50 en su G3, necesita un cambio (P23.13).
...
**Observado al abrir G2D** (`git fetch --all --prune`, 2026-09-12) y verificado de nuevo justo antes del commit:

```text
origin/architecture/motor-expresiones-parametricas = 4df9480638bbeb9a95549a690db098b7f32a28ea  (= Proposal V3)
origin/main                                        = 46fcac2b071929d2bd5b07aa28373941417f74a8  (merge de I-51)
I-50  origin/feature/cotas-independientes-por-vista  = 8ceb3a72c093b511d4f128510e2f4bfd07b78138
I-51  integrada en main (46fcac2); rama retirada
I-52  origin/feature/rackmirror-espejo-semantico     = 339b3abd238a3a7a42d60c9edd45a1500a776f62  (solo docs)
I-53  origin/feature/cabeceras-configurables-multidestino = c8476cccc98809fdb7fd0aeccef288521126f442  (solo docs)
I-54  origin/architecture/propiedades-personalizadas = 7c197af81b91df88366c873eddf9e11ddc5e87bb  (solo docs)
```

No hay más ramas remotas. Ninguna de las novedades crea una colisión real que obligue a cambiar el contrato de V4.

### 12.1 I-50

- **FACT** — Desde V3 avanzó dos commits con producción: `a4806ff` («I-50 G5 (C): la politica viaja por Push Back
  (C-13, C-14) y T-15 Dinamico/Push Back») y `8ceb3a7` («I-50 G6: la politica llega al Push Back compuesto A/B (C-15),
  T-08 y T-16»). Solo tocan archivos de Push Back y pruebas. Su cambio acumulado frente a `main` fuera de `docs/` son
  20 archivos de `src/` y 19 de pruebas.
...
- **Cruces con V4:**
  ...
  | Archivo | I-49 V4 | I-50 | Tratamiento |
  |---|---|---|---|
  | `RackSelectiveWindow.xaml.cs` | Un miembro, `Describe` (P23.13), en G10 | `BuildDesign` / `LoadDesign` en su G3, todavía no editado | Procedimiento de colisión antes de editar; integración serializada |
  | `SelectivePalletDesignDocument.cs` | **Ninguno**: V-0 y aislamiento en la frontera de I-49 (P23.16) | Modificado | Sin cruce. Las citas de V4 a `:316-330` y `:321` son de `a4d88f1`; en la rama de I-50 se desplazan |
  | `SelectiveGeometryResolver.cs` | **Ninguno** en V4 (CR-1) | Modificado | **Sin cruce**: la colisión que V3 abría por `FootInches` desaparece |
  | `LinkedPropertyReconciler.cs`, `SelectiveEffectiveDesignResolver.cs`, `SelectiveAuthoredAuthority.cs`, `SelectiveLibraryExport.cs` | Sí (P23.15–P23.16, P24) | No cambian (`:464-468`) | Sin cruce. I-50 depende del camino `WithDesign → From(design)`, que V4 conserva sobre el clon |
...
- **Condiciones de `OWNER_OVERRIDE_I49_I50_PARALLEL`**: G2D solo crea este documento, así que la condición de
...
- **FACT** — I-51 está **INTEGRADA y CERRADA** en `main` (merge `46fcac2`), con su rama remota retirada. Su producción
  ...
  pruebas) no la cita ni la modifica V4. `main` no avanzó desde V3.
- **FACT (HANDOFF de `main`)** — Registra que «**I-49** —motor de expresiones— declara que duplicar y re-estampar no
  cambian» (`docs/HANDOFF.md:1564-1570` en `46fcac2`). V4 lo mantiene (P24.8).
- **Divergencia declarada**: la rama de I-49 va 6 commits por delante y 9 por detrás de `main`. WORKFLOW §4.2 pide
  rebasar al abrir una sesión cuando el trunk avanzó; esta ejecución documental **no** rebasa por instrucción expresa
  del Coordinador, porque un documento nuevo no necesita el rebase para continuar. La reconciliación con `main` queda
  pendiente antes del primer gate productivo y, en todo caso, antes de escribir la prueba de P24.8, que tiene que
  ejercitar el restamp integrado.
...
|---|---|---|---|
| I-52 — RACKMIRROR, espejo semántico | G1 Discovery | Su Discovery prevé conservar en el espejo las entradas de kind desconocido, `expression` incluido, y una prueba cruzada con I-49; si generaliza el planificador o el restamp, avisa a I-49 antes de editar (`docs/initiatives/I-52-discovery.md:722,754-768,878` en su rama) | Coordinación: el espejo conserva `expression` sin interpretarla, igual que la duplicación (P24.8). Sin cambio de contrato |
| I-53 — cabeceras configurables multidestino | G1 Discovery | Declara que no toca expresiones, Project Variables ni `LinkedPropertyEditor` (`docs/initiatives/I-53-discovery.md:55,688-692` en su rama) | Sin cruce |
| I-54 — propiedades personalizadas (ID24) | G2A, Proposal V1 | Declara un punto de extensión hacia expresiones sin sintaxis ni dependencia de I-49, y cita la reserva de `Rack`/`Project` (`docs/initiatives/I-54-proposal-v1.md:456-465,701` en su rama) | Sin cruce: la gramática de §4 ya reserva `palabra.` como sintaxis de namespace. Un namespace futuro para ID24 exigiría su propio ADR |
...
- Numeración de ADR: 0035 es de I-50, ya aceptado en su rama; I-49 e I-54 no reservan número (§9).
...
es ADR nuevo y decisión del Owner», V4 deja constancia de que **la decisión del Owner existe** (encargo de G2A.1) y de
que **el ADR es obligatorio** antes de cualquier producción (§9). Además:

- Si la revisión final exige **cambiar** una invariante de I-47/I-48 más allá de lo que enumera §2.3: detenerse; eso
...
V3 → Architect Re-review → V4 Reconciliation (§0.1):
...
30 puntos      = P1..P30 (§3)
Pruebas        = 21 escenarios de G2C (P28.5) + 11 de G2D (P28.6)
Gates          = G0..G12 (§8): G0 y G1 HECHAS · G2 EN CURSO (V4) · G3 PENDIENTE · G4..G12 NO AUTORIZADAS

Base: Proposal V3   @ 4df9480638bbeb9a95549a690db098b7f32a28ea
      Proposal V2   @ 9aef7d04d9e65b4225af2cb36750ee636a6689d5 (historial)
      Proposal V1   @ b15e40a076d7157ce8ca73537af9659049bf8576 (historial)
      Discovery G1  @ 4cf02b167f183fb66d93988c9843296838277dc4

Siguiente paso: revisión final del Architect sobre V4. G2D no la solicita y no autoriza implementación.
```

**Historial conservado:** [V3](I-49-proposal-v3.md), [V2](I-49-proposal-v2.md) y [V1](I-49-proposal-v1.md) quedan
**intactas**.
