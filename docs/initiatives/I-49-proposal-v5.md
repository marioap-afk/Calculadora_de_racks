# I-49 — Proposal V5: motor de expresiones paramétricas (Expression Engine, ID22B)

> # ⚠ COORDINATOR PROPOSAL V5 — ARCHITECT FINAL REVIEW V4 RECONCILED — NOT CONSENSUS
>
> # ⛔ Implementation remains BLOCKED
>
> ```text
> Proposal Version = V5
> Coordinator      = AGREED WITH V5
> Architect        = PENDING FINAL RE-REVIEW
>
> CR-1..CR-5       = CLOSED
> FR-1             = CLOSED IN V5
>
...
> ```
>
> Gate **G2E**. V5 reconcilia la **Architect Final Review de V4** (`Architect = CHANGES REQUIRED`) sobre la base exacta
> de V4. La revisión final dio CR-1…CR-5 = SATISFIED y encontró un único hallazgo material, **FR-1 (MEDIUM)**: el
> `PlanReadSet` de V4 contradecía R2 y rompía `Delete` y `UnlinkAllAndDelete`. El Coordinador **acepta** FR-1 (§0.1).
>
> - V5 cambia **únicamente** el contrato del `PlanReadSet` y los textos y pruebas que derivan directamente de él, más
>   una aclaración no material del mensaje de recuperación (N-1, §7.2).
> - CR-1…CR-5 siguen SATISFIED y CLOSED. Ninguna otra decisión arquitectónica se reabre (§0.2, §0.8).
>
> ```text
> Base V4          = ccf21c69d9aa6e8d9b622e6389fb61f430d53f03   (I-49-proposal-v4.md; CI 34730120119 success)
...
**Resultado**: FR-1 queda cerrado en V5 y N-1 incorporada. No queda nada abierto.

### 0.2 Veredictos de la revisión final que V5 conserva
...
| CR-2 — contrato raíz con casos A–D | SATISFIED | CLOSED, sin cambio | §0.6, P8.10, T-V4-01 |
...
| Decisiones cerradas: A-03, OPEN A, OPEN B, V-0, operaciones, modelo híbrido, `=X`, unidades, funciones, `#GUID`, tokens, aislamiento de estado, `RegistryEvaluation`, costuras, un plan y un commit | Coherentes, salvo la interacción de FR-1 con R2 y con las eliminaciones | Sin cambio | §0.8 |
...
### 0.3 `PlanReadSet` — observaciones con fase y resultado esperado **[V5 · FR-1]**
...
**Resultado**: 5 de 5 cerrados en V4. Ninguno queda abierto. La revisión final de V4 los dio por satisfechos (§0.2).

### 0.5 Veredictos de la re-revisión de V3 que V4 conservó (heredado de V4)

| Tema | Veredicto de la re-revisión | Estado en V4 | Dónde |
|---|---|---|---|
| A-01…A-12, A-14…A-18 | SATISFIED | Sin cambio | §0.7 |
| A-13 | NOT SATISFIED (ampliación de V3) | **ACCEPTED sin ampliación** por CR-1 | §0.7, P9.7 |
| A-03 modificado | ACCEPTED | CLOSED; CR-2 completa su aplicación en propagación | §0.6, P8.10 |
...
### 0.6 A-03 — remedio modificado (ACCEPTED; CR-2 completa su aplicación)
...
- **Cómo lo aplica V5** (P8.10) **[V4 · CR-2]**. Para cada símbolo del cierre `I`, igual para literal y expresión:
...
### 0.7 V2 → Architect Review → Coordinator Reconciliation (heredada de V3)

La tabla de V3 se conserva como historial de la reconciliación de A-01…A-18. En V4 solo cambió A-13, que quedó
**ACCEPTED sin ampliación**. En V5 solo se precisa la regla vigente de A-05 (FR-1).

| # | Severidad (Architect) | Regla V2 | Decisión del Architect | Decisión del Coordinador | Regla vigente en V5 |
|---|---|---|---|---|---|
...
| **A-03** | HIGH | P21.7, P17.7 | Sin dominio en variables; dominio de la propiedad sobre el efectivo de toda fuente no literal | **ACEPTADA EN PRINCIPIO y MODIFICADA**; la re-revisión la **acepta** y CR-2 completa su aplicación en propagación (§0.6) | P5.5, P8.10, P21.7, P23.6, P24.5 |
...
| **A-05** | MEDIUM | P21.6 | La re-lectura de commit compara el conjunto leído del plan, no solo `I` | **ACEPTADA**: `PlanReadSet` contractual, declarado en V4 como revisión acotada de V8-R05 (CR-5) y formado en V5 por observaciones con fase y resultado esperado (FR-1) | P21.6 |
...
### 0.8 Decisiones cerradas (no se reabren en V5)

| Tema | Estado | Dónde |
|---|---|---|
| CR-1…CR-5 | CLOSED — SATISFIED en la revisión final de V4 | §0.2, §0.4 |
| A-03 modificado (tres fronteras) | CLOSED — ACCEPTED | §0.6, P8.10 |
...
| `PlanReadSet`, también sin `RegistryMutation` | CLOSED; FR-1 corrige en V5 cómo se forman y se comparan sus lecturas, sin cambiar la regla exacta de CR-5 | P21.6 |
...
### 0.9 Qué conserva V5 de V4

Todo lo que no enumeran §0.1 y §0.3:

- CR-1…CR-5 tal como la revisión final los dio por satisfechos;
- A-03 con sus tres fronteras y los casos A–D; OPEN A; OPEN B con R1–R3; V-0 con sus cuatro condiciones;
- los veredictos de `UnlinkAllAndDelete` y `RepairBrokenRack`;
- las dos superficies de expresión con el mismo control; el modelo híbrido con `=X` canónico; `#GUID` solo como
  diagnóstico; las tablas explícitas de tokens;
...
- los 21 escenarios de P28.5 y los 11 de P28.6, con T-V3-10 expresado con la fase de su observación;
- ADR REQUIRED sin número.

**Cambios editoriales**: las menciones a la Proposal vigente pasan de V4 a V5, las secciones heredadas de §0 se
renumeran (§0.4 a §0.9) y P28.7–P28.8 pasan a P28.8–P28.9. Las marcas `[V4 · CR-n]`, los escenarios `T-V4-xx` y las
tablas heredadas conservan su versión.
...
- **Es** la Proposal V5 del Coordinador, con `Coordinator = AGREED WITH V5`, sometida a **re-revisión final** del
  Arquitecto. Cada regla está **propuesta** hasta que haya consenso.
- **No es** consenso, **no** autoriza implementación, **no** es un ADR y **no** modifica V1, V2, V3, V4 ni el
...
### 2.3 Lo que V5 extiende o revisa
...
| ADR-0034 §11: `Delete` bloqueado con consumidores; reparación de bindings rotos | `Delete` y `UnlinkAllAndDelete` también bloqueados por fórmulas; `RepairBrokenRack` repara también fallos semánticos, con aviso de recuperación condicional | P20, §7 |
| ADR-0034 §13, C4-8 y C4-9 | Kinds `expression` fail-closed en builds anteriores; **V-0**; payload de Expression cerrado | §6 |
| Decisiones de I-48 §4 (V8-R01): re-lectura y re-acreditación en commit | La re-lectura también valida ids y compara las observaciones del `PlanReadSet` en su fase —`Before` sobre la re-lectura, `After` sobre el documento cambiado—, y ocurre también en planes sin `RegistryMutation` que escriben racks | P21.6 |
...
| 21 | propagation | R1–R3; contrato raíz A–D; `PlanReadSet` de observaciones `Before`/`After` como revisión acotada de V8-R05; reparación generalizada; un plan y un `Regen` | A-01 · A-03 · A-05 · A-12 · OPEN B · **CR-2** · **CR-4** · **CR-5** · **FR-1** |
...
| 27 | ID28/29 information preservation | `BoundExpression`, diagnósticos, traza opt-in, dependientes y conjuntos afectado y leído; el leído, con fase y resultado esperado | A-05 · **FR-1** |
| 28 | tests | 21 escenarios de G2C (tres ajustados), 11 de G2D y 9 de G2E; guarda de unidades acotada | V3 · **CR-1…CR-5** · **FR-1** |
| 29 | migration | Sin migración de datos ni de versión | — |
| 30 | non-goals | Actualizado con las decisiones cerradas, CR-1, CR-4, CR-5 y FR-1 | V3 · **CR-1** · **CR-4** · **CR-5** · **FR-1** |
...
- **P1.11 — Texto sin `=`.** Sigue siendo un literal parseado **exactamente como hoy** en cada superficie: regla
  localizada de ADR-0015 en RACKVARIABLES e invariante en el editor vinculable. V5 no resuelve esa divergencia
...
- **P5.3 — Ámbitos.** Existe **uno** activo, `Project`: un valor por dibujo. Queda reservado `Rack`: un valor por
  ...
  expresión de propiedad se evalúa en el contexto de su rack y en V5 solo ve símbolos `Project`.
...
- **P5.7 — Tres fronteras de validez** **[V3 · A-03 modificada]** (DR-5; §0.6):
...
- **P6.7 — Contexto de una propiedad de rack.** Para evaluar la expresión de una propiedad, el resolver usa el
  contexto del snapshot y los valores **ya evaluados** de `RegistryEvaluation` (P14.9). En V5 no añade símbolos
  propios del rack.
...
- **P7.7 — Enlace contra el snapshot mostrado** **[V3 · A-04]**. Corrige P22.7 de V2, que decía «contra el registro
  ...
  - **Regla**: una lectura posterior **solo** valida ids (presentes y sin identidad ambigua) y los resultados
    observados del `PlanReadSet` (P21.6). Nunca vuelve a resolver texto humano.
...
- **P8.8 — Expresión de propiedad o de definición.** Puede referenciar cualquier variable de `VariableType`
  numérico; en V5 todos lo son, porque solo existe `Length`. El resultado es un `double` que el consumidor
...
- **P8.10 — Contrato raíz del `VariableType`** **[V3 · A-03 modificada · V4 · CR-2]** (§0.6). El adaptador lo
  ...
  - **`Length` al escribir.** El contrato vigente del producto es `> 0`: RACKVARIABLES rechaza cualquier otro valor
    ...
    §10.3, §11.4). En V5 se aplica a **cada símbolo del cierre `I`** del estado «después», según su estado «antes»:
...
- **P9.6 — Clasificación de las constantes existentes** **[V4 · CR-1]** (hechos verificados en `a4d88f1`). Una
  ...
  | Constante | Ubicación | Papel | Clase | Tratamiento en V5 |
  |---|---|---|---|---|
...
- **P10.6 — Candidatos evaluados y DIFERIDOS: `ROUND`, `CEILING`, `FLOOR`.**
  ...
  | Pregunta que bloquea | Por qué no se decide en V5 |
  |---|---|
...
- **P12.2** — Nodos: los símbolos de la tabla (en V5, las variables de proyecto). Aristas: dueña → dependencia, con
...
- **P12.7 — Conjuntos derivados que expone cada operación** **[V3 · A-05]**:
  ...
  - **conjunto leído**: las observaciones del `PlanReadSet` del plan, cada una con su fase (P21.6).
...
- **P13.6** — Las expresiones de propiedad **no** forman ciclos: nada puede referenciar una propiedad en V5 (ID21
...
- **P15.2 — Severidad.** En V5 todo diagnóstico es `Error`: no hay avisos que permitan continuar.
- **P15.3 — Catálogo de V5.**
...
- **P15.8 — Cambios respecto de V2.**
  ...
  - Por A-03 modificada: `OutOfRange` **se conserva** para la raíz de una variable, pero solo al escribir. El
    ...
    §0.6, y la re-revisión aceptó ese remedio. Al leer ninguna variable recibe `OutOfRange`, lo que coincide con la
...
- **P18.2 — Qué hace hoy una build I-47/I-48** ante lo que V5 persiste (hechos del código en `a4d88f1`):
...
- **P18.7 — Qué NO justifica V-0.** El número de archivos tocados no es un argumento. Tampoco el precedente de I-50
  sobre el mismo portador, que V5 registra solo como hecho de coordinación (§12).
...
- **P21.6 — `PlanReadSet` y re-lectura de commit** **[V3 · A-05 · V4 · CR-5 · V5 · FR-1]**. V2 afirmaba que el
  efectivo de cada rack dependía solo de los valores de `I` y de su authored. Es falso: un rack planificado puede leer
  variables fuera de `I`. V4 hizo contractual el conjunto leído, pero como una lista plana de valores esperados de
  «después» que abortaba si cualquier símbolo fallaba. Eso contradecía R2 y rompía `Delete` y `UnlinkAllAndDelete`
  (FR-1). V5 lo sustituye por observaciones.
  - **Observación** (nombres ilustrativos):
    ...
    - `After`: información que el preflight usó sobre el estado hipotético «después», el que produce `ApplyTo`. Aquí
      viven, entre otros, los valores con los que se construyen los racks finales del plan.
    ...
    - Un mismo `SymbolId` puede tener una observación `Before` y otra `After` si el plan realmente lo usa en las dos.
    - El `PlanReadSet` **no** es una lista plana de valores de «después» y **no** mete de forma automática todo `I` en
      las dos fases.
  - **Regla de derivación.** Una observación entra en el `PlanReadSet` si y solo si el preflight usó el resultado de
    ese símbolo, en esa fase, para decidir el plan o para producir un valor que el plan escribe:

    | Operación | `RegistryMutation` | Observaciones `Before` | Observaciones `After` | Nunca |
    |---|---|---|---|---|
    ...
    | `RepairBrokenRack(rack)` | Ninguna | — | Cada símbolo que leen las fuentes que el rack conserva | — |
    ...
    - **Sin dependencias transitivas por separado.** El resultado de un símbolo observado ya incorpora el de sus
      ...
      escribe. V4 añadía las dependencias transitivas a la lista (ALT-41).
    ...
    - **Un id ausente no es un símbolo.** Su ausencia forma parte del fallo tipado del símbolo que lo referencia.
  - **Invariantes del plan**, verificados al construirlo:
    - ninguna observación `After` nombra un símbolo que la `RegistryMutation` elimina;
    - no hay dos observaciones con el mismo `SymbolId` y la misma fase;
    - cada `ExpectedResult` sale de la `RegistryEvaluation` de su fase en el preflight, nunca de una evaluación
      propia;
    - el orden es determinista: primero las `Before` y después las `After`, cada fase en el orden de P11.1.
  - **Qué no es una observación.** Las precondiciones propias de la mutación se validan con su contrato
    ...
    (V8-R01), existencia de los ids que el plan usa (P7.7, P22.7) y forma enlazada. El authored de los racks tampoco
    es una observación («Qué no cubre», abajo).
  ...
  - **Secuencia en commit**, dentro de Application (`RegistryCommit.Prepare`, Discovery §3.1), no en el Plugin:
    ...
    before     = RegistryEvaluation(accredited)             // solo si hay observaciones Before
    ...
    - El aborto es tipado: informa la primera observación que difiere, en el orden de los invariantes, con su fase,
      su `SymbolId` y los resultados esperado y observado. El texto lo produce la capa de P15.4.
  ...
  - **También sin `RegistryMutation`.** Hoy el ejecutor solo re-lee el registro cuando el plan lo escribe
    ...
    V4, todo plan con observaciones, como una reparación que deja fuentes sanas, re-lee, re-acredita y compara sus
    observaciones en la misma transacción **antes** de escribir la primera vista, aunque no escriba el registro. Un
    plan sin `RegistryMutation` y sin observaciones no re-lee.
  - **Una sola autoridad.** La lógica vive en Application y el ejecutor del Plugin solo la invoca; el Plugin sigue
    ...
    contra los resultados esperados que el plan transporta como datos. No hay una segunda autoridad de evaluación, y
    el commit **no** vuelve a ejecutar el preflight: no re-aplica R1, R2 ni el contrato raíz, solo comprueba que lo
    que el preflight usó sigue igual.
  ...
  - **Casos fijados por prueba**:
    ...
    - **sin `RegistryMutation`**: una reparación que deja `palletTolerance = Y + 1` aborta antes de la primera vista
      si Y cambió; una reparación que deja el rack sin símbolos no re-lee;
...
- **P21.12 — Edición desde RACKEDITAR.**
  ...
  - **Sin `PlanReadSet`** **[V4 · CR-5]**. El reconciliador no produce un `MutationPlan`: resuelve contra la lectura
    única del comando y escribe dentro del mismo comando modal (`RackSelectivoCommands.cs:66,144-151`). V5 no le
...
- **P22.9 — Panel de diagnóstico y reparación** **[V3 · OPEN B · A-10]**. Queda fuera del panel editable, como hoy
  ...
  - para cada fuente con causa superior, el mensaje de recuperación **condicional** de §7.2: solo promete recuperar
    la fórmula corrigiendo la variable cuando es demostrable en el estado diagnosticado **[V4 · CR-4]**;
...
- **P23.12 — Qué cambia respecto del contrato de I-48**, y que el ADR tiene que formalizar (§9):
  ...
  | Comportamiento de I-48 | V5 | Pines afectados (Discovery §14.2–§14.3) |
  |---|---|---|
...
- **P23.13 — `RackSelectiveWindow`.** Su `Describe` usa hoy `Text.TrimStart('=')` como nombre de la variable
  (Discovery §9.6). V5 lo sustituye por el texto de estado que produce la sesión: un cambio de un miembro.
...
- **P24.8 — Duplicación y restamp.** La copia conserva `PropertyValues`, entradas `expression` incluidas, con los
  ...
  el Plugin. V5 no toca ese camino y prueba en un archivo **propio**, contra el restamp integrado, que una entrada
...
- **P25.5** — Las expresiones de propiedad son el consumidor natural de un futuro ámbito `Rack`: el contexto de P6.7
  es donde ID20 añadiría símbolos del rack. V5 no añade ninguno.
...
- **P25.7 — Lo que ID20 tendrá que resolver, y V5 no**: `Rack.Frentes` es un conteo **por fondo**, `Rack.Niveles`
...
- **P26.4 — Lo que añadiría ID23, y V5 no**: sus namespaces y ámbitos, un resultado entero y la semántica de redondeo
  para `Quantity` (hoy `int`, Discovery §16.3; P10.6), la persistencia de fórmulas de BOM y su propio ADR.
- **P26.5** — Sin ensamblado separado en V5 (§10, ALT-17): la guarda marca la frontera.
...
- **P27.2 — ID29 (Impact Preview)**. Se expone **antes** de ejecutar:
  ...
  - **conjunto leído**: las observaciones del `PlanReadSet`, cada una con su fase y su resultado esperado, sea un
    valor o un fallo tipado (P21.6) **[V5 · FR-1]**.

  Hoy el preflight solo expone los racks (Discovery §16.4). V5 lo usa en los mensajes de RACKVARIABLES.
...
- **P28.6 — Escenarios obligatorios añadidos en V4** **[V4 · CR-1…CR-5]**. Cada uno es una prueba de comportamiento
  ...
  | T-V4-06 | El mensaje NO promete recuperación con otra falla propia | El mismo rack con además `verticalClearance = ABS(A, B)` → mensaje de bloqueo con la fórmula canónica `Holgura + 2`; `ChangeDefinition(Holgura)` queda bloqueado por R1 | CR-4 | G9, G10 |
  ...
  | T-V4-09 | `PlanReadSet` sin `RegistryMutation` | `RepairBrokenRack` que deja `palletTolerance = Y + 1`: si Y cambia, aborta en la misma transacción antes de la primera vista; una reparación que deja el rack sin símbolos no re-lee el registro | CR-5 | G9 |
...
- **P28.8** — V5 no exige refactorizar los fixtures duplicados que el Discovery contó (≈25 clases, Discovery §14.1).
  Las pruebas nuevas pueden introducir un único constructor de registros y fuentes con expresiones.
- **P28.9** — Checklist de validación del Owner: §8.3.
...
- **P30.26** — Abortar un commit porque un símbolo observado falle con el mismo resultado que el preflight admitió,
  ...
  comparar texto de diagnósticos (P21.6, FR-1).
...
8. **Fórmulas de propiedad rotas**: dos vías. Corregir la variable superior, que las recupera sin quitarlas **solo
   ...
   promete la primera vía si no es demostrable en el estado diagnosticado (§7.2) **[V4 · CR-4]**. Las estructurales
   no se reparan, para no destruir datos que esta build no entiende.
9. **Re-lectura de commit** (P21.6) **[V4 · CR-5 · V5 · FR-1]**:
...
10. **`RepairBrokenRack`**: una sola operación con el conjunto reparable ampliado, confirmación consciente de la
    fuente y mensaje de recuperación condicional (§7.2).
...
- **Mensaje de recuperación condicional** **[V4 · CR-4]**. El diagnóstico nunca promete siempre «corregir X
  ...
  2. **Condición de promesa.** Para una fuente con causa superior y causa raíz única X, el diagnóstico puede prometer
     la recuperación **solo** si, en el estado diagnosticado, todas las fuentes fallidas de todos los racks que
     consumen el cierre de X tienen causa superior con causa raíz exactamente X. Solo entonces ninguna otra fuente
     inválida conocida impide la recuperación, y un `ChangeDefinition` de X que supere la validación normal del estado
     resultante deja esos racks resueltos sin reparar **[V5 · N-1]**.
...
> §8). La división de G5–G10 puede ajustarse en la re-revisión final.
...
| **G2** | Proposal | V1 `b15e40a` → V2 `9aef7d0` (`Architect = CHANGES REQUIRED`) → V3 `4df9480` (re-revisión: `CHANGES REQUIRED`, CR-1…CR-5) → V4 `ccf21c6` (revisión final: `CHANGES REQUIRED`, FR-1) → **V5** (G2E) → re-revisión final del Architect, hasta `Architect = AGREED` | Architect Reviews | **EN CURSO** (G2E) |
...
| **G4** | ADR + alineación documental | ADR aceptado por el Owner, con número asignado **en ese momento**, que formaliza también la revisión acotada de V8-R05 (CR-5) con la semántica de observaciones de V5 (FR-1); contrato (§1, §3.2, §11.2) y ROADMAP alineados con la Proposal congelada; en `ideas-futuras.md`: L1–L5, la unificación del dominio de la ventana en el descriptor (P24.5) y el factor lb/ft→kg/m de `tools/` (P9.6) | ADR en estado `aceptado` | NO AUTORIZADA |
...
| **G12** | Owner Validation + integración | Checklist de §8.3 en AutoCAD 2025; integración **serializada** con I-50 y con las iniciativas activas; cierre documental (HANDOFF y ROADMAP); CI posterior al merge | WORKFLOW §4.5 | NO AUTORIZADA |
...
- Antes de editar **cualquier** archivo productivo, cada gate de G5 a G10 ejecuta el procedimiento de colisión de
  [`decisions/I-49.md`](../automation/decisions/I-49.md) §8 contra I-50, y comprueba el `main` vigente y las demás
  iniciativas activas (§12).
...
14. Mensaje de recuperación: un rack cuya única fuente inválida depende de una variable rota muestra «Corregir
    ...
    tiene además una fuente con fallo propio, el mensaje no promete recuperación y muestra la fórmula que eliminaría
    la reparación.
...
- **Por qué es obligatorio.** V5 cambia contratos persistidos (P17), extiende ADR-0034 (§2.3), revisa de forma
...
- **Número NO asignado.** Se asigna en G4 contra la numeración remota de ese momento. ADR-0035 es de I-50 y ya está
  **aceptado** en su rama; I-52 propone ADR-0036 en la suya (§12.3). I-49 no reserva ningún número de antemano.
- **No se escribe en G2E.**
- **Lo que tendrá que explicar de forma explícita**, tras G3:
  ...
  - el motor adimensional y las **tres fronteras de validez**: motor sin dominio, contrato raíz del `VariableType`
    ...
    endurecimiento declarado (§0.6, P8.10, P24.5);
  ...
  - los veredictos de `UnlinkAllAndDelete` y `RepairBrokenRack`, con el mensaje de recuperación condicional que no
    relaja R1 ni la decisión C4-13 de I-47 y cuyo «permitiría» no promete que R1 admita la corrección (§7);
...
| ALT-12 | **`ROUND`, `CEILING` y `FLOOR`** ya en V5 | **Diferida**: semántica abierta (P10.6); confirmado por el Architect |
...
| ALT-17 | Núcleo en un **ensamblado separado** | **Diferida**: frontera más dura, pero añade un proyecto a builds, CI y despliegue; V5 usa namespace + guarda (P26.5) |
...
| ALT-21 | **Sin contrato en la raíz** de las variables definidas por expresión, con `A = -2` válido (remedio del Architect para A-03) | **Rechazada por el Coordinador** y aceptada así en la re-revisión: la validez dependería de la sintaxis (§0.6) |
...
| Pruebas | Núcleo nuevo, ≈20 archivos existentes, la evolución de P28.4, los 21 escenarios de P28.5, los 11 de P28.6 y los 9 de P28.7 | Discovery §14 |
...
- `RackSelectiveWindow`, que I-50 ya modificó en su G3 e I-53 prevé tocar en su G5, necesita un cambio (P23.13,
  §12).
...
**Observado al abrir G2E** (`git fetch --all --prune`, 2026-09-12) y verificado de nuevo justo antes del commit:

```text
origin/architecture/motor-expresiones-parametricas = ccf21c69d9aa6e8d9b622e6389fb61f430d53f03  (= Proposal V4)
origin/main                                        = 46fcac2b071929d2bd5b07aa28373941417f74a8  (merge de I-51)
I-50  origin/feature/cotas-independientes-por-vista  = 6cd2970c36a906cb9e784797008bed5e8111e433
I-51  integrada en main (46fcac2); rama retirada
I-52  origin/feature/rackmirror-espejo-semantico     = 0fc7032bf15d03e7d478bbd9350f156708621c9d  (solo docs)
I-53  origin/feature/cabeceras-configurables-multidestino = f38362d32a737f62d69a229854dc5c1812adf063  (solo docs)
I-54  origin/architecture/propiedades-personalizadas = 36c337b84c47f9ac7c97d860fce42d3a5f4370e8  (solo docs)
```

No hay más ramas remotas. Ninguna novedad invalida una premisa de V5 ni obliga a cambiar su contrato. Lo que sigue es
coordinación para el preflight productivo y para la integración serializada.

### 12.1 I-50

- **FACT** — Desde V4 avanzó tres commits, los tres de su G3 y con producción de UI:
  - `ed50cbd` («I-50 G3 (A): el editor del Selectivo edita las cotas por tipo de vista (C-01)»):
    `RackSelectiveWindow.xaml` (+11/−1), `RackSelectiveWindow.xaml.cs` (+64) y pruebas de UI;
  - `2765a45` («I-50 G3 (B): el editor del Dinamico edita las cotas por tipo de vista (C-07)»):
    `RackDynamicSystemWindow.xaml/.cs` y pruebas de UI;
  - `6cd2970` («I-50 G3 (C): el editor de Push Back edita las cotas por tipo de vista (C-12)»):
    `RackPushBackSystemWindow.xaml/.cs` y pruebas de UI.

  Su cambio acumulado frente a `main` son 62 archivos: 26 de `src/`, 26 de pruebas y 10 de `docs/`.
- **FACT** — En `RackSelectiveWindow.xaml.cs`, su diff frente a `main` añade cinco zonas (hunks en `:14`, `:260`,
  `:1701`, `:2398` y `:2747` de su rama) y no contiene `Describe`, `TryEffective`, los editores vinculables,
  `CommitPendingEditors`, `TryStage`, `ApplyStaged`, `VariableId`, `PropertyValues` ni `Expression`.
- **FACT** — Sigue modificando `SelectivePalletDesignDocument.cs` (`DimensionViews`, C-06) y
  `SelectiveGeometryResolver.cs` (C-04). No toca `WithDesign`, el reconciliador, el resolver efectivo, el editor
  vinculable ni Project Variables.
...
- **Cruces con V5:**
  ...
  | Archivo | I-49 V5 | I-50 | Tratamiento |
  |---|---|---|---|
  | `RackSelectiveWindow.xaml.cs` | Un miembro, `Describe` (P23.13), en G10 | **Ya editado** en su G3 (A) (`ed50cbd`), sin tocar `Describe` ni los editores vinculables | Cruce de archivo, no de miembro: procedimiento de colisión antes de editar en G10, trabajo sobre lo que I-50 haya integrado e integración serializada |
  | `RackDynamicSystemWindow.xaml/.cs`, `RackPushBackSystemWindow.xaml/.cs` | **Ninguno** (P30.15) | Modificados en su G3 (B) y (C) | Sin cruce |
  | `SelectivePalletDesignDocument.cs` | **Ninguno**: V-0 y aislamiento en la frontera de I-49 (P23.16) | Modificado | Sin cruce. Las citas de V5 a `:316-330` y `:321` son de `a4d88f1`; en la rama de I-50 se desplazan |
  | `SelectiveGeometryResolver.cs` | **Ninguno** en V5 (CR-1) | Modificado | **Sin cruce**: la colisión que V3 abría por `FootInches` desaparece |
  | `LinkedPropertyReconciler.cs`, `SelectiveEffectiveDesignResolver.cs`, `SelectiveAuthoredAuthority.cs`, `SelectiveLibraryExport.cs` | Sí (P23.15–P23.16, P24) | No cambian (`:464-468`) | Sin cruce. I-50 depende del camino `WithDesign → From(design)`, que V5 conserva sobre el clon |
  | `SelectivePropertyValueDocument.cs`, editor vinculable, Project Variables y núcleo | Sí | No (`:58-59`) | Sin cruce |
  | `StructuralSectionUnits.cs` | Dos alias (P9.7), en G6 | No | Sin cruce |
...
- **Condiciones de `OWNER_OVERRIDE_I49_I50_PARALLEL`**: G2E solo crea este documento, así que la condición de
...
- **FACT** — I-51 está **INTEGRADA y CERRADA** en `main` (merge `46fcac2`), con su rama remota retirada. Su producción
  ...
  pruebas) no la cita ni la modifica V5. `main` no avanzó desde V3.
- **FACT (HANDOFF de `main`)** — Registra que «**I-49** —motor de expresiones— declara que duplicar y re-estampar no
  cambian» (`docs/HANDOFF.md:1564-1570` en `46fcac2`). V5 lo mantiene (P24.8).
- **Divergencia declarada**: la rama de I-49 va 8 commits por delante de `main`, contando este documento, y 9 por
  detrás. WORKFLOW §4.2 pide rebasar al abrir una sesión cuando el trunk avanzó; esta ejecución documental **no**
  rebasa por instrucción expresa del Coordinador, porque un documento nuevo no necesita el rebase para continuar. La
  reconciliación con `main` queda pendiente antes del primer gate productivo y, en todo caso, antes de escribir la
  prueba de P24.8, que tiene que ejercitar el restamp integrado.
- **Condición de parada**: si un gate de I-49 necesitara cambiar la API o la semántica de `RackDuplicationPlan`,
  ...
  editar** y lo reporta (contrato §12): ya es código de `main` validado por el Owner.

### 12.3 I-52, I-53 e I-54 (solo documentación)

| Iniciativa | Estado observado | Cruce con I-49 | Tratamiento |
|---|---|---|---|
| I-52 — RACKMIRROR, espejo semántico | G2: Proposal V1 y ADR-0036 en estado `propuesto`, en su rama (`0fc7032`) | Conserva las entradas `expression` como portador, sin reimplementar su evaluación; si I-49 integra antes, RACKMIRROR invoca la autoridad efectiva integrada (PV-6, T-M19). Su fachada de G4 toca `RackDuplicationPlan.cs` y avisa a I-49 antes de editar (`docs/initiatives/I-52-proposal-v1.md:718,834,911` en su rama) | Coordinación: el espejo conserva `expression` sin interpretarla, igual que la duplicación (P24.8). Si `RackDuplicationPlan` cambia en `main` antes del primer gate productivo de I-49, la prueba de P24.8 se escribe contra esa versión (§12.2). Su G7 añade RACKMIRROR y lleva el censo de comandos a 34 (`docs/initiatives/I-52-proposal-v1.md:889`): I-49 sigue sin añadir comandos (P22.1), y si I-52 integra antes, sus guardas de censo se escriben contra el censo vigente al rebasar. Sin cambio de contrato |
| I-53 — cabeceras configurables multidestino | G2: Proposal V1 (`f38362d`) | Mide cero archivos de I-49 fuera de `docs/` y deja bloqueados para su G5 `RackSelectiveWindow.xaml/.cs` mientras el G3 de I-50 esté abierto o sin integrar (`docs/initiatives/I-53-proposal-v1.md:91-93,622,636,945` en su rama). Su tabla de solapes no registra el `Describe` que I-49 prevé en G10 (P23.13) | Sin cruce de contrato. Coordinación a tres bandas en `RackSelectiveWindow.xaml.cs`: I-50 (G3), I-53 (G5) e I-49 (G10). Cada gate ejecuta su procedimiento de colisión antes de editar, y la integración es serializada |
...
- I-51 ya integró su fila de ROADMAP y su registro final en `docs/ideas-futuras.md`. I-49, I-50, I-52, I-53 e I-54
  insertan sus filas sobre esa versión de `main`.
- I-49 registrará sus seguimientos en `docs/ideas-futuras.md` en G4.
- Numeración de ADR: 0035 es de I-50, ya aceptado en su rama; I-52 propone 0036 en la suya; I-49, I-53 e I-54 no
  reservan número (§9). `docs/adr/README.md` recibirá sus filas en orden numérico según integren.
...
es ADR nuevo y decisión del Owner», V5 deja constancia de que **la decisión del Owner existe** (encargo de G2A.1) y de
que **el ADR es obligatorio** antes de cualquier producción (§9). Además:

- Si la re-revisión final exige **cambiar** una invariante de I-47/I-48 más allá de lo que enumera §2.3: detenerse; eso
...
- Colisión productiva material con I-50 u otra iniciativa activa, o necesidad de cambiar código integrado por I-51:
...
V4 → Architect Final Review → V5 Reconciliation (§0.1):
  ...
  N-1          «permitiría» no promete que R1 admita la corrección; R1 intacto  ACEPTADA · INCORPORADA

Revisión final de V4 conservada (§0.2):
  CR-1..CR-5 = SATISFIED · decisiones cerradas coherentes · coordinación paralela sin cambio de contrato

V3 → Architect Re-review → V4 Reconciliation (§0.4, heredada):
...
Veredictos cerrados (§0.8):
  ...
  Operaciones     §7   UnlinkAllAndDelete bloquea fórmulas y dependientes;
                       ...
                       mensaje de recuperación condicional
  ...
  PlanReadSet     P21.6  observaciones Before/After; comparación por resultado; revisión acotada de V8-R05

30 puntos      = P1..P30 (§3)
Pruebas        = 21 escenarios de G2C (P28.5) + 11 de G2D (P28.6) + 9 de G2E (P28.7)
Gates          = G0..G12 (§8): G0 y G1 HECHAS · G2 EN CURSO (V5) · G3 PENDIENTE · G4..G12 NO AUTORIZADAS

Base: Proposal V4   @ ccf21c69d9aa6e8d9b622e6389fb61f430d53f03
      Proposal V3   @ 4df9480638bbeb9a95549a690db098b7f32a28ea (historial)
      Proposal V2   @ 9aef7d04d9e65b4225af2cb36750ee636a6689d5 (historial)
      Proposal V1   @ b15e40a076d7157ce8ca73537af9659049bf8576 (historial)
      Discovery G1  @ 4cf02b167f183fb66d93988c9843296838277dc4

Siguiente paso: re-revisión final del Architect sobre V5. G2E no la solicita y no autoriza implementación.
```

**Historial conservado:** [V4](I-49-proposal-v4.md), [V3](I-49-proposal-v3.md), [V2](I-49-proposal-v2.md) y
[V1](I-49-proposal-v1.md) quedan **intactas**.
