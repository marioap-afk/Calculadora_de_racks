# I-54 — Proposal V5: Custom Properties Foundation (ID24)

```text
Documento      = PROPOSAL V5 — NOT CONSENSUS
Gate           = G2I — reconciliacion final de la Architect Review exact-SHA de V4
Executor       = PROPOSED V5
...
POST_REBASE_BASE_SHA      = f8deb675c6d1ef0e64693b157d69c4cc170d7b24   (origin/main usado en el rebase de G2G; sin cambios en G2I)
...
REVIEW_V3      = Architect Review — I-54 Proposal V3 @ PRE_REBASE_V3_SHA 5d25da89972df2468f1d03243301761f5463e6eb
                 AGREED WITH CHANGES · G2F CLOSED · AR-54-V3-01 y AR-54-V3-02 (MINOR)
REBASED_V3_EQUIVALENT_SHA = ff98b9ee95ea7c4f8da0b90fe949b8ebb42157ed   (V3 byte a byte identica a la revisada: §2.3)
PROPOSAL_V4    = 8bc991c0e1854bc9eb4013421e01f0515c2e77ca   (historica; no se edita; CI 34742026072 verde)
REVIEW_V4      = Architect Review — I-54 Proposal V4 @ 8bc991c: AGREED WITH CHANGES · G2H CLOSED
                 Solo AR-54-V4-01 (MINOR de redaccion) · Coordinator AGREED · el Coordinador acepta el cambio requerido
PROPOSAL_V5    = el commit que introduce este archivo (como obtenerlo: paquete §1)
Historicas     = V1..V4 no se editan; mapa de SHAs pre y post rebase en §2.2
Discovery      = I-54-discovery.md, sin cambios en G2G ni en G2I
Paquete        = I-54-architect-review-package.md, reescrito para G2J: solo AR-54-V4-01 / C-F3
```

> Esta V5 **no autoriza** nada y **sustituye a V4** como propuesta vigente. V1..V4 quedan en el arbol como historia
> y no se modifican. V5 es V4 con **exactamente una** correccion, **C-F3** (AR-54-V4-01), y sus trazas. Todo lo
> demas es identico a V4 y quedo acordado en G2H. Cada punto modificado cita **C-F3**. El consenso exige
...
## 0. Que cambia respecto de V4

El delta V3 → V4 (C-F1 y C-F2) sigue en V4 §0 y quedo verificado en G2H. Este es el delta V4 → V5, y **no hay
ningun otro**:

| Cambio | Tema | V4 | V5 | Cierra |
|---|---|---|---|---|
| **C-F3** | Como fallan los consumidores existentes ante F-14b | D-08.4, D-13 y RP-11 decian que los flujos que reescriben el sobre «fallan sin escribir» | La reserializacion del sobre afectado lanza y **ese** sobre no se escribe; el punto exacto de fallo depende de la **granularidad transaccional** de cada consumidor existente. `RACKEDITAR` confirma una transaccion por vista y puede quedar **parcialmente actualizado** entre vistas, como en BASE. La atomicidad de D-22.10 es **exclusiva** del ejecutor de Rack de I-54. F-14a y F-14b quedan separados | AR-54-V4-01 |
| **Trazas** | Revision y paquete (cambio 2 de G2H) | Matrices de V4 | §0, §1 (solo el titulo de la tabla), §2, §3 (P-37), §13, §14 y §17..§22 actualizados; paquete limitado a C-F3; contrato | — |

Estado de lo anterior tras G2H:

```text
C-F1        = VERIFIED / CLOSED G2H
C-F2        = VERIFIED EN LO ESENCIAL / AR-54-V4-01 MINOR DE REDACCION
C-F3        = INCORPORATED
AR-54-V3-01 = CLOSED
AR-54-V3-02 = CLOSED
AR-54-V2-03 = CLOSED
AR-54-V4-01 = CLOSED IN V5, pendiente de la revision final exact-SHA (G2J)
RD-04       = AGREED
RD-07       = ACCEPTED WITH FINAL RECONCILIATION, pendiente solo de confirmar la redaccion
```

**No cambian**: D-22.10, INV-07, las pruebas (T-CHR-04, T-ENV-13 y T-MUT-08 incluidas), C-F1, C-1..C-8, el orden de
autoridad ni el alcance del ADR (§15).

## 1. Vocabulario

| Termino | Significado en V5 |
|---|---|
...
## 2. Estado verificado en G2G y en G2I

§2.1..§2.3 registran el rebase de G2G y siguen vigentes; §2.4 registra el preflight de G2I.
...
### 2.4 Preflight de G2I

```text
Fecha                = 2026-09-13, git fetch --all --prune en el preflight y de nuevo antes de publicar
Rama I-54            = architecture/propiedades-personalizadas; HEAD = upstream = 8bc991c (V4); 0/0; arbol limpio
Stash / operaciones  = ninguno; sin MERGE_HEAD, REBASE_HEAD, CHERRY_PICK_HEAD, REVERT_HEAD, BISECT_LOG, sequencer ni rebase-*
origin/main          = f8deb675c6d1ef0e64693b157d69c4cc170d7b24   (sin avanzar desde G2G: la rama ya lo contiene)
Rebase               = no aplica (WORKFLOW §4.2 solo lo exige si el trunk avanzo)
```

V5 se crea sobre el historial publicado en G2G, sin reescribirlo, y se publica con un push normal.
...
| P-37 | **Granularidad de `RACKEDITAR`** (C-F3). El `RACKEDITAR` del Selectivo recorre las vistas en secuencia. Para cada una compone y serializa su payload (`WrapSelectivePayload`: `Compose` y `RackEmbedStore.Serialize`) justo antes de redibujarla con `RedrawInPlace`, sin captura por iteracion. Cada `SystemBlockWriter.RedrawInPlace` abre y **confirma su propia transaccion**. Por tanto, si el sobre de una vista posterior no se puede reserializar (F-14b), las vistas anteriores ya quedaron escritas. El Discovery registra tambien una transaccion por vista en Dinamico, Push Back, Cantilever y Cabecera | `RackSelectivoCommands.cs:166-223,382-394`; `SystemBlockWriter.cs:55-65` [E]; [L, D §7.2] |
...
4. **Contrato de aislamiento. Afirmacion permitida:**
   ...
   **Residuales declarados.** Ninguno lo puede recuperar el store de propiedades, porque ocurren en
   ...
   - **F-14a — UTF-16 crudo invalido en el texto de un sobre** (residual **preexistente**, no introducido por
     ...
     lanza `ArgumentException` **al leer** [X P-31]. El barrido de I-54 (`ScanEnvelopes`) lanza con el **antes de
     cualquier mutacion**, asi que no llega al ejecutor de Rack de I-54 ni escribe nada, aunque sin el diagnostico
     por definicion de D-09.3. Los consumidores existentes que leen ese sobre fallan en su propia lectura (C-F3).
   - **F-14b — escape JSON de un surrogate suelto en un sobre** (residual **preexistente**, no introducido por
     I-54; C-F2 y C-F3):
     1. el sobre **se deserializa** bien, en BASE y en la variante, tanto si el escape esta dentro de
        `CustomProperties` como en cualquier otro campo;
     2. al **reserializar ese sobre** —`Serialize` del mismo objeto, `Compose(source)` seguido de `Serialize` o el
        restamp, que serializa el mismo objeto deserializado— se lanza `JsonException` [X P-36];
     3. el sobre cuya serializacion falla **no se escribe**;
     4. I-54 **no cambia** como fallan los consumidores existentes, ni intenta corregirlo;
     5. el punto exacto de fallo depende de la **granularidad transaccional** de cada consumidor existente: un flujo
        con una sola transaccion global aborta segun esa transaccion; un flujo que escribe por vista o por etapa
        puede conservar cambios ya confirmados antes del sobre afectado;
     6. **caso verificado**: `RACKEDITAR` procesa las vistas en secuencia y `SystemBlockWriter.RedrawInPlace`
        confirma una transaccion por vista. Una vista posterior con F-14b puede dejar las vistas anteriores ya
        redibujadas y escritas: el rack queda **parcialmente actualizado** entre vistas [E P-37; RP-03];
     7. eso **ya ocurre en BASE**;
     8. el **ejecutor de Rack de I-54** si evita escrituras parciales: serializa todos los payloads antes de la
        primera escritura y, si alguno lanza, no hay commit ni se escribe ningun miembro (D-22.10). Esa garantia es
        **propia** del ejecutor de I-54 y **no** se extrapola a `RACKEDITAR`, `RACKDUPLICAR`, `RACKLAYOUT`, Project
        Variables ni a otros consumidores existentes.

     Si el escape esta dentro de `CustomProperties`, el store de propiedades clasifica el miembro
   ...
   - Para F-14a y F-14b:
     ...
     - La entrada de **F-14b** usara la redaccion de esta viñeta (C-F3): falla la reserializacion del sobre
       afectado; la granularidad y las posibles escrituras previas dependen del consumidor; `RACKEDITAR` puede
       quedar parcialmente actualizado entre vistas; I-54 no introduce ni corrige el residual; y su ejecutor propio
       garantiza la atomicidad por D-22.10.
...
**Cambio desde V4 (C-F3).** F-14a y F-14b quedan separados:
- F-14a lanza en el barrido, antes de mutar.
- F-14b ya no dice que los consumidores existentes «fallan sin escribir». Describe que falla la reserializacion
  del sobre afectado, que el punto de fallo depende de la granularidad de cada consumidor y que `RACKEDITAR` puede
  quedar parcialmente actualizado.
- La atomicidad de D-22.10 es exclusiva del ejecutor de I-54.
...
«Ninguno» significa dos cosas distintas (C-F3):

- **A. Garantia de aislamiento de I-54.** Para los sobres que no caen en F-14a ni en F-14b, el dibujo,
  `RACKEDITAR`, el BOM, `RACKLISTA`, `RACKDUPLICAR`, `RACKLAYOUT` y `RACKVARIABLES` se comportan igual que con un
  contenedor ausente, **dentro del alcance de aislamiento de D-08.4** (INV-07): I-54 no introduce un modo de fallo
  nuevo en sobres que BASE puede leer **y reserializar**.
- **B. Residuales preexistentes del sobre (D-08.4), que «ninguno» no cubre**, tampoco si el miembro es
  `PresentButUnreadable` por un surrogate escapado:
  - **F-14a** puede fallar **al leer**, en el barrido y antes de mutar.
  - **F-14b** puede fallar **al reescribir**. El punto exacto de fallo depende de la granularidad transaccional del
    consumidor existente, y no se afirma que ese consumidor falle sin escribir. `RACKEDITAR` puede quedar
    parcialmente actualizado entre vistas, porque confirma una transaccion por vista (P-37). Solo el ejecutor de Rack
    de I-54 garantiza que no se escribe ningun miembro (D-22.10).

No hay recuperacion destructiva en V1 (N-11).
...
**Cambio desde V4 (C-F3).** La nota separa la garantia de I-54 (A) de los residuales (B), y deja de afirmar que los
consumidores existentes fallan sin escribir ante F-14b.
...
| Riesgo del Discovery | Mitigacion en V5 |
|---|---|
...
| RP-11 | UTF-16 invalido en un sobre (C-F3). **F-14a**: el texto crudo invalido hace lanzar el barrido al leer, antes de cualquier mutacion, y no llega al ejecutor de I-54. **F-14b**: con un surrogate escapado el sobre se lee, pero su reserializacion lanza y ese sobre no se escribe. Los consumidores existentes conservan su granularidad preexistente: `RACKEDITAR` puede dejar ya redibujadas las vistas anteriores (P-37). El ejecutor de Rack de I-54 evita escrituras parciales (D-22.10). Ninguno de los dos casos tiene diagnostico tipado | Residuales **preexistentes** del `RackEmbedStore` de BASE, documentados en D-08.4 y caracterizados en T-CHR-04 y T-ENV-13; no se arreglan en I-54; atomicidad del ejecutor propio en D-22.10 y T-MUT-08; F-14a y F-14b se registran en G2-FREEZE (C-F2, C-F3) |
...
## 13. Ramas paralelas (medidas en G2I)

Solo se mide el impacto sobre C-F3 (AR-54-V4-01, residual F-14b) y sobre la costura que toca: la reserializacion
del sobre. La medicion de G2G sigue en V4 §13.

| Rama | SHA al publicar V5 (completo) | En G2H | Cambio desde G2H | Impacto en C-F3 / F-14b |
|---|---|---|---|---|
| `main` | `f8deb675c6d1ef0e64693b157d69c4cc170d7b24` | `f8deb67` | Ninguno; sin rebase (§2.4) | Ninguno: `RackEmbedDocument.cs`, `RackEmbedComposer.cs`, `RackEnvelopeRestamp.cs`, `RackSelectivoCommands.cs` y `SystemBlockWriter.cs` son identicos a los de `BASE_SHA` |
| I-49 `architecture/motor-expresiones-parametricas` | `364d6c06e44273a63a7b6f6509daf357611ea77a` | `2beec61` | +2 commits **solo docs**: aceptacion de ADR-0038 por el Owner y consensus freeze de su Proposal V6; base `f8deb67`; sin `src/`, `tests/` ni `assets/` propios | Ninguno: no toca el sobre ni su reserializacion |
| I-52 `feature/rackmirror-espejo-semantico` | `545c2229de8d7850e03981d85aced0ac63c24040` | `545c222` | Ninguno | Ninguno sobre la correccion. Su espejo reserializa el sobre (`Compose` y restamp), asi que puede compartir el residual F-14b; eso no cambia C-F3 |
| I-53 `feature/cabeceras-configurables-multidestino` | `4e00a273e22634cb3abbc1b3fb3778edf49dbc7e` | `4e00a27` | Ninguno | Ninguno: su G3 no toca el sobre ni su reserializacion |

Medido con `git fetch --all --prune` en el preflight de G2I y de nuevo inmediatamente antes de publicar V5.
...
| G2G | Rebase obligatorio sobre `origin/main` y Proposal V4 (C-F1, C-F2) | **HECHO**: `8bc991c`, `--force-with-lease`, CI 34742026072 verde |
| G2H | Revision exact-SHA post-rebase de V4, limitada a C-F1, C-F2 y la integridad del rebase | **CERRADO**: Arquitecto AGREED WITH CHANGES @ `8bc991c` (solo AR-54-V4-01, MINOR); Coordinator AGREED; el Coordinador acepta el cambio requerido |
| G2I | Esta Proposal V5 (C-F3) | commit documental, push normal y CI del SHA exacto |
| G2J | Revision final exact-SHA de V5 por Coordinador y Arquitecto, **limitada a AR-54-V4-01 / C-F3** | veredictos sobre el mismo SHA |
| G2-FREEZE | Consenso sobre el mismo SHA; contrato actualizado; **ADR `propuesto`** con el alcance de §15; F-01..F-13, **F-14a y F-14b** (residuales del `RackEmbedStore` al leer y al reescribir, con la redaccion de D-08.4 corregida por C-F3) y la mejora de D-11.6 en `ideas-futuras.md`; aprobacion del Owner, informado de R-12 | commit documental y CI |
...
### 17.1 Cambio de G2H: C-F3 y trazas

| Cambio de G2H | Cambio V5 | Hallazgo | Secciones V5 | Disposicion | Notas |
|---|---|---|---|---|---|
| **1** Descripcion exacta de F-14b en los consumidores existentes | **C-F3** | AR-54-V4-01 | P-37, D-08.4 (F-14a y F-14b), D-13 (nota A/B), RP-11 | **INCORPORATED** | Falla la reserializacion del sobre afectado y ese sobre no se escribe; el punto de fallo depende de la granularidad del consumidor; `RACKEDITAR` puede quedar parcialmente actualizado entre vistas (P-37); I-54 no introduce ni corrige el residual; la atomicidad de D-22.10 es exclusiva del ejecutor de I-54; F-14a separado (barrido antes de mutar); la entrada futura de F-14b usa esta redaccion |
| **2** Trazas y revision limitada | Trazas | — | §0, §1 (titulo), §2.4, §13, §14, §17..§22, paquete y contrato | **INCORPORATED** | Paquete limitado a C-F3 para G2J; sin rebase porque `main` no avanzo |

**Sin cambios en V5**: D-22.10, INV-07, las pruebas (T-CHR-04 y T-ENV-13 caracterizan la falla preexistente;
T-MUT-08 demuestra la atomicidad del ejecutor de I-54), C-F1, C-1..C-8 y §15. **Contradicciones de C-F3 con lo
acordado: ninguna.** C-F3 no cambia ninguna decision: corrige la descripcion de un comportamiento preexistente y
queda coherente con P-07 y RP-03, que ya reconocian las transacciones por vista y los fallos a medias de
`RACKEDITAR`.

### 17.2 Cambios de G2F: C-F1, C-F2 y trazas
...
**Verificacion de G2H sobre esta matriz**: C-F1 **VERIFIED**; C-F2 **VERIFIED en lo esencial**, con AR-54-V4-01
(MINOR de redaccion) resuelto en V5 por C-F3; rebase **VERIFIED**; trazabilidad **VERIFIED**.
...
### 17.3 Cambios de G2D: C-1..C-8 (verificados en G2F)
...
### 17.4 Cambios de G2B: V2-1..V2-22 (heredados de V2, con su verificacion en G2D)
...
### 18.1 Hallazgos de la revision de G2H sobre V4

| Hallazgo | Severidad | Afecta | Disposicion V5 | Cambio | Estado |
|---|---|---|---|---|---|
| **AR-54-V4-01** Consumidores existentes que «fallan sin escribir» ante F-14b | MINOR (redaccion) | D-08.4 (F-14b), D-13, RP-11 | Falla la reserializacion del sobre afectado; punto de fallo segun la granularidad del consumidor; `RACKEDITAR` puede quedar parcialmente actualizado; atomicidad solo del ejecutor de I-54 (D-22.10) | C-F3 | **CLOSED IN V5** (pendiente de la revision final exact-SHA, G2J) |

### 18.2 Hallazgos de la revision de G2F sobre V3

| Hallazgo | Severidad | Afecta | Disposicion | Cambio | Estado |
|---|---|---|---|---|---|
| **AR-54-V3-01** `Name` no normalizable con UTF-16 bien formado | MINOR | §1, P-32, D-05.1, D-05.2, D-07.4, INV-23, T-STO-08, -16, -18, -20 | Un `Name` con no-caracteres es invalido al escribir y `PresentButUnreadable` al leer, antes de NFC; `Value` sin cambios | C-F1 | **CLOSED** (verificado en G2H) |
| **AR-54-V3-02** Residual de reescritura con surrogate escapado | MINOR | P-31, D-08.4, D-13, D-22, INV-07, RP-11, T-CHR-04, T-ENV-13, T-MUT-08 | F-14b declarado junto a F-14a; garantias acotadas; sin commit si una serializacion lanza | C-F2 | **CLOSED** (verificado en G2H) |

### 18.3 Hallazgos de la revision de G2D sobre V2
...
| **AR-54-V2-03** UTF-16 mal formado y excepciones | MINOR | D-05.1, D-05.4, D-07.4, D-08.4 A y G, T-STO-08, -16, -18, -20 | Clases capturadas explicitas; validacion antes de NFC y de serializar; residual preexistente declarado. En V4, no-caracteres en `Name` (3B) y residual F-14b (3C) | C-3; C-F1 y C-F2 | **CLOSED** (reabierto en G2F; verificado cerrado en G2H: 3B y 3C) |
...
### 18.4 Hallazgos de la revision de G2B sobre V1
...
Los hallazgos MINOR AR-54-06..18 no se reabrieron en G2D, G2F ni G2H y siguen cerrados sin cambios. «CLOSED IN
V5» es la disposicion del ejecutor: la **re-verificacion** corresponde a la revision final exact-SHA de G2J
(paquete §6).

## 19. RD-01..RD-20 en V5

G2H reviso solo RD-04 y RD-07. RD-04 queda acordada; RD-07 solo espera confirmar la redaccion de C-F3. Las demas
conservan su estado de V4.

| RD | Estado V3 | G2F (Arquitecto, sobre V3) | V5 | Cambio desde V3 | Seccion |
|---|---|---|---|---|---|
...
| RD-04 | ACCEPTED WITH RECONCILIATION | AGREE WITH CHANGE | **AGREED** (G2H: AGREE) | V4: un `Name` con no-caracteres es invalido; NFC solo recibe nombres acreditados; `Value` sin cambios (C-F1). Sin cambios en V5 | D-05.1, D-05.2, D-05.4 |
| RD-05 | ACCEPTED | — | **ACCEPTED** | Sin cambios | D-03 |
| RD-06 | ACCEPTED | — | **ACCEPTED** | Sin cambios | D-07 |
| RD-07 | ACCEPTED WITH RECONCILIATION | AGREE WITH CHANGE | **ACCEPTED WITH FINAL RECONCILIATION**, pendiente solo de confirmar la redaccion (G2H: AGREE WITH CHANGE) | V4: store sin excepciones de NFC (C-F1); residuales F-14a y F-14b, garantias acotadas y fallo sin commit (C-F2). V5: descripcion exacta de F-14b en los consumidores existentes (C-F3). Acota tambien el texto de D-13 e INV-07, que cita RD-17, sin cambiar su estado | D-07.4, D-08.4, D-13, D-22.10, INV-07, INV-23, RP-11 |
...
## 20. Cambios que V5 no hace

- No reescribe V1..V4, ni rebasa, ni sustituye SHAs historicos (§2.2, §2.4).
- No cambia D-22.10, INV-07, las pruebas, C-F1, C-1..C-8, el orden de autoridad ni el alcance candidato del ADR:
  §15 es identica a V3 y V4.
- No convierte C-F3 en una decision nueva ni promete atomicidad de los consumidores existentes.
...
- No toca produccion, pruebas productivas, `docs/HANDOFF.md`, `docs/ROADMAP.md` ni `docs/ideas-futuras.md`: F-14a y
  F-14b se registran en G2-FREEZE con la redaccion de D-08.4.
- No modifica `RackEmbedStore` ni arregla F-14a o F-14b.
- No modifica el Discovery.
- No reabre ninguna RD distinta de RD-07, y en ella solo la redaccion de C-F3.

## 21. Precisiones del ejecutor: estado final

**OPEN DISAGREEMENT = NINGUNO.** Estado de cada precision tras G2F; sin cambios en G2H ni en V5:

| # | Precision | Estado V5 | Nota |
|---|---|---|---|
...
Siguiente gate: **G2J**, revision final exact-SHA de esta V5 por Coordinador y Arquitecto, limitada a
AR-54-V4-01 / C-F3. Si converge, no habra V6 y el siguiente paso sera G2-FREEZE. Nada de esta V5 autoriza G3, el
ADR definitivo, codigo, pruebas, validacion del Owner, tags ni merge.
