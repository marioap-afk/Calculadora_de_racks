# I-52 — Proposal V11: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V11 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V11 se redacta SIN rebase, sobre 636f7fd)
...
> Proposal V1..V9   = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd
>                     (historial)
> Proposal V10      = 636f7fd0787c001f61a83c863104402329ebe877   (historial; CI 34806167254 4/4)
> Revision Arq. V10 = «Architect: CHANGES REQUIRED — PROPOSAL V11»; sin BLOCKER ni HIGH; MEDIUM AR10-01 (tipo de linea
>                     complejo alcanzado por fuentes simbolicas y resuelto en el dibujo destino), AR10-02 (huellas de
>                     objetos posteriores sin inventario cerrado, sin independencia de la visibilidad y sin politica de
>                     entorno) y AR10-03 (reconciliacion con I-55 antes del freeze); LOW AR10-04..AR10-09; cambios
>                     exactos V11-01..V11-10 de la revision (registro: docs/automation/decisions/I-52.md §62)
> Coordinador       = orden de V11 con V11-01..V11-15 vinculantes (registro §63)
> main y paralelas  = §15.1 (preflight de V11 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V11 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V11 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V11 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V10 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V10, sus revisiones de Arquitecto y esta V11 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V11 la registra
> como **indicativa**, ligada a su SHA-256, y no como contrato. La revision de V10 no inspecciono la biblioteca: midio
> codigo a SHA exacto (`LateralHeaderDrawer`, `LayerHelper`, `CantileverViewMaterializer`, la ventana de ayuda de
> comandos y las guardas de I-54) y leyo la Proposal V1 de I-55 y los ADR-0040, ADR-0041 y ADR-0042.

---

## 0. Reconciliacion V10 → V11

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`) y el Discovery (`339b3ab`) **se conservan sin cambios**. **Donde V10 y V11
difieran, manda V11.** Las marcas `[V10-D01]`..`[V10-D23]` siguen vigentes salvo donde una marca `[V11-Dnn]` las
modifica, reabre o sustituye (§0.4); las marcas `[V9-Dnn]`, `[V8-Dnn]`, `[V7-Dnn]`, `[V6-Dnn]` y `[V5-Dnn]` siguen como
las dejaron V10, V9, V8, V7 y V6 (§0.5, §0.6, §0.7, §0.8, §0.9).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V11** | V11 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V11. **Debe ser NONE** |

### 0.2 Matriz `[V11-D01]`..`[V11-D19]`

En la columna «Origen»: `AR10-nn` es el hallazgo de la revision de V10; `R:V11-nn`, su cambio exacto; `C:V11-nn`, la
precision vinculante del Coordinador en la orden de este gate. Las dos series V11-nn se numeran por separado. **Donde el
cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del
Coordinador**: la revision agrupaba en un solo cambio el inventario de huellas de objetos posteriores, la visibilidad y
el estado global, y dejaba declarar como limitacion la ocupacion que depende del entorno; el Coordinador los separa en
tres decisiones (`ModelSpaceFootprintPolicy`, huella de orden independiente de la visibilidad y ocupacion dependiente
del entorno) y decide que lo que CT-49 no demuestre conservador bajo todos los estados admitidos es `UNKNOWN`, con la
consecuencia visible en O-1 (L-34, L-37). La revision proponia tambien una guarda durable de productores; el Coordinador
exige que sea estructural y no solo de hashes de archivos (G-M24).

| Marca | Origen | V10 | Correccion V11 | Estado | Donde |
|---|---|---|---|---|---|
| `[V11-D01]` | AR10-01 · R:V11-01 · C:V11-01 | Fuentes visuales simbolicas comparadas con el contexto de la autoridad origen; «tipo complejo ⇒ `UNKNOWN`» tipado solo para `Explicit`; §11.3 punto 3 afirmaba que las capas del dibujo con otras propiedades no invalidan la evidencia de simetria | **RESOLVED PRESENTATION ELIGIBILITY**: dos preguntas **separadas**, que no se sustituyen: (A) igualdad de las fuentes visuales simbolicas tipadas entre homologas; (B) si la representacion **resuelta** esta soportada. Para cada primitiva, `VisualSource` → resolucion en el `DestinationEvaluationContext` → `ResolvedPresentationEligibility`, recorriendo `Explicit`, `ByLayer(Named)`, `InheritLayer`, `InheritBlock`, las cadenas de capa `0`, la presentacion de creacion de la referencia de pieza (CT-50) y la presentacion exterior de ST-13, con DRAWING WINS. Para `Linetype` se resuelve el `LinetypeTableRecord` **efectivo** del dibujo destino: tipo complejo con formas o texto, registro no determinable o contenido no caracterizado ⇒ `UNKNOWN`, tambien con `LinetypeSource = ByLayer` o heredada y tambien si un nombre `Explicit` existe en el dibujo con contenido complejo. La frase de V10 queda limitada: las otras propiedades de capas homonimas no invalidan la **igualdad de fuentes simbolicas**, pero pueden invalidar la **elegibilidad de la variante soportada**. Los registros resueltos usados entran en `FootprintInputFingerprint`, en el `MaterializationContextReadSet` y en la evidencia y su fingerprint cuando la elegibilidad depende de ellos | CLOSED BY V11 (contrato); resolucion real: DEFERRED TO G3/G6 WITH FAIL-CLOSED | §11.3 puntos 3, 4, 6.1, 6.2 y 10; X-17b; X-22; CT-36; CT-49; CT-50; T-M63; T-M71; ADR-0036 decision 13 |
| `[V11-D02]` | AR10-02 · R:V11-02 · C:V11-02 | Huella de un objeto existente = ocupacion de modelo «con sus registros resueltos»; `Extents3d` ampliado solo por mascaras, marcos y anchos geometricos, sin inventario por clase | **MODELSPACE FOOTPRINT POLICY**: autoridad conceptual distinta para objetos arbitrarios del dibujo, `ModelSpaceFootprintPolicy`, por **tipo exacto en tiempo de ejecucion**; cada clase es `SUPPORTED(metodo)` o `UNKNOWN`, **nunca** un fallback optimista. CT-49 inventaria como minimo `Line`, `Arc`, `Circle`, `Ellipse`, `Spline`, `LWPOLYLINE`, `Solid`, `Trace`, `Hatch`, `BlockReference` con atributos, `DBText`, `MText` con fondo y columnas, `Dimension` con flechas, relleno y demas componentes, `MLeader`, `Table`, `Point`, `Wipeout`, raster y underlay, `XLINE`/`RAY`, nube de puntos y proxy o entidad custom; no es obligatorio soportarlas todas. Clase, componente o estado no caracterizado ⇒ huella `UNKNOWN` ⇒ E10 si el objeto es posterior relevante para ST-19. `Extents3d` solo en las clases donde CT-49 demuestre que es una cota conservadora despues de añadir todas las ocupaciones de modelo necesarias | CLOSED BY V11 (contrato); inventario real: DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-19; §11.3 punto 5; CT-49; T-M73; G-M23; M-35 |
| `[V11-D03]` | AR10-02 · R:V11-02 · C:V11-03 | Sin regla sobre capas apagadas o inutilizadas, `Entity.Visible` ni representaciones anotativas visibles | **VISIBILITY-INDEPENDENT ORDER FOOTPRINT**: para ST-19 la huella representa la ocupacion **latente** que puede volverse visible sin cambiar el orden de dibujo. No se excluye geometria porque hoy su capa este apagada, inutilizada o inutilizada en una ventana, porque `Entity.Visible = false` ni por el valor actual de `ANNOALLVISIBLE`: la relacion de orden persiste si esa visibilidad cambia. Huella de orden = independiente del estado transitorio de visibilidad; clase o representacion que no puede producir una ocupacion conservadora independiente de esos estados ⇒ `UNKNOWN` ⇒ E10. `VisibilityPath` de las piezas sigue existiendo para la equivalencia visual; esta regla es especifica de ST-19 sobre objetos del dibujo (las huellas de las copias ya no excluyen geometria por visibilidad) | CLOSED BY V11 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-19; §11.3 punto 5; CT-49; T-M73; M-35 |
| `[V11-D04]` | AR10-02 · R:V11-02 · C:V11-04 | Ocupacion dependiente de `PDMODE`, `PDSIZE`, `LTSCALE`, `MSLTSCALE`, representaciones anotativas o metricas de fuente, sin clasificar | **ENVIRONMENT-DEPENDENT MODEL OCCUPANCY**: si CT-49 demuestra una ocupacion de modelo conservadora bajo **todos** los estados relevantes que admite el contrato ⇒ `SUPPORTED`, con esos estados declarados como observaciones consumidas en el `FootprintInputFingerprint` del objeto; si no ⇒ `UNKNOWN`. No se inventa una formula universal. O-1 (L-34, L-37) declara que los objetos posteriores cuya representacion depende del entorno y no tienen huella conservadora pueden hacer fallar `RACKMIRROR` | CLOSED BY V11 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §1.3 L-34 y L-37; §11.3 punto 5; CT-49; CT-50; T-M73 |
| `[V11-D05]` | AR10-05 · R:V11-05 · C:V11-05 | §9.2 re-verificaba solo los objetos posteriores ya capturados (identidad, existencia, huella visual y posicion) | **COMPLETE POSTERIOR OBJECT SNAPSHOT**: R6 vuelve a enumerar **todos** los objetos posteriores a cada fuente y captura `ModelSpaceOrderTailSnapshot { identidades ordenadas, clases, contexto efectivo de orden (SortentsTable y variables de CT-49) }`; al inicio de MUTATE se vuelve a enumerar. Objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente, o con identidad o clase cambiada ⇒ fail-closed (E10 en PREPARE; excepcion y rollback en MUTATE). No basta revisar solo los objetos capturados antes; un objeto anterior que sigue siendo anterior no invalida | CLOSED BY V11 | §4.1 ST-19; §9.1; §9.2; PRE-18; T-M69; M-35 |
| `[V11-D06]` | AR10-04 · R:V11-04 · C:V11-06 | `DIMSTYLE` y `TEXTSTYLE` como observaciones sin cierre transitivo; «escala de anotacion» sin distinguir la del diseño de la del dibujo | **TRANSITIVE TEXT / DIMENSION CONTEXT**: el `MaterializationContextReadSet` contiene solo lo consumido, con **cierre transitivo**. Cota: registro efectivo del estilo de cota (el estilo con nombre del plan si existe en el dibujo, con el resultado de esa busqueda, o el `DIMSTYLE` actual), el estilo de texto que use, los bloques de flecha que use, los tipos de linea, el relleno y las demas propiedades que afecten a la geometria o a la huella, y lo que CT-50 demuestre material. `DBText`: registro efectivo del estilo de texto y las metricas de fuente o tipografia que la huella necesite. Se distingue la `AnnotationScale` authored del rack (intencion del plan) de `CANNOSCALE` y del contexto de anotacion del dibujo; `CANNOSCALE` no entra salvo que CT-50 demuestre su consumo. Consumo no determinable ⇒ `UNKNOWN` | CLOSED BY V11 (contrato); consumo real: DEFERRED TO G3 WITH FAIL-CLOSED | §8.7; §9.2; PRE-17; CT-50; T-M72; M-37 |
| `[V11-D07]` | AR10-06 · R:V11-06 · C:V11-07 | «Ningun margen se calcula en el marco del BTR para escalarlo despues» sugeria añadir anchuras locales sin transformar a una linea media ya transformada | **MODEL-WIDTH TRANSFORM**: la geometria local ocupada completa —incluidos los anchos reales de modelo— se transforma con la transformacion compuesta completa; con escala uniforme el ancho escala por `|s|`; una transformacion anidada o no uniforme que no se soporte exactamente ⇒ `UNKNOWN`. `LineWeight` y las anchuras de pantalla o de papel siguen fuera de la huella, como en V10 | CLOSED BY V11 | §11.3 punto 5; T-M71 |
| `[V11-D08]` | AR10-07 · R:V11-07 · C:V11-08 | Consumo declarado por CT-50 sin guarda durable frente a cambios posteriores de los productores | **DURABLE CONTEXT CONSUMER GUARD**: guarda estructural nueva **G-M24** sobre los sitios que crean o configuran entidades relevantes (`LateralHeaderDrawer`, `CantileverViewMaterializer`, `LayerHelper` y todo productor que identifique CT-50): si un productor empieza o deja de asignar una propiedad visual, cambia el uso de capas, estilos o valores por defecto, crea una clase de entidad relevante nueva o cambia la resolucion de estilos, la guarda pasa a RED hasta actualizar CT-50, el `MaterializationContextReadSet`, el `FootprintInputFingerprint` y las pruebas. No se basa solo en hashes de archivos | CLOSED BY V11 (contrato); implementacion: G5..G7 | §8.7; §13 G-M24; §20.2 |
| `[V11-D09]` | AR10-08 · R:V11-08 · C:V11-09 | Entrada de ayuda sin alias con T-GRD-08 reapuntado; la ventana de ayuda pinta el chip del alias aunque este vacio y el par del censo termina en «/» | **HELP ENTRY WITHOUT ALIAS**: decision de producto sin cambio (`RACKMIRROR` comando principal, **sin alias** en V1, **con** entrada de ayuda). Ademas de T-GRD-08: `RackCommandHelpWindow` no pinta chip de alias si `Alias` es nulo o vacio (hoy lo pinta siempre, `RackCommandHelpWindow.cs:90-104` @ `dad4e77`); en el censo por pares el alias ausente es un estado explicito y no produce «/» final (hoy `Pair` concatena `Command + "/" + Alias`, `CustomPropertiesHelpCensusTests.cs:106` @ `dad4e77`); prueba de UI nueva (T-M74); no se inventa alias | CLOSED BY V11 | PDC-8; T-M16; T-M74; §20.1; §20.2 |
| `[V11-D10]` | AR10-03 · R:V11-03 · C:V11-10 | R-45 «antes de G3»; proceso sin reconciliacion entre iniciativas antes del freeze; I-55 registrada en G1.1 | **I-55 CROSS-INITIATIVE GATE**: I-55 (`39fa759`, Proposal V1 NOT CONSENSUS, ADR-0042 propuesto, solo docs): arquitectura hoy NON-MATERIAL; coordinacion HIGH / ACTIVE; solape de produccion NONE. I-52 **no** espera al consenso de I-55, pero el proceso cambia: tras el consenso tecnico sobre V11, rebase sobre el main vigente → **reconciliacion obligatoria entre iniciativas** → solo entonces freeze. La reconciliacion I-52 ↔ I-55 (con I-49 donde aplique) registra la propiedad de: (1) taxonomia de tipo de vista; (2) codec semantico de `View`/`Section`; (3) autoridad «sistema resuelto → plan de vista»; (4) comparador authored; (5) primitivo de materializacion del Plugin; (6) transformaciones de colocacion y proyeccion. Principio: **ninguna autoridad duplicada**. Siguen siendo de I-52 aunque consuman un primitivo compartido: ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia del espejo con `μ_k`. **STOP simetrico**: si cualquiera intenta congelar una autoridad incompatible antes de la reconciliacion, **ningun freeze** para ninguna hasta resolverlo. R-45 pasa de «antes de G3» a **«antes del freeze»**. G4 avisa tambien a I-55 antes de tocar la fundacion compartida. Si ADR-0042 se acepta antes del freeze de I-52, se relee la autoridad que usan C-2 y Actualizar y se reconcilia. La segunda iniciativa en integrar re-establece C-2 sobre el main combinado | CLOSED BY V11 (proceso) | §14.1; §14.2 G2 y G4; §15.2; §15.3; §17 R-45; §19 |
| `[V11-D11]` | AR10-09 · R:V11-09 · C:V11-11 | I-49 `5a3714f` con el Amendment A2 solo docs y el ADR de reemplazo sin numero; ADR-0040 como autoridad vigente | **I-49 CURRENT AUTHORITY**: I-49 observada en `3f17c21` (el Coordinador observo `2e8090a`; despues, solo docs: nuevo Consensus Freeze sobre V6, A1, A2 y ADR-0041). ADR-0041 **aceptado** por el Owner y ADR-0040 **reemplazado**; Proposal V6 + A1 + A2; G6 sigue NOT CLOSED, sin produccion nueva tras el candidato equivalente (`988ab9c`, mismo patch-id de `src` y `tests` que `5f969cc`). Para I-52, ADR-0041 es la autoridad vigente: D19 `PlanReadSet` y D20 son identicos a los de ADR-0040 y la semantica que I-52 consume no cambia (D5 añade la validez neutral de la clave sin cambiar la identidad ni la igualdad de `VariableId`). Contrato NON-MATERIAL; coordinacion MATERIAL. Censo de ADR hasta 0042, sin numeros posteriores (§16) | CLOSED BY V11 (registro) | §6.5.5; §9.2.1 RS-3; §10.2 PV-6; §15; §16; CT-46 |
| `[V11-D12]` | AR10-09 · R:V11-09 · C:V11-12 | I-55 registrada en G1.1 (`cfdb702`) | **I-55 / ADR-0042 WATCH**: se registran la Proposal V1 (`d1918ab`), los paquetes de revision (`39fa759`) y ADR-0042 **propuesto**, que se presenta como sucesor de ADR-0010, en el que I-52 se apoya a traves de C-2 y de la regeneracion como Actualizar. Si su estado cambia durante V11, se re-evalua antes del commit; mientras siga propuesto no hay STOP salvo que su texto cree ya una contradiccion material, y hoy no la crea: declara conservada la semantica de Actualizar e Insertar de ADR-0010 y solo añade, entre otros, vistas adicionales en el flujo de creacion y que un redibujo fallido impida insertar (ADR-0042, «Semantica de edicion que se conserva de ADR-0010», @ `39fa759`) | CLOSED BY V11 (registro) | §15; §16; §19; ADR-0036 referencias |
| `[V11-D13]` | C:V11-13 | Plan de G3 de V10 | CT-36, CT-49, CT-50 y las pruebas relevantes, ampliados con casos obligatorios. **Tipo de linea complejo resuelto:** capa con nombre compleja por `ByLayer`, capa `0` heredada compleja, ruta de creacion de la referencia de pieza con `CELTYPE`/`CLAYER`, nombre `Explicit` cuyo registro del dibujo es complejo, mismo nombre en biblioteca y dibujo con DRAWING WINS. **Inventario de huellas de objetos posteriores:** `Point` con `PDMODE`/`PDSIZE`, `MText` con fondo y columnas, atributos, componentes de cota, `MLeader`, `Table`, underlay/raster, proxy/custom; no soportado ⇒ `UNKNOWN`. **Visibilidad:** capa apagada, inutilizada o inutilizada en ventana, `Entity.Visible = false`, reactivacion posterior, huella de orden sin cambio y conservadora. **Contexto global:** clasificaciones `SUPPORTED`/`UNKNOWN` y read-set de lo realmente consumido. **Cola posterior:** objeto añadido tras R6, eliminado o reordenado, objeto que pasa de antes a despues de la fuente o al reves. **Texto y cota transitivos:** cambio de una dependencia de `DIMSTYLE`, cambio de `TEXTSTYLE` o de metricas de fuente, `AnnotationScale` authored frente a `CANNOSCALE`. **Ancho de modelo:** `s ≠ 1` uniforme, escala anidada, no uniforme no soportada ⇒ `UNKNOWN`. **Ayuda:** sin chip de alias y sin «/» final. G3 sigue sin produccion; contradiccion material ⇒ Proposal V12 | CLOSED BY V11 | §12; §13; §14 |
| `[V11-D14]` | C:V11-14 | ADR-0036 alineado con V10 | ADR-0036 sigue **PROPOSED** y actualiza las decisiones 11 a 13 con la elegibilidad de la presentacion resuelta, la `ModelSpaceFootprintPolicy` cerrada, la huella de orden independiente de la visibilidad, la politica de ocupacion dependiente del entorno, la cola completa de objetos posteriores, el read-set transitivo del contexto de materializacion, la transformacion de anchos de modelo y la reconciliacion con I-55 antes del freeze; vigila las referencias a ADR-0010 frente a ADR-0042; nunca presenta resultados de G3; «la caracterizacion **DEBE** demostrar» | CLOSED BY V11 | ADR-0036 |
| `[V11-D15]` | C:V11-15 · R:V11-10 | Contradiccion material de V10 ⇒ Proposal V11 | Nueva matriz `[V11-Dnn]` sobre AR10-01..AR10-09 y las decisiones vinculantes del Coordinador; `[V10-D01]`, `[V10-D02]`, `[V10-D16]`, `[V10-D17]` y `[V10-D18]` se reabren y se cierran explicitamente (§0.4); lo demas de V10 sigue vigente; `OPEN MATERIAL = NONE` para revision; toda contradiccion material de V11 ⇒ **Proposal V12**; proceso: acuerdo sobre V11 → consenso tecnico → rebase sobre el main vigente → **reconciliacion obligatoria I-52/I-55/I-49** → freeze por SHA exacto → re-check exact-SHA → O-1 → G3 → (Proposal V12 si G3 contradice materialmente / ADR-0036 aceptado → G4) | CLOSED BY V11 | §1.2; §7.0; §14.1; §16; §19 |
| `[V11-D16]` | Precision al redactar (revision de V10, §7) | «Una base por pieza tambien es valida si reproduce el mismo resultado» | **Base auxiliar por pieza**: solo es admisible si reproduce **exactamente** la misma resolucion global que la precedencia DRAWING FIRST con un unico `EnsureForPlan`: mismo estado del dibujo previo a importar, mismo conjunto de nombres importados y misma biblioteca efectiva identificada por SHA-256; si no puede demostrarse ⇒ `UNKNOWN` | CLOSED BY V11 | §11.3 punto 4; CT-36; T-M63 |
| `[V11-D17]` | Precision al redactar | O-1 sobre L-01..L-37 con L-26, L-34, L-36 y L-37 de V10 | O-1 sigue sobre **L-01..L-37**: L-30 reescrita (tipo de linea efectivo complejo, no determinable o no caracterizado, resuelto en el destino, incluida la herencia); L-34 reescrita (objetos posteriores de clases sin huella soportada, ocupacion latente con independencia de la visibilidad, representacion dependiente del entorno sin huella conservadora y cola de Model Space cambiada); L-36 precisada (read-set transitivo y `AnnotationScale` authored frente a `CANNOSCALE`); L-37 reescrita (objetos posteriores cuya representacion depende del entorno y no tiene huella conservadora pueden hacer fallar el comando) | CLOSED BY V11 | §1.2; §1.3 |
| `[V11-D18]` | Precision al redactar | Preflight de V10 | Preflight de V11 (§15.1): I-52 `636f7fd` exacta (arbol limpio, sin stash ni operaciones a medias); main `dad4e77` sin cambios; I-49 `3f17c21` (el Coordinador observo `2e8090a`; un commit posterior solo docs); I-55 `39fa759`; sin ramas nuevas; ADR 0035..0042 sin numeros posteriores ni colisiones; biblioteca con el mismo SHA-256; censo de `[CommandMethod(`: main 35, I-49 35, I-55 35, I-52 33. Ninguna paralela modifica una autoridad de I-52: sin STOP | CLOSED BY V11 (registro) | §15.1 |
| `[V11-D19]` | Precision al redactar | Terminologia de V10: huella de la referencia, huella visual, `FootprintInputFingerprint` y `TraceFingerprint` | **Terminologia**: se añaden `ResolvedPresentationEligibility` (pregunta B, §11.3 punto 6.2), `ModelSpaceFootprintPolicy` (clasificacion por tipo exacto de los objetos del dibujo, §11.3 punto 5) y `ModelSpaceOrderTailSnapshot` (cola ordenada de Model Space posterior a cada fuente, §9.2), distintos de la huella de la referencia, de `VisualFootprintResult`, de `FootprintInputFingerprint`, de `TraceFingerprint` y de `GeometrySymmetryResult`; ninguno sustituye a otro | CLOSED BY V11 | §9.1; §9.2; §11.3 puntos 5, 6.2 y 10 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V12 o reduccion de alcance |
...
| **Guarda durable de los productores de contexto** (G-M24, `[V11-D08]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas ⇒ RED | G5..G7 y despues de integrar |
...
| **Ocupacion de modelo conservadora por clase** (anchos geometricos transformados con la transformacion compuesta, mascaras, limites de texto y de cota, marcos, anidados; `[V9-D02]`, `[V10-D01]`, `[V11-D07]`) | huella no garantizada conservadora ⇒ `UNKNOWN` (E10 en ST-19, X-22 en orden de plan) | G3 (CT-36, CT-49) |
| **`ModelSpaceFootprintPolicy` por tipo exacto de los objetos del dibujo** (`[V11-D02]`) | clase, componente o estado no caracterizado ⇒ huella `UNKNOWN` ⇒ E10 si el objeto es posterior relevante (ST-19); `Extents3d` solo donde CT-49 lo demuestre cota conservadora | G3 (CT-49) y G6/G7 (T-M73) |
| **Huella de orden independiente de la visibilidad** (ocupacion latente con capas apagadas o inutilizadas, `Entity.Visible = false` o `ANNOALLVISIBLE`; `[V11-D03]`) | clase o representacion sin ocupacion conservadora independiente de esos estados ⇒ `UNKNOWN` ⇒ E10 | G3 (CT-49) y G6/G7 (T-M73) |
| **Ocupacion de modelo dependiente del entorno** (`PDMODE`, `PDSIZE`, `LTSCALE`, `MSLTSCALE`, representaciones anotativas, metricas de fuente y lo que descubra CT-49; `[V11-D04]`) | sin ocupacion conservadora demostrada bajo todos los estados admitidos ⇒ `UNKNOWN`; con ella, estados declarados como observaciones consumidas | G3 (CT-49, CT-50) |
...
| **Superposiciones con todo objeto posterior a la fuente y orden efectivo en Model Space** (`[V9-D01]`, `[V9-D04]`, `[V10-D06]`, `[V10-D07]`, `[V11-D02]`, `[V11-D03]`) | superposicion material o no clasificable, huella no disponible o no conservadora, `XLINE`/`RAY` sin interseccion analitica, conflicto de ordenes de pantalla y trazado con copias superpuestas o contexto de orden no legible ⇒ E10 (ST-19) | G3 (CT-49) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V12 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V12 | G3 (CT-37, CT-38) |
...
| **Guardas de I-49 G6** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones; ADR-0041 vigente; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18 | reconciliacion posterior a la integracion de I-49 |
| **Reconciliacion con I-55 (e I-49 donde aplique) antes del freeze** (View Placement & Projection, Proposal V1 sin consenso, ADR-0042 propuesto; `[V11-D10]`, `[V11-D12]`, R-45) | sin reconciliacion registrada de las seis autoridades compartidas no hay freeze; una autoridad incompatible congelada por cualquiera de las dos ⇒ ningun freeze para ninguna hasta resolverlo; I-52 no espera al consenso de I-55 | tras el consenso tecnico y el rebase, antes del freeze |

### 0.4 Estado de las marcas de V10 bajo V11

| Marca V10 | Estado en V11 |
|---|---|
...
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V12) |
...
### 0.5 Estado de las marcas de V9 (fijado por V10; sin cambios en V11)
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V12) |
| `[V9-D16]` | vigente |
...
### 0.6 Estado de las marcas de V8 (fijado por V9; sin cambios en V10 ni V11)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V12) |
...
### 0.7 Estado de las marcas de V7 (fijado por V8; sin cambios en V9, V10 ni V11)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V12) |
...
### 0.8 Estado de las marcas de V6 (fijado por V7; sin cambios en V8, V9, V10 ni V11)
...
### 0.9 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8, V9, V10 ni V11)
...
### 0.10 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V11) |
|---|---|
...
### 0.11 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR10-01 (tipo de linea complejo resuelto en el destino) | `[V11-D01]`, `[V11-D16]` |
| AR10-02 (huellas de objetos posteriores: inventario por tipo exacto, visibilidad y entorno) | `[V11-D02]`, `[V11-D03]`, `[V11-D04]` |
| AR10-03 (reconciliacion con I-55 antes del freeze) | `[V11-D10]`, `[V11-D12]` |
| AR10-04 (contexto transitivo de texto y cota) | `[V11-D06]` |
| AR10-05 (cola completa de objetos posteriores) | `[V11-D05]` |
| AR10-06 (transformacion de anchos de modelo) | `[V11-D07]` |
| AR10-07 (guarda durable de productores) | `[V11-D08]` |
| AR10-08 (ayuda sin alias) | `[V11-D09]` |
| AR10-09 (hechos de paralelas) | `[V11-D11]`, `[V11-D12]`, `[V11-D18]` |
| Cambios exactos R:V11-01..R:V11-10 de la revision | V11-01 → D01, D16; V11-02 → D02, D03, D04; V11-03 → D10; V11-04 → D06; V11-05 → D05; V11-06 → D07; V11-07 → D08; V11-08 → D09; V11-09 → D11, D12; V11-10 → D15 |
| Precisiones del Coordinador C:V11-01..C:V11-15 | V11-01 → D01; V11-02 → D02; V11-03 → D03; V11-04 → D04; V11-05 → D05; V11-06 → D06; V11-07 → D07; V11-08 → D08; V11-09 → D09; V11-10 → D10; V11-11 → D11; V11-12 → D12; V11-13 → D13; V11-14 → D14; V11-15 → D15 |
| ADR-0036 (decisiones 11..13 y alineacion general) | `[V11-D14]` |
| Paralelas y main observados en el preflight y en el re-fetch de V11 | `[V11-D10]`, `[V11-D11]`, `[V11-D12]`, `[V11-D18]` |
| Precisiones del ejecutor | `[V11-D16]`..`[V11-D19]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V11; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V12 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V11; si la contradice ⇒ Proposal V12; **precondicion de G4** |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10 y V11) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10 y V11 (ST-19 completo, huellas
visuales de ocupacion de modelo en el contexto de evaluacion destino, frescura tras `EnsureForPlan`, orden de plan sobre
pares invertidos, read-set transitivo del contexto de materializacion, elegibilidad de la presentacion resuelta,
politica de huellas por tipo exacto de los objetos del dibujo y cola completa de Model Space) no se han medido sobre
esta biblioteca y las miden CT-36, CT-48, CT-49 y CT-50. Estado: **DEFERRED TO G3** con fail-closed (X-17a, X-17b, X-22,
X-23).
...
| **Objeto posterior sin huella de orden soportada** | clase, componente o estado no caracterizado por la `ModelSpaceFootprintPolicy`, ocupacion latente no acotable con independencia de la visibilidad o representacion dependiente del entorno sin huella conservadora (`[V11-D02]`..`[V11-D04]`) | E10 (ST-19) |
...
| **ST-19** | PREPARE | Orden visual en Model Space (`[V9-D01]`, `[V9-D04]`, `[V9-D19]`, `[V10-D01]`..`[V10-D03]`, `[V10-D06]`, `[V10-D07]`, `[V11-D02]`..`[V11-D05]`): las copias se crean en el **orden efectivo relativo de sus fuentes** (orden de dibujo de Model Space con su `SortentsTable` y el contexto de orden que fije CT-49); cada copia queda despues de todo objeto existente. Para cada copia `c` de su fuente `s` y **todo** objeto existente `o` de Model Space con `o ≠ s` que va **despues** de `s` en el orden efectivo —otra fuente admitida seleccionada, un objeto seleccionado pero ignorado por C1..C3, un objeto no seleccionado, otro rack o cualquier entidad— se clasifica la superposicion de `VisualFootprint(c)` y `VisualFootprint(o)` (§11.3 punto 5) con las clases de §11.3 punto 6.5 ampliadas a Model Space (rellenos, degradados, solidos, wipeouts, imagenes, OLE, texto con mascara, transparencia no opaca y referencias que los contengan). Las huellas son **ocupacion de modelo en WCS**, sin anchuras de linea, de papel ni de pantalla (L-37): `VisualFootprint(c)` = union de las huellas de las instancias de los planes **definitivos** de cada vista seleccionada, con el nombre destino, evaluadas en el `DestinationEvaluationContext` (§11.3 punto 4) y construidas **despues** de transformar a WCS con `P'`, mas las de sus anotaciones y cotas; `VisualFootprint(o)` = ocupacion de modelo **latente** del objeto segun la `ModelSpaceFootprintPolicy` de su tipo exacto en tiempo de ejecucion (`SUPPORTED(metodo)` o `UNKNOWN`, nunca un fallback optimista; `Extents3d` solo en las clases donde CT-49 demuestre que es una cota conservadora tras añadir todas las ocupaciones de modelo necesarias; sin dilatar por `LineWeight`; `XLINE` y `RAY` ⇒ `UNKNOWN` salvo huella o interseccion analitica caracterizada por CT-49), independiente del estado transitorio de visibilidad (capa apagada, inutilizada o inutilizada en una ventana, `Entity.Visible = false`, `ANNOALLVISIBLE`), porque el orden persiste si la visibilidad cambia, y, si su representacion depende del entorno, `SUPPORTED` solo con una ocupacion conservadora demostrada bajo todos los estados que admite el contrato (`[V11-D02]`..`[V11-D04]`). Los objetos anteriores a `s` no se clasifican; la fuente `s` es excepcion deliberada (la copia puede quedar encima de ella); las copias entre si siguen el orden de sus fuentes. Si CT-49 encuentra contextos de pantalla y de trazado con ordenes distintos, «posterior» significa posterior en cualquiera de ellos y, si dos fuentes admitidas tienen orden relativo distinto entre esos contextos y sus copias se superponen de forma material o no clasificable, no existe un orden de creacion que conserve los dos. La evaluacion anterior a `EnsureForPlan` es rechazo anticipado; la decision definitiva se toma tras releer el dibujo real, con las huellas refrescadas y la cola de Model Space posterior a cada fuente enumerada de nuevo en R6 (`ModelSpaceOrderTailSnapshot`: identidades ordenadas, clases y contexto efectivo de orden), que se vuelve a enumerar al inicio de MUTATE; un objeto anterior que sigue siendo anterior no invalida (PRE-18, `[V11-D05]`) | superposicion **material o no clasificable**, huella no disponible, infinita o dudosa, `XLINE`/`RAY` posterior sin interseccion analitica, ordenes de pantalla y de trazado contradictorios con copias superpuestas, orden efectivo no legible, huella que cambia tras importar sin re-demostrarse, objeto posterior de una clase, un componente o un estado no soportados por la `ModelSpaceFootprintPolicy`, ocupacion latente no acotable, representacion dependiente del entorno sin huella conservadora o cola de Model Space cambiada (objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada) ⇒ **fail-closed (E10)** en PREPARE, antes de MUTATE (`[V8-D25]`); en MUTATE, excepcion y rollback (E11) |
...
equivalencia de la fachada: **STOP → Proposal V12**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
- **Si I-49 integra antes** (hoy: V6 y Amendments A1 y A2 congelados, ADR-0041 aceptado en lugar de ADR-0040, que ya
  ...
  semantico —enlace y evaluacion de un arbol— en su rama, sin integrar, sin dominio aplicado y sin `PlanReadSet`;
  `[V9-D12]`, `[V9-D21]`, `[V10-D14]`, `[V11-D11]`): el dominio que su descriptor declara **como dato** y aplica al
  valor efectivo (I-49 V6 P24.5: `>= 0` en las dos propiedades Selectivas) es la cota `b`. V-DEP la consume como dato;
  I-52 no la reimplementa ni la infiere. Se re-mide en la reconciliacion (§15.3). Un dominio `>= 0` **no** convierte en
  universal la evidencia visual-geometrica de DEP-09/DEP-10a..c (sigue siendo un continuo de estados).
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V11: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V11: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V11 ⇒ **STOP → Proposal V12**.
...
| `LengthUnitsAuthorityGuardTests` (nueva) y `ExpressionCoreGuardTests`/`ExpressionParserGuardTests` extendidas | I-49 G6 `988ab9c` (antes `5f969cc`, mismo patch-id) | ningun archivo de producto fuera de `RackCad.Application.Units` (y del alias `StructuralSectionUnits`) declara literales o conversiones de unidades de longitud (`25.4`, `InchesPerFoot`, sufijos `[mm]`); el nucleo y las unidades no nombran otras capas ni tipos dimensionales | I-52 no convierte unidades ni nombra el nucleo; al reconciliar con I-49 rige la version vigente (CT-46, G-M18, `[V9-D21]`, `[V10-D14]`) |
...
## 7. Matriz de representabilidad V11
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V11)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V12**. T-M47 lo hace ejecutable desde
G5.
...
MaterializationContext (nombre no contractual; no es schema persistido)
  observaciones candidatas: CLAYER · CECOLOR · CELTYPE · CELTSCALE · CELWEIGHT · CETRANSPARENCY · CPLOTSTYLE ·
                            ...
                            que descubran CT-36, CT-49 o CT-50
...
MaterializationContextReadSet (nombre ilustrativo)
  ...
  en la operacion (piezas y colocaciones de HeaderRun, DBText, cotas, curvas Cantilever), con CIERRE TRANSITIVO:
    cota    ⇒ registro efectivo del estilo de cota · estilo de texto que use · bloques de flecha que use · tipos de
               linea, relleno y demas propiedades que afecten a su geometria o a su huella · lo que CT-50
               demuestre material
...
Regla
  ...
  G-M24 protege los sitios productores: si uno empieza o deja de asignar una propiedad visual, cambia el uso de
    capas, estilos o valores por defecto, crea una clase de entidad relevante nueva o cambia la resolucion de
    estilos, la guarda queda en RED hasta actualizar CT-50, este read-set, el FootprintInputFingerprint y las
    pruebas ([V11-D08])
```
...
- Las huellas visuales de anotaciones y cotas consumen los registros de estilo de cota y de texto, con su cierre
  ...
  demuestra su consumo (§11.3 punto 5); esas observaciones entran en su `FootprintInputFingerprint` (§9.2, PRE-18).
- Consecuencia para O-1 (L-36): las piezas internas de la copia se regeneran como Actualizar; no se copian fisicamente
  ...
  pieza (§1.4).
- No crea obligacion de tocar los drawers existentes (§20.3); G-M8 y G-M11 no cambian, y G-M24 (nueva) vigila sus sitios
  productores sin modificarlos.
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R2  huellas visuales en el DestinationEvaluationContext de las instancias de esos planes, anotaciones y cotas
      incluidas, con la elegibilidad de su presentacion resuelta ([V11-D01]), y huellas de orden de los objetos de
      Model Space posteriores a cada fuente segun la ModelSpaceFootprintPolicy de su tipo exacto, independientes de
      la visibilidad transitoria ([V11-D02]..[V11-D04]; §4.1, §11.3 punto 5)
...
(§11.3 punto 6.2). Ninguno sustituye a otro (`[V10-D19]`, `[V11-D19]`).
...
| orden efectivo en Model Space de las fuentes y su contexto de pantalla y de trazado (`SortentsTable` y las variables que fije CT-49) y, desde R6, la **`ModelSpaceOrderTailSnapshot`**: enumeracion completa y de nuevo de **todos** los objetos posteriores a cada fuente, con identidades ordenadas, clases y contexto efectivo de orden, y la huella de orden de cada uno con sus observaciones consumidas; al inicio de MUTATE se vuelve a enumerar, sin limitarse a los objetos capturados antes | objeto añadido, eliminado, reordenado, movido de antes a despues de la fuente o con identidad o clase cambiada, o huella de orden cambiada, invalida la demostracion del orden; un objeto anterior que sigue siendo anterior no invalida (ST-19, `[V8-D25]`, `[V9-D01]`, `[V11-D05]`) |
...
| PV-6 | Si I-49 integra antes (hoy: V6 y Amendments A1 y A2 congelados (`3f17c21`), ADR-0041 aceptado en lugar de ADR-0040; G5 sintactico `892a601`, antes `977f873` y `c4880af`, y G6 `988ab9c`, antes `5f969cc`, con enlace y evaluacion de un arbol en su rama; sin integrar y sin `PlanReadSet` ni dominio aplicado; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`): RACKMIRROR invoca la autoridad efectiva integrada, preserva `expression` como portador, adopta su `PlanReadSet` para el read-set (§9.2.1) y su dominio de consumidor como cota autoritativa de V-DEP (§6.5.5), y **no** reimplementa su evaluacion ni un segundo motor de expresiones (P24.5 y P24.8 de I-49 V6) |
...
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   - **Construccion (`[V11-D07]`).** La geometria local ocupada **completa** —incluidos los anchos reales de modelo:
     ancho geometrico, mascara, marco y limites de texto o de cota— se transforma con la transformacion compuesta
     completa: anidados con su transformacion completa, la instancia y `P'`, con la escala uniforme `s` de la fuente
     (ST-11). Con escala uniforme cada ancho de modelo escala por el `|s|` acumulado; una transformacion anidada o no
     uniforme que no se soporte exactamente ⇒ `UNKNOWN`. Nunca se transforma una linea media para añadirle despues un
     ancho local sin transformar, y ninguna anchura de presentacion entra en la huella.
   ...
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`).** Autoridad conceptual distinta para los objetos arbitrarios del
     dibujo que ST-19 clasifica:
     ...
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       ...
       resultado por clase: SUPPORTED(metodo) | UNKNOWN              nunca un fallback optimista
       inventario minimo de CT-49: Line · Arc · Circle · Ellipse · Spline · LWPOLYLINE · Solid · Trace · Hatch ·
         BlockReference con sus atributos · DBText · MText con fondo y columnas · Dimension con flechas, relleno y
         demas componentes · MLeader · Table · Point · Wipeout · raster y underlay · XLINE/RAY · nube de puntos ·
         proxy o entidad custom (no es obligatorio soportarlas todas)
       clase, componente o estado no caracterizado ⇒ huella UNKNOWN ⇒ E10 si el objeto es posterior relevante (ST-19)
       Extents3d solo donde CT-49 demuestre que es cota conservadora tras añadir todas las ocupaciones de modelo
     ```

   - **Ocupacion latente, independiente de la visibilidad (`[V11-D03]`).** Para ST-19, la huella de orden representa la
     ocupacion **latente** que puede volverse visible sin cambiar el orden de dibujo. No se excluye geometria porque hoy
     su capa este apagada, inutilizada o inutilizada en una ventana, porque `Entity.Visible = false` ni por el valor
     actual de `ANNOALLVISIBLE`: la relacion de orden persiste si esa visibilidad cambia. Una clase o representacion que
     no pueda producir una ocupacion conservadora independiente de esos estados ⇒ `UNKNOWN` ⇒ E10. `VisibilityPath`
     (punto 6.3) sigue rigiendo la equivalencia visual de las piezas; esta regla es especifica de ST-19 sobre los
     objetos del dibujo, y las huellas de las copias tampoco excluyen geometria por visibilidad.
   - **Ocupacion dependiente del entorno (`[V11-D04]`).** `PDMODE`, `PDSIZE`, `LTSCALE`, `MSLTSCALE`, las
     representaciones anotativas, las metricas de fuente o de texto y lo que descubra CT-49: si CT-49 demuestra una
     ocupacion de modelo conservadora bajo **todos** los estados relevantes que admite el contrato, la clase es
     `SUPPORTED` y esos estados se declaran como observaciones consumidas en el `FootprintInputFingerprint` del objeto;
     si no, `UNKNOWN`. No se inventa una formula universal. Un objeto posterior asi, sin huella conservadora, puede
     hacer fallar el comando (L-34, L-37).
   ...
   - **Frescura (`[V10-D03]`).** Cada huella lleva su `FootprintInputFingerprint`: identidades de las definiciones
     ...
     efectivo, identidades de las dependencias anidadas, estados del entorno de una huella `SUPPORTED` (`[V11-D04]`) y
     cualquier otra entrada que altere la huella. Tras `EnsureForPlan` se relee el dibujo real; toda huella cuyo
     fingerprint cambie se vuelve a producir y se re-evaluan el orden de plan y ST-19 (PRE-18). No se usa
     `TraceFingerprint` para esto (punto 10).
   ...
   - **Una sola autoridad** de extraccion de huellas y de sus margenes de modelo (G-M23). CT-36 y CT-49 caracterizan las
     ocupaciones reales por clase; T-M71 las prueba con dobles en Core. La `ModelSpaceFootprintPolicy` es la unica
     clasificacion de los objetos del dibujo y T-M73 prueba su consumo (`[V11-D02]`..`[V11-D04]`).
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V12.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V11 no introduce tolerancia; CT-36 registra
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V11 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V11 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V11 no introduce una tolerancia nueva: CT-36
...
12. **Estados futuros por variables.** La verificacion de un estado concreto **no** basta si el mismo rack puede cambiar ese
    ...
    dominio ni `PlanReadSet`) no cambian esto (`[V9-D12]`, `[V9-D21]`, `[V10-D14]`); si I-49 integra su dominio y su
    `PlanReadSet`, se consumen sin analisis propio.
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **STOP → Proposal V12**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V11 abre **Proposal V12**.
...
| **CT-46** | **Guardas cruzadas vigentes**: relee en el SHA de la reconciliacion las guardas de texto de `ProjectVariablesConformanceTests.cs` (main y, si integra, I-49), C-08/C-10, las del nucleo de expresiones (extendidas por I-49 G6 `988ab9c`, antes `5f969cc`, con `LengthUnitsAuthorityGuardTests` sobre todo `src/`) y T-GRD-01..T-GRD-08 de I-54, ya en `main @ ba497f1` (incluidas las del borde fisico, T-GRD-04..T-GRD-07, y los censos por nombre de comandos, ayuda y ventanas, T-GRD-08); fija la lista exacta que G-M18 aplica a los archivos de I-52 y la autoridad vigente de I-49 (ADR-0041, aceptado y que reemplaza a ADR-0040, `[V8-D18]`, `[V11-D11]`) | Core |
| **CT-47** | **Corpus indicativo de la biblioteca actual**: recalcula el **SHA-256** del archivo antes de usar la evidencia (identidad; ruta, tamaño y fecha son diagnostico) y, si cambia, no reutiliza la evidencia previa; recalcula en `acad.exe`, con evaluacion de estados, la tabla de §1.4 por pieza y vista con las reglas de V11 (politica cerrada de variantes con inventario cerrado, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, `VisibilityPath`, `LinetypeScale` y contexto de patron, orientacion, multiplicidad, orden visual dentro de la pieza y entre piezas sobre huellas de ocupacion de modelo en el contexto destino y pares invertidos, degradados, falsos positivos y negativos de la canonicalizacion) y la compara con la evidencia indicativa; una reduccion material de la envolvente ⇒ Proposal V12 | evidencia registrada |
...
→ rebase / reconciliacion sobre el main vigente         (solo docs; hoy main = dad4e77 con I-53 E1, I-53S
                                                          ...
                                                          §15.1)
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55, con I-49 donde aplique; registra la
                                                          propiedad de la taxonomia de tipo de vista, del codec
                                                          semantico de View/Section, de «sistema resuelto → plan
                                                          de vista», del comparador authored, del primitivo de
                                                          materializacion del Plugin y de las transformaciones de
                                                          colocacion y proyeccion; sin autoridades duplicadas;
                                                          [V11-D10])
...
### 15.1 main y paralelas observadas en el preflight de V11
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ba497f1, que integraba I-54 sobre 104ef3a; sin cambios desde V10;
                                                     ancla de las citas de V11: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = 636f7fd0787c001f61a83c863104402329ebe877 (V10; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = 3f17c21cde429946248fef6f15588092c156ad12 (Consensus Freeze sobre
                                                     V6, A1, A2 y ADR-0041, solo docs; ADR-0041 aceptado en 2e8090a y
                                                     ADR-0040 reemplazado; rama rebasada sobre dad4e77: G5 892a601 y
                                                     G6 988ab9c, equivalente a 5f969cc; G6 NOT CLOSED; sin integrar)
I-55 feature/creacion-de-vistas                    = 39fa75964a5875b3fbbb61186e385b254ded083a (View Placement &
                                                     Projection: Proposal V1 d1918ab NOT CONSENSUS y paquetes de
                                                     revision 39fa759; ADR-0042 propuesto; solo docs; sin integrar)
...
Censo de [CommandMethod(                          = main dad4e77: 35 · I-49: 35 · I-55: 35 · I-52: 33
ADR 0035..0042                                     = 0035, 0037 y 0039 aceptados en main; 0036 propuesto solo en I-52;
                                                     ...
                                                     en I-55; sin numeros posteriores ni colision
...
**main desde la publicacion de V10.** Sin cambios: `dad4e77`. Ninguna cita de V11 se desplaza respecto de V10; las citas
nuevas de V11 (`LateralHeaderDrawer.cs:266-280`, `:375`, `:385`; `SelectiveAnnotations.cs:15`, `:21`;
`DynamicViewDecorations.cs:53`; `RackCommandHelpWindow.cs:90-104`) son identicas en `1b091be` y en `dad4e77`, y
`CustomPropertiesHelpCensusTests.cs:106` se lee en `dad4e77`.

**Cambios de paralelas desde la publicacion de V10.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V11 declaraba
como ultimos observados main `dad4e77`, I-52 `636f7fd`, I-49 `2e8090a` e I-55 `39fa759`; el preflight de V11 los
confirmo, con I-49 un commit mas adelante (`3f17c21`, solo docs), sin ramas nuevas y con ADR-0041 aceptado y ADR-0042
propuesto (`[V11-D18]`). La historia de paralelas anterior esta en Proposal V10 §15.1, en Proposal V9 §15.1, en
Proposal V8 §15.1, en Proposal V7 §15.1 y en el registro §42, §48, §54 y §60. La numeracion de ADR esta en §16.
...
| **I-49** (V6 y Amendments A1 y A2 congelados en `3f17c21`; ADR-0041 **aceptado**, que reemplaza a ADR-0040, que a su vez reemplazo a ADR-0038; rama rebasada sobre `dad4e77`: G5 sintactico `892a601` y G6 `988ab9c` productivo, sin integrar; G6 NOT CLOSED) | `expression` en `PropertyValues`; `PlanReadSet` (D19); dominio del consumidor declarado por el descriptor (P24.5); su freeze exige que la prueba de supervivencia de `expression` (P24.8) se escriba contra `RackDuplicationPlan` y el restamp vigentes en main; guardas de texto evolucionadas en G5 y, en G6, `LengthUnitsAuthorityGuardTests` sobre todo `src/`; ADR-0041 como sucesor aceptado de ADR-0040 (cambian solo la validez de la clave y la sintaxis del cualificador: D5..D8 y D25; D19 identico) | **I-52 coordina con I-49 antes de tocar `RackDuplicationPlan.cs`** (G4); T-M19. Si I-49 integra antes: su `PlanReadSet` es RS-3 (§9.2.1), su dominio es la cota `b` de V-DEP, su descriptor se clasifica en la guarda (§6.7) y sus guardas de texto rigen (G-M18, CT-46). `8ed15f7` es **NON-MATERIAL** (`[V8-D18]`) y `5f969cc` tambien lo es para el contrato, sin `PlanReadSet` ni dominio (`[V9-D21]`, `[V10-D14]`), y A2 con ADR-0041 aceptado solo cambia la validez de la clave y la sintaxis del cualificador de `VariableId`, sin tocar lo que I-52 consume (`[V11-D11]`): las referencias de I-52 a ADR-0038 y a ADR-0040 se leen en ADR-0041, con D19 identico. V11 **no** presupone el motor |
...
| **I-55** (View Placement & Projection; rama `feature/creacion-de-vistas` @ `39fa759`, sobre `dad4e77`; Proposal V1 `d1918ab` NOT CONSENSUS; ADR-0042 propuesto; solo docs) | ID17, ID18 e ID19 sobre una foundation de preparacion de vistas antes de materializar; §17.2 de su Proposal reconcilia con I-52 en X-1..X-6: nucleo neutral de seleccion; plan por vista y codec de `View`/`Section` en ubicacion neutral; comparador authored por kind; primitivo «definicion + sobre + referencia» con las politicas de I-52 (ST-13, ST-19, PRE-17, PRE-18, read-set) fuera del primitivo; aviso antes de editar archivos STOP o NO TOCAR de I-52; re-establecimiento de C-2 por la segunda en integrar. ADR-0042 se propone como sucesor de ADR-0010 conservando Actualizar e Insertar | arquitectura hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**; I-52 **no** espera al consenso de I-55. Tras el consenso tecnico sobre V11 y el rebase, **reconciliacion obligatoria antes del freeze** de la propiedad de las seis autoridades compartidas (taxonomia de tipo de vista, codec de `View`/`Section`, «sistema resuelto → plan de vista», comparador authored, primitivo de materializacion del Plugin, transformaciones de colocacion y proyeccion), sin autoridades duplicadas; siguen siendo de I-52 ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`; STOP simetrico ante una autoridad incompatible congelada antes de reconciliar; si ADR-0042 se acepta antes del freeze de I-52, releer la autoridad de C-2 y Actualizar; la segunda iniciativa en integrar re-establece C-2 (R-45, `[V11-D10]`, `[V11-D12]`) |
...
§19; si `main` avanzo, rebase segun WORKFLOW antes de escribir codigo; y, tras el consenso tecnico y el rebase, la
reconciliacion obligatoria con I-55 e I-49 antes del freeze (§14.1, `[V11-D10]`). Ademas:
...
- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  ...
  nucleo semantico y la autoridad de unidades con `LengthUnitsAuthorityGuardTests` sobre todo `src/`: al reconciliar,
  CT-46 la relee y G-M18 la aplica a los archivos de I-52. Su Consensus Freeze `3f17c21` habilita la correccion acotada
  de G6; si esa correccion o un gate posterior de I-49 introduce `PlanReadSet` o dominio aplicado, o toca
...
- **I-55 (`feature/creacion-de-vistas`, View Placement & Projection, solo docs; gate de coordinacion activo,
  `[V11-D10]`, `[V11-D12]`):** I-52 no espera a su consenso, pero no congela sin la reconciliacion obligatoria posterior
  al consenso tecnico sobre V11 y al rebase, que registra la propiedad de la taxonomia de tipo de vista, del codec
  semantico de `View`/`Section`, de la autoridad «sistema resuelto → plan de vista», del comparador authored, del
  primitivo de materializacion del Plugin y de las transformaciones de colocacion y proyeccion, sin autoridades
  duplicadas. Siguen siendo de I-52, aunque consuman un primitivo compartido, ST-13, ST-18, ST-19, PRE-17, PRE-18,
  `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`. Si cualquiera de las dos intenta
  congelar una autoridad incompatible antes de reconciliar: STOP, ningun freeze para ninguna (R-45, §19). G4 avisa a
  I-55 antes de tocar la fundacion compartida (nucleo de seleccion en `RackDuplicationPlan.cs`). Si ADR-0042 cambia de
  estado, se re-evalua antes del siguiente commit y, si se acepta antes del freeze de I-52, se relee la autoridad que
  usan C-2 y Actualizar. Si su G2 toca rutas de materializacion, `RackEmbedComposer.Compose`, `SystemBlockWriter.cs`,
  comandos o `RackCommandReference.cs`, se re-evaluan §8.1, §8.7, `MaterializationContextReadSet`, PRE-17, PRE-18,
  T-GRD-02 y el censo `B`; la segunda iniciativa en integrar re-establece C-2 sobre el main combinado.
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V11.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V11 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   (`2026-09-14T00:45:13-06:00`), **propuesto** como sucesor de ADR-0010. No hay numeros posteriores a 0042 en ningun
   ref (`[V11-D11]`, `[V11-D12]`).
...
no contradice materialmente V11, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V12. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V12 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V12 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V12 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V12 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V12 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V12 |
...
| **R-45** | I-55 (View Placement & Projection, Proposal V1 `d1918ab` NOT CONSENSUS con X-1..X-6 y ADR-0042 propuesto, solo docs) preve una foundation de preparacion de vistas antes de materializar y una proyeccion multi-rack con una transformacion comun (ID19) que cruzan la autoridad pura de planes por vista de I-52 (§8.1, C2-1), su materializador (G6), el contexto de materializacion (§8.7) y la colocacion `P'`: dos autoridades paralelas podrian divergir o duplicarse | I-52 no depende de I-55 ni espera a su consenso; **antes del freeze** (no antes de G3) se hace la reconciliacion obligatoria que registra la propiedad de las seis autoridades compartidas, sin duplicarlas; STOP simetrico si cualquiera congela una autoridad incompatible antes de reconciliar; G4 avisa a I-55 antes de tocar la fundacion compartida (§14.1, §15.3, §19, `[V11-D10]`) |
| **R-46** | La elegibilidad de la presentacion resuelta (`[V11-D01]`) puede dejar en `UNKNOWN` piezas de dibujos cuyos usuarios usan tipos de linea complejos en capas homonimas, en la capa de la fuente o en `CLAYER`/`CELTYPE`, y reducir el alcance util | caracterizacion en CT-36, CT-49 y CT-50; fail-closed explicito (X-17b, L-30); si afecta a kinds enteros o a configuraciones habituales ⇒ Proposal V12 y O-1 se reevalua |
| **R-47** | La `ModelSpaceFootprintPolicy`, la huella latente independiente de la visibilidad y la politica de entorno (`[V11-D02]`..`[V11-D04]`) pueden hacer fallar RACKMIRROR con frecuencia en dibujos con objetos posteriores de clases no soportadas, en capas ocultas o dependientes del entorno | inventario de CT-49 con tantas clases `SUPPORTED(metodo)` como se puedan demostrar; E10 atribuible; declarado en O-1 (L-34, L-37); reduccion material ⇒ Proposal V12 |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10 y V11 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V10-8** | G3, ADR y proceso (C:V11-13..C:V11-15) | G3 ampliado; ADR-0036 alineado; desde V11, contradiccion material ⇒ Proposal V12 | §12; §13; §14.1; ADR-0036 |
...
- **AR10-02: el Coordinador separa tres decisiones.** La revision de V10 agrupaba el inventario de huellas de objetos
  posteriores, la visibilidad y el estado global, y dejaba declarar como limitacion la ocupacion que depende del
  entorno. El Coordinador fija la `ModelSpaceFootprintPolicy` por tipo exacto, la huella de orden latente e
  independiente de la visibilidad y la regla de entorno: lo que CT-49 no demuestre conservador bajo todos los estados
  admitidos es `UNKNOWN`, con la consecuencia visible en O-1 (L-34, L-37). El numero de E10 que eso produzca en dibujos
  reales lo mide G3 (R-47).
...
- **G-M24 (`[V11-D08]`).** Es una guarda estructural por productor, no un hash de archivos; su forma concreta se fija en
  G5..G7 sin tocar los drawers (§20.3).
...
- **Terminologia (`[V10-D19]`, `[V11-D19]`).** Huella de la referencia, huella visual, `FootprintInputFingerprint`,
  `TraceFingerprint`, `ResolvedPresentationEligibility`, `ModelSpaceFootprintPolicy` y `ModelSpaceOrderTailSnapshot` son
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V12 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V11 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V11 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V12 y O-1 se reevalua.
...
- **ADR-0041 aceptado en I-49 (`[V11-D11]`).** Reemplaza a ADR-0040 y conserva lo que I-52 consume (D19 `PlanReadSet` y
  el dominio del consumidor); V11 lo usa como autoridad vigente de I-49.
- **I-55 (`[V11-D10]`, `[V11-D12]`, R-45).** I-52 no espera al consenso de I-55, pero no congela sin la reconciliacion
  obligatoria de las seis autoridades compartidas; hoy no hay contradiccion material presente: X-1..X-6 de su
  Proposal V1 son compatibles con las responsabilidades de V11 y ADR-0042 declara conservada la semantica de Actualizar
  e Insertar. V11 registra el reparto como pendiente de esa reconciliacion y no lo decide.
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V12** o reduccion
  de alcance.
...
- **Reconciliacion entre iniciativas (`[V11-D10]`).** Sin la reconciliacion obligatoria I-52/I-55 (e I-49 donde aplique)
  ...
  C-2 y Actualizar y se reconcilia antes de congelar.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V12.
...
Caracterizaciones CT-01..CT-50; pruebas T-M01..T-M74 (con T-M17b); guardas G-M1..G-M24 (G-M24 estructural sobre los
productores de contexto); evidencia de viabilidad de la sonda en `acad.exe` y corpus indicativo con la huella de la
...
`CustomPropertiesEnvelopeGuardTests.cs` clasificado con motivo (7 → 8); guardas de I-51 reapuntadas en
...
G2 = V11 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd) y V10 (636f7fd) = historial; revision de V10: Architect: CHANGES REQUIRED — PROPOSAL V11
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V11, rebase sobre main, reconciliacion con I-55 e I-49, freeze, re-check
                     y O-1)
