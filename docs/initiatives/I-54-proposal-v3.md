# I-54 — Proposal V3: Custom Properties Foundation (ID24)

```text
Documento      = PROPOSAL V3 — NOT CONSENSUS
Gate           = G2E — reconciliacion de la Architect Review exact-SHA de V2
Executor       = PROPOSED V3
...
PROPOSAL_V2    = 36c337b84c47f9ac7c97d860fce42d3a5f4370e8   (historica; no se edita)
REVIEW_V1      = Architect Review — I-54 Proposal V1 @ 7c197af: AGREED WITH CHANGES (cambios V2-1..V2-22)
REVIEW_V2      = Architect Review — I-54 Proposal V2 @ 36c337b: AGREED WITH CHANGES
                 AR-54-04 REOPENED · 2 MATERIAL (AR-54-V2-01..02) · 4 MINOR (AR-54-V2-03..06) · cambios C-1..C-8
                 El Coordinador acepta C-1..C-8 como base de la reconciliacion
PROPOSAL_V3    = el commit que introduce este archivo (como obtenerlo: paquete §1)
Discovery      = I-54-discovery.md; addendum G2E §2.7 en el mismo commit (re-medicion de paralelas)
Paquete        = I-54-architect-review-package.md, reescrito para revisar ESTA version y solo C-1..C-8
```

> Esta V3 **no autoriza** nada y **sustituye a V2** como propuesta vigente. V1 y V2 quedan en el arbol como
> historia y no se modifican. V3 es V2 con **exactamente** los ocho cambios vinculantes de G2D (**C-1..C-8**); las
> secciones que ningun cambio toca son identicas a V2 y no se re-revisan. Cada punto modificado cita su cambio
> (**C-n**) y el hallazgo que cierra (**AR-54-V2-nn**). El consenso exige `Coordinator = AGREED` y
> `Architect = AGREED` sobre el **mismo** SHA de este documento, el ADR en estado `propuesto` y la aprobacion del
> Owner.
...
## 0. Que cambia respecto de V2

El delta V1 → V2 sigue en V2 §0. Este es el delta V2 → V3, y **no hay ningun otro**:

| Cambio | Tema | V2 | V3 | Cierra |
|---|---|---|---|---|
| **C-1** | `Kind` no vacio desconocido | Escribible si es comun a todos los miembros (PR-12) | **`UnknownKind`: solo lectura**. Application recibe `isKnownKind` inyectado desde el borde; PR-12 queda **rechazada** | AR-54-V2-01 |
| **C-2** | Cota de profundidad | Normativa, pero su valor fuera del ADR; «eliminar siempre se permite» | **16 = constante del formato 1.x**, en el ADR; subirla exige MAJOR; documento legible mas profundo = **`DepthLimitExceeded`: solo lectura**, sin ninguna escritura | AR-54-V2-02, AR-54-04 |
| **C-3** | UTF-16 mal formado | «El store nunca lanza», sin clases de excepcion; NFC y serializacion sin validar | Clases capturadas explicitas → `PresentButUnreadable`; `Name` y `Value` validados antes de NFC y de serializar; residual preexistente del sobre declarado | AR-54-V2-03 |
| **C-4** | Nombres JSON repetidos | Solo raiz y entrada | **Cualquier objeto a cualquier profundidad** → `PresentButUnreadable`, detectado antes de mapear; residual del sobre precisado | AR-54-V2-04 |
| **C-5** | Unificacion | Origen `Absent` sin definir; solo se revalida el origen; INV-05 fisica | Origen `Absent` = **documento vacio canonico**; se revalida la forma mostrada de **cada** miembro; atomicidad **logica** | AR-54-V2-05 |
| **C-6** | Id en mutaciones | «`Id` exacto» | «`Id` igual por valor; su representacion persistida se escribe en `D` minusculas» | AR-54-V2-06 |
| **C-7** | Alcance candidato del ADR | 15 puntos; valor de la cota excluido | 16 puntos alineados con G2D; entran la cota 16 (7A), la regla de `Kind` (7B) y la igualdad canonica completa (7C) | AR-54-V2-01, -02, -04 |
| **C-8** | Trazas y publicacion | Matrices de V2 | §0, §2, §12, §13, §14, §17, §18, §19, §21 y §22 actualizados; paquete limitado a C-1..C-8; paralelas re-medidas | — |

## 1. Vocabulario

| Termino | Significado en V3 |
|---|---|
...
| **UTF-16 bien formado** | Texto sin surrogates sueltos: cada surrogate alto va seguido de uno bajo (D-05.1, C-3) |

## 2. Estado verificado en G2E

```text
Fecha               = 2026-09-12, con git fetch --all --prune en el preflight y de nuevo antes de publicar
Rama I-54           = architecture/propiedades-personalizadas; al abrir, HEAD = upstream = 36c337b; 0/0; arbol limpio
...
origin/main         = 46fcac2 (= BASE_SHA)               f8deb67 «Merge I-50: cotas independientes por tipo de vista»
I-49                = 2783acc                            048a508 (Proposal V6; solo docs)
I-50                = c9a5f0a (cierre documental)        integrada en main por f8deb67; rama remota retirada
I-52                = 0fc7032                            0445718 (Proposal V2 y ADR-0036 corregido; solo docs)
I-53                = d7f17ad (Proposal V2, ADR-0037)    d7f17ad
```

Ningun avance invalida C-1..C-8 ni una costura de I-54 (§13; Discovery §2.7). Consecuencias que se mantienen
de V2: la guarda T-GRD-01 se limita a `src/`; el espejo de I-52 cumple D-21 y añadira una llamada a `Compose`
(T-GRD-02: 7 → 8); el censo de comandos cambiara con I-52 (33 → 34). Nueva: ADR-0035 (I-50) esta **aceptado en
`main`**, y 0036 (I-52) y 0037 (I-53) estan tomados en sus ramas.

**Sin rebase en G2E.** WORKFLOW §4.2 exige rebasar **al abrir** una sesion si el trunk avanzo. Al abrir G2E,
`origin/main` seguia en `46fcac2`; el merge de I-50 se publico durante la sesion. V3 se publica sobre `BASE_SHA`
**sin force**, y toda la evidencia sigue citada sobre `BASE_SHA`. Medido sobre `f8deb67`:
- ningun archivo de codigo del mapa de I-54 (§11) cambia entre `46fcac2` y `f8deb67`, ni ningun archivo de codigo
  citado por esta V3, salvo `DimensionViewsRestampTests.cs`, que llega con I-50 y V3 ya citaba desde su rama;
- de los documentos citados o del mapa cambian `HANDOFF.md` (la cita `:1133-1136` de D-22.8 es `:1199-1202` en
  `main`, con el mismo texto), `ROADMAP.md`, `ideas-futuras.md` y `validacion-manual-autocad.md`, citados sin
  linea, y `adr/README.md`, cuyas lineas `:19-26` no cambian;
- `src/` mantiene un solo `new RackEmbedDocument` (`RackEmbedComposer.cs:24`), 7 llamadas a `Compose` y 33
  `[CommandMethod(`;
- `git merge-tree` entre esta rama y `origin/main` no da conflictos.

El rebase pendiente lo exige WORKFLOW §4.2 al abrir la proxima sesion que escriba en la rama. Como reescribe los
SHAs que citan las revisiones exact-SHA (V1, V2 y esta V3), su momento exacto —G2-FREEZE o una V4, y en todo caso
antes de G3— lo fija la orden del Coordinador. Ninguna decision de V3 depende de ese orden.
...
1. **Nombre al escribir**, en este orden:
   1. **validar que es UTF-16 bien formado** (C-3); si no, el intent es invalido y el preflight lo rechaza con un
      resultado tipado. `Normalize(FormC)` lanza `ArgumentException` con un surrogate suelto [X P-32], asi que
      **nunca** recibe texto mal formado;
   2. normalizar a Unicode **NFC** (`NormalizationForm.FormC`);
   3. recortar;
   4. validar 1..80 caracteres UTF-16, sin caracteres de control (U+0000–U+001F, U+007F);
   5. comprobar unicidad frente a las **otras** entradas, comparando con `OrdinalIgnoreCase` sobre la forma NFC
      de ambos lados.
...
2. **Nombre al leer**: debe ser string; vacio tras recortar → `PresentButUnreadable`. Los nombres que no se
   ...
   diagnostico. Un nombre leido ya es UTF-16 bien formado, porque el store clasifica como `PresentButUnreadable`
   todo string que no se pueda decodificar (D-07.4), asi que NFC tampoco recibe texto mal formado al leer.
...
4. **Orden de clasificacion, determinista y primero la version**:
   ...
   3. `Entries` ausente, nulo o no array; entrada que no es objeto; `Id` sin la forma de D-02 o `Guid.Empty`;
      `Name` o `Value` que no son string; o `Name` vacio → `PresentButUnreadable`;
   ...
   **Ningun contenido externo produce una excepcion** (V2-5.G, C-3). El store **no** usa `catch (Exception)`:
   ...
   operacion que la produce, y las convierte en `PresentButUnreadable`:
   ...
   la igualdad canonica (D-09.7) pueden lanzar por contenido.
...
4. **Contrato de aislamiento. Afirmacion permitida:**
   ...
   **Residuales declarados.** Ninguno lo puede recuperar el store de propiedades, porque ocurren en
   ...
   - **UTF-16 crudo invalido en el texto de un sobre** (residual **preexistente**, no introducido por I-54): el
     `RackEmbedStore.Deserialize` de BASE solo captura `JsonException` (`RackEmbedDocument.cs:98`) y lanza
     `ArgumentException` [X P-31]. `ScanEnvelopes` lanza con el, asi que los flujos de Rack de I-54 —como los
     demas comandos que barren— fallan cerrados, sin escribir, pero sin el diagnostico por definicion de
     D-09.3.
     - **No se arregla en I-54.**
     - Es el hallazgo lateral **F-14**, que se registra en `ideas-futuras.md` en G2-FREEZE (§14).
...
5. **Se revalida sobre la lectura fresca**, reevaluando el orden unico de D-09.8, y se aborta sin escribir si
   ...
   - **nombre y limites**: D-05 sobre lo que la operacion introduce, con UTF-16 bien formado (C-3);
...
| INV-23 | Ningun contenido externo hace lanzar al store de propiedades, y ningun `Name` o `Value` que no sea UTF-16 bien formado llega a normalizarse o a persistirse (C-3) | **nuevo en V3** |
...
| Riesgo del Discovery | Mitigacion en V3 |
|---|---|
...
| RP-11 | UTF-16 crudo invalido en el texto de un sobre hace lanzar el barrido y los flujos de Rack fallan cerrados sin diagnostico | Residual **preexistente** del `RackEmbedStore` de BASE (D-08.4); no se arregla en I-54; F-14 para G2-FREEZE |
...
**kind desconocido, documentos de profundidad > 16, UTF-16 mal formado, nombres repetidos** ni payloads no
colocados ilegibles (regla de I-47 y V5-R07 de I-48). Los cubren T-STO, T-AUT y T-MUT.
...
## 13. Ramas paralelas (medidas en G2E)

Solo se mide el impacto sobre C-1..C-8 y sobre las costuras de I-54; los conflictos textuales no reabren
arquitectura. La medicion completa anterior esta en V2 §13.

| Rama | SHA al publicar V3 (completo) | Al cerrar G2D | Diff desde G2D | Impacto en C-1..C-8 y en costuras de I-54 | Conflictos | Supuestos invalidados |
|---|---|---|---|---|---|---|
| `main` | `f8deb675c6d1ef0e64693b157d69c4cc170d7b24` | `46fcac2` | Integracion de I-50 (`c9a5f0a` y merge `f8deb67`): 52 archivos de `src/`, `tests/` y `assets/`, mas `HANDOFF.md`, `ROADMAP.md`, ADR-0035 `aceptado`, `adr/README.md`, `ideas-futuras.md`, `guias/validacion-manual-autocad.md` y los documentos de I-50 | Ninguno. **Ningun** archivo de codigo del mapa de I-54 ni de los citados por V3 cambia, salvo la prueba `DimensionViewsRestampTests.cs` de I-50 (detalle de documentos en §2); `src/` sigue con 1 `new RackEmbedDocument` (`RackEmbedComposer.cs:24`), 7 llamadas a `Compose` y 33 `[CommandMethod(`, asi que T-GRD-01, T-GRD-02 y los censos de D-18.7 no cambian. La frase de ADR-0035 sobre `RackEmbedComposer` sigue en `:97-102` (§15) | `git merge-tree` con esta rama: **sin conflictos**. Documental al integrar: ROADMAP, `adr/README.md`, `ideas-futuras.md` (F-14) y `validacion-manual-autocad.md` | Ninguno. Rebase pendiente segun §2 |
| I-49 `architecture/motor-expresiones-parametricas` | `048a508e570e210467c3693a11ac4a487f383944` | `2783acc` | +1 commit **solo docs**: `I-49-proposal-v6.md` | Sigue reservando `Rack` y `Project` como nombres reservados y la sintaxis de namespace para ID20 (`I-49-proposal-v6.md:622-624,2930`); censos de 33 comandos y 29 ventanas intactos (`:1914,2082,2311`); su fila de I-54 mantiene el punto de extension solo como restriccion (`:3157`). D-16 y §15.15 sin cambio | Documental: fila de ROADMAP y numeracion de ADR | Ninguno |
| I-50 `feature/cotas-independientes-por-vista` | rama remota retirada tras integrar | `6cd2970` | Cierre documental `c9a5f0a` e integracion en `main` (fila anterior) | Ver `main` | Ver `main` | Ninguno |
| I-52 `feature/rackmirror-espejo-semantico` | `04457183fc2d7dcda5d6c1b988a4789ed4ea2f8f` | `0fc7032` | +1 commit **solo docs**: `I-52-proposal-v2.md`, ADR-0036 corregido (`propuesto`) y `automation/decisions/I-52.md` | D-21 compatible: reflejar → `Compose(sobreFuente, …)` → `RestampEnvelope`, sin modificar `RackEnvelopeRestamp.cs` (`I-52-proposal-v2.md:458-459`); su ADR-0036 corregido mantiene intactos los portadores de iniciativas integradas, propiedades de rack incluidas (`0036-…md:88-90`), y cita D-21 (`:162`); `RACKMIRROR` sin alias, censo 33 → 34 (`:96`); T-M20 (`:1243`) | Textual: censos de comandos y de `Compose`; ADR-0036 en su rama | Ninguno |
| I-53 `feature/cabeceras-configurables-multidestino` | `d7f17addb47ea943b61ff50b4b4a5b23010d6fab` | `f38362d` | +1 commit **solo docs**: `I-53-proposal-v2.md`, `docs/adr/0037-reutilizacion-de-cabecera-por-copia-y-distribucion-por-lotes.md` (`propuesto`), `docs/adr/README.md`, `automation/decisions/I-53.md` y su contrato | Ninguno. Sin datos persistidos; su UI va en las ventanas de editores y solo actualiza censos de `x:Name`, no de comandos ni de ventanas (`I-53-proposal-v2.md:883-884,917,921`); declara «sin archivos de I-53» en I-54 (`:896`) | Documental: ADR-0037 en su rama | Ninguno |

Medido con `git fetch --all --prune` en el preflight de G2E y de nuevo inmediatamente antes de publicar V3.
Entre ambas mediciones avanzaron `main` (integracion de I-50), I-49 e I-52; la tabla da el estado al publicar y
el addendum §2.7 del Discovery conserva las dos mediciones.
...
| G2E | Esta Proposal V3 | commit documental y CI del SHA exacto |
| G2F | Revision exact-SHA de V3 por Coordinador y Arquitecto, **limitada a C-1..C-8**, AR-54-04, AR-54-V2-01..06, PR-03/06/07/12/13, las RD afectadas y el alcance del ADR modificado | veredictos sobre el mismo SHA |
| G2-FREEZE | Consenso sobre el mismo SHA; contrato actualizado; **ADR `propuesto`** con el alcance de §15; F-01..F-13, **F-14** (residual de UTF-16 del `RackEmbedStore`, D-08.4) y la mejora de D-11.6 en `ideas-futuras.md`; aprobacion del Owner, informado de R-12 | commit documental y CI |
...
Ninguna bloquea el consenso. **Sin cambios en V3**, salvo una aclaracion en OQ-02 que no cambia su clasificacion.
...
### 17.2 Cambios de G2B: V2-1..V2-22 (heredados de V2, con su verificacion en G2D)
...
### 18.1 Hallazgos de la revision de G2D sobre V2

| Hallazgo | Severidad | Afecta | Disposicion V3 | Cambio | Estado |
|---|---|---|---|---|---|
| **AR-54-V2-01** `Kind` desconocido escribible | MATERIAL | D-09.5, D-09.8, D-18.5, T-AUT-10, §15, PR-12 | `UnknownKind` de solo lectura con `isKnownKind` inyectado; PR-12 rechazada | C-1 | **CLOSED IN V3** (pendiente de re-verificacion) |
| **AR-54-V2-02** Cota normativa fuera del ADR y contradicciones de edicion | MATERIAL | D-04.3, D-05.6, D-05.7, D-08.4 B, D-13, D-18, §9, §15, INV-20, T-STO-09, U-02 | Constante 16 del formato 1.x en el ADR; `DepthLimitExceeded` de solo lectura; eliminar limitado a conteo y longitud | C-2 | **CLOSED IN V3** (pendiente de re-verificacion) |
| **AR-54-V2-03** UTF-16 mal formado y excepciones | MINOR | D-05.1, D-05.4, D-07.4, D-08.4 A y G, T-STO-08, -16, -18, -20 | Clases capturadas explicitas; validacion antes de NFC y de serializar; residual preexistente declarado (F-14) | C-3 | **CLOSED IN V3** (pendiente de re-verificacion) |
| **AR-54-V2-04** Nombres repetidos anidados | MINOR | D-01.5, D-07.4, D-09.7, D-08.4, T-STO-12, T-AUT-04 | Cualquier objeto a cualquier profundidad → `PresentButUnreadable`; residuales precisados | C-4 | **CLOSED IN V3** (pendiente de re-verificacion) |
| **AR-54-V2-05** Huecos de la unificacion | MINOR | D-09.10, D-22.5, INV-05, T-MUT-07, T-MUT-10 | Origen `Absent` canonico; revalidacion de todos los miembros; atomicidad logica | C-5 | **CLOSED IN V3** (pendiente de re-verificacion) |
| **AR-54-V2-06** Id textual frente a igualdad por valor | MINOR | D-10.1, T-MUT-02 | `Id` igual por valor, persistido en `D` minusculas | C-6 | **CLOSED IN V3** (pendiente de re-verificacion) |

### 18.2 Hallazgos de la revision de G2B sobre V1
...
| AR-54-04 Sobreafirmacion del aislamiento | MATERIAL | RD-07, RD-17 | Afirmacion acotada y A..G; en V3, la cota 16 como constante del formato y `DepthLimitExceeded` | D-05.7, D-08.4, INV-07, INV-20, §15.5 | **REOPENED IN G2D → CLOSED IN V3** (pendiente de re-verificacion) |
...
Los hallazgos MINOR AR-54-06..18 no se reabrieron en G2D y siguen cerrados sin cambios en V3. «CLOSED IN V3» es
la disposicion del ejecutor: la **re-verificacion** corresponde a la revision exact-SHA de G2F (paquete §6).

## 19. RD-01..RD-20 en V3

| RD | G2D (Arquitecto, sobre V2) | V3 | Que cambio en V3 | Seccion |
|---|---|---|---|---|
| RD-01 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Filas MAJOR nuevas en la regla de evolucion: cota de profundidad y dependencia del `Kind` (C-1, C-2) | D-04.3 |
| RD-02 | AGREE | **ACCEPTED** | Sin cambios | D-01, D-08.2 |
| RD-03 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | `Id` igual por valor en las mutaciones, persistido en `D` minusculas (C-6) | D-10.1, T-MUT-02 |
| RD-04 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Eliminar permitido solo frente a conteo y longitud (C-2); UTF-16 validado antes de NFC y de serializar (C-3) | D-05.1, D-05.4, D-05.6 |
| RD-05 | AGREE | **ACCEPTED** | Sin cambios | D-03 |
| RD-06 | AGREE | **ACCEPTED** | Sin cambios de decision; D-07.4 recibe las clases de excepcion de C-3 | D-07 |
| RD-07 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Cota 16 como constante del formato con `DepthLimitExceeded` (C-2); clases de excepcion y residual preexistente (C-3); nombres repetidos a cualquier profundidad (C-4) | D-05.7, D-07.4, D-08.4 |
| RD-08 | AGREE | **ACCEPTED** | Sin cambios | D-08.5 |
| RD-09 | AGREE | **ACCEPTED** | Sin cambios | D-08.3, D-21 |
| RD-10 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | `UnknownKind` de solo lectura (C-1); igualdad sobre documentos sin duplicados y sobre todo el contenido salvo el minor (C-4, C-7C); orden unico de resultados | D-09.5..D-09.8 |
| RD-11 | AGREE | **ACCEPTED** | Sin cambios | D-09.3, D-09.4 |
| RD-12 | AGREE | **ACCEPTED** | Sin cambios de decision; D-22 recibe `isKnownKind` y la revalidacion de C-1, C-2, C-3 y C-5 | D-22 |
| RD-13 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Origen `Absent` canonico; revalidacion de todos los miembros mostrados; atomicidad logica (C-5) | D-09.10, D-22.5, INV-05 |
| RD-14 | AGREE | **ACCEPTED** | Sin cambios | D-10 |
| RD-15 | AGREE | **ACCEPTED** | Sin cambios | D-11 |
| RD-16 | AGREE | **ACCEPTED** | Sin cambios | D-12 |
| RD-17 | AGREE | **ACCEPTED** | Sin cambios; el residual preexistente de UTF-16 queda declarado en D-08.4 (C-3) | D-13, INV-07 |
| RD-18 | AGREE | **ACCEPTED** | Sin cambios | D-15..D-17 |
| RD-19 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Solo lectura por `UnknownKind` y `DepthLimitExceeded` desde la lectura (C-1, C-2); origen «vacio / sin propiedades» (C-5A) | D-18.3..D-18.5, U-02, U-04 |
| RD-20 | AGREE WITH CHANGE | **ACCEPTED WITH RECONCILIATION** | Alcance del ADR en 16 puntos con 7A, 7B y 7C (C-7); gates G2E y G2F; F-14 en el freeze | D-20, §14, §15 |

**DEFERRED a nivel de RD: ninguna. OPEN DISAGREEMENT: ninguno.**

## 20. Cambios que V3 no hace

- No reescribe V1 ni V2, y no reabre decisiones acordadas en G2D que C-1..C-8 no tocan.
- No redacta ni numera el ADR (§15).
- No toca produccion, pruebas productivas, `docs/HANDOFF.md`, `docs/ROADMAP.md` ni `docs/ideas-futuras.md` (F-14
  se registra en G2-FREEZE).
- No reescribe el Discovery: solo añade el addendum factual §2.7.
- No amplia I-54 para arreglar el `RackEmbedStore` de BASE (C-3C).

## 21. Precisiones del ejecutor: estado final

**OPEN DISAGREEMENT = NINGUNO.** Estado de cada precision tras G2D y C-1..C-8:

| # | Precision | Estado V3 | Nota |
|---|---|---|---|
| PR-01 | En la igualdad canonica, `SchemaVersion` participa **solo por su major**; el minor no participa | **AGREE** | Acordada en G2D; ahora forma parte de [7C] |
| PR-02 | `MixedKind` para `Kind` en blanco, con un mensaje que remite a `RACKEDITAR` **sin prometer reparacion** | **AGREE** | Acordada en G2D |
| PR-03 | Cota de profundidad = 16 | **RECONCILED** | C-2: constante del formato 1.x en el ADR, subirla exige MAJOR; la lectura tolera, pero un documento mas profundo es `DepthLimitExceeded`, de solo lectura, y no admite ninguna escritura |
| PR-04 | Los limites de escritura validan lo que la operacion introduce | **AGREE** | Acordada en G2D; C-2 solo precisa que eliminar no se permite sobre estados de solo lectura |
| PR-05 | Las definiciones `IsDependent` no son miembros, no bloquean y nunca se escriben | **AGREE** | Acordada en G2D; OV-14 |
| PR-06 | Nombres de miembro repetidos → `PresentButUnreadable`; residual del sobre | **RECONCILED** | C-4: cualquier objeto y cualquier profundidad, antes de mapear; residuales por clave exacta y por capitalizacion |
| PR-07 | Unificar escribe solo los miembros canonicamente distintos del origen | **RECONCILED** | C-5C: atomicidad del **estado logico**; C-5A: origen `Absent` = vacio canonico |
...
| PR-12 | Un `Kind` no vacio, desconocido pero comun, no bloquea | **REJECTED / RECONCILED** | C-1: un kind desconocido comun es **`UnknownKind`, de solo lectura**. La precision de V2 no queda como decision valida |
| PR-13 | Unificar revalida la coleccion origen fresca | **RECONCILED** | C-5B: se revalida la forma mostrada de **todos** los miembros y el conjunto, no solo el origen |

## 22. Estado de V3

Siguiente gate: **G2F**, revision exact-SHA de esta V3 por Coordinador y Arquitecto, limitada a C-1..C-8,
AR-54-04, AR-54-V2-01..06, PR-03/06/07/12/13, las RD afectadas y el alcance del ADR modificado. Solo si converge
se abre G2-FREEZE. Nada de esta V3 autoriza G3, el ADR definitivo, codigo ni pruebas.
