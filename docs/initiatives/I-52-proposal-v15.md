# I-52 — Proposal V15: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V15 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V15 se redacta SIN rebase, sobre e59bc89)
...
> Proposal V1..V13  = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd ·
>                     636f7fd · 2275f21 · 915a520 · dd45b0f (historial)
> Proposal V14      = e59bc89335c776a24cd48b30f54547696f820dfa   (historial; CI 34883383475 4/4)
> Revision Arq. V14 = «Architect: CHANGES REQUIRED — PROPOSAL V15»; sin BLOCKER; HIGH AR14-01 (sesiones de edicion sin
>                     inventario cerrado ni fallo cerrado por defecto: ARRAYEDIT Source y ventana de prueba de
>                     bloques); MEDIUM AR14-02 (G-M24 sin la familia de escritura sobre objetos o contexto existentes)
>                     y AR14-03 (precedencia causal de los campos); LOW L1..L6; cambios exactos V15-01..V15-07 de la
>                     revision (registro: docs/automation/decisions/I-52.md §86)
> Coordinador       = orden de V15 con V15-01..V15-13 vinculantes (registro §87)
> main y paralelas  = §15.1 (preflight de V15 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V15 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V15 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V15 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V14 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V14, sus revisiones de Arquitecto y esta V15 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V15 la registra
...
> prueba de bloques y midio en main los sitios de apertura para escritura del Plugin. CT-49 y CT-50 deben confirmarlo en
> AutoCAD 2025. Ninguna revision ejecuto AutoCAD.

---

## 0. Reconciliacion V14 → V15

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`), V11 (`2275f21`), V12 (`915a520`), V13 (`dd45b0f`), V14 (`e59bc89`) y el
Discovery (`339b3ab`) **se conservan sin cambios**. **Donde V14 y V15 difieran, manda V15.** Las marcas
`[V14-D01]`..`[V14-D15]` siguen vigentes salvo donde una marca `[V15-Dnn]` las modifica, reabre o sustituye (§0.4); las
marcas `[V13-Dnn]`, `[V12-Dnn]`, `[V11-Dnn]`, `[V10-Dnn]`, `[V9-Dnn]`, `[V8-Dnn]`, `[V7-Dnn]`, `[V6-Dnn]` y `[V5-Dnn]`
siguen como las dejaron V14, V13, V12, V11, V10, V9, V8, V7 y V6 (§0.5 a §0.13).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V15** | V15 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V15. **Debe ser NONE** |

### 0.2 Matriz `[V15-D01]`..`[V15-D15]`

En la columna «Origen»: `AR14-nn` y `L1`..`L6` son los hallazgos de la revision de V14; `R:V15-nn`, su cambio exacto;
`C:V15-nn`, la precision vinculante del Coordinador en la orden de este gate. Las dos series V15-nn se numeran por
separado. **Donde el cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision
vinculante del Coordinador**: la revision pedia un censo cerrado de sesiones con fallo cerrado por defecto (R:V15-01);
el Coordinador lo convierte en una politica con dos resultados (`CHARACTERIZED_SAFE` o `BLOCKED`) y fija el primer corte
de ARRAYEDIT Source y de la ventana de prueba de bloques (C:V15-01, C:V15-02); separa la identidad de contenido de los
XREF (C:V15-05) de la precedencia de los campos (C:V15-04), que declara EDIT CAUSE > FIELD REEVALUATION; exige tres
violaciones RED en T-M75 (C:V15-03); y fija la salida de R-55 ante la cancelacion de I-49 como decision futura del
Coordinador, sin desbloqueo automatico (C:V15-07). **Precisiones del ejecutor:** (1) la Amendment A3-R1 de I-49
(`c8cfee2`), publicada antes de este gate, clasifica ADR-0041 D19 como «leida con A3» (su texto aceptado sigue siendo
cierto y el resultado comparable del `PlanReadSet` se completa con M3, M4 y M10(a)) y declara que cambian directamente
D20, D18 puntos 1-4 y D14 decision 7; V15 conserva la formula de la orden (A3 y A3-R1 cambian la semantica futura de las
observaciones del `PlanReadSet` de D19 y D20) con esa clasificacion exacta y sin afirmar que el texto de D19 se
enmienda; (2) los prerrequisitos globales, incluida la politica de sesiones, se re-verifican **siempre** en R6 y al
inicio de MUTATE, tambien si CT-49 demuestra que un estado no puede cambiar durante el comando; (3) en el primer corte
todo el inventario minimo activo es `BLOCKED`, y reclasificar uno de ellos como `CHARACTERIZED_SAFE` exige Proposal V16;
(4) la identidad de contenido de un XREF solo se necesita en ejecucion cuando el contenido cargado puede no corresponder
a su resolucion actual y, entonces, sin abrir archivos externos, identidad no demostrable ⇒ `UNKNOWN`; (5) el censo
inicial de escrituras de CT-50 se mide en `main @ dad4e77`.

| Marca | Origen | V14 | Correccion V15 | Estado | Donde |
|---|---|---|---|---|---|
| `[V15-D01]` | AR14-01 · R:V15-01 · C:V15-01 | ST-20(G) nombraba REFEDIT o transaccion larga, Editor de bloques y apertura parcial, y dejaba el resto en «o equivalente» o «cualquier otro prerrequisito global que caracterice CT-49», sin fallo cerrado por defecto para un estado de sesion no caracterizado | **CLOSED EDITING SESSION POLICY** (`EditingSessionPolicy`, nombre conceptual): toda sesion de edicion en sitio o alternativa relevante es `CHARACTERIZED_SAFE` o `BLOCKED`; **estado de edicion o de sesion no caracterizado o no demostrablemente ausente ⇒ E12**; inventario minimo: REFEDIT (`REFEDITNAME`), transaccion larga (`LongTransactionManager`), Editor de bloques (`BLOCKEDITOR`), ventana de prueba de bloques (`BLOCKTESTWINDOW`, BTESTBLOCK o equivalente), estado de edicion de la fuente de una matriz asociativa (ARRAYEDIT Source, que se cierra con ARRAYCLOSE) y todo otro modo de edicion en sitio que descubra CT-49, que queda `BLOCKED` salvo `CHARACTERIZED_SAFE` demostrado; P0 comprueba la politica de forma global y R6 y MUTATE la re-verifican; RACKMIRROR no ejecuta REFCLOSE, BCLOSE ni ARRAYCLOSE y no muta el estado de ninguna sesion | CLOSED BY V15 (contrato); deteccion fiable en AutoCAD 2025: DEFERRED TO G3 WITH FAIL-CLOSED | §1.3 L-34; §2.3; §4.1 ST-20; §7.9 PRE-19; §9.1; §9.4 E12; §11.3 punto 5; §12 CT-49, T-M73; §13 M-35; ADR-0036 decisiones 11 y 13 |
| `[V15-D02]` | AR14-01 · C:V15-02 | ARRAYEDIT Source y la ventana de prueba de bloques no estaban nombrados: en ARRAYEDIT Source los objetos fuente se editan en Model Space y, segun el comportamiento documentado de AutoCAD (a confirmar por CT-49), los objetos nuevos se aplican a la fuente de la matriz; ST-19 razonaria sobre geometria temporal y las copias podrian confirmarse en otra estructura estable | **ARRAYEDIT / BLOCK TEST FIRST CUT**: ARRAYEDIT Source activo ⇒ E12; `BLOCKTESTWINDOW` activo ⇒ E12; RACKMIRROR no intenta razonar sobre la geometria fuente temporal ni sobre su replicacion; CT-49 caracteriza la deteccion fiable de ambos estados en AutoCAD 2025; G3 con los casos activo y cerrado | CLOSED BY V15 (contrato); deteccion: DEFERRED TO G3 WITH FAIL-CLOSED | §1.3 L-34; §2.3; §4.1 ST-20; §9.4 E12; §11.3 punto 5; §12 CT-49; §13 M-35 |
| `[V15-D03]` | AR14-02 · R:V15-02 · C:V15-03 | El lado B de G-M24 censaba construccion y alta, importacion y clonado, altas en tablas de simbolos, lecturas de cabecera, evaluacion grafica, atributos, rutas de comando o COM y helpers, pero no las rutas de escritura que mutan objetos o contexto existentes sin construir ni añadir | **G-M24 EXISTING-OBJECT WRITE FAMILY**: familia nueva «escritura o mutacion de objeto o contexto existente» con **apertura para escritura** (`Transaction.GetObject(..., OpenMode.ForWrite)`, apertura o `GetObject` de un `ObjectId` para escritura, `UpgradeOpen`, `OpenCloseTransaction` y envoltorios equivalentes), **mutacion de presentacion de entidades** (`Layer`/`LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `Visible`, `PlotStyleName`, `SetPropertiesFrom` y `TransformBy` donde sea material), **mutacion de registros de simbolo existentes** (setters de `LayerTableRecord`, `LinetypeTableRecord`, `TextStyleTableRecord` y `DimStyleTableRecord` existentes, `Database.SetDimstyleData` y mutaciones equivalentes) y **escritura de estado global** (`SetSystemVariable` y setters de `Clayer`, `Cecolor`, `Celtype`, `Textstyle`, `Dimstyle` y equivalentes de `Database`); se conservan todas las familias de V14; cada sitio relevante `DECLARED` o `EXCLUDED` con motivo; **T-M75 con tres violaciones RED** (productor por clonado; lectura de cabecera o de contexto de `Database`; mutacion de presentacion de un objeto existente); CT-50 registra en su censo inicial los sitios de escritura actuales | CLOSED BY V15 (contrato); implementacion: G5..G7 | §0.3; §8.7; §12 CT-50, T-M75, G-M24; §17 R-53; ADR-0036 decision 7 |
| `[V15-D04]` | AR14-03 · R:V15-03 · C:V15-04 | La evaluacion de campos era ESTADO con una cota para «todos los resultados que admite el contrato», sin decir si incluian los resultados que solo se alcanzan editando un objeto referenciado; con la precedencia explicita de `DataLink` cabian dos implementaciones conformes | **FIELD CAUSAL PRECEDENCE**: la evaluacion de un campo **no** decide por si sola ESTADO o EDICION; se clasifica por la **causa** del cambio del resultado: ESTADO si solo cambia un estado admitido (visibilidad, carga o resolucion, variable del sistema, fecha u hora, nombre de archivo, estado del conjunto de planos, guardado o trazado, refresco de `DataLink` u otro estado admitido explicitamente); EDICION si se edito el contenido logico referenciado (geometria o propiedad del objeto referenciado, edicion manual de texto o de contenido, otra edicion futura); **EDIT CAUSE > FIELD REEVALUATION**; «todos los resultados que admite el contrato» = los alcanzables **solo** por cambios de ESTADO; sin cota conservadora sobre ellos ⇒ `UNKNOWN`; `DataLink` sigue siendo la decision especial explicita (datos externos cambiados + refresco = ESTADO) | CLOSED BY V15 | §1.3 L-37; §11.3 punto 5; §12 CT-49; ADR-0036 decision 11 |
| `[V15-D05]` | L4 · R:V15-06 · C:V15-05 | «Mismo contenido referenciado» de un XREF sin definir: una ruta distinta con el mismo contenido podia leerse como «otro archivo» (edicion) o como «mismo contenido» (estado) | **XREF SAME CONTENT IDENTITY**: «mismo contenido referenciado» = misma identidad del contenido referenciado **efectivo**, no la misma cadena de ruta; los cambios de ruta, de rutas de busqueda o de `PROJECTNAME` se clasifican por el contenido resultante: mismo contenido efectivo ⇒ ESTADO; contenido efectivo distinto ⇒ EDICION futura; identidad no demostrable cuando RACKMIRROR la necesita ⇒ `UNKNOWN` o fail-closed, sin abrir archivos externos | CLOSED BY V15 | §1.3 L-37; §11.3 punto 5; §12 CT-49; ADR-0036 decision 11 |
| `[V15-D06]` | L1, L2 · R:V15-04 · C:V15-06 | §15.2, §15.3, PV-6 y las referencias del ADR decian «D19 identico» o «sin cambio» y describian el «hoy» de I-49 sin A3; I-49 publico A3-R1 (`c8cfee2`) antes de este gate | **I-49 CURRENT AUTHORITY WORDING**: solo como hecho historico, D19 de ADR-0040 = D19 de ADR-0041 antes de A3; ADR-0041 D19 **no** se presenta como autoridad vigente de RS-3; A3, revisado por A3-R1, cambia la semantica futura de las observaciones del `PlanReadSet` de D19 (leida con M3, M4 y M10(a)) y D20; RS-3 = PENDING I-49 FINAL AUTHORITY; antes del freeze de I-52 se relee la autoridad final acordada, aceptada y congelada de I-49 y, si no esta disponible, I-52 FREEZE = BLOCKED; el «hoy» de PV-6 incluye A3-R1 (revision exacta previa de A3 = CHANGES REQUIRED; A3-R1 pendiente de re-revision exacta; ADR de reemplazo y freeze nuevo pendientes; G7 y G8 BLOCKED) | CLOSED BY V15 (redaccion) | §0.3; §9.2.1 RS-3; §10.2 PV-6; §15.1; §15.2; §15.3; ADR-0036 referencias |
| `[V15-D07]` | L6 · R:V15-04 · C:V15-07 | R-55 no tenia salida si I-49 se cancela o abandona antes de que exista su autoridad final | **I-49 CANCELLATION EXIT**: si I-49 se cancela o abandona antes de su autoridad final, **no** hay desbloqueo automatico ni eliminacion silenciosa de RS-3; I-52 necesita una decision futura del Coordinador en una Proposal: (A) retirar del alcance de I-52 el soporte futuro de expresiones de I-49; (B) adoptar otra autoridad integrada; o (C) mantener el freeze bloqueado; hasta esa decision, FREEZE BLOCKED | CLOSED BY V15 (proceso) | §9.2.1 RS-3; §14.1; §17 R-55; §19 |
| `[V15-D08]` | L3 · R:V15-05 · C:V15-08 | Los marcadores provisionales decian «X-n, AGREED en `d091eeb`», que puede leerse como consenso formal | **I-55 MARKER WORDING**: «X-n acordada en la revision tecnica adversarial de G2C @ `d091eeb`, reclasificada despues por la Proposal V3 de I-55 como no formal; las diferencias de V3 siguen pendientes de revision formal y reconciliacion»; `d091eeb` **no** es consenso formal del Arquitecto; las diferencias de V3 siguen sin adoptar | CLOSED BY V15 (redaccion) | §3.3; §3.7; §3.8; §5.1; §5.4; §6.1; §8.1..§8.4; §15.4 |
| `[V15-D09]` | C:V15-09 | I-56 registrado en `0d66df2` (G1.1) | **I-56 CURRENT**: ultimo observado `18401dafad2dbe93d356051be1f7061bd214b486` (G2: Proposal V1 PUBLISHED, NOT CONSENSUS; Coordinator y Architect REVIEW REQUIRED; Owner NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE; `WORKFLOW_V2_EFFECTIVE_SHA` no existe; I-49, I-52 e I-55 grandfathered); ninguna regla de la Proposal V1 se aplica a I-52; solo se actualizan los hechos | CLOSED BY V15 (registro) | §0.3; §15.1; §15.2; ADR-0036 referencias |
| `[V15-D10]` | L5 · R:V15-06 · C:V15-10 | Casos de G3 de V14 | Casos obligatorios nuevos: **sesiones de edicion** (ARRAYEDIT Source activo y cerrado; `BLOCKTESTWINDOW` activo y cerrado; estado de sesion desconocido ⇒ E12; se mantienen los de REFEDIT y del Editor de bloques); **campos** (campo de propiedad de objeto, `AcObjProp` o equivalente, que referencia geometria; objeto referenciado cambiado por EDICION; reevaluacion solo por ESTADO; `DataLink` sigue siendo estado); **dinamicos** (tabla de busqueda solo de visibilidad; tabla de busqueda de visibilidad y geometria o estiramiento); **XREF** (ruta distinta con el mismo contenido efectivo; ruta distinta con contenido efectivo distinto); **G-M24** (productor por clonado ⇒ RED; lectura de cabecera de `Database` ⇒ RED; apertura para escritura + mutacion de presentacion ⇒ RED); G3 sigue NOT OPEN; contradiccion material ⇒ Proposal V16 | CLOSED BY V15 (plan); ejecucion: G3 y G5..G7 | §12 CT-49, CT-50, T-M73, T-M75; §14.2 G3 |
| `[V15-D11]` | C:V15-11 | L-34, L-37 y M-35 sin ARRAYEDIT, ventana de prueba de bloques ni estado de sesion desconocido; L-37 sin precedencia causal de campos ni identidad de contenido de XREF | **O-1 PRACTICAL LIMITATIONS**: L-34 y M-35 exponen que REFEDIT, el Editor de bloques, ARRAYEDIT Source y la ventana de prueba de bloques bloquean RACKMIRROR, que un estado de sesion desconocido lo bloquea, el riesgo de la carga bajo demanda y que un objeto `UNKNOWN` lejano lo hace fallar; L-37 expone la precedencia causal de los campos, ERASE/OOPS fuera de la garantia y el XREF con el mismo contenido efectivo (estado) frente al de contenido distinto (edicion). O-1 no se pide | CLOSED BY V15 | §1.3 L-34 y L-37; §13 M-35; §17 R-52 |
| `[V15-D12]` | R:V15-07 · C:V15-12 | ADR-0036 alineado con V14 | ADR-0036 sigue **PROPOSED**; decision 7 con la familia de escritura sobre objetos o contexto existentes; decisiones 11 y 13 con la politica cerrada de sesiones de edicion, ARRAYEDIT Source, la ventana de prueba de bloques, la precedencia causal de los campos y la identidad de contenido de los XREF; referencias de I-49 (A3-R1), I-55 (marcadores) e I-56 (Proposal V1) actualizadas; ningun resultado de G3 como hecho | CLOSED BY V15 | `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md` |
| `[V15-D13]` | R:V15-07 · C:V15-13 | Contradiccion material de V14 ⇒ Proposal V15 | Nueva matriz `[V15-Dnn]`; `[V14-D01]`, `[V14-D03]`, `[V14-D05]`, `[V14-D11]` y `[V14-D12]` se reabren y se cierran, y `[V14-D07]` y `[V14-D14]` se precisan; `OPEN MATERIAL = NONE` para la revision; toda contradiccion material de V15 ⇒ **Proposal V16**; proceso: acuerdo de Coordinador y Arquitecto sobre V15 → consenso tecnico → rebase sobre el main VIGENTE → reconciliacion obligatoria I-52/I-55/I-49 → estado formal de la Proposal V3 y de las entradas X-* de I-55 resuelto → autoridad FINAL de I-49 para RS-3 → freeze por SHA exacto → re-check exact-SHA → O-1 → G3; I-52 sigue grandfathered; si G3 contradice materialmente ⇒ Proposal V16; si no, el Owner acepta ADR-0036 → G4 | CLOSED BY V15 (proceso) | §0; §14.1; §14.2; §19; §21 |
| `[V15-D14]` | Precision al redactar | Preflight de V14 | Preflight de V15 (§15.1): I-52 `e59bc89` exacta (arbol limpio, sin stash); main `dad4e77` sin cambios; I-49 `c8cfee2` (A3-R1, solo docs, sobre `f6f0991`, CI `34886985747` success: resuelve MAT-1..MAT-5 de la revision exacta de A3; segun A3-R1 cambian directamente ADR-0041 D20, D18 puntos 1-4 y D14 decision 7, y D19 se lee con A3; REPLACEMENT ADR REQUIRED; re-revision exacta del Arquitecto y Owner PENDING; G7 y G8 BLOCKED); I-55 `110cd39` en el preflight y `8068368` en el re-fetch previo al commit (G2E, solo docs: Proposal V4 `fe70d7a`, CI `34889818061` success, con la decision CQ-01 de su Coordinador —redibujo atomico de las hermanas antes de Insertar— y ADR-0042 decision 6 precisada, y paquetes de revision V4 `8068368`, CI `34890384637` success; X-4 con crear y redefinir en la transaccion del llamador, X-5 y X-6 con los disparadores de I-52 V14 y C2-2b con redibujo todo o nada; Coordinador de I-55 REVIEW REQUIRED, Arquitecto formal PENDING; sin adoptar por I-52); I-56 `18401da` (Proposal V1, solo docs, CI `34887275886` success); sin ramas, produccion ni ADR nuevos; censo 35/35/35/35/33; biblioteca `b4ca2248…d560e8`; clasificacion: I-49 contrato NON-MATERIAL (RS-3 delega en la autoridad final, que sigue pendiente) y coordinacion MATERIAL; I-55 contrato NON-MATERIAL (ADR-0042 sigue propuesto e I-52 ya condiciona C-2, Actualizar e Insertar a su aceptacion) y coordinacion HIGH / ACTIVE; I-56 NON-MATERIAL; sin STOP | CLOSED BY V15 (registro) | §15.1; §15.2; §15.4 |
| `[V15-D15]` | Precision al redactar | Terminologia de V14 | **Terminologia**: `EditingSessionPolicy` es un nombre conceptual (no schema persistido) con los resultados `CHARACTERIZED_SAFE` y `BLOCKED`; los prerrequisitos globales de ST-20 incluyen esa politica; en el primer corte todo el inventario minimo activo es `BLOCKED`; sin numeros nuevos de ST, PRE, E, CT, T-M ni G-M (T-M75 añade una tercera violacion, y CT-49 y CT-50 casos nuevos) | CLOSED BY V15 | §4.1; §7.9; §9.1; §11.3 punto 5; §12 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V16 o reduccion de alcance |
...
| **Guarda durable de los productores de contexto** (G-M24 con censo por familias de simbolos de API, incluida la escritura o mutacion de objetos o contexto existentes, `[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`, `[V15-D03]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas, o un call-site productor, consumidor o de escritura nuevo sin clasificar (clonados, importaciones, altas en tablas de simbolos, evaluacion grafica, lecturas de cabecera de `Database`, atributos, rutas de comando o COM, aperturas para escritura con mutacion de presentacion, mutaciones de registros de simbolo existentes y escrituras de estado global incluidos) ⇒ RED | G3 (CT-50: censo inicial, con los sitios de escritura actuales), G5..G7 y despues de integrar |
...
| **Politica cerrada de sesiones de edicion** (`EditingSessionPolicy`: REFEDIT, transaccion larga, Editor de bloques, ventana de prueba de bloques, ARRAYEDIT Source y los modos de edicion en sitio que descubra CT-49; `[V14-D01]`, `[V15-D01]`, `[V15-D02]`) | cada sesion relevante es `CHARACTERIZED_SAFE` o `BLOCKED`; estado de edicion o de sesion no caracterizado o no demostrablemente ausente ⇒ E12 con mensaje por causa; en el primer corte todo el inventario minimo activo ⇒ E12; sin REFCLOSE, BCLOSE, ARRAYCLOSE ni cambios de sesion; sin deteccion fiable confirmada, el estado no es demostrablemente ausente ⇒ E12 | G3 (CT-49) y G6/G7 (T-M73) |
| **Frontera exacta entre estado y edicion** (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`, `[V15-D04]`, `[V15-D05]`) | estado dentro de la garantia, con huella latente conservadora o `UNKNOWN`; edicion futura fuera (ERASE/OOPS, redefinicion, XREF editado y recargado o resuelto a contenido efectivo distinto, fuente de imagen, underlay o nube de puntos reemplazada, parametro dinamico que cambia geometria); `DataLink` refrescado sigue siendo estado; un campo se clasifica por la causa del cambio de su resultado (EDIT CAUSE > FIELD REEVALUATION); la identidad de un XREF es la de su contenido efectivo, no la de su ruta | contrato (sin diferir) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V16 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V16 | G3 (CT-37, CT-38) |
...
| **Autoridad de proceso frente a I-56** (Initiative Workflow V2 en G2 `18401da`: Proposal V1 PUBLISHED y NOT CONSENSUS, con Coordinator y Architect REVIEW REQUIRED y Owner NOT REQUESTED; Evidence Audit COMPLETE; I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; `[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`) | ninguna regla de la Proposal V1 de I-56 se aplica a I-52; reglas de I-56 no integradas e hipotesis del Evidence Audit no se adoptan; antes de cada transicion del proceso se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo | cada transicion del proceso |

### 0.4 Estado de las marcas de V14 bajo V15

| Marca V14 | Estado en V15 |
|---|---|
...
### 0.5 Estado de las marcas de V13 bajo V14 (fijado por V14; sin cambios en V15)
...
### 0.6 Estado de las marcas de V12 (fijado por V13; sin cambios en V14 ni V15)
...
### 0.7 Estado de las marcas de V11 (fijado por V12; sin cambios en V13, V14 ni V15)
...
### 0.8 Estado de las marcas de V10 (fijado por V11; sin cambios en V12, V13, V14 ni V15)
...
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V16) |
...
### 0.9 Estado de las marcas de V9 (fijado por V10; sin cambios en V11, V12, V13, V14 ni V15)
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V16) |
| `[V9-D16]` | vigente |
...
### 0.10 Estado de las marcas de V8 (fijado por V9; sin cambios en V10, V11, V12, V13, V14 ni V15)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V16) |
...
### 0.11 Estado de las marcas de V7 (fijado por V8; sin cambios en V9, V10, V11, V12, V13, V14 ni V15)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V16) |
...
### 0.12 Estado de las marcas de V6 (fijado por V7; sin cambios en V8, V9, V10, V11, V12, V13, V14 ni V15)
...
### 0.13 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8, V9, V10, V11, V12, V13, V14 ni V15)
...
### 0.14 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V15) |
|---|---|
...
### 0.15 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR14-01 (sesiones de edicion sin inventario cerrado ni fallo cerrado por defecto; ARRAYEDIT Source y ventana de prueba de bloques) | `[V15-D01]`, `[V15-D02]`, `[V15-D10]`, `[V15-D11]` |
| AR14-02 (G-M24 sin la familia de escritura sobre objetos o contexto existentes) | `[V15-D03]`, `[V15-D10]` |
| AR14-03 (precedencia causal de los campos) | `[V15-D04]`, `[V15-D10]`, `[V15-D11]` |
| L1 y L2 (redaccion de D19 y «hoy» de PV-6) | `[V15-D06]` |
| L3 (marcadores «X-n, AGREED en `d091eeb`») | `[V15-D08]` |
| L4 (identidad de contenido de los XREF) | `[V15-D05]` |
| L5 (caso G3 de tabla de busqueda solo de visibilidad) | `[V15-D10]` |
| L6 (salida de R-55 si I-49 se cancela) | `[V15-D07]` |
| Cambios exactos R:V15-01..R:V15-07 de la revision | V15-01 → D01; V15-02 → D03; V15-03 → D04; V15-04 → D06, D07; V15-05 → D08; V15-06 → D05, D10; V15-07 → D12, D13 |
| Precisiones del Coordinador C:V15-01..C:V15-13 | V15-01 → D01; V15-02 → D02; V15-03 → D03; V15-04 → D04; V15-05 → D05; V15-06 → D06; V15-07 → D07; V15-08 → D08; V15-09 → D09; V15-10 → D10; V15-11 → D11; V15-12 → D12; V15-13 → D13 |
| ADR-0036 (decisiones 7, 11 y 13 y referencias) | `[V15-D12]` |
| Paralelas y main observados en el preflight y en el re-fetch de V15 | `[V15-D06]`, `[V15-D09]`, `[V15-D14]` |
| Precisiones del ejecutor | clasificacion exacta de A3-R1, re-verificacion siempre en R6 y MUTATE, inventario minimo `BLOCKED`, identidad de XREF en ejecucion y censo de escrituras en `dad4e77` (§0.2), `[V15-D14]`, `[V15-D15]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V15; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V16 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V15; si la contradice ⇒ Proposal V16; **precondicion de G4** |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10, V11, V12, V13, V14 y V15) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10, V11, V12, V13, V14 y V15 (ST-19
completo, huellas visuales de ocupacion de modelo en el contexto de evaluacion destino, frescura tras `EnsureForPlan`,
orden de plan sobre pares invertidos, read-set transitivo del contexto de materializacion, elegibilidad de la
presentacion resuelta, politica de huellas por tipo exacto y por mecanismo de los objetos del dibujo, cola completa de
Model Space, entorno de la copia, transformaciones uniformes reflejadas, completitud de la base de datos y de la
enumeracion, carga bajo demanda de XREF y mecanismos latentes añadidos) no se han medido sobre esta biblioteca y las
miden CT-36, CT-48, CT-49 y CT-50. Estado: **DEFERRED TO G3** con fail-closed (X-17a, X-17b, X-22, X-23).
...
| **Estado de sesion de edicion no caracterizado o no demostrablemente ausente** | un modo de edicion en sitio que CT-49 no caracterizo, o un estado del inventario minimo que no puede detectarse de forma fiable, puede exponer geometria temporal o absorber las copias; un modo descubierto es `BLOCKED` salvo `CHARACTERIZED_SAFE` demostrado (`EditingSessionPolicy`, `[V15-D01]`) | E12 (ST-20), antes de pedir la linea; E10 en R6 o E11 en MUTATE si aparece despues |
...
Antes de C1, una vez por comando: ST-20, prerrequisitos globales (P0): apertura parcial o EditingSessionPolicy
         incumplida (REFEDIT, transaccion larga, Editor de bloques, ventana de prueba de bloques, ARRAYEDIT Source
         o estado de sesion no demostrable) ⇒ E12 (fin, sin linea); se re-verifican en R6 (E10) y en MUTATE (E11)
...
equivalencia de la fachada: **STOP → Proposal V16**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V = 1 ⇒ W = max(70, 76 + 3)  = 79 < 80: la fila de N1 se apila desde la izquierda en el original y en la copia.
    V15: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V = 4 ⇒ W = 92 ≥ 88;  V = 1 ⇒ W = 83 < 88: la frontal del fondo 1 desborda.
    V15: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V15 ⇒ **STOP → Proposal V16**.
...
## 7. Matriz de representabilidad V15
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V15)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V16**. T-M47 lo hace ejecutable desde
G5.
...
| PRE-19 | `DatabaseVisibilityCompleteness` no demostrable (ST-20, `[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`): **prerrequisito global** incumplido o no demostrable —dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente) o `EditingSessionPolicy` incumplida: edicion de referencia en curso (`REFEDITNAME` no vacio), transaccion larga, Editor de bloques abierto (`BLOCKEDITOR = 1`), ventana de prueba de bloques abierta (`BLOCKTESTWINDOW`), edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) u otro estado de edicion o de sesion no caracterizado o no demostrablemente ausente, segun caracterice CT-49—; o **completitud relativa a la fuente** no demostrable (cola posterior a una fuente o contenido relevante); o cualquiera de esos estados cambiado entre P0, R2, R6 y el inicio de MUTATE; sin PARTIALOAD, REFCLOSE, BCLOSE, ARRAYCLOSE, cargas forzadas ni cambios de capas, variables o sesion | FAIL_CLOSED antes de ST-19 definitivo (E12 en P0 para los prerrequisitos globales; E10 en R2 o R6; E11 en MUTATE) | P0; PREPARE (R2 y R6); inicio de MUTATE | §4.1 ST-20; §9.1; CT-49 |
...
Application, fuera del comando. **Su ubicacion no la fija V15**: puede quedar junto al contrato de cada kind o en un
...
Marcas de V12, V13, V14 y V15 en esta seccion: `[V12-D07]` (regla de entorno tambien en la copia), `[V12-D09]` (censo
global de G-M24), `[V13-D07]` (censo por familias de simbolos de API), `[V14-D05]` (familias ampliadas) y `[V15-D03]`
(escritura o mutacion de objetos o contexto existentes).
...
- No crea obligacion de tocar los drawers existentes (§20.3); G-M8 y G-M11 no cambian, y G-M24 vigila, sin modificarlos,
  ...
  reflexion, aperturas para escritura con mutacion de presentacion, mutaciones de registros de simbolo existentes y
  escrituras de estado global incluidos (`[V12-D09]`, `[V13-D07]`, `[V15-D03]`).
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           ...
           prerrequisitos globales de ST-20 (apertura parcial y EditingSessionPolicy) y estado de carga de los XREF
           ([V13-D01]..[V13-D03], [V14-D01], [V15-D01])
...
PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  P0  prerrequisitos globales de ST-20             apertura parcial (Database.IsPartiallyOpened o el mecanismo
                                                   de CT-49) o EditingSessionPolicy incumplida: REFEDIT
                                                   (REFEDITNAME), transaccion larga (LongTransactionManager),
                                                   Editor de bloques (BLOCKEDITOR), ventana de prueba de bloques
                                                   (BLOCKTESTWINDOW), ARRAYEDIT Source u otro estado de sesion no
                                                   caracterizado o no demostrablemente ausente ⇒ E12 con mensaje
                                                   por causa, antes de C1 y de la linea; no promete la completitud
                                                   de la cola por fuente; sin PARTIALOAD, REFCLOSE, BCLOSE,
                                                   ARRAYCLOSE ni cambios ([V13-D01], [V13-D02], [V14-D01],
                                                   [V14-D04], [V15-D01], [V15-D02])
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R6  enumerar de NUEVO todos los objetos de Model Space posteriores a cada fuente y capturar la
      ...
      prerrequisitos globales, incluida la EditingSessionPolicy, y decidir la completitud relativa a la fuente
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           ...
           PRE-17, PRE-18) · prerrequisitos globales (EditingSessionPolicy incluida) y completitud relativa a la
...
| E12 | Prerrequisito global de ST-20 incumplido o no demostrable en P0 (PRE-19; `[V13-D01]`, `[V13-D02]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`): dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente) o `EditingSessionPolicy` incumplida: edicion de referencia en curso (`REFEDITNAME` no vacio), transaccion larga, Editor de bloques abierto (`BLOCKEDITOR = 1`), ventana de prueba de bloques abierta (`BLOCKTESTWINDOW`), edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) u otro estado de edicion o de sesion no caracterizado o no demostrablemente ausente. E12 **no** cubre la completitud relativa a la fuente, que va en E10 | P0 | fin, sin linea y cero mutacion, con mensaje propio por causa («dibujo abierto parcialmente», «edicion de referencia en curso (REFEDIT): cierrela antes de reflejar», «Editor de bloques abierto: cierrelo antes de reflejar», «ventana de prueba de bloques abierta: cierrela antes de reflejar», «matriz asociativa en edicion (ARRAYEDIT): cierre la edicion antes de reflejar», «transaccion larga activa», «estado de sesion de edicion no demostrable»), nunca un aviso; sin PARTIALOAD, REFCLOSE, BCLOSE, ARRAYCLOSE, cargas forzadas ni cambios de capas, variables o sesion; si aparece despues de P0, E10 en R6 o E11 en MUTATE |
...
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`, `[V13-D01]`..`[V13-D05]`).** Autoridad
     ...
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       ...
       UNKNOWN:
         ...
         sesiones         EditingSessionPolicy: CHARACTERIZED_SAFE | BLOCKED por sesion; REFEDIT (REFEDITNAME) ·
                          transaccion larga (LongTransactionManager) · Editor de bloques (BLOCKEDITOR) · ventana de
                          prueba de bloques (BLOCKTESTWINDOW) · ARRAYEDIT Source · modos de edicion en sitio que
                          descubra CT-49; inventario minimo activo = BLOCKED; no caracterizado o no demostrablemente
                          ausente ⇒ E12; P0, re-verificado en R6 y MUTATE ([V14-D01], [V15-D01], [V15-D02])
       ...
       frontera ([V13-D06], [V14-D02], [V14-D03], [V15-D04], [V15-D05]): estado dentro (visibilidad, carga,
         resolucion al mismo contenido efectivo con cualquier ruta, carga bajo demanda, recortes, solo visibilidad
         dinamica, campos cuyo resultado cambia solo por estado, DataLink, proxies, aislamiento, anotacion y
         entorno);
         edicion fuera (ERASE/OOPS, geometria, redefinicion, texto, mover, parametro dinamico que cambia geometria,
         edicion del contenido referenciado por un campo, XREF editado o resuelto a contenido efectivo distinto,
         fuente de imagen, underlay o nube de puntos reemplazada); EDIT CAUSE > FIELD REEVALUATION; identidad de
         contenido no demostrable cuando se necesita ⇒ UNKNOWN
   ...
   - **XREF (`[V12-D03]`).** Primer corte: XREF descargado ⇒ `UNKNOWN`; XREF no resuelto ⇒ `UNKNOWN`; overlay anidado o
     ...
     ausente. **Identidad de contenido (`[V15-D05]`):** «mismo contenido referenciado» es la misma identidad del
     contenido referenciado efectivo, no la misma cadena de ruta; un cambio de ruta, de rutas de busqueda o de
     `PROJECTNAME` se clasifica por el contenido resultante (mismo contenido efectivo ⇒ ESTADO; contenido efectivo
     distinto ⇒ EDICION futura). En ejecucion esa identidad solo se necesita cuando el contenido cargado de un XREF
     posterior puede no corresponder a su resolucion actual (por ejemplo, ruta cambiada sin recargar o archivo
     modificado desde la carga, segun caracterice CT-49); si entonces no puede demostrarse sin abrir archivos externos ⇒
     `UNKNOWN`.
   - **Completitud de la base de datos y de la enumeracion (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`,
     `[V14-D04]`, `[V15-D01]`, `[V15-D02]`).** Tiene dos partes. Los **prerrequisitos globales** se comprueban en P0,
     antes de conocer las fuentes: la base de datos no esta abierta parcialmente y se cumple la **politica cerrada de
     sesiones de edicion** (`EditingSessionPolicy`): toda sesion de edicion en sitio o alternativa relevante es
     `CHARACTERIZED_SAFE` o `BLOCKED`; el inventario minimo —REFEDIT (`REFEDITNAME` no vacio), transaccion larga
     (`LongTransactionManager`), Editor de bloques (`BLOCKEDITOR = 1`), ventana de prueba de bloques (`BLOCKTESTWINDOW`,
     BTESTBLOCK o equivalente) y edicion de la fuente de una matriz asociativa (ARRAYEDIT Source)— es `BLOCKED` cuando
     esta activo en el primer corte; un modo de edicion en sitio que descubra CT-49 es `BLOCKED` salvo
     `CHARACTERIZED_SAFE` demostrado (deteccion fiable, sin geometria temporal en Model Space que ST-19 pueda enumerar y
     sin incorporar objetos nuevos a otra estructura); y un estado de edicion o de sesion no caracterizado o no
     demostrablemente ausente ⇒ E12. En ARRAYEDIT Source RACKMIRROR no intenta razonar sobre la geometria fuente
     temporal ni sobre su replicacion. Un fallo ⇒ E12, y los prerrequisitos se re-verifican siempre en R6 y al inicio de
     MUTATE. La **completitud relativa a la fuente** se evalua despues de clasificar las fuentes y de la LINE: R2
     enumera de forma temprana la cola posterior a cada fuente y las entradas de huella, R3 solo puede rechazar, y R6
     re-enumera y decide la completitud definitiva —toda la cola en el orden efectivo y el contenido relevante de cada
     objeto enumerado (definicion referenciada, anidados, atributos y contenido de XREF) enumerado o clasificado
     `UNKNOWN`— antes de ST-19 definitivo. Las etapas siguen el orden logico del bloque anterior; si una falla, el
     resultado es `UNKNOWN` o fail-closed y nunca se salta a la huella. Un objeto que no esta cargado o que no aparece
     en la base de datos **nunca** se trata como un objeto que no ocupa. Los objetos borrados no forman parte de la
     enumeracion relevante: borrarlos y restaurarlos (ERASE/OOPS) es edicion futura (`[V14-D02]`). RACKMIRROR no ejecuta
     PARTIALOAD, REFCLOSE, BCLOSE ni ARRAYCLOSE, no fuerza cargas, no altera capas, no cambia variables del dibujo y no
     muta ninguna sesion.
   ...
   - **Campos (`[V12-D06]`, `[V15-D04]`).** Si la ocupacion de modelo de un objeto depende de la evaluacion de un campo
     (`DBText`, `MText`, atributos, celdas de tabla u otras clases con campos que se descubran), es `SUPPORTED` solo con
     una cota conservadora valida para **todos** los resultados que admite el contrato; si no ⇒ `UNKNOWN`. El valor
     actual o `FIELDEVAL` no son autoridad suficiente. **Precedencia causal:** la evaluacion de un campo no decide por
     si sola si un cambio es ESTADO o EDICION; se clasifica por la **causa** del cambio del resultado. Es ESTADO si el
     resultado cambia porque solo cambia un estado admitido: visibilidad, carga o resolucion, una variable del sistema,
     la fecha o la hora, el nombre del archivo, el estado del conjunto de planos, el guardado o el trazado, el refresco
     de `DataLink` u otro estado admitido explicitamente. Es EDICION si cambia porque se edito el contenido logico
     referenciado: la geometria o una propiedad del objeto referenciado (por ejemplo, un campo `AcObjProp` con el area
     de una polilinea), una edicion manual de texto o de contenido u otra edicion futura. **EDIT CAUSE > FIELD
     REEVALUATION.** Por eso «todos los resultados que admite el contrato» son los alcanzables **solo** por cambios de
     ESTADO; sin una cota conservadora sobre ellos ⇒ `UNKNOWN`. `DataLink` sigue siendo la decision especial explicita:
     datos externos cambiados + refresco = ESTADO.
   ...
   - **Frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`, `[V15-D04]`, `[V15-D05]`).** **ESTADO,
     dentro de la garantia:** visibilidad (capas, `Entity.Visible` y representaciones anotativas); carga y descarga de
     XREF; resolucion de un XREF al **mismo** contenido referenciado efectivo, con cualquier ruta; carga bajo demanda;
     recortes, con sus marcos; **solo** la visibilidad dinamica; evaluacion de campos cuyo resultado cambia solo por un
     estado admitido; refresco de `DataLink`; visualizacion de proxies; aislamiento de objetos; y estado de
     visualizacion de anotacion y de entorno. **EDICION FUTURA, fuera:** ERASE/OOPS y la restauracion programatica de
     objetos borrados; edicion de geometria; BEDIT o redefinicion de bloque; edicion manual de texto; edicion de la
     geometria o de una propiedad de un objeto referenciado por un campo; mover o estirar; un parametro dinamico que
     cambia la geometria; editar el archivo externo de un XREF y recargarlo; resolver un XREF a un contenido efectivo
     **distinto**, cualquiera sea la ruta; y reemplazar la fuente o el contenido de una imagen, un underlay o una nube
     de puntos. **Precedencias:** el refresco de `DataLink` sigue siendo ESTADO aunque su fuente externa haya cambiado,
     y el contrato exige una cota conservadora para todos sus resultados o `UNKNOWN`; una operacion dinamica (tabla de
     busqueda o parametro combinado) que cambia la geometria ademas de la visibilidad es EDICION, y una tabla de
     busqueda que solo cambia la visibilidad es ESTADO; un cambio del resultado de un campo causado por la edicion del
     contenido referenciado es EDICION (EDIT CAUSE > FIELD REEVALUATION); un cambio de ruta, de rutas de busqueda o de
     `PROJECTNAME` se clasifica por el contenido efectivo resultante, y una identidad de contenido que RACKMIRROR
     necesita y no puede demostrar ⇒ `UNKNOWN`. CT-49 puede caracterizar la restauracion del handle y la posicion en el
     orden tras OOPS, pero eso no la incorpora a la garantia. RACKMIRROR no garantiza la invariancia del orden de dibujo
     frente a ediciones futuras arbitrarias (L-37).
...
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V16.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V15 no introduce tolerancia; CT-36 registra
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V15 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V15 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V15 no introduce una tolerancia nueva: CT-36
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Metodo de inventario cerrado (`[V9-D08]`, `[V12-D01]`, `[V13-D01]`, `[V13-D04]`, `[V13-D05]`).** CT-36 (contenido
    ...
    por tipo exacto en ejecucion:
      ...
      antes de clasificar ([V13-D01], [V13-D04], [V14-D01], [V14-D04], [V15-D01]): prerrequisitos globales en P0
        (apertura parcial y EditingSessionPolicy) y completitud relativa a la fuente en R2 y R6 (cola
    ...
    **STOP → Proposal V16**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V15 abre **Proposal V16**.
...
| **T-M75** | Censo de sitios productores de G-M24 por familias de simbolos de API (`[V12-D09]`, `[V13-D07]`, `[V14-D05]`, `[V15-D03]`): un call-site inventado que construye una entidad relevante, la añade con `AppendEntity` o `AddNewlyCreatedDBObject`, da de alta un registro en una tabla de simbolos, reevalua graficos, añade un atributo, invoca un comando o COM, abre para escritura un objeto o registro existente y muta su presentacion, escribe estado global, o delega la creacion en un helper nuevo, sin declararse ni excluirse con motivo, pone G-M24 en RED. **Tres violaciones artificiales obligatorias:** (1) **un productor basado en clonado** (`DeepCloneObjects`, `WblockCloneObjects` o `Clone` hacia el camino de materializacion) sin clasificar ⇒ RED; (2) **una lectura de cabecera o de contexto de `Database`** (`Database.Dimstyle`, `Clayer`, `Textstyle` o equivalente) sin clasificar ⇒ RED; (3) **una mutacion de presentacion de un objeto existente** (apertura para escritura con `UpgradeOpen` o `OpenMode.ForWrite` seguida de un setter de `Layer`, `Color`, `Linetype`, `LineWeight`, `Transparency`, `Visible` o `PlotStyleName`, o de `SetPropertiesFrom`) sin clasificar ⇒ RED. Un productor declarado que cambia lo que asigna, usa o crea tambien; la guarda no depende de los nombres actuales de archivos o clases ni de hashes | G5..G7 |
...
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55 ↔ I-49, con las ultimas entradas X-*
                                                          ...
                                                          CROSS-INITIATIVE RECONCILIATION en V15: taxonomia de
...
Proceso de I-52 ([V12-D13], [V13-D10], [V13-D14], [V14-D13], [V15-D09], [V15-D13]): I-52 esta grandfathered y sigue el
workflow bajo el que se reclamo. Antes de cada transicion se relee la autoridad de proceso vigente; las futuras
autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52
conserva su workflow de reclamo, salvo que una autoridad de proceso integrada cambie expresamente las reglas de
transicion aplicables a las iniciativas grandfathered. Nunca se adoptan reglas de I-56 no integradas; ninguna regla de
la Proposal V1 de I-56 se aplica a I-52
```
...
### 15.1 main y paralelas observadas en el preflight de V15
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ...
                                                     ancla de las citas de V15: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = e59bc89335c776a24cd48b30f54547696f820dfa (V14; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = c8cfee28ce568856728393515314b0b4a6a03243 (Amendment A3-R1,
                                                     ...
                                                     integrar)
I-55 feature/creacion-de-vistas                    = 806836883ef18d06644cb69099aa21b8e0a875f1 (110cd39 en el
                                                     preflight; G2E: Proposal V4 fe70d7a, CI 34889818061 success,
...
I-56 docs/initiative-workflow-v2                   = 18401dafad2dbe93d356051be1f7061bd214b486 (Initiative Workflow V2:
                                                     G2 Proposal V1, solo docs, CI 34887275886 success; PUBLISHED y
                                                     NOT CONSENSUS; Coordinator y Architect REVIEW REQUIRED; Owner
                                                     NOT REQUESTED; WORKFLOW V2 NOT EFFECTIVE; I-49, I-52 e I-55
                                                     grandfathered; sin integrar)
...
**main desde la publicacion de V14.** Sin cambios: `dad4e77`. Las citas de codigo nuevas de V15 son el censo inicial de
escrituras de CT-50, medido en `dad4e77`; las de los productores declarados (`LateralHeaderDrawer.cs`,
`CantileverViewMaterializer.cs` y `LayerHelper.cs`) son identicas en `1b091be`.
...
**Cambios de paralelas desde la publicacion de V14.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V15 declaraba
como ultimos observados main `dad4e77`, I-52 `e59bc89`, I-49 `c8cfee2`, I-55 `110cd39` e I-56 `18401da`; el preflight de
V15 los confirmo, con ADR-0041 aceptado, ADR-0042 propuesto, sin ramas nuevas, produccion nueva ni numeros de ADR
reservados, y el re-fetch previo al commit encontro los mismos tips salvo I-55 en `8068368` (Proposal V4 y paquetes de
revision, solo docs) (`[V15-D14]`). La historia de paralelas anterior esta en Proposal V14 §15.1, en Proposal V13 §15.1,
en Proposal V12 §15.1, en Proposal V11 §15.1, en Proposal V10 §15.1, en Proposal V9 §15.1, en Proposal V8 §15.1, en
Proposal V7 §15.1 y en el registro §42, §48, §54, §60, §66, §72, §78 y §84. La numeracion de ADR esta en §16.
...
- **I-55 (`feature/creacion-de-vistas` @ `8068368`, View Placement & Projection, Proposal V4 en revision con Arquitecto
  ...
  V15 y al rebase, con X-1..X-8 como entrada vinculante (§15.4), que registra la propiedad de las autoridades
  compartidas que V15 marca como provisionales —taxonomia de tipo de vista, codec semantico de `View`/`Section`,
...
- **I-56 (`docs/initiative-workflow-v2`, Initiative Workflow V2, G1.1 `0d66df2`, solo documentacion y proceso;
  `[V12-D13]`, `[V13-D10]`, `[V14-D10]`):** NON-MATERIAL hoy; Evidence Audit COMPLETE y Proposal NOT STARTED; su
  contrato declara a I-49, I-52 e I-55 grandfathered y que ninguna politica de la V2 rige sin acuerdo tecnico y
  aprobacion del Owner (WORKFLOW V2 NOT EFFECTIVE). Ninguna hipotesis del Evidence Audit cambia la politica de I-52, que
  sigue el workflow bajo el que se reclamo. Antes de cada transicion del proceso de I-52 (consenso, rebase,
  reconciliacion, freeze, re-check, O-1, G3 y gates posteriores) se relee la autoridad de proceso vigente en
  `origin/main`; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante
  grandfathered aplique, I-52 conserva su workflow de reclamo; nunca se adoptan reglas de I-56 no integradas.
...
diferencias de sus Proposals V3 y V4 siguen pendientes de revision formal y reconciliacion. V13 las registro y V15 las
...
freeze, nunca de forma unilateral. V15 **no** adopta todavia los requisitos del lado de I-52 ni congela namespace,
...
con la tabla anterior, que **no** estan acordadas y que ni V14 ni V15 adoptan: X-1 con primer gate simetrico (I-52 G4 o
I-55 G13); **X-2 y X-8 con extractor unico I-55** (G6, G7a y G7b), lo que acoplaria G4..G6 de I-52 a esos gates o
exigiria una excepcion registrada de §14.2 y §20.3; X-4 con un requisito estructural puro de bloques
...
C2-2b con redibujo todo o nada. Esas precisiones **no** estan acordadas y V15 **no** las adopta; la reconciliacion
...
**Contraste con V15.** Ningun X presenta contradiccion material con V15: V15 no fija la propiedad ni la ubicacion fisica
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V15.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V15 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   se censara frente a 0036 cuando se publique; en el preflight de V15 no hay numeros reservados publicados. I-56 no
...
no contradice materialmente V15, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V16. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V16 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V16 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V16 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V16 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V16 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V16 |
...
| **R-52** | ST-20 (`[V13-D01]`..`[V13-D04]`, `[V14-D01]`, `[V14-D04]`, `[V15-D01]`, `[V15-D02]`) puede hacer fallar RACKMIRROR en dibujos abiertos parcialmente, con una edicion de referencia en curso, una transaccion larga, el Editor de bloques o la ventana de prueba de bloques abiertos, la edicion de la fuente de una matriz asociativa (ARRAYEDIT Source) en curso, un estado de sesion de edicion no demostrablemente ausente o XREF cargados bajo demanda con indices, que pueden aparecer en dibujos corrientes con el `XLOADCTL` por defecto; y mientras CT-49 no confirme las API exactas y la deteccion fiable ningun prerrequisito se demuestra | E12 atribuible por causa antes de pedir la linea y E10 que nombra el XREF; CT-49 caracteriza las API, las sesiones y los estados de `XLOADCTL` e `INDEXCTL` antes de cualquier implementacion; declarado en O-1 (L-34, M-35); reduccion material ⇒ Proposal V16 y O-1 se reevalua |
...
| **R-57** | Si CT-49 no encuentra en AutoCAD 2025 una deteccion fiable de ARRAYEDIT Source, de la ventana de prueba de bloques, de una transaccion larga o de otro modo de edicion en sitio, la regla «no demostrablemente ausente ⇒ E12» (`[V15-D01]`, `[V15-D02]`) hace fallar RACKMIRROR en todos los dibujos | CT-49 caracteriza la deteccion antes de cualquier implementacion; sin deteccion fiable ⇒ reduccion material de la envolvente ⇒ Proposal V16 y O-1 se reevalua; la regla nunca se relaja en implementacion ni se sustituye por un aviso |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10, V11, V12, V13, V14 y V15 | §11.3, §6.4, §6.5.5, §6.7 |
...
- **AR14-01: politica frente a censo.** La revision de V14 pedia un censo cerrado de sesiones; el Coordinador lo
  ...
  reclasificarlo exige Proposal V16, y la re-verificacion en R6 y MUTATE se hace siempre. Sin deteccion fiable, la regla
...
- **I-56 (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`, `[V15-D09]`).** Vigilancia de proceso, sin adoptar reglas no integradas
  ni hipotesis del Evidence Audit; su Proposal V1 (`18401da`) no tiene consenso y declara no aplicarse a I-52; su
  contrato declara a I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; las futuras autoridades de proceso se leen con sus
  reglas de transicion y, mientras el invariante aplique, I-52 conserva su workflow de reclamo.
- **Terminologia (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`, `[V13-D16]`, `[V15-D15]`).** Huella de la referencia, huella
  visual, `FootprintInputFingerprint`, `TraceFingerprint`, `ResolvedPresentationEligibility`,
  `ModelSpaceFootprintPolicy`, `ModelSpaceOrderTailSnapshot`, `ContentAvailability`, `VisibilityMechanism`,
  `ProducerSiteCensus`, `DatabaseVisibilityCompleteness` y `EditingSessionPolicy` son autoridades o registros distintos
  (§9.2).
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V16 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V15 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V15 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V16 y O-1 se reevalua.
...
- **I-55 (`[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`, R-45).** I-52 no espera al consenso de I-55,
  ...
  entrada vinculante; V15 mantiene ambas cosas, no presenta `d091eeb` como consenso formal, registra sin adoptarlas las
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V16** o reduccion
  de alcance.
...
- Se intenta ejecutar RACKMIRROR con ARRAYEDIT Source, la ventana de prueba de bloques, una transaccion larga u otro
  ...
  del inventario minimo sin Proposal V16, integrar un sitio de escritura o mutacion de objetos o contexto existentes sin
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V16.
...
G2 = V15 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd), V10 (636f7fd), V11 (2275f21), V12 (915a520), V13 (dd45b0f) y V14 (e59bc89) = historial
     revision de V14: Architect: CHANGES REQUIRED — PROPOSAL V15
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V15, rebase sobre el main vigente, reconciliacion con I-55 e I-49 con
