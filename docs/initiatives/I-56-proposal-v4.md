# I-56 G6 — Workflow V2 Proposal V4

> ```text
> PROPOSAL V4 — NOT CONSENSUS
> Proposal Version = V4
> Coordinator = REVIEW REQUIRED
> Architect = REVIEW REQUIRED ON EXACT V4
...
al Owner: una unidad nueva de esas lineas se detiene hasta OWN-E; esta V4 no elige entre ellas.
...
`docs/automation/decisions/I-56.md` cuando su gate autorice crearlo. No existe consenso por el nombre «V4» ni por una cabecera.

## Cambios respecto de V3 — reconciliacion minima G6

Base publicada: [Proposal V3](I-56-proposal-v3.md), commit `4b04b23f8661d0d8a2a656985339c48796861966`,
blob `6ad956c0234d0db6fbbe9e0fb877054301acd351`. Entrada adversarial independiente:
[Architect Review V3](I-56-architect-review-v3.md), publicada en `5ecb19f122d8267e6c201bb2bcfbf032831610db`,
blob `108281b5f1d95e5145e51ceee66979be7e04f02c`. V4 conserva todas las decisiones, racionales, salvaguardas,
alternativas no reabiertas y LOW diferidos de V3 salvo estas dos correcciones y sus referencias mecanicamente necesarias:

| RC | Correccion verificable | Destino | Estado |
|---|---|---|---|
| RC-V3-01 / AR-V3-01 | `FOUNDATIONS` sigue descriptivo; OWN-K reserva existencia, ubicacion normativa, alcance/proposito y subordinacion, mientras Coordinator + Architect materializan y conforman tecnicamente cada entrada contra fuente, codigo, pruebas y discrepancias A/B | P-06, P-24, P-25, §6 | RESUELTO EN V4 |
| RC-V3-02 / AR-V3-02 | La cobertura `[KEEP]` queda como informacion «COVERAGE POLICY: no change proposed», sin ID OWN, respuesta independiente ni efecto sobre completitud/aprobacion parcial | P-13, §6, U-03 | RESUELTO EN V4 |

**Independencia de I-56.** La revision V3 es evidencia adversarial independiente y exige V4; no es consenso sobre esta nueva
identidad. Tras aceptacion del Coordinator, la V4 exacta requiere revision final en **SEPARATE SESSION** o **EXTERNAL HUMAN**,
con solo entradas versionadas. No se abre esa revision, el dry-run ni el paquete del Owner en G6.

### Reconciliacion V2 preservada de V3
...
**Antecedente de independencia.** La revision V2 fue evidencia adversarial de G4; la revision independiente aplicable a V3
es la entrada de G6 identificada arriba. El autocontrol de esta sesion sigue siendo interno.
...
|---|---|---|
| H.1-1 | Solo documentacion y proceso; sin implementacion de producto, pruebas, CI ni scripts | P-21 solo diseña scripts; P-24 solo recomienda archivos; G6 crea un unico documento. Editar `AGENTS.md` (fuera de `docs/**`) en la integracion normativa exige declararlo y una orden que lo autorice (contrato I-56 §12) |
...
| P-06 fundaciones | Prosa repetida → registro descriptivo verificado `[NEW]` | EA §6, B-35 | A STOP, B ambas confirmaciones; OWN-K para politica del registro; materializacion tecnica Coordinator + Architect; reglas lifecycle, metadata FOUNDATIONS |
...
| P-13 cobertura | Sin cambio `[KEEP]` | EA C-06/C-07 | No dispatch sobre MERGE_SHA; informacion sin decision separada; WORKFLOW |
...
| P-24 archivos | Tres archivos con dominio propio `[NEW]` | Evitar duplicacion/contradicciones | Una autoridad por regla; OWN-K/L/O; plan, no edicion G6 |
...
### P-06 — Registro de fundaciones (diseño; NO se crea en G6)
...
**Poblacion inicial, verificacion por entrada (RC-16, RC-V3-01).** Despues de que el Owner apruebe la politica y la version completa —incluido OWN-K sobre existencia, ubicacion normativa, alcance/proposito y caracter descriptivo/subordinado del registro—, Coordinator + Architect construyen y conforman tecnicamente cada entrada durante la materializacion normativa. Para cada candidata contrastan la fuente aceptada/integrada, las clausulas, el codigo, los simbolos y las pruebas actuales, y registran concordancias y discrepancias A/B. Se aplica completa la regla 5: A consumida = STOP; duda = A; B solo con demostracion y confirmacion de ambos. Una diferencia con Freeze V1 no se legitima copiando codigo. El Owner no aprueba la redaccion factual, los simbolos, DTO ni pruebas de una entrada. Si el Owner quiere cambiar **lo que debe ser**, lo hace mediante una decision Owner de alcance, un ADR aceptado, una politica aprobada u otra fuente normativa propia; FOUNDATIONS solo refleja despues esa autoridad. Si la materializacion descubre una contradiccion semantica con la Proposal acordada, se detiene: nueva version coherente, nuevo consenso Coordinator + Architect y nueva aprobacion del Owner; nunca se inserta politica silenciosamente. Las entradas sin fuente elegible no se publican como STABLE, y ninguna limitacion pendiente autoriza consumir un invariante contradictorio. FOUNDATIONS es metadata descriptiva: su texto nunca prevalece sobre la fuente que resume.
...
ARCHITECTURE sigue en U-08, sin decision aqui. **Para la revision final de I-56 rige la orden particular G6:** solo SEPARATE
SESSION o EXTERNAL HUMAN, despues de que Coordinator acepte la V4 exacta y con entradas versionadas. El autocontrol G6 no es esa
...
**Categoria**: `[KEEP]`; propuesta: **sin cambio de politica de cobertura**. Esta continuidad se informa en el paquete del
Owner, pero no constituye una decision ni requiere respuesta separada (RC-V3-02).
...
I-56 final: solo SEPARATE SESSION o EXTERNAL HUMAN; no se abre en G6
```
...
al administrador capaz de bypass o de cambiar la configuracion. G6 no configura GitHub. Si Owner rechaza esta dependencia,
...
| H9 Revision independiente vs de rol | P-08 `Review mode` (obligacion general pendiente U-08; I-56 final separada por orden particular, reiterada en G6) | Tasas de hallazgos equivalentes entre modos en una comparacion controlada |

### P-23 — Dry-run obligatorio futuro (metodo y casos)

**Categoria**: `[NEW]`. G6 **no** ejecuta el dry-run normativo; la revision adversarial interna de §11 no es el dry-run ni lo abre.
...
**Categoria**: `[NEW]` recomendacion. G6 solo crea esta Proposal; no modifica normas ni crea archivos de este plan.
...
| FOUNDATIONS | Solo metadata descriptiva de fuentes verificadas P-06, materializada y conformada tecnicamente por Coordinator + Architect | ADR/Freeze + A-n y codigo/pruebas; reglas de poblacion/verificacion en lifecycle, no autoridad arquitectonica independiente ni aprobacion factual del Owner |
...
Architect es sesion separada o humano externo; dry-run futuro P-23; Owner aprueba la politica y la version completa (§6), incluido
OWN-K sobre existencia, ubicacion normativa, alcance/proposito y caracter descriptivo/subordinado de FOUNDATIONS. Despues se
abren gates normativos, con autorizacion expresa de AGENTS y decisiones previas requeridas. En esa materializacion posterior,
Coordinator + Architect construyen y conforman tecnicamente las entradas iniciales contra fuente autoritativa, codigo, pruebas y
discrepancias A/B; una clase A es STOP. Si aparece una contradiccion semantica con la Proposal acordada, no se introduce politica
silenciosa: STOP, nueva version, reconsenso y nueva aprobacion del Owner. Todas las normas entran en **un unico merge normativo**,
por V1. Si no caben juntas o se rechaza una parte, se aplica el mismo circuito; no hay activacion parcial.
...
durable de fin; obligatoriedad/excepciones quedan OWN-P, no decididas en G6.
...
- Sin decidir una obligacion general de revision independiente para todo arquetipo (U-08); la revision final particular de I-56 **si** exige sesion separada o humano externo por orden particular, reiterada en G6.
...
Todo item de la tabla queda **PENDIENTE**, incluidas las opciones recomendadas. Decisiones tecnicas propuestas no equivalen a
aprobacion global. La tabla contiene 17 decisiones: OWN-A..OWN-H, OWN-J..OWN-Q y OWN-S.
...
| OWN-K | Tres superficies y cadencia documental, hashes/conteos; existencia, ubicacion normativa, alcance/proposito y caracter descriptivo/subordinado del registro FOUNDATIONS | P-06/15/24 |
...
| OWN-O | Orden expresa para editar AGENTS.md, fuera del alcance docs de I-56; no la concede G6 | Contrato §12 |
...
OWN-N, OWN-O, OWN-P, OWN-Q y OWN-S recogen las materias de politica faltantes señaladas por la revision V2. El diseño de la
escalera/snapshots, reglas A/B, verificacion del DLL construido, formato de campos y comandos manuales es trabajo tecnico de
Coordinator + Architect dentro del paquete; tambien lo son la construccion y conformidad factual de cada entrada FOUNDATIONS.
No se pregunta al Owner por redaccion factual, simbolos, DTO, pruebas ni otros detalles que no cambian politica. Proponer una
redaccion aqui no la vuelve vigente.

**INFORMACION DEL PAQUETE — COBERTURA (RC-V3-02).**
...
P-13 permanece `[KEEP]` sin cambio de poblacion, disparadores ni cadencia. Esta declaracion esta fuera de la tabla de decisiones, no queda PENDIENTE y
no requiere respuesta separada. El silencio del Owner sobre ella no vuelve incompleta la aprobacion. Si el Owner solicita cambiar
la cobertura, se detiene el paquete y el cambio se redacta en una nueva version coherente, con nuevo consenso Coordinator +
Architect y nueva aprobacion completa del Owner.

**Aprobacion parcial (RC-31).** Ninguna aprobacion parcial activa parte de la Proposal. Si Owner rechaza/modifica una decision
de la tabla o no aprueba integra la identidad de consenso: (1) registrar las decisiones individuales; (2) redactar version nueva coherente;
...
materializa por fragmentos mientras tanto. La informacion de cobertura no participa en esta regla. El mismo circuito rige
cambios exigidos por dry-run, una nueva opcion de politica o una contradiccion semantica descubierta al materializar FOUNDATIONS;
una correccion factual conforme con la politica acordada sigue siendo trabajo tecnico de Coordinator + Architect.
...
prohibidos por EA H.1 y esta propuesta no los contiene bajo ningun nombre (autocontrol de V1 §11 SC-01..SC-03 y de esta V4 §11).
...
| U-03 | ¿El dispatch de cobertura del Candidato en la rama (4.5.2.bis) y el de `main` (4.5.7) sobre el mismo SHA medido son la misma evidencia, dado que la definicion del workflow puede diferir? | Architect; si se propone cambiar politica, nueva version y Owner |
...
| U-08 | ¿Toda NEW ARCHITECTURE debe exigir revision en sesion separada? | Owner, con evidencia P-22; I-56 final ya tiene obligacion particular reiterada por G6 |
...
| U-16 | Proteccion de tags hoy ausente | Diseño P-20: crear/verificar antes de merge normativo si OWN-H aprueba; no accion en G6 |
...
## 11. Autocontrol adversarial G6 (interno, sin consenso)
...
no demuestra que el diseño de proceso sea correcto. Las fuentes son contrato, audit cerrado, V3 publicada, review V3, V1
vigente y orden G6. La matriz G4 preserva los controles anteriores; este autocontrol añade el foco RC-V3-01/02.
...
| Owner discrepa con redaccion factual, simbolo, DTO o prueba de FOUNDATIONS | No cambia metadata: usa autoridad normativa para cambiar lo que debe ser; contradiccion semantica exige STOP, version nueva, reconsenso y Owner |
| Coordinator + Architect encuentran discrepancia A al materializar FOUNDATIONS | STOP; la conformidad tecnica no puede rebajarla ni pedir al Owner que apruebe el hecho |
...
| Owner aprueba decisiones y guarda silencio sobre `COVERAGE POLICY: no change proposed` | Aprobacion no queda incompleta: es informacion `[KEEP]`, sin respuesta separada |
| Owner solicita cambiar cobertura | STOP; nueva version con la politica propuesta, nuevo consenso y nueva aprobacion completa |

**Alcance de G6:** solo `I-56-proposal-v4.md`; V1/V2/V3/reviews/audit/contrato y normas compartidas quedan intactos. No se implementan
...
U-01..18 y las 17 decisiones OWN pendientes no se resuelven por silencio; la informacion de cobertura no es una decision y los
LOW diferidos estan individualizados en la matriz inicial.

```text
PROPOSAL V4 = PUBLISHED / NOT CONSENSUS
COORDINATOR = REVIEW REQUIRED
ARCHITECT = REVIEW REQUIRED ON EXACT V4
OWNER = NOT REQUESTED
DRY-RUN = NOT OPEN
