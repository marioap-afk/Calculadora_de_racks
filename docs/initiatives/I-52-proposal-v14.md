# I-52 — Proposal V14: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V14 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V14 se redacta SIN rebase, sobre dd45b0f)
...
> Proposal V1..V12  = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd ·
>                     636f7fd · 2275f21 · 915a520 (historial)
> Proposal V13      = dd45b0ff28d3febcb79aaee141dbb0d3e01168d3   (historial; CI 34876750791 4/4)
> Revision Arq. V13 = «Architect: CHANGES REQUIRED — PROPOSAL V14»; sin BLOCKER ni HIGH; MEDIUM AR13-01 (estados de
>                     base de datos y de sesion y frontera estado/edicion sin clasificar: objetos borrados recuperables
>                     con OOPS, sesiones REFEDIT o Editor de bloques activas, resolucion de XREF a otro contenido y
>                     precedencia de DataLink); LOW AR13-02..AR13-07; cambios exactos V14-01..V14-11 de la revision
>                     (registro: docs/automation/decisions/I-52.md §80)
> Coordinador       = orden de V14 con V14-01..V14-13 vinculantes (registro §81)
> main y paralelas  = §15.1 (preflight de V14 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V14 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V14 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V14 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V13 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V13, sus revisiones de Arquitecto y esta V14 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V14 la registra
...
> el comportamiento documentado de AutoCAD para OOPS, REFEDIT, el Editor de bloques, los marcos de recorte y
> `XLOADCTL`; CT-49 debe confirmarlo en AutoCAD 2025. Ninguna revision ejecuto AutoCAD.

---

## 0. Reconciliacion V13 → V14

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`), V11 (`2275f21`), V12 (`915a520`), V13 (`dd45b0f`) y el Discovery
(`339b3ab`) **se conservan sin cambios**. **Donde V13 y V14 difieran, manda V14.** Las marcas `[V13-D01]`..`[V13-D16]`
siguen vigentes salvo donde una marca `[V14-Dnn]` las modifica, reabre o sustituye (§0.4); las marcas `[V12-Dnn]`,
`[V11-Dnn]`, `[V10-Dnn]`, `[V9-Dnn]`, `[V8-Dnn]`, `[V7-Dnn]`, `[V6-Dnn]` y `[V5-Dnn]` siguen como las dejaron V13, V12,
V11, V10, V9, V8, V7 y V6 (§0.5 a §0.12).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V14** | V14 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V14. **Debe ser NONE** |

### 0.2 Matriz `[V14-D01]`..`[V14-D15]`

En la columna «Origen»: `AR13-nn` es el hallazgo de la revision de V13; `R:V14-nn`, su cambio exacto; `C:V14-nn`, la
precision vinculante del Coordinador en la orden de este gate. Las dos series V14-nn se numeran por separado. **Donde el
cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del
Coordinador**: la revision agrupaba en un cambio (R:V14-01) las sesiones de edicion y los objetos borrados; el
Coordinador los separa (C:V14-01 y C:V14-02), fija la precedencia exacta entre estado y edicion (una operacion dinamica
que cambia la geometria ademas de la visibilidad es edicion; el refresco de `DataLink` sigue siendo estado aunque cambie
su fuente externa), amplia las familias del censo de G-M24 y exige dos violaciones artificiales en T-M75, convierte la
autoridad final de I-49 en condicion del freeze (RS-3 PENDING ⇒ FREEZE BLOCKED) y separa los hechos de I-56 (C:V14-10)
de O-1 (C:V14-09). **Precisiones del ejecutor:** la lista de estado de la orden no nombra la visibilidad de capas,
`Entity.Visible` ni las representaciones anotativas, que V12 y V13 ya protegian; V14 la conserva de forma explicita para
no retroceder. E12 lleva un mensaje por causa (apertura parcial, edicion de referencia en curso, Editor de bloques
abierto), y los prerrequisitos globales se re-verifican siempre en R6 y al inicio de MUTATE, tambien si CT-49 demuestra
que no pueden cambiar durante el comando.

| Marca | Origen | V13 | Correccion V14 | Estado | Donde |
|---|---|---|---|---|---|
| `[V14-D01]` | AR13-01 · R:V14-01 · C:V14-01 | ST-20 solo cubria la apertura parcial; una sesion REFEDIT (working set temporal en Model Space que vuelve a la definicion al cerrar, y copias que podrian incorporarse al working set) o el Editor de bloques activo no tenian precondicion | **ACTIVE EDITING SESSION PRECONDITIONS**, prerrequisitos globales de ST-20 en P0 (PRE-19): **REFEDIT activo** —`REFEDITNAME` no vacio, transaccion larga o edicion en sitio de una referencia activa, o el estado equivalente que caracterice CT-49— ⇒ FAIL_CLOSED antes de ST-19; **Editor de bloques activo** —`BLOCKEDITOR = 1` o el equivalente caracterizado— ⇒ FAIL_CLOSED antes de ST-19. E12 con mensaje atribuible por causa. RACKMIRROR **no** ejecuta REFCLOSE ni BCLOSE, no fuerza el cierre de ningun editor, no mueve entidades del working set y no muta el estado de la sesion. Los prerrequisitos globales se re-verifican en R6 (E10) y al inicio de MUTATE (E11) | CLOSED BY V14 (contrato); API y estados reales: DEFERRED TO G3 WITH FAIL-CLOSED | §2.3; §4.1 ST-20; §7.9 PRE-19; §9.1 P0, R6 y MUTATE; §9.4 E10..E12; CT-49; T-M73; M-35; L-34; ADR-0036 decision 11 |
| `[V14-D02]` | AR13-01 · R:V14-01 · C:V14-02 | Borrar y restaurar objetos no estaba clasificado: un objeto borrado no aparece en la enumeracion y `OOPS` puede restaurarlo con su handle despues de RACKMIRROR | **ERASE / OOPS BOUNDARY** (decision normativa): ERASE, OOPS, borrar y restaurar y la recuperacion programatica de objetos borrados son **edicion futura**, fuera de la garantia de huella latente. Los objetos borrados no forman parte de la enumeracion relevante de ST-20. CT-49 puede caracterizar la restauracion del `ObjectId` o del handle y la posicion en el orden de dibujo tras OOPS, pero eso **no** los incorpora a la garantia de ST-19 | CLOSED BY V14 | §1.3 L-37; §4.1 ST-20; §11.3 punto 5; CT-49; ADR-0036 decision 11 |
| `[V14-D03]` | AR13-01 · R:V14-02 · C:V14-03 | «Resolucion» dentro sin calificar el contenido; `DataLink` dentro y «reemplazar el contenido de un archivo referenciado» fuera sin precedencia normativa (solo §18.2); fuente cambiada de imagenes, underlays y nubes de puntos sin clasificar; tablas de busqueda dinamicas sin regla | **STATE VS EDIT PRECEDENCE**. **ESTADO (dentro):** visibilidad (capas, `Entity.Visible` y representaciones anotativas, que V12 y V13 ya protegian); carga y descarga de XREF; resolucion de XREF al **mismo** contenido referenciado; carga bajo demanda; recortes; **solo** la visibilidad dinamica; evaluacion de campos; refresco de `DataLink`; visualizacion de proxies; aislamiento de objetos; estado de visualizacion de anotacion y de entorno. **EDICION (fuera):** ERASE/OOPS; edicion de geometria; BEDIT o redefinicion de bloque; edicion manual de texto; mover o estirar; parametro dinamico que cambia la geometria; editar el archivo externo de un XREF y recargarlo; resolver un XREF a **otro** archivo o contenido; reemplazar la fuente o el contenido de una imagen, un underlay o una nube de puntos. **Precedencias:** el refresco de `DataLink` sigue siendo ESTADO aunque su fuente externa haya cambiado, y el contrato exige una cota conservadora o `UNKNOWN`; una operacion dinamica (tabla de busqueda o parametro combinado) que cambia la geometria ademas de la visibilidad es EDICION | CLOSED BY V14 | §1.3 L-37; §11.3 punto 5; §18.2; ADR-0036 decision 11 |
| `[V14-D04]` | AR13-02 · R:V14-03 · C:V14-04 | P0 prometia la completitud de la cola por fuente antes de conocer las fuentes; §4.1 exigia ST-20 antes de todo ST-19 en PREPARE mientras R3 ejecutaba ST-19 temprano antes de la re-verificacion de R6 | **P0 / R2 / R3 / R6 ORDER**: P0 comprueba **solo** los prerrequisitos globales, conocibles antes de las colas relativas a cada fuente: base de datos abierta parcialmente, REFEDIT o transaccion larga activa, Editor de bloques activo y demas prerrequisitos globales de `DatabaseVisibilityCompleteness`; P0 **no** promete la completitud de la cola por fuente. Tras clasificar las fuentes y la LINE: **R2** construye la enumeracion temprana de la cola posterior a cada fuente y las entradas de huella; **R3** es un rechazo anticipado opcional de ST-19 que nunca aprueba; **R6** hace la re-enumeracion definitiva, la completitud relativa a la fuente definitiva (ST-20) y ST-19 definitivo; **MUTATE** re-verifica la postcondicion y revierte ante un cambio. E12 solo en P0; E10 en PREPARE (R2 temprano o R6 definitivo); E11 en MUTATE | CLOSED BY V14 | §4.1 ST-20 y bloque de precedencia; §7.9 PRE-19; §9.1; §9.4 E10..E12; §11.3 punto 5 |
| `[V14-D05]` | AR13-03 · R:V14-04 · C:V14-05 | Lista minima de simbolos de V13 sin propiedades de cabecera de `Database` (`db.Dimstyle`, `LateralHeaderDrawer.cs:391`), altas en tablas de simbolos (`LateralHeaderDrawer.cs:244`; `LayerHelper.cs:26`; `CantileverViewMaterializer.cs:48`, `:111`, `:195`), evaluacion grafica (`LateralHeaderDrawer.cs:375`), importacion ni rutas de comando o COM | **G-M24 API FAMILY EXPANSION**, conservando las familias de V13: **cabeceras y valores por defecto de `Database`** (`Database.Dimstyle`, `Clayer`, `Cecolor`, `Celtype`, `Textstyle`, `Cannoscale`, `GetDimstyleData` y propiedades visuales o por defecto equivalentes que se descubran); **creacion en tablas de simbolos** (`BlockTable.Add`, `LayerTable.Add`, `LinetypeTable.Add`, `TextStyleTable.Add`, `DimStyleTable.Add`, `SymbolTable.Add` y patrones de insercion de registros); **evaluacion grafica** (`RecomputeDimensionBlock`, `EvaluateHatch`, `GenerateLayout` y `RecordGraphicsModified` donde sea relevante); **importacion y clonado** (`ReadDwgFile`, `Database.Insert`, `Wblock`, `WblockCloneObjects`, `DeepCloneObjects`, `Clone`, `GetTransformedCopy`); **atributos** (`AttributeCollection.AppendAttribute`, `SetAttributeFromBlock`); **comando y COM** (`Editor.Command`, `SendStringToExecute`, `ModelSpace.Add*`, `InsertBlock` y envoltorios equivalentes). Cada call-site relevante `DECLARED` o `EXCLUDED` con motivo. T-M75 añade **dos** violaciones artificiales —un productor por clonado y una lectura de cabecera o de contexto de `Database`— y ambas ponen G-M24 en RED. CT-50 ancla como minimo la creacion, `RecomputeDimensionBlock` y `db.Dimstyle` de `LateralHeaderDrawer`, `layerTable.Add` de `LayerHelper` y las altas de bloque y de capa de `CantileverViewMaterializer` | CLOSED BY V14 (contrato); implementacion: G5..G7 | §8.7; §12.1 CT-50; §12.2 T-M75; §12.3 G-M24 |
| `[V14-D06]` | AR13-04 · R:V14-05 · C:V14-06 | Recortes con «ocupacion sin recortar», sin el contorno o marco del recorte, que puede quedar fuera del contenido y hacerse visible; modelos de coordinacion fuera del inventario de clases | **CLIP FRAME OCCUPANCY**: mecanismos de marco y contorno `XCLIPFRAME`, `FRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, `PDFFRAME`, `DWFFRAME`, `DGNFRAME`, `POINTCLOUDCLIPFRAME` y equivalentes que se descubran. Huella latente de un objeto recortado = ocupacion conservadora del contenido sin recortar **∪** ocupacion del contorno o marco que pueda hacerse visible; si la ocupacion del marco no puede acotarse ⇒ `UNKNOWN`. **Modelos de coordinacion** (NWD, NWC y clases similares) en el inventario por tipo exacto; pueden quedar `UNKNOWN` | CLOSED BY V14 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §2.3; §4.1 ST-19; §11.3 puntos 5 y 14; CT-49; T-M73; G-M23 |
| `[V14-D07]` | AR13-05 · R:V14-06 · C:V14-07 | V13 describia A3 como un cambio de D20 (`Upstream`), que I-52 no consume, con `RootCauses` solo como dato adicional del `PlanReadSet`; §15.3 permitia releer I-49 «si ya existen» y, con ello, congelar sobre ADR-0041 D19 obsoleto | **I-49 A3 / FINAL PLANREADSET AUTHORITY**: A3 afecta a ADR-0041 **D19** —`SymbolResultObservation` y semantica futura del `PlanReadSet`: M2, M3, M4 y M10(a), con `RootCauses`— y a **D20** —`Upstream`, M10(b)—. El contrato actual de I-52 sigue NON-MATERIAL porque RS-3 delega en la autoridad final integrada de I-49. **Antes del freeze de I-52** es obligatorio releer la autoridad final de I-49: V6, A1, A2, A3, ADR de reemplazo, nuevo Consensus Freeze y estado de implementacion. Si esa autoridad final no esta disponible: **RS-3 = PENDING I-49 FINAL AUTHORITY** e **I-52 FREEZE = BLOCKED**. Se elimina «si ya existen» y toda redaccion que permita congelar sobre ADR-0041 D19 obsoleto | CLOSED BY V14 (proceso) | §9.2.1 RS-3; §10.2 PV-6; §12.1 CT-46; §14.1; §15.3; §18.2; §19; ADR-0036 referencias |
| `[V14-D08]` | AR13-06 · R:V14-07 · C:V14-08 | §14.2 G4..G7 fijaba sin marca provisional ubicaciones compartidas (nucleo neutral en `Persistence/`, contrato con `DescribeView` e `IsSameAuthority` en `Mirror/`, materializador en `Plugin/Systems/Shared/`, autoridades de plan) | **SHARED LOCATIONS REMAIN PROVISIONAL**: en §14.2, las ubicaciones de G4..G7 que tocan X-1..X-8 quedan **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION**; antes de la reconciliacion no se congelan `Persistence/`, `Mirror/`, `Application.Views`, `Plugin/Systems/Shared/` ni ningun namespace, ruta o forma de API de responsabilidades compartidas. La Proposal fija la responsabilidad y la semantica que necesita I-52, no la propiedad neutral ni la ubicacion fisica. X-1..X-8 siguen siendo entrada vinculante | CLOSED BY V14 (proceso) | §14.2 G4..G7; §15.4; §20.1 |
| `[V14-D09]` | AR13-07 · R:V14-08 · C:V14-09 | L-34, L-37 y M-35 sin REFEDIT, Editor de bloques ni ERASE/OOPS, y sin el efecto practico de la carga bajo demanda en dibujos corrientes | **O-1 PRACTICAL LIMITATIONS**: L-34, L-37 y M-35 exponen que REFEDIT activo bloquea RACKMIRROR; que el Editor de bloques activo lo bloquea; que ERASE/OOPS es edicion futura no protegida; que la apertura parcial y la enumeracion incompleta siguen cubiertas; que la carga bajo demanda de XREF puede causar fallos conservadores en dibujos corrientes y CT-49 debe caracterizar el comportamiento con el `XLOADCTL` por defecto; y que `UNKNOWN` sigue fallando aunque parezca lejos. O-1 no se pide | CLOSED BY V14 | §1.3 L-34 y L-37; §13 M-35; §17 R-52 |
| `[V14-D10]` | AR13-07 · R:V14-08 · C:V14-10 | I-56 registrado en `12fb509` (G0.1) | **I-56 CURRENT**: ultimo observado `0d66df26882cb5cdd2efc21155c121f58d2af669` (G1.1, correccion de la clasificacion normativa del Evidence Audit, solo docs; G1 en `c6839e2`): Evidence Audit COMPLETE, Proposal NOT STARTED, WORKFLOW V2 NOT EFFECTIVE, I-52 grandfathered. Ninguna hipotesis del Evidence Audit cambia la politica de I-52; solo se actualizan los hechos | CLOSED BY V14 (registro) | §15; ADR-0036 referencias |
| `[V14-D11]` | R:V14-09 · C:V14-11 | Casos de G3 de V13 | Casos obligatorios nuevos: **ERASE/OOPS** (solo caracterizar y demostrar la redaccion fuera de la garantia); **sesiones de edicion** (REFEDIT activo, REFEDIT cerrado, Editor de bloques activo y Model Space normal); **estado/edicion** (XREF que resuelve al mismo archivo o contenido, XREF que resuelve a otro contenido, XREF externo editado y recargado, `DataLink` con la fuente externa cambiada y refrescado, cambio de visibilidad dinamica y tabla de busqueda dinamica que cambia la geometria); **marcos** (`XCLIPFRAME`, `IMAGEFRAME`, `WIPEOUTFRAME`, marco PDF, DWF o DGN y marco de recorte de nube de puntos); **G-M24** (productor artificial por clonado ⇒ RED; lectura de cabecera de `Database` ⇒ RED); **I-49** (sin implementacion aqui; solo el requisito de reconciliacion). G3 sigue NOT OPEN; contradiccion material ⇒ Proposal V15 | CLOSED BY V14 | §12; §13; §14 |
| `[V14-D12]` | R:V14-10 · C:V14-12 | ADR-0036 alineado con V13 | ADR-0036 sigue **PROPOSED** y actualiza las decisiones 7, 11, 12 y 13 con el fallo cerrado ante REFEDIT o el Editor de bloques activos, ERASE/OOPS fuera de la garantia, la frontera exacta entre estado y edicion, P0 frente a la completitud relativa a la fuente, el censo de API ampliado, la ocupacion de los marcos de recorte, D19 y D20 de I-49 con la compuerta de autoridad final, las ubicaciones fisicas compartidas provisionales y el estado actual del Evidence Audit de I-56; nunca presenta resultados de G3 como hechos; «la caracterizacion **DEBE** demostrar» | CLOSED BY V14 | ADR-0036 |
| `[V14-D13]` | C:V14-13 · R:V14-11 | Contradiccion material de V13 ⇒ Proposal V14 | Nueva matriz `[V14-Dnn]` sobre AR13-01..AR13-07 y las decisiones vinculantes del Coordinador; `[V13-D01]`, `[V13-D06]`, `[V13-D11]`, `[V13-D12]`, `[V13-D13]` y `[V13-D14]` se reabren y se cierran, y `[V13-D04]`, `[V13-D05]`, `[V13-D07]`, `[V13-D08]` y `[V13-D15]` se precisan (§0.4); `OPEN MATERIAL = NONE` para la revision; toda contradiccion material de V14 ⇒ **Proposal V15**; proceso: Coordinador y Arquitecto de acuerdo sobre V14 → consenso tecnico → rebase sobre el main vigente → reconciliacion obligatoria I-52/I-55/I-49 → autoridad final de I-49 disponible para RS-3 → freeze por SHA exacto → re-check exact-SHA → O-1 del Owner → G3 (Proposal V15 si G3 contradice materialmente / Owner acepta ADR-0036 → G4). I-52 sigue grandfathered | CLOSED BY V14 | §1.2; §7.0; §14.1; §16; §19 |
| `[V14-D14]` | Precision al redactar | Preflight de V13 | Preflight de V14 (§15.1): I-52 `dd45b0f` exacta (arbol limpio, sin stash ni operaciones a medias); main `dad4e77` sin cambios; I-49 `f6f0991` sin cambios (A3 solo docs, CI `34875190797` success; Arquitecto de I-49 PENDING sobre A3 exacto; ADR de reemplazo pendiente y sin numero; G7 BLOCKED); I-55 en `110cd39` (G2D, solo docs; Proposal V3 `8e35a51`, CI `34881178359` y `34881680833` success): Coordinador de I-55 = CHANGES REQUIRED → V3 sobre V2 y REVIEW REQUIRED sobre V3; Arquitecto formal PENDING; la revision de G2C (`d091eeb`) queda reclasificada como revision tecnica adversarial, no como veredicto formal; la tabla X-1..X-8 de G2C queda aceptada del lado de I-55 por su Coordinador; V3 propone diferencias en X-1, X-2, X-4, X-5, X-7 y X-8 y una tabla de adopcion para I-52, sin darlas por adoptadas y sin MATERIAL CONFLICT de contenido; ADR-0042 propuesto, que complementa a ADR-0010 (§15.4); I-56 `0d66df2` (G1 `c6839e2` y G1.1 `0d66df2`, solo docs, CI `34877304129` y `34879848606` success); sin ramas nuevas ni produccion nueva; ADR 0035..0042 sin cambios de estado, sin numeros posteriores y sin numeros reservados publicados; biblioteca con el mismo SHA-256; censo de `[CommandMethod(`: main 35, I-49 35, I-55 35, I-56 35, I-52 33. Ninguna paralela modifica una autoridad de I-52: sin STOP | CLOSED BY V14 (registro) | §15.1 |
| `[V14-D15]` | Precision al redactar | Terminologia de V13 | **Terminologia**: `DatabaseVisibilityCompleteness` se divide en **prerrequisitos globales** (P0: apertura parcial, sesion de edicion de referencia o transaccion larga, Editor de bloques y lo que caracterice CT-49) y **completitud relativa a la fuente** (R2 temprana y R6 definitiva: cola posterior a cada fuente y contenido relevante); `ProducerSiteCensus` por las familias de simbolos de API ampliadas; el **marco o contorno de recorte** forma parte de la huella latente. Sin numeros nuevos de ST, PRE, E, CT, T-M ni G-M | CLOSED BY V14 | §4.1; §7.9; §9.1; §9.2; §11.3 punto 5; §12.3 G-M24 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V15 o reduccion de alcance |
...
| **Guarda durable de los productores de contexto** (G-M24 con censo por familias de simbolos de API, `[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas, o un call-site productor o consumidor nuevo sin clasificar (clonados, importaciones, altas en tablas de simbolos, evaluacion grafica, lecturas de cabecera de `Database`, atributos y rutas de comando o COM incluidos) ⇒ RED | G3 (CT-50: censo inicial), G5..G7 y despues de integrar |
...
| **Campos** (`DBText`, `MText`, atributos, celdas de tabla y otras clases; `[V12-D06]`) | sin cota conservadora para todos los resultados admitidos ⇒ `UNKNOWN`; el valor actual no es autoridad | G3 (CT-49) |
...
| **Sesiones de edicion activas** (REFEDIT o transaccion larga, Editor de bloques; `[V14-D01]`) | `REFEDITNAME` no vacio, transaccion larga, `BLOCKEDITOR = 1` o el estado equivalente que caracterice CT-49 ⇒ E12 con mensaje por causa; sin REFCLOSE, BCLOSE ni cambios de sesion; sin API confirmada, el prerrequisito no se demuestra | G3 (CT-49) |
| **Frontera exacta entre estado y edicion** (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`) | estado dentro de la garantia, con huella latente conservadora o `UNKNOWN`; edicion futura fuera (ERASE/OOPS, redefinicion, XREF a otro contenido o editado y recargado, fuente de imagen, underlay o nube de puntos reemplazada, parametro dinamico que cambia geometria); `DataLink` refrescado sigue siendo estado | contrato (sin diferir) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V15 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V15 | G3 (CT-37, CT-38) |
...
| **Guardas de I-49 G6 y autoridad final de I-49** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones, con la correccion G6-C2 de `75f1862`; ADR-0041 vigente; el Amendment A3 de `f6f0991` cambia D19 y D20 y pide un ADR de reemplazo sin numero; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V13-D15]`, `[V14-D07]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18; RS-3 = PENDING I-49 FINAL AUTHORITY mientras no existan V6 + A1 + A2 + A3, el ADR de reemplazo y el Consensus Freeze nuevo, y con ese estado **I-52 FREEZE = BLOCKED** | reconciliacion antes del freeze de I-52 |
| **Reconciliacion con I-55 (e I-49 donde aplique) antes del freeze** (View Placement & Projection, Proposal V3 `8e35a51` en revision con Arquitecto formal PENDING; tabla X-1..X-8 de G2C aceptada del lado de I-55 como entrada vinculante y diferencias de V3 pendientes de acordar; ADR-0042 propuesto que complementa a ADR-0010; autoridades compartidas y sus ubicaciones provisionales; `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V13-D08]`, `[V13-D09]`, `[V14-D08]`, `[V14-D14]`, R-45) | sin reconciliacion registrada de las autoridades compartidas no hay freeze; la reconciliacion parte de las ultimas entradas X-* acordadas; una autoridad incompatible congelada por cualquiera de las dos ⇒ ningun freeze para ninguna hasta resolverlo; I-55 aplica la misma regla; I-52 no espera al consenso de I-55 | tras el consenso tecnico y el rebase, antes del freeze |
| **Autoridad de proceso frente a I-56** (Initiative Workflow V2 en G1.1 `0d66df2`: Evidence Audit COMPLETE, Proposal NOT STARTED, I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; `[V12-D13]`, `[V13-D10]`, `[V14-D10]`) | reglas de I-56 no integradas e hipotesis del Evidence Audit no se adoptan; antes de cada transicion del proceso se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo | cada transicion del proceso |

### 0.4 Estado de las marcas de V13 bajo V14

| Marca V13 | Estado en V14 |
|---|---|
...
### 0.5 Estado de las marcas de V12 (fijado por V13; sin cambios en V14)
...
### 0.6 Estado de las marcas de V11 (fijado por V12; sin cambios en V13 ni V14)
...
### 0.7 Estado de las marcas de V10 (fijado por V11; sin cambios en V12, V13 ni V14)
...
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V15) |
...
### 0.8 Estado de las marcas de V9 (fijado por V10; sin cambios en V11, V12, V13 ni V14)
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V15) |
| `[V9-D16]` | vigente |
...
### 0.9 Estado de las marcas de V8 (fijado por V9; sin cambios en V10, V11, V12, V13 ni V14)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V15) |
...
### 0.10 Estado de las marcas de V7 (fijado por V8; sin cambios en V9, V10, V11, V12, V13 ni V14)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V15) |
...
### 0.11 Estado de las marcas de V6 (fijado por V7; sin cambios en V8, V9, V10, V11, V12, V13 ni V14)
...
### 0.12 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8, V9, V10, V11, V12, V13 ni V14)
...
### 0.13 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V14) |
|---|---|
...
### 0.14 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR13-01 (estados de base de datos y de sesion y frontera estado/edicion: OOPS, REFEDIT, Editor de bloques, resolucion de XREF, `DataLink`) | `[V14-D01]`..`[V14-D03]`, `[V14-D09]`, `[V14-D11]` |
| AR13-02 (orden P0/R2/R3/R6) | `[V14-D04]` |
| AR13-03 (familias de API de G-M24 que el codigo ya usa) | `[V14-D05]` |
| AR13-04 (marcos y contornos de recorte; modelos de coordinacion) | `[V14-D06]` |
| AR13-05 (A3 de I-49 afecta a D19 y D20; autoridad final antes del freeze) | `[V14-D07]` |
| AR13-06 (ubicaciones compartidas de §14.2 provisionales) | `[V14-D08]` |
| AR13-07 (impacto practico en O-1; hechos de I-56) | `[V14-D09]`, `[V14-D10]` |
| Cambios exactos R:V14-01..R:V14-11 de la revision | V14-01 → D01, D02; V14-02 → D03; V14-03 → D04; V14-04 → D05; V14-05 → D06; V14-06 → D07; V14-07 → D08; V14-08 → D09, D10; V14-09 → D11; V14-10 → D12; V14-11 → D13 |
| Precisiones del Coordinador C:V14-01..C:V14-13 | V14-01 → D01; V14-02 → D02; V14-03 → D03; V14-04 → D04; V14-05 → D05; V14-06 → D06; V14-07 → D07; V14-08 → D08; V14-09 → D09; V14-10 → D10; V14-11 → D11; V14-12 → D12; V14-13 → D13 |
| ADR-0036 (decisiones 7, 11, 12 y 13 y alineacion general) | `[V14-D12]` |
| Paralelas y main observados en el preflight y en el re-fetch de V14 | `[V14-D07]`, `[V14-D10]`, `[V14-D14]` |
| Precisiones del ejecutor | mensajes de E12 por causa y re-verificacion siempre en R6 y MUTATE (§0.2), `[V14-D14]`, `[V14-D15]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V14; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V15 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V14; si la contradice ⇒ Proposal V15; **precondicion de G4** |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10, V11, V12, V13 y V14) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10, V11, V12, V13 y V14 (ST-19
...
| **Edicion de referencia en curso (REFEDIT) o Editor de bloques abierto** | el working set temporal vuelve a su definicion al cerrar la sesion y las copias podrian incorporarse a el; en el Editor de bloques el espacio de trabajo no es Model Space; RACKMIRROR no ejecuta REFCLOSE ni BCLOSE (`[V14-D01]`) | E12 (ST-20), antes de pedir la linea; E10 en R6 o E11 en MUTATE si aparece despues |
...
> descomposicion sobre `Transform2D` son fundacion compartida con I-55 (X-7, AGREED en `d091eeb`, con custodio I-52:
> `Transform2D` mas un valor de colocacion, con la reflexion como politica de I-52 y la rigidez como politica de I-55).
> El algebra de la reflexion y las comprobaciones de esta seccion son de I-52; nombre, ubicacion y forma de API del
> valor de colocacion no los congela I-52 sola (§8.3, §14.1, §15.4).
...
> codec de `View`/`Section` son fundacion compartida con I-55 (X-2, AGREED en `d091eeb`; ADR-0042 decisiones 2 y 8,
> propuesto). La semantica que I-52 necesita —la lectura de produccion, la forma canonica y el fallo cerrado de lo no
> inequivoco— puede acordarse; propiedad, namespace, ubicacion, forma de API y codec neutral definitivo no los congela
> I-52 sola: se fijan en la reconciliacion obligatoria previa al freeze, con X-2 como entrada vinculante (§14.1, §15.4).
> Tambien son provisionales el origen y el tramo del eje de la vista que fijan `c` (X-8) y la caracterizacion unica
> CT-04.
...
> AGREED en `d091eeb`), su matriz normativa y la disponibilidad sobre el sistema resuelto comparten fundacion con la
> matriz de I-55, y su ubicacion y forma de API se fijan en la reconciliacion (§14.1, §15.4).
...
```text
Antes de C1, una vez por comando: ST-20, prerrequisitos globales (P0): apertura parcial, REFEDIT o transaccion
         larga activa o Editor de bloques abierto ⇒ E12 (fin, sin linea); se re-verifican en R6 (E10) y en MUTATE (E11)
...
> fundacion compartida con I-55 (X-1, AGREED en `d091eeb`, con custodio I-52 y CT-16 como caracterizacion unica). Los
> hechos neutrales y la equivalencia de la fachada son necesarios para I-52; propiedad, namespace, ubicacion y forma de
> API definitivos del nucleo no los congela I-52 sola: se fijan en la reconciliacion y G4 avisa a I-49 y a I-55 antes de
> tocarlo (§14.1, §15.4).
...
equivalencia de la fachada: **STOP → Proposal V15**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
> (`IsSameAuthority`) es fundacion compartida con I-55 (X-3, AGREED en `d091eeb`, con custodio I-52: el comparador vive
> en el contrato del kind, fuera del reflector del espejo, que solo lo referencia). La politica de miembros de I-52 (las
> vistas seleccionadas) y el fallo E3 son de I-52; donde se declara el comparador y su forma de API se fijan en la
> reconciliacion (§6.1, §14.1, §15.4).
...
> `AdmissibleViews` e `IsSameAuthority` consumen fundacion compartida con I-55 (X-2, X-3 y X-8, AGREED en `d091eeb`):
> decodificacion sintactica, disponibilidad sobre el sistema resuelto, descriptor de origen y tramo del eje y comparador
> authored en el contrato del kind. `Assess`, `Reflect`, `μ_k`, `Expone` y `F` son politica de I-52. La forma de API de
> este contrato, en lo que toca a esas autoridades, no la congela I-52 sola (§14.1, §15.4).
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V = 1 ⇒ W = max(70, 76 + 3)  = 79 < 80: la fila de N1 se apila desde la izquierda en el original y en la copia.
    V14: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V = 4 ⇒ W = 92 ≥ 88;  V = 1 ⇒ W = 83 < 88: la frontal del fondo 1 desborda.
    V14: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V14 ⇒ **STOP → Proposal V15**.
...
## 7. Matriz de representabilidad V14
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V14)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V15**. T-M47 lo hace ejecutable desde
G5.
...
| PRE-19 | `DatabaseVisibilityCompleteness` no demostrable (ST-20, `[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`, `[V14-D04]`): **prerrequisito global** incumplido o no demostrable —dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente), edicion de referencia en curso (`REFEDITNAME` no vacio, transaccion larga o equivalente) o Editor de bloques abierto (`BLOCKEDITOR = 1` o equivalente), segun caracterice CT-49—; o **completitud relativa a la fuente** no demostrable (cola posterior a una fuente o contenido relevante); o cualquiera de esos estados cambiado entre P0, R2, R6 y el inicio de MUTATE; sin PARTIALOAD, REFCLOSE, BCLOSE, cargas forzadas ni cambios de capas, variables o sesion | FAIL_CLOSED antes de ST-19 definitivo (E12 en P0 para los prerrequisitos globales; E10 en R2 o R6; E11 en MUTATE) | P0; PREPARE (R2 y R6); inicio de MUTATE | §4.1 ST-20; §9.1; CT-49 |
...
> la resolucion del diseño que hoy precede al plan son fundacion compartida con I-55 (X-2 y X-8, AGREED en `d091eeb`;
> ADR-0042 decisiones 1 y 8, propuesto, que parte del sistema ya resuelto). X-2 acuerda `Build` = `Resolve` + `Plan`,
> con `Plan` para todas las direcciones y la resolucion compartida con los handlers del BOM, como entrada vinculante de
> la reconciliacion. La semantica que I-52 necesita puede acordarse; propiedad, namespace, ubicacion y forma definitiva
> de la autoridad no los congela I-52 sola (§14.1, §15.4).
...
> referencia» en la transaccion del llamador es fundacion compartida con I-55 (X-4, AGREED en `d091eeb`, con custodio
> I-52: Z y presentacion explicitas y comprobacion de faltantes por instancia); ST-13, ST-19, ST-20, PRE-17, PRE-18,
> PRE-19, el `MaterializationContextReadSet` y la equivalencia visual siguen siendo politica de I-52 fuera del
> primitivo. Propiedad, ubicacion, forma de API y primitivo neutral definitivo no los congela I-52 sola (§14.1, §15.4).
...
> composicion sobre `Transform2D` son fundacion compartida con I-55 (X-7, AGREED en `d091eeb`): la rigidez es invariante
> de I-55 y la reflexion es politica de I-52, y el valor no lleva `Origin` ni Z. Propiedad, ubicacion y forma de API
> definitivas se fijan en la reconciliacion (§3.3, §14.1, §15.4).
...
> taxonomia de tipo de vista son fundacion compartida con I-55 (X-2, AGREED en `d091eeb`, con custodio I-55). La entrada
> acordada situa el nucleo de decodificacion fuera de `Mirror/` y deja en `Mirror/` `A_k` y la exposicion; se adopta o
> se vuelve a acordar en la reconciliacion. La semantica que I-52 necesita puede acordarse; propiedad, namespace,
> ubicacion y forma de API del codec no los congela I-52 sola (§3.7, §14.1, §15.4).

La decodificacion `SectionRaw → semantica → canonica`, la exposicion y el conjunto `A_k` (§3.7, §3.8) viven en
Application, fuera del comando. **Su ubicacion no la fija V14**: puede quedar junto al contrato de cada kind o en un
...
> las lineas base de C-2 (C2-2a, C2-2b, C2-4, G-M9) son fundacion compartida con I-55 (X-3 y X-6, AGREED en `d091eeb`;
> X-6 pide un `Plan` compartido de la planta Cantilever con `PlantaVisibility` y un fixture de C-2 con brazos y tensores
> visibles). Si ADR-0042 se acepta e I-55 re-enruta Insertar antes que I-52, los fixtures de C2-2b cumplen sus
> precondiciones nuevas (propiedades personalizadas acotadas al `RackId` y redibujo previo sin fallos) y la segunda
> iniciativa en integrar re-establece C-2 (`[V12-D17]`, `[V13-D09]`). Propiedad y ubicacion del comparador no los
> congela I-52 sola (§5.4, §14.1, §15.4).
...
Marcas de V12, V13 y V14 en esta seccion: `[V12-D07]` (regla de entorno tambien en la copia), `[V12-D09]` (censo
global de G-M24), `[V13-D07]` (censo por familias de simbolos de API) y `[V14-D05]` (familias ampliadas).
...
Regla
  ...
  G-M24 protege a los productores con dos lados ([V11-D08], [V12-D09]):
    ...
    B) ProducerSiteCensus por familias de simbolos de API ([V13-D07], [V14-D05]), no solo por new sintactico o
       AppendEntity, en los sitios del Plugin y del camino de materializacion:
...
- No crea obligacion de tocar los drawers existentes (§20.3); G-M8 y G-M11 no cambian, y G-M24 vigila, sin modificarlos,
  a los productores declarados y, con su censo por familias de simbolos de API, todo call-site productor o consumidor
  nuevo del camino de materializacion, clonados, copias transformadas, explosiones y construccion por reflexion
  incluidos (`[V12-D09]`, `[V13-D07]`).
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           ...
           prerrequisitos globales de ST-20 (apertura parcial, REFEDIT, Editor de bloques) y estado de carga de los
           XREF ([V13-D01]..[V13-D03], [V14-D01])
...
PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  P0  prerrequisitos globales de ST-20             apertura parcial (Database.IsPartiallyOpened o el mecanismo
                                                   de CT-49), REFEDIT o transaccion larga activa (REFEDITNAME) o
                                                   Editor de bloques abierto (BLOCKEDITOR) ⇒ E12 con mensaje por
                                                   causa, antes de C1 y de la linea; no promete la completitud de
                                                   la cola por fuente; sin PARTIALOAD, REFCLOSE, BCLOSE ni cambios
                                                   ([V13-D01], [V13-D02], [V14-D01], [V14-D04])
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R6  enumerar de NUEVO todos los objetos de Model Space posteriores a cada fuente y capturar la
      ...
      prerrequisitos globales y decidir la completitud relativa a la fuente DEFINITIVA (ST-20; fallo ⇒ E10 antes
      de ST-19) · orden visual de plan (§11.3 punto 6.6) y ST-19 re-evaluados con las huellas refrescadas y con
      esa cola: decision DEFINITIVA (PRE-18, PRE-19, [V11-D05], [V13-D01], [V14-D04])
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           ...
           PRE-17, PRE-18) · prerrequisitos globales y completitud relativa a la fuente de ST-20 re-verificados
           (PRE-19, [V14-D04]) · enumeracion de NUEVO de la cola posterior a cada fuente frente a la
...
clasificable, ⇒ E10. Los prerrequisitos globales de ST-20 se comprueban en P0 y se re-verifican en R6 y al inicio de
MUTATE; la completitud relativa a la fuente se evalua de forma temprana en R2 y de forma definitiva en R6, antes de
ST-19 definitivo, y R3 solo puede rechazar. Con un prerrequisito global incumplido o con una cola no demostrablemente
completa, ST-19 no toma ninguna decision que permita mutar (`[V13-D01]`, `[V13-D02]`, `[V14-D01]`, `[V14-D04]`).
...
| RS-3 Dependencias transitivas | **solo si I-49 integra antes**: las observaciones de su `PlanReadSet` (resultado de cada simbolo realmente usado, incluidas las dependencias de expresiones) segun la **autoridad final** de I-49. **Hoy: PENDING I-49 FINAL AUTHORITY** (`[V14-D07]`): el Amendment A3 (`f6f0991`) cambia la semantica futura de D19 —`SymbolResultObservation` con todos los diagnosticos de un simbolo no evaluado (M2), su orden (M3), la cadena representativa de cada `DependencyFailed` (M4) y `RootCauses` (M10(a))— y el `Upstream` de D20 (M10(b)), con un ADR de reemplazo pendiente; I-52 adopta esas observaciones con la autoridad de I-49, sin calculo propio. Mientras la autoridad final no este disponible, **I-52 FREEZE = BLOCKED** (§14.1) | I-49 V6 + A1 + A2 + A3, ADR de reemplazo de ADR-0041 y Consensus Freeze nuevo, releidos antes del freeze de I-52; hoy vigente ADR-0041 (aceptado @ `2e8090a`, que reemplaza a ADR-0040; `[V11-D11]`), no apto para congelar RS-3 |
...
| E12 | Prerrequisito global de ST-20 incumplido o no demostrable en P0 (PRE-19; `[V13-D01]`, `[V13-D02]`, `[V14-D01]`, `[V14-D04]`): dibujo abierto parcialmente (`Database.IsPartiallyOpened` o equivalente), edicion de referencia en curso (`REFEDITNAME` no vacio, transaccion larga o equivalente) o Editor de bloques abierto (`BLOCKEDITOR = 1` o equivalente). E12 **no** cubre la completitud relativa a la fuente, que va en E10 | P0 | fin, sin linea y cero mutacion, con mensaje propio por causa («dibujo abierto parcialmente», «edicion de referencia en curso (REFEDIT): cierrela antes de reflejar», «Editor de bloques abierto: cierrelo antes de reflejar»), nunca un aviso; sin PARTIALOAD, REFCLOSE, BCLOSE, cargas forzadas ni cambios de capas, variables o sesion; si aparece despues de P0, E10 en R6 o E11 en MUTATE |
...
| PV-6 | Si I-49 integra antes (hoy: V6 y Amendments A1 y A2 congelados (`3f17c21`), ADR-0041 aceptado en lugar de ADR-0040; G5 sintactico `892a601`, antes `977f873` y `c4880af`, y G6 `988ab9c`, antes `5f969cc`, con enlace y evaluacion de un arbol en su rama, mas la correccion G6-C2 `75f1862`; sin integrar y sin `PlanReadSet` ni dominio aplicado; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`): RACKMIRROR invoca la autoridad efectiva integrada, preserva `expression` como portador, adopta su `PlanReadSet` para el read-set (§9.2.1) y su dominio de consumidor como cota autoritativa de V-DEP (§6.5.5), y **no** reimplementa su evaluacion ni un segundo motor de expresiones (P24.5 y P24.8 de I-49 V6). **Autoridad final (`[V14-D07]`):** la Amendment A3 (`f6f0991`, solo docs) cambia D19 (M2, M3, M4 y M10(a)) y D20 (M10(b)) y pide un ADR de reemplazo y un Consensus Freeze nuevo; antes del freeze de I-52 se relee la autoridad final de I-49 (V6, A1, A2, A3, ADR de reemplazo, freeze nuevo y estado de implementacion) y, si no esta disponible, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED |
...
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`, `[V13-D01]`..`[V13-D05]`).** Autoridad
     ...
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       ...
       UNKNOWN:
         ...
         sesiones         REFEDIT o transaccion larga (REFEDITNAME) · Editor de bloques (BLOCKEDITOR); P0
                          ([V14-D01])
       ...
       frontera ([V13-D06], [V14-D02], [V14-D03]): estado dentro (visibilidad, carga, resolucion al mismo contenido,
         carga bajo demanda, recortes, solo visibilidad dinamica, campos, DataLink, proxies, aislamiento, anotacion
         y entorno);
         edicion fuera (ERASE/OOPS, geometria, redefinicion, texto, mover, parametro dinamico que cambia geometria,
         XREF editado o a otro contenido, fuente de imagen, underlay o nube de puntos reemplazada)
   ...
   - **Completitud de la base de datos y de la enumeracion (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`, `[V14-D01]`,
     `[V14-D04]`).** Tiene dos partes. Los **prerrequisitos globales** se comprueban en P0, antes de conocer las
     fuentes: la base de datos no esta abierta parcialmente, no hay una edicion de referencia en curso (`REFEDITNAME` no
     vacio, transaccion larga o estado equivalente) y el Editor de bloques no esta abierto (`BLOCKEDITOR = 1` o
     equivalente); fallo ⇒ E12, y se re-verifican en R6 y al inicio de MUTATE. La **completitud relativa a la fuente**
     se evalua despues de clasificar las fuentes y de la LINE: R2 enumera de forma temprana la cola posterior a cada
     fuente y las entradas de huella, R3 solo puede rechazar, y R6 re-enumera y decide la completitud definitiva —toda
     la cola en el orden efectivo y el contenido relevante de cada objeto enumerado (definicion referenciada, anidados,
     ...
     edicion futura (`[V14-D02]`). RACKMIRROR no ejecuta PARTIALOAD, REFCLOSE ni BCLOSE, no fuerza cargas, no altera
     capas, no cambia variables del dibujo y no muta ninguna sesion.
   ...
   - **Campos (`[V12-D06]`).** Si la ocupacion de modelo de un objeto depende de la evaluacion de un campo (`DBText`,
     `MText`, atributos, celdas de tabla u otras clases con campos que se descubran), es `SUPPORTED` solo con una cota
     conservadora valida para **todos** los resultados que admite el contrato; si no ⇒ `UNKNOWN`. El valor actual o
     `FIELDEVAL` no son autoridad suficiente.
   ...
   - **Frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`).** **ESTADO, dentro de la garantia:**
     visibilidad (capas, `Entity.Visible` y representaciones anotativas); carga y descarga de XREF; resolucion de un
     XREF al **mismo** contenido referenciado; carga bajo demanda; recortes, con sus marcos; **solo** la visibilidad
     dinamica; evaluacion de campos; refresco de `DataLink`; visualizacion de proxies; aislamiento de objetos; y estado
     de visualizacion de anotacion y de entorno. **EDICION FUTURA, fuera:** ERASE/OOPS y la restauracion programatica de
     objetos borrados; edicion de geometria; BEDIT o redefinicion de bloque; edicion manual de texto; mover o estirar;
     un parametro dinamico que cambia la geometria; editar el archivo externo de un XREF y recargarlo; resolver un XREF
     a **otro** archivo o contenido; y reemplazar la fuente o el contenido de una imagen, un underlay o una nube de
     puntos. **Precedencias:** el refresco de `DataLink` sigue siendo ESTADO aunque su fuente externa haya cambiado, y
     el contrato exige una cota conservadora para todos sus resultados o `UNKNOWN`; una operacion dinamica (tabla de
     busqueda o parametro combinado) que cambia la geometria ademas de la visibilidad es EDICION. CT-49 puede
     caracterizar la restauracion del handle y la posicion en el orden tras OOPS, pero eso no la incorpora a la
     garantia. RACKMIRROR no garantiza la invariancia del orden de dibujo frente a ediciones futuras arbitrarias (L-37).
...
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V15.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V14 no introduce tolerancia; CT-36 registra
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V14 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V14 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V14 no introduce una tolerancia nueva: CT-36
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Metodo de inventario cerrado (`[V9-D08]`, `[V12-D01]`, `[V13-D01]`, `[V13-D04]`, `[V13-D05]`).** CT-36 (contenido
    ...
    por tipo exacto en ejecucion:
      ...
      antes de clasificar ([V13-D01], [V13-D04], [V14-D01], [V14-D04]): prerrequisitos globales en P0 (apertura
        parcial, REFEDIT o transaccion larga, Editor de bloques) y completitud relativa a la fuente en R2 y R6 (cola
    ...
    **STOP → Proposal V15**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V14 abre **Proposal V15**.
...
| **T-M75** | Censo de sitios productores de G-M24 por familias de simbolos de API (`[V12-D09]`, `[V13-D07]`, `[V14-D05]`): un call-site inventado que construye una entidad relevante, la añade con `AppendEntity` o `AddNewlyCreatedDBObject`, da de alta un registro en una tabla de simbolos, reevalua graficos, añade un atributo, invoca un comando o COM, o delega la creacion en un helper nuevo, sin declararse ni excluirse con motivo, pone G-M24 en RED. **Dos violaciones artificiales obligatorias:** (1) **un productor basado en clonado** (`DeepCloneObjects`, `WblockCloneObjects` o `Clone` hacia el camino de materializacion) sin clasificar ⇒ RED; (2) **una lectura de cabecera o de contexto de `Database`** (`Database.Dimstyle`, `Clayer`, `Textstyle` o equivalente) sin clasificar ⇒ RED. Un productor declarado que cambia lo que asigna, usa o crea tambien; la guarda no depende de los nombres actuales de archivos o clases ni de hashes | G5..G7 |
...
| **G-M24** | Guarda durable de productores de contexto (`[V11-D08]`, `[V12-D09]`, `[V13-D07]`, `[V14-D05]`), estructural y no solo de hashes de archivos, con **dos lados**: (A) productores y consumidores de contexto declarados (`LateralHeaderDrawer`, `CantileverViewMaterializer`, `LayerHelper` y todo productor que identifique CT-50), con las propiedades visuales que asigna cada uno, el uso de capas, estilos y valores por defecto, las clases de entidad que crea y la resolucion de estilos; (B) `ProducerSiteCensus` por familias de simbolos de API, no solo por `new` sintactico o `AppendEntity`, en los sitios del Plugin y del camino de materializacion: **construccion y alta** (subclases relevantes de `Entity`, `AppendEntity`, `AddNewlyCreatedDBObject`, `Activator.CreateInstance` y construccion por reflexion); **importacion y clonado** (`ReadDwgFile`, `Database.Insert`, `Wblock`, `WblockCloneObjects`, `DeepCloneObjects`, `Clone`, `GetTransformedCopy`, `Explode`, `ExplodeToOwnerSpace`); **tablas de simbolos** (`BlockTable.Add`, `LayerTable.Add`, `LinetypeTable.Add`, `TextStyleTable.Add`, `DimStyleTable.Add`, `SymbolTable.Add` y patrones de insercion de registros); **cabeceras y valores por defecto de `Database`** (`Dimstyle`, `Clayer`, `Cecolor`, `Celtype`, `Textstyle`, `Cannoscale`, `GetDimstyleData`, propiedades equivalentes, `GetSystemVariable` y `SetDatabaseDefaults`); **evaluacion grafica** (`RecomputeDimensionBlock`, `EvaluateHatch`, `GenerateLayout`, `RecordGraphicsModified` y `TransformBy` donde sean relevantes); **atributos** (`AttributeCollection.AppendAttribute`, `SetAttributeFromBlock`); **comando y COM** (`Editor.Command`, `SendStringToExecute`, `ModelSpace.Add*`, `InsertBlock` y envoltorios equivalentes); lectura o resolucion de capas, estilos, valores por defecto o contexto de materializacion; y delegacion en helpers o factorias, cada call-site relevante `DECLARED` o `EXCLUDED` con motivo. Un call-site productor o consumidor nuevo sin clasificar, o un productor declarado que empieza o deja de asignar una propiedad visual, cambia ese uso, crea una clase relevante nueva o cambia la resolucion de estilos, pasa a RED hasta actualizar CT-50, el `MaterializationContextReadSet`, el `FootprintInputFingerprint` y las pruebas (T-M75); no depende solo de los nombres actuales de archivos o clases ni de hashes |
...
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55 ↔ I-49, con las ultimas entradas X-*
                                                          ...
                                                          de G2C aceptada del lado de I-55, con las diferencias
                                                          de su Proposal V3 pendientes de acordar, §15.4;
                                                          registra la propiedad de las
                                                          autoridades compartidas, todas PROVISIONAL UNTIL
                                                          CROSS-INITIATIVE RECONCILIATION en V14: taxonomia de
...
→ autoridad FINAL de I-49 disponible para RS-3            (V6 + A1 + A2 + A3, ADR de reemplazo de ADR-0041,
                                                          Consensus Freeze nuevo y estado de implementacion,
                                                          releidos; [V14-D07])
...
Proceso de I-52 ([V12-D13], [V13-D10], [V13-D14], [V14-D13]): I-52 esta grandfathered y sigue el workflow bajo el que se
reclamo. Antes de cada transicion se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen
junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de
reclamo, salvo que una autoridad de proceso integrada cambie expresamente las reglas de transicion aplicables a las
iniciativas grandfathered. Nunca se adoptan reglas de I-56 no integradas
```
...
### 15.1 main y paralelas observadas en el preflight de V14
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ...
                                                     ancla de las citas de V14: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = dd45b0ff28d3febcb79aaee141dbb0d3e01168d3 (V13; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = f6f0991fa5f9a1f97817ca403bd21568f72c0aaa (Amendment A3 de V6,
                                                     solo docs, CI 34875190797 success, sobre G6-C2 75f1862, que
                                                     cerro G6; cambia D19 (M2, M3, M4, M10a) y D20 (M10b) de
                                                     ADR-0041; REPLACEMENT ADR REQUIRED, sin numero; freeze
                                                     3f17c21 historico para G7; Arquitecto de I-49 PENDING sobre
                                                     A3 exacto; G7 BLOCKED; sin cambios desde V13; sin integrar)
I-55 feature/creacion-de-vistas                    = 110cd39 (View Placement & Projection, G2D: Proposal V3
                                                     8e35a51 y paquetes de revision V3; Coordinador de I-55 REVIEW
                                                     REQUIRED sobre V3; Arquitecto formal PENDING; G2C reclasificada
                                                     como revision tecnica adversarial; tabla X-1..X-8 aceptada del
                                                     lado de I-55 y diferencias de V3 sin adoptar; ADR-0042
                                                     propuesto; CI 34881680833 success; solo docs; sin integrar)
I-56 docs/initiative-workflow-v2                   = 0d66df26882cb5cdd2efc21155c121f58d2af669 (Initiative Workflow V2:
                                                     G1 Evidence Audit c6839e2 y correccion G1.1 0d66df2, solo docs,
                                                     CI 34877304129 y 34879848606 success; Evidence Audit COMPLETE;
                                                     Proposal NOT STARTED; WORKFLOW V2 NOT EFFECTIVE; I-49, I-52 e
                                                     I-55 grandfathered; sin integrar)
...
**main desde la publicacion de V13.** Sin cambios: `dad4e77`. Las citas de codigo nuevas de V14
(`LateralHeaderDrawer.cs:244`, `:375` y `:391`; `LayerHelper.cs:26`; `CantileverViewMaterializer.cs:48`, `:111` y
`:195`) son identicas en `1b091be` y en `dad4e77`.
...
**Cambios de paralelas desde la publicacion de V13.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V14 declaraba
como ultimos observados main `dad4e77`, I-52 `dd45b0f`, I-49 `f6f0991`, I-55 `d091eeb` e I-56 `0d66df2`; el preflight de
V14 los confirmo, con ADR-0041 aceptado, ADR-0042 propuesto, sin ramas nuevas, produccion nueva ni numeros de ADR
reservados, y el re-fetch previo al commit encontro los mismos tips salvo I-55 en `110cd39` (Proposal V3 y paquetes de
revision, solo docs) (`[V14-D14]`). La historia de paralelas anterior esta en Proposal V13 §15.1, en Proposal V12 §15.1,
en Proposal V11 §15.1, en Proposal V10 §15.1, en Proposal V9 §15.1, en Proposal V8 §15.1, en Proposal V7 §15.1 y en el
registro §42, §48, §54, §60, §66, §72 y §78. La numeracion de ADR esta en §16.
...
| **I-49** (V6 y Amendments A1 y A2 congelados en `3f17c21`; ADR-0041 **aceptado**, que reemplaza a ADR-0040, que a su vez reemplazo a ADR-0038; rama rebasada sobre `dad4e77`: G5 sintactico `892a601`, G6 `988ab9c` productivo y la correccion G6-C2 `75f1862`, que cierra G6, y el Amendment A3 `f6f0991`, solo docs, que cambia D19 y D20, pide un ADR de reemplazo de ADR-0041 sin numero y deja el freeze historico para G7; sin integrar) | `expression` en `PropertyValues`; `PlanReadSet` (D19); dominio del consumidor declarado por el descriptor (P24.5); su freeze exige que la prueba de supervivencia de `expression` (P24.8) se escriba contra `RackDuplicationPlan` y el restamp vigentes en main; guardas de texto evolucionadas en G5 y, en G6 y G6-C2, `LengthUnitsAuthorityGuardTests` sobre todo `src/` y las guardas del nucleo; ADR-0041 como sucesor aceptado de ADR-0040 (cambian solo D5..D8: validez de la clave, formas de referencia, sintaxis del cualificador y redaccion del recuento de tokens; D19, D20 y D25 identicos) | **I-52 coordina con I-49 antes de tocar `RackDuplicationPlan.cs`** (G4); T-M19. Si I-49 integra antes: su `PlanReadSet` es RS-3 (§9.2.1), su dominio es la cota `b` de V-DEP, su descriptor se clasifica en la guarda (§6.7) y sus guardas de texto rigen (G-M18, CT-46). `8ed15f7` es **NON-MATERIAL** (`[V8-D18]`) y `5f969cc` tambien lo es para el contrato, sin `PlanReadSet` ni dominio (`[V9-D21]`, `[V10-D14]`); A2 con ADR-0041 aceptado y la correccion G6-C2 solo cambian la validez de la clave y la sintaxis del cualificador de `VariableId`, sin tocar lo que I-52 consume (`[V11-D11]`, `[V12-D12]`): las referencias de I-52 a ADR-0038 y a ADR-0040 se leen en ADR-0041, con D19 identico. A3 cambia la semantica futura de D19 —la `SymbolResultObservation` del `PlanReadSet`: M2, M3, M4 y M10(a), con `RootCauses`— y el `Upstream` de D20 (M10(b)). Para el contrato actual es **NON-MATERIAL** solo porque RS-3 delega en la autoridad final integrada de I-49; para la reconciliacion es **MATERIAL**: antes del freeze de I-52 se relee esa autoridad final y, si no esta disponible, RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED (`[V14-D07]`). V13 **no** presupone el motor |
...
| **I-55** (View Placement & Projection; rama `feature/creacion-de-vistas` @ `110cd39`, sobre `dad4e77`; Proposal V3 `8e35a51` con Coordinador de I-55 REVIEW REQUIRED y Arquitecto formal PENDING; revision de G2C reclasificada como revision tecnica adversarial; tabla X-1..X-8 de G2C aceptada del lado de I-55 y diferencias de V3 sin adoptar; Open Material M-01; Owner PENDING; ADR-0042 propuesto; solo docs) | ID17, ID18 e ID19 sobre una foundation de preparacion de vistas antes de materializar y una colocacion de grupo con una transformacion rigida comun; §17.2 de su Proposal V2 reconcilia con I-52 en X-1..X-8: nucleo neutral de seleccion (X-1); plan desde el sistema resuelto y codec de `View`/`Section` neutral con politica por consumidor (X-2); comparador authored por kind (X-3); primitivo «definicion + sobre + referencia» con las politicas de I-52 (ST-13, ST-19, PRE-17, PRE-18, read-set) fuera del primitivo (X-4); reporte antes de fijar archivos o autoridades que I-52 vigila, que la Proposal V2 ya dispara (X-5); re-establecimiento de C-2 por la segunda en integrar (X-6); valor de colocacion sobre `Transform2D` con rigidez de I-55 y reflexion de I-52 (X-7); un descriptor de origen y tramo del eje por vista (X-8); la revision G2C de I-55 los acuerda los ocho (§15.4). ADR-0042 complementa ADR-0010 (vista adicional; dos precondiciones nuevas de Insertar; Actualizar sin cambio) y su Proposal V3 lo registra como complemento de ADR-0010, que sigue aceptado, con nota posterior fechada solo si se acepta; V3 §17.2 propone diferencias en X-1, X-2, X-4, X-5, X-7 y X-8 (§15.4) | arquitectura hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**; I-52 **no** espera al consenso de I-55. Tras el consenso tecnico sobre V14 y el rebase, **reconciliacion obligatoria antes del freeze** de la propiedad de las autoridades compartidas, con la ultima tabla X-1..X-8 acordada como entrada vinculante y las diferencias de la Proposal V3 de I-55 por decidir (§15.4), y marcadas como provisionales en V14 (taxonomia de tipo de vista, codec de `View`/`Section`, «sistema resuelto → plan de vista» con origen y tramo del eje, comparador authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, nucleo de seleccion), sin autoridades duplicadas; siguen siendo de I-52 ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`; I-55 aplica la misma regla y el STOP simetrico es mutuo; si ADR-0042 se acepta antes del freeze de I-52, se releen las autoridades de C-2, Actualizar e Insertar; la segunda iniciativa en integrar re-establece C-2 (R-45, `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`) |
| **I-56** (Initiative Workflow V2; rama `docs/initiative-workflow-v2` @ `0d66df2`, sobre `dad4e77`; G1 Evidence Audit `c6839e2` y correccion G1.1; Evidence Audit COMPLETE; Proposal NOT STARTED; solo documentacion y proceso; I-49, I-52 e I-55 grandfathered; WORKFLOW V2 NOT EFFECTIVE) | puede cambiar el proceso de iniciativas (WORKFLOW, AGENTS, AUTOMATION_PLAN, `TEMPLATE.md` y afines) cuando su Proposal lo fije e integre; hoy solo toca su contrato, su fila de ROADMAP y el Evidence Audit, que no es normativo | **NON-MATERIAL** hoy; el WORKFLOW vigente sigue siendo la autoridad hasta que I-56 integre y, aun integrada, su invariante de transicion deja a I-52 bajo el workflow de su reclamo y ninguna politica de la V2 rige sin acuerdo tecnico y aprobacion del Owner; antes de cada transicion del proceso de I-52 se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo; nunca se adoptan reglas no integradas ni hipotesis del Evidence Audit (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`) |
...
- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  guardas de texto (CT-46). ADR-0041 esta aceptado y reemplaza a ADR-0040: las referencias de I-52 se leen en ADR-0041,
  con D19 y el dominio del consumidor sin cambio (`[V8-D18]`, `[V11-D11]`). Su G6 (`988ab9c`, antes `5f969cc`) añade el
  nucleo semantico y la autoridad de unidades con `LengthUnitsAuthorityGuardTests` sobre todo `src/`, y la correccion
  G6-C2 (`75f1862`) ajusta el cualificador de clave exacta y las guardas del nucleo solo en `Expressions`: al
  reconciliar, CT-46 las relee y G-M18 las aplica a los archivos de I-52. El Amendment A3 (`f6f0991`, solo docs) cambia
  D19 (M2, M3, M4 y M10(a)) y D20 (M10(b)) y pide un ADR de reemplazo de ADR-0041 sin numero y un Consensus Freeze
  nuevo. **Antes del freeze de I-52** se relee obligatoriamente la autoridad final de I-49 —V6, A1, A2, A3, ADR de
  reemplazo, Consensus Freeze nuevo y estado de implementacion— y, si no esta disponible, RS-3 = PENDING I-49 FINAL
  AUTHORITY e I-52 FREEZE = BLOCKED (`[V14-D07]`); el censo de §16 verifica el numero del ADR de reemplazo. Si un gate
  posterior de I-49 introduce `PlanReadSet` o dominio aplicado, o toca ProjectVariables, Persistence o Systems, reportar
  antes de continuar (`[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V13-D15]`, `[V14-D07]`).
...
- **I-55 (`feature/creacion-de-vistas` @ `110cd39`, View Placement & Projection, Proposal V3 en revision con Arquitecto
  formal PENDING, tabla X-1..X-8 de G2C aceptada del lado de I-55 y diferencias de V3 sin adoptar, solo docs; gate de
  coordinacion activo, `[V14-D14]`, `[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`):** I-52 no espera a
  su consenso, pero no congela sin la reconciliacion obligatoria posterior al consenso tecnico sobre V14 y al rebase,
  con X-1..X-8 como entrada vinculante (§15.4), que registra la propiedad de las autoridades compartidas que V14 marca
  como provisionales —taxonomia de tipo de vista, codec semantico de `View`/`Section`, autoridad «sistema resuelto →
  plan de vista» con origen y tramo del eje de la vista, comparador authored, primitivo de materializacion del Plugin,
  valor de colocacion y transformaciones de proyeccion, y nucleo de seleccion (X-1..X-8)—, sin autoridades duplicadas.
  Siguen siendo de I-52, aunque consuman un primitivo compartido, ST-13, ST-18, ST-19, PRE-17, PRE-18,
  `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`. Si cualquiera de las dos intenta
  congelar una autoridad incompatible antes de reconciliar: STOP, ningun freeze para ninguna (R-45, §19); I-55 V2 aplica
  la misma regla a su Consensus Freeze. G4 avisa a I-55 antes de tocar la fundacion compartida (nucleo de seleccion en
  `RackDuplicationPlan.cs`). Si ADR-0042 cambia de estado, se re-evalua antes del siguiente commit y, si se acepta antes
  del freeze de I-52, se releen las autoridades que usan C-2, Actualizar e Insertar. Cuando un gate de I-55 toque rutas
  de materializacion, `RackEmbedComposer.Compose`, `SystemBlockWriter.cs`, comandos o `RackCommandReference.cs` (hoy
  previsto en sus G5..G15, X-5), se re-evaluan §8.1, §8.7, `MaterializationContextReadSet`, PRE-17, PRE-18, T-GRD-02 y
  el censo `B`; la segunda iniciativa en integrar re-establece C-2 sobre el main combinado. Si I-55 publica una
  Proposal V3 o cambia algun X, se relee antes del siguiente commit y la reconciliacion parte de la ultima version
  acordada. Las ubicaciones fisicas de §14.2 G4..G7 que tocan X-1..X-8 siguen provisionales (`[V14-D08]`).
...
Arquitecto. La tabla X-1..X-8 queda **aceptada del lado de I-55** por su Coordinador, sin MATERIAL CONFLICT con I-52; el
Arquitecto formal de I-55 sigue PENDING y su Owner tambien. V13 los registra como **entrada vinculante** del gate de
reconciliacion posterior al consenso tecnico y al rebase (§14.1): la reconciliacion parte de las ultimas entradas X-*
acordadas, y una desviacion se vuelve a acordar con I-55 antes del freeze, nunca de forma unilateral. V13 **no** adopta
todavia los requisitos del lado de I-52 ni congela namespace, ubicacion ni forma de API; la politica propia del espejo
sigue siendo de I-52.
...
con la tabla anterior, que **no** estan acordadas y que V14 **no** adopta: X-1 con primer gate simetrico (I-52 G4 o I-55
G13); **X-2 y X-8 con extractor unico I-55** (G6, G7a y G7b), lo que acoplaria G4..G6 de I-52 a esos gates o exigiria
una excepcion registrada de §14.2 y §20.3; X-4 con un requisito estructural puro de bloques
...
sigue siendo la ultima acordada y las ubicaciones fisicas siguen provisionales (`[V14-D08]`).

**Contraste con V14.** Ningun X presenta contradiccion material con V14: V14 no fija la propiedad ni la ubicacion fisica
de ninguna autoridad compartida, mantiene ST-20, PRE-19 y E12 —ahora tambien con las sesiones de edicion— como politica
propia del espejo fuera del primitivo (X-4) y amplia la marca provisional a las secciones que esos requisitos nombran y,
desde V14, a las ubicaciones de §14.2 G4..G7 (`[V14-D08]`). Si I-55 publica una Proposal V3 o cambia algun X antes de la
reconciliacion, se relee y la reconciliacion parte de la ultima version acordada.
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V14.
- Congela: espejo semantico por copia; `μ_k` canonica por kind; admisibilidad por vista con seccion canonica y
  ...
  enumeracion completas, huellas visuales de ocupacion de modelo en el contexto destino y refrescadas tras
  `EnsureForPlan`, y read-set del contexto de materializacion; obligacion 9b y V-DEP; allowlist de portadores y metadata
  exterior; cierre transitivo de cobertura; C-2 con Insertar; verificacion dinamica por rack; taxonomia en tres
  dimensiones con regla de cierre de G3; alcance dependiente de la biblioteca efectiva y seleccion todo-o-nada.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V14 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   ADR-0010, que sigue aceptado, con una nota posterior fechada en ADR-0010 solo si el Owner acepta ADR-0042. El
   Amendment A3 de I-49 (`f6f0991`) anuncia un ADR de reemplazo de ADR-0041 sin numero asignado —cambia D19 y D20—, que
   se censara frente a 0036 cuando se publique; en el preflight de V14 no hay numeros reservados publicados. I-56 no
...
no contradice materialmente V14, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V15. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V15 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V15 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V15 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V15 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V15 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V15 |
...
| **R-52** | ST-20 (`[V13-D01]`..`[V13-D04]`, `[V14-D01]`, `[V14-D04]`) puede hacer fallar RACKMIRROR en dibujos abiertos parcialmente, con una edicion de referencia en curso, con el Editor de bloques abierto o con XREF cargados bajo demanda con indices, que pueden aparecer en dibujos corrientes con el `XLOADCTL` por defecto; y mientras CT-49 no confirme las API exactas ningun prerrequisito se demuestra | E12 atribuible por causa antes de pedir la linea y E10 que nombra el XREF; CT-49 caracteriza las API, las sesiones y los estados de `XLOADCTL` e `INDEXCTL` antes de cualquier implementacion; declarado en O-1 (L-34); reduccion material ⇒ Proposal V15 y O-1 se reevalua |
| **R-53** | El censo de G-M24 por familias de simbolos de API ampliadas (`[V13-D07]`, `[V14-D05]`) puede clasificar muchos call-sites ajenos al camino de materializacion (por ejemplo, `TransformBy`, lecturas de cabecera de `Database` o altas en tablas de simbolos en otros comandos) | «donde sea relevante» y `EXCLUDED` con motivo; el censo inicial lo fija CT-50 con sus anclajes minimos y la forma concreta se decide en G5..G7 sin tocar los drawers |
| **R-54** | La frontera entre estado y edicion (`[V13-D06]`, `[V14-D02]`, `[V14-D03]`) deja fuera las ediciones futuras (ERASE/OOPS, un XREF editado y recargado o resuelto a otro contenido, una redefinicion, un texto editado o una tabla de busqueda que cambia geometria pueden quedar por encima o por debajo de la copia), mientras que el refresco de `DataLink` queda dentro y una tabla con `DataLink` sera en la practica `UNKNOWN` | limitacion declarada en O-1 (L-37); E10 atribuible para los datos externos sin cota; una garantia frente a ediciones seria un cambio de alcance para una Proposal posterior |
| **R-55** | La autoridad final de I-49 (V6 + A1 + A2 + A3, ADR de reemplazo y Consensus Freeze nuevo) puede no estar disponible cuando I-52 este lista para congelar, lo que bloquea el freeze de I-52 aunque I-49 no vaya a integrar antes (`[V14-D07]`) | RS-3 = PENDING I-49 FINAL AUTHORITY e I-52 FREEZE = BLOCKED; se reporta al Coordinador en la reconciliacion; ninguna congelacion sobre ADR-0041 D19 obsoleto |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10, V11, V12, V13 y V14 | §11.3, §6.4, §6.5.5, §6.7 |
...
- **I-56 (`[V12-D13]`, `[V13-D10]`, `[V14-D10]`).** Vigilancia de proceso, sin adoptar reglas no integradas ni hipotesis
  del Evidence Audit (G1.1 `0d66df2`); su contrato declara a I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; las futuras
  autoridades de proceso se leen con sus reglas de transicion y, mientras el invariante aplique, I-52 conserva su
  workflow de reclamo.
- **Terminologia (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`, `[V13-D16]`).** Huella de la referencia, huella visual,
  `FootprintInputFingerprint`, `TraceFingerprint`, `ResolvedPresentationEligibility`, `ModelSpaceFootprintPolicy`,
  `ModelSpaceOrderTailSnapshot`, `ContentAvailability`, `VisibilityMechanism`, `ProducerSiteCensus` y
  `DatabaseVisibilityCompleteness` son autoridades o registros distintos (§9.2).
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V15 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V14 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V14 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V15 y O-1 se reevalua.
...
- **ADR-0041 aceptado en I-49 (`[V11-D11]`, `[V12-D12]`, `[V13-D15]`, `[V14-D07]`).** Reemplaza a ADR-0040, cambia solo
  D5..D8 y conserva lo que I-52 consume hoy (D19 `PlanReadSet` y el dominio del consumidor); la correccion G6-C2
  (`75f1862`) solo toca `Expressions`. ADR-0041 es la autoridad vigente, pero **no** apta para congelar RS-3: el
  Amendment A3 (`f6f0991`) cambia la semantica futura de D19 —`SymbolResultObservation` del `PlanReadSet`: M2, M3, M4 y
  M10(a)— y el `Upstream` de D20 (M10(b)). Corrige V13, que presentaba A3 como un cambio de D20 que no tocaba el
  contrato de I-52: el contrato actual sigue NON-MATERIAL solo porque RS-3 delega en la autoridad final integrada de
  I-49, y antes del freeze de I-52 esa autoridad final debe estar disponible y releida o RS-3 = PENDING I-49 FINAL
  AUTHORITY e I-52 FREEZE = BLOCKED. D20 es la `RepairDecisionObservation` de `RepairBrokenRack`, que I-52 no usa.
- **I-55 (`[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`, R-45).** I-52 no espera al consenso de I-55,
  ...
  complementa ADR-0010 sin cambiar Actualizar. V13 marca esas autoridades como provisionales, registra X-1..X-8 como
  entrada vinculante y no decide el reparto.
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V15** o reduccion
  de alcance.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V15.
...
G2 = V14 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd), V10 (636f7fd), V11 (2275f21), V12 (915a520) y V13 (dd45b0f) = historial
     revision de V13: Architect: CHANGES REQUIRED — PROPOSAL V14
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V14, rebase sobre main, reconciliacion con I-55 e I-49 con las ultimas
                     entradas X-* acordadas, autoridad final de I-49 disponible para RS-3, freeze, re-check y O-1;
                     antes de cada transicion, relectura de la autoridad de proceso vigente con sus reglas de
                     transicion; I-52 grandfathered)
