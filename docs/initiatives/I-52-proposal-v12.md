# I-52 — Proposal V12: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V12 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V12 se redacta SIN rebase, sobre 2275f21)
...
> Proposal V1..V10  = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd ·
>                     636f7fd (historial)
> Proposal V11      = 2275f21b88409f5ee86b919317a5eb5efd5e213e   (historial; CI 34861084313 4/4)
> Revision Arq. V11 = «Architect: CHANGES REQUIRED — PROPOSAL V12»; sin BLOCKER ni HIGH; MEDIUM AR11-01 (huella
>                     latente y ModelSpaceFootprintPolicy cerradas por clase pero no por mecanismo de ocultacion,
>                     carga o recorte: XREF descargado o no resuelto, filtros espaciales, recortes, estados de
>                     visibilidad dinamica y campos); LOW AR11-02..AR11-07; cambios exactos V12-01..V12-08 de la
>                     revision (registro: docs/automation/decisions/I-52.md §68)
> Coordinador       = orden de V12 con V12-01..V12-16 vinculantes (registro §69)
> main y paralelas  = §15.1 (preflight de V12 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V12 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V12 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V12 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V11 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V11, sus revisiones de Arquitecto y esta V12 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V12 la registra
...
> ventana de ayuda de comandos y sus censos) y leyeron los ADR y las Proposals de las paralelas.

---

## 0. Reconciliacion V11 → V12

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`), V11 (`2275f21`) y el Discovery (`339b3ab`) **se conservan sin cambios**.
**Donde V11 y V12 difieran, manda V12.** Las marcas `[V11-D01]`..`[V11-D19]` siguen vigentes salvo donde una marca
`[V12-Dnn]` las modifica, reabre o sustituye (§0.4); las marcas `[V10-Dnn]`, `[V9-Dnn]`, `[V8-Dnn]`, `[V7-Dnn]`,
`[V6-Dnn]` y `[V5-Dnn]` siguen como las dejaron V11, V10, V9, V8, V7 y V6 (§0.5 a §0.10).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V12** | V12 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V12. **Debe ser NONE** |

### 0.2 Matriz `[V12-D01]`..`[V12-D20]`

En la columna «Origen»: `AR11-nn` es el hallazgo de la revision de V11; `R:V12-nn`, su cambio exacto; `C:V12-nn`, la
precision vinculante del Coordinador en la orden de este gate. Las dos series V12-nn se numeran por separado. **Donde el
cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del
Coordinador**: la revision agrupaba en un solo cambio (R:V12-01) la huella latente por mecanismo; el Coordinador la
divide en seis decisiones (mecanismo, inventario obligatorio, XREF, recortes, visibilidad dinamica y campos), fija que
un XREF descargado, no resuelto o con contenido anidado no disponible es `UNKNOWN` en el primer corte, sin abrir
archivos externos, y extiende la regla de entorno a la copia con casos obligatorios de CT-50; ademas añade la vigilancia
de I-56 y la relectura del WORKFLOW vigente antes de cada transicion del proceso.

| Marca | Origen | V11 | Correccion V12 | Estado | Donde |
|---|---|---|---|---|---|
| `[V12-D01]` | AR11-01 · R:V12-01 · C:V12-01 | Huella de orden latente definida por una lista cerrada de estados de visibilidad (capa apagada, inutilizada o inutilizada en ventana, `Entity.Visible = false`, `ANNOALLVISIBLE`); `UNKNOWN` solo sin huella independiente de «esos estados» | **LATENT FOOTPRINT BY MECHANISM**: la huella de orden representa **todo** contenido que pueda volverse visible **sin cambiar** la relacion de orden de dibujo. La `ModelSpaceFootprintPolicy` sigue clasificando por tipo exacto en tiempo de ejecucion y ahora clasifica ademas, de forma obligatoria, por **`ContentAvailability`** (contenido cargado, descargado, no resuelto o no disponible) y por **`VisibilityMechanism`** (mecanismo por el que el contenido puede estar oculto, no cargado o recortado hoy y reaparecer despues). Para declarar `SUPPORTED(metodo)` hay que cubrir la clase, sus componentes y todos esos mecanismos; un mecanismo no clasificado ⇒ huella `UNKNOWN` ⇒ E10 si el objeto es posterior. **Nunca** «contenido ausente hoy = huella vacia» si puede reaparecer sin cambiar el orden. El metodo de inventario cerrado de §11.3 punto 14 se extiende a la definicion referenciada y a los componentes de los objetos de Model Space, con resultado E10 | CLOSED BY V12 (contrato); inventario real: DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-19; §11.3 puntos 5 y 14; CT-49; T-M73; G-M23; M-35; L-34; ADR-0036 decision 11 |
| `[V12-D02]` | AR11-01 · R:V12-01 · C:V12-02 | Inventario minimo de CT-49 por clase («`BlockReference` con sus atributos»), sin mecanismos | **REQUIRED MECHANISM INVENTORY**: CT-49 inventaria como minimo **visibilidad** (capa apagada, capa inutilizada, inutilizada en ventana, `Entity.Visible`, representaciones anotativas y `ANNOALLVISIBLE`); **XREF** (cargado, descargado, no resuelto, overlay y disponibilidad de anidados descargados); **`BlockReference`** (estados de visibilidad dinamica, `AttributeCollection` y disponibilidad de la definicion referenciada); **filtros espaciales** (XCLIP, `ACAD_FILTER` y otros filtros soportados); **raster, underlay y nube de puntos** (limites de recorte, capas internas y controles de visualizacion, disponibilidad del contenido o estado descargado); **campos** (`DBText`, `MText`, atributos, celdas de tabla y otras clases con campos que se descubran). No es obligatorio soportarlos todos: cada caso es `SUPPORTED(metodo conservador)` o `UNKNOWN` | CLOSED BY V12 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §11.3 punto 5; CT-49; T-M73 |
| `[V12-D03]` | AR11-01 · R:V12-01 · C:V12-03 | XREF sin politica: un aplanado del contenido cargado podia dar una huella vacia | **XREF POLICY** (primer corte): XREF descargado ⇒ `UNKNOWN`; XREF no resuelto ⇒ `UNKNOWN`; overlay anidado o contenido no disponible ⇒ `UNKNOWN`; XREF cargado ⇒ `SUPPORTED` solo si CT-49 demuestra una huella latente conservadora que incluya el contenido anidado relevante, las capas del XREF, sus mecanismos de recorte y sus dependencias de presentacion. RACKMIRROR **no** abre archivos externos para adivinar geometria ausente. Objeto posterior `UNKNOWN` ⇒ E10 | CLOSED BY V12 (contrato); XREF cargado: DEFERRED TO G3 WITH FAIL-CLOSED | §11.3 punto 5; CT-49; T-M73; M-35; L-34 |
| `[V12-D04]` | AR11-01 · R:V12-01 · C:V12-04 | Un metodo que respetara un recorte podia excluir geometria latente | **CLIPPING POLICY**: un recorte **no** reduce la huella de orden latente si quitarlo o cambiarlo puede hacer reaparecer contenido sin cambiar el orden. Para XCLIP, filtros espaciales, recortes de imagen, de underlay y de nube de puntos y recortes por capas internas o controles de visualizacion se usa la ocupacion latente sin recortar o conservadora, si puede calcularse; si no ⇒ `UNKNOWN` | CLOSED BY V12 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-19; §11.3 punto 5; CT-49; T-M73; M-35; L-34; ADR-0036 decision 11 |
| `[V12-D05]` | AR11-01 · R:V12-01 · C:V12-05 | Estados de visibilidad dinamica sin regla | **DYNAMIC VISIBILITY**: para un `BlockReference` posterior con visibilidad dinamica, el estado actual **no** basta. Si CT-49 puede construir la union conservadora de todos los estados que pueden activarse sin cambiar el orden ⇒ `SUPPORTED`; si no ⇒ `UNKNOWN`. El primer corte no obliga a implementar una enumeracion universal de estados | CLOSED BY V12 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §11.3 punto 5; CT-49; T-M73 |
| `[V12-D06]` | AR11-01 · R:V12-01 · C:V12-06 | Campos sin regla, aunque su contenido cambia al evaluarse | **FIELDS**: si la ocupacion de modelo de un objeto depende de la evaluacion de un campo, es `SUPPORTED` solo con una cota conservadora valida para **todos** los resultados que admite el contrato; si no ⇒ `UNKNOWN`. Cubre `DBText`, `MText`, atributos, celdas de tabla y otras clases con campos que se descubran. El valor actual o `FIELDEVAL` **no** son autoridad suficiente | CLOSED BY V12 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §11.3 punto 5; CT-49; T-M73 |
| `[V12-D07]` | AR11-03 · R:V12-03 · C:V12-07 | Regla de ocupacion dependiente del entorno redactada para las clases de los objetos posteriores; las anotaciones y cotas de la copia se evaluaban con el contexto actual | **ENVIRONMENT RULE APPLIES TO COPY TOO**: la politica de ocupacion dependiente del entorno se aplica tambien a las huellas de la propia copia: `DBText`, cotas, anotaciones, piezas y contenido anidado. CT-50 añade como obligatorios el estilo de texto anotativo, el estilo de cota anotativo, `DIMSCALE = 0`, el estilo hijo de la familia (`<estilo>$0`), la fuente ausente, `FONTALT` o la sustitucion de fuentes y toda dependencia del entorno que se descubra. Sin huella conservadora ⇒ `UNKNOWN` (E10 en ST-19, X-22 en el orden de plan), o limitacion explicita de O-1 donde corresponda (L-36, L-37) | CLOSED BY V12 (contrato); consumo real: DEFERRED TO G3 WITH FAIL-CLOSED | §8.7; §11.3 punto 5; CT-50; T-M71; T-M72; M-37; L-36; L-37 |
| `[V12-D08]` | AR11-04 · R:V12-04 · C:V12-08 | «Escala uniforme» sin definir con reflexiones, aunque las colocaciones de grupo y las referencias de pieza usan `Scale3d(-1, 1, 1)` y `(±1, ±1, 1)` (`LateralHeaderDrawer.cs:62`, `:147`, `:313` @ `dad4e77`) | **UNIFORM TRANSFORM WITH REFLECTION**: magnitud uniforme `|sx| = |sy| = s`; los signos de reflexion se admiten, y `MirroredX` o `MirroredY` no implican por si solos una escala no uniforme. Ancho de modelo bajo una transformacion uniforme reflejada: `ancho' = |s| · ancho`. Transformacion anidada: si la magnitud efectiva sigue soportada y uniforme, la geometria ocupada se transforma normalmente; si no ⇒ `UNKNOWN`. `LineWeight` sigue fuera de la huella. CT-36 y T-M71 prueban `MirroredX`, `MirroredY`, `(-1,1)`, `(1,-1)`, `(-1,-1)` y anidados reflejados uniformes | CLOSED BY V12 | §11.3 punto 5; CT-36; T-M71 |
| `[V12-D09]` | AR11-02 · R:V12-02 · C:V12-09 | G-M24 atada a productores con nombre | **G-M24 GLOBAL PRODUCER CENSUS**: dos lados: (A) productores y consumidores de contexto declarados; (B) censo estructural de los sitios productores relevantes del Plugin y del camino de materializacion: sitios que construyen o crean subclases relevantes de `Entity`, añaden `DBObject` graficos, crean o configuran `BlockReference`, `DBText`, cotas, curvas y demas, resuelven o leen capas, estilos, valores por defecto o contexto de materializacion, o delegan la creacion en un helper o factoria. Cada sitio es `DECLARED` o `EXCLUDED` con motivo; un sitio productor o consumidor nuevo sin clasificar ⇒ RED. No depende solo de los nombres actuales de archivos o clases ni de hashes de archivos; un cambio que deje CT-50 obsoleta ⇒ RED hasta actualizar CT-50, el read-set, el fingerprint y las pruebas (T-M75) | CLOSED BY V12 (contrato); implementacion: G5..G7 | §8.7; §12.2 T-M75; §12.3 G-M24; §20.2 |
| `[V12-D10]` | AR11-07 · R:V12-07 · C:V12-10 | «Aunque este lejos» solo para `XLINE` y `RAY` | **O-1: UNKNOWN AUNQUE PAREZCA LEJOS**: L-34 y M-35 dicen expresamente que todo objeto posterior con huella `UNKNOWN` hace fallar ST-19 aunque visualmente parezca estar lejos del destino, porque `UNKNOWN` significa que no existe autoridad para demostrar la no superposicion. L-37 se amplia con los mecanismos de carga, visibilidad y recorte y con la ocupacion dependiente del entorno, tambien en la copia. No es un defecto | CLOSED BY V12 | §1.3 L-34 y L-37; §13 M-35; CT-49 |
| `[V12-D11]` | AR11-05 · R:V12-05 · C:V12-11 | §3.7, §8.1, §8.2, §8.4 y §20.1 fijaban propiedad y ubicacion de autoridades compartidas; §8.4 («junto al contrato de cada kind») contradecia §20.1 (`Mirror/`) | **SHARED AUTHORITIES PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION**: se mantiene el gate de V11 y quedan marcadas **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** las partes donde I-52 e I-55 reclaman fundacion compartida: §3.7 y §3.8 (taxonomia, codec de `View`/`Section` y `A_k`), §5.1 (nucleo neutral de seleccion), §8.1 (plan desde el sistema resuelto y origen y tramo del eje de la vista), §8.2 (primitivo de materializacion), §8.3 (valor de colocacion), §8.4 (codec), §8.5 (comparador authored y lineas base de C-2) y §20.1. La **semantica** que I-52 necesita puede acordarse; **propiedad, namespace, ubicacion y primitivo neutral definitivo no los congela I-52 sola**. La contradiccion entre §8.4 y §20.1 se resuelve: ninguna fija la ubicacion del codec antes de la reconciliacion, y `Mirror/` solo aloja lo propio del espejo. Ultimo observado de I-55: Proposal V2 (`f84f303`), NOT CONSENSUS, M-01 y X-1..X-8 abiertos, solo docs. La reconciliacion obligatoria antes del freeze sigue siendo I-52 ↔ I-55 ↔ I-49 | CLOSED BY V12 (proceso) | §3.7; §3.8; §5.1; §8.1..§8.5; §14.1; §15.3; §19; §20.1 |
| `[V12-D12]` | AR11-06 · R:V12-06 · C:V12-12 | I-49 en `3f17c21`; §15.2 afirmaba «D5..D8 y D25» y la fila documental listaba 0040 como aceptado | **I-49 CURRENT**: I-49 en `75f18623e7ced836de7eb2e36d6db8e311efb76d`, G6-C2 productivo: correccion del cualificador de clave exacta de A2 solo en `src/RackCad.Application/Expressions/` y sus pruebas (CI `34862975299` success); sin `ProjectVariables`, `Persistence`, `Systems`, Plugin, UI ni `PlanReadSet`. Contrato NON-MATERIAL; coordinacion MATERIAL; la linea base de CT-46 y G-M18 incluye las guardas del nucleo que G6-C2 cambia. Hechos corregidos: tip actual; ADR-0041 cambia solo D5..D8 (D19, D20 y D25 identicos a ADR-0040); indice de ADR: 0038 REPLACED, 0040 REPLACED, 0041 ACCEPTED y 0042 PROPOSED en I-55 | CLOSED BY V12 (registro) | §6.5.5; §6.7; §10.2 PV-6; §11.3 punto 12; §15; §16; CT-46 |
| `[V12-D13]` | C:V12-13 | — | **I-56 WATCH**: I-56 (`docs/initiative-workflow-v2`, Initiative Workflow V2; bootstrap `bd864a4` y, durante la redaccion de V12, correccion G0.1 `12fb509`) esta en G0.1, solo documentacion y proceso, con Evidence Audit y Proposal NOT STARTED; su contrato declara que el WORKFLOW vigente sigue siendo la autoridad hasta que I-56 integre, que I-49, I-52 e I-55 quedan **grandfathered** (terminan bajo el workflow con que se reclamaron e I-56 no reescribe sus contratos) y que ninguna politica de la V2 rige sin Coordinator y Architect AGREED sobre la misma version y Owner APPROVED (`WORKFLOW_V2_EFFECTIVE_SHA` todavia no existe). NON-MATERIAL hoy. Si I-56 integra un WORKFLOW cambiado antes del freeze de I-52 o de un gate posterior, se relee la autoridad de proceso vigente antes de continuar, incluidas sus reglas de transicion; no se adoptan reglas de I-56 no integradas | CLOSED BY V12 (registro) | §14.1; §15; §19 |
| `[V12-D14]` | C:V12-14 | Casos de G3 de V11 | CT-36, CT-49, CT-50, T-M71, T-M73, T-M75, G-M24 y M-35 con casos obligatorios. **XREF:** cargado, descargado, no resuelto y overlay anidado no disponible. **Recortes:** XCLIP, recorte de imagen, recorte de underlay y recorte o capas internas de nube de puntos donde existan. **Dinamico:** estados visibles A y B; estados latentes no disponibles ⇒ `UNKNOWN`. **Campos:** en `DBText`, `MText`, atributo y celda de tabla donde se soporte. **Visibilidad:** se conservan apagado, inutilizado y `Visible`. **Entorno en la copia:** texto y cota anotativos, `DIMSCALE = 0`, estilo hijo `$0`, fuente ausente y `FONTALT`. **Transformacion:** ancho con reflexion uniforme, anidado reflejado uniforme y no uniforme ⇒ `UNKNOWN`. **G-M24:** un sitio productor inventado pone la guarda en RED. **O-1:** objeto `UNKNOWN` lejano. G3 sigue sin produccion; contradiccion material ⇒ Proposal V13 | CLOSED BY V12 | §12; §13; §14 |
| `[V12-D15]` | C:V12-15 | ADR-0036 alineado con V11 | ADR-0036 sigue **PROPOSED** y actualiza las decisiones 11 a 13 con la politica latente por mecanismo, la disponibilidad de XREF, los recortes, la visibilidad dinamica, los campos, el entorno en la copia, las transformaciones uniformes reflejadas, el censo global de G-M24, las autoridades compartidas provisionales hasta la reconciliacion, ADR-0041 vigente en I-49, la Proposal V2 de I-55 con ADR-0042 y la vigilancia de I-56; nunca presenta resultados de G3; «la caracterizacion **DEBE** demostrar» | CLOSED BY V12 | ADR-0036 |
| `[V12-D16]` | C:V12-16 · R:V12-08 | Contradiccion material de V11 ⇒ Proposal V12 | Nueva matriz `[V12-Dnn]` sobre AR11-01..AR11-07 y las decisiones vinculantes del Coordinador; `[V11-D02]`, `[V11-D03]`, `[V11-D13]`, `[V11-D14]` y `[V11-D15]` se reabren y se cierran, y `[V11-D04]`, `[V11-D06]`, `[V11-D07]`, `[V11-D08]`, `[V11-D10]`, `[V11-D11]`, `[V11-D17]` y `[V11-D18]` se precisan (§0.4); `OPEN MATERIAL = NONE` para la revision del Arquitecto; toda contradiccion material de V12 ⇒ **Proposal V13**; proceso: acuerdo sobre V12 → consenso tecnico → rebase sobre el main vigente → reconciliacion obligatoria I-52/I-55/I-49 → freeze por SHA exacto → re-check exact-SHA → O-1 → G3 (Proposal V13 si G3 contradice materialmente / ADR-0036 aceptado → G4); antes de cada transicion del proceso se relee el WORKFLOW vigente y, si I-56 integro cambios de proceso, manda la autoridad integrada | CLOSED BY V12 | §1.2; §7.0; §14.1; §16; §19 |
| `[V12-D17]` | Precision al redactar | I-55 `39fa759` con Proposal V1 y ADR-0042 como sucesor de ADR-0010 | **I-55 V2 Y ADR-0042 V2**: I-55 en `1e1241471377ad76e4edbf1a1f6dc8e71db33c32` (Proposal V2 `f84f303` NOT CONSENSUS y paquetes de revision; CI `34864075451` success; solo docs). Open Material propio M-01 (colocacion de grupo y proyeccion ortografica, marcos fuente y destino) y X-1..X-8 con I-52; X-7 (valor de colocacion sobre `Transform2D`: rigidez de I-55 y reflexion de I-52) y X-8 (un descriptor de origen y tramo del eje por vista y una sola caracterizacion, frente a CT-05 y al tramo de `ViewPlanResult`) son nuevos. I-55 adopta la misma regla: sin la reconciliacion registrada no hay Consensus Freeze de I-55 ni G3. Su X-5 declara que la Proposal V2 dispara el reporte previsto en §15.3: se registra y se re-evaluan §8.1, §8.7, el `MaterializationContextReadSet`, PRE-17, PRE-18, T-GRD-02 y el censo `B`, sin cambio hoy porque nada esta congelado ni implementado; T-GRD-02 y `B` se re-miden en el SHA de reconciliacion. ADR-0042 V2 sigue **propuesto** y ya **no** reemplaza a ADR-0010: lo complementa y enmienda (vista adicional sobre un rack materializado o una intencion de creacion aceptada; Insertar gana la comprobacion de propiedades personalizadas acotada al `RackId` y el bloqueo por redibujo fallido; Actualizar no cambia). Para C-2: si se acepta e implementa antes que I-52, los fixtures de C2-2b cumplen esas precondiciones y la segunda iniciativa en integrar re-establece C-2. Arquitectura NON-MATERIAL; coordinacion HIGH / ACTIVE; solape de produccion NONE; sin STOP | CLOSED BY V12 (registro) | §8.5; §15; §16; §17 R-45; ADR-0036 referencias |
| `[V12-D18]` | Precision al redactar | Preflight de V11 | Preflight de V12 (§15.1): I-52 `2275f21` exacta (arbol limpio, sin stash ni operaciones a medias); main `dad4e77` sin cambios; I-49 `75f1862`; I-55 `1e12414`; rama nueva de I-56 `docs/initiative-workflow-v2` @ `bd864a4`, que el re-fetch previo al commit encontro en `12fb509` (G0.1, solo su contrato); ADR 0035..0042 sin numeros posteriores ni colisiones; biblioteca con el mismo SHA-256; censo de `[CommandMethod(`: main 35, I-49 35, I-55 35, I-56 35, I-52 33. Ninguna paralela modifica una autoridad de I-52: sin STOP | CLOSED BY V12 (registro) | §15.1 |
| `[V12-D19]` | Precision al redactar | O-1 sobre L-01..L-37 con L-30, L-34, L-36 y L-37 de V11 | O-1 sigue sobre **L-01..L-37**: L-34 reescrita (mecanismos de carga, visibilidad y recorte, estados dinamicos, campos y objeto posterior `UNKNOWN` aunque parezca lejos), L-36 precisada (entorno de la copia: estilos anotativos, `DIMSCALE = 0`, estilo hijo y fuentes) y L-37 ampliada (mecanismos y ocupacion dependiente del entorno, tambien en la copia) | CLOSED BY V12 | §1.2; §1.3 |
| `[V12-D20]` | Precision al redactar | Terminologia de V11 | **Terminologia**: se añaden `ContentAvailability` (disponibilidad del contenido de un objeto: cargado, descargado, no resuelto o no disponible), `VisibilityMechanism` (mecanismo por el que el contenido puede estar oculto, no cargado o recortado hoy y reaparecer sin cambiar el orden) y `ProducerSiteCensus` (lado B de G-M24), distintos de `ModelSpaceFootprintPolicy`, `ModelSpaceOrderTailSnapshot`, `ResolvedPresentationEligibility`, `VisualFootprintResult`, `FootprintInputFingerprint` y `TraceFingerprint`; ninguno sustituye a otro | CLOSED BY V12 | §9.2; §11.3 punto 5; §12.3 G-M24 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V13 o reduccion de alcance |
...
| **Guarda durable de los productores de contexto** (G-M24 con censo global de sitios productores, `[V11-D08]`, `[V12-D09]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas, o un sitio productor o consumidor nuevo sin clasificar ⇒ RED | G3 (CT-50: censo inicial), G5..G7 y despues de integrar |
...
| **Huella de orden latente por mecanismo** (capas, `Entity.Visible`, `ANNOALLVISIBLE`, XREF, filtros espaciales, recortes, estados dinamicos y campos; `[V11-D03]`, `[V12-D01]`..`[V12-D06]`) | contenido que puede reaparecer sin cambiar el orden y sin huella conservadora, o mecanismo no clasificado ⇒ `UNKNOWN` ⇒ E10; nunca contenido ausente = huella vacia | G3 (CT-49) y G6/G7 (T-M73) |
| **Disponibilidad de XREF** (`[V12-D03]`) | descargado, no resuelto u overlay anidado no disponible ⇒ `UNKNOWN`; cargado solo con huella latente conservadora demostrada; nunca se abren archivos externos | G3 (CT-49) y G6/G7 (T-M73) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V13 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V13 | G3 (CT-37, CT-38) |
...
| **Guardas de I-49 G6** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones, con la correccion G6-C2 de `75f1862`; ADR-0041 vigente; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18 | reconciliacion posterior a la integracion de I-49 |
| **Reconciliacion con I-55 (e I-49 donde aplique) antes del freeze** (View Placement & Projection, Proposal V2 sin consenso con M-01 y X-1..X-8 abiertos, ADR-0042 propuesto que complementa ADR-0010; autoridades compartidas provisionales; `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V12-D17]`, R-45) | sin reconciliacion registrada de las autoridades compartidas no hay freeze; una autoridad incompatible congelada por cualquiera de las dos ⇒ ningun freeze para ninguna hasta resolverlo; I-55 aplica la misma regla; I-52 no espera al consenso de I-55 | tras el consenso tecnico y el rebase, antes del freeze |
| **Autoridad de proceso frente a I-56** (Initiative Workflow V2 en G0.1, con I-52 grandfathered; `[V12-D13]`) | reglas de I-56 no integradas no se adoptan; antes de cada transicion del proceso se relee el WORKFLOW vigente y manda la autoridad integrada | cada transicion del proceso |

### 0.4 Estado de las marcas de V11 bajo V12

| Marca V11 | Estado en V12 |
|---|---|
| `[V11-D01]` | vigente |
...
### 0.5 Estado de las marcas de V10 (fijado por V11; sin cambios en V12)
...
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V13) |
...
### 0.6 Estado de las marcas de V9 (fijado por V10; sin cambios en V11 ni V12)
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V13) |
| `[V9-D16]` | vigente |
...
### 0.7 Estado de las marcas de V8 (fijado por V9; sin cambios en V10, V11 ni V12)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V13) |
...
### 0.8 Estado de las marcas de V7 (fijado por V8; sin cambios en V9, V10, V11 ni V12)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V13) |
...
### 0.9 Estado de las marcas de V6 (fijado por V7; sin cambios en V8, V9, V10, V11 ni V12)
...
### 0.10 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8, V9, V10, V11 ni V12)
...
### 0.11 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V12) |
|---|---|
...
### 0.12 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR11-01 (huella latente por mecanismo: XREF, recortes, visibilidad dinamica y campos) | `[V12-D01]`..`[V12-D06]` |
| AR11-02 (censo global de sitios productores de G-M24) | `[V12-D09]` |
| AR11-03 (regla de entorno tambien en la copia; casos de CT-50) | `[V12-D07]` |
| AR11-04 (transformacion uniforme con reflexion) | `[V12-D08]` |
| AR11-05 (autoridades compartidas provisionales; §8.4 frente a §20.1) | `[V12-D11]` |
| AR11-06 (hechos de I-49 y del indice de ADR) | `[V12-D12]`, `[V12-D18]` |
| AR11-07 (O-1: `UNKNOWN` aunque parezca lejos) | `[V12-D10]`, `[V12-D19]` |
| Cambios exactos R:V12-01..R:V12-08 de la revision | V12-01 → D01..D06; V12-02 → D09; V12-03 → D07; V12-04 → D08; V12-05 → D11; V12-06 → D12; V12-07 → D10; V12-08 → D16 |
| Precisiones del Coordinador C:V12-01..C:V12-16 | V12-01 → D01; V12-02 → D02; V12-03 → D03; V12-04 → D04; V12-05 → D05; V12-06 → D06; V12-07 → D07; V12-08 → D08; V12-09 → D09; V12-10 → D10; V12-11 → D11; V12-12 → D12; V12-13 → D13; V12-14 → D14; V12-15 → D15; V12-16 → D16 |
| ADR-0036 (decisiones 11..13 y alineacion general) | `[V12-D15]` |
| Paralelas y main observados en el preflight y en el re-fetch de V12 | `[V12-D12]`, `[V12-D13]`, `[V12-D17]`, `[V12-D18]` |
| Precisiones del ejecutor | `[V12-D17]`..`[V12-D20]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V12; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V13 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V12; si la contradice ⇒ Proposal V13; **precondicion de G4** |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10, V11 y V12) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10, V11 y V12 (ST-19 completo,
...
entorno de la copia y transformaciones uniformes reflejadas) no se han medido sobre esta biblioteca y las miden CT-36,
CT-48, CT-49 y CT-50. Estado: **DEFERRED TO G3** con fail-closed (X-17a, X-17b, X-22, X-23).
...
salvo los de PREPARE (re-verificacion de definiciones, ST-19 temprano y definitivo, frescura de las huellas visuales,
cola de Model Space y read-set del contexto de materializacion, E10). La vista del Owner es la envolvente de §1.3; la
evidencia indicativa de la biblioteca inspeccionada esta en §1.4.
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** La taxonomia de tipo de vista y el codec de
> `View`/`Section` son fundacion compartida con I-55 (X-2; ADR-0042 decisiones 2 y 8, propuesto). La semantica que I-52
> necesita —la lectura de produccion, la forma canonica y el fallo cerrado de lo no inequivoco— puede acordarse;
> propiedad, namespace, ubicacion y codec neutral definitivo no los congela I-52 sola: se fijan en la reconciliacion
> obligatoria previa al freeze (§14.1).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** La exposicion de `μ_k` y la politica de fallo
> cerrado son de I-52; el conjunto de vistas soportadas por sistema (`A_k`) y su matriz normativa comparten fundacion
> con la matriz de I-55 y su ubicacion se fija en la reconciliacion (§14.1).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** El nucleo neutral de seleccion es fundacion
> compartida con I-55 (X-1). Los hechos neutrales y la equivalencia de la fachada son necesarios para I-52; propiedad,
> namespace y ubicacion definitivos del nucleo no los congela I-52 sola: se fijan en la reconciliacion y G4 avisa a I-49
> y a I-55 antes de tocarlo (§14.1).
...
equivalencia de la fachada: **STOP → Proposal V13**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V = 1 ⇒ W = max(70, 76 + 3)  = 79 < 80: la fila de N1 se apila desde la izquierda en el original y en la copia.
    V12: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V = 4 ⇒ W = 92 ≥ 88;  V = 1 ⇒ W = 83 < 88: la frontal del fondo 1 desborda.
    V12: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V12 ⇒ **STOP → Proposal V13**.
...
## 7. Matriz de representabilidad V12
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V12)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V13**. T-M47 lo hace ejecutable desde
G5.
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** La autoridad «sistema resuelto → plan de vista»,
> el descriptor de origen y tramo del eje de cada vista (el tramo que devuelve `ViewPlanResult`, X-8) y la resolucion
> del diseño que hoy precede al plan son fundacion compartida con I-55 (X-2 y X-8; ADR-0042 decisiones 1 y 8, propuesto,
> que parte del sistema ya resuelto). La semantica que I-52 necesita puede acordarse; propiedad, namespace, ubicacion y
> forma definitiva de la autoridad no los congela I-52 sola (§14.1).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** El primitivo «definicion + sobre + referencia» en
> la transaccion del llamador es fundacion compartida con I-55 (X-4); ST-13, ST-19, PRE-17, PRE-18, el
> `MaterializationContextReadSet` y la equivalencia visual siguen siendo politica de I-52 fuera del primitivo.
> Propiedad, ubicacion y primitivo neutral definitivo no los congela I-52 sola (§14.1).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** El valor de colocacion y su composicion sobre
> `Transform2D` son fundacion compartida con I-55 (X-7): la rigidez es invariante de I-55 y la reflexion es politica de
> I-52. Propiedad y ubicacion definitivas se fijan en la reconciliacion (§14.1).
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** El codec de `View`/`Section` y la taxonomia de
> tipo de vista son fundacion compartida con I-55 (X-2). La semantica que I-52 necesita puede acordarse; propiedad,
> namespace y ubicacion del codec no los congela I-52 sola (§3.7, §14.1).

La decodificacion `SectionRaw → semantica → canonica`, la exposicion y el conjunto `A_k` (§3.7, §3.8) viven en
Application, fuera del comando. **Su ubicacion no la fija V12**: puede quedar junto al contrato de cada kind o en un
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** El comparador authored por kind y las lineas base
> de C-2 (C2-2a, C2-2b, C2-4, G-M9) son fundacion compartida con I-55 (X-3 y X-6). Si ADR-0042 se acepta e I-55
> re-enruta Insertar antes que I-52, los fixtures de C2-2b cumplen sus precondiciones nuevas (propiedades personalizadas
> acotadas al `RackId` y redibujo previo sin fallos) y la segunda iniciativa en integrar re-establece C-2 (`[V12-D17]`).
> Propiedad y ubicacion del comparador no los congela I-52 sola (§14.1).
...
Marcas de V12 en esta seccion: `[V12-D07]` (regla de entorno tambien en la copia) y `[V12-D09]` (censo global de G-M24).
...
Regla
  ...
  G-M24 protege a los productores con dos lados ([V11-D08], [V12-D09]):
    ...
    B) ProducerSiteCensus: censo estructural de los sitios del Plugin y del camino de materializacion que
       construyen o crean subclases relevantes de Entity, añaden DBObject graficos, crean o configuran
       BlockReference, DBText, cotas o curvas, resuelven o leen capas, estilos, valores por defecto o contexto de
       materializacion, o delegan la creacion en un helper o factoria; cada sitio DECLARED o EXCLUDED con motivo;
       sitio nuevo sin clasificar ⇒ RED; no depende de los nombres actuales de archivos o clases ni de hashes
...
- No crea obligacion de tocar los drawers existentes (§20.3); G-M8 y G-M11 no cambian, y G-M24 vigila, sin modificarlos,
  a los productores declarados y, con su censo estructural, todo sitio productor o consumidor nuevo del camino de
  materializacion (`[V12-D09]`).
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R2  huellas visuales en el DestinationEvaluationContext de las instancias de esos planes, anotaciones y cotas
      ...
      frente a visibilidad, carga, recortes, estados dinamicos y campos ([V11-D02]..[V11-D04],
      [V12-D01]..[V12-D06]; §4.1, §11.3 punto 5)
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           ...
           PRE-17, PRE-18) · enumeracion de NUEVO de la cola posterior a cada fuente frente a la
           ModelSpaceOrderTailSnapshot de R6: objeto añadido, eliminado, reordenado, movido de antes a despues de la
           fuente o con identidad o clase cambiada ⇒ excepcion y rollback ([V11-D05]) · todas las definiciones nuevas
...
`ProducerSiteCensus` es el lado B de G-M24 (§8.7). Ninguno sustituye a otro (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`).
...
| RS-3 Dependencias transitivas | **solo si I-49 integra antes**: las observaciones de su `PlanReadSet` (resultado de cada simbolo realmente usado, incluidas las dependencias de expresiones) | I-49 V6 D19; ADR-0041 (aceptado @ `2e8090a`; reemplaza a ADR-0040, que reemplazaba a ADR-0038, con D19 identico; `[V11-D11]`) |
...
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`).** Autoridad conceptual distinta para los
     objetos arbitrarios del dibujo que ST-19 clasifica:
     ...
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       ...
       ejes obligatorios ([V12-D01]): ContentAvailability (cargado · descargado · no resuelto · no disponible) y
         VisibilityMechanism (mecanismos por los que el contenido puede estar oculto, no cargado o recortado hoy y
         reaparecer despues sin cambiar el orden)
       ...
       inventario minimo de mecanismos (CT-49, [V12-D02]); cada caso SUPPORTED(metodo conservador) o UNKNOWN:
       ...
       nunca «contenido ausente hoy = huella vacia» si puede reaparecer sin cambiar el orden
   ...
   - **Frescura (`[V10-D03]`).** Cada huella lleva su `FootprintInputFingerprint`: identidades de las definiciones
     ...
     copia (`[V11-D04]`, `[V12-D07]`), estado de disponibilidad y de mecanismo (`[V12-D01]`) y cualquier otra entrada
     que altere la huella. Tras `EnsureForPlan` se relee el dibujo real; toda huella cuyo fingerprint cambie se vuelve a
     producir y se re-evaluan el orden de plan y ST-19 (PRE-18). No se usa `TraceFingerprint` para esto (punto 10).
   ...
   - **Una sola autoridad** de extraccion de huellas y de sus margenes de modelo (G-M23). CT-36 y CT-49 caracterizan las
     ...
     `ContentAvailability` y `VisibilityMechanism`, es la unica clasificacion de los objetos del dibujo y T-M73 prueba
     su consumo (`[V11-D02]`..`[V11-D04]`, `[V12-D01]`..`[V12-D06]`).
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V13.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V12 no introduce tolerancia; CT-36 registra
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V12 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V12 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V12 no introduce una tolerancia nueva: CT-36
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Metodo de inventario cerrado (`[V9-D08]`, `[V12-D01]`).** CT-36 (contenido de las piezas evaluadas) y CT-49
    (referencia fuente y objetos de Model Space, con la definicion referenciada y sus componentes) declaran como
    descubren propiedades, estado y mecanismos, con un harness fuera de `src/`:
    ...
    por tipo exacto en ejecucion:
      ...
      en objetos de Model Space tambien: definicion referenciada y disponibilidad de su contenido (XREF cargado,
        descargado, no resuelto u overlay) · estados de visibilidad dinamica · recortes de imagen, underlay y nube de
        puntos · capas internas y controles de visualizacion · campos ([V12-D01], [V12-D02])
    ...
    sin clasificar o UNSUPPORTED ⇒ UNKNOWN (contenido evaluado, X-17b), E4 (referencia fuente, ST-18) o huella
      UNKNOWN ⇒ E10 (objeto de Model Space posterior, ST-19)
    ```
    ...
    **STOP → Proposal V13**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V12 abre **Proposal V13**.
...
| **T-M75** | Censo de sitios productores de G-M24 (`[V12-D09]`): un sitio inventado que construye una entidad relevante, añade un `DBObject` grafico, lee un estilo, una capa o un valor por defecto, o delega la creacion en un helper nuevo, sin declararse ni excluirse con motivo, pone G-M24 en RED; un productor declarado que cambia lo que asigna, usa o crea tambien; la guarda no depende de los nombres actuales de archivos o clases ni de hashes | G5..G7 |
...
| **G-M23** | Autoridad unica de huellas visuales (`[V9-D02]`, `[V10-D01]`..`[V10-D03]`): un solo productor de `VisualFootprintResult` para piezas, curvas, anotaciones, cotas y objetos de Model Space; ningun archivo del espejo usa `GeometrySymmetryResult` como huella, `TraceFingerprint` como `FootprintInputFingerprint` ni `Extents3d`/`GeometricExtents` sin la declaracion de §11.3 punto 5; ningun archivo dilata una huella por `LineWeight`, `LWDISPLAY`, grosores de CTB/STB, milimetros de papel o escala de trazado, ni construye regiones antes de transformar a WCS; ninguna huella de instancia del plan se resuelve con el contexto de la biblioteca; ningun consumidor trata una huella `UNKNOWN` como no superpuesta; ninguna clasificacion de objetos del dibujo fuera de la `ModelSpaceFootprintPolicy`, ningun fallback optimista para una clase no caracterizada, ninguna exclusion de geometria por la visibilidad transitoria en la huella de orden y ningun ancho local sin transformar (`[V11-D02]`, `[V11-D03]`, `[V11-D07]`); ninguna exclusion de contenido por carga, recorte, estado dinamico o valor actual de un campo, ninguna huella vacia por contenido ausente y ninguna magnitud reflejada tratada como no uniforme (`[V12-D01]`..`[V12-D06]`, `[V12-D08]`) |
| **G-M24** | Guarda durable de productores de contexto (`[V11-D08]`, `[V12-D09]`), estructural y no solo de hashes de archivos, con **dos lados**: (A) productores y consumidores de contexto declarados (`LateralHeaderDrawer`, `CantileverViewMaterializer`, `LayerHelper` y todo productor que identifique CT-50), con las propiedades visuales que asigna cada uno, el uso de capas, estilos y valores por defecto, las clases de entidad que crea y la resolucion de estilos; (B) `ProducerSiteCensus`: censo estructural de los sitios del Plugin y del camino de materializacion que construyen o crean subclases relevantes de `Entity`, añaden `DBObject` graficos, crean o configuran `BlockReference`, `DBText`, cotas, curvas y demas, resuelven o leen capas, estilos, valores por defecto o contexto de materializacion, o delegan la creacion en un helper o factoria, cada uno `DECLARED` o `EXCLUDED` con motivo. Un sitio productor o consumidor nuevo sin clasificar, o un productor declarado que empieza o deja de asignar una propiedad visual, cambia ese uso, crea una clase relevante nueva o cambia la resolucion de estilos, pasa a RED hasta actualizar CT-50, el `MaterializationContextReadSet`, el `FootprintInputFingerprint` y las pruebas (T-M75); no depende solo de los nombres actuales de archivos o clases ni de hashes |
...
→ rebase / reconciliacion sobre el main vigente         (solo docs; hoy main = dad4e77 con I-53 E1, I-53S
                                                          ...
                                                          §15.1)
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55 ↔ I-49; registra la propiedad de las
                                                          autoridades compartidas, todas PROVISIONAL UNTIL
                                                          CROSS-INITIATIVE RECONCILIATION en V12: taxonomia de
                                                          tipo de vista, codec semantico de View/Section, «sistema
                                                          resuelto → plan de vista» con origen y tramo del eje de
                                                          la vista, comparador authored, primitivo de
                                                          materializacion del Plugin, valor de colocacion y
                                                          transformaciones de proyeccion, y nucleo de seleccion
                                                          (X-1..X-8); sin autoridades duplicadas; [V11-D10],
                                                          [V12-D11])
...
Antes de cada transicion del proceso: releer el WORKFLOW vigente; si I-56 integro cambios de proceso, manda la
autoridad integrada, y nunca se adoptan reglas de I-56 no integradas ([V12-D13], [V12-D16])
```
...
### 15.1 main y paralelas observadas en el preflight de V12
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ...
                                                     ancla de las citas de V12: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = 2275f21b88409f5ee86b919317a5eb5efd5e213e (V11; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = 75f18623e7ced836de7eb2e36d6db8e311efb76d (G6-C2: correccion del
                                                     cualificador de clave exacta de A2 en Application/Expressions y
                                                     sus pruebas, sobre el Consensus Freeze 3f17c21 de V6, A1, A2 y
                                                     ADR-0041; ADR-0041 aceptado y ADR-0040 reemplazado; G6 sin cierre
                                                     registrado; sin integrar)
I-55 feature/creacion-de-vistas                    = 1e1241471377ad76e4edbf1a1f6dc8e71db33c32 (View Placement &
                                                     Projection: Proposal V2 f84f303 NOT CONSENSUS y paquetes de
                                                     revision 1e12414; Open Material M-01 y X-1..X-8; ADR-0042
                                                     propuesto, que complementa y enmienda ADR-0010; solo docs; sin
                                                     integrar)
I-56 docs/initiative-workflow-v2                   = 12fb5095100696bcb6d2fd7cef69e110fbcd8dd9 (rama nueva:
                                                     Initiative Workflow V2; reclamo 50ba035, bootstrap bd864a4 y
                                                     correccion G0.1 12fb509; solo documentacion y proceso; I-49, I-52
                                                     e I-55 grandfathered; V2 NOT EFFECTIVE; sin integrar)
...
**main desde la publicacion de V11.** Sin cambios: `dad4e77`. Ninguna cita de V12 se desplaza respecto de V11; las citas
nuevas de V12 (`LateralHeaderDrawer.cs:62`, `:147` y `:313`) se leen en `dad4e77` y son identicas en `1b091be`.
...
**Cambios de paralelas desde la publicacion de V11.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V12 declaraba
como ultimos observados main `dad4e77`, I-52 `2275f21`, I-49 `75f1862`, I-55 `1e12414` e I-56 `bd864a4`; el preflight de
V12 los confirmo, con ADR-0041 aceptado, ADR-0042 propuesto y sin mas ramas nuevas, y el re-fetch previo al commit
encontro I-56 en `12fb509` (G0.1, solo su contrato) (`[V12-D18]`). La historia de paralelas anterior esta en
Proposal V11 §15.1, en Proposal V10 §15.1, en Proposal V9 §15.1, en Proposal V8 §15.1, en Proposal V7 §15.1 y en el
registro §42, §48, §54, §60 y §66. La numeracion de ADR esta en §16.
...
| **I-55** (View Placement & Projection; rama `feature/creacion-de-vistas` @ `1e12414`, sobre `dad4e77`; Proposal V2 `f84f303` NOT CONSENSUS con Open Material M-01 y X-1..X-8; ADR-0042 propuesto; solo docs) | ID17, ID18 e ID19 sobre una foundation de preparacion de vistas antes de materializar y una colocacion de grupo con una transformacion rigida comun; §17.2 de su Proposal V2 reconcilia con I-52 en X-1..X-8: nucleo neutral de seleccion (X-1); plan desde el sistema resuelto y codec de `View`/`Section` neutral con politica por consumidor (X-2); comparador authored por kind (X-3); primitivo «definicion + sobre + referencia» con las politicas de I-52 (ST-13, ST-19, PRE-17, PRE-18, read-set) fuera del primitivo (X-4); reporte antes de fijar archivos o autoridades que I-52 vigila, que la Proposal V2 ya dispara (X-5); re-establecimiento de C-2 por la segunda en integrar (X-6); valor de colocacion sobre `Transform2D` con rigidez de I-55 y reflexion de I-52 (X-7); un descriptor de origen y tramo del eje por vista (X-8). ADR-0042 V2 complementa y enmienda ADR-0010 (vista adicional; dos precondiciones nuevas de Insertar; Actualizar sin cambio) | arquitectura hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**; I-52 **no** espera al consenso de I-55. Tras el consenso tecnico sobre V12 y el rebase, **reconciliacion obligatoria antes del freeze** de la propiedad de las autoridades compartidas, marcadas como provisionales en V12 (taxonomia de tipo de vista, codec de `View`/`Section`, «sistema resuelto → plan de vista» con origen y tramo del eje, comparador authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, nucleo de seleccion), sin autoridades duplicadas; siguen siendo de I-52 ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`; I-55 aplica la misma regla y el STOP simetrico es mutuo; si ADR-0042 se acepta antes del freeze de I-52, se releen las autoridades de C-2, Actualizar e Insertar; la segunda iniciativa en integrar re-establece C-2 (R-45, `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V12-D17]`) |
| **I-56** (Initiative Workflow V2; rama `docs/initiative-workflow-v2` @ `12fb509`, sobre `dad4e77`; G0.1; solo documentacion y proceso; I-49, I-52 e I-55 grandfathered) | puede cambiar el proceso de iniciativas (WORKFLOW, AGENTS, AUTOMATION_PLAN, `TEMPLATE.md` y afines) cuando su Proposal lo fije e integre; hoy solo toca su contrato y su fila de ROADMAP | **NON-MATERIAL** hoy; el WORKFLOW vigente sigue siendo la autoridad hasta que I-56 integre y, aun integrada, su invariante de transicion deja a I-52 bajo el workflow de su reclamo y ninguna politica de la V2 rige sin acuerdo tecnico y aprobacion del Owner; antes de cada transicion del proceso de I-52 se relee el WORKFLOW vigente y, si I-56 integro cambios, manda la autoridad integrada; nunca se adoptan reglas no integradas (`[V12-D13]`) |
...
transicion, releer el WORKFLOW vigente (`[V12-D13]`). Ademas:
...
- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  ...
  reconciliar, CT-46 las relee y G-M18 las aplica a los archivos de I-52. Si un gate posterior de I-49 introduce
  `PlanReadSet` o dominio aplicado, o toca ProjectVariables, Persistence o Systems, reportar antes de continuar
  (`[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`).
...
- **I-55 (`feature/creacion-de-vistas`, View Placement & Projection, Proposal V2 NOT CONSENSUS, solo docs; gate de
  coordinacion activo, `[V11-D10]`, `[V12-D11]`, `[V12-D17]`):** I-52 no espera a su consenso, pero no congela sin la
  reconciliacion obligatoria posterior al consenso tecnico sobre V12 y al rebase, que registra la propiedad de las
  autoridades compartidas que V12 marca como provisionales —taxonomia de tipo de vista, codec semantico de
  `View`/`Section`, autoridad «sistema resuelto → plan de vista» con origen y tramo del eje de la vista, comparador
  authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, y nucleo de
  seleccion (X-1..X-8)—, sin autoridades duplicadas. Siguen siendo de I-52, aunque consuman un primitivo compartido,
  ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`. Si
  cualquiera de las dos intenta congelar una autoridad incompatible antes de reconciliar: STOP, ningun freeze para
  ninguna (R-45, §19); I-55 V2 aplica la misma regla a su Consensus Freeze. G4 avisa a I-55 antes de tocar la fundacion
  compartida (nucleo de seleccion en `RackDuplicationPlan.cs`). Si ADR-0042 cambia de estado, se re-evalua antes del
  siguiente commit y, si se acepta antes del freeze de I-52, se releen las autoridades que usan C-2, Actualizar e
  Insertar. Cuando un gate de I-55 toque rutas de materializacion, `RackEmbedComposer.Compose`, `SystemBlockWriter.cs`,
  comandos o `RackCommandReference.cs` (hoy previsto en sus G5..G15, X-5), se re-evaluan §8.1, §8.7,
  `MaterializationContextReadSet`, PRE-17, PRE-18, T-GRD-02 y el censo `B`; la segunda iniciativa en integrar
  re-establece C-2 sobre el main combinado.
- **I-56 (`docs/initiative-workflow-v2`, Initiative Workflow V2, G0.1, solo documentacion y proceso; `[V12-D13]`):**
  NON-MATERIAL hoy; su contrato declara a I-49, I-52 e I-55 grandfathered y que ninguna politica de la V2 rige sin
  acuerdo tecnico y aprobacion del Owner. Antes de cada transicion del proceso de I-52 (consenso, rebase,
  reconciliacion, freeze, re-check, O-1, G3 y gates posteriores) se relee el WORKFLOW vigente en `origin/main`; si I-56
  integro cambios de proceso, manda la autoridad integrada, y nunca se adoptan reglas de I-56 no integradas.
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V12.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V12 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   ADR-0010, sino como complemento que enmienda reglas acotadas. I-56 no tiene ADR. No hay numeros posteriores a 0042 en
   ningun ref (`[V11-D11]`, `[V11-D12]`, `[V12-D12]`, `[V12-D17]`).
...
no contradice materialmente V12, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V13. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V13 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V13 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V13 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V13 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V13 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V13 |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10, V11 y V12 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V11-8** | G3, ADR y proceso (C:V12-14..C:V12-16) | G3 ampliado; ADR-0036 alineado; relectura del WORKFLOW antes de cada transicion; desde V12, contradiccion material ⇒ Proposal V13 | §12; §13; §14.1; ADR-0036 |
...
- **AR11-01: el Coordinador divide la huella latente por mecanismo.** La revision de V11 pedia un solo cambio
  (R:V12-01); el Coordinador fija seis decisiones —mecanismo, inventario obligatorio, XREF, recortes, visibilidad
  dinamica y campos— y decide que un XREF descargado, no resuelto o con contenido anidado no disponible es `UNKNOWN` en
  el primer corte, sin abrir archivos externos. El numero de E10 que eso produzca en dibujos reales lo mide G3 (R-47,
  R-49).
...
- **G-M24 (`[V11-D08]`, `[V12-D09]`).** Tiene dos lados: productores declarados y censo estructural de sitios
  productores; no depende de nombres de archivos ni de hashes. Su forma concreta (analisis estructural, lista declarada
  y exclusiones con motivo) se fija en G5..G7 sin tocar los drawers (§20.3), y CT-50 fija el censo inicial.
...
- **Autoridades compartidas (`[V12-D11]`).** V12 fija la semantica que I-52 necesita y deja propiedad, namespace,
  ubicacion y primitivo neutral a la reconciliacion; ninguna seccion fija ya la ubicacion del codec.
- **I-56 (`[V12-D13]`).** Vigilancia de proceso, sin adoptar reglas no integradas; su contrato G0.1 declara a I-52
  grandfathered.
- **Terminologia (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`).** Huella de la referencia, huella visual,
  `FootprintInputFingerprint`, `TraceFingerprint`, `ResolvedPresentationEligibility`, `ModelSpaceFootprintPolicy`,
  `ModelSpaceOrderTailSnapshot`, `ContentAvailability`, `VisibilityMechanism` y `ProducerSiteCensus` son autoridades o
  registros distintos (§9.2).
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V13 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V12 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V12 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V13 y O-1 se reevalua.
...
- **ADR-0041 aceptado en I-49 (`[V11-D11]`, `[V12-D12]`).** Reemplaza a ADR-0040, cambia solo D5..D8 y conserva lo que
  I-52 consume (D19 `PlanReadSet`, D20 y el dominio del consumidor); la correccion G6-C2 (`75f1862`) solo toca
  `Expressions`. V12 lo usa como autoridad vigente de I-49.
- **I-55 (`[V11-D10]`, `[V12-D11]`, `[V12-D17]`, R-45).** I-52 no espera al consenso de I-55, pero no congela sin la
  reconciliacion obligatoria de las autoridades compartidas; hoy no hay contradiccion material presente: X-1..X-8 de su
  Proposal V2 proponen resoluciones compatibles con la semantica de V12, I-55 adopta la misma regla de reconciliacion y
  ADR-0042 V2 complementa ADR-0010 sin cambiar Actualizar. V12 marca esas autoridades como provisionales y no decide el
  reparto.
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V13** o reduccion
  de alcance.
...
- **Reconciliacion entre iniciativas (`[V11-D10]`).** Sin la reconciliacion obligatoria I-52/I-55 (e I-49 donde aplique)
  ...
  C-2, Actualizar e Insertar y se reconcilia antes de congelar. Las partes marcadas PROVISIONAL UNTIL CROSS-INITIATIVE
  RECONCILIATION (§3.7, §3.8, §5.1, §8.1..§8.5, §20.1) no se congelan por I-52 sola (`[V12-D11]`).
- **Proceso vigente (`[V12-D13]`).** Antes de cada transicion del proceso se relee el WORKFLOW vigente; adoptar una
  regla de I-56 no integrada: **prohibido**; si I-56 integro cambios de proceso, manda la autoridad integrada.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V13.
...
> **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (`[V12-D11]`).** Las ubicaciones de esta tabla que tocan fundacion
> compartida con I-55 —nucleo neutral en `Persistence/`, valor de colocacion junto a `Transform2D`, autoridades de plan
> por kind, codec de vistas y `A_k`, primitivo de materializacion en `Systems/Shared/` y comparador authored— son
> probables y no las congela I-52 sola: se fijan en la reconciliacion previa al freeze (§14.1).
...
productores declarados y censo global de sitios productores, `[V12-D09]`); evidencia de viabilidad de la sonda en
`acad.exe` y corpus indicativo con la huella de la biblioteca en `docs/automation/evidence/` y, si hace falta, un
harness en `tools/` (nunca en `src/` durante G3); `SelectiveEditorOpenTests.cs` (censo `B + 1`), los censos de T-GRD-08
de I-54, ya en main, reapuntados con motivo (`CustomPropertiesCommandGuardTests.cs`,
`CustomPropertiesHelpCensusTests.cs`) y el censo T-GRD-02 de `CustomPropertiesEnvelopeGuardTests.cs` clasificado con
motivo (hoy 7 → 8, re-medido en el SHA de reconciliacion); guardas de I-51 reapuntadas en
`SelectiveDuplicationFailClosedTests.cs` solo si se extrae el SNAPSHOT; pruebas de UI en `tests/RackCad.UI.Tests` para
C-2 (G6), la de la ayuda sin alias (T-M74, G7) y las caracterizaciones de UI (G3).
...
G2 = V12 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd), V10 (636f7fd) y V11 (2275f21) = historial
     revision de V11: Architect: CHANGES REQUIRED — PROPOSAL V12
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V12, rebase sobre main, reconciliacion con I-55 e I-49, freeze, re-check
                     y O-1; antes de cada transicion, relectura del WORKFLOW vigente)
