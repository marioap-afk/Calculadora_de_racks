# I-52 — Proposal V9: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V9 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V9 se redacta SIN rebase, sobre e0a779f)
> CITAS             = archivo:linea contra origin/main @ 1b091bedafceb67ca57054a9eb3bf5259efff774 (I-53 E1 integrada).
>                     main esta hoy en 104ef3a (merge de I-53S E2: en src/tests solo RackSelectiveWindow y pruebas de
>                     UI); las citas siguen ancladas en 1b091be, las que I-53S desplazo se listan en §15.1 y se re-anclan
>                     en el rebase posterior al consenso. Las citas de paralelas llevan su SHA
...
> Proposal V1..V7   = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf   (historial)
> Proposal V8       = e0a779f0d12e79074272919531d71c7f4f3aaa7b   (historial; CI 34791391929 4/4)
> Revision Arq. V8  = «Architect: CHANGES REQUIRED — PROPOSAL V9»; sin BLOCKER ni HIGH; MEDIUM AR8-01 (ST-19 solo
>                     clasificaba objetos no seleccionados) y AR8-02 (orden de plan sin huella de las piezas que no
>                     pasan por la sonda); LOW AR8-03..AR8-09; cambios exactos V9-01..V9-10 de la revision (registro:
>                     docs/automation/decisions/I-52.md §50)
> Coordinador       = orden de V9 con V9-01..V9-15 vinculantes (registro §51)
> main y paralelas  = §15.1 (preflight de V9 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V9 (§14.1); V9 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V9 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V8 ni el Discovery y **no** es el contrato vinculante.
> El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V8, sus revisiones de Arquitecto y esta V9 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V9 la registra
> como **indicativa**, ligada a su SHA-256, y no como contrato.

---

## 0. Reconciliacion V8 → V9

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`) y el Discovery (`339b3ab`) **se conservan sin cambios**. **Donde V8 y V9 difieran, manda V9.** Las marcas
`[V8-D01]`..`[V8-D26]` siguen vigentes salvo donde una marca `[V9-Dnn]` las modifica o sustituye (§0.4); las marcas
`[V7-Dnn]`, `[V6-Dnn]` y `[V5-Dnn]` siguen como las dejaron V8, V7 y V6 (§0.5, §0.6, §0.7).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V9** | V9 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V9. **Debe ser NONE** |

### 0.2 Matriz `[V9-D01]`..`[V9-D22]`

En la columna «Origen»: `AR8-nn` es el hallazgo de la revision de V8; `R:V9-nn`, su cambio exacto; `C:V9-nn`, la precision
vinculante del Coordinador en la orden de este gate. Las dos series V9-nn se numeran por separado. **Donde el cambio
propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del Coordinador**: para
AR8-05 la revision dejaba al Coordinador elegir entre declarar el contexto de materializacion o fijar la presentacion
interna de la copia, y el Coordinador decide que I-52 es fiel al plan regenerado, como `RACKEDITAR` → Actualizar (§8.7).

| Marca | Origen | V8 | Correccion V9 | Estado | Donde |
|---|---|---|---|---|---|
| `[V9-D01]` | AR8-01 · R:V9-01 · C:V9-01 | ST-19 clasificaba solo objetos **no seleccionados** posteriores a la fuente | **ST-19 COMPLETO**: para cada copia `c` de su fuente `s` y **todo** objeto existente `o` de Model Space con `o ≠ s` que va **despues** de `s` en el orden efectivo de dibujo —otra fuente admitida seleccionada, objeto seleccionado pero ignorado por C1..C3, objeto no seleccionado, otro rack o cualquier entidad—, una superposicion material de `VisualFootprint(c)` y `VisualFootprint(o)`, o una superposicion no clasificable, ⇒ E10 en PREPARE. Los objetos anteriores a `s` no requieren clasificacion; la fuente `s` es excepcion deliberada (la copia puede quedar encima de su propia fuente); las copias entre si se crean en el orden de sus fuentes. Sin ninguna restriccion a «objeto no seleccionado» | CLOSED BY V9 (contrato); clasificacion real de superposiciones: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §4.1 ST-19; §2.3; §9.1; §9.2; §18.2; PRE-16; L-34; T-M69; CT-49; M-35; ADR-0036 decision 11 |
| `[V9-D02]` | AR8-02 · R:V9-02 · C:V9-02 | Sin autoridad de huella: `GeometrySymmetryResult` sin geometria de superposicion; P7a y P7b solo para piezas con cambio de mano | **VISUAL FOOTPRINT AUTHORITY**: `VisualFootprintResult` (nombre ilustrativo, no persistido), autoridad separada de `GeometrySymmetryResult`. P7a enumera por vista admisible las piezas que requieren `GeometricEvidence` y **toda otra instancia** que participe potencialmente en el orden visual de plan, en ST-19 o en una clasificacion de superposicion; P7b produce huellas tambien para piezas **sin** cambio de mano. La huella es una **sobre-aproximacion** de lo visible que nunca subestima (trazo expandido, grosor, rellenos, regiones sensibles a transparencia, mascaras y fondos de texto, anotaciones y cotas, marcos y bordes, contenido anidado aplanado); no disponible, infinita o no garantizada conservadora ⇒ `UNKNOWN` (ST-19 ⇒ E10; orden de plan ⇒ X-22). `GeometrySymmetryResult` demuestra simetria; `VisualFootprintResult` demuestra alcance visual y superposicion | CLOSED BY V9 (contrato); dilataciones reales por clase: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §9.1 P7; §11.3 punto 5; G-M23; T-M71; CT-36; CT-49 |
| `[V9-D03]` | AR8-02 · R:V9-02 · C:V9-03 | §11.3 punto 6.6 juzgaba la superposicion «sobre la geometria aplanada de cada instancia» sin fuente de datos | **PLAN-LEVEL ORDER sobre huellas**: para cada par de instancias de pieza, **con o sin cambio de mano**, cuyo orden relativo cambia entre `F ∘ Π(D)` y `Π(μ_k D)`, se obtienen sus huellas; sin superposicion material, el orden es irrelevante; con superposicion material, `RelativePlanVisualOrder` tiene que conservarse; huella o superposicion no clasificable ⇒ X-22. CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance | CLOSED BY V9 (contrato); pares reales y alcance: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §6.6; §11.1; §11.3 punto 6.6; T-M67; CT-48; R-35 |
| `[V9-D04]` | AR8-01 · R:V9-01 · C:V9-04 | «Extension evaluada de la copia» sin definir; orden efectivo solo con `SortentsTable` | **MODEL SPACE FOOTPRINT / ORDER CONTEXT**: `VisualFootprint(c)` = union conservadora de las huellas de las instancias del plan de cada vista seleccionada, transformadas por `P'`, mas las anotaciones y cotas regeneradas; `VisualFootprint(o)` = huella conservadora del objeto existente. `Extents3d` nunca se usa sin declarar sus limites: como primera sobre-aproximacion se dilata segun grosor, mascaras, marcos y representacion conocida; extension no disponible, infinita o dudosa ⇒ E10. CT-49 caracteriza el orden efectivo con `SortentsTable`, `SORTENTS`, `DRAWORDERCTL` y cualquier contexto adicional; si el contexto efectivo no puede leerse, no se afirma equivalencia de orden (E10) | CLOSED BY V9 (contrato); contexto y dilataciones reales: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §4.1 ST-19; §9.1; §11.3 punto 5; CT-49; T-M69 |
| `[V9-D05]` | AR8-05 · R:V9-03 · C:V9-05 | Solo `CELTSCALE` declarado como valor por defecto de creacion; presentacion interna de la copia sin contrato | **MATERIALIZATION CONTEXT** (decision vinculante del Coordinador): I-52 es fiel al **plan regenerado** del documento authored, como `RACKEDITAR` → Actualizar, y **no** clona la presentacion historica de cada referencia interna del bloque fuente. `MaterializationContext` (nombre no contractual) se captura para la generacion de `Π(D)` y `Π(μ_k D)` y caracteriza al menos `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT`, `CETRANSPARENCY` y `CPLOTSTYLE`/`PSTYLEMODE`, mas cualquier otra autoridad de valores por defecto de creacion que descubran CT-36, CT-49 o CT-50. Regla: `Π(D)` y `Π(μ_k D)` se evaluan y materializan bajo el **mismo** contexto; un productor que aplique valores por defecto de forma asimetrica ⇒ `UNKNOWN` / `FAIL_CLOSED`. La presentacion EXTERIOR de la referencia sigue preservada por ST-13. Sin obligacion de tocar drawers existentes | CLOSED BY V9 (contrato); autoridades reales de valores por defecto: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §8.7; §11.3 puntos 4 y 6.2; L-36; PRE-17; T-M72; CT-50; M-37 |
| `[V9-D06]` | AR8-03 · R:V9-04 · C:V9-06 | Censo literal `33 → 34` (PDC-8, CT-14, T-M16, G7, §20.2) | **COMMAND CENSUS = B + 1**: `B` = numero de `[CommandMethod(` del Plugin en el SHA de reconciliacion inmediatamente anterior a introducir `RACKMIRROR`; esperado tras I-52 = `B + 1`; `RACKMIRROR` sin alias inicial. CT-14, T-M16, el gate G7 y las guardas usan `B`. Preflight de V9: `main @ 104ef3a` = 33 (re-medido); I-54 G7B (`18cf2dc`, sin integrar) = 35 en su rama. Si I-54 integra primero, previsiblemente `B = 35` y `RACKMIRROR` lleva el censo a 36: se re-mide, nunca se asume. `SelectiveEditorOpenTests.cs` y `RackCommandReference.cs` se serializan frente a las iniciativas que integren antes | CLOSED BY V9 | PDC-8; CT-14; T-M16; §14.2 G7; §15.2; §20.2 |
| `[V9-D07]` | AR8-07 · R:V9-05 · C:V9-07 | «Retorno 0 o bloque ausente ⇒ `UNKNOWN`»; G-M20 ambigua frente al clonado desde el dibujo | **BLOCK LIBRARY CLONE RESULT**: el retorno de `EnsureBlocks` es **diagnostico** (tambien devuelve 0 cuando no faltaba ningun bloque, `BlockLibraryImporter.cs:69-72`). Autoridad: tras el intento de clonado, **todos** los BTR requeridos existen en la base auxiliar; si falta alguno ⇒ `UNKNOWN`. Dos rutas autorizadas: (1) definicion de biblioteca: `BlockLibraryImporter.EnsureBlocks(auxDb, nombres)` o la API publica vigente; (2) definicion ya presente en el dibujo: clonado de solo lectura del dibujo hacia la base auxiliar por la costura de AutoCAD autorizada y caracterizada. G-M20 prohibe la reflexion sobre `cachedLibrary` y la mutacion directa de la cache; **no** prohibe el clonado de solo lectura desde el dibujo ni `EnsureBlocks` hacia la base auxiliar | CLOSED BY V9 | §9.5 (IF-8 y punto 7); §11.3 punto 14; G-M20; T-M63 |
| `[V9-D08]` | AR8-08 · R:V9-06 · C:V9-08 | Completitud por CT-36 y CT-49 sin metodo declarado; referencia anidada sin exigencia sobre atributos ni XREF | **CLOSED INVENTORY METHOD**: CT-36 y CT-49 declaran como descubren propiedades y estado, con un harness fuera de `src/`: tipo exacto en ejecucion; propiedades publicas relevantes por reflexion; codigos DXF y estado serializable por filer o mecanismo soportado; diccionario de extension; XData; filtros espaciales; atributos; estado anotativo; material; sombras; estilo visual; cualquier otra presentacion descubrible. Toda propiedad, entrada o estado encontrado se clasifica como `TRANSPORTED`, `PLACEMENT`, `COMMON_CONTEXT`, `NON_VISUAL` o `UNSUPPORTED`; nada queda sin clasificar; sin clasificacion ⇒ `UNKNOWN` / E4. La referencia anidada del primer corte exige ademas `AttributeCollection` vacia y definicion no XREF, no overlay y no dependiente, junto a tipo exacto `BlockReference`, definicion no anotativa, sin filtro espacial y `Normal ≈ +Z` | CLOSED BY V9 (metodo); inventario real: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §4.1 ST-18; §11.3 puntos 6.1 y 14; CT-36; CT-49; T-M64; T-M68 |
| `[V9-D09]` | AR8-06 · R:V9-07 · C:V9-09 | `ColorBook(libro, nombre)`; `PlotStyleName` asignado siempre; «en el mismo dibujo lo son siempre» | **TYPED SOURCES PRECISION**: identidad de `ColorBook` = `(libro, nombre, valor de color almacenado)`, no solo libro y nombre. ST-13: con estilos de trazado con nombre (STB) se preserva `PlotStyleName` segun contrato; con estilos dependientes del color (CTB) el estilo de trazado deriva de `Color` y **no** se asigna una propiedad independiente incompatible. CT-49 confirma la transportabilidad; se elimina «en el mismo dibujo siempre es transportable» y rige el comportamiento caracterizado o el fail-closed | CLOSED BY V9 | §4.1 ST-13; §8.2; §11.3 punto 6.2; T-M54; T-M68; CT-49 |
| `[V9-D10]` | AR8-04 · R:V9-08 · C:V9-10 | CA-4b descrita como «coleccion de definiciones» del NOD y «la copia resuelve sus propiedades de rack contra la coleccion del NOD» | **CA-4b TERMINOLOGY**: CA-4b = Custom Properties de alcance **PROYECTO**, persistidas en el NOD `RACKCAD_CUSTOM_PROPERTIES`. Proyecto y Rack son alcances separados (ADR-0039 §3: identidad local al alcance y sin herencia ni valor por defecto entre Proyecto y Rack en V1). La regla de I-52 no cambia: RACKMIRROR no lee CA-4b para reflejar el rack, no la copia, no la transforma y no la escribe; CA-4 (alcance Rack) sigue en el sobre | CLOSED BY V9 | §6.4; §10.4; §15.2; G-M22; ADR-0036 decision 10 |
| `[V9-D11]` | C:V9-11 | I-54 `5c16ae8` (G7A solo docs; G7 autorizado) | **I-54 G7B productivo** (`18cf2dc`, sin integrar) y **G7-CLOSE** `02987bd` (solo docs; G8 NOT AUTHORIZED): `RackPropiedadesCommands.cs` (`RACKPROPIEDADES` y `RPR`), `RackCustomPropertiesWindow`, una entrada en `RackCommandReference.cs` y censos 33 → 35, con T-GRD-08. Clasificacion: materialidad arquitectonica para I-52 = **NON-MATERIAL**; materialidad de coordinacion = **MATERIAL**; censo de comandos = **MATERIAL**; `RackCommandReference.cs` como archivo caliente = **MATERIAL TO INTEGRATION ORDER**. Si I-54 integra antes de I-52 G7: re-medir `B`, serializar `RackCommandReference.cs` y re-ejecutar las guardas de comandos y de ayuda. No necesita Proposal nueva por si mismo | CLOSED BY V9 | §10.4; §15; §19; T-M16; G-M18 |
| `[V9-D12]` | AR8-09 · R:V9-09 · C:V9-12 | «Hoy main = 1b091be»; I-53S en el protocolo de diff; «gate posterior de I-53S»; textos de estado de I-49 con ADR-0038, `c4880af` y G6 BLOCKED | **Paralelas y textos de proceso**: hoy main = `104ef3a`; I-53S esta integrada y no es rama paralela; I-53D es la linea funcional futura: abrio G7 durante V9 (`d280197`) y se evalua frente a C2-4 en `[V9-D22]`; I-49: ADR-0040 es la autoridad vigente, ADR-0038 esta reemplazado y `PlanReadSet`/D19 no cambian materialmente; su G6 (`5f969cc`, observado durante V9) es productivo en `RackCad.Application.Expressions` y `RackCad.Application.Units`, sin `PlanReadSet` ni dominio (`[V9-D21]`) | CLOSED BY V9 | §6.5.5; §6.7; §10.2; §11.3 punto 12; §14.1; §15; §19 |
| `[V9-D13]` | C:V9-13 · R:V9-01..R:V9-09 | ADR-0036 con ST-19 restringido, «coleccion de definiciones» y sin huellas ni contexto de materializacion | ADR-0036 sigue **PROPOSED** y se alinea: ST-19 completo (todo objeto posterior, no solo no seleccionado); `VisualFootprintResult`; orden visual de plan sobre huellas; `MaterializationContext`; censo `B + 1` donde corresponde; CA-4b de alcance Proyecto; `ColorBook` y estilo de trazado tipados; autoridad del clonado de bloques; «la caracterizacion **DEBE** demostrar»; consecuencias genericas | CLOSED BY V9 | ADR-0036 |
| `[V9-D14]` | C:V9-14 | Plan de G3 de V8 | CT-36, CT-47, CT-48 y CT-49 actualizadas y CT-50 nueva (`MaterializationContext`); T-M67 y T-M69 ampliadas y T-M71 y T-M72 nuevas; G-M20 precisada y G-M23 nueva. Casos de ST-19: otra fuente seleccionada posterior, objeto seleccionado e ignorado posterior, objeto no seleccionado posterior, objeto coincidente, `XLINE` o extension no finita, mascaras, rellenos y grosor, contextos `SORTENTS`/`DRAWORDERCTL`. Orden de plan: piezas sin cambio de mano, huellas ausentes, pares invertidos, superposicion material. `MaterializationContext`: cambiar `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT` y `CETRANSPARENCY` y comprobar las dos generaciones. CT-49 con inventario cerrado. G3 sigue sin produccion; contradiccion material ⇒ Proposal V10 | CLOSED BY V9 | §12; §13; §14 |
| `[V9-D15]` | C:V9-15 · R:V9-10 | Contradiccion material de V8 ⇒ Proposal V9 | Nueva matriz `[V9-Dnn]` sobre AR8-01..AR8-09 y las decisiones vinculantes del Coordinador; lo demas de V8 sigue vigente; `OPEN MATERIAL = NONE` para revision; toda contradiccion material de V9 ⇒ **Proposal V10**; proceso: acuerdo sobre V9 → rebase/reconciliacion sobre el main vigente → freeze por SHA exacto → re-check exact-SHA → O-1 → G3 → (Proposal V10 si G3 contradice materialmente / ADR-0036 aceptado) → G4 | CLOSED BY V9 | §1.2; §7.0; §14.1; §16; §19 |
| `[V9-D16]` | Precision al redactar | §1.4 con la medicion de la revision de V7 | §1.4 registra lo que midio la revision de V8 sobre el volcado de la revision de V6 (misma biblioteca, mismo SHA-256): 0 ATTRIB y 0 SEQEND en las 151 definiciones (el recorrido con `entnext` incluye subentidades), asi que ninguna referencia anidada lleva atributos; y 1 435 entidades en capa `0`, que heredan la capa de su referencia de pieza (relevante para `MaterializationContext`). Las reglas nuevas de V9 no se midieron sobre la biblioteca: sus efectos los miden CT-48, CT-49 y CT-50 | CLOSED BY V9 (registro) | §1.4 |
| `[V9-D17]` | Precision al redactar | — | I-54 G7B añade **T-GRD-08**, que fija por NOMBRE el censo de comandos (los 33 de apertura mas `RACKPROPIEDADES` y `RPR`; `CustomPropertiesCommandGuardTests.cs`) y por PARES el de la ayuda (14 de apertura mas `RACKPROPIEDADES/RPR`; `CustomPropertiesHelpCensusTests.cs`), y reapunta `GUARDA_EL_CENSO_DE_COMANDOS_NO_CAMBIA` de 33 a 35. Si I-54 integra antes de I-52 G7, T-M16 reapunta con motivo esas guardas añadiendo `RACKMIRROR` sin alias, sin debilitar la clasificacion por nombre ni por pares. El censo de ventanas (29 → 30 en I-54) no cambia con I-52: RACKMIRROR no abre ventana | CLOSED BY V9 | §15.1; §15.2; T-M16; §14.2 G7 |
| `[V9-D18]` | Precision al redactar | — | Notacion: §11.1 define `A` = plan reflejado semanticamente (`Π(μ_k D)`) y `B` = plan original transformado (`F ∘ Π(D)`); la orden de V9 usa esas letras al reves en V9-03. V9 conserva la notacion de §11.1 y escribe las expresiones explicitas en §11.3 punto 6.6 | CLOSED BY V9 | §11.1; §11.3 punto 6.6 |
| `[V9-D19]` | Precision al redactar | — | ST-19 y contexto: (a) si CT-49 encuentra contextos de pantalla y de trazado con ordenes efectivos distintos, «posterior» significa posterior en cualquiera de ellos; (b) `MaterializationContext` se captura en SNAPSHOT, entra en la huella de §9.2 y se re-verifica en PREPARE y en MUTATE (cambio ⇒ E10/E11), para que la generacion y la materializacion usen el mismo contexto; (c) ST-19 consume en PREPARE las huellas de P7 transformadas por `P'`, sin importar bloques | CLOSED BY V9 | §4.1; §8.7; §9.1; §9.2; PRE-17 |
| `[V9-D20]` | Precision al redactar | L-01..L-35 | O-1 se reevalua sobre L-01..L-36: L-36 nueva (piezas internas regeneradas bajo el `MaterializationContext` vigente, como Actualizar); L-34 y L-35 reescritas (todo objeto posterior a la fuente; pares invertidos con huellas); L-30 ampliada (atributos y definiciones XREF anidados, inventario cerrado) | CLOSED BY V9 | §1.2; §1.3 |
| `[V9-D21]` | Precision al redactar | — | Paralelas movidas durante la redaccion de V9, registradas sin bloquear: **I-49 G6** `5f969cc` (productivo, sin integrar: nucleo semantico del motor —A1, enlace, evaluacion de un arbol y P2.5— y autoridad neutral de unidades `RackCad.Application.Units`, con `LengthUnitsAuthorityGuardTests` sobre todo `src/` y `StructuralSectionUnits` como alias con API y valores identicos; cero cambios en ProjectVariables, Persistence, Systems, Domain, UI y Plugin; sin `PlanReadSet`, dominio aplicado ni evaluacion del registro) ⇒ contrato de I-52 **NON-MATERIAL** y coordinacion **MATERIAL** (CT-46 y G-M18 releen sus guardas al reconciliar; I-52 no declara literales ni conversiones de unidades de longitud); **I-54 G7-CLOSE** `02987bd` (solo docs; G8 NOT AUTHORIZED) ⇒ **NON-MATERIAL** | CLOSED BY V9 (registro) | §6.7; §15.1; §15.2; §15.3; CT-46; G-M18 |
| `[V9-D22]` | C:V9-12 · Precision al redactar | I-53D `aa0f817` (bootstrap solo docs; «si abre G7 durante V9, evaluar su UI Dinamica frente a C2-4») | **I-53D G7 observado durante V9** (`d280197`, sin integrar): `RackDynamicSystemWindow` conecta `DynamicHeaderBatch`, `DynamicHeaderBatchState`, `DynamicModuleTargets` y `DynamicRackRebuild` (fundacion de I-53 E1) y `DynamicEditorDesignAssembler` pierde el par ordinal `SnapshotHeaderFondos`/`RestoreHeaderFondos`; cero cambios en `Systems.Shared`, DTO, persistencia, Domain, Plugin, Selectivo y Push Back productivo. Evaluacion frente a C2-4, con la misma clasificacion que I-53S: contrato de I-52 = **NON-MATERIAL**; coordinacion = **MATERIAL**; baseline de C2-4 del Dinamico = **MATERIAL / CHANGED** (`DynamicEditorDesignAssembler` es productor de C2-4 y la ventana pasa ahora por el lote y la reconstruccion). Si integra **antes** del Candidato de I-52: tras el rebase, CT-26, C2-4 del Dinamico, T-M30 y las guardas relacionadas sobre el arbol combinado condicionan G6/G7; si integra **despues**: reportar al Coordinador y re-medir contra un main que contenga I-52. No toca ninguna autoridad de §19: sin STOP | CLOSED BY V9 (registro y regla) | §6.7; §8.5; §14.2 G6; §15; §19; CT-41 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V10 o reduccion de alcance |
...
| **Completitud de las propiedades y el estado por clase soportada, con el metodo cerrado** (`[V8-D01]`, `[V9-D08]`) | propiedad, entrada o estado sin clasificar o `UNSUPPORTED` ⇒ `UNKNOWN` (X-17b) | G3 (CT-36) |
| **`Elevation` admitida en HATCH solido** (`[V8-D04]`) | `Elevation ≠ 0` ⇒ `UNKNOWN` | G3 (CT-36) |
| **Contexto reproducible de la base auxiliar** (`PSTYLEMODE`, `INSUNITS`, escala de anotacion y lo que se descubra) (`[V8-D07]`) | contexto no reproducible o no verificado ⇒ `UNKNOWN` (X-17a) | G3 (CT-36, dibujo STB y CTB) |
...
| **Autoridades reales de los valores por defecto de creacion** (`MaterializationContext`, `[V9-D05]`) | productor asimetrico ⇒ `UNKNOWN` (X-17a); contexto cambiado antes de MUTATE ⇒ E10/E11 | G3 (CT-50, CT-36, CT-49) |
...
| **Pares de instancias con orden relativo invertido por kind y vista, y superposicion de sus huellas** (`[V9-D03]`) | par con huellas superpuestas de forma material o huella `UNKNOWN` ⇒ X-22 | G3 (CT-48, CT-36) |
| **Dilataciones conservadoras de las huellas visuales por clase** (`[V9-D02]`) | huella no garantizada conservadora ⇒ `UNKNOWN` (E10 en ST-19, X-22 en orden de plan) | G3 (CT-36, CT-49) |
| **Inventario cerrado de la presentacion de la referencia fuente** (`[V8-D14]`, `[V9-D08]`) | propiedad, entrada o estado sin clasificar o `UNSUPPORTED` ⇒ E4 (ST-18) | G3 (CT-49) |
| **Superposiciones con todo objeto posterior a la fuente y orden efectivo en Model Space** (`[V9-D01]`, `[V9-D04]`) | superposicion material o no clasificable, huella no disponible o no conservadora, o contexto de orden no legible ⇒ E10 (ST-19) | G3 (CT-49) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V10 |
| Contenido de la cache de biblioteca medido antes y despues | contrato READ / CLONE ONLY: `EnsureBlocks` hacia la base auxiliar o clonado de solo lectura desde el dibujo, y presencia de todos los BTR requeridos (el retorno es diagnostico); G-M20 | G3 (CT-36, harness fuera de `src/`) y G6/G7 (T-M63) |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V10 | G3 (CT-37, CT-38) |
...
| **C2-4 del Dinamico frente a I-53D G7** (`d280197`, `[V9-D22]`) | sin integrar: si integra antes del Candidato de I-52, G6/G7 no cierran sin CT-26, C2-4 del Dinamico y T-M30 verdes sobre el arbol combinado; si integra despues, reporte y re-medicion contra un main con I-52 | rebase, G6/G7 o reconciliacion posterior |
| **Censo `B` y censos por nombre** (`[V9-D06]`, `[V9-D17]`) | `B` medido en el SHA de reconciliacion; censo o nombres distintos ⇒ RED en G7 | rebase y G7 |
| **Guardas del borde fisico y de comandos de I-54** (T-GRD-04..T-GRD-08, `[V8-D19]`, `[V9-D11]`) | sin llamadas de I-52 a sus autoridades (G-M22); al integrar, re-medicion, G-M18 y T-GRD-08 reapuntado con motivo (T-M16) | reconciliacion posterior a la integracion de I-54 |
| **Guardas de I-49 G6** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones, `[V9-D21]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18 | reconciliacion posterior a la integracion de I-49 |

### 0.4 Estado de las marcas de V8 bajo V9

| Marca V8 | Estado en V9 |
|---|---|
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V10) |
...
### 0.5 Estado de las marcas de V7 (fijado por V8; sin cambios en V9)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V10) |
...
### 0.6 Estado de las marcas de V6 (fijado por V7; sin cambios en V8 ni V9)
...
### 0.7 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8 ni V9)
...
### 0.8 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V9) |
|---|---|
...
### 0.9 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR8-01 (ST-19 completo y huella de Model Space) | `[V9-D01]`, `[V9-D04]`, `[V9-D19]` |
| AR8-02 (huellas visuales y orden de plan) | `[V9-D02]`, `[V9-D03]` |
| AR8-03 (censo de comandos) | `[V9-D06]`, `[V9-D17]` |
| AR8-04 (CA-4b) | `[V9-D10]` |
| AR8-05 (contexto de materializacion) | `[V9-D05]`, `[V9-D20]` |
| AR8-06 (fuentes tipadas y estilo de trazado) | `[V9-D09]` |
| AR8-07 (clonado de bloques) | `[V9-D07]` |
| AR8-08 (metodo de inventario y referencia anidada) | `[V9-D08]`, `[V9-D16]` |
| AR8-09 (textos obsoletos) | `[V9-D12]` |
| Cambios exactos R:V9-01..R:V9-10 de la revision | V9-01 → D01, D04; V9-02 → D02, D03; V9-03 → D05; V9-04 → D06; V9-05 → D07; V9-06 → D08; V9-07 → D09; V9-08 → D10; V9-09 → D12; V9-10 → D15 |
| Precisiones del Coordinador C:V9-01..C:V9-15 | V9-01 → D01; V9-02 → D02; V9-03 → D03; V9-04 → D04; V9-05 → D05; V9-06 → D06; V9-07 → D07; V9-08 → D08; V9-09 → D09; V9-10 → D10; V9-11 → D11; V9-12 → D12, D22; V9-13 → D13; V9-14 → D14; V9-15 → D15 |
| Paralelas observadas en el preflight y durante la redaccion de V9 | `[V9-D11]`, `[V9-D12]`, `[V9-D17]`, `[V9-D21]`, `[V9-D22]` |
| Precisiones del ejecutor | `[V9-D16]`..`[V9-D21]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V9; **no** son consenso final.
...
| **PDC-8** | Unico comando nuevo `RACKMIRROR`, **sin alias**. Censo de `[CommandMethod(` **B → B + 1** | `B` = censo del Plugin en el SHA de reconciliacion inmediatamente anterior a introducir `RACKMIRROR`; se re-mide y nunca se asume (hoy `main @ 104ef3a` = 33; I-54 G7B, sin integrar, lleva su rama a 35). El literal 33 → 34 de V1..V8 queda sustituido (`[V9-D06]`) |
| **PDC-9** | Solo Model Space; MINSERT ⇒ fail-closed; linea por dos puntos UCS→WCS; fuente canonica | `Origin = 0` (ST-14), `View`/`Section` decodificables (ST-15), bloques dinamicos, anonimos o anotativos ⇒ fail-closed (ST-17); las restricciones estrictas solo se aplican a candidatos RackCad (precedencia C1..C5, §4.1); la copia conserva la presentacion exterior de su referencia fuente (ST-13), un estado de presentacion no transportable o sin clasificar falla cerrado (ST-18), el orden visual en Model Space se demuestra frente a todo objeto posterior a cada fuente o falla cerrado (ST-19) y las piezas internas se regeneran bajo el `MaterializationContext` vigente (§8.7) (`[V8-D13]`, `[V8-D14]`, `[V9-D01]`, `[V9-D05]`) |
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-36), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V10 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-3** | **Evidencia de simetrias** | ningun bloque se asume simetrico; la prueba es la **equivalencia visual-geometrica** por estado evaluado, con politica cerrada de clases y variantes, fuentes visuales simbolicas tipadas, `VisibilityPath` y orden visual dentro de la pieza y entre piezas sobre huellas visuales conservadoras (`GeometrySymmetryResult` y `VisualFootprintResult`, §11.3); la confirmacion del Owner **nunca** es la unica prueba | G3 (CT-36, CT-47, CT-48, CT-49) y G9 (M-15, M-27, M-32, M-36) |
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V9; si la contradice ⇒ Proposal V10; **precondicion de G4** |
...
| **L-34** | **Orden visual en Model Space no demostrable**: una copia se superpone de forma visualmente material, o no clasificable, con **cualquier** objeto de Model Space posterior a su fuente en el orden efectivo (otra fuente admitida, un objeto seleccionado pero ignorado, uno no seleccionado u otro rack), o alguna huella visual o el contexto de orden no pueden garantizarse (`[V9-D01]`, `[V9-D04]`) | fail-closed en PREPARE, antes de MUTATE (E10) | ST-19, PRE-16 |
...
| **L-36** | **Presentacion interna regenerada bajo el contexto vigente** (`[V9-D05]`): las referencias internas de la copia (piezas y colocaciones de grupos) se crean con los valores por defecto de creacion del dibujo al materializar (`MaterializationContext`: `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT`, `CETRANSPARENCY`, `CPLOTSTYLE`), igual que `RACKEDITAR` → Actualizar; **no** se copian fisicamente desde la definicion vieja, asi que la copia puede verse distinta de una fuente dibujada con otros valores por defecto. La presentacion exterior de la referencia se conserva (ST-13); un productor que aplique esos valores de forma asimetrica entre las dos generaciones falla cerrado | regenerada como Actualizar; asimetria o contexto cambiado ⇒ fail-closed (X-17a, E10/E11) | §8.7; ST-13; PRE-17 |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8 y V9) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9 (ST-19 completo, huellas visuales,
orden de plan sobre pares invertidos y `MaterializationContext`) no se han medido sobre esta biblioteca y las miden
...
salvo los de PREPARE (re-verificacion de definiciones, ST-19 y `MaterializationContext`, E10). La vista del Owner es la
envolvente de §1.3; la evidencia indicativa de la biblioteca inspeccionada esta en §1.4.
...
| **Productor que aplica los valores por defecto de creacion de forma asimetrica entre `Π(D)` y `Π(μ_k D)`, o `MaterializationContext` cambiado antes de MUTATE** | las dos generaciones y la materializacion no comparten el mismo contexto (§8.7) | X-17a; PRE-17; L-36 |
| **Orden visual entre piezas del plan no conservado** | huellas visuales superpuestas de forma material entre instancias de pieza (con o sin cambio de mano) cuyo orden relativo cambia en el diseño reflejado, o huella o superposicion no clasificable | X-22 (§11.3 punto 6.6) |
...
| **Orden visual en Model Space no demostrable** | una copia se superpone de forma material, o no clasificable, con cualquier objeto de Model Space posterior a su fuente (otra fuente admitida, objeto seleccionado e ignorado, objeto no seleccionado u otro rack), o una huella visual o el contexto de orden no pueden garantizarse | ST-19 (E10) |
...
PREPARE (depende de la linea): ST-19 orden visual en Model Space frente a todo objeto posterior a cada fuente, con
         las huellas de P7, antes de EnsureForPlan (§9.1)
```
...
| **ST-19** | PREPARE | Orden visual en Model Space (`[V9-D01]`, `[V9-D04]`, `[V9-D19]`): las copias se crean en el **orden efectivo relativo de sus fuentes** (orden de dibujo de Model Space con su `SortentsTable` y el contexto de orden que fije CT-49); cada copia queda despues de todo objeto existente. Para cada copia `c` de su fuente `s` y **todo** objeto existente `o` de Model Space con `o ≠ s` que va **despues** de `s` en el orden efectivo —otra fuente admitida seleccionada, un objeto seleccionado pero ignorado por C1..C3, un objeto no seleccionado, otro rack o cualquier entidad— se clasifica la superposicion de `VisualFootprint(c)` y `VisualFootprint(o)` (§11.3 punto 5) con las clases de §11.3 punto 6.5 ampliadas a Model Space (rellenos, degradados, solidos, wipeouts, imagenes, OLE, texto con mascara, transparencia no opaca y referencias que los contengan). `VisualFootprint(c)` = union conservadora de las huellas de las instancias del plan de cada vista seleccionada transformadas por `P'`, mas anotaciones y cotas regeneradas; `VisualFootprint(o)` = huella conservadora del objeto (`Extents3d` solo dilatado segun grosor, mascaras, marcos y representacion conocida). Los objetos anteriores a `s` no se clasifican; la fuente `s` es excepcion deliberada (la copia puede quedar encima de ella); las copias entre si siguen el orden de sus fuentes. Si CT-49 encuentra contextos de pantalla y de trazado con ordenes distintos, «posterior» significa posterior en cualquiera de ellos | superposicion **material o no clasificable**, huella no disponible, infinita o dudosa, u orden efectivo no legible ⇒ **fail-closed (E10)** en PREPARE, tras calcular `P'` y antes de `EnsureForPlan` y de MUTATE (`[V8-D25]`) |
...
**Presentacion y orden de la referencia (`[V8-D13]`, `[V8-D14]`, `[V9-D01]`, `[V9-D04]`, `[V9-D09]`).** La apariencia
exterior de la referencia RackCad fuente forma parte del contrato. La copia conserva las propiedades de ST-13 con el
comportamiento que caracterice CT-49; lo que no se pueda transportar o no este caracterizado falla cerrado antes de
MUTATE (ST-18) y nunca se sustituye por un aviso. CT-49 inventaria, con el metodo cerrado de §11.3 punto 14, las
propiedades y el estado de `Entity` y `BlockReference` y los clasifica como `TRANSPORTED` (ST-13), `PLACEMENT` (`P'`),
`COMMON_CONTEXT`, `NON_VISUAL` o `UNSUPPORTED`; sin clasificar o `UNSUPPORTED` ⇒ E4. Las piezas internas de la copia no
copian la presentacion historica de la definicion fuente: se regeneran bajo el `MaterializationContext` vigente (§8.7),
como Actualizar. El orden en Model Space se demuestra en PREPARE (ST-19): las copias entre si siguen el orden de sus
fuentes; frente a un objeto anterior a la fuente la relacion se conserva sin clasificar nada, porque la copia queda
despues de ambos; frente a **todo** objeto posterior a la fuente, seleccionado o no, admitido o ignorado, la
superposicion de huellas se clasifica y, si es material o no clasificable, la operacion falla cerrado (E10); una copia
que se superpone con su propia fuente queda encima de ella, como la copia del `MIRROR` nativo, sin relacion previa que
conservar.
...
**STOP → Proposal V10**. No se duplica la agrupacion en un segundo planificador. Las guardas G-R1..G-R6 leen el codigo de
...
- RACKMIRROR tiene **su propio** SNAPSHOT neutral, en un archivo nuevo del Plugin. Captura por referencia: handle,
  ...
  `MaterializationContext` (§8.7), `Origin` de la definicion y payload. Lee el registro **una** vez por comando y
  registra su read-set (§9.2).
...
| CA-4 | `CustomProperties` (I-54) | miembro declarado del sobre cuando I-54 este integrada; **antes**, clave `CustomProperties` de `ExtensionData` del sobre | **portador RACK-LEVEL** (`RackEmbedDocument.CustomProperties`; D-21 de I-54; `JsonElement?` heredado por `Compose`): se **preserva con la copia**; ADR-0039 aceptado en su rama sin cambiar ese contrato; G6 de I-54 (`deca1e0`, antes `ca71962`) escribe esos valores con `RackBlockData.Write` sin cambiarlo y G7B (`18cf2dc`) solo añade el comando `RACKPROPIEDADES` y su ventana (`[V8-D19]`, `[V9-D11]`) | I-54 Proposal V5 @ `26ca923` §D-21; ADR-0039 @ `3866252` (copias tras rebasar I-54 sobre `104ef3a`: `1e62e84` y `c5662e2`; antes `64efe8d` y `238cbf7`); I-54 G6 `deca1e0` (antes `ca71962`) y G7B `18cf2dc` |
| CA-4b | NOD `RACKCAD_CUSTOM_PROPERTIES` (I-54 G6) | diccionario de objetos con nombre del **dibujo**, fuera del sobre (`CustomPropertiesData.DictKey`) | **Custom Properties de alcance PROYECTO**, no portador del rack (`[V9-D10]`): Proyecto y Rack son alcances separados (ADR-0039 §3: identidad local al alcance y sin herencia ni valor por defecto entre Proyecto y Rack en V1); RACKMIRROR **no** lee CA-4b para reflejar el rack, **no** la copia, **no** la transforma y **no** la escribe; no entra en V-META porque no esta en el sobre; ningun archivo de I-52 llama a sus autoridades ni a su commit fisico (G-M22) | I-54 G6 `deca1e0`, antes `ca71962` (`src/RackCad.Plugin/CustomPropertiesData.cs`, `src/RackCad.Plugin/CustomPropertiesExecutor.cs`); G6-CLOSE `d9f8c57`, antes `b4e99e4`; ADR-0039 §3 (I-54 @ `18cf2dc`) |
...
- **CA-4b no es portador.** Figura en la tabla solo para fijar que el espejo no toca las Custom Properties de alcance
  Proyecto de I-54 (`[V9-D10]`). Las propiedades de alcance Rack de la copia viajan en el sobre (CA-4) y no se resuelven
  contra CA-4b: son alcances separados. La allowlist de portadores sigue siendo CA-1..CA-6.
...
- **Si I-49 integra antes** (hoy: V6 y Amendment A1 congelados, ADR-0040 aceptado en lugar de ADR-0038, G5 sintactico
  ...
  dominio aplicado y sin `PlanReadSet`; `[V9-D12]`, `[V9-D21]`): el dominio que su descriptor declara **como dato** y
  aplica al valor efectivo (I-49 V6 P24.5: `>= 0` en las dos propiedades Selectivas) es la cota `b`. V-DEP la consume
  como dato; I-52 no la reimplementa ni la infiere. Se re-mide en la reconciliacion (§15.3). Un dominio `>= 0` **no**
  convierte en universal la evidencia visual-geometrica de DEP-09/DEP-10a..c (sigue siendo un continuo de estados).
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V9: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V9: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V9 ⇒ **STOP → Proposal V10**.
...
- Desde V8, y sobre huellas visuales desde V9, V-VIEW-ALL no es un multiconjunto puro para los pares de instancias de
  pieza, con o sin cambio de mano, cuyo orden relativo cambia y cuyas huellas se superponen de forma visualmente
  material: su orden relativo de plan se conserva o la vista falla (X-22; §11.3 punto 6.6, `[V8-D12]`, `[V9-D03]`).
...
| `LengthUnitsAuthorityGuardTests` (nueva) y `ExpressionCoreGuardTests`/`ExpressionParserGuardTests` extendidas | I-49 G6 `5f969cc` | ningun archivo de producto fuera de `RackCad.Application.Units` (y del alias `StructuralSectionUnits`) declara literales o conversiones de unidades de longitud (`25.4`, `InchesPerFoot`, sufijos `[mm]`); el nucleo y las unidades no nombran otras capas ni tipos dimensionales | I-52 no convierte unidades ni nombra el nucleo; al reconciliar con I-49 rige la version vigente (CT-46, G-M18, `[V9-D21]`) |
...
- I-53S G5 (`e528ef2`) conecta `ApplyHeaderBatch` a `RackSelectiveWindow` y **ya esta integrada en main** (`104ef3a`,
  ...
  del Candidato (por ejemplo I-53D, cuyo G7 `d280197` ya conecta el Dinamico en su rama, `[V9-D22]`), se reporta al
  Coordinador y se vuelve a medir contra un main que contenga I-52 (§15.3).
- I-53D G7 (`d280197`, sin integrar) conecta `DynamicHeaderBatch` y `DynamicRackRebuild` a `RackDynamicSystemWindow` y
  retira de `DynamicEditorDesignAssembler` el par ordinal `SnapshotHeaderFondos`/`RestoreHeaderFondos`. Clasificacion
  exacta (`[V9-D22]`): materialidad del contrato de I-52 = NON-MATERIAL; materialidad de coordinacion = MATERIAL;
  baseline de C2-4 del Dinamico = MATERIAL / CHANGED. Si I-53D integra antes del Candidato de I-52, tras el rebase
  CT-26, C2-4 del Dinamico, T-M30 y las guardas relacionadas se ejecutan sobre el arbol combinado y condicionan el
  cierre de G6/G7; si integra despues, se reporta al Coordinador y se vuelve a medir contra un main que contenga I-52
  (§15.3).

---

## 7. Matriz de representabilidad V9
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V9)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V10**. T-M47 lo hace ejecutable desde G5.
...
| X-04 | Colocacion y presentacion de la referencia | `P' = G·P·F`; presentacion exterior de la fuente conservada (ST-13); piezas internas regeneradas bajo el `MaterializationContext` (§8.7) | R | CODE_SUPPORTED | ALLOW | §3; §4.1; §8.7 |
...
| PRE-16 | Orden visual en Model Space no demostrable para alguna copia frente a todo objeto posterior a su fuente, o huella visual o contexto de orden no disponibles (ST-19, `[V9-D01]`, `[V9-D04]`) | FAIL_CLOSED (E10) | PREPARE, antes de `EnsureForPlan` | §4.1; §9.1; CT-49 |
| PRE-17 | `MaterializationContext` distinto entre SNAPSHOT, PREPARE y MUTATE (§8.7, `[V9-D05]`, `[V9-D19]`) | FAIL_CLOSED (E10/E11) | PREPARE/MUTATE | §8.7; §9.2 |
...
- **Presentacion y orden de la referencia** (`[V8-D13]`, `[V9-D01]`, `[V9-D05]`, `[V9-D09]`): la referencia nueva lleva
  la presentacion exterior de su fuente (ST-13, con el estilo de trazado segun STB o CTB) y las referencias se crean en
  el orden efectivo relativo de las fuentes (ST-19). El contenido de la definicion nueva mantiene el orden de
  materializacion de §11.3 punto 6.6 y se crea bajo el `MaterializationContext` vigente (§8.7); el drawer no cambia.
...
| C2-4 | Estado del editor → sistema/documento | `SelectiveEditorState`, `DynamicEditorDesignAssembler`, `PushBackEditorDesignAssembler`, `CantileverLineEditorAssembler` en Core; lo que solo pasa por la ventana, en `tests/RackCad.UI.Tests` (en el Dinamico, si I-53D G7 integra, tambien `DynamicHeaderBatch` y `DynamicRackRebuild` a traves de la ventana, `[V9-D22]`) |
...
### 8.7 Contexto de materializacion (`[V9-D05]`, decision vinculante del Coordinador)

I-52 es fiel al **plan regenerado** del documento authored, como `RACKEDITAR` → Actualizar: **no** clona la presentacion
historica de cada referencia interna del bloque fuente. Hecho del codigo: `LateralHeaderDrawer` crea las colocaciones de
grupos y las referencias de pieza sin asignar presentacion (`LateralHeaderDrawer.cs:60-65`, `:308-317`) y ningun archivo
de `src/` llama a `SetDatabaseDefaults`; su presentacion la fijan los valores por defecto de creacion, cuyo mecanismo
exacto en AutoCAD 2025 caracterizan CT-36 y CT-50. `CantileverViewMaterializer` asigna la capa de rol desde el plan
(`CantileverViewMaterializer.cs:207-214`) y el drawer pone anotaciones y cotas en sus capas propias
(`LateralHeaderDrawer.cs:270`, `:356`).

```text
MaterializationContext (nombre no contractual; no es schema persistido)
  CLAYER · CECOLOR · CELTYPE · CELTSCALE · CELWEIGHT · CETRANSPARENCY · CPLOTSTYLE · PSTYLEMODE
  + toda otra autoridad de valores por defecto de creacion que descubran CT-36, CT-49 o CT-50

Regla
  se captura en SNAPSHOT, entra en la huella de §9.2 y se re-verifica en PREPARE y en MUTATE
    (cambio ⇒ E10 en PREPARE; excepcion y rollback en MUTATE; PRE-17)
  Π_(V,σ)(Eff(D, R)) y Π_(V,σ')(Eff(μ_k D, R)) se evaluan y materializan bajo el MISMO contexto
  la sonda reproduce en la base auxiliar lo que de ese contexto pueda afectar a la evaluacion (§11.3 punto 4)
  productor que aplica los valores por defecto de forma asimetrica entre las dos generaciones
    ⇒ UNKNOWN ⇒ FAIL_CLOSED (X-17a)
...
- La referencia de pieza es la raiz comun `·` de las dos mitades (§11.3 punto 6.2): el contexto no entra en
  `VisualSignature` y la equivalencia se evalua con el mismo contexto en las dos generaciones.
- Hecho del codigo, indicativo: los productores actuales no aplican valores por defecto de forma asimetrica (las
  colocaciones de grupos y las piezas sueltas se crean igual, y las capas de rol, anotaciones y cotas salen del plan);
  CT-50 lo confirma o lo refuta.
- Consecuencia para O-1 (L-36): las piezas internas de la copia se regeneran bajo el contexto vigente, igual que
  Actualizar; no se copian fisicamente desde la definicion vieja, asi que la copia puede verse distinta de una fuente
  dibujada con otros valores por defecto. En la biblioteca inspeccionada, 1 435 entidades de las definiciones estan en
  capa `0` y heredan la capa de su referencia de pieza (§1.4).
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           presentacion tipada, estado no transportable o sin clasificar, orden efectivo en Model Space y su contexto,
           MaterializationContext (§5.2, §8.7), definiciones con Origin y payload, UNA lectura del registro de
           variables, huellas y read-set (§9.2)
...
PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  ...
  P7  V-VIEW-ALL                                   todas las (View, Section canonica) admisibles del rack logico (§3.8):
        ...
        P7b  el Plugin evalua cada estado con la sonda y la semantica de parametros del materializador, en una base
             auxiliar con el contexto de la autoridad origen y sin mutar el DWG ni la cache de biblioteca (§11.3):
             GeometrySymmetryResult (piezas con cambio de mano) con variantes, valores efectivos, fuentes visuales
             simbolicas tipadas, VisibilityPath, contexto de patron, orden visual y transformacion efectiva; y
             VisualFootprintResult (toda instancia enumerada), sobre-aproximacion conservadora de lo visible
...
PREPARE    colocaciones P' · orden visual en Model Space frente a todo objeto posterior a cada fuente, con las huellas
           de P7 (ST-19; antes de importar) · planes efectivos de las vistas SELECCIONADAS (§8.1) · payloads
           definitivos · EnsureForPlan (infraestructura, §9.5) · re-verificacion de bloques · re-verificacion de
           huellas, read-set y MaterializationContext (§8.7)
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           re-verificacion de huellas, read-set y MaterializationContext · todas las definiciones nuevas
...
**ST-19 en PREPARE (`[V8-D25]`, `[V9-D01]`, `[V9-D04]`, `[V9-D19]`).** El orden visual en Model Space depende de la
linea (destino de cada copia), asi que no cabe en PREFLIGHT. Se evalua en PREPARE tras calcular `P'` y **antes** de
`EnsureForPlan`, con las huellas de P7 transformadas por `P'` y las de anotaciones y cotas regeneradas, frente a
**todo** objeto de Model Space posterior a cada fuente: un fallo termina en E10 sin importar bloques. Esos objetos, con
su huella y su posicion en el orden efectivo, entran en la huella de la referencia (§9.2) y MUTATE los re-verifica. Si
CT-49 encuentra contextos de pantalla y de trazado con ordenes efectivos distintos, «posterior» significa posterior en
cualquiera de ellos.
...
| orden efectivo en Model Space de las fuentes y su contexto (`SortentsTable` y las variables que fije CT-49) y, desde PREPARE, de **todos** los objetos de Model Space posteriores a cada fuente que intervienen en ST-19 (identidad, existencia, huella visual y posicion en el orden de dibujo) | un cambio invalida la demostracion del orden (ST-19, `[V8-D25]`, `[V9-D01]`) |
| `MaterializationContext` (§8.7) | la generacion de las dos mitades y la materializacion usan el mismo contexto (`[V9-D05]`, `[V9-D19]`, PRE-17) |
...
| E10 | Orden visual en Model Space no demostrable frente a algun objeto posterior a una fuente, o huella visual o contexto de orden no disponibles (ST-19, antes de importar), fallo de plan, bloque ausente tras importar, definicion real de una pieza con cambio de mano cambiada y ya no equivalente en geometria, fuentes visuales, visibilidad u orden visual (PRE-13), huella, read-set o `MaterializationContext` cambiados (PRE-17) | PREPARE | fin; **ninguna** definicion, referencia ni payload RackCad nuevo; lo importado por `EnsureForPlan` puede permanecer (§9.5) |
| E11 | Excepcion en MUTATE, incluidas huella, read-set o `MaterializationContext` cambiados y `MissingInstances.Count > 0` | MUTATE | sin commit ⇒ rollback de **toda** la operacion semantica |
...
7. **Sonda (`[V9-D07]`).** La sonda obtiene cada definicion por una de dos rutas autorizadas: (1) si solo existe en la
   biblioteca, con `EnsureBlocks(auxDb, nombres)` (o la API publica vigente) sobre una base auxiliar privada —IF-1 no
   aplica a esa base, que no es el documento (CT-36 lo confirma)—; (2) si ya existe en el dibujo, con un clonado de solo
   lectura del dibujo hacia la base auxiliar por la costura de AutoCAD que caracterice CT-36. El retorno de
   ...
   existen en la base auxiliar tras el intento de clonado; si falta alguno ⇒ `UNKNOWN` (X-17a). Nada de esto modifica
   `BlockLibraryImporter.cs` (§20.3).
...
| PV-6 | Si I-49 integra antes (hoy: V6 y Amendment A1 congelados, ADR-0040 aceptado en lugar de ADR-0038; G5 sintactico `977f873`, antes `c4880af`, y G6 `5f969cc` con enlace y evaluacion de un arbol en su rama; sin integrar y sin `PlanReadSet` ni dominio aplicado; `[V9-D21]`): RACKMIRROR invoca la autoridad efectiva integrada, preserva `expression` como portador, adopta su `PlanReadSet` para el read-set (§9.2.1) y su dominio de consumidor como cota autoritativa de V-DEP (§6.5.5), y **no** reimplementa su evaluacion ni un segundo motor de expresiones (P24.5 y P24.8 de I-49 V6) |
...
- **I-54 (Proposal V5 con consenso congelado; ADR-0039 aceptado; G3..G6 cerrados y G7 en curso en su rama, rebasada
  sobre `104ef3a` —G4B `0010144` (antes `7b0f6a9`) declara el miembro en `RackEmbedDocument`, hace que `Compose` lo
  herede con la regla F y añade T-GRD-01..T-GRD-03; G5 `0572be4` (antes `3cf0d35`) es la autoridad y el commit puro en
  `RackCad.Application.CustomProperties`; G6 `deca1e0` (antes `ca71962`) añade en el Plugin la coleccion de proyecto en
  el NOD `RACKCAD_CUSTOM_PROPERTIES` y los ejecutores de Proyecto y de Rack, sin llamadores y con T-GRD-04..T-GRD-07;
  G6-CLOSE `d9f8c57` y G7A `5c16ae8` solo docs; G7B `18cf2dc` productivo: `RackPropiedadesCommands.cs`
  (`RACKPROPIEDADES` y `RPR`), `RackCustomPropertiesWindow`, una entrada de `RackCommandReference.cs`, censos 33 → 35 y
  T-GRD-08—; sin integrar y sin tocar `RackEmbedDocument`, `RackEmbedComposer`, restamp ni `ProjectVariables`,
  `[V8-D19]`, `[V9-D11]`)**: las propiedades de alcance Rack viven en el miembro `CustomProperties` del sobre
  (`JsonElement?`); `Compose` lo hereda y no hay promocion de major. Antes de integrarse, un build sin I-54 lo conserva
  en `ExtensionData` del sobre. En ambos casos es **portador RACK-LEVEL por nombre** (CA-4, §6.4) y el espejo lo
  conserva; las Custom Properties de alcance Proyecto del NOD (CA-4b) son un alcance separado que el espejo no lee para
  reflejar el rack ni toca (G-M22, `[V9-D10]`). **D-21 sin cambio**: el espejo cumple el invariante (A y despues B). V4
  y V5 de I-54 declaran residuales preexistentes del store del sobre que afectan a todo flujo que lo reescribe, tambien
  al espejo: **F-14a** (UTF-16 crudo invalido: la lectura lanza `ArgumentException`) ⇒ E2 (ST-2c); **F-14b** (surrogate
  suelto escapado: la reescritura lanza `JsonException`) ⇒ E8 en el ensayo de P10; y su **regla F** (`Compose` rechaza
  un origen con una clave de `ExtensionData` igual, sin distinguir mayusculas, a un miembro declarado) ⇒ E8. Si I-54
  integra antes del Candidato: T-M20; re-medicion del censo T-GRD-02 (7 → 8: la llamada a `Compose` del espejo se
  clasifica en esa guarda; T-GRD-08, censo por nombre de comandos y de ayuda, se reapunta con motivo por T-M16);
  T-GRD-01 (solo el compositor construye sobres) y T-GRD-03 (forma del restamp) siguen verdes, porque el espejo no
  construye sobres (ID-5) ni modifica el restamp (ID-6); y, como `Compose` normaliza entonces un `CustomProperties`
  `Null` o `Undefined` a ausente para todos los productores (contrato de I-54, T-ENV-05), T-M10 y T-M20 comparan con esa
  normalizacion. V5 de I-54 (C-F3) precisa que en los consumidores existentes el punto de fallo de F-14b depende de su
  granularidad transaccional (`RACKEDITAR` puede quedar parcialmente actualizado entre vistas); en el espejo el ensayo
  de P10 serializa todos los payloads **antes** de MUTATE, asi que F-14b termina en E8 sin ninguna mutacion parcial.
...
3. **Ruta primaria: STATE-EVALUATED VISUAL-GEOMETRIC EVIDENCE.** Para cada pieza de **cualquier** vista admisible del
   ...
   prueba de simetria, para producir su `VisualFootprintResult` (punto 5, `[V9-D02]`).
   ...
   **Fuente de la definicion.** Si el bloque ya existe en el dibujo, la evidencia corresponde a **esa** definicion,
   ...
   igual que para el rack original. Como la equivalencia compara **fuentes** y no valores resueltos (punto 6), las capas
   que el dibujo ya tuviera con otras propiedades no la invalidan. El resultado depende de la **biblioteca efectiva**
   (L-28).
4. **Sin mutacion del DWG del usuario ni de la cache.** La evaluacion se hace en solo lectura o clonando y evaluando en una
   ...
   **Contexto de la base auxiliar (`[V8-D07]`).** La base auxiliar reproduce el contexto necesario de la **autoridad
   origen** de la definicion: como minimo `PSTYLEMODE`, `INSUNITS`, la escala de anotacion cuando pueda afectar a la
   evaluacion y cualquier contexto adicional que descubra CT-36. La autoridad origen es el **dibujo actual** si la
   definicion ya existe en el; si solo existe en la biblioteca, la autoridad inicial es la base de la biblioteca y la
   definicion real importada se vuelve a verificar en PREPARE (punto 3), ya en el contexto del dibujo. Si el contexto
   relevante no puede reproducirse o verificarse en la base auxiliar ⇒ `UNKNOWN` (X-17a). Ninguna fuente se lee despues de
   un clonado que haya cambiado ese contexto (por ejemplo, entre modos de estilos de trazado distintos). CT-36 incluye un
   dibujo con estilos de trazado con nombre (STB) y otro dependiente del color (CTB). El `MaterializationContext` (§8.7)
   se reproduce en la base auxiliar en lo que pueda afectar a la evaluacion.
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **Huella visual (`VisualFootprintResult`, `[V9-D02]`).** Es una autoridad **distinta** de `GeometrySymmetryResult`:
   `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult` demuestra **alcance visual y superposicion**.
   Ninguna sustituye a la otra (G-M23).

   ```text
   VisualFootprintResult {                 // nombres no contractuales; no es schema persistido
       Instance                            // instancia de plan (pieza o curva) o objeto de Model Space que describe
       Frame                               // marco de las regiones: BTR evaluado de la pieza, definicion del sistema o WCS
       Regions[]                           // SOBRE-APROXIMACION de todo lo visible, por clase material:
                                           //   trazo expandido con su grosor visible · relleno · region sensible a
                                           //   transparencia · mascara o fondo de texto · anotacion o cota ·
                                           //   marco o borde · contenido anidado aplanado
       StrokeSignatures                    // firmas visuales de los trazos (para «trazos coincidentes con firma distinta»)
       Status                              // CONSERVATIVE | UNKNOWN
       Diagnostics                         // clase, dilatacion aplicada, motivo de UNKNOWN
   }
   ```

   - **Nunca subestima.** La huella cubre todo lo que la instancia puede dibujar en su estado evaluado. Si no se puede
     garantizar que cubre la representacion (grosor, mascaras, marcos, contenido anidado, texto), o si no esta disponible o
     es infinita, `Status = UNKNOWN`.
   - **Quien la produce (P7b).** Para toda instancia que enumere P7a, tenga o no cambio de mano: las piezas de la familia
     `HeaderRun`, con su definicion evaluada en la base auxiliar por el mismo clonado y aplanado que la sonda (sin la
     prueba de simetria cuando la pieza no cambia de mano); las curvas Cantilever, desde su plan y su rol visual; las
     anotaciones y cotas regeneradas, desde el plan, con la envolvente de texto y de cota que caracterice CT-49. En Model
     Space (ST-19), la huella de un objeto existente sale del propio objeto (§4.1).
   - **`Extents3d`.** Solo es admisible como primera sobre-aproximacion **declarada** y dilatada segun el grosor visible,
     las mascaras, los marcos y la representacion conocida de la clase; una extension no disponible, infinita (`XLINE`,
     `RAY`) o dudosa ⇒ `UNKNOWN`.
   ...
   - **Una sola autoridad** de extraccion y dilatacion de huellas (G-M23). CT-36 y CT-49 caracterizan las dilataciones
     reales por clase; T-M71 las prueba con dobles en Core.
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V10.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V9 no introduce tolerancia; CT-36 registra las
       desviaciones reales).
   ...
   - **Escalas globales de tipo de linea (`[V8-D09]`).** `LTSCALE`, `PSLTSCALE` y `MSLTSCALE` son contexto global del
     ...
     evaluada, y la referencia de pieza que crea el materializador es la raiz comun `·`; forma parte del
     `MaterializationContext` (§8.7, `[V9-D05]`) y CT-36 y CT-50 lo caracterizan.
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V9 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - Para cada par de instancias `(a, b)`, **con o sin cambio de mano**, cuyo orden relativo **cambia** entre
     ...
     clases de 6.5 evaluadas sobre las huellas: relleno ↔ relleno, relleno ↔ trazo, trazos coincidentes con
     `VisualSignature` distinta, entidades con `TransparencySource` no `Explicit` opaca y lo que descubra CT-36—, el
     orden relativo tiene que conservarse; como cambia, la vista no es equivalente (X-22).
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V9 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V9 no introduce una tolerancia nueva: CT-36
...
10. **`TraceFingerprint`.** No prueba simetria. Solo sirve para **trazabilidad**, **deteccion de obsolescencia** y **enlace
    ...
    aplanada (con orientacion cuando importa), variantes, **fuentes visuales simbolicas tipadas** de cada primitiva con sus
    **rutas canonicas estables** (6.2, `[V8-D11]`), `SourceKind`, `PolylineGeneration`, **`LinetypeScale`**,
    `LinetypePatternContext`, **`VisibilityPath`**, `FillSignature`, relaciones de orden visual materiales, definiciones
    anidadas por identidad estable, BTR evaluados, **transformacion efectiva** y contexto de la base auxiliar (punto 4); en
    **serializacion canonica** (ordenada por contenido, nunca por el orden de
...
12. **Estados futuros por variables.** La verificacion de un estado concreto **no** basta si el mismo rack puede cambiar ese
    ...
    cambian esto (`[V9-D12]`, `[V9-D21]`); si I-49 integra su dominio y su `PlanReadSet`, se consumen sin analisis
    propio.
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Acceso a la cache de biblioteca (`[V7-D15]`, `[V8-D10]`).** La `Database` cacheada por `BlockLibraryImporter`
    ...
    ```text
    base auxiliar privada con el contexto del punto 4
    → ruta 1, definicion solo de biblioteca: BlockLibraryImporter.EnsureBlocks(auxDb, requiredNames)
        (:37-119; API publica, o la API publica equivalente vigente); clona desde la cache privada hacia la base
        auxiliar, sin modificar BlockLibraryImporter.cs (§20.3); su retorno es solo diagnostico (IF-8)
      ruta 2, definicion ya presente en el dibujo: clonado de solo lectura del dibujo hacia auxDb por la costura de
        AutoCAD autorizada y caracterizada por CT-36, sin pasar por la cache
    ...
    hacia la base auxiliar (`[V9-D07]`). G-M20 lo hace ejecutable. CT-36 y T-M63 pueden observar el contenido de la
    cache (HANDSEED, inventario de BTR y huella) **solo desde un harness fuera de `src/`**, si hace falta para demostrar
    que no cambio.
    ...
    **Metodo de inventario cerrado (`[V9-D08]`).** CT-36 (contenido de las piezas evaluadas) y CT-49 (referencia fuente y
    ...
    Nada queda sin clasificar. La reflexion del harness no contradice G-M20, que rige los archivos productivos del espejo.
    ...
    **Identidad de la evidencia (`[V7-D17]`).** CT-36 y CT-47 registran el **SHA-256** del contenido binario de la biblioteca;
    ...
    CT-36 cubre ademas (`[V9-D14]`): politica de clases y variantes con tipo exacto (grosor, normales, elevacion,
    `MInsertBlock`, definiciones anidadas anotativas, XCLIP y filtros espaciales, atributos y definiciones XREF, overlay
    o dependientes anidados, tipos de linea complejos, `HatchLoopTypes`, y completitud de las propiedades y el estado
    por clase con el metodo de inventario cerrado); `SourceKind`, `Plinegen` y `Closed`; fuentes visuales simbolicas
    tipadas por propiedad (capa `0`, `ByLayer`, `ByBlock`, anidados, estados de la API, `Color` con `ColorBook` y valor
    almacenado, `LineWeight` y `Transparency` tipados, y `PlotStyle` segun el modo de la autoridad origen); contexto de
    la base auxiliar con un dibujo STB y otro CTB, `INSUNITS` y escala de anotacion; `MaterializationContext` (con
    CT-50); `VisibilityPath` con estados de capas y ancestros variados; `LinetypeScale`, escalas y `LinetypeScale` de
    ancestros, fase y segmentacion de tipos de linea, y las escalas globales `LTSCALE`, `PSLTSCALE`, `MSLTSCALE` y
    `CELTSCALE`; multiplicidad; orden visual dentro de la pieza y, con CT-48, entre piezas; huellas visuales
    conservadoras (`VisualFootprintResult`) de toda instancia enumerada y sus dilataciones; poligono SOLID/TRACE; bucles
    y estilo de islas de HATCH; degradados y patrones; evaluacion de estados nuevos; anidados con ruta estable;
    `applied`; punto de observacion; BTR evaluado, `Origin` y transformacion efectiva; valores efectivos y paridad con
    el materializador (`[V6-D14]`); clonado por `EnsureBlocks` y desde el dibujo con verificacion de todos los BTR
    requeridos; canonicalizacion (falsos positivos y negativos); definicion local frente a biblioteca; obsolescencia y
    rendimiento preliminar. Si G3 demuestra que la ruta no es viable, que las clases o variantes reales no estan
    soportadas, que el contexto no se puede reproducir, que las huellas no pueden garantizarse conservadoras, que la
    biblioteca real reduce materialmente la envolvente mas alla de O-1 o que la canonicalizacion es materialmente
    ambigua ⇒ **STOP → Proposal V10**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad
    de planes o de materializacion (previsiblemente G6/G7); Application solo consume `GeometrySymmetryResult` y
...
16. **Sin `GeometrySymmetryResult` verificado** (sonda no disponible, sin paridad, sin punto de observacion soportado o
    sin contexto reproducible de la base auxiliar, BTR requeridos ausentes tras el intento de clonado, productor
    asimetrico del `MaterializationContext`, ruta anidada o correspondencia ambigua, eje oblicuo o simetria no
    verificada ⇒ X-17a; clase o variante fuera de la politica cerrada o ciclo ⇒ X-17b): la vista no es equivalente y el
    rack falla cerrado (E6). Con fuentes visuales simbolicas tipadas, `SourceKind`, `PolylineGeneration`,
    `LinetypePatternContext` o `VisibilityPath` distintas, mecanismo de visibilidad no modelado, orden visual no
    demostrado dentro de la pieza, orden de plan no conservado entre piezas o huella visual `UNKNOWN` en un par
    invertido ⇒ X-22. Con transformacion efectiva alterada ⇒ X-23. Con estado dependiente de un vinculo sin evidencia
    universal ⇒ X-17c (S-26, S-27a..S-27c).
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V9 abre **Proposal V10**.
...
| CT-14 | Censo de `[CommandMethod(` = `B` y unicidad de nombres en el SHA de reconciliacion (linea base; hoy 33 en `main @ 104ef3a` y 35 en la rama de I-54 G7B) y, si I-54 integro, censos por nombre de T-GRD-08 (comandos y ayuda) (`[V9-D06]`, `[V9-D17]`) | Core |
...
| **CT-46** | **Guardas cruzadas vigentes**: relee en el SHA de la reconciliacion las guardas de texto de `ProjectVariablesConformanceTests.cs` (main y, si integra, I-49), C-08/C-10, las del nucleo de expresiones (extendidas por I-49 G6 `5f969cc`, con `LengthUnitsAuthorityGuardTests` sobre todo `src/`) y, si I-54 integra, T-GRD-01..T-GRD-08 (incluidas las del borde fisico de G6, T-GRD-04..T-GRD-07, y los censos por nombre de comandos y ayuda de G7B, T-GRD-08); fija la lista exacta que G-M18 aplica a los archivos de I-52 y la autoridad vigente de I-49 (ADR-0040, aceptado y que reemplaza a ADR-0038, `[V8-D18]`) | Core |
| **CT-47** | **Corpus indicativo de la biblioteca actual**: recalcula el **SHA-256** del archivo antes de usar la evidencia (identidad; ruta, tamaño y fecha son diagnostico) y, si cambia, no reutiliza la evidencia previa; recalcula en `acad.exe`, con evaluacion de estados, la tabla de §1.4 por pieza y vista con las reglas de V9 (politica cerrada de variantes con inventario cerrado, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, `VisibilityPath`, `LinetypeScale` y contexto de patron, orientacion, multiplicidad, orden visual dentro de la pieza y entre piezas sobre huellas visuales y pares invertidos, degradados, falsos positivos y negativos de la canonicalizacion) y la compara con la evidencia indicativa; una reduccion material de la envolvente ⇒ Proposal V10 | evidencia registrada |
| **CT-48** | **Orden visual de plan por kind y vista** (`[V8-D12]`, `[V9-D03]`): orden de emision de cada builder de vista admitida y orden de materializacion de `LateralHeaderDrawer` (colocaciones de grupos y despues piezas sueltas, en `CreateSystemBlock` y `RedefineSystemBlock`) y de `CantileverViewMaterializer`; **enumeracion de los pares de instancias cuyo orden relativo cambia** entre `F ∘ Π(D)` y `Π(μ_k D)`, incluidas las piezas **sin** cambio de mano; superposicion de sus huellas (`VisualFootprintResult`, con la geometria evaluada de CT-36 o con dobles) y su clasificacion material; huellas ausentes o `UNKNOWN`; correspondencia de homologas ambigua; **reduccion de alcance** registrada por kind y vista | Core (con resultados geometricos de CT-36) |
| **CT-49** | **Presentacion de la referencia fuente y orden en Model Space en `acad.exe`** (`[V8-D13]`, `[V8-D14]`, `[V9-D01]`, `[V9-D04]`, `[V9-D08]`, `[V9-D09]`), **fuera de `src/`**: **inventario cerrado** de las propiedades y el estado de `Entity` y `BlockReference` de AutoCAD 2025 con el metodo de §11.3 punto 14, clasificados como `TRANSPORTED` (ST-13), `PLACEMENT`, `COMMON_CONTEXT`, `NON_VISUAL` o `UNSUPPORTED` (ST-18), sin nada sin clasificar; lectura y copia fiel de las propiedades de ST-13, con `PlotStyleName` en un dibujo STB y la derivacion del color en uno CTB; deteccion de XCLIP y otras entradas de `ACAD_FILTER`, referencias de atributo, entradas del diccionario de extension y XData de efecto visual; **orden efectivo** de Model Space con `SortentsTable`, `SORTENTS`, `DRAWORDERCTL` y cualquier contexto adicional encontrado, en pantalla y en trazado; **huellas** de copias y de objetos existentes (dilatacion de `Extents3d` por grosor, mascaras y marcos; envolventes de texto y cota; extensiones no finitas); clasificacion de la superposicion de una copia con **todo** objeto posterior a su fuente: otra fuente seleccionada posterior, objeto seleccionado e ignorado posterior, objeto no seleccionado posterior, objeto coincidente, `XLINE` o extension no finita, mascaras, rellenos, grosor, degradados, wipeouts, imagenes, OLE y referencias que los contengan; orden resultante de las copias creadas en el orden de sus fuentes; valores por defecto de creacion observables en la referencia y en las piezas nuevas (con CT-50) | evidencia registrada; Core consume la clasificacion |
| **CT-50** | **`MaterializationContext` en `acad.exe`** (`[V9-D05]`), **fuera de `src/`**: autoridad real de los valores por defecto de creacion de las referencias que crean `LateralHeaderDrawer` (colocaciones de grupos y piezas) y `CantileverViewMaterializer` (curvas), al menos `CLAYER`, `CECOLOR`, `CELTYPE`, `CELTSCALE`, `CELWEIGHT`, `CETRANSPARENCY`, `CPLOTSTYLE` y `PSTYLEMODE`, y cualquier otra que se descubra; cambiar cada uno y comprobar que `Π(D)` y `Π(μ_k D)` se evaluan y materializan bajo el mismo contexto; productor que los aplique de forma asimetrica ⇒ `UNKNOWN`; efecto sobre la presentacion interna frente a una fuente dibujada con otros valores (L-36) | evidencia registrada; Core consume la clasificacion |
...
| T-M16 | Censo `B` → **`B + 1`** (`GUARDA_EL_CENSO_DE_COMANDOS_NO_CAMBIA` reapuntada con motivo; `B` medido en el SHA de reconciliacion, `[V9-D06]`) y, si I-54 integro antes, los censos por nombre de T-GRD-08 (comandos y ayuda) reapuntados con motivo añadiendo `RACKMIRROR` sin alias, sin debilitar la clasificacion por nombre ni por pares (`[V9-D17]`) | G7 |
...
| **T-M63** | Acceso a la cache de biblioteca y clonado (`[V9-D07]`): la sonda obtiene las definiciones de biblioteca solo con `BlockLibraryImporter.EnsureBlocks(auxDb, nombres)` (o la API publica equivalente vigente) y las del dibujo por clonado de solo lectura, y verifica que existan **todos** los BTR requeridos en la base auxiliar tras el intento de clonado; el retorno de `EnsureBlocks` es diagnostico (un 0 sin faltantes no es fallo); BTR ausente ⇒ `UNKNOWN`; ni reflexion ni acceso al campo cacheado; HANDSEED, inventario de BTR y huella del contenido de la cache iguales antes y despues, observados solo desde un harness fuera de `src/`; la inicializacion perezosa no cuenta como mutacion (prueba del Plugin donde sea ejecutable; efecto real en CT-36) | G6/G7 |
...
| **T-M67** | Orden visual de plan (`[V8-D12]`, `[V9-D03]`): pares cuyo orden relativo no cambia no necesitan huella; en los pares invertidos, huellas sin superposicion material ⇒ sin relacion y con superposicion material ⇒ X-22; **pieza sin cambio de mano** en un par invertido (su huella tambien se evalua); huella ausente o `UNKNOWN` ⇒ X-22; correspondencia de homologas ambigua ⇒ X-22; un plan con el mismo multiconjunto y el orden invertido en un par material hace que la vista no conmute (V-VIEW-ALL) | G6 |
...
| **T-M69** | Orden visual en Model Space (`[V9-D01]`, `[V9-D04]`, `[V9-D19]`): copias creadas en el orden efectivo relativo de sus fuentes; objeto anterior a la fuente ⇒ sin clasificar; **todo** objeto posterior a la fuente con superposicion de huellas material o no clasificable ⇒ E10 en PREPARE, antes de `EnsureForPlan`, con los casos: otra fuente seleccionada posterior; objeto seleccionado e ignorado posterior; objeto no seleccionado posterior; objeto coincidente; `XLINE` o extension no finita; mascaras, rellenos y grosor; huella no disponible o no conservadora; contexto de orden no legible; copia sobre su propia fuente permitida; cambio del contexto de orden o de las huellas entre PREPARE y MUTATE ⇒ excepcion y rollback | G7 |
| **T-M70** | Contexto de la base auxiliar (`[V8-D07]`): `PSTYLEMODE`, `INSUNITS` y escala de anotacion reproducidos desde la autoridad origen (dibujo para una definicion existente; biblioteca para una solo de biblioteca, con re-verificacion en PREPARE); contexto no reproducible o no verificado ⇒ `UNKNOWN` (X-17a); ninguna fuente leida tras un clonado que cambie el contexto | G6/G7 |
| **T-M71** | `VisualFootprintResult` (`[V9-D02]`): autoridad separada de `GeometrySymmetryResult`; sobre-aproximacion que nunca subestima (trazo expandido con grosor, rellenos, regiones con transparencia, mascaras y fondos de texto, anotaciones y cotas, marcos, anidados aplanados); huella no disponible, infinita o no garantizada conservadora ⇒ `UNKNOWN`; ST-19 con huella `UNKNOWN` ⇒ E10; orden de plan con huella `UNKNOWN` ⇒ X-22; P7a enumera tambien las instancias sin cambio de mano; `Extents3d` solo dilatado y declarado; dobles en Core | G6 |
| **T-M72** | `MaterializationContext` (`[V9-D05]`, `[V9-D19]`): se captura en SNAPSHOT y se re-verifica en PREPARE y MUTATE (cambio ⇒ E10/E11); `Π(D)` y `Π(μ_k D)` bajo el mismo contexto; productor que aplica valores por defecto de forma asimetrica ⇒ `UNKNOWN` (X-17a); ST-13 sigue fijando la presentacion exterior; ningun drawer existente cambia | G6 (consumo puro) y G7 (captura) |
...
| **G-M23** | Autoridad unica de huellas visuales (`[V9-D02]`): un solo productor de `VisualFootprintResult` para piezas, curvas, anotaciones, cotas y objetos de Model Space; ningun archivo del espejo usa `GeometrySymmetryResult` como huella ni `Extents3d`/`GeometricExtents` sin la dilatacion declarada de §11.3 punto 5; ningun consumidor trata una huella `UNKNOWN` como no superpuesta |
...
| **M-37** | `MaterializationContext`: con `CLAYER`, `CECOLOR` o `CELTSCALE` distintos de los que habia al dibujar la fuente, la copia se regenera como Actualizar (piezas internas con el contexto vigente) y la presentacion exterior de la referencia sigue siendo la de la fuente (ST-13); cambiar el contexto durante el comando (por ejemplo con `'CLAYER` en la peticion de la linea) hace fallar sin mutacion semantica (PRE-17) |

---

## 14. Proceso y gates

### 14.1 Secuencia normativa (sustituye a la de V8)

```text
Coordinador + Arquitecto de acuerdo sobre V9 (misma version, exact-SHA)
→ rebase / reconciliacion sobre el main vigente         (solo docs; hoy main = 104ef3a con I-53 E1 e I-53S
                                                          E2; indice de ADR 0035..0040; §15.1)
...
de una caracterizacion o de una prueba con el contrato congelado ⇒ STOP → Proposal V10**; nunca un parche silencioso.

| Gate | Entrada | Archivos permitidos | RED esperado | PASS requerido | Parada | Evidencia |
|---|---|---|---|---|---|---|
| **G2** (este) | revision de V8 | `docs/**` | — | consenso de Coordinador y Arquitecto sobre la misma version (V9); despues, rebase/reconciliacion sobre el main vigente, freeze por SHA, re-check normativo y **O-1** del Owner sobre la envolvente de §1.3; contrato reescrito; hallazgos laterales en `ideas-futuras.md` | desacuerdo material ⇒ Proposal V10 | SHA acordado, SHA de freeze + CI |
| **G3** Characterization | freeze de G2 + O-1 aceptada (**sin** ADR aceptado) | `tests/RackCad.Tests/**` (caracterizaciones y fixtures nuevos); `tests/RackCad.UI.Tests/**` si una caracterizacion lo exige; `docs/automation/evidence/**`; la viabilidad de la sonda se demuestra **fuera de `src/`** (`tools/`, harness externo o evidencia del Owner) **en `acad.exe` / AutoCAD 2025**, en la ubicacion que fije el Coordinador al abrir G3; **ninguna sonda productiva en `src/`** | ninguno: CT-01..CT-50 pasan sobre produccion intacta | Core completo (UI si se toco); **viabilidad visual-geometrica de la sonda en `acad.exe` (CT-36: politica cerrada de clases y variantes con inventario cerrado, huellas visuales conservadoras, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, contexto de la base auxiliar STB y CTB, `VisibilityPath` con estados de capas, ancestros y anotativos, `LinetypeScale` y contexto de patron, fase, segmentacion y multiplicidad, orden visual dentro de la pieza, SOLID/TRACE/HATCH con `HatchLoopTypes`, `applied` y punto de observacion, valores efectivos, `Origin` y transformacion efectiva, ruta anidada estable, degradados, clonado por `EnsureBlocks` con la cache intacta, identidad SHA-256, `DBMOD`/`HANDSEED`/UNDO/`WorkingDatabase`) antes que `PlanPlacementEvidence` (CT-06)**; corpus indicativo con la huella de la biblioteca (CT-47); orden visual de plan por kind y vista con pares invertidos y huellas (CT-48); presentacion de la referencia fuente con inventario cerrado, huellas y orden en Model Space frente a todo objeto posterior (CT-49); `MaterializationContext` (CT-50); cierre transitivo sobre el contrato real, enums, retirados con RM-7 y guardas cruzadas (CT-31, CT-32, CT-42, CT-46); campos start/end de cabecera (CT-45); tabla de `c`; auditoria de anclajes por pieza (CT-17); semantica observable de I-51; decodificacion de secciones e historia de `View`; lineas base de store; topes, con el tope posterior de Push Back activo por defecto y `OffCells` latentes (CT-15, CT-43); estabilidad 9b incondicional con casos A/B, cotas, cambio de gobernante, BOM, firmas de estado y DEP-10a..c (CT-27, CT-28, CT-37, CT-38, CT-44); S-14 asociado; enumeracion `A_k`; tarimas del Selectivo y de Push Back (CT-20); precedencia de fuente (CT-39); read-set (CT-40); costuras de Actualizar e Insertar (CT-26); baseline de I-53 y clasificacion de I-53S (CT-41); fuentes dinamicas/anonimas; V-RT-STORE separado de C-2; **regla de cierre: ninguna fila `G3_PENDING` ni `EvidenceStatus` implicito** (§7.0) | contradiccion material con V9; sonda no viable, sin paridad, sin punto de observacion o sin contexto reproducible; clases o variantes reales no soportadas; huellas visuales no garantizables en clases enteras; productor asimetrico del `MaterializationContext`; biblioteca real que reduce materialmente la envolvente mas alla de O-1; canonicalizacion materialmente ambigua ⇒ **STOP → Proposal V10** | SHA + conteos + CI + registros de evidencia con la huella de la biblioteca. Tras PASS: se pide **O-4** |
...
### 15.1 main y paralelas observadas en el preflight de V9

`git fetch --all --prune` el 2026-09-13, antes de redactar V9 y de nuevo antes de publicarla (tips del re-fetch previo al
commit):

```text
origin/main                                        = 104ef3a1b1df249d6e0a56dc4ad3846912b24f12 (Merge I-53S E2; sin cambios desde V8;
                                                     ancla de las citas de V9: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = e0a779f0d12e79074272919531d71c7f4f3aaa7b (V8; base 46fcac2; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = 5f969cc87acb6cbd8572af3e2b517ba3468e5c41 (G6 productivo sobre el Consensus Freeze
                                                     8ed15f7: nucleo semantico de expresiones y autoridad de unidades;
                                                     sin PlanReadSet ni dominio; sin integrar; ADR-0040 aceptado)
I-54 architecture/propiedades-personalizadas       = 02987bd18ef0904a0332556928503a9afaeddcd1 (G7-CLOSE solo docs sobre G7B 18cf2dc productivo:
                                                     RACKPROPIEDADES + RPR, ventana, ayuda y censos 33 → 35 con
                                                     T-GRD-08; G8 NOT AUTHORIZED; sin integrar; ADR-0039 aceptado)
...
I-53D feature/cabeceras-multidestino-dinamico      = d28019747da4ca92ad4a6fbe6879a04f00960b94 (G7 productivo sobre el bootstrap
                                                     aa0f817: UI del Dinamico con DynamicHeaderBatch y DynamicRackRebuild;
                                                     DynamicEditorDesignAssembler sin el par ordinal; sin integrar)
I-50                                               = integrada en main; rama remota retirada
Censo de [CommandMethod(                          = main 104ef3a: 33 · I-52: 33 · I-49: 33 · I-53D: 33 · I-54 G7B: 35
...
**Cambios de paralelas desde la publicacion de V8.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V9 declaraba como
ultimos observados main `104ef3a`, I-52 `e0a779f`, I-49 `8ed15f7`, I-54 `18cf2dc` (G7B productivo) e I-53D `aa0f817`; el
preflight de V9 los confirmo sin ramas nuevas y, durante la redaccion, I-49 publico su G6 (`5f969cc`), I-54 su G7-CLOSE
(`02987bd`) e I-53D su G7 (`d280197`), clasificados arriba (`[V9-D11]`, `[V9-D12]`, `[V9-D21]`, `[V9-D22]`). La historia
de paralelas anterior esta en Proposal V8 §15.1, en Proposal V7 §15.1 y en el registro §42 y §48. La numeracion de ADR
esta en §16.
...
| **I-53 E1** (**integrada** en main `1b091be`; ADR-0037 **aceptado** en main) e **I-53S E2** (**integrada** en main `104ef3a`) | tipos `Header*` en `Systems.Shared` con C-08/C-10; consumidores Selectivo y Dinamico aditivos; I-53S G5 conecta `ApplyHeaderBatch` a `RackSelectiveWindow` en main | baseline: I-52 no crea tipos en `Systems.Shared` (G-M14, CT-41) ni modifica `SharedFoundationInspection`. **I-53S, clasificacion exacta (`[V8-D17]`)**: materialidad del contrato de I-52 = NON-MATERIAL; materialidad de coordinacion = MATERIAL; baseline de C2-4 = MATERIAL / CHANGED. `C2_4_I52_TheEditorStateAfterADistribution_EqualsTheReopenedDocument_InEveryView` es evidencia de I-53S y **no** cierra C2-4 de I-52. Como I-53S integro **antes** del Candidato: tras el rebase, CT-26, C2-4, T-M30 y las guardas relacionadas se ejecutan sobre el arbol combinado y condicionan el cierre de G6/G7; una integracion **posterior** al Candidato (por ejemplo I-53D, cuyo G7 `d280197` ya conecta el Dinamico en su rama, `[V9-D22]`) se reporta al Coordinador y se re-mide contra un main que contenga I-52; todo tipo persistido nuevo de cabecera o de modulo queda en RED en el cierre transitivo; serializar si C-1 llegara a tocar comandos de `RACKEDITAR` |
| **I-49** (V6 y Amendment A1 congelados; ADR-0040 **aceptado**, que reemplaza a ADR-0038; rama rebasada sobre `104ef3a`: G5 sintactico `977f873`, A1 `c7fabbe`, aceptacion de ADR-0040 `8f6d583` y Consensus Freeze `8ed15f7`; G6 `5f969cc` productivo, sin integrar) | `expression` en `PropertyValues`; `PlanReadSet` (D19); dominio del consumidor declarado por el descriptor (P24.5); su freeze exige que la prueba de supervivencia de `expression` (P24.8) se escriba contra `RackDuplicationPlan` y el restamp vigentes en main; guardas de texto evolucionadas en G5 y, en G6, `LengthUnitsAuthorityGuardTests` sobre todo `src/`; ADR-0040 como sucesor aceptado de ADR-0038 (solo cambia la guarda del parser de D8) | **I-52 coordina con I-49 antes de tocar `RackDuplicationPlan.cs`** (G4); T-M19. Si I-49 integra antes: su `PlanReadSet` es RS-3 (§9.2.1), su dominio es la cota `b` de V-DEP, su descriptor se clasifica en la guarda (§6.7) y sus guardas de texto rigen (G-M18, CT-46). `8ed15f7` es **NON-MATERIAL** (`[V8-D18]`) y `5f969cc` tambien lo es para el contrato, sin `PlanReadSet` ni dominio (`[V9-D21]`): con ADR-0040 aceptado, las referencias de I-52 a ADR-0038 se leen en ADR-0040 sin cambio semantico. V9 **no** presupone el motor |
| **I-54** (V5 congelada; ADR-0039 **aceptado**; G3..G6 cerrados y G7 en curso en su rama, rebasada sobre `104ef3a`: G5 `0572be4` en `RackCad.Application.CustomProperties`, G6 `deca1e0` en el Plugin, G6-CLOSE `d9f8c57`, G7A `5c16ae8`, **G7B `18cf2dc` productivo** y G7-CLOSE `02987bd`, solo docs; G8 NOT AUTHORIZED) | `CustomProperties` (`JsonElement?`) heredado por `Compose`, ya implementado en su rama por G4B (`0010144`, antes `7b0f6a9`) con la regla F y las guardas T-GRD-01..T-GRD-03; G5 consume el sobre y `WithCustomProperties` sin modificarlos ni añadir llamadas a `Compose`; G6 escribe las Custom Properties de alcance Proyecto en el NOD `RACKCAD_CUSTOM_PROPERTIES` y los valores por rack con `RackBlockData.Write`, con T-GRD-04..T-GRD-07; G7B añade `RACKPROPIEDADES`/`RPR`, su ventana, la entrada de ayuda y T-GRD-08 (censos de comandos y de ayuda por nombre); **D-21 vigente** (el espejo cumple: A y despues B); T-GRD-02 7 → 8; residuales F-14a/F-14b vigentes | materialidad arquitectonica = NON-MATERIAL; coordinacion y guardas cruzadas = MATERIAL; censo de comandos = MATERIAL; `RackCommandReference.cs` = MATERIAL TO INTEGRATION ORDER (`[V8-D19]`, `[V9-D11]`). **CA-4** portador RACK-LEVEL que se preserva con la copia; **CA-4b** Custom Properties de alcance PROYECTO que el espejo no lee para reflejar el rack y no copia, refleja, transforma ni escribe (§6.4, `[V9-D10]`); G-M22; al integrar, re-medir sus guardas y añadir T-GRD-04..T-GRD-07 a G-M18; si integra antes de I-52 G7, re-medir `B`, serializar `RackCommandReference.cs` y `SelectiveEditorOpenTests.cs` y reapuntar con motivo T-GRD-08 (T-M16); F-14a ⇒ E2 (ST-2c); F-14b y regla F ⇒ E8 en P10 (§9.4); ID-5 intacto; quien integre despues re-apunta T-GRD-02; T-GRD-01 y T-GRD-03 compatibles con ID-5 e ID-6; T-M20 |
| Documental | Filas de ROADMAP, final de `ideas-futuras.md`, `docs/adr/README.md` (0035 y 0037 en main; 0036 de I-52; 0038, reemplazado, y 0040, aceptado, de I-49; 0039 de I-54), censo de comandos `B → B + 1` (hoy `B = 33` en main; I-54 G7B lleva su rama a 35), censos por nombre de T-GRD-08 si I-54 integra, `src/RackCad.UI/RackCommandReference.cs` y `SelectiveEditorOpenTests.cs`, que tocan I-52 G7 e I-54 G7B | Quien integre despues conserva todas las filas en orden numerico, re-mide los censos (por numero y por nombre) y serializa esos archivos |
...
I-49, I-54 e I-53D (y de cualquier iniciativa nueva); leer sus Proposals y contratos en el SHA exacto; buscar
`docs/adr/0036-*` y citas de «ADR-0036» en **todos** los refs (§16); comprobar los archivos de §19; si `main` avanzo,
rebase segun WORKFLOW antes de escribir codigo. Ademas:
...
- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  ...
  Systems, reportar antes de continuar (`[V9-D21]`).
- **I-54:** si integra, re-medir CA-4, T-GRD-01..T-GRD-08 y G-M22, y añadir T-GRD-04..T-GRD-07 a G-M18 (`[V8-D19]`); G7B
  (`18cf2dc`) ya es productivo: si integra antes de I-52 G7, re-medir `B`, serializar `RackCommandReference.cs` y
  `SelectiveEditorOpenTests.cs` y reapuntar con motivo los censos por nombre de T-GRD-08 (T-M16, `[V9-D11]`,
  `[V9-D17]`); si un gate posterior a G7B toca el sobre, el compositor, el restamp, `ProjectVariables` o las autoridades
  de CA-4 o CA-4b, reportar antes de continuar.
- **I-53S/I-53D:** I-53S ya integro antes del Candidato de I-52 (`104ef3a`) y no es rama paralela: tras el rebase,
  ejecutar CT-26, C2-4, T-M30 y las guardas relacionadas sobre el arbol combinado, como condicion de cierre de G6/G7
  (`[V8-D17]`). I-53D abrio G7 durante V9 (`d280197`, sin integrar) y cambia la baseline de C2-4 del Dinamico: si
  integra antes del Candidato de I-52, lo mismo sobre el arbol combinado; si integra despues, reportar al Coordinador y
  volver a medir contra un main que contenga I-52; un gate posterior de I-53D que cambie la frontera estado del editor →
  sistema mas alla de §15.1 se reporta antes de continuar (`[V9-D22]`).
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V9.
- Congela: espejo semantico por copia; `μ_k` canonica por kind; admisibilidad por vista con seccion canonica y
  ...
  **equivalencia visual-geometrica** evaluada por estado sobre la definicion real, con politica cerrada de clases y
  ...
  fuente conservada, orden en Model Space demostrado frente a todo objeto posterior a cada fuente, huellas visuales
  conservadoras y contexto de materializacion comun; obligacion 9b y V-DEP; allowlist de portadores y metadata exterior;
  cierre transitivo de cobertura; C-2 con Insertar; verificacion dinamica por rack; taxonomia en tres dimensiones con
  regla de cierre de G3; alcance dependiente de la biblioteca efectiva y seleccion todo-o-nada.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V9 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   `52afa69` y `c5662e2` (antes `4907fc3` y `238cbf7`). ADR-0040 lo publico I-49 en `f8dddb6`
   (`2026-09-13T16:30:55-06:00`) como sucesor de ADR-0038 y el Owner lo **acepto** en `2eeeab1`
   (`2026-09-13T17:07:58-06:00`), reemplazando a ADR-0038; tras rebasar I-49 sobre `104ef3a`, sus copias en el ref son
   `cc75a18` y `8f6d583`. En cada commit citado la fila del indice y el encabezado del ADR coinciden (`[V5-D30]`). Todas
   son posteriores a 0036 y usan otro numero: **sin colision**.
...
no contradice materialmente V9, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse si
G3 abre Proposal V10. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V10 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V10 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V10 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V10 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V10 y nueva O-1 |
...
| **R-37** | ST-19 (`[V9-D01]`) puede hacer fallar copias cuyo destino se superpone con cualquier objeto posterior a su fuente (sombreados, wipeouts, imagenes, otras fuentes seleccionadas u objetos seleccionados e ignorados), y clasificar esas superposiciones tiene coste | se clasifican todos los objetos posteriores a cada fuente cuya huella interseca la de la copia; E10 antes de importar; M-35; medicion en G7 |
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V10 |
...
| **R-40** | Las huellas conservadoras (`[V9-D02]`) pueden sobre-estimar lo visible (grosor, mascaras, marcos) y producir superposiciones que el comando trata como materiales sin serlo, o quedar `UNKNOWN` en clases sin dilatacion caracterizada | fail-closed deliberado; CT-36 y CT-49 miden las dilataciones; relajacion solo con evidencia; reduccion material ⇒ Proposal V10 |
| **R-41** | El contexto de materializacion (`[V9-D05]`) hace que la copia pueda verse distinta de una fuente dibujada con otros valores por defecto (capa `0` heredada, color o escala de tipo de linea por defecto) | declarado en O-1 (L-36) con la misma semantica de Actualizar; CT-50 y M-37; la presentacion exterior se conserva (ST-13) |
| **R-42** | I-54 G7B e I-52 G7 comparten `RackCommandReference.cs`, `SelectiveEditorOpenTests.cs` y los censos por nombre de T-GRD-08 | censo `B + 1` re-medido; serializacion por orden de integracion; reapuntado con motivo sin debilitar (T-M16, `[V9-D17]`) |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8 y V9 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V8-8** | Paralelas, ADR, G3 y proceso (C:V9-11..C:V9-15) | I-54 G7B material para coordinacion; I-49 G6 e I-53D G7 registrados sin cambio de contrato (C2-4 del Dinamico re-medido si I-53D integra antes); ADR-0036 alineado; CT-50, T-M71, T-M72 y G-M23; desde V9, contradiccion material ⇒ Proposal V10 | §15; ADR-0036; §12; §14.1 |
...
- **AR8-05: la decision vinculante del Coordinador fija la semantica.** La revision de V8 dejaba elegir entre declarar
  el contexto de materializacion o fijar la presentacion interna. El Coordinador decide que I-52 es fiel al plan
  regenerado, como Actualizar (§8.7): el `MaterializationContext` es comun a las dos generaciones y a la
  materializacion, un productor asimetrico falla cerrado y la diferencia visible frente a una fuente dibujada con otros
  valores por defecto se declara en O-1 (L-36).
- **ST-19 completo (`[V9-D01]`, `[V9-D19]`).** Desaparece la restriccion a objetos no seleccionados: se clasifican toda
  fuente admitida, todo objeto seleccionado e ignorado y todo objeto no seleccionado que vayan despues de la fuente.
  Precision del ejecutor: si los contextos de pantalla y de trazado dan ordenes efectivos distintos, «posterior» es
  posterior en cualquiera de ellos. La fuente propia sigue siendo excepcion deliberada, como en el `MIRROR` nativo.
- **Huellas visuales (`[V9-D02]`).** Son una autoridad separada: `GeometrySymmetryResult` no se reutiliza como huella.
  Una huella conservadora puede sobre-estimar y hacer fallar casos visualmente inocuos (R-40); es el precio del
  fail-closed. P7 evalua ahora tambien las piezas sin cambio de mano, con el coste que mide R-24.
- **Notacion de §11.1 (`[V9-D18]`).** La orden de V9 escribe `A = F ∘ Π(D)` y `B = Π(μD)`; V9 conserva las letras de
  §11.1 (`A` = plan reflejado, `B` = original transformado) y usa las expresiones explicitas en §11.3 punto 6.6. No
  cambia ninguna regla.
- **Censo `B + 1` y T-GRD-08 (`[V9-D06]`, `[V9-D17]`).** El G7B de I-54 no solo sube el numero: fija los nombres de los
  comandos y los pares de la ayuda. Si I-54 integra antes, I-52 G7 los reapunta con motivo añadiendo `RACKMIRROR` sin
  alias, igual que I-54 reapunto `GUARDA_EL_CENSO_DE_COMANDOS_NO_CAMBIA`; reapuntar con proteccion igual no es
  debilitar.
- **CA-4b (`[V9-D10]`).** Corrige la descripcion de V8, no la regla: el NOD guarda Custom Properties de alcance
  Proyecto, un alcance separado del Rack segun ADR-0039.
- **Clonado de bloques (`[V9-D07]`).** El retorno de `EnsureBlocks` deja de ser autoridad; la tiene la verificacion de
  presencia de todos los BTR requeridos. G-M20 permite expresamente el clonado de solo lectura desde el dibujo.
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V10 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V9 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V9 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V10 y O-1 se reevalua.
...
- **I-53S integrada; I-53D G7 en su rama.** I-53S (`e528ef2`, integrada en `104ef3a`) no es rama paralela: CT-26, C2-4 y
  T-M30 se ejecutan sobre el arbol combinado tras el rebase. I-53D abrio G7 durante V9 (`d280197`, sin integrar) y
  cambia la baseline de C2-4 del Dinamico con la misma clasificacion que tuvo I-53S (`[V9-D22]`).
- **ADR-0040 aceptado en I-49.** Reemplaza a ADR-0038 y conserva lo que I-52 consume (D19 `PlanReadSet` y el dominio del
  consumidor); V9 lo usa como autoridad vigente de I-49.
- **I-54 G7B productivo (`18cf2dc`).** No cambia el contrato de I-52; su impacto es de coordinacion (censo, ayuda y
  T-GRD-08) y de orden de integracion (`RackCommandReference.cs`), `[V9-D11]`.
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  visuales no pueden garantizarse conservadoras en clases enteras, que un productor aplica el `MaterializationContext`
  de forma asimetrica o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V10** o reduccion de
  alcance.
...
- Una paralela o una integracion en main modifica materialmente `RackDuplicationPlan.cs`, `RackEmbedComposer.cs`,
  ...
  una infraestructura aditiva no conectada se registra sin bloquear; el G7B productivo de I-54 (`18cf2dc`) no modifica
  ninguno de esos archivos y se registra sin bloquear (`[V9-D11]`).
- Un gate de I-53D posterior a G7 (`d280197`) cambia la frontera estado del editor → sistema mas alla de lo registrado
  en §15.1 (I-53S G5 `e528ef2`, integrada en `104ef3a`; I-53D G7 `d280197`, sin integrar), o I-54 abre un gate posterior
  a G7B (`18cf2dc`; G6 `deca1e0`, registrados en §15.1) que toca el sobre, el compositor, el restamp,
  `ProjectVariables`, el contrato de CA-4 o CA-4b, o los archivos que I-52 G7 comparte (`RackCommandReference.cs`,
  `SelectiveEditorOpenTests.cs`, censos de T-GRD-08) mas alla de lo registrado: **reportar antes de continuar** (§15.3).
  Como I-53S integro antes del Candidato de I-52, G6/G7 no cierran sin CT-26, C2-4 y T-M30 verdes sobre el arbol
  combinado; lo mismo rige para el Dinamico si I-53D integra antes (`[V9-D22]`).
...
- Se intenta admitir un cambio de mano sin `GeometrySymmetryResult` visual-geometrico verificado, admitir una clase por
  ...
  usar `Extents3d` sin dilatacion declarada, fusionar familias distintas o primitivas sin continuidad demostrable al
  canonicalizar, derivar un centro de planes o de la caja envolvente sin verificar, inferir universalidad con muestras
  finitas o interpretar grafos de acciones de bloques dinamicos: **prohibido** (G-M15, G-M17, G-M21, G-M23).
- La sonda escribe en el dibujo del usuario o en la cache de biblioteca, accede a la cache por reflexion o por una ruta
  distinta de las dos autorizadas (§11.3 punto 14), evalua sin el contexto de la autoridad origen, deja
...
- Se intenta crear una copia sin la presentacion exterior de su referencia fuente, asignar un estilo de trazado
  ...
  **todo** objeto posterior a cada fuente (ST-19) o materializar bajo un `MaterializationContext` distinto del capturado
  (PRE-17): **prohibido**.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V10.
...
- Debilitar una guarda de I-51, de main (incluidas las de I-53), de I-49 o de I-54 (incluidos los censos por nombre de
  T-GRD-08) para facilitar el cambio: **prohibido**; reapuntar con motivo y proteccion igual o mayor no es debilitar
  (T-M16).
...
| Plugin | `RackMirrorCommands.cs` (nuevo) y SNAPSHOT propio; materializador en `Systems/Shared/`; `Systems/Shared/SystemBlockWriter.cs` (`CreateInTransaction`); **sonda visual-geometrica por estado** (solo lectura o `Database` auxiliar privada con el contexto de la autoridad origen; definiciones de biblioteca solo por `BlockLibraryImporter.EnsureBlocks` y verificacion de bloques), politica cerrada de variantes, canonicalizacion conservadora, fuentes visuales simbolicas tipadas, `VisibilityPath`, contexto de patron, ruta anidada estable, orden visual, transformacion efectiva y `TraceFingerprint`, en G6 o G7 (§11.3); presentacion de la referencia fuente (ST-13, ST-18), orden en Model Space (ST-19) y `MaterializationContext` (§8.7) en el SNAPSHOT, PREPARE y `CreateReference`; productor unico de huellas visuales (`VisualFootprintResult`, G-M23); costura unica de aplicacion de parametros compartida con el materializador si es viable (G-M19) |
...
un harness en `tools/` (nunca en `src/` durante G3); `SelectiveEditorOpenTests.cs` (censo `B + 1`) y, si I-54 integro
antes, los censos por nombre de T-GRD-08 reapuntados con motivo; guardas de I-51 reapuntadas en
...
autoridad del NOD de I-54 (`CustomPropertiesData.cs`, `CustomPropertiesExecutor.cs`) y sus guardas T-GRD-01..T-GRD-08
(T-GRD-08 solo se reapunta con motivo por T-M16).
...
G2 = V9 EN REVISION  (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf) y V8 (e0a779f)
     = historial; revision de V8: Architect: CHANGES REQUIRED — PROPOSAL V9
O-1 = PENDING        (envolvente de §1.3, L-01..L-36; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V9, rebase/reconciliacion sobre main, freeze, re-check y O-1)
