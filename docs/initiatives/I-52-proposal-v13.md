# I-52 — Proposal V13: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V13 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V13 se redacta SIN rebase, sobre 915a520)
...
> Proposal V1..V11  = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38 · b70b5bf · e0a779f · deb08cd ·
>                     636f7fd · 2275f21 (historial)
> Proposal V12      = 915a52056423291b5fcaff9955ed07a984c094ba   (historial; CI 34870304220 4/4)
> Revision Arq. V12 = «Architect: CHANGES REQUIRED — PROPOSAL V13»; sin BLOCKER ni HIGH; MEDIUM AR12-01
>                     (completitud a nivel de base de datos y carga parcial sin regla: dibujo abierto parcialmente y
>                     XREF cargado bajo demanda con indices); LOW AR12-02..AR12-07; cambios exactos V13-01..V13-08 de
>                     la revision (registro: docs/automation/decisions/I-52.md §74)
> Coordinador       = orden de V13 con V13-01..V13-14 vinculantes (registro §75)
> main y paralelas  = §15.1 (preflight de V13 y re-fetch previo al commit)
> Rebase            = despues del consenso tecnico sobre V13 y antes de la reconciliacion entre iniciativas (§14.1);
>                     V13 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V13 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V12 ni el Discovery y **no** es el contrato
> vinculante. El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V12, sus revisiones de Arquitecto y esta V13 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada, y la revision de V8 releyo el volcado de la de V6; V13 la registra
> como **indicativa**, ligada a su SHA-256, y no como contrato. Las revisiones de V10 y V11 no inspeccionaron la
> biblioteca: midieron codigo a SHA exacto (`LateralHeaderDrawer`, `LayerHelper`, `CantileverViewMaterializer`, la
> ventana de ayuda de comandos y sus censos) y leyeron los ADR y las Proposals de las paralelas. La revision de V12 no
> inspecciono la biblioteca ni ejecuto AutoCAD: contrasto V12 con el comportamiento documentado de AutoCAD para la
> apertura parcial de dibujos, la carga bajo demanda de referencias externas y otros mecanismos de visualizacion, que
> CT-49 debe confirmar en AutoCAD 2025, y con la reconciliacion X-1..X-8 registrada por I-55.

---

## 0. Reconciliacion V12 → V13

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`), V8
(`e0a779f`), V9 (`deb08cd`), V10 (`636f7fd`), V11 (`2275f21`), V12 (`915a520`) y el Discovery (`339b3ab`) **se conservan
sin cambios**. **Donde V12 y V13 difieran, manda V13.** Las marcas `[V12-D01]`..`[V12-D20]` siguen vigentes salvo donde
una marca `[V13-Dnn]` las modifica, reabre o sustituye (§0.4); las marcas `[V11-Dnn]`, `[V10-Dnn]`, `[V9-Dnn]`,
`[V8-Dnn]`, `[V7-Dnn]`, `[V6-Dnn]` y `[V5-Dnn]` siguen como las dejaron V12, V11, V10, V9, V8, V7 y V6 (§0.5 a §0.11).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V13** | V13 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V13. **Debe ser NONE** |

### 0.2 Matriz `[V13-D01]`..`[V13-D16]`

En la columna «Origen»: `AR12-nn` es el hallazgo de la revision de V12; `R:V13-nn`, su cambio exacto; `C:V13-nn`, la
precision vinculante del Coordinador en la orden de este gate. Las dos series V13-nn se numeran por separado. **Donde el
cambio propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del
Coordinador**: la revision agrupaba en un solo cambio (R:V13-01) la completitud a nivel de base de datos; el Coordinador
la divide en cuatro decisiones (precondicion de completitud anterior a ST-19, apertura parcial, carga bajo demanda de
XREF y principio de completitud de la enumeracion con un orden logico de etapas), amplia el censo de G-M24 por simbolos
de API con `AppendEntity`, `AddNewlyCreatedDBObject`, `SetDatabaseDefaults`, `TransformBy` y la delegacion en helpers o
factorias, extiende la marca provisional a los conceptos compartidos, registra X-1..X-8 de I-55 como entrada vinculante
de la reconciliacion y fija la redaccion de la transicion de I-56. **Precision del ejecutor:** la orden admite «E10 o
error operativo equivalente» para la apertura parcial; V13 la detecta antes de pedir la linea con un error propio (E12,
en P0) y la re-verifica en R6 (E10) y al inicio de MUTATE (E11), siempre antes de ST-19.

| Marca | Origen | V12 | Correccion V13 | Estado | Donde |
|---|---|---|---|---|---|
| `[V13-D01]` | AR12-01 · R:V13-01 · C:V13-01 | La `ContentAvailability` se clasificaba por objeto; ninguna regla exigia que la base de datos del dibujo y la enumeracion de Model Space estuvieran completas antes de ST-19 | **DATABASE-LEVEL COMPLETENESS**: precondicion nueva `DatabaseVisibilityCompleteness` (nombre conceptual; ST-20, PRE-19) **anterior a ST-19**. ST-19 solo puede ejecutarse si puede demostrarse que (1) la enumeracion relevante de Model Space es completa y (2) el contenido relevante de los objetos enumerados esta suficientemente disponible para una huella latente conservadora. «No esta cargado» o «no aparece en la base de datos» **nunca** se interpreta como «no ocupa». Si no puede demostrarse ⇒ fail-closed antes de clasificar ningun objeto | CLOSED BY V13 (contrato); API y estados reales: DEFERRED TO G3 WITH FAIL-CLOSED | §4.1 ST-19 y ST-20; §7.9 PRE-19; §9.1; §9.2; §9.4; §11.3 puntos 5 y 14; CT-49; T-M73; ADR-0036 decision 11 |
| `[V13-D02]` | AR12-01 · R:V13-01 · C:V13-02 | Sin regla para un dibujo abierto parcialmente: los objetos no cargados no estaban en la cola que enumeran R6 y MUTATE y, tras PARTIALOAD, reaparecian con su orden original debajo de la copia | **PARTIAL OPEN** (primer corte): si `Database.IsPartiallyOpened`, o el mecanismo equivalente que CT-49 caracterice, indica apertura parcial ⇒ **FAIL_CLOSED antes de ST-19** con mensaje propio: E12 en P0, antes de pedir la linea, re-verificado en R6 (E10) y al inicio de MUTATE (E11). RACKMIRROR **no** ejecuta PARTIALOAD, no fuerza cargas, no altera capas y no muta el dibujo para completarlo. CT-49 confirma la API exacta en AutoCAD 2025; mientras no este confirmada, la completitud no se demuestra y la operacion falla cerrado | CLOSED BY V13 (contrato); API exacta: DEFERRED TO G3 WITH FAIL-CLOSED | §2.3; §4.1 ST-20; §7.9 PRE-19; §9.1 P0, R6 y MUTATE; §9.4 E10..E12; CT-49; T-M73; M-35; L-34; ADR-0036 decision 11 |
| `[V13-D03]` | AR12-01 · R:V13-01 · C:V13-03 | «XREF cargado ⇒ `SUPPORTED` solo con huella latente conservadora», sin distinguir un XREF cargado bajo demanda cuyo contenido en capas inutilizadas o fuera del recorte puede no estar en memoria | **XREF DEMAND LOADING**: «XREF cargado» **no** basta; se clasifica ademas si su contenido potencialmente visible esta **completamente disponible** para la huella latente. CT-49 caracteriza `XLOADCTL`, `INDEXCTL`, el indice de capas, el indice espacial y toda API o estado relacionado que descubra (por ejemplo, la base de datos de la referencia externa y su propia indicacion de carga parcial). XREF cargado cuya carga bajo demanda puede dejar contenido relevante fuera de memoria o de la enumeracion ⇒ `UNKNOWN`, salvo que CT-49 demuestre contenido completo. Sin forzar cargas, sin abrir archivos externos y sin cambiar variables del dibujo. El estado usado entra en la `ModelSpaceOrderTailSnapshot` y en el `FootprintInputFingerprint` | CLOSED BY V13 (contrato); estados reales: DEFERRED TO G3 WITH FAIL-CLOSED | §2.3; §4.1 ST-19; §9.2; §11.3 punto 5; CT-49; T-M73; M-35; L-34 |
| `[V13-D04]` | AR12-01 · R:V13-01 · C:V13-04 | Las etapas de la clasificacion no tenian orden normativo: nada impedia clasificar huellas sobre una enumeracion incompleta | **ENUMERATION-COMPLETENESS PRINCIPLE**: orden logico obligatorio `completitud de la base de datos → completitud de la enumeracion → tipo exacto en ejecucion → componentes → ContentAvailability → VisibilityMechanism → ModelSpaceFootprintPolicy → VisualFootprint`. Si una etapa falla ⇒ `UNKNOWN` o fail-closed, y **nunca** se salta a la clasificacion de la huella con una enumeracion incompleta. La enumeracion es completa si la cola posterior a cada fuente contiene todo objeto de Model Space en el orden efectivo y si el contenido relevante de cada objeto enumerado (definicion referenciada, anidados, atributos y contenido de XREF) esta enumerado o se clasifica `UNKNOWN` | CLOSED BY V13 | §4.1 ST-19 y ST-20; §9.1 R2 y R6; §11.3 puntos 5 y 14; T-M73 |
| `[V13-D05]` | AR12-02 · R:V13-02 · C:V13-05 | Inventario minimo de mecanismos sin atributos invisibles, aislamiento de objetos, visualizacion de proxies ni datos externos de tabla | **ADDITIONAL LATENT MECHANISMS** en CT-49: **atributos** (`AttributeReference.Invisible`, `ATTDISP`); **aislamiento de objetos** (`HIDEOBJECTS`, `ISOLATEOBJECTS`, `OBJECTISOLATIONMODE`); **proxy** (`PROXYSHOW`, `PROXYGRAPHICS` y todo otro estado de visualizacion de proxies que se descubra); **tabla y datos externos** (`DataLink`, formulas de tabla y contenido que depende de una actualizacion). Cada caso `SUPPORTED(metodo latente conservador)` o `UNKNOWN`; no hace falta soportarlos todos | CLOSED BY V13 (contrato); DEFERRED TO G3 WITH FAIL-CLOSED | §2.3; §11.3 puntos 5 y 14; CT-49; T-M73; M-35 |
| `[V13-D06]` | AR12-03 · R:V13-03 · C:V13-06 | Sin frontera entre contenido que reaparece por un estado y contenido que cambia por una edicion futura | **STATE VS EDIT BOUNDARY** (normativa): **dentro** de la garantia de huella latente queda el contenido existente que aparece o desaparece por un estado **sin cambiar** el orden de dibujo: visibilidad; disponibilidad por carga, descarga o resolucion; carga bajo demanda; recortes; estado de visibilidad dinamica; evaluacion de campos y de vinculos de datos; visualizacion de proxies; aislamiento de objetos; y estado de anotacion y de entorno. **Fuera** quedan las ediciones futuras del contenido: edicion de geometria; BEDIT o redefinicion; edicion manual de texto; mover o estirar; editar el archivo de un XREF y recargarlo; reemplazar el contenido de un archivo referenciado; editar un parametro que cambia la geometria o el contenido real. RACKMIRROR **no** garantiza la invariancia del orden de dibujo frente a ediciones futuras arbitrarias. Se añade a O-1 (L-37) | CLOSED BY V13 | §1.3 L-37; §11.3 punto 5; ADR-0036 decision 11 |
| `[V13-D07]` | AR12-04 · R:V13-04 · C:V13-07 | Lado B de G-M24 descrito por semantica («construyen o crean», «añaden `DBObject` graficos»), que un censo por `new` o `AppendEntity` podia leer sin clonados, copias transformadas, explosiones ni construccion por reflexion | **G-M24 API SYMBOL CENSUS**: el lado B (`ProducerSiteCensus`) censa por **familias de simbolos de API**, no solo por `new` sintactico o `AppendEntity`. Como minimo considera `AppendEntity`, `AddNewlyCreatedDBObject`, `WblockCloneObjects`, `DeepCloneObjects`, `Clone`, `GetTransformedCopy`, `Explode`, `ExplodeToOwnerSpace`, `SetAttributeFromBlock`, `Activator.CreateInstance`, construccion por reflexion, `GetSystemVariable`, `SetDatabaseDefaults`, `TransformBy` donde sea relevante y la delegacion en helpers o factorias. Cada call-site relevante es `DECLARED` o `EXCLUDED` con motivo. T-M75 añade un productor artificial basado en **clonado** y comprueba RED | CLOSED BY V13 (contrato); implementacion: G5..G7 | §8.7; §12.2 T-M75; §12.3 G-M24 |
| `[V13-D08]` | AR12-05 · R:V13-05 · C:V13-08 | Marca provisional solo en §3.7, §3.8, §5.1, §8.1..§8.5 y §20.1; §3.3, §5.4, §6.1, CT-04, CT-05 y CT-16 sin marcar; X-1..X-8 de I-55 descritos como abiertos | **I-55 AGREED X-1..X-8 INPUT**: X-1..X-8, acordados en la reconciliacion que registra la revision de Arquitecto de I-55 (`d091eeb`), son **entrada vinculante** del futuro gate de reconciliacion (§15.4, con los requisitos del lado de I-52 pendientes de adoptar alli). **PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION** se amplia a todo texto que intente congelar propiedad compartida: §3.3, §3.7, §3.8, §5.1, §5.4, §6.1, §8.1, §8.2, §8.3, §8.4, §8.5, §20.1, CT-04, CT-05 y CT-16, y a los conceptos valor de colocacion, origen y tramo del eje de la vista, `DescribeView`, `ViewExposure`, `IsSameAuthority`, codec, `Resolve` + `Plan` y primitivo de materializacion. I-52 sola no congela namespace, ubicacion ni forma de API; la politica propia del espejo sigue siendo de I-52 | CLOSED BY V13 (proceso) | §3.3; §3.7; §3.8; §5.1; §5.4; §6.1; §8.1..§8.5; §12.1; §14.1; §15.4; §19; §20.1 |
| `[V13-D09]` | AR12-07 · R:V13-07 · C:V13-09 | I-55 en `1e12414` con Proposal V2 NOT CONSENSUS y X-1..X-8 abiertos; ADR-0042 «complementa y enmienda» ADR-0010 | **I-55 CURRENT FACTS**: I-55 en `d091eeb1e96d3cb7cd8989151b6cfdfc6ac0a7fb` (G2C, solo docs; CI success): Proposal V2 revisada; Arquitecto de I-55 = CHANGES REQUIRED — PROPOSAL V3; X-1..X-8 = AGREED, sin MATERIAL CONFLICT con I-52; Coordinador de I-55 sin AGREED al emitir G2C; Owner PENDING; ADR-0042 = PROPOSED; sin produccion. A-8: ADR-0042 «complementa y precisa ADR-0010» (redaccion que debe fijar su Proposal V3; el archivo en `d091eeb` aun dice «complementa y enmienda»), con una nota posterior fechada en ADR-0010 si ADR-0042 se acepta. No se afirma ninguna aceptacion del Owner | CLOSED BY V13 (registro) | §8.5; §15; §16; §17 R-45; ADR-0036 referencias |
| `[V13-D10]` | AR12-06 · R:V13-06 · C:V13-10 | «si I-56 integro cambios de proceso, manda la autoridad integrada», sin sus reglas de transicion, en §0.3, §14.1, §15.3, §19 y las consecuencias del ADR | **I-56 GRANDFATHERING WORDING**: I-56 en `12fb509` (G0.1): I-52 IS GRANDFATHERED y WORKFLOW V2 NOT EFFECTIVE. I-52 sigue el workflow bajo el que se reclamo y no adopta reglas de la V2 no integradas. Redaccion vinculante: «las futuras autoridades de proceso se leen junto con sus reglas de transicion; mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo»; **nunca** «si se integra Workflow V2, gobierna automaticamente I-52» | CLOSED BY V13 (registro) | §0.3; §14.1; §15.2; §15.3; §17 R-51; §18.2; §19; ADR-0036 consecuencias y referencias |
| `[V13-D11]` | AR12-01 · AR12-03 · C:V13-11 | L-34 y M-35 sin apertura parcial, completitud de la enumeracion ni carga bajo demanda; L-37 sin frontera entre estado y edicion | **O-1**: L-34 y M-35 añaden que un dibujo abierto parcialmente hace fallar el comando; que si la completitud de la enumeracion no puede demostrarse, falla; que un XREF cargado bajo demanda puede hacerlo fallar aunque se muestre como cargado; y que un objeto `UNKNOWN` puede hacerlo fallar aunque parezca lejos. L-37 añade la frontera entre estado y edicion futura. O-1 **no** se pide todavia | CLOSED BY V13 | §1.2; §1.3 L-34 y L-37; §13 M-35 |
| `[V13-D12]` | C:V13-12 | Casos de G3 de V12 | Casos obligatorios nuevos: **base de datos** (dibujo normal abierto completo; dibujo abierto parcialmente); **carga bajo demanda de XREF** (estados de `XLOADCTL` e `INDEXCTL` caracterizados; cargado con contenido incompleto ⇒ `UNKNOWN` o fallo; contenido completo demostrable ⇒ candidato admitido); **atributos** (`Invisible`, `ATTDISP`); **aislamiento de objetos** (oculto, aislado y restaurado); **proxy** (`PROXYSHOW`, `PROXYGRAPHICS`); **tabla** (`DataLink` y actualizacion de formulas); **G-M24** (productor artificial por clonado ⇒ RED); **I-55** (sin implementacion; solo reconciliacion documental posterior). G3 sigue NOT OPEN; contradiccion material ⇒ Proposal V14 | CLOSED BY V13 | §12; §13; §14 |
| `[V13-D13]` | C:V13-13 | ADR-0036 alineado con V12 | ADR-0036 sigue **PROPOSED** y actualiza las decisiones 7 y 11..13 con la completitud de la base de datos y de la enumeracion, la apertura parcial, la carga bajo demanda de XREF, los mecanismos latentes añadidos, la frontera entre estado y edicion, el censo de productores por simbolos de API, X-1..X-8 de I-55 como entrada de la reconciliacion y la redaccion de la transicion de I-56; nunca presenta resultados de G3 como hechos; «la caracterizacion **DEBE** demostrar» | CLOSED BY V13 | ADR-0036 |
| `[V13-D14]` | C:V13-14 · R:V13-08 | Contradiccion material de V12 ⇒ Proposal V13 | Nueva matriz `[V13-Dnn]` sobre AR12-01..AR12-07 y las decisiones vinculantes del Coordinador; `[V12-D01]`, `[V12-D02]`, `[V12-D03]`, `[V12-D14]`, `[V12-D15]` y `[V12-D16]` se reabren y se cierran, y `[V12-D06]`, `[V12-D09]`, `[V12-D11]`, `[V12-D13]`, `[V12-D17]` y `[V12-D19]` se precisan (§0.4); `OPEN MATERIAL = NONE` para la revision; toda contradiccion material de V13 ⇒ **Proposal V14**; proceso: Coordinador y Arquitecto de acuerdo sobre V13 → consenso tecnico → rebase sobre el main vigente → reconciliacion obligatoria I-52/I-55/I-49 con las ultimas entradas X-* acordadas → freeze por SHA exacto → re-check exact-SHA → O-1 del Owner → G3 (Proposal V14 si G3 contradice materialmente / Owner acepta ADR-0036 → G4). I-52 sigue bajo su workflow grandfathered salvo que una autoridad de proceso integrada cambie expresamente las reglas de transicion aplicables a las iniciativas grandfathered | CLOSED BY V13 | §1.2; §7.0; §14.1; §16; §19 |
| `[V13-D15]` | Precision al redactar | Preflight de V12 | Preflight de V13 (§15.1): I-52 `915a520` exacta (arbol limpio, sin stash ni operaciones a medias); main `dad4e77` sin cambios; I-49 `75f1862` en el preflight (G6-C2 productivo solo en `Expressions`, G6 CLOSED, ADR-0041 ACCEPTED, sin cambios en `PlanReadSet`) y, en el re-fetch previo al commit, `f6f0991`: Amendment A3 de V6 (`f6f0991`, solo docs: fallos con varias causas y conjunto `RootCauses`; cambia el dato estable `Upstream` de ADR-0041 D20, `RepairDecisionObservation`, y añade `RootCauses` como dato comparable adicional del `PlanReadSet`; REPLACEMENT ADR REQUIRED sin numero; freeze `3f17c21` historico para G7; Arquitecto de I-49 PENDING sobre A3 y Owner PENDING); contrato de I-52 NON-MATERIAL, porque I-52 no consume D20 y adopta el `PlanReadSet` de I-49 con su autoridad (RS-3, PV-6); coordinacion MATERIAL; I-55 `d091eeb` (G2C, solo docs); I-56 `12fb509` sin cambios (G0.1; I-52 grandfathered; WORKFLOW V2 NOT EFFECTIVE); sin ramas nuevas ni produccion nueva; ADR 0035..0042 sin cambios de estado y sin numeros posteriores; biblioteca con el mismo SHA-256; censo de `[CommandMethod(`: main 35, I-49 35, I-55 35, I-56 35, I-52 33. Ninguna paralela modifica una autoridad de I-52: sin STOP | CLOSED BY V13 (registro) | §15.1 |
| `[V13-D16]` | Precision al redactar | Terminologia de V12 | **Terminologia**: se añade `DatabaseVisibilityCompleteness` (precondicion conceptual: base de datos del dibujo abierta completa, enumeracion relevante de Model Space completa y contenido relevante suficientemente disponible), distinta de `ContentAvailability` (disponibilidad por objeto), de `VisibilityMechanism` y de la `ModelSpaceOrderTailSnapshot`; `ProducerSiteCensus` censa por familias de simbolos de API; ninguno sustituye a otro. Numeracion nueva: ST-20, PRE-19 y E12; sin CT, T-M ni G-M nuevas | CLOSED BY V13 | §4.1; §7.9; §9.2; §9.4; §11.3 punto 5; §12.3 G-M24 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V14 o reduccion de alcance |
...
| **Guarda durable de los productores de contexto** (G-M24 con censo global de sitios productores por familias de simbolos de API, `[V11-D08]`, `[V12-D09]`, `[V13-D07]`) | un productor que cambia lo que asigna, usa o crea sin actualizar CT-50, el read-set, el `FootprintInputFingerprint` y las pruebas, o un call-site productor o consumidor nuevo sin clasificar (clonados, copias transformadas, explosiones, construccion por reflexion y lecturas de variables incluidas) ⇒ RED | G3 (CT-50: censo inicial), G5..G7 y despues de integrar |
...
| **Completitud de la base de datos y de la enumeracion** (`DatabaseVisibilityCompleteness`, `[V13-D01]`, `[V13-D04]`) | base de datos, enumeracion de Model Space o contenido relevante no demostrablemente completos ⇒ fail-closed antes de ST-19 (E12 en P0, E10 en R6, E11 en MUTATE); nunca «no cargado» = «no ocupa»; nunca se clasifica una huella sobre una enumeracion incompleta | G3 (CT-49) y G6/G7 (T-M73) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V14 |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V14 | G3 (CT-37, CT-38) |
...
| **Guardas de I-49 G6** (`LengthUnitsAuthorityGuardTests` y nucleo de expresiones, con la correccion G6-C2 de `75f1862`; ADR-0041 vigente; el Amendment A3 de `f6f0991` anuncia un ADR de reemplazo sin numero; `[V9-D21]`, `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V13-D15]`) | I-52 no declara literales ni conversiones de unidades de longitud ni nombra el nucleo; al reconciliar, CT-46 y G-M18 | reconciliacion posterior a la integracion de I-49 |
| **Reconciliacion con I-55 (e I-49 donde aplique) antes del freeze** (View Placement & Projection, Proposal V2 revisada con Arquitecto = CHANGES REQUIRED — PROPOSAL V3, X-1..X-8 AGREED como entrada vinculante, ADR-0042 propuesto que complementa y precisa ADR-0010; autoridades compartidas provisionales; `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V13-D08]`, `[V13-D09]`, R-45) | sin reconciliacion registrada de las autoridades compartidas no hay freeze; la reconciliacion parte de las ultimas entradas X-* acordadas; una autoridad incompatible congelada por cualquiera de las dos ⇒ ningun freeze para ninguna hasta resolverlo; I-55 aplica la misma regla; I-52 no espera al consenso de I-55 | tras el consenso tecnico y el rebase, antes del freeze |
| **Autoridad de proceso frente a I-56** (Initiative Workflow V2 en G0.1, con I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; `[V12-D13]`, `[V13-D10]`) | reglas de I-56 no integradas no se adoptan; antes de cada transicion del proceso se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo | cada transicion del proceso |

### 0.4 Estado de las marcas de V12 bajo V13

| Marca V12 | Estado en V13 |
|---|---|
...
### 0.5 Estado de las marcas de V11 (fijado por V12; sin cambios en V13)
...
### 0.6 Estado de las marcas de V10 (fijado por V11; sin cambios en V12 ni V13)
...
| `[V10-D18]` | **reabierta y cerrada** por `[V11-D10]` (reconciliacion antes del freeze) y `[V11-D15]` (Proposal V14) |
...
### 0.7 Estado de las marcas de V9 (fijado por V10; sin cambios en V11, V12 ni V13)
...
| `[V9-D15]` | sustituida por `[V10-D18]` (Proposal V14) |
| `[V9-D16]` | vigente |
...
### 0.8 Estado de las marcas de V8 (fijado por V9; sin cambios en V10, V11, V12 ni V13)
...
| `[V8-D22]` | sustituida por `[V9-D15]` (Proposal V14) |
...
### 0.9 Estado de las marcas de V7 (fijado por V8; sin cambios en V9, V10, V11, V12 ni V13)
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V14) |
...
### 0.10 Estado de las marcas de V6 (fijado por V7; sin cambios en V8, V9, V10, V11, V12 ni V13)
...
### 0.11 Estado de las marcas de V5 (fijado por V6; sin cambios en V7, V8, V9, V10, V11, V12 ni V13)
...
### 0.12 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V13) |
|---|---|
...
### 0.13 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR12-01 (completitud a nivel de base de datos: apertura parcial, carga bajo demanda de XREF y completitud de la enumeracion) | `[V13-D01]`..`[V13-D04]`, `[V13-D11]`, `[V13-D12]` |
| AR12-02 (mecanismos latentes añadidos: atributos, aislamiento, proxies y datos externos de tabla) | `[V13-D05]` |
| AR12-03 (frontera entre estado y edicion futura) | `[V13-D06]`, `[V13-D11]` |
| AR12-04 (censo de G-M24 por simbolos de API y productor por clonado en T-M75) | `[V13-D07]` |
| AR12-05 (marca provisional ampliada; X-1..X-8 como entrada de la reconciliacion) | `[V13-D08]` |
| AR12-06 (redaccion de la transicion de I-56) | `[V13-D10]` |
| AR12-07 (hechos de I-55 y redaccion de ADR-0042) | `[V13-D09]`, `[V13-D15]` |
| Cambios exactos R:V13-01..R:V13-08 de la revision | V13-01 → D01..D04; V13-02 → D05; V13-03 → D06; V13-04 → D07; V13-05 → D08; V13-06 → D10; V13-07 → D09; V13-08 → D14 |
| Precisiones del Coordinador C:V13-01..C:V13-14 | V13-01 → D01; V13-02 → D02; V13-03 → D03; V13-04 → D04; V13-05 → D05; V13-06 → D06; V13-07 → D07; V13-08 → D08; V13-09 → D09; V13-10 → D10; V13-11 → D11; V13-12 → D12; V13-13 → D13; V13-14 → D14 |
| ADR-0036 (decisiones 7 y 11..13 y alineacion general) | `[V13-D13]` |
| Paralelas y main observados en el preflight y en el re-fetch de V13 | `[V13-D09]`, `[V13-D10]`, `[V13-D15]` |
| Precisiones del ejecutor | E12 en P0 (§0.2), `[V13-D15]`, `[V13-D16]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V13; **no** son consenso final.
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-37), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V14 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V13; si la contradice ⇒ Proposal V14; **precondicion de G4** |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7, V8, V9, V10, V11, V12 y V13) | **identidad de la evidencia** |
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas; las de V9, V10, V11, V12 y V13 (ST-19 completo,
huellas visuales de ocupacion de modelo en el contexto de evaluacion destino, frescura tras `EnsureForPlan`, orden de
plan sobre pares invertidos, read-set transitivo del contexto de materializacion, elegibilidad de la presentacion
resuelta, politica de huellas por tipo exacto y por mecanismo de los objetos del dibujo, cola completa de Model Space,
entorno de la copia, transformaciones uniformes reflejadas, completitud de la base de datos y de la enumeracion, carga
bajo demanda de XREF y mecanismos latentes añadidos) no se han medido sobre esta biblioteca y las miden CT-36, CT-48,
CT-49 y CT-50. Estado: **DEFERRED TO G3** con fail-closed (X-17a, X-17b, X-22, X-23).
...
| **Enumeracion de Model Space o contenido relevante no demostrablemente completos** | sin completitud demostrada no se clasifica ninguna huella (`[V13-D01]`, `[V13-D04]`) | E10 (ST-20 en R6) |
...
```text
Antes de C1, una vez por comando: ST-20 DatabaseVisibilityCompleteness del dibujo (P0): apertura parcial o
         completitud no demostrable ⇒ E12 (fin, sin linea); se re-verifica en R6 (E10) y al inicio de MUTATE (E11)
...
PREPARE (depende de la linea): ST-20 re-verificado y, solo si pasa, ST-19 orden visual en Model Space frente a todo
         objeto posterior a cada fuente, con huellas de ocupacion de modelo en el contexto destino: rechazo
         anticipado antes de EnsureForPlan y decision definitiva tras releer el dibujo real (§9.1, PRE-18, PRE-19)
```
...
se ejecuta si ST-20 no demuestra la completitud de la base de datos y de la enumeracion (`[V13-D01]`). Las
...
equivalencia de la fachada: **STOP → Proposal V14**. No se duplica la agrupacion en un segundo planificador. Las guardas
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V = 1 ⇒ W = max(70, 76 + 3)  = 79 < 80: la fila de N1 se apila desde la izquierda en el original y en la copia.
    V13: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V = 4 ⇒ W = 92 ≥ 88;  V = 1 ⇒ W = 83 < 88: la frontal del fondo 1 desborda.
    V13: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V13 ⇒ **STOP → Proposal V14**.
...
## 7. Matriz de representabilidad V13
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V13)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V14**. T-M47 lo hace ejecutable desde
G5.
...
| PRE-19 | `DatabaseVisibilityCompleteness` no demostrable (ST-20, `[V13-D01]`, `[V13-D02]`, `[V13-D04]`): dibujo abierto parcialmente (`Database.IsPartiallyOpened` o el mecanismo equivalente que caracterice CT-49), enumeracion de Model Space o contenido relevante no demostrablemente completos, o ese estado cambiado entre P0, R6 y el inicio de MUTATE; sin PARTIALOAD, cargas forzadas ni cambios de capas o de variables | FAIL_CLOSED antes de ST-19 (E12 en P0; E10 en R6; E11 en MUTATE) | P0; PREPARE (R6); inicio de MUTATE | §4.1 ST-20; §9.1; CT-49 |
...
Application, fuera del comando. **Su ubicacion no la fija V13**: puede quedar junto al contrato de cada kind o en un
...
Marcas de V12 y V13 en esta seccion: `[V12-D07]` (regla de entorno tambien en la copia), `[V12-D09]` (censo global de
G-M24) y `[V13-D07]` (censo por familias de simbolos de API).
...
Regla
  ...
  G-M24 protege a los productores con dos lados ([V11-D08], [V12-D09]):
    ...
    B) ProducerSiteCensus por familias de simbolos de API ([V13-D07]), no solo por new sintactico o AppendEntity,
       en los sitios del Plugin y del camino de materializacion: construccion de subclases relevantes de Entity ·
       AppendEntity · AddNewlyCreatedDBObject · WblockCloneObjects · DeepCloneObjects · Clone ·
       GetTransformedCopy · Explode · ExplodeToOwnerSpace · SetAttributeFromBlock · Activator.CreateInstance y
       construccion por reflexion · GetSystemVariable · SetDatabaseDefaults · TransformBy donde sea relevante ·
       lectura o resolucion de capas, estilos, valores por defecto o contexto de materializacion · delegacion en
       helpers o factorias; cada call-site relevante DECLARED o EXCLUDED con motivo; call-site nuevo sin
       clasificar ⇒ RED; no depende de los nombres actuales de archivos o clases ni de hashes
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           ...
           estado de apertura de la base de datos del dibujo y estado de carga de los XREF (ST-20, [V13-D01]..[V13-D03])

PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  P0  completitud de la base de datos              ST-20 DatabaseVisibilityCompleteness: apertura parcial
                                                   (Database.IsPartiallyOpened o el mecanismo de CT-49) o enumeracion
                                                   no demostrablemente completa ⇒ E12, antes de C1 y de la linea;
                                                   sin PARTIALOAD, cargas forzadas ni cambios ([V13-D01], [V13-D02])
...
PREPARE    (dentro del lock y fuera de la transaccion de escritura; cualquier fallo ⇒ E10; [V10-D03], [V10-D04],
  ...
  R2  huellas visuales en el DestinationEvaluationContext de las instancias de esos planes, anotaciones y cotas
      incluidas, con la elegibilidad de su presentacion resuelta ([V11-D01]) y la regla de entorno tambien en la
      copia ([V12-D07]), y huellas de orden de los objetos de Model Space posteriores a cada fuente segun la
      ModelSpaceFootprintPolicy de su tipo exacto y de su ContentAvailability y sus VisibilityMechanism, latentes
      frente a visibilidad, carga, carga bajo demanda de XREF, recortes, estados dinamicos, campos, atributos
      invisibles, aislamiento, proxies y datos externos de tabla, siempre sobre una enumeracion completa y en el
      orden logico de etapas ([V11-D02]..[V11-D04], [V12-D01]..[V12-D06], [V13-D03]..[V13-D05]; §4.1, §11.3
      punto 5)
  ...
  R6  re-verificar ST-20 (apertura parcial y completitud de la enumeracion; fallo ⇒ E10 antes de ST-19,
      [V13-D01]) · enumerar de NUEVO todos los objetos de Model Space posteriores a cada fuente y capturar la
      ModelSpaceOrderTailSnapshot (identidades ordenadas, clases, contexto efectivo de orden) · orden visual de plan
      (§11.3 punto 6.6) y ST-19 re-evaluados con las huellas refrescadas y con esa cola: decision DEFINITIVA
      (PRE-18, [V11-D05])
...
MUTATE     un DocumentLock · UNA transaccion de escritura:
           ...
           PRE-17, PRE-18) · ST-20 re-verificado (PRE-19, [V13-D01]) · enumeracion de NUEVO de la cola posterior a
           cada fuente frente a la ModelSpaceOrderTailSnapshot de R6: objeto añadido, eliminado, reordenado, movido
...
clasificable, ⇒ E10. ST-20 precede a ST-19 en P0, en R6 y al inicio de MUTATE: con la base de datos abierta parcialmente
o con la enumeracion no demostrablemente completa, ST-19 no se ejecuta (`[V13-D01]`, `[V13-D02]`).
...
`ProducerSiteCensus` es el lado B de G-M24, por familias de simbolos de API (§8.7). `DatabaseVisibilityCompleteness` es
la precondicion de completitud de la base de datos y de la enumeracion que precede a ST-19 (ST-20). Ninguno sustituye a
otro (`[V10-D19]`, `[V11-D19]`, `[V12-D20]`, `[V13-D16]`).
...
| RS-3 Dependencias transitivas | **solo si I-49 integra antes**: las observaciones de su `PlanReadSet` (resultado de cada simbolo realmente usado, incluidas las dependencias de expresiones) | I-49 V6 D19; ADR-0041 (aceptado @ `2e8090a`; reemplaza a ADR-0040, que reemplazaba a ADR-0038, con D19 identico; `[V11-D11]`); el Amendment A3 (`f6f0991`, pendiente de revision y de un ADR de reemplazo) añade `RootCauses` como dato comparable adicional, que I-52 adopta con la autoridad de I-49 sin calculo propio (`[V13-D15]`) |
...
| E12 | Completitud de la base de datos no demostrable (ST-20, PRE-19; `[V13-D01]`, `[V13-D02]`): dibujo abierto parcialmente (`Database.IsPartiallyOpened` o el mecanismo equivalente que caracterice CT-49) o enumeracion de Model Space no demostrablemente completa | P0 | fin, sin linea y cero mutacion, con mensaje propio («dibujo abierto parcialmente o contenido incompleto: RACKMIRROR no puede demostrar el orden de dibujo»), nunca un aviso; sin PARTIALOAD, cargas forzadas ni cambios de capas o de variables; si aparece despues de P0, E10 en R6 o E11 en MUTATE |
...
5. **Salida pura, paridad de parametros y punto de observacion.** La infraestructura AutoCAD del Plugin produce datos
   ...
   **distinta** de `GeometrySymmetryResult`: `GeometrySymmetryResult` demuestra **simetria**; `VisualFootprintResult`
   ...
   - **`ModelSpaceFootprintPolicy` (`[V11-D02]`, `[V12-D01]`, `[V12-D02]`, `[V13-D01]`..`[V13-D05]`).** Autoridad
     ...
     ModelSpaceFootprintPolicy (nombre ilustrativo; no es schema persistido)
       ...
       inventario minimo de clases (CT-49): Line · Arc · Circle · Ellipse · Spline · LWPOLYLINE · Solid · Trace ·
         ...
         puntos · proxy o entidad custom (no es obligatorio soportarlas todas)
       ...
       frontera ([V13-D06]): los estados que no cambian el orden estan dentro de la garantia; las ediciones futuras
         del contenido, fuera
   ...
   - **XREF (`[V12-D03]`).** Primer corte: XREF descargado ⇒ `UNKNOWN`; XREF no resuelto ⇒ `UNKNOWN`; overlay anidado o
     ...
     ausente.
   - **Completitud de la base de datos y de la enumeracion (`[V13-D01]`, `[V13-D02]`, `[V13-D04]`).** La clasificacion
     solo empieza si ST-20 demuestra que la base de datos del dibujo no esta abierta parcialmente, que la cola posterior
     a cada fuente contiene todo objeto de Model Space en el orden efectivo y que el contenido relevante de los objetos
     enumerados (definicion referenciada, anidados, atributos y contenido de XREF) esta enumerado o se clasifica
     `UNKNOWN`. Las etapas siguen el orden logico del bloque anterior; si una falla, el resultado es `UNKNOWN` o
     fail-closed y nunca se salta a la huella. Un objeto que no esta cargado o que no aparece en la base de datos
     **nunca** se trata como un objeto que no ocupa. RACKMIRROR no ejecuta PARTIALOAD, no fuerza cargas, no altera
     capas y no cambia variables del dibujo.
   ...
   - **Recortes (`[V12-D04]`).** Un recorte **no** reduce la huella de orden latente si quitarlo o cambiarlo puede hacer
     ...
     recortar o conservadora si puede calcularse; si no ⇒ `UNKNOWN`.
   ...
   - **Mecanismos añadidos (`[V13-D05]`).** Atributos invisibles (`AttributeReference.Invisible`) y `ATTDISP`; objetos
     ocultos o aislados (`HIDEOBJECTS`, `ISOLATEOBJECTS`, `OBJECTISOLATIONMODE`); visualizacion de proxies
     (`PROXYSHOW`, `PROXYGRAPHICS`); y vinculos de datos (`DataLink`), formulas de tabla y contenido que depende de una
     actualizacion. Cada caso es `SUPPORTED(metodo latente conservador)` o `UNKNOWN`; el estado actual de visualizacion
     o el valor ya actualizado no son autoridad suficiente.
   - **Frontera entre estado y edicion (`[V13-D06]`).** La garantia de huella latente cubre el contenido existente que
     aparece o desaparece por un estado sin cambiar el orden de dibujo: visibilidad; disponibilidad por carga, descarga
     o resolucion; carga bajo demanda; recortes; estado de visibilidad dinamica; evaluacion de campos y de vinculos de
     datos; visualizacion de proxies; aislamiento de objetos; y estado de anotacion y de entorno. Las ediciones futuras
     del contenido quedan **fuera**: editar geometria, BEDIT o redefinir un bloque, editar un texto a mano, mover o
     estirar, editar el archivo de un XREF y recargarlo, reemplazar el contenido de un archivo referenciado o editar un
     parametro que cambia la geometria o el contenido real. RACKMIRROR no garantiza la invariancia del orden de dibujo
...
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3, `[V9-D08]`).** CT-36 enumera, por clase soportada y con el metodo de inventario cerrado del
     ...
     reduce materialmente la envolvente ⇒ Proposal V14.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V13 no introduce tolerancia; CT-36 registra
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V13 no define una teoria global de DRAWORDER para objetos que no se
   ...
   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`, `[V9-D03]`).** 6.5 controla el orden **dentro** de
   ...
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V13 **no** crea una teoria global de
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V13 no introduce una tolerancia nueva: CT-36
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Metodo de inventario cerrado (`[V9-D08]`, `[V12-D01]`, `[V13-D01]`, `[V13-D04]`, `[V13-D05]`).** CT-36 (contenido
    ...
    por tipo exacto en ejecucion:
      ...
      en objetos de Model Space tambien: definicion referenciada y disponibilidad de su contenido (XREF cargado,
        ...
        formulas de tabla ([V12-D01], [V12-D02], [V13-D03], [V13-D05])
      antes de clasificar, por dibujo y por cola ([V13-D01], [V13-D04]): apertura parcial de la base de datos y
        completitud de la enumeracion de Model Space y del contenido relevante; sin completitud demostrada no se
        clasifica ninguna propiedad ni ningun mecanismo de los objetos de Model Space (ST-20)
    ...
    sin clasificar o UNSUPPORTED ⇒ UNKNOWN (contenido evaluado, X-17b), E4 (referencia fuente, ST-18) o huella
      UNKNOWN ⇒ E10 (objeto de Model Space posterior, ST-19); completitud no demostrable ⇒ E12 o E10 (ST-20)
    ```
    ...
    **STOP → Proposal V14**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V13 abre **Proposal V14**.
...
| **T-M75** | Censo de sitios productores de G-M24 por familias de simbolos de API (`[V12-D09]`, `[V13-D07]`): un call-site inventado que construye una entidad relevante, la añade con `AppendEntity` o `AddNewlyCreatedDBObject`, lee un estilo, una capa, un valor por defecto o una variable (`GetSystemVariable`), o delega la creacion en un helper nuevo, sin declararse ni excluirse con motivo, pone G-M24 en RED; **un productor artificial basado en clonado** (`DeepCloneObjects`, `WblockCloneObjects` o `Clone` hacia el camino de materializacion), sin clasificar, tambien pone G-M24 en RED; un productor declarado que cambia lo que asigna, usa o crea tambien; la guarda no depende de los nombres actuales de archivos o clases ni de hashes | G5..G7 |
...
→ reconciliacion OBLIGATORIA entre iniciativas          (I-52 ↔ I-55 ↔ I-49, con las ultimas entradas X-*
                                                          acordadas como entrada vinculante: hoy X-1..X-8 AGREED
                                                          en I-55 d091eeb, §15.4; registra la propiedad de las
                                                          autoridades compartidas, todas PROVISIONAL UNTIL
                                                          CROSS-INITIATIVE RECONCILIATION en V13: taxonomia de
                                                          ...
                                                          caracterizaciones unicas CT-04, CT-05 y CT-16; sin
...
Proceso de I-52 ([V12-D13], [V13-D10], [V13-D14]): I-52 esta grandfathered y sigue el workflow bajo el que se
...
### 15.1 main y paralelas observadas en el preflight de V13
...
origin/main                                        = dad4e77f4f267b9fa248ecb0c8bfd8a74bbab093 (merge de I-53D E3 sobre
                                                     ...
                                                     ancla de las citas de V13: 1b091be)
I-52 feature/rackmirror-espejo-semantico           = 915a52056423291b5fcaff9955ed07a984c094ba (V12; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = f6f0991fa5f9a1f97817ca403bd21568f72c0aaa (Amendment A3 de V6,
                                                     solo docs, sobre G6-C2 75f1862, que cerro G6; fallos con varias
                                                     causas y RootCauses; REPLACEMENT ADR REQUIRED para D20 de
                                                     ADR-0041, sin numero; freeze 3f17c21 historico para G7;
                                                     Arquitecto de I-49 PENDING sobre A3; ADR-0041 aceptado; sin
                                                     cambios en PlanReadSet de produccion; sin integrar)
I-55 feature/creacion-de-vistas                    = d091eeb1e96d3cb7cd8989151b6cfdfc6ac0a7fb (View Placement &
                                                     Projection, G2C: revision de Arquitecto de la Proposal V2 f84f303
                                                     = CHANGES REQUIRED — PROPOSAL V3; X-1..X-8 AGREED sin MATERIAL
                                                     CONFLICT; Coordinador de I-55 sin AGREED; Owner PENDING; ADR-0042
                                                     propuesto; CI 34870781445 success; solo docs; sin integrar)
I-56 docs/initiative-workflow-v2                   = 12fb5095100696bcb6d2fd7cef69e110fbcd8dd9 (Initiative Workflow V2,
                                                     G0.1; solo documentacion y proceso; I-49, I-52 e I-55
                                                     grandfathered; V2 NOT EFFECTIVE; sin cambios desde V12; sin
                                                     integrar)
...
**main desde la publicacion de V12.** Sin cambios: `dad4e77`. V13 no añade citas de codigo nuevas: su contenido nuevo es
de proceso, de contrato y de caracterizacion en AutoCAD 2025 (CT-49).
...
**Cambios de paralelas desde la publicacion de V12.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V13 declaraba
como ultimos observados main `dad4e77`, I-52 `915a520`, I-49 `75f1862`, I-55 `d091eeb` e I-56 `12fb509`; el preflight de
V13 los confirmo, con ADR-0041 aceptado, ADR-0042 propuesto, sin ramas nuevas ni produccion nueva, y el re-fetch previo
al commit encontro los mismos tips salvo I-49 en `f6f0991` (Amendment A3, solo docs) (`[V13-D15]`). La historia de
paralelas anterior esta en Proposal V12 §15.1, en Proposal V11 §15.1, en Proposal V10 §15.1, en Proposal V9 §15.1, en
Proposal V8 §15.1, en Proposal V7 §15.1 y en el registro §42, §48, §54, §60, §66 y §72. La numeracion de ADR esta en
§16.
...
| **I-55** (View Placement & Projection; rama `feature/creacion-de-vistas` @ `d091eeb`, sobre `dad4e77`; Proposal V2 `f84f303` revisada: Arquitecto de I-55 = CHANGES REQUIRED — PROPOSAL V3, X-1..X-8 AGREED sin MATERIAL CONFLICT, Open Material M-01, Coordinador de I-55 sin AGREED y Owner PENDING; ADR-0042 propuesto; solo docs) | ID17, ID18 e ID19 sobre una foundation de preparacion de vistas antes de materializar y una colocacion de grupo con una transformacion rigida comun; §17.2 de su Proposal V2 reconcilia con I-52 en X-1..X-8: nucleo neutral de seleccion (X-1); plan desde el sistema resuelto y codec de `View`/`Section` neutral con politica por consumidor (X-2); comparador authored por kind (X-3); primitivo «definicion + sobre + referencia» con las politicas de I-52 (ST-13, ST-19, PRE-17, PRE-18, read-set) fuera del primitivo (X-4); reporte antes de fijar archivos o autoridades que I-52 vigila, que la Proposal V2 ya dispara (X-5); re-establecimiento de C-2 por la segunda en integrar (X-6); valor de colocacion sobre `Transform2D` con rigidez de I-55 y reflexion de I-52 (X-7); un descriptor de origen y tramo del eje por vista (X-8); la revision G2C de I-55 los acuerda los ocho (§15.4). ADR-0042 complementa ADR-0010 (vista adicional; dos precondiciones nuevas de Insertar; Actualizar sin cambio) y, segun A-8, su Proposal V3 lo redacta como «complementa y precisa», con nota posterior fechada en ADR-0010 si se acepta | arquitectura hoy **NON-MATERIAL**; coordinacion **HIGH / ACTIVE**; solape de produccion **NONE**; I-52 **no** espera al consenso de I-55. Tras el consenso tecnico sobre V13 y el rebase, **reconciliacion obligatoria antes del freeze** de la propiedad de las autoridades compartidas, con X-1..X-8 AGREED como entrada vinculante (§15.4) y marcadas como provisionales en V13 (taxonomia de tipo de vista, codec de `View`/`Section`, «sistema resuelto → plan de vista» con origen y tramo del eje, comparador authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de proyeccion, nucleo de seleccion), sin autoridades duplicadas; siguen siendo de I-52 ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la equivalencia con `μ_k`; I-55 aplica la misma regla y el STOP simetrico es mutuo; si ADR-0042 se acepta antes del freeze de I-52, se releen las autoridades de C-2, Actualizar e Insertar; la segunda iniciativa en integrar re-establece C-2 (R-45, `[V11-D10]`, `[V11-D12]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`) |
| **I-56** (Initiative Workflow V2; rama `docs/initiative-workflow-v2` @ `12fb509`, sobre `dad4e77`; G0.1; solo documentacion y proceso; I-49, I-52 e I-55 grandfathered; WORKFLOW V2 NOT EFFECTIVE) | puede cambiar el proceso de iniciativas (WORKFLOW, AGENTS, AUTOMATION_PLAN, `TEMPLATE.md` y afines) cuando su Proposal lo fije e integre; hoy solo toca su contrato y su fila de ROADMAP | **NON-MATERIAL** hoy; el WORKFLOW vigente sigue siendo la autoridad hasta que I-56 integre y, aun integrada, su invariante de transicion deja a I-52 bajo el workflow de su reclamo y ninguna politica de la V2 rige sin acuerdo tecnico y aprobacion del Owner; antes de cada transicion del proceso de I-52 se relee la autoridad de proceso vigente; las futuras autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de reclamo; nunca se adoptan reglas no integradas (`[V12-D13]`, `[V13-D10]`) |
...
- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus
  ...
  reconciliar, CT-46 las relee y G-M18 las aplica a los archivos de I-52. El Amendment A3 (`f6f0991`, solo docs) pide un
  ADR de reemplazo de ADR-0041 sin numero y un Consensus Freeze nuevo: al reconciliar se leen V6 + A1 + A2 + A3 y ese
  ADR si ya existen, y el censo de §16 verifica su numero. Si un gate posterior de I-49 introduce `PlanReadSet` o
  dominio aplicado, o toca ProjectVariables, Persistence o Systems, reportar antes de continuar (`[V9-D21]`,
  `[V10-D14]`, `[V11-D11]`, `[V12-D12]`, `[V13-D15]`).
...
- **I-55 (`feature/creacion-de-vistas`, View Placement & Projection, Proposal V2 revisada con Arquitecto = CHANGES
  REQUIRED — PROPOSAL V3 y X-1..X-8 AGREED, solo docs; gate de coordinacion activo, `[V11-D10]`, `[V12-D11]`,
  `[V12-D17]`, `[V13-D08]`, `[V13-D09]`):** I-52 no espera a su consenso, pero no congela sin la reconciliacion
  obligatoria posterior al consenso tecnico sobre V13 y al rebase, con X-1..X-8 como entrada vinculante (§15.4), que
  registra la propiedad de las autoridades compartidas que V13 marca como provisionales —taxonomia de tipo de vista,
  codec semantico de `View`/`Section`, autoridad «sistema resuelto → plan de vista» con origen y tramo del eje de la
  vista, comparador authored, primitivo de materializacion del Plugin, valor de colocacion y transformaciones de
  proyeccion, y nucleo de seleccion (X-1..X-8)—, sin autoridades duplicadas. Siguen siendo de I-52, aunque consuman un
  primitivo compartido, ST-13, ST-18, ST-19, PRE-17, PRE-18, `VisualFootprintResult`, `GeometrySymmetryResult` y la
  equivalencia con `μ_k`. Si cualquiera de las dos intenta congelar una autoridad incompatible antes de reconciliar:
  STOP, ningun freeze para ninguna (R-45, §19); I-55 V2 aplica la misma regla a su Consensus Freeze. G4 avisa a I-55
  antes de tocar la fundacion compartida (nucleo de seleccion en `RackDuplicationPlan.cs`). Si ADR-0042 cambia de
  estado, se re-evalua antes del siguiente commit y, si se acepta antes del freeze de I-52, se releen las autoridades
  que usan C-2, Actualizar e Insertar. Cuando un gate de I-55 toque rutas de materializacion,
  `RackEmbedComposer.Compose`, `SystemBlockWriter.cs`, comandos o `RackCommandReference.cs` (hoy previsto en sus
  G5..G15, X-5), se re-evaluan §8.1, §8.7, `MaterializationContextReadSet`, PRE-17, PRE-18, T-GRD-02 y el censo `B`; la
  segunda iniciativa en integrar re-establece C-2 sobre el main combinado. Si I-55 publica una Proposal V3 o cambia
  algun X, se relee antes del siguiente commit y la reconciliacion parte de la ultima version acordada.
- **I-56 (`docs/initiative-workflow-v2`, Initiative Workflow V2, G0.1, solo documentacion y proceso; `[V12-D13]`,
  `[V13-D10]`):** NON-MATERIAL hoy; su contrato declara a I-49, I-52 e I-55 grandfathered y que ninguna politica
  de la V2 rige sin acuerdo tecnico y aprobacion del Owner (WORKFLOW V2 NOT EFFECTIVE). I-52 sigue el workflow bajo
  el que se reclamo. Antes de cada transicion del proceso de I-52 (consenso, rebase, reconciliacion, freeze,
  re-check, O-1, G3 y gates posteriores) se relee la autoridad de proceso vigente en `origin/main`; las futuras
  autoridades de proceso se leen junto con sus reglas de transicion y, mientras el invariante grandfathered
  aplique, I-52 conserva su workflow de reclamo; nunca se adoptan reglas de I-56 no integradas.
...
Fuente: revision de Arquitecto de I-55 sobre su Proposal V2, registrada en `feature/creacion-de-vistas` @ `d091eeb`
(G2C). Los ocho X estan **AGREED** del lado de I-55, sin MATERIAL CONFLICT con I-52; el Coordinador de I-55 no habia
dado AGREED al emitir G2C y su Owner sigue PENDING. V13 los registra como **entrada vinculante** del gate de
...
**Contraste con V13.** Ningun X presenta contradiccion material con V13: V13 no fija la propiedad de ninguna autoridad
compartida, añade ST-20, PRE-19 y E12 como politica propia del espejo fuera del primitivo (X-4) y amplia la marca
provisional a las secciones que esos requisitos nombran. Si I-55 publica una Proposal V3 o cambia algun X antes de la
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V13.
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V13 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
3. **Otros numeros observados:** ADR-0035 es de I-50 (aceptado e integrado en main). ADR-0037 lo publico I-53 en
   ...
   **aceptado** en `5efaf7e` y ahora esta **en main** por el merge `1b091be`. ADR-0038 lo publico I-49 en `a160560`
   ...
   ADR-0010, sino como complemento que enmienda reglas acotadas, y la revision G2C de I-55 (`d091eeb`) fija para su
   Proposal V3 la redaccion «complementa y precisa ADR-0010», con nota posterior fechada en ADR-0010 si se acepta. El
   Amendment A3 de I-49 (`f6f0991`) anuncia un ADR de reemplazo de ADR-0041 sin numero asignado, que se censara frente a
   0036 cuando se publique. I-56 no tiene ADR. No hay numeros posteriores a 0042 en ningun ref (`[V11-D11]`,
   `[V11-D12]`, `[V12-D12]`, `[V12-D17]`, `[V13-D09]`).
...
no contradice materialmente V13, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse
si G3 abre Proposal V14. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V14 si el cambio es material; nunca parche silencioso |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V14 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V14 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V14 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`, `[V9-D03]`) puede dejar fuera kinds o vistas: bajo `μ_k` se invierten los pares de una misma pasada y cualquier par invertido con huellas superpuestas de forma material, o con huella `UNKNOWN`, falla (X-22) | CT-48 enumera los pares invertidos por kind y vista, mide su superposicion y registra la reduccion de alcance; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V14 y nueva O-1 |
...
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V14 |
...
| **R-52** | ST-20 (`[V13-D01]`..`[V13-D04]`) puede hacer fallar RACKMIRROR en dibujos abiertos parcialmente o con XREF cargados bajo demanda con indices, frecuentes en proyectos grandes, y mientras CT-49 no confirme la API exacta ninguna completitud se demuestra | E12 atribuible antes de pedir la linea y E10 que nombra el XREF; CT-49 caracteriza la API y los estados de `XLOADCTL` e `INDEXCTL` antes de cualquier implementacion; declarado en O-1 (L-34); reduccion material ⇒ Proposal V14 y O-1 se reevalua |
| **R-53** | El censo de G-M24 por familias de simbolos de API (`[V13-D07]`) puede clasificar muchos call-sites ajenos al camino de materializacion (por ejemplo, `TransformBy` o `GetSystemVariable` en otros comandos) | «donde sea relevante» y `EXCLUDED` con motivo; el censo inicial lo fija CT-50 y la forma concreta se decide en G5..G7 sin tocar los drawers |
| **R-54** | La frontera entre estado y edicion (`[V13-D06]`) deja fuera las ediciones futuras (un XREF editado y recargado, una redefinicion o un texto editado pueden quedar por encima o por debajo de la copia), mientras que los vinculos de datos quedan dentro y una tabla con `DataLink` sera en la practica `UNKNOWN` | limitacion declarada en O-1 (L-37); E10 atribuible para los datos externos sin cota; una garantia frente a ediciones seria un cambio de alcance para una Proposal posterior |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7, V8, V9, V10, V11, V12 y V13 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V12-8** | G3, ADR y proceso (C:V13-12..C:V13-14) | G3 ampliado; ADR-0036 alineado; entradas X-* acordadas en la reconciliacion; desde V13, contradiccion material ⇒ Proposal V14 | §12; §13; §14.1; ADR-0036 |
...
- **AR12-01: el Coordinador divide la completitud a nivel de base de datos.** La revision de V12 pedia un solo cambio
  (R:V13-01); el Coordinador fija cuatro decisiones: precondicion de completitud anterior a ST-19, apertura parcial,
  carga bajo demanda de XREF y orden logico de etapas. Precision del ejecutor: la apertura parcial se detecta en P0 con
  E12 y se re-verifica en R6 (E10) y al inicio de MUTATE (E11). Mientras CT-49 no confirme la API exacta, la completitud
  no se demuestra y todo comando fallaria cerrado; G3 lo resuelve antes de cualquier implementacion (R-52).
- **Entrada X-1..X-8 (`[V13-D08]`).** Es vinculante para la reconciliacion: una desviacion se vuelve a acordar con I-55,
  nunca de forma unilateral. V13 no adopta todavia los requisitos del lado de I-52 (§15.4); los registra para la
  reconciliacion posterior al rebase, como pide I-55, y amplia la marca provisional a sus secciones.
- **Vinculos de datos dentro de la garantia (`[V13-D06]`).** La evaluacion de campos y de vinculos de datos esta dentro
  de la garantia aunque su fuente sea externa, mientras que editar y recargar el archivo de un XREF queda fuera; por eso
  una tabla con `DataLink` solo es `SUPPORTED` con cota para todos sus resultados y en la practica sera `UNKNOWN`
  (R-54).
- **Censo por simbolos (`[V13-D07]`).** «Donde sea relevante» evita clasificar `TransformBy` o `GetSystemVariable` fuera
  del camino de materializacion sin motivo; toda exclusion lleva su motivo (R-53).
...
- **G-M24 (`[V11-D08]`, `[V12-D09]`, `[V13-D07]`).** Tiene dos lados: productores declarados y censo de sitios
  productores por familias de simbolos de API; no depende de nombres de archivos ni de hashes. Su forma concreta
  (analisis estructural, lista declarada y exclusiones con motivo) se fija en G5..G7 sin tocar los drawers (§20.3), y
  CT-50 fija el censo inicial.
...
- **I-56 (`[V12-D13]`, `[V13-D10]`).** Vigilancia de proceso, sin adoptar reglas no integradas; su contrato G0.1 declara
  a I-52 grandfathered y WORKFLOW V2 NOT EFFECTIVE; las futuras autoridades de proceso se leen con sus reglas de
  transicion y, mientras el invariante aplique, I-52 conserva su workflow de reclamo.
...
- **`[V9-D03]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero
  ...
  G3 abre Proposal V14 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V13 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V13 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V14 y O-1 se reevalua.
...
- **ADR-0041 aceptado en I-49 (`[V11-D11]`, `[V12-D12]`, `[V13-D15]`).** Reemplaza a ADR-0040, cambia solo D5..D8 y
  conserva lo que I-52 consume (D19 `PlanReadSet` y el dominio del consumidor); la correccion G6-C2 (`75f1862`) solo
  toca `Expressions`. V13 lo usa como autoridad vigente de I-49. Precision del ejecutor: V11 y V12 listaban tambien D20
  entre lo consumido, pero D20 es la `RepairDecisionObservation` de `RepairBrokenRack`, que I-52 no usa; por eso el
  Amendment A3 (`f6f0991`), que cambia su dato `Upstream` y exige un ADR de reemplazo, no toca el contrato de I-52.
- **I-55 (`[V11-D10]`, `[V12-D11]`, `[V12-D17]`, `[V13-D08]`, `[V13-D09]`, R-45).** I-52 no espera al consenso de I-55,
  ...
  presente: la revision G2C de I-55 (`d091eeb`) acuerda X-1..X-8 sin MATERIAL CONFLICT con I-52, I-55 adopta la misma
  regla de reconciliacion y ADR-0042 complementa ADR-0010 sin cambiar Actualizar. V13 marca esas autoridades como
  provisionales, registra X-1..X-8 como entrada vinculante y no decide el reparto.
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  consumo no puede determinarse, o que la canonicalizacion es materialmente ambigua: **STOP → Proposal V14** o reduccion
  de alcance.
...
- **Reconciliacion entre iniciativas (`[V11-D10]`).** Sin la reconciliacion obligatoria I-52/I-55 (e I-49 donde aplique)
  ...
  marcadas PROVISIONAL UNTIL CROSS-INITIATIVE RECONCILIATION (§3.3, §3.7, §3.8, §5.1, §5.4, §6.1, §8.1..§8.5, §20.1,
  CT-04, CT-05 y CT-16) no se congelan por I-52 sola (`[V12-D11]`, `[V13-D08]`).
- **Proceso vigente (`[V12-D13]`, `[V13-D10]`).** I-52 esta grandfathered y sigue el workflow bajo el que se reclamo.
  Antes de cada transicion del proceso se relee la autoridad de proceso vigente; las futuras autoridades de proceso se
  leen junto con sus reglas de transicion y, mientras el invariante grandfathered aplique, I-52 conserva su workflow de
  reclamo; adoptar una regla de I-56 no integrada: **prohibido**.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V14.
...
productores declarados y censo global de sitios productores por familias de simbolos de API, `[V12-D09]`, `[V13-D07]`);
evidencia de viabilidad de la sonda en `acad.exe` y corpus indicativo con la huella de la biblioteca en
`docs/automation/evidence/` y, si hace falta, un harness en `tools/` (nunca en `src/` durante G3);
...
G2 = V13 EN REVISION (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38), V7 (b70b5bf), V8 (e0a779f),
     V9 (deb08cd), V10 (636f7fd), V11 (2275f21) y V12 (915a520) = historial
     revision de V12: Architect: CHANGES REQUIRED — PROPOSAL V13
O-1 = PENDING        (envolvente de §1.3, L-01..L-37; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V13, rebase sobre main, reconciliacion con I-55 e I-49 con las ultimas
                     entradas X-* acordadas, freeze, re-check y O-1; antes de cada transicion, relectura de la autoridad
