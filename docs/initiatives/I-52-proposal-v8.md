# I-52 — Proposal V8: RACKMIRROR, espejo semantico de uno o varios racks (ID16)

> # ⚠ PROPOSAL V8 — NOT CONSENSUS
>
...
> BASE de la rama   = 46fcac2b071929d2bd5b07aa28373941417f74a8   (V8 se redacta SIN rebase, sobre b70b5bf)
> CITAS             = archivo:linea contra origin/main @ 1b091bedafceb67ca57054a9eb3bf5259efff774 (I-53 E1 integrada).
>                     Los cuatro archivos productivos que I-53 toco estan re-anclados (§15.1). Durante V8 main avanzo a
>                     104ef3a (merge de I-53S E2: en src/tests solo RackSelectiveWindow y pruebas de UI); las citas
>                     siguen ancladas en 1b091be, las que I-53S desplazo se listan en §15.1 y se re-anclan en el rebase
>                     posterior al consenso. Las citas de paralelas llevan su SHA
...
> Proposal V1..V6   = 0fc7032 · 0445718 · 545c222 · 8e2ae4f · e998a2b · 91bdd38   (historial)
> Proposal V7       = b70b5bf5bc689a31f4f2aac9803d675d75665093   (historial; CI 34785294820 4/4)
> Revision Arq. V7  = «Architect: CHANGES REQUIRED — PROPOSAL V8»; sin BLOCKER ni HIGH; MEDIUM AR7-01 (latente);
>                     LOW AR7-02..AR7-11; cambios exactos V8-01..V8-12 de la revision (registro:
>                     docs/automation/decisions/I-52.md §44)
> Coordinador       = orden de V8 con V8-01..V8-15 vinculantes (registro §45)
> main y paralelas  = §15.1 (preflight y re-fetch de V8)
> Rebase            = despues del consenso tecnico sobre V8 (§14.1); V8 no rebasea
> Estado de gates   = G0 ACCEPTED · G1 ACCEPTED (Discovery) · G2 V8 EN REVISION · G3 NOT OPEN
> ```
>
> Este documento **no** autoriza produccion, **no** modifica V1..V7 ni el Discovery y **no** es el contrato vinculante.
> El contrato se reescribe en el freeze de G2 (§14.1).
>
> **Nota de transparencia.** V1..V7, sus revisiones de Arquitecto y esta V8 las redacto el mismo agente en roles
> distintos. La evidencia de biblioteca de §1.4 la obtuvieron las revisiones de V5, V6 y V7 con inspecciones de solo
> lectura sobre copias de la biblioteca configurada; V8 la registra como **indicativa**, ligada a su SHA-256, y no como
> contrato.

---

## 0. Reconciliacion V7 → V8

V1 (`0fc7032`), V2 (`0445718`), V3 (`545c222`), V4 (`8e2ae4f`), V5 (`e998a2b`), V6 (`91bdd38`), V7 (`b70b5bf`) y el
Discovery (`339b3ab`) **se conservan sin cambios**. **Donde V7 y V8 difieran, manda V8.** Las marcas `[V7-D01]`..`[V7-D27]`
siguen vigentes salvo donde una marca `[V8-Dnn]` las modifica o sustituye (§0.4); las marcas `[V6-Dnn]` y `[V5-Dnn]`
siguen como las dejaron V7 y V6 (§0.5, §0.6).

### 0.1 Estados de reconciliacion

| Estado | Significado |
|---|---|
| **CLOSED BY V8** | V8 fija el contrato; lo que quede es verificacion ordinaria de gate |
...
| **OPEN MATERIAL** | Impide presentar V8. **Debe ser NONE** |

### 0.2 Matriz `[V8-D01]`..`[V8-D26]`

En la columna «Origen»: `AR7-nn` es el hallazgo de la revision de V7; `R:V8-nn`, su cambio exacto; `C:V8-nn`, la precision
vinculante del Coordinador en la orden de este gate. Las dos series V8-nn se numeran por separado. **Donde el cambio
propuesto por la revision y la precision del Coordinador difieren, manda la precision vinculante del Coordinador**: para
AR7-06 (orden entre piezas) V8 exige demostrar el orden en lugar de declararlo limitacion, y para AR7-07 (presentacion de
la referencia fuente) V8 exige conservarla o fallar cerrado en lugar de copiar solo la capa (§18.2).

| Marca | Origen | V7 | Correccion V8 | Estado | Donde |
|---|---|---|---|---|---|
| `[V8-D01]` | AR7-01 · R:V8-01 · C:V8-01 | Allowlist por clase; solo la fila de polilinea fijaba atributos | **CLOSED VARIANT POLICY**: clase + variantes soportadas. Una clase soportada solo lo es con los atributos de representacion enumerados; subclase, atributo visual o geometrico adicional, modo de representacion o variante descubierta sin clasificar ⇒ `UNKNOWN` ⇒ `FAIL_CLOSED` (X-17b). Comprobacion por **tipo exacto**, nunca por herencia. Tabla del primer corte: `Line`/`Arc`/`Circle` con `Thickness = 0` y `Normal ≈ +Z`; `Ellipse`/`Spline` planas en XY con `Normal ≈ +Z`; LWPOLYLINE con anchos 0, `Thickness = 0`, `Normal ≈ +Z`, `Plinegen` y `Closed` registrados; `Solid`/`Trace` con `Thickness = 0` y `Normal ≈ +Z` | CLOSED BY V8 (contrato); completitud de atributos por clase: DEFERRED TO G3/G5 WITH FAIL-CLOSED (atributo no clasificado ⇒ `UNKNOWN`) | §11.3 punto 6.1; X-17b; T-M64; CT-36 |
| `[V8-D02]` | AR7-01 · R:V8-01 · C:V8-01 | Firma sin clase de origen; polilineas comparadas por segmentos | `VisualSignature` añade `SourceKind` y, para LWPOLYLINE, `PolylineGeneration` (`Closed` siempre; `Plinegen` cuando `LinetypeSource` no tiene continuidad demostrable). Una polilinea **no** se empareja ni se fusiona con `Line` o `Arc` sueltos solo porque su geometria sea equivalente | CLOSED BY V8 | §11.3 puntos 6.4 y 8; T-M55 |
| `[V8-D03]` | AR7-01 · R:V8-01 · C:V8-01 | «BlockReference anidado: soportada, recursiva» | Referencia anidada solo con tipo **exacto** `BlockReference`: `MInsertBlock` ⇒ `UNKNOWN`; definicion anotativa ⇒ `UNKNOWN`; XCLIP u otro filtro espacial ⇒ `UNKNOWN`; `Normal ≈ +Z`. Los objetos y definiciones anotativos se añaden a los mecanismos de visibilidad no modelados | CLOSED BY V8 | §11.3 puntos 6.1 y 6.3; T-M60; T-M64 |
| `[V8-D04]` | AR7-01 · R:V8-01 · C:V8-01 | SOLID/TRACE por poligono; HATCH por bucles y estilo de islas | HATCH SOLID: no degradado, patron `SOLID`, `Normal ≈ +Z`, `Elevation` solo segun caracterizacion (hasta entonces `0`), bucles con su `HatchLoopTypes` en la forma canonica; SOLID/TRACE con `Thickness = 0` y `Normal ≈ +Z` | CLOSED BY V8 (contrato); `Elevation` admitida: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §11.3 puntos 6.1 y 8; T-M62 |
| `[V8-D05]` | AR7-01 · R:V8-01 · C:V8-01 | — | Tipo de linea **complejo** (con formas o texto) en cualquier primitiva ⇒ `UNKNOWN` en el primer corte, salvo evidencia expresa posterior | CLOSED BY V8 | §11.3 puntos 6.1 y 6.2; T-M64 |
| `[V8-D06]` | AR7-02 · R:V8-02 · C:V8-02 | `Explicit(valor)` | **TYPED EXPLICIT SOURCES**: `Explicit` conserva metodo + valor. `ACI(7) ≠ RGB(255,255,255)`; `ColorBook(libro, nombre)` solo igual al mismo libro y nombre; `LineWeight` por estado enumerado + valor; `Transparency` por metodo + alfa; estilo de trazado segun `PSTYLEMODE`; `LinetypeScale` por igualdad exacta. Nunca solo el valor visual resuelto | CLOSED BY V8 | §11.3 punto 6.2; T-M54 |
| `[V8-D07]` | AR7-02 · R:V8-02 · C:V8-03 | Estilo de trazado segun el modo «de la biblioteca inspeccionada»; modo desconocido ⇒ `UNKNOWN` | **SCRATCH DATABASE CONTEXT**: la base auxiliar reproduce el contexto necesario de la autoridad origen (como minimo `PSTYLEMODE`, `INSUNITS`, escala de anotacion cuando pueda afectar a la evaluacion y lo que descubra CT-36). Autoridad = dibujo actual para una definicion existente; biblioteca para una definicion solo de biblioteca, con re-verificacion de la definicion importada en PREPARE. Contexto no reproducible ⇒ `UNKNOWN` | CLOSED BY V8 (contrato); contexto exacto: DEFERRED TO G3/G5 WITH FAIL-CLOSED (CT-36 con dibujo STB y CTB) | §11.3 puntos 3, 4 y 6.2; T-M61; T-M70 |
| `[V8-D08]` | AR7-03 · R:V8-03 · C:V8-04 | «`LinetypeScale`: valor por entidad, sin herencia» | `LinetypeScale` por entidad sigue **siempre** en la firma; para primitivas sin continuidad demostrable se añade `LinetypePatternContext`: factores de escala absolutos y `LinetypeScale` de cada ancestro relevante de la ruta. Un factor de ancestro que altere fase o escala sin modelar (incluida la escala no uniforme) ⇒ `UNKNOWN` | CLOSED BY V8 (contrato); efecto real: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §11.3 puntos 6.2, 6.4 y 8; T-M65 |
| `[V8-D09]` | AR7-03 · C:V8-04 | — | `LTSCALE`, `PSLTSCALE` y `MSLTSCALE` = contexto global comun a ambas mitades, caracterizado por CT-36; `CELTSCALE` = afecta a la creacion de entidades nuevas y no altera retroactivamente las de la pieza evaluada; se caracteriza | CLOSED BY V8 (declaracion); confirmacion: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §11.3 punto 6.2; CT-36 |
| `[V8-D10]` | AR7-04 · R:V8-04 · C:V8-05 | Definicion «clonada desde la cache de sesion» (`BlockLibraryImporter.cs:121-170`) | **CACHE ACCESS**: la sonda no accede por reflexion ni muta la cache privada. Ruta permitida: `BlockLibraryImporter.EnsureBlocks(auxDb, requiredNames)` (o la API publica equivalente vigente), que clona desde la cache hacia la base auxiliar; despues se verifica que existan **todos** los bloques requeridos; retorno 0 o bloque ausente ⇒ `UNKNOWN` / `FAIL_CLOSED`. G-M20: solo la API publica autorizada, sin reflexion ni acceso al campo cacheado. CT-36/T-M63 observan la cache solo desde un harness fuera de `src/` | CLOSED BY V8 | §11.3 puntos 3 y 14; §9.5; G-M20; T-M63 |
| `[V8-D11]` | AR7-05 · R:V8-05 · C:V8-06 | Paso de ruta por «el nombre de su definicion evaluada» y ordinal | **CANONICAL NESTED PATH** estable: anidado dinamico = identidad estable del `DynamicBlockTableRecord` o definicion base + valores dinamicos efectivos + ordinal canonico por contenido; estatico = identidad estable de la definicion + ordinal. Nunca `ObjectId`, handle ni nombre anonimo generado (`*U…`). Empate no resoluble ⇒ `UNKNOWN`. `TraceFingerprint` usa esta ruta | CLOSED BY V8 | §11.3 puntos 6.2 y 10; T-M66 |
| `[V8-D12]` | AR7-06 · R:V8-06 · C:V8-07 | Orden visual solo dentro de la pieza; V-VIEW-ALL por multiconjunto | **PLAN-LEVEL VISUAL ORDER** entre instancias de pieza: sin superposicion material, orden irrelevante; con superposicion material que afecta a la apariencia, `RelativePlanVisualOrder(fuente) ≡ RelativePlanVisualOrder(reflejado)` para las homologas. El orden sale de la emision y materializacion del builder y de las relaciones de Sortents aplicables; no demostrable ⇒ X-22 / `FAIL_CLOSED`. V-VIEW-ALL deja de tratar esos pares como multiconjunto puro. Sin teoria global de DRAWORDER fuera de los pares materiales | CLOSED BY V8 (contrato); orden por kind y vista: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §6.6; §11.1; §11.2; §11.3 punto 6.6; T-M67; CT-48 |
| `[V8-D13]` | AR7-07 · R:V8-07 · C:V8-08 | ST-13: la referencia nueva usa solo la capa de su fuente (como I-51) | **SOURCE BLOCKREFERENCE PRESENTATION**: la apariencia de la referencia RackCad fuente forma parte del contrato. La referencia nueva conserva, tipados y asignados explicitamente, `LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `PlotStyle` y `Visible` | CLOSED BY V8 | §4.1 ST-13; §5.2; §8.2; §9.2; T-M68 |
| `[V8-D14]` | AR7-07 · C:V8-08 | — | **ST-18**: estado de presentacion no transportable o no caracterizado (XCLIP, filtro espacial, presentacion u override desconocido, fuente anotativa, dinamica o anonima, otro estado visual no modelado) ⇒ `FAIL_CLOSED` (E4) antes de MUTATE; **no** es un aviso | CLOSED BY V8 (contrato); inventario de propiedades: DEFERRED TO G3/G5 WITH FAIL-CLOSED (propiedad no clasificada ⇒ E4) | §4.1; PRE-15; L-33; T-M68; CT-49 |
| `[V8-D15]` | AR7-07 · C:V8-08 | — | **ST-19**: orden visual en Model Space. Las copias se crean en el orden efectivo relativo de sus fuentes; si una copia se superpone de forma visualmente material (o no clasificable) con un objeto no seleccionado que va despues de su fuente en el orden efectivo, el orden relativo no puede preservarse ⇒ `FAIL_CLOSED` (E10) en PREPARE, antes de MUTATE | CLOSED BY V8 (contrato); clasificacion real de superposiciones y orden: DEFERRED TO G3/G5 WITH FAIL-CLOSED | §4.1; §9.1; PRE-16; L-34; T-M69; CT-49 |
| `[V8-D16]` | AR7-08 · R:V8-08 · C:V8-09 | Caso F con siete variantes | Caso F con doce: se añaden (8) `LinetypeSource` `ByLayer` frente a `Explicit` con el mismo tipo actual; (9) `FillSignature` distinta; (10) `SourceKind` distinta; (11) `Plinegen` o `Closed` distintos; (12) contexto de escala de ancestros o de patron distinto. Matriz clase × caso donde tenga sentido; guarda de autoridad unica de extraccion de `VisualSignature` (G-M21); CT-37 con dobles, no AutoCAD | CLOSED BY V8 | §11.3 punto 9; T-M40; T-M54; G-M21 |
| `[V8-D17]` | AR7-09 · R:V8-09 · C:V8-10 | I-53S `e528ef2` registrado como NON-MATERIAL; re-medicion «cuando I-53S se integre» | Clasificacion exacta: materialidad del contrato de I-52 = NON-MATERIAL; materialidad de coordinacion = MATERIAL; baseline de C2-4 = MATERIAL / CHANGED. `C2_4_I52_TheEditorStateAfterADistribution_EqualsTheReopenedDocument_InEveryView` es evidencia de I-53S y **no** cierra C2-4 de I-52. Si I-53S integra **antes** del Candidato de I-52: tras el rebase, CT-26, C2-4, T-M30 y las guardas relacionadas se ejecutan sobre el arbol combinado y condicionan el cierre de G6/G7. Si integra **despues**: reportar al Coordinador y volver a medir contra un main que contenga I-52. **Observado en el re-fetch de V8: I-53S ya esta integrada en main (`104ef3a`, merge de I-53S E2), antes del Candidato, asi que rige la rama «antes»**; la regla «despues» queda para integraciones posteriores (por ejemplo, I-53D) | CLOSED BY V8 | §6.7; §15; CT-41 |
| `[V8-D18]` | AR7-10 · R:V8-10 · C:V8-11 | I-49 `cae7a9f` | I-49 `c5b9ede` (ultimo observado por el Coordinador) añadia ADR-0040 `propuesto` y el registro de A1. **Observado en el re-fetch de V8**: I-49 `8ed15f7`, rebasada sobre `104ef3a` (`git range-diff` identico para G5 `977f873` y A1 `c7fabbe`, antes `fb9f630` y `cae7a9f`); el Owner **acepto ADR-0040** (`2eeeab1`; tras el rebase, `8f6d583`), que **reemplaza a ADR-0038** conservando D1–D7 y D9–D25 y sustituyendo solo la guarda del parser de D8; nuevo Consensus Freeze `8ed15f7`; G6 de I-49 sigue BLOCKED; sin `PlanReadSet`, evaluador ni dominio nuevos. Se cumple la condicion de V8-11: las referencias de I-52 a ADR-0038 se releen en ADR-0040, que pasa a ser la autoridad vigente de I-49, sin cambio en lo que I-52 consume (D19 `PlanReadSet`, dominio del consumidor, `expression`) ⇒ **NON-MATERIAL**. Censo ADR-0035..0040 | CLOSED BY V8 | §9.2.1; §10.2; §15; §16 |
| `[V8-D19]` | AR7-10 · R:V8-10 · C:V8-12 | I-54 `6b438be`; CA-4 como portador del sobre | I-54 `ca71962` (G6: `CustomPropertiesData`, `CustomPropertiesExecutor`, NOD `RACKCAD_CUSTOM_PROPERTIES` en el Plugin): materialidad arquitectonica para I-52 = NON-MATERIAL; coordinacion y guardas cruzadas = MATERIAL. **CA-4** = `RackEmbedDocument.CustomProperties`, portador **RACK-LEVEL** que se preserva con la copia. **CA-4b** = NOD `RACKCAD_CUSTOM_PROPERTIES`, autoridad **DRAWING-LEVEL** compartida por origen y copia en el mismo dibujo, que RACKMIRROR **no** copia, **no** refleja, **no** transforma y **no** escribe. Al integrar I-54: re-medir sus guardas y añadir T-GRD-04..T-GRD-07 a la reconciliacion y a G-M18; I-52 no llama a sus autoridades ni a su commit fisico (G-M22). **Observado en el re-fetch de V8**: I-54 `5c16ae8`, rebasada sobre `104ef3a` (`git range-diff` identico: G6 `deca1e0` y G6-CLOSE `d9f8c57`, antes `ca71962` y `b4e99e4`) con G7A `5c16ae8` solo docs (el Owner acepta `RACKPROPIEDADES` con alias `RPR`; G7 autorizado, G8+ no); su G7B afectara al censo de comandos y previsiblemente a `RackCommandReference.cs`, que I-52 G7 tambien toca: coordinacion y serializacion, sin cambio de contrato | CLOSED BY V8 | §6.4; §10.4; §15; G-M18; G-M22 |
| `[V8-D20]` | AR7-11 · R:V8-11 · C:V8-13 | Decision 13: «La caracterizacion demuestra…» | ADR-0036 sigue `propuesto`; decision 13: «la caracterizacion **debe** demostrar»; principio de variantes cerradas; `SourceKind`, `Plinegen`/`Closed`, `Explicit` tipado, orden visual de plan, presentacion de la referencia fuente, ruta anidada estable, contexto de la base auxiliar y CA-4b en la parte de coordinacion y persistencia (no como propiedad de la pieza); consecuencias genericas sin piezas concretas de §1.4 | CLOSED BY V8 | ADR-0036 |
| `[V8-D21]` | C:V8-14 | Plan de G3 de V7 | CT-36, CT-47 y las pruebas caracterizan: politica de clase y variante; `Plinegen`; `Closed`; grosor; normales; `MInsertBlock`; definiciones anidadas anotativas; tipos de linea complejos; `HatchLoopTypes`; `Color`, `Transparency` y `LineWeight` tipados; contexto STB/CTB de la base auxiliar; escala de anotacion; escalas de ancestros y patron de tipo de linea; ruta publica de clonado desde la cache; ruta anidada estable; superposicion y orden de plan; presentacion de la referencia fuente; XCLIP y filtros espaciales; C2-4 combinado de I-53S cuando aplique; guardas cruzadas de I-54 cuando aplique. CT-48 y CT-49 nuevas; G3 sigue sin produccion; contradiccion material de V8 ⇒ STOP → Proposal V9 | CLOSED BY V8 | §12; §14 |
| `[V8-D22]` | C:V8-15 · R:V8-12 | Contradiccion material de V7 ⇒ Proposal V8 | Nueva matriz `[V8-Dnn]` sobre AR7-01..AR7-11 y las precisiones del Coordinador; lo demas de V7 sigue vigente; `OPEN MATERIAL = NONE`; toda contradiccion material de V8 ⇒ **Proposal V9**; proceso: acuerdo sobre V8 → rebase/reconciliacion → freeze → re-check → O-1 → G3 → (V9 si G3 contradice / ADR-0036 aceptado) → G4 | CLOSED BY V8 | §1.2; §7.0; §14.1; §16; §19 |
| `[V8-D23]` | Precision al redactar | §1.4 con la medicion de la revision de V6 | §1.4 registra la medicion de la revision de V7 sobre la copia (mismo SHA-256): 0 emparejamientos entre clases distintas; `Closed` igual en todas las polilineas homologas; las 40 LWPOLYLINE con `Plinegen` usan `Continuous` explicito y estan fuera de las vistas medidas; 0 diferencias de escala o `LinetypeScale` de ancestros en trazos sin continuidad demostrable; 0 anidados anonimos y 0 escalas no uniformes; 0 SORTENTS, anotativos, MINSERT y XCLIP; tipos de linea simples. Las reglas nuevas de V8 no quitan piezas estructurales medidas | CLOSED BY V8 (registro) | §1.4 |
| `[V8-D24]` | Precision al redactar | — | Paralelas posteriores a los ultimos SHA observados por el Coordinador, registradas sin bloquear: I-54 `b4e99e4` (G6-CLOSE, solo docs) en el preflight; en el re-fetch previo al commit, main `104ef3a` (I-53S E2 integrada), I-49 `8ed15f7` (rebasada; ADR-0040 aceptado), I-54 `5c16ae8` (rebasada; G7A solo docs) e I-53D `aa0f817` (reclamo `c5a1404` y bootstrap, solo docs). `git range-diff` sin cambios de produccion en I-49 ni en I-54; las condiciones que preven V8-10 y V8-11 se cumplen sin cambiar el contrato | CLOSED BY V8 (registro) | §15.1 |
| `[V8-D25]` | Precision al redactar | PREPARE sin comprobacion de orden en Model Space | ST-19 depende de la linea: se evalua en PREPARE tras calcular `P'` y **antes** de `EnsureForPlan`, asi que un fallo no importa bloques; la huella de §9.2 incluye el contexto de orden de Model Space y MUTATE lo re-verifica | CLOSED BY V8 | §9.1; §9.2; §9.4 |
| `[V8-D26]` | Precision al redactar | L-01..L-32 | O-1 se reevalua sobre L-01..L-35: L-33 (presentacion no transportable), L-34 (orden en Model Space no demostrable) y L-35 (orden visual entre piezas no conservado); L-22, L-29 y L-30 ampliadas con el contexto de la base auxiliar, las fuentes tipadas, `SourceKind`, `Plinegen`/`Closed`, el contexto de patron y las variantes cerradas | CLOSED BY V8 | §1.2; §1.3 |
...
|---|---|---|
| Viabilidad, paridad y rendimiento de la sonda por estado en `acad.exe` (CT-36) | sin sonda viable o sin paridad ⇒ X-17a en toda pieza con cambio de mano | G3; si no es viable ⇒ Proposal V9 o reduccion de alcance |
...
| **Completitud de los atributos de representacion por clase soportada** (`[V8-D01]`) | atributo, subclase o variante sin clasificar ⇒ `UNKNOWN` (X-17b) | G3 (CT-36) |
...
| **Orden de emision y materializacion por kind y vista, y superposiciones entre piezas** (`[V8-D12]`) | par material sin orden demostrado ⇒ X-22 | G3 (CT-48, CT-36) |
| **Inventario de propiedades de presentacion de la referencia fuente** (`[V8-D14]`) | propiedad o estado no clasificado ⇒ E4 (ST-18) | G3 (CT-49) |
| **Superposiciones y orden efectivo en Model Space** (`[V8-D15]`) | superposicion no clasificable con un objeto posterior a la fuente ⇒ E10 (ST-19) | G3 (CT-49) |
...
| Tasa de falsos positivos y negativos de la canonicalizacion (CT-36, CT-47) | correspondencia ambigua ⇒ `UNKNOWN` | G3; ambiguedad material ⇒ Proposal V9 |
| Contenido de la cache de biblioteca medido antes y despues | contrato READ / CLONE ONLY por `EnsureBlocks` y verificacion de bloques; G-M20 | G3 (CT-36, harness fuera de `src/`) y G6/G7 (T-M63) |
...
| Estabilidad 9b de DEP-05..DEP-07 y cierres de descriptores | contraejemplo ⇒ STOP → Proposal V9 | G3 (CT-37, CT-38) |
...
| **Guardas del borde fisico de I-54** (T-GRD-04..T-GRD-07, `[V8-D19]`) | sin llamadas de I-52 a sus autoridades (G-M22); al integrar, re-medicion y G-M18 | reconciliacion posterior a la integracion de I-54 |

### 0.4 Estado de las marcas de V7 bajo V8

| Marca V7 | Estado en V8 |
|---|---|
...
| `[V7-D23]` | sustituida por `[V8-D22]` (Proposal V9) |
...
### 0.5 Estado de las marcas de V6 (fijado por V7; sin cambios en V8)
...
### 0.6 Estado de las marcas de V5 (fijado por V6; sin cambios en V7 ni V8)
...
### 0.7 Alias historicos de V2 (solo reconciliacion)

| Alias V2 | Equivalente (V3..V8) |
|---|---|
...
### 0.8 Cobertura explicita

| Hallazgo o cambio | Marca |
|---|---|
| AR7-01 (a) politica de variantes y tipo exacto | `[V8-D01]`, `[V8-D03]`, `[V8-D04]` |
| AR7-01 (b) clase de origen, `Plinegen` y `Closed` | `[V8-D02]` |
| AR7-01 (c) tipos de linea complejos | `[V8-D05]` |
| AR7-02 | `[V8-D06]`, `[V8-D07]` |
| AR7-03 | `[V8-D08]`, `[V8-D09]` |
| AR7-04 | `[V8-D10]` |
| AR7-05 | `[V8-D11]` |
| AR7-06 | `[V8-D12]` |
| AR7-07 | `[V8-D13]`..`[V8-D15]`, `[V8-D25]`, `[V8-D26]` |
| AR7-08 | `[V8-D16]` |
| AR7-09 | `[V8-D17]` |
| AR7-10 | `[V8-D18]`, `[V8-D19]` |
| AR7-11 | `[V8-D20]` |
| Cambios exactos R:V8-01..R:V8-12 de la revision | V8-01 → D01..D05; V8-02 → D06, D07; V8-03 → D08, D09; V8-04 → D10; V8-05 → D11; V8-06 → D12; V8-07 → D13..D15; V8-08 → D16; V8-09 → D17; V8-10 → D18, D19; V8-11 → D20; V8-12 → D22 |
| Precisiones del Coordinador C:V8-01..C:V8-15 | V8-01 → D01..D05; V8-02 → D06; V8-03 → D07; V8-04 → D08, D09; V8-05 → D10; V8-06 → D11; V8-07 → D12; V8-08 → D13..D15, D25, D26; V8-09 → D16; V8-10 → D17; V8-11 → D18; V8-12 → D19; V8-13 → D20; V8-14 → D21; V8-15 → D22 |
| Paralelas observadas en el preflight de V8 | `[V8-D17]`..`[V8-D19]`, `[V8-D24]` |
| Precisiones del ejecutor | `[V8-D23]`..`[V8-D26]` |
...
Registro: `docs/automation/decisions/I-52.md` §4. Vinculan V8; **no** son consenso final.
...
| **PDC-9** | Solo Model Space; MINSERT ⇒ fail-closed; linea por dos puntos UCS→WCS; fuente canonica | `Origin = 0` (ST-14), `View`/`Section` decodificables (ST-15), bloques dinamicos, anonimos o anotativos ⇒ fail-closed (ST-17); las restricciones estrictas solo se aplican a candidatos RackCad (precedencia C1..C5, §4.1); la copia conserva la presentacion de su referencia fuente (ST-13), un estado de presentacion no transportable falla cerrado (ST-18) y el orden visual en Model Space se demuestra o falla cerrado (ST-19) (`[V8-D13]`..`[V8-D15]`) |
...
|---|---|---|---|
| **O-1** | **Alcance del primer corte.** El Owner acepta la **envolvente maxima de alcance** de §1.3 (L-01..L-35), su politica fail-closed, la **seleccion todo-o-nada** y la **dependencia de la biblioteca efectiva**. O-1 **no** promete que G3 verifique todos los candidatos y **no** acepta como resultado universal la inspeccion de una biblioteca concreta (§1.4). Si G3 reduce materialmente la envolvente ⇒ Proposal V9 y O-1 se reevalua | envolvente de §1.3 | **pendiente**; se pide **despues del freeze tecnico** y **antes de G3**; **no** se pide en este gate |
...
| **O-3** | **Evidencia de simetrias** | ningun bloque se asume simetrico; la prueba es la **equivalencia visual-geometrica** por estado evaluado, con politica cerrada de clases y variantes, fuentes visuales simbolicas tipadas, `VisibilityPath` y orden visual dentro de la pieza y entre piezas (`GeometrySymmetryResult`, §11.3); la confirmacion del Owner **nunca** es la unica prueba | G3 (CT-36, CT-47, CT-48) y G9 (M-15, M-27, M-32, M-36) |
| **O-4** | **ADR-0036** | `propuesto` | se pide **despues de G3** si G3 no contradice materialmente V8; si la contradice ⇒ Proposal V9; **precondicion de G4** |
...
| **L-30** | **Pieza con una clase o variante fuera de la politica cerrada** (§11.3 punto 6.1): subclase de una clase soportada (`MInsertBlock` anidado), definicion anidada anotativa, referencia anidada con XCLIP, grosor, normal, elevacion o ancho fuera de la tabla, tipo de linea complejo, HATCH de degradado (hoy `TARIMA_GENERICA`, §1.4) o de patron, SOLID/TRACE/HATCH no canonizable de forma inequivoca, o atributo de representacion sin clasificar | fail-closed | X-17b |
...
| **L-34** | **Orden visual en Model Space no demostrable**: una copia se superpone de forma visualmente material, o no clasificable, con un objeto no seleccionado que va despues de su fuente en el orden efectivo de Model Space | fail-closed en PREPARE, antes de MUTATE (E10) | ST-19, PRE-16 |
| **L-35** | **Orden visual entre piezas del plan no conservado**: instancias de pieza distintas que se superponen de forma visualmente material y cuyo orden relativo de emision no se conserva en el diseño reflejado, o no puede demostrarse | fail-closed | X-22 (§11.3 punto 6.6) |
...
| Campo | Valor | Papel |
|---|---|---|
| SHA-256 del contenido binario | `b4ca2248db9c3d72487ac8b5b1e5510cdd8aba231ab340541d91bebca2d560e8` (re-verificado en los preflights de V7 y V8) | **identidad de la evidencia** |
...
entidades sin continuidad demostrable, **orientacion** en coordenadas de la pieza. La revision de V7 añadio, sobre el
mismo corpus, la clase de origen de cada primitiva, el indicador `Closed` de las polilineas, los factores de escala y la
`LinetypeScale` de los ancestros, y un inventario de mecanismos (SORTENTS, anotativos, MINSERT, XCLIP, `Plinegen` y tipos
de linea). Coordenadas con 9 decimales (las desviaciones de 1e-9 no son concluyentes, `[V6-D30]`); sin evaluar estados
nuevos, sin estilo de trazado ni orden visual; **no** es CT-36.
...
quitan, en esta medicion, ninguna de las piezas estructurales medidas. Estado: **DEFERRED TO G3** con fail-closed
(X-17a, X-17b, X-22, X-23).
...
| **Pieza con una clase o variante fuera de la politica cerrada** (subclase como `MInsertBlock`, anotativo o XCLIP anidados, grosor, normal, elevacion o ancho fuera de la tabla, tipo de linea complejo, HATCH de degradado o patron, SOLID/TRACE/HATCH no canonizable) | fuera de la politica de geometria del primer corte (hoy `TARIMA_GENERICA`, §1.4) | X-17b |
...
| **Orden visual entre piezas del plan no conservado** | superposicion material entre instancias de pieza cuyo orden relativo de emision no se conserva en el diseño reflejado | X-22 (§11.3 punto 6.6) |
...
| **Referencia RackCad fuente con estado de presentacion no transportable** (XCLIP, atributos, entrada de diccionario de extension o XData de efecto visual no clasificada, otro estado visual no modelado) | la copia no conservaria la apariencia de la fuente | ST-18 (E4) |
| **Orden visual en Model Space no demostrable** | una copia se superpone de forma material, o no clasificable, con un objeto no seleccionado que va despues de su fuente | ST-19 (E10) |
...
Para cada objeto seleccionado o, en este orden (la primera regla que decide, decide):
  ...
  C4  restricciones estrictas del candidato, en este orden; la primera que falla ⇒ E4 de TODA la operacion:
        ST-3 MINSERT · ST-17 dinamica, anonima o anotativa · ST-18 presentacion no transportable · ST-5 Normal ·
...
PREPARE (depende de la linea): ST-19 orden visual en Model Space, antes de EnsureForPlan (§9.1)
```
...
| **ST-18** | C4 | Candidato con estado de presentacion **no transportable o no caracterizado** (`[V8-D14]`): filtro espacial (XCLIP) u otra entrada de `ACAD_FILTER` en su diccionario de extension; referencias de atributo (`AttributeCollection` no vacia); cualquier otra entrada del diccionario de extension o XData de efecto visual que CT-49 no clasifique como no visual; propiedad de representacion de `Entity` o `BlockReference` fuera del inventario de CT-49 (por ejemplo, un material distinto del de la capa); u otro estado visual no modelado. Las fuentes dinamicas, anonimas o anotativas ya las cubre ST-17 | **fail-closed explicito (E4)** antes de MUTATE, con mensaje propio; **nunca** un aviso ni una copia sin esa presentacion |
...
| ST-13 | C5 | Presentacion de la referencia (`[V8-D13]`) | la referencia nueva conserva, tipados como en §11.3 punto 6.2, `LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `PlotStyleName` y `Visible` de **su** referencia fuente (capturados en SNAPSHOT), asignados explicitamente y nunca por los valores por defecto de la base. RACKDUPLICAR (I-51) sigue copiando solo la capa (G-M12) |
...
| **ST-19** | PREPARE | Orden visual en Model Space (`[V8-D15]`): las copias se crean en el **orden efectivo relativo de sus fuentes** (orden de dibujo de Model Space con su `SortentsTable`); cada copia queda despues de todo objeto existente. Para cada objeto no seleccionado que va **despues** de la fuente de una copia en ese orden y cuya extension se interseca con la extension evaluada de la copia en su destino, la superposicion se clasifica con las clases de §11.3 punto 6.5 ampliadas a Model Space (rellenos, degradados, solidos, wipeouts, imagenes, OLE, texto con mascara, transparencia no opaca y referencias que los contengan, evaluadas con la politica de §11.3 punto 6.1) | superposicion **material o no clasificable** ⇒ el orden relativo no puede preservarse ⇒ **fail-closed (E10)** en PREPARE, tras calcular `P'` y antes de `EnsureForPlan` y de MUTATE (`[V8-D25]`) |
...
**Presentacion y orden de la referencia (`[V8-D13]`..`[V8-D15]`).** La apariencia de la referencia RackCad fuente forma
parte del contrato. La copia conserva las ocho propiedades de ST-13 cuando son representables; en el mismo dibujo lo son
siempre, porque capa, tipo de linea y estilo de trazado son los mismos objetos de la base. Todo estado que no se puede
transportar o que no esta caracterizado hace fallar la operacion antes de MUTATE (ST-18); nunca se sustituye por un aviso.
CT-49 inventaria en AutoCAD 2025 las propiedades de `Entity` y `BlockReference` y clasifica cada una como transportada
(ST-13), colocacion (`P'`), restriccion estricta (ST-3, ST-5..ST-10, ST-17), no visual o no transportable (ST-18); una
propiedad sin clasificar ⇒ E4. El orden en Model Space se demuestra en PREPARE (ST-19): entre copias de fuentes
seleccionadas se conserva siempre por construccion; frente a un objeto no seleccionado que va antes que la fuente se
conserva sin clasificar nada, porque la copia queda despues de ambos; una copia que se superpone con su propia fuente queda
encima de ella, como la copia del `MIRROR` nativo, sin relacion previa que conservar.
...
**STOP → Proposal V9**. No se duplica la agrupacion en un segundo planificador. Las guardas G-R1..G-R6 leen el codigo de
...
- RACKMIRROR tiene **su propio** SNAPSHOT neutral, en un archivo nuevo del Plugin. Captura por referencia: handle,
  ...
  `LinetypeScale`, `LineWeight`, `Transparency`, `PlotStyleName`, `Visible`; ST-13), estado no transportable (filtro
  espacial, atributos, diccionario de extension y XData de efecto visual; ST-18), orden efectivo en Model Space
  (`SortentsTable`; ST-19), `Origin` de la definicion y payload. Lee el registro **una** vez por comando y registra su
  read-set (§9.2).
...
- **CA-4b no es portador.** Figura en la tabla solo para fijar que el espejo no toca la autoridad de nivel dibujo de I-54:
  la copia, en el mismo dibujo, resuelve sus propiedades de rack (CA-4) contra la misma coleccion del NOD. La allowlist de
  portadores sigue siendo CA-1..CA-6.
...
Caso A — un fondo, valores positivos. Claro i sin medio frente.
  ...
  V8: Dep(i) verdadero; W_lb(i) = max(70, −∞) = 70 < 80 ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED;
...
Caso B — dos fondos, sin overrides.
  ...
  V8: Dep(i) verdadero; W_lb(i) = −∞ ⇒ ¬DEP-ROW ⇒ S-17d UNKNOWN ⇒ FAIL_CLOSED (y S-26 UNKNOWN).
...
- **CT-37 (C:V6-10, incondicional; Core, `[V7-D18]`):** diferencial con dos o mas estados del registro por **cada**
  ...
  DEP-09/DEP-10a..c. Un contraejemplo no previsto por V-DEP contradice V8 ⇒ **STOP → Proposal V9**.
...
V-VIEW-ALL  para cada (V,σ) ∈ A_k(μ_k D):  Π_(V,σ')(Eff(μ_k D, R)) ≡ F ∘ Π_(V,σ)(Eff(D, R))     (§3.8, §11)
            ≡ = multiconjunto de piezas + orden visual relativo de los pares con superposicion material (§11.1)
...
- Desde V8, V-VIEW-ALL no es un multiconjunto puro para los pares de instancias de pieza con superposicion visualmente
  material: su orden relativo de plan se conserva o la vista falla (X-22; §11.3 punto 6.6, `[V8-D12]`).
...
- I-53S G5 (`e528ef2`) conecta `ApplyHeaderBatch` a `RackSelectiveWindow` y **ya esta integrada en main** (`104ef3a`,
  merge de I-53S E2). Clasificacion exacta (`[V8-D17]`): materialidad del contrato de I-52 = NON-MATERIAL; materialidad de
  coordinacion = MATERIAL; baseline de C2-4 = MATERIAL / CHANGED. Su prueba
  `C2_4_I52_TheEditorStateAfterADistribution_EqualsTheReopenedDocument_InEveryView` es evidencia de I-53S y **no** cierra
  C2-4 de I-52. Como I-53S integro antes del Candidato de I-52, tras el rebase CT-26, C2-4, T-M30 y las guardas
  relacionadas se ejecutan sobre el arbol combinado y condicionan el cierre de G6/G7. Si otra integracion ocurre despues
  del Candidato (por ejemplo I-53D, reclamada en `c5a1404`, al conectar el Dinamico), se reporta al Coordinador y se
  vuelve a medir contra un main que contenga I-52 (§15.3).

---

## 7. Matriz de representabilidad V8
...
EvidenceStatus               (de donde sale la clasificacion)
  ...
  G3_VERIFIED       confirmada por la caracterizacion de G3 (ninguna fila en V8)
...
envolvente de O-1), el ADR, una regla de reflexion o la arquitectura ⇒ **Proposal V9**. T-M47 lo hace ejecutable desde G5.
...
| X-04 | Colocacion y presentacion de la referencia | `P' = G·P·F`; presentacion de la fuente conservada (ST-13) | R | CODE_SUPPORTED | ALLOW | §3; §4.1 |
...
| X-17a | Pieza con cambio de mano sin equivalencia visual-geometrica verificada (`GeometrySymmetryResult`): sonda no disponible, sin paridad con el materializador, sin punto de observacion soportado o sin contexto reproducible de la base auxiliar (`[V8-D07]`); bloques requeridos ausentes en la base auxiliar tras `EnsureBlocks` (`[V8-D10]`); simetria no verificada (incluida la canonicalizacion conservadora); ruta anidada estable (`[V8-D11]`) o correspondencia ambigua; eje oblicuo | la equivalencia no esta demostrada | UK | CODE_SUPPORTED | FAIL_CLOSED | §11.3 |
| X-17b | Geometria evaluada con una clase o variante fuera de la politica cerrada (`[V8-D01]`..`[V8-D05]`): subclase de una clase soportada (`MInsertBlock`), definicion anidada anotativa, XCLIP anidado, grosor, normal, elevacion o ancho fuera de la tabla, tipo de linea complejo, HATCH de degradado o patron, SOLID/TRACE/HATCH no canonizable, atributo de representacion sin clasificar, o un ciclo de anidados | fuera de la politica del primer corte | UK | CODE_SUPPORTED | FAIL_CLOSED | §11.3 puntos 6.1 y 8 |
...
| X-22 | Fuentes visuales simbolicas tipadas, `SourceKind`, `PolylineGeneration`, `LinetypePatternContext` o `VisibilityPath` distintas entre homologas, mecanismo de visibilidad no modelado, orden visual relativo no demostrado dentro de la pieza, u orden visual de plan no conservado entre instancias de pieza con superposicion material (`[V8-D12]`) | la apariencia de la copia podria no ser el espejo del original en algun estado | UK | CODE_SUPPORTED | FAIL_CLOSED | §11.3 puntos 6.2..6.6 |
...
| PRE-14 | Sonda no disponible, fallida, sin paridad con el materializador, sin punto de observacion soportado o sin contexto reproducible de la base auxiliar para una pieza con cambio de mano; bloques requeridos ausentes en la base auxiliar tras `EnsureBlocks`; postcondicion del dibujo incumplida o contenido de la cache de biblioteca alterado (§11.3 puntos 4 y 14) | FAIL_CLOSED (E6, X-17a) | P7 | §11.3 |
| PRE-15 | Candidato RackCad con estado de presentacion no transportable o no caracterizado (ST-18, `[V8-D14]`) | FAIL_CLOSED (E4) | P1 | §4.1; CT-49 |
| PRE-16 | Orden visual en Model Space no demostrable para alguna copia (ST-19, `[V8-D15]`) | FAIL_CLOSED (E10) | PREPARE, antes de `EnsureForPlan` | §4.1; §9.1; CT-49 |
...
CreateReference(db, tx, definitionId, colocacion, presentacion) → referenceId      (Model Space; transaccion del llamador)
  ...
  LayerId, Color, Linetype, LinetypeScale, LineWeight, Transparency, PlotStyleName, Visible = los de la referencia
    fuente (SNAPSHOT, tipados; asignados explicitamente, nunca por los valores por defecto de la base)      (ST-13)
...
- **Presentacion y orden de la referencia** (`[V8-D13]`, `[V8-D15]`): la referencia nueva lleva la presentacion de su
  fuente (ST-13) y las referencias se crean en el orden efectivo relativo de las fuentes (ST-19). El contenido de la
  definicion nueva mantiene el orden de materializacion de §11.3 punto 6.6; el drawer no cambia.
...
SNAPSHOT   una transaccion de lectura: referencias y sus banderas de fuente (MINSERT, dinamica, anonima, anotativa),
           presentacion tipada, estado no transportable y orden efectivo en Model Space (§5.2), definiciones con
           Origin y payload, UNA lectura del registro de variables, huellas y read-set (§9.2)
...
PREFLIGHT  (sin mutacion; un solo orden; cualquier fallo ⇒ mensaje atribuible y FIN, cero mutacion)
  ...
  P7  V-VIEW-ALL                                   todas las (View, Section canonica) admisibles del rack logico (§3.8):
        ...
        P7b  el Plugin evalua cada estado con la sonda y la semantica de parametros del materializador, en una base
             ...
             GeometrySymmetryResult con variantes, valores efectivos, fuentes visuales simbolicas tipadas,
             VisibilityPath, contexto de patron, orden visual y transformacion efectiva
        P7c  Application completa la equivalencia visual-geometrica con los resultados, el orden visual de plan
             entre piezas (§11.3 punto 6.6), DEP-09/DEP-10a..c y PlanPlacementEvidence
...
PREPARE    colocaciones P' · orden visual en Model Space (ST-19; antes de importar) · planes efectivos de las vistas
           SELECCIONADAS (§8.1) · payloads definitivos · EnsureForPlan (infraestructura, §9.5) · re-verificacion de
           bloques · re-verificacion de huellas y read-set
...
**ST-19 en PREPARE (`[V8-D25]`).** El orden visual en Model Space depende de la linea (destino de cada copia), asi que no
cabe en PREFLIGHT. Se evalua en PREPARE tras calcular `P'` y **antes** de `EnsureForPlan`: un fallo termina en E10 sin
importar bloques. Los objetos relevantes y su orden efectivo entran en la huella (§9.2) y MUTATE los re-verifica.
...
| presentacion tipada (capa, color, tipo de linea, escala de tipo de linea, grosor, transparencia, estilo de trazado y visibilidad) y estado no transportable (filtro espacial, atributos, diccionario de extension, XData de efecto visual) | la referencia nueva copia la presentacion (ST-13); ST-18 |
| orden efectivo en Model Space de las fuentes y, desde PREPARE, de los objetos no seleccionados relevantes para ST-19 (identidad, existencia y posicion en el orden de dibujo) | un cambio invalida la demostracion del orden (ST-19, `[V8-D25]`) |
...
7. **Sonda (`[V8-D10]`).** La sonda obtiene las definiciones de biblioteca con `EnsureBlocks(auxDb, nombres)` sobre una
   base auxiliar privada: IF-1 no aplica a esa base, que no es el documento (CT-36 lo confirma); por IF-3, IF-4 e IF-6 el
   retorno 0 es silencioso, asi que la sonda verifica despues que existen **todos** los bloques requeridos en la base
   auxiliar; ausente ⇒ `UNKNOWN` (X-17a). Nada de esto modifica `BlockLibraryImporter.cs` (§20.3).
...
- **I-54 (Proposal V5 con consenso congelado; ADR-0039 aceptado; G3..G6 cerrados y G7 autorizado en su rama, rebasada
  ...
  G6-CLOSE `d9f8c57` y G7A `5c16ae8` solo docs—; sin integrar y sin tocar `RackEmbedDocument`, `RackEmbedComposer`,
  restamp, UI ni `ProjectVariables`, `[V8-D19]`)**: las propiedades de alcance Rack viven en el miembro
  `CustomProperties` del sobre (`JsonElement?`); `Compose` lo hereda y no hay promocion de major. Antes de integrarse,
  un build sin I-54 lo conserva en `ExtensionData` del sobre. En ambos casos es **portador RACK-LEVEL por nombre**
  (CA-4, §6.4) y el espejo lo conserva; la coleccion de definiciones del NOD es autoridad de nivel dibujo (CA-4b) que la
  copia comparte y el espejo no toca (G-M22). **D-21 sin cambio**: el espejo cumple el invariante (A y despues B). V4 y
  V5 de I-54 declaran residuales preexistentes del store del sobre que afectan a todo flujo que lo reescribe, tambien al
  espejo: **F-14a** (UTF-16 crudo invalido: la lectura lanza `ArgumentException`) ⇒ E2 (ST-2c); **F-14b** (surrogate
  ...
  `CustomProperties` `Null` o `Undefined` a ausente para todos los productores (contrato de I-54, T-ENV-05), T-M10 y
  T-M20 comparan con esa normalizacion. V5 de I-54 (C-F3) precisa que en los consumidores existentes el punto de fallo
  de F-14b depende de su granularidad transaccional (`RACKEDITAR` puede quedar parcialmente actualizado entre vistas);
  en el espejo el ensayo de P10 serializa todos los payloads **antes** de MUTATE, asi que F-14b termina en E8 sin
  ninguna mutacion parcial.
...
3. **Ruta primaria: STATE-EVALUATED VISUAL-GEOMETRIC EVIDENCE.** Para cada pieza de **cualquier** vista admisible del
   ...
   (BlockName, definicion real, vector de parametros dinamicos, marco local de insercion)
           ↓  crear una base auxiliar privada con el contexto de la autoridad origen (punto 4) y clonar alli la
              definicion: desde el dibujo si ya existe; si no, con BlockLibraryImporter.EnsureBlocks(auxDb, nombres)
              y verificacion de todos los bloques requeridos (punto 14); nunca evaluar en el dibujo ni en la cache
   ...
   **Fuente de la definicion.** Si el bloque ya existe en el dibujo, la evidencia corresponde a **esa** definicion,
   ...
   requeridos (punto 14, `[V8-D10]`). Tras `EnsureForPlan`, **PREPARE** vuelve a verificar la definicion **real** que
   quedo en el dibujo para las vistas que se dibujan: `DuplicateRecordCloning.Ignore` conserva una definicion local con
   el mismo nombre (`BlockLibraryImporter.cs:110`), asi que la biblioteca sola nunca basta si el DWG ya contiene el
   bloque. Para las vistas admisibles no seleccionadas, la equivalencia se afirma sobre la definicion evaluada en
   PREFLIGHT; la que un Insertar futuro importe queda bajo la limitacion general de §9.5 punto 6, igual que para el rack
   original. Como la equivalencia compara **fuentes** y no valores resueltos (punto 6), las capas que el dibujo ya
   tuviera con otras propiedades no la invalidan. El resultado depende de la **biblioteca efectiva** (L-28).
4. **Sin mutacion del DWG del usuario ni de la cache.** La evaluacion se hace en solo lectura o clonando y evaluando en una
   ...
   **Contexto de la base auxiliar (`[V8-D07]`).** La base auxiliar reproduce el contexto necesario de la **autoridad
   ...
   dibujo con estilos de trazado con nombre (STB) y otro dependiente del color (CTB).
...
6. **Politica visual-geometrica.**
   ...
   **6.1 CLOSED VARIANT POLICY: clase + variantes soportadas (`[V8-D01]`..`[V8-D05]`).** La allowlist ya no es solo por
   ...
   - **Completitud (G3).** CT-36 enumera, por clase soportada, los atributos de representacion que expone AutoCAD 2025 y
     confirma que la tabla los cubre. Un atributo que altere la representacion y no figure en la tabla deja la clase o la
     variante en `UNKNOWN` hasta clasificarse; si eso reduce materialmente la envolvente ⇒ Proposal V9.
   - Medido en la biblioteca inspeccionada (§1.4, `[V8-D23]`): ninguna variante excluida aparece en las piezas
     estructurales medidas; la politica no les quita alcance.
   ...
   **6.2 SYMBOLIC VISUAL SOURCES tipadas (`[V7-D01]`..`[V7-D03]`, `[V7-D27]`, `[V8-D06]`..`[V8-D09]`, `[V8-D11]`).** La
   ...
   PropertySource  := Explicit(metodo, valor)     // tipado: metodo + valor, nunca solo el valor (V8)
                    | ByLayer(Named(capa))
   ...
   - **`Explicit` tipado (`[V8-D06]`).** `Explicit` conserva el **metodo** y el **valor**, nunca solo el valor resuelto:
     - color: `ACI(n)`, `RGB(r, g, b)` o `ColorBook(libro, nombre)`; `ACI(7) ≠ RGB(255,255,255)`, y un color de libro solo
       es igual a otro del mismo libro y nombre, nunca por su RGB;
     ...
     - `LinetypeScale`: `double` por igualdad exacta del valor almacenado (V8 no introduce tolerancia; CT-36 registra las
       desviaciones reales).
   ...
   - **Contrato por propiedad** (estados de la API; CT-36 confirma los exactos y un estado encontrado y no modelado ⇒
     ...
     |---|---|---|
     | `Color` | `ByLayer`, `ByBlock`, color indexado, color verdadero, libro de colores | `Explicit(ACI n)`, `Explicit(RGB r,g,b)`, `Explicit(ColorBook libro, nombre)`, `ByLayer(Named)`, `InheritLayer`, `InheritBlock`; otro metodo ⇒ `UNKNOWN` |
   ...
   - **Escalas globales de tipo de linea (`[V8-D09]`).** `LTSCALE`, `PSLTSCALE` y `MSLTSCALE` son contexto global del
     ...
     evaluada, y la referencia de pieza que crea el materializador es la raiz comun `·`; CT-36 lo caracteriza.
   ...
   **visualmente material**:
   ...
   Si no puede demostrarse ⇒ X-22 / `FAIL_CLOSED`. V8 no define una teoria global de DRAWORDER para objetos que no se
   solapan; entre instancias de pieza distintas rige 6.6.

   **6.6 Orden visual de plan entre instancias de pieza (`[V8-D12]`).** 6.5 controla el orden **dentro** de una pieza; V8
   añade el orden **entre** instancias de pieza distintas de una misma vista (y entre las curvas de la familia
   Cantilever):

   - Si dos instancias distintas **no** se superponen de forma visualmente material, su orden es irrelevante y no generan
     relacion.
   - Si se superponen de forma visualmente material —las clases de 6.5 evaluadas sobre la geometria aplanada de cada
     instancia en el marco de la definicion: relleno ↔ relleno, relleno ↔ trazo, trazos coincidentes con `VisualSignature`
     distinta, entidades con `TransparencySource` no `Explicit` opaca y lo que descubra CT-36— y el orden afecta a la
     apariencia, para cada par `(a, b)` del plan fuente y sus homologas `(a', b')` del plan reflejado:

     ```text
     RelativePlanVisualOrder(fuente)(a, b) == RelativePlanVisualOrder(reflejado)(a', b')
     ```
   ...
   - Correspondencia de homologas ambigua para una instancia que participa en un par material, u orden relativo no
     demostrable ⇒ X-22 / `FAIL_CLOSED`.
   - V-VIEW-ALL deja de tratar esos pares como un multiconjunto puro (§6.6, §11.1). V8 **no** crea una teoria global de
     DRAWORDER fuera de los pares materiales.
   - Hecho del codigo, indicativo (CT-48 lo fija por kind y vista): el frontal Selectivo emite los postes, con sus placas
     y la seguridad de cada poste, antes que los largueros de los claros (`SelectiveFrontalBuilder.cs:82-152`), de modo que
     la relacion poste-larguero no depende del sentido de los frentes.
...
8. **Canonicalizacion conservadora y verificacion (`[V7-D10]`..`[V7-D12]`, `[V8-D02]`, `[V8-D04]`).** Antes de comparar,
   ...
   `GeometryTolerance.Length` (y `GeometryTolerance.Angle` donde aplique). V8 no introduce una tolerancia nueva: CT-36
...
14. **Contrato de caracterizacion en AutoCAD 2025 (CT-36).** G3 **no** añade la sonda productiva a `src/`: demuestra su
    ...
    **Acceso a la cache de biblioteca (`[V7-D15]`, `[V8-D10]`).** La `Database` cacheada por `BlockLibraryImporter`
    ...
    → BlockLibraryImporter.EnsureBlocks(auxDb, requiredNames)      (:37-119; API publica, o la API publica equivalente vigente)
        clona desde la cache privada hacia la base auxiliar, sin modificar BlockLibraryImporter.cs (§20.3)
    → verificar que TODOS los bloques requeridos existen en auxDb
        retorno 0 o bloque ausente ⇒ UNKNOWN ⇒ FAIL_CLOSED (X-17a; IF-3, IF-4 e IF-6 de §9.5)
    → evaluar en auxDb → descartar auxDb
    ```

    Queda prohibido: reflexion sobre `BlockLibraryImporter` o acceso a su campo cacheado; insertar referencias temporales o
    aplicar parametros dinamicos en la `Database` cacheada; abrir alli objetos `ForWrite`; y usarla como base auxiliar. Una
    definicion que ya existe en el dibujo se clona desde el dibujo, en solo lectura, sin pasar por la cache. G-M20 lo hace
    ejecutable. CT-36 y T-M63 pueden observar el contenido de la cache (HANDSEED, inventario de BTR y huella) **solo desde
    un harness fuera de `src/`**, si hace falta para demostrar que no cambio.
    ...
    **Identidad de la evidencia (`[V7-D17]`).** CT-36 y CT-47 registran el **SHA-256** del contenido binario de la biblioteca;
    ...
    CT-36 cubre ademas (`[V8-D21]`): politica de clases y variantes con tipo exacto (grosor, normales, elevacion,
    `MInsertBlock`, definiciones anidadas anotativas, XCLIP y filtros espaciales, tipos de linea complejos,
    `HatchLoopTypes`, y completitud de los atributos de representacion por clase); `SourceKind`, `Plinegen` y `Closed`;
    fuentes visuales simbolicas tipadas por propiedad (capa `0`, `ByLayer`, `ByBlock`, anidados, estados de la API,
    `Color`, `LineWeight` y `Transparency` tipados, y `PlotStyle` segun el modo de la autoridad origen); contexto de la
    base auxiliar con un dibujo STB y otro CTB, `INSUNITS` y escala de anotacion; `VisibilityPath` con estados de capas
    y ancestros variados; `LinetypeScale`, escalas y `LinetypeScale` de ancestros, fase y segmentacion de tipos de
    linea, y las escalas globales `LTSCALE`, `PSLTSCALE`, `MSLTSCALE` y `CELTSCALE`; multiplicidad; orden visual dentro
    de la pieza y, con CT-48, entre piezas; poligono SOLID/TRACE; bucles y estilo de islas de HATCH; degradados y
    patrones; evaluacion de estados nuevos; anidados con ruta estable; `applied`; punto de observacion; BTR evaluado,
    `Origin` y transformacion efectiva; valores efectivos y paridad con el materializador (`[V6-D14]`); clonado por
    `EnsureBlocks` con verificacion de bloques; canonicalizacion (falsos positivos y negativos); definicion local frente
    a biblioteca; obsolescencia y rendimiento preliminar. Si G3 demuestra que la ruta no es viable, que las clases o
    variantes reales no estan soportadas, que el contexto no se puede reproducir, que la biblioteca real reduce
    materialmente la envolvente mas alla de O-1 o que la canonicalizacion es materialmente ambigua ⇒
    **STOP → Proposal V9**. La sonda productiva nace en el Plugin en el gate en que la necesite la autoridad de planes o
    de materializacion (previsiblemente G6/G7); Application solo consume `GeometrySymmetryResult` (con dobles en las
    pruebas Core). Riesgo de rendimiento: R-24, medido en G7.
...
16. **Sin `GeometrySymmetryResult` verificado** (sonda no disponible, sin paridad, sin punto de observacion soportado o sin
    contexto reproducible de la base auxiliar, bloques requeridos ausentes tras `EnsureBlocks`, ruta anidada o
    correspondencia ambigua, eje oblicuo o simetria no verificada ⇒ X-17a; clase o variante fuera de la politica cerrada o
    ciclo ⇒ X-17b): la vista no es equivalente y el rack falla cerrado (E6). Con fuentes visuales simbolicas tipadas,
    `SourceKind`, `PolylineGeneration`, `LinetypePatternContext` o `VisibilityPath` distintas, mecanismo de visibilidad no
    modelado, orden visual no demostrado dentro de la pieza u orden de plan no conservado entre piezas ⇒ X-22. Con
    transformacion efectiva alterada ⇒ X-23. Con estado dependiente de un vinculo sin evidencia universal ⇒ X-17c (S-26,
    S-27a..S-27c).
...
- Cada anclaje asimetrico produce una fila `UNKNOWN` con `FAIL_CLOSED` o una regla explicita aprobada por el Owner.
  Nunca un parche silencioso: una contradiccion material con V8 abre **Proposal V9**.
...
| **CT-36** | **Viabilidad de la STATE-EVALUATED VISUAL-GEOMETRIC EVIDENCE en `acad.exe` / AutoCAD 2025**, **fuera de `src/`** (`tools/`, harness externo, evidencia del Owner o artefactos en `docs/automation/evidence/`): evaluacion de estados nuevos de bloques dinamicos en una `Database` auxiliar privada **con el contexto de la autoridad origen** (`PSTYLEMODE`, `INSUNITS` y escala de anotacion; un dibujo STB y otro CTB), clonando las definiciones de biblioteca solo con `BlockLibraryImporter.EnsureBlocks(auxDb, nombres)` y verificando todos los bloques requeridos, sin mutar la cache ni el DWG, midiendo antes y despues `DBMOD`, `HANDSEED`, UNDO, `WorkingDatabase` (restaurada incluso ante excepcion), objetos persistidos y el contenido de la cache (HANDSEED, inventario de BTR y huella estable, observados solo desde el harness); paridad de aplicacion de parametros con `LateralHeaderDrawer.ApplyDynamicParameters` (busqueda sin distinguir mayusculas, ultima clave duplicada, `ReadOnly`, ausentes, conversion, orden y `applied`) y representacion evaluada igual a la del materializador en un documento real; **punto de observacion** soportado; valores efectivos; BTR evaluado, `Origin` y **transformacion efectiva** (anidados incluidos); **politica cerrada de clases y variantes** por tipo exacto (grosor, normales, elevacion, `MInsertBlock`, definiciones anidadas anotativas, XCLIP y filtros espaciales, tipos de linea complejos, `HatchLoopTypes` y completitud de atributos por clase); `SourceKind`, `Plinegen` y `Closed`; **fuentes visuales simbolicas tipadas** por propiedad (capa `0`, `ByLayer`, `ByBlock`, anidados, estados de la API, `Color`, `LineWeight` y `Transparency` tipados y `PlotStyle` segun el modo de la autoridad origen); **`VisibilityPath`** variando apagado, inutilizado, inutilizado en ventana, `Visible` de ancestros y anotativos; `LinetypeScale`, escalas y `LinetypeScale` de ancestros, fase y segmentacion de tipos de linea, escalas globales (`LTSCALE`, `PSLTSCALE`, `MSLTSCALE`, `CELTSCALE`) y multiplicidad; **orden visual** de superposiciones materiales dentro de la pieza y entre piezas (con CT-48); ruta anidada estable; poligono SOLID/TRACE; bucles y estilo de islas de HATCH; degradados y patrones; canonicalizacion conservadora con tasa de falsos positivos y negativos; definicion local frente a biblioteca (`DuplicateRecordCloning.Ignore`); obsolescencia (`TraceFingerprint`); casos C, D, E y F (doce variantes); rendimiento preliminar por rack; **SHA-256** de la biblioteca registrado. `accoreconsole` solo como comparacion. Si la ruta no es viable ⇒ Proposal V9 o reduccion de alcance | evidencia registrada; Core consume los resultados |
...
| **CT-46** | **Guardas cruzadas vigentes**: relee en el SHA de la reconciliacion las guardas de texto de `ProjectVariablesConformanceTests.cs` (main y, si integra, I-49), C-08/C-10, las del nucleo de expresiones y, si I-54 integra, T-GRD-01..T-GRD-07 (incluidas las del borde fisico de G6, T-GRD-04..T-GRD-07); fija la lista exacta que G-M18 aplica a los archivos de I-52 y la autoridad vigente de I-49 (ADR-0040, aceptado y que reemplaza a ADR-0038, `[V8-D18]`) | Core |
| **CT-47** | **Corpus indicativo de la biblioteca actual**: recalcula el **SHA-256** del archivo antes de usar la evidencia (identidad; ruta, tamaño y fecha son diagnostico) y, si cambia, no reutiliza la evidencia previa; recalcula en `acad.exe`, con evaluacion de estados, la tabla de §1.4 por pieza y vista con las reglas de V8 (politica cerrada de variantes, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, `VisibilityPath`, `LinetypeScale` y contexto de patron, orientacion, multiplicidad, orden visual dentro de la pieza y entre piezas, degradados, falsos positivos y negativos de la canonicalizacion) y la compara con la evidencia indicativa; una reduccion material de la envolvente ⇒ Proposal V9 | evidencia registrada |
| **CT-48** | **Orden visual de plan por kind y vista** (`[V8-D12]`): orden de emision de cada builder de vista admitida y orden de materializacion de `LateralHeaderDrawer` (colocaciones de grupos y despues piezas sueltas, en `CreateSystemBlock` y `RedefineSystemBlock`) y de `CantileverViewMaterializer`; pares de instancias con superposicion material en fixtures por kind (con la geometria evaluada de CT-36 o con dobles) y su orden relativo en `F ∘ Π(D)` frente a `Π(μ_k D)`; correspondencia de homologas ambigua | Core (con resultados geometricos de CT-36) |
| **CT-49** | **Presentacion de la referencia fuente y orden en Model Space en `acad.exe`** (`[V8-D13]`..`[V8-D15]`), **fuera de `src/`**: inventario de las propiedades de `Entity` y `BlockReference` de AutoCAD 2025 clasificadas como transportadas (ST-13: `LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `PlotStyleName`, `Visible`), colocacion, restriccion estricta, no visuales o no transportables (ST-18); lectura y copia fiel de las ocho propiedades, tambien en un dibujo STB; deteccion de XCLIP y otras entradas de `ACAD_FILTER`, referencias de atributo, entradas del diccionario de extension y XData de efecto visual; orden efectivo de Model Space con `SortentsTable`; clasificacion de las superposiciones de una copia con objetos no seleccionados posteriores a su fuente (rellenos, degradados, wipeouts, imagenes, OLE, texto con mascara, transparencia, referencias que los contengan) y orden resultante de las copias creadas en el orden de sus fuentes | evidencia registrada; Core consume la clasificacion |
...
| **T-M54** | Fuentes visuales simbolicas tipadas y firma: resolucion de capa `0`, `ByLayer` y `ByBlock` por la cadena de anidados en `Explicit(metodo, valor)`, `ByLayer(Named)`, `InheritLayer` e `InheritBlock` con rutas canonicas estables y **nunca** por valor resuelto; `ACI(7) ≠ RGB(255,255,255)`, `ColorBook` igual solo con el mismo libro y nombre, `LineWeight` por estado y valor enumerado, `Transparency` por metodo y alfa, `LinetypeScale` exacta; son **no** simetricas las doce variantes del caso F: (1) mismo color actual con `ByLayer` frente a `Explicit`; (2) `InheritLayer` frente a `InheritBlock`; (3) `VisibilityPath` distinta; (4) `LineWeight` distinto; (5) `Transparency` distinta; (6) `LinetypeScale` distinta; (7) `PlotStyle` distinto si el entorno lo permite caracterizar; (8) `LinetypeSource` `ByLayer` frente a `Explicit` con el mismo tipo actual; (9) `FillSignature` distinta; (10) `SourceKind` distinta; (11) `Plinegen` o `Closed` distintos; (12) contexto de escala de ancestros o de patron distinto; matriz clase × caso; metodo o estado de la API no modelado ⇒ `UNKNOWN`; dobles en Core (CT-37), sin AutoCAD | G6 |
...
| **T-M63** | Acceso a la cache de biblioteca: la sonda obtiene las definiciones solo con `BlockLibraryImporter.EnsureBlocks(auxDb, nombres)` (o la API publica equivalente vigente) y verifica que existan **todos** los bloques requeridos en la base auxiliar; retorno 0 o bloque ausente ⇒ `UNKNOWN`; ni reflexion ni acceso al campo cacheado; HANDSEED, inventario de BTR y huella del contenido de la cache iguales antes y despues, observados solo desde un harness fuera de `src/`; la inicializacion perezosa no cuenta como mutacion (prueba del Plugin donde sea ejecutable; efecto real en CT-36) | G6/G7 |
| **T-M64** | Politica cerrada de clases y variantes (`[V8-D01]`..`[V8-D05]`): tipo exacto (una subclase, incluido `MInsertBlock`, no hereda el soporte); `Line`, `Arc` o `Circle` con `Thickness ≠ 0` o normal no +Z ⇒ `UNKNOWN`; `Ellipse` o `Spline` no planas en XY ⇒ `UNKNOWN`; definicion anidada anotativa, XCLIP anidado y tipo de linea complejo ⇒ `UNKNOWN`; atributo de representacion no clasificado ⇒ `UNKNOWN` (X-17b) | G6 |
...
| **T-M67** | Orden visual de plan (`[V8-D12]`): pares de instancias sin superposicion material no generan relacion; con superposicion material, orden relativo conservado en las homologas ⇒ equivalente; invertido o no demostrable ⇒ X-22; un plan con el mismo multiconjunto y el orden invertido en un par material hace que la vista no conmute (V-VIEW-ALL) | G6 |
| **T-M68** | Presentacion de la referencia fuente (`[V8-D13]`, `[V8-D14]`): la referencia nueva lleva `LayerId`, `Color`, `Linetype`, `LinetypeScale`, `LineWeight`, `Transparency`, `PlotStyleName` y `Visible` tipados de su fuente, asignados explicitamente; fuente con XCLIP u otra entrada de `ACAD_FILTER`, referencias de atributo, entrada de diccionario de extension o XData de efecto visual no clasificada, o propiedad fuera del inventario de CT-49 ⇒ E4 con mensaje propio antes de MUTATE; nunca un aviso ni una copia sin presentacion | G4 (clasificacion pura) y G7 (captura y creacion) |
| **T-M69** | Orden visual en Model Space (`[V8-D15]`, `[V8-D25]`): copias creadas en el orden efectivo relativo de sus fuentes; objeto no seleccionado anterior a la fuente ⇒ sin clasificar; objeto posterior a la fuente con superposicion material o no clasificable con la copia en su destino ⇒ E10 en PREPARE, antes de `EnsureForPlan`; cambio del contexto de orden entre PREPARE y MUTATE ⇒ excepcion y rollback | G7 |
...
| **G-M20** | La sonda no escribe ni lee por reflexion la cache de `BlockLibraryImporter` (`[V8-D10]`): los archivos productivos del espejo solo usan la API publica autorizada (`BlockLibraryImporter.EnsureBlocks(auxDb, nombres)` o la equivalente vigente) hacia una base auxiliar privada; ninguna reflexion sobre `BlockLibraryImporter` ni acceso a su campo cacheado (`GetField`, `GetMethod`, `BindingFlags.NonPublic`, `AcquireLibrary`, `cachedLibrary`); ninguna referencia temporal, parametro dinamico ni `ForWrite` sobre la `Database` cacheada; tras clonar se verifica la presencia de todos los bloques requeridos (§11.3 punto 14) |
...
| **M-33** | Rack fuente cuya referencia tiene presentacion no por defecto (color explicito, tipo de linea y escala de tipo de linea, grosor, transparencia, estilo de trazado en un dibujo STB, `Visible`): la copia conserva cada propiedad (ST-13) |
...
| **M-35** | Orden en Model Space: dos racks seleccionados que se superponen, con DRAWORDER explicito, conservan su orden relativo en las copias; un sombreado o wipeout creado despues de la fuente que se superpone con el destino de la copia hace fallar el comando en PREPARE, sin mutacion semantica (ST-19) |
...
### 14.1 Secuencia normativa (sustituye a la de V7)

```text
Coordinador + Arquitecto de acuerdo sobre V8 (misma version, exact-SHA)
...
de una caracterizacion o de una prueba con el contrato congelado ⇒ STOP → Proposal V9**; nunca un parche silencioso.

| Gate | Entrada | Archivos permitidos | RED esperado | PASS requerido | Parada | Evidencia |
|---|---|---|---|---|---|---|
| **G2** (este) | revision de V7 | `docs/**` | — | consenso de Coordinador y Arquitecto sobre la misma version (V8); despues, rebase/reconciliacion sobre el main vigente, freeze por SHA, re-check normativo y **O-1** del Owner sobre la envolvente de §1.3; contrato reescrito; hallazgos laterales en `ideas-futuras.md` | desacuerdo material ⇒ Proposal V9 | SHA acordado, SHA de freeze + CI |
| **G3** Characterization | freeze de G2 + O-1 aceptada (**sin** ADR aceptado) | `tests/RackCad.Tests/**` (caracterizaciones y fixtures nuevos); `tests/RackCad.UI.Tests/**` si una caracterizacion lo exige; `docs/automation/evidence/**`; la viabilidad de la sonda se demuestra **fuera de `src/`** (`tools/`, harness externo o evidencia del Owner) **en `acad.exe` / AutoCAD 2025**, en la ubicacion que fije el Coordinador al abrir G3; **ninguna sonda productiva en `src/`** | ninguno: CT-01..CT-49 pasan sobre produccion intacta | Core completo (UI si se toco); **viabilidad visual-geometrica de la sonda en `acad.exe` (CT-36: politica cerrada de clases y variantes, `SourceKind`, `Plinegen` y `Closed`, fuentes visuales simbolicas tipadas, contexto de la base auxiliar STB y CTB, `VisibilityPath` con estados de capas, ancestros y anotativos, `LinetypeScale` y contexto de patron, fase, segmentacion y multiplicidad, orden visual dentro de la pieza, SOLID/TRACE/HATCH con `HatchLoopTypes`, `applied` y punto de observacion, valores efectivos, `Origin` y transformacion efectiva, ruta anidada estable, degradados, clonado por `EnsureBlocks` con la cache intacta, identidad SHA-256, `DBMOD`/`HANDSEED`/UNDO/`WorkingDatabase`) antes que `PlanPlacementEvidence` (CT-06)**; corpus indicativo con la huella de la biblioteca (CT-47); orden visual de plan por kind y vista (CT-48); presentacion de la referencia fuente y orden en Model Space (CT-49); cierre transitivo sobre el contrato real, enums, retirados con RM-7 y guardas cruzadas (CT-31, CT-32, CT-42, CT-46); campos start/end de cabecera (CT-45); tabla de `c`; auditoria de anclajes por pieza (CT-17); semantica observable de I-51; decodificacion de secciones e historia de `View`; lineas base de store; topes, con el tope posterior de Push Back activo por defecto y `OffCells` latentes (CT-15, CT-43); estabilidad 9b incondicional con casos A/B, cotas, cambio de gobernante, BOM, firmas de estado y DEP-10a..c (CT-27, CT-28, CT-37, CT-38, CT-44); S-14 asociado; enumeracion `A_k`; tarimas del Selectivo y de Push Back (CT-20); precedencia de fuente (CT-39); read-set (CT-40); costuras de Actualizar e Insertar (CT-26); baseline de I-53 y clasificacion de I-53S (CT-41); fuentes dinamicas/anonimas; V-RT-STORE separado de C-2; **regla de cierre: ninguna fila `G3_PENDING` ni `EvidenceStatus` implicito** (§7.0) | contradiccion material con V8; sonda no viable, sin paridad, sin punto de observacion o sin contexto reproducible; clases o variantes reales no soportadas; biblioteca real que reduce materialmente la envolvente mas alla de O-1; canonicalizacion materialmente ambigua ⇒ **STOP → Proposal V9** | SHA + conteos + CI + registros de evidencia con la huella de la biblioteca. Tras PASS: se pide **O-4** |
...
### 15.1 main y paralelas observadas en el preflight de V8

`git fetch --all --prune` el 2026-09-13, antes de redactar V8 y de nuevo antes de publicarla (tips del re-fetch previo al
commit):

```text
origin/main                                        = 104ef3a1b1df249d6e0a56dc4ad3846912b24f12 (Merge I-53S E2, 2026-09-13T17:12:58;
                                                     antes 1b091be, ancla de las citas de V8)
I-52 feature/rackmirror-espejo-semantico           = b70b5bf5bc689a31f4f2aac9803d675d75665093 (V7; base 46fcac2; sin rebase)
I-49 architecture/motor-expresiones-parametricas   = 8ed15f782f7fc2878693053e54cedca8a041d247 (rebasada sobre 104ef3a:
                                                     G5 977f873, A1 c7fabbe, ADR-0040 propuesto cc75a18 y aceptado
                                                     8f6d583, que reemplaza a ADR-0038; Consensus Freeze 8ed15f7;
                                                     G6 BLOCKED)
I-54 architecture/propiedades-personalizadas       = 5c16ae8a8134e3b956459bd91ba15d097a24fc34 (rebasada sobre 104ef3a:
                                                     G4B 0010144, G5 0572be4, G6 deca1e0 y G6-CLOSE d9f8c57, antes
                                                     7b0f6a9, 3cf0d35, ca71962 y b4e99e4; G7A 5c16ae8 solo docs;
                                                     ADR-0039 aceptado)
...
I-53D feature/cabeceras-multidestino-dinamico      = aa0f817c205c2426969b5e0dd2af467b9fb1ff9d (reclamo c5a1404 y bootstrap;
                                                     solo docs; base 104ef3a)
...
**Cambios de paralelas desde la publicacion de V7.**
...
Ninguno modifica materialmente una autoridad de I-52 (§19): se registran **sin bloquear**. La orden de V8 declaraba como
ultimos observados main `1b091be`, I-49 `c5b9ede`, I-54 `ca71962` e I-53S `e528ef2`; el preflight encontro ademas
`b4e99e4` (G6-CLOSE de I-54, solo docs) y el re-fetch previo al commit, el merge de I-53S E2 en main, I-49 e I-54
rebasadas sobre `104ef3a` (con ADR-0040 aceptado y G7A de I-54) y el reclamo de I-53D (`[V8-D24]`). La historia de
paralelas anterior a V7 esta en Proposal V7 §15.1 y en el registro §42. La numeracion de ADR esta en §16.
...
| **I-53 E1** (**integrada** en main `1b091be`; ADR-0037 **aceptado** en main) e **I-53S E2** (**integrada** en main `104ef3a`) | tipos `Header*` en `Systems.Shared` con C-08/C-10; consumidores Selectivo y Dinamico aditivos; I-53S G5 conecta `ApplyHeaderBatch` a `RackSelectiveWindow` en main | baseline: I-52 no crea tipos en `Systems.Shared` (G-M14, CT-41) ni modifica `SharedFoundationInspection`. **I-53S, clasificacion exacta (`[V8-D17]`)**: materialidad del contrato de I-52 = NON-MATERIAL; materialidad de coordinacion = MATERIAL; baseline de C2-4 = MATERIAL / CHANGED. `C2_4_I52_TheEditorStateAfterADistribution_EqualsTheReopenedDocument_InEveryView` es evidencia de I-53S y **no** cierra C2-4 de I-52. Como I-53S integro **antes** del Candidato: tras el rebase, CT-26, C2-4, T-M30 y las guardas relacionadas se ejecutan sobre el arbol combinado y condicionan el cierre de G6/G7; una integracion **posterior** al Candidato (por ejemplo I-53D, reclamada en `c5a1404`, al conectar el Dinamico) se reporta al Coordinador y se re-mide contra un main que contenga I-52; todo tipo persistido nuevo de cabecera o de modulo queda en RED en el cierre transitivo; serializar si C-1 llegara a tocar comandos de `RACKEDITAR` |
| **I-49** (V6 y Amendment A1 congelados; ADR-0040 **aceptado**, que reemplaza a ADR-0038; rama rebasada sobre `104ef3a`: G5 sintactico `977f873`, A1 `c7fabbe`, aceptacion de ADR-0040 `8f6d583` y Consensus Freeze `8ed15f7`; G6 BLOCKED) | `expression` en `PropertyValues`; `PlanReadSet` (D19); dominio del consumidor declarado por el descriptor (P24.5); su freeze exige que la prueba de supervivencia de `expression` (P24.8) se escriba contra `RackDuplicationPlan` y el restamp vigentes en main; guardas de texto evolucionadas en G5; ADR-0040 como sucesor aceptado de ADR-0038 (solo cambia la guarda del parser de D8) | **I-52 coordina con I-49 antes de tocar `RackDuplicationPlan.cs`** (G4); T-M19. Si I-49 integra antes: su `PlanReadSet` es RS-3 (§9.2.1), su dominio es la cota `b` de V-DEP, su descriptor se clasifica en la guarda (§6.7) y sus guardas de texto rigen (G-M18, CT-46). `8ed15f7` es **NON-MATERIAL** (`[V8-D18]`): con ADR-0040 aceptado, las referencias de I-52 a ADR-0038 se leen en ADR-0040 sin cambio semantico. V8 **no** presupone el motor |
| **I-54** (V5 congelada; ADR-0039 **aceptado**; G3..G6 cerrados y G7 autorizado en su rama, rebasada sobre `104ef3a`: G5 `0572be4` en `RackCad.Application.CustomProperties`, G6 `deca1e0` en el Plugin, G6-CLOSE `d9f8c57` y G7A `5c16ae8`) | `CustomProperties` (`JsonElement?`) heredado por `Compose`, ya implementado en su rama por G4B (`0010144`, antes `7b0f6a9`) con la regla F y las guardas T-GRD-01..T-GRD-03; G5 consume el sobre y `WithCustomProperties` sin modificarlos ni añadir llamadas a `Compose`; G6 escribe la coleccion de proyecto en el NOD `RACKCAD_CUSTOM_PROPERTIES` y los valores por rack con `RackBlockData.Write`, con T-GRD-04..T-GRD-07; **D-21 vigente** (el espejo cumple: A y despues B); T-GRD-02 7 → 8; residuales F-14a/F-14b vigentes | materialidad arquitectonica = NON-MATERIAL; coordinacion y guardas cruzadas = MATERIAL (`[V8-D19]`). **CA-4** portador RACK-LEVEL que se preserva con la copia; **CA-4b** autoridad DRAWING-LEVEL que el espejo no copia, refleja, transforma ni escribe (§6.4); G-M22; al integrar, re-medir sus guardas y añadir T-GRD-04..T-GRD-07 a G-M18; G7B (`RACKPROPIEDADES`, `RPR`) cambiara el censo de comandos y previsiblemente `RackCommandReference.cs`: serializar con I-52 G7; F-14a ⇒ E2 (ST-2c); F-14b y regla F ⇒ E8 en P10 (§9.4); ID-5 intacto; quien integre despues re-apunta T-GRD-02; T-GRD-01 y T-GRD-03 compatibles con ID-5 e ID-6; T-M20 |
| Documental | Filas de ROADMAP, final de `ideas-futuras.md`, `docs/adr/README.md` (0035 y 0037 en main; 0036 de I-52; 0038, reemplazado, y 0040, aceptado, de I-49; 0039 de I-54), censo de comandos 33 → 34 (I-54 G7 añadira `RACKPROPIEDADES` con alias `RPR`) y `src/RackCad.UI/RackCommandReference.cs`, que tocan I-52 G7 e I-54 G7 | Quien integre despues conserva todas las filas en orden numerico, re-mide los censos y serializa `RackCommandReference.cs` |
...
I-49 e I-54 (y de cualquier iniciativa nueva, incluidas I-53S/I-53D); leer sus Proposals y contratos en el SHA exacto;
buscar `docs/adr/0036-*` y citas de «ADR-0036» en **todos** los refs (§16); comprobar los archivos de §19; si `main`
avanzo, rebase segun WORKFLOW antes de escribir codigo. Ademas:

- **I-49:** si integra, re-medir la cota autoritativa de V-DEP (§6.5.5), la autoridad del read-set (§9.2.1) y sus guardas
  de texto (CT-46); ADR-0040 ya esta aceptado y reemplaza a ADR-0038: las referencias de I-52 se leen en ADR-0040
  (`[V8-D18]`).
- **I-54:** si integra, re-medir CA-4, T-GRD-01..T-GRD-07 y G-M22, y añadir T-GRD-04..T-GRD-07 a G-M18 (`[V8-D19]`); si un
  gate posterior a G7A toca el sobre, el compositor, el restamp, el Plugin, la UI o `ProjectVariables`, o cambia la clave o
  el contrato de CA-4b, reportar antes de continuar; G7B cambiara el censo de comandos y previsiblemente
  `RackCommandReference.cs`, que se serializa con I-52 G7.
- **I-53S/I-53D:** I-53S ya integro antes del Candidato de I-52 (`104ef3a`): tras el rebase, ejecutar CT-26, C2-4, T-M30 y
  las guardas relacionadas sobre el arbol combinado, como condicion de cierre de G6/G7 (`[V8-D17]`). Si I-53D u otro gate
  cambia la frontera estado del editor → sistema mas alla de lo registrado en §15.1, reportar antes de continuar; si una
  integracion ocurre despues del Candidato, reportar al Coordinador y volver a medir contra un main que contenga I-52.
...
- Archivo: `docs/adr/0036-rackmirror-espejo-semantico-por-copia.md`, estado **`propuesto`**, con fila en el indice de la
  rama. Actualizado en este gate con las correcciones de V8.
- Congela: espejo semantico por copia; `μ_k` canonica por kind; admisibilidad por vista con seccion canonica y
  ...
  conservadoras con `SourceKind`, ruta anidada estable, parametros efectivos con paridad exacta, transformacion efectiva,
  base auxiliar con el contexto de la autoridad origen, cache de biblioteca aislada y accedida por su API publica,
  degradado fail-closed e identidad SHA-256 de la evidencia; presentacion de la referencia fuente conservada y orden en
  Model Space demostrado; obligacion 9b y
...
2. **Registro de I-52:** ADR-0036 se publico por primera vez en `origin/feature/rackmirror-espejo-semantico` con el
   ...
   `2026-09-13T01:59:44Z`). En los preflights de V3 a V8 ningun otro ref contiene `docs/adr/0036-*`; las citas de
...
no contradice materialmente V8, y **es precondicion de G4**. Mientras tanto el ADR sigue `propuesto` y puede corregirse si
G3 abre Proposal V9. `docs/adr/README.md` no cambia en este gate: la fila de 0036 de la rama sigue correcta, y la
...
| R-12 | La auditoria de anclajes (CT-17) puede degradar muchas filas `R` + `G3_PENDING` a `UK` | Regla de cierre de G3; Proposal V9 si el cambio es material; nunca parche silencioso |
...
| **R-24** | Rendimiento de la sonda por estado en selecciones grandes (muchas piezas y estados, variantes, fuentes simbolicas tipadas, `VisibilityPath`, orden visual dentro de la pieza y entre piezas, canonicalizacion, contexto de la base auxiliar y clonado por `EnsureBlocks`) | cache por comando por `(definicion evaluada, valores efectivos)`; medicion preliminar en CT-36 y medicion en G7 |
...
| **R-29** | La canonicalizacion puede quedar ambigua en piezas con geometria solapada o degenerada | `UNKNOWN` ante ambiguedad; tasa medida en CT-36 y CT-47; ambiguedad material ⇒ Proposal V9 |
| **R-30** | Con la biblioteca inspeccionada, gran parte de Dinamico y Push Back fallaria cerrado por los separadores de planta (§1.4) | evidencia indicativa; G3 la confirma o refuta; si reduce materialmente la envolvente ⇒ Proposal V9 y nueva O-1; la equivariancia de builders queda fuera de I-52 |
...
| **R-33** | La base auxiliar puede no ofrecer un punto de observacion soportado equivalente al materializador | X-17a; si afecta a kinds enteros ⇒ Proposal V9 o reduccion de alcance |
...
| **R-35** | El orden visual de plan (`[V8-D12]`) puede dejar fuera kinds o vistas cuyo builder emite piezas solapadas en un orden que el diseño reflejado invierte | CT-48 por kind y vista; hoy el frontal Selectivo emite postes antes que largueros (§11.3 punto 6.6); reduccion material ⇒ Proposal V9 y nueva O-1 |
...
| **R-37** | ST-19 puede hacer fallar copias cuyo destino se superpone con objetos creados despues de su fuente (sombreados, wipeouts, imagenes), y clasificar esas superposiciones tiene coste | solo se clasifican objetos posteriores a la fuente con extension intersecada; E10 antes de importar; M-35; medicion en G7 |
| **R-38** | AutoCAD 2025 puede no permitir reproducir en una base auxiliar el contexto del dibujo (por ejemplo, `PSTYLEMODE` con estilos de trazado con nombre) | `UNKNOWN` ⇒ X-17a en las piezas con cambio de mano afectadas; CT-36 con dibujos STB y CTB; si afecta a kinds enteros ⇒ Proposal V9 |
...
| CR-V4-1..CR-V4-6 | Evidencia por estado, estados futuros, G3 y sonda, O-1, contrato de serializacion, proceso | vigentes con las precisiones de V6, V7 y V8 | §11.3, §6.4, §6.5.5, §6.7 |
...
| **CR-V7-9** | ADR, G3 y proceso (C:V8-13..C:V8-15) | ADR-0036 «debe demostrar»; CT-36, CT-47, CT-48 y CT-49; desde V8, contradiccion material ⇒ Proposal V9 | ADR-0036; §12; §14.1 |
...
- **AR7-06 y AR7-07: la precision vinculante del Coordinador sustituye al cambio propuesto por la revision.** La revision
  de V7 proponia declarar como limitaciones el orden visual entre piezas (L-33) y la presentacion de la referencia fuente
  (copiar solo la capa, L-34). El Coordinador ordeno controlarlos: V8 exige demostrar el orden relativo de los pares
  materiales entre piezas (`[V8-D12]`) y conservar la presentacion de la fuente o fallar cerrado (`[V8-D13]`..`[V8-D15]`).
  La numeracion L-33..L-35 de V8 describe esas reglas de alcance, no las limitaciones que proponia la revision.
- **ST-19, forma de la demostracion.** Las copias se crean en el orden efectivo relativo de sus fuentes, asi que el orden
  entre copias de fuentes seleccionadas se conserva siempre. Cada copia se añade al final del orden de Model Space: frente
  a un objeto no seleccionado que va **antes** que su fuente, la relacion se conserva sin clasificar nada; solo un objeto
  que va **despues** de la fuente y se superpone con la copia en su destino exige clasificar la superposicion, y si es
  material o no clasificable el orden no puede preservarse ⇒ E10. Una copia que se superpone con su propia fuente queda
  por encima de ella, como la copia del `MIRROR` nativo (no hay relacion previa que conservar).
- **ST-13 frente a I-51.** RACKDUPLICAR sigue copiando solo la capa: I-52 no lo cambia (G-M12). RACKMIRROR conserva la
  presentacion completa por decision del Coordinador; la diferencia entre comandos es deliberada.
- **CA-4b no es portador.** Figura en la tabla de §6.4 para fijar que el espejo no toca la autoridad de nivel dibujo de
  I-54; la allowlist de portadores sigue siendo CA-1..CA-6.
...
- **`[V8-D12]` orden de plan.** El frontal Selectivo emite los postes antes que los largueros (§11.3 punto 6.6), pero cada
  kind y vista tiene su propio orden de emision; CT-48 lo fija y, si un kind entero pierde alcance por el orden, G3 abre
  Proposal V9 y O-1 se reevalua (R-35).
- **`[V7-D11]` continuidad demostrable.** V8 mantiene como no continuos los tipos de linea `ByLayer`, `InheritLayer` e
  `InheritBlock`. Medido en la biblioteca inspeccionada, no quita ninguna pieza estructural medida (§1.4); otra biblioteca
  podria perder piezas con curvas `ByLayer` sin anidado espejado (R-32).
...
- La consecuencia indicativa de §1.4 sigue sugiriendo que, con la biblioteca inspeccionada, gran parte de Dinamico y
  Push Back no se reflejaria (separadores de planta). V8 la registra sin convertirla en limitacion normativa; si G3 la
  confirma y reduce materialmente la envolvente, se abre Proposal V9 y O-1 se reevalua.
- `[V6-D14]` (paridad por recomputacion grafica) y `[V7-D14]` (punto de observacion) siguen siendo las dos condiciones de
  viabilidad de la sonda en G3, ahora con el contexto de la base auxiliar (`[V8-D07]`): sin ellas, los bloques afectados
  fallan cerrado (X-17a).
- **I-53S G5 (`e528ef2`), integrada en main (`104ef3a`).** Clasificacion exacta de V8: contrato de I-52 NON-MATERIAL;
  coordinacion MATERIAL; baseline de C2-4 MATERIAL / CHANGED; su prueba `C2_4_I52_…` es evidencia de I-53S. Como integro
  antes del Candidato, CT-26, C2-4 y T-M30 se ejecutan sobre el arbol combinado tras el rebase (§15.2).
- **ADR-0040 aceptado en I-49.** Reemplaza a ADR-0038 y conserva lo que I-52 consume (D19 `PlanReadSet` y el dominio del
  consumidor); V8 lo usa como autoridad vigente de I-49 (`[V8-D18]`).
- **I-54 G7 autorizado (G7A `5c16ae8`).** `RACKPROPIEDADES` y `RPR` cambiaran el censo de comandos y previsiblemente
  `RackCommandReference.cs`, que I-52 G7 tambien toca: quien integre despues re-mide el censo y serializa ese archivo; un
  gate de I-54 que toque el Plugin o la UI se reporta antes de continuar (§15.3, §19).
...
- G3 demuestra que la sonda por estado no es viable en `acad.exe` / AutoCAD 2025, que no tiene paridad con el
  ...
  envolvente mas alla de O-1, que el orden de plan o el de Model Space dejan fuera kinds enteros o que la canonicalizacion
  es materialmente ambigua: **STOP → Proposal V9** o reduccion de alcance.
...
- Una paralela o una integracion en main modifica materialmente `RackDuplicationPlan.cs`, `RackEmbedComposer.cs`,
  ...
  `JsonSerializerOptions`, los descriptores vinculables, o la autoridad del NOD de I-54 (`CustomPropertiesData.cs`,
  `CustomPropertiesExecutor.cs`) en un sentido que afecte a CA-4 o CA-4b: **detenerse** antes de editar y reportar SHA y
  diff. Un avance
...
- Un gate posterior de I-53S o I-53D cambia la frontera estado del editor → sistema mas alla de lo registrado en §15.1
  (I-53S G5 `e528ef2`, integrada en `104ef3a`), o I-54 abre un gate posterior a G7A (`5c16ae8`; G6 `deca1e0`,
  registrados en §15.1) que toca el sobre, el compositor, el restamp, el Plugin, la UI, `ProjectVariables` o el contrato de
  CA-4 o CA-4b: **reportar antes de continuar** (§15.3). Como I-53S integro antes del Candidato de I-52, G6/G7 no cierran
  sin CT-26, C2-4 y T-M30 verdes sobre el arbol combinado.
...
- Se intenta admitir un cambio de mano sin `GeometrySymmetryResult` visual-geometrico verificado, admitir una clase por
  ...
  fuentes simbolicas tipadas, emparejar primitivas con `VisualSignature` (incluida `SourceKind`), `VisibilityPath` u orden
  visual distintos, emparejar una polilinea con `Line` o `Arc` sueltos, usar nombres anonimos en una ruta anidada,
  aceptar un par material entre piezas con orden no conservado, fusionar familias distintas o primitivas sin continuidad
...
- La sonda escribe en el dibujo del usuario o en la cache de biblioteca, accede a la cache por reflexion o sin la API
  publica autorizada, evalua sin el contexto de la autoridad origen, deja `WorkingDatabase` cambiada, aplica parametros
...
- Se intenta crear una copia sin la presentacion de su referencia fuente, sustituir ST-18 por un aviso o mutar sin
  demostrar el orden en Model Space (ST-19): **prohibido**.
...
- Un tipo de I-52 aparece en `RackCad.Application.Systems.Shared` o un archivo de I-52 viola una guarda de texto vigente:
  **RED** (G-M14, G-M18).
- Aparece un parametro dinamico con semantica de lado sin regla: fail-closed y registro para Proposal V9.
...
| Plugin | `RackMirrorCommands.cs` (nuevo) y SNAPSHOT propio; materializador en `Systems/Shared/`; `Systems/Shared/SystemBlockWriter.cs` (`CreateInTransaction`); **sonda visual-geometrica por estado** (solo lectura o `Database` auxiliar privada con el contexto de la autoridad origen; definiciones de biblioteca solo por `BlockLibraryImporter.EnsureBlocks` y verificacion de bloques), politica cerrada de variantes, canonicalizacion conservadora, fuentes visuales simbolicas tipadas, `VisibilityPath`, contexto de patron, ruta anidada estable, orden visual, transformacion efectiva y `TraceFingerprint`, en G6 o G7 (§11.3); presentacion de la referencia fuente (ST-13, ST-18) y orden en Model Space (ST-19) en el SNAPSHOT, PREPARE y `CreateReference`; costura unica de aplicacion de parametros compartida con el materializador si es viable (G-M19) |
...
Caracterizaciones CT-01..CT-49; pruebas T-M01..T-M70 (con T-M17b); guardas G-M1..G-M22; evidencia de viabilidad de la
...
autoridad del NOD de I-54 (`CustomPropertiesData.cs`, `CustomPropertiesExecutor.cs`) y sus guardas T-GRD-01..T-GRD-07.
...
G2 = V8 EN REVISION  (este documento + ADR-0036 propuesto actualizado + docs/automation/decisions/I-52.md)
     V1 (0fc7032), V2 (0445718), V3 (545c222), V4 (8e2ae4f), V5 (e998a2b), V6 (91bdd38) y V7 (b70b5bf) = historial;
     revision de V7: Architect: CHANGES REQUIRED — PROPOSAL V8
O-1 = PENDING        (envolvente de §1.3, L-01..L-35; se pide despues del freeze tecnico y antes de G3; no en este gate)
G3 = NOT OPEN        (requiere consenso sobre V8, rebase/reconciliacion sobre main, freeze, re-check y O-1)
