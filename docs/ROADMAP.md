...
| I-39B | `architecture/interaccion-editores-ricos` | **Caracterización y fundación de interacción de los editores ricos.** Fija por pruebas el comportamiento observable **actual** de las seis ventanas del arquetipo A —Enter, Escape, caminos de cierre, dirty, acciones y sus motivos, preview y foco—, donde hoy **no existe ninguna cobertura**; corrige en la autoridad común la **dependencia latente de recursos** de `RackEditorVisualShell` (seis tokens `DynamicResource` que solo resuelven si el consumidor mergeó `AppStyles`; `ShellZoneSpacing` caería a `Thickness(0)`), medida como **no observable** porque los cuatro consumidores ya mergean, cada `x:Key` se define una sola vez y ninguno redefine claves; y retira los tres `using` hacia namespaces de sistema en `Editor/` (D12). **Cero cambio observable**, por eso **no lleva ✋**. **NO** implementa la política de cierre, **no** toca Push Back ni Cabecera y **no** adopta `EditorAction`: la mitad observable de los editores ricos es una iniciativa hermana que se registra cuando se diseñe | M | I-39A (integrada) | I-39C, I-39D y la hermana de adopción observable | pendiente |
