...
> Actualizado: 2026-07-28 (**registro autorizado de I-36D — Perfiles AISC S/IPS y geometría visual
> derivada**. **No es una integración**: la fila nace `en curso` y la iniciativa está reclamada, no
> entregada. Se registra desde la rama `feature/perfiles-aisc-s` porque el Owner lo **autorizó
> expresamente**, igual que al abrir I-36A; sin esa autorización WORKFLOW §8 lo prohibiría. I-36D es la
> **iniciativa separada** que I-36B dejó escrita como **requisito futuro obligatorio**: incorpora las
> **28 filas** `Type = S` que I-36A excluyó —contadas y declaradas— como **familia propia** con token
> `S`. Lo que la obliga está **medido contra el libro**: la AISC Shapes Database v16.0 **no publica la
> pendiente del patín ni ningún radio explícito**, y una S sin pendiente se lee como una **W** —pierde
> la familia, no el detalle—. Por eso separa la **autoridad tabulada** (AISC: dimensiones, `A`, peso,
> propiedades) de la **autoridad visual derivada** (RackCad: pendiente `1:6`, `tf` como espesor medio
> del vuelo libre, radio visual y punta aguda), en un eje **ortogonal** a `SectionFidelity`, con
> advertencia obligatoria de geometría aproximada no apta para CNC. **ADR-0023 nace `propuesto`** y
> solo el Owner lo acepta tras ver el dibujo. **I-37 pasa a depender de I-36D.** Siguiente habilitada:
> **I-36D**, en curso.)
>
