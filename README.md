...
| **Sistema dinamico (pallet flow)** | `RackDynamicSystemWindow` | Vista lateral actual: cabeceras a lo largo del tramo (celosia espejeada) como bloques anidados compartidos, separadores por nivel, postes derivados con refuerzo opcional, altura automatica, presets por modulo y BOM. Internamente usa `DynamicRackDesign -> DynamicRackSystemResolver`; frontal/planta, varios frentes/anchos y cama integrada siguen pendientes. |
...
- `seguridad.csv` (elementos de seguridad: bota, protector lateral, tope, desviador, parrilla; con costo/moneda/unidad para cotizacion)
