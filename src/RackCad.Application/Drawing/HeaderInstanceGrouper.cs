...
{
    /// <summary>
    /// Collapses a FLAT list of block instances into a structured <see cref="HeaderRunPlan"/> (the ARRAY
    ...
    /// <see cref="HeaderRunPlan.Flatten"/> reproduces the input as a multiset.
    /// </summary>
    public static class HeaderInstanceGrouper
    {
        public static HeaderRunPlan Group(IReadOnlyList<HeaderBlockInstance> instances, string namePrefix)
        {
            var loose = new List<HeaderBlockInstance>();
            if (instances == null || instances.Count == 0)
            {
                return new HeaderRunPlan(new List<HeaderGroup>(), loose);
            }
        ...
        /// not, and <see cref="HeaderRunPlan.Flatten"/> reproduces the input exactly.</summary>
