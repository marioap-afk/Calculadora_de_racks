...
namespace RackCad.Application.Systems.Selective
{
    ...
    public sealed class SelectivePlantaBuilder
    {
        ...
        /// </summary>
        public HeaderRunPlan BuildPlan(SelectiveRackSystem system, RackCatalog catalog)
        {
            ...
            for (var i = 0; i < frenteYs.Count; i++)
            {
                ...
                for (var k = 0; k < offsets.Count; k++)
                {
                    if (i >= SelectiveDepthLayout.BaysOfFondo(system, k).Count + 1) continue; // fondo k doesn't reach this frente
                    if (offsets[k] < frenteFront) frenteFront = offsets[k];
            ...
            return new HeaderRunPlan(groups.Select(g => g.ToGroup()).ToList(), loose);
        }
