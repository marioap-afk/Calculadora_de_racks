...
namespace RackCad.Application.Systems
{
    ...
    /// it. This session gives an editor a place to put edits that the user has not confirmed yet.
    /// </para>
    /// <para>
    ...
    /// longitudinal sequence, which is a property of a rack and not of a system family.
    /// </para>
    /// </summary>
    public sealed class RackModuleEditSession
    {
        ...
        public bool HasPendingChanges => !AreEquivalent(committed, working);
        ...
        /// and which, in the base, no Push Back surface ever requests.
        /// </summary>
        public bool StandardRestoreRequested => standardRestoreRequested;

        /// <summary>Stage a new longitudinal length (the header's fondo) for one module.</summary>
        ...
        /// is dropped, so whoever resolves next builds the standard one at the current inputs. Per module — the
        /// rack-wide reset is <see cref="RequestStandardRestore"/>.
        /// </summary>
        ...
        /// <summary>Discard every staged change and the restore request; the session returns to its last committed
        /// state. This is the rollback the base does not have anywhere.</summary>
        public void Cancel()
        {
            working = committed.Select(CopyIntent).ToList();
        ...
        /// <summary>
        /// Accept the staged state and return it as independent intents the caller may assemble. The session
        /// re-baselines on the result, so a later <see cref="Cancel"/> reverts to THIS state and not to the original.
        /// </summary>
        public IReadOnlyList<DynamicRackModuleDesign> Commit()
        {
            committed = working.Select(CopyIntent).ToList();
            standardRestoreRequested = false;
            return committed.Select(CopyIntent).ToList();
        }
