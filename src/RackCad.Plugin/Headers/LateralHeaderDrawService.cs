...
namespace RackCad.Plugin.Headers
{
    ...
    public sealed class LateralHeaderDrawService
    {
        ...
        /// <summary>Jig-place an already-created block and report the outcome. Facade over
        /// <see cref="BlockPlacement.PlaceAndReport"/> (I-16 F2); public so every DrawService keeps its API.</summary>
        public HeaderPlacementResult PlaceAndReport(Document document, RackCatalog catalog, LateralHeaderBlockResult block)
            => BlockPlacement.PlaceAndReport(document, catalog, block);
        ...
        /// and this service's own edit/insert paths report missing blocks without changing.</summary>
        internal static string[] DescribeMissing(RackCatalog catalog, LateralHeaderDrawOutcome outcome)
            => BlockPlacement.DescribeMissing(catalog, outcome);
